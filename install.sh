...
# brotli is only linked from pkg-config on Linux/BSD (macOS uses the system
# libcompression), but listing it unconditionally is harmless there: the
# optional install only triggers when a probe below saw a gap.
optional_pkgs() {
    case "$PKG_INSTALL" in
...
# brotli: macOS has shipped the codec in libcompression since 12.0 (nothing to
# probe there); everywhere else it is libbrotli via pkg-config — the Makefile's
# own probe — and its absence silently drops the brotli codecs from BOTH
# dyna:compress and dyna:stream, so the preflight must say which way it went.
brotli_version() {
    command -v pkg-config >/dev/null 2>&1 || return 1
    local pc=""
    if command -v brew >/dev/null 2>&1; then
...
# OpenSSL >= 3 visible to pkg-config (the Makefile's own probe: the brew keg
# first, then the system copy). Its presence decides CONFIG_TLS=y, which is what
# buys HTTPS, JWT and the AEAD ciphers -- without it the build succeeds and the
# features are silently absent, so the preflight must say which way it went.
openssl_version() {
    command -v pkg-config >/dev/null 2>&1 || return 1
    local pc=""
    if command -v brew >/dev/null 2>&1; then
...
ensure_deps() {
    ...
    local want_brotli=0
    [ "$OS" = "macos" ] || { brotli_version >/dev/null 2>&1 || want_brotli=1; }
    ...
    if [ "$WITH_DEPS" -eq 1 ] &&
       { [ -n "$missing" ] || [ -z "$PKG_INSTALL" ] ||
        ...
        if [ -n "$PKG_INSTALL" ]; then
            ...
            # These decide whether dyna:net's SQLite class and the brotli
            # codecs (dyna:compress + dyna:stream) exist at all, so
            ...
            if [ "$want_sqlite" -eq 1 ] || [ "$want_brotli" -eq 1 ]; then
                ...
                if sqlite_version >/dev/null 2>&1; then
                ...
                elif brotli_version >/dev/null 2>&1; then
                    step "brotli $(brotli_version) — dyna:compress + dyna:stream codecs will be built in"
                else
...
preflight() {
    ...
    tls="$(openssl_version 2>/dev/null || true)"
    if [ "$OS" = "macos" ]; then brotli="system (libcompression)"
    else brotli="$(brotli_version 2>/dev/null || true)"; fi
...
# ----------------------------------------------------------------------------- clone + build
# The single source mechanism: a shallow clone of master's latest commit. There
# is no version selection and no local-tree shortcut, so what a user builds is
# always exactly what upstream master builds.
fetch_source() {
    local t0=$SECONDS
...
build() {
    ...
    # CONFIG_TLS is opt-in and the Makefile ERRORS on it without OpenSSL >= 3,
    # so it is only added when the probe already saw the library.
    if openssl_version >/dev/null 2>&1; then
...
verify() {
    ...
    # The streaming core is the newest stdlib member; prove it loads AND
    # computes: lines() over an in-memory source must yield exactly 3 lines.
    local slines
