...
optional_pkgs() {
    case "$PKG_INSTALL" in
        brew*)       echo "pkg-config sqlite" ;;
        *apt*)       echo "pkg-config libsqlite3-dev" ;;
        *dnf*|*yum*) echo "pkgconf-pkg-config sqlite-devel" ;;
        *pacman*)    echo "pkgconf sqlite" ;;
        *pkg*)       echo "pkgconf sqlite3" ;;
        *)           echo "" ;;
    esac
}
...
free_space() {
    # Report against the nearest EXISTING ancestor: df on a path that does not
    # exist yet fails, and the build cache usually does not exist on a first run.
    local d="$1"
    while [ ! -d "$d" ] && [ "$d" != "/" ]; do d="$(dirname "$d")"; done
    df -Pk "$d" 2>/dev/null | awk 'NR==2 { printf "%.1f GB free on %s", $4/1048576, $6 }'
}

preflight() {
    local missing sqlite
    missing="$(missing_tools)"
    ...
    # Optional, and silent when absent: no pkg-config or no sqlite3 and the
    # Makefile leaves dyna:net's SQLite class out without saying so.
    if [ -n "$sqlite" ]; then
...
install_binary() {
    ...
    if [ "$use_sudo" -eq 1 ]; then
        run sudo install -m 0755 "$SRC_DIR/$BINARY_NAME" "$dest"
    else
        run install -m 0755 "$SRC_DIR/$BINARY_NAME" "$dest"
    fi
