...
grammar they follow, the discipline around option bags, and the cases where
going bare is allowed on purpose. It exists because a cross-module audit
...
## 2a. The option-bag refusal: bare by design

The shared options validator (`unknown option "%s" (valid: %s)`) is exempt
from the prefix rule. It emits the bare shape

    unknown option "leve" (valid: level)

on every refused key, and that is deliberate and shipped: the same wording
runs in 20+ modules (`config`, `compress`, `csv`, `crypto`, `file`, `graph`,
`http`, `libc`, `log`, `ml`, `net-*`, `scrape`, `structures`, `term`,
`uuid`, `xml`, `yaml`, ...), it is README-documented ("a typo can never
quietly change behavior"), and the message already carries the context a
prefix would add — the offending key, quoted, plus the full accepted set.
A `csv.read: unknown option "leve" ...` would spend twelve characters
re-stating which call the user is looking at while the useful half is the
valid list. New option bags must reuse this validator's exact format rather
than inventing a prefixed variant.

The same reconciliation covers the **signature-led usage line**: arity and
shape refusals that name the call by its signature —
`sample(arr, n): n is required`, `JaroWinkler(a, b): two strings required`,
`appendUvarint(buf, value, offset?)` — are the accepted spelling of the
entry-point tier for overloads whose plain name is ambiguous or whose
argument shape is the whole message. The signature in parens is the
context; do not double it into a `csv.sample(arr, n):` prefix.
