...
/* The OVERLAPPING rescan in countIn/allIn restarts the kernel one byte past
 * each match. simd.strfind is first+last: a restart scans only the GAP to the
 * next match and verifies ~plen bytes there, so a whole count costs
 * n + k*plen byte-steps -- the honest model this budget charges (gap + plen
 * per find). An earlier charge of (t.len - pos) per match modelled every
 * restart as a scan-to-end-of-text and refused ORDINARY inputs: counting a
 * word ~47k times in a 1 MB document tripped it (measured, ws-matcher
 * 2026-09-16) although the real work is one linear pass. The budget now trips
 * only where the work is real: self-overlapping LONG patterns over a periodic
 * text ("abab.." x1000 over "abab..", k*plen ~ 4e9) still refuse with a clean
 * RangeError instead of a second-scale scan. */
#define DYN_MATCH_MAX_SCAN_STEPS (1000ULL * 1000ULL * 1000ULL)

/* Total pattern BYTES MultiMatcher accepts. Each automaton state is a 1 KiB
 * goto table (dyn-ac.h), so an unbounded pattern set degenerates into
 * reallocation churn before the OOM path fires -- 65536 patterns x 1 KiB each
 * asks for ~65M states (~65 GB) and only then fails "cleanly". 16 MiB of
 * pattern bytes bounds the worst case to ~16.8M states and trips long before
 * the churn; checked per add, at insertion time, so the refusal is immediate. */
...
/* The byte offset of code-unit index `units`, or (size_t)-1 past the end.
   Only meaningful for a non-direct text (byte offset != code-unit offset);
   steps MUST mirror dyn_m_units, including the astral plane costing TWO code
   units, so the landing spot is always a code-unit boundary -- a UTF-8 match
   can never begin inside another character, so no match is skipped or
   double-counted by starting the kernel there. */
...
typedef struct {
    uint8_t *pat;
    size_t plen;        /* UTF-8 BYTE length: what the scan kernel consumes */
    size_t ulen;        /* CODE-UNIT length: what the getter reports, so one
                         * match's span is `length` code units at `firstIn`'s
                         * offsets (for ASCII the two coincide) */
    int algo;
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    /* the getter reports CODE UNITS (the module's offset convention: a hit at
     * firstIn's offset spans `length` code units). For ASCII, byte length IS
     * code-unit length; only a non-ASCII pattern needs the translation, in the
     * same breath as the ASCII scan above. */
...
/* The budget-exceeded error, shared by countIn and allIn. */
 ...
 * firstIn takes an optional fromIndex with String.prototype.indexOf
 * conventions: a negative start is 0, a start past the end is -1, and
 * (like the rest of the module) the offset is in UTF-16 code units. */
                                ...
                                JSValueConst *argv, int magic)
{
    ...
    /* Coerce fromIndex BEFORE resolving the handle (dyna-nat.h's rule): a
       valueOf that runs user JS must not strand the pattern pointer. */
    if (magic == 0 && argc > 1 && !JS_IsUndefined(argv[1])) {
        if (JS_ToInt64(ctx, &from, argv[1]))
            return JS_EXCEPTION;
        if (from < 0)
            from = 0;                   /* String.prototype.indexOf clamps */
    }
    ...
    if (m->plen == 0) {
        ...
        if (magic == 0 && from > 0) {
            /* firstIn still honours fromIndex: indexOf("", pos) answers
             * min(pos, length), so clamp to the text's code-unit length */
    ...
    if (pos != SIZE_MAX) {
        ...
        if (magic == 2) {
            ...
            while (pos != SIZE_MAX) {
                count++;
                /* charged work: the bytes the kernel actually advanced (gap
                 * from the last search start) plus the ~plen-byte verify */
                           ...
                           JSValueConst *argv)
{
    ...
    }
    {
        ...
        for (i = 0; i < n; i++) {
            ...
            /* DYN_MATCH_MAX_PATTERN_BYTES, charged per add: past the cap the
             * automaton is refused HERE rather than after the churn (see the
             * constant's comment). */
            total += (int64_t)sl;
...
/* ---- replaceAllIn: non-overlapping replacement over the automaton -- */

/* A growable output byte buffer. The exact-size allocation Matcher's
 * replaceAllIn does (count matches, allocate once) needs a counting pass that
 * would visit every overlap twice here; a doubling buffer keeps the scan
 * one-pass, and the worst case (every byte a replacement) is output-shaped,
 * so the doubling amortizes the same way a JS string builder would. */
typedef struct {
    char *buf;
    size_t len, cap;
} dyn_m_buf_t;
...
typedef struct {
    JSContext *ctx;
    const dyn_ac_t *a;
    size_t last_end;        /* byte offset: the next hit must start here or later */
    dyn_m_buf_t out;
    JSValue repl_fn;        /* callback form, or JS_UNDEFINED */
    const char *repl_str;   /* string form, or NULL */
    size_t repl_len;
    const char *text;       /* C-owned snapshot of the subject */
    size_t tlen;
    int failed;             /* pending JS exception; scan stopped */
    int oom;
} dyn_ac_repl_t;

/* Runs in dyn_ac_run's emission order -- by end position, longest pattern
   first at one end -- which is exactly the greedy leftmost pass a replacement
   needs: a hit is replaced when it starts at or after the previous
   replacement's end, so overlapping hits keep only their first (longest)
   claim. Returns 1 to stop the scan (an exception is pending or the buffer
   died); 0 to keep scanning. */
static int dyn_ac_emit_repl(void *ud, int pat, size_t end_byte)
{
    ...
    if (end_byte > s->tlen || plen > end_byte)
        return 0;                              /* cannot happen; paranoia */
    start = end_byte - plen;
    if (start < s->last_end)
        return 0;                              /* overlaps the last replacement */
    /* the untouched text BETWEEN the last replacement and this one */
    if (start > s->last_end &&
        dyn_m_buf_put(&s->out, s->text + s->last_end, start - s->last_end) < 0) {
        s->oom = 1;
        return 1;
    }
    ...
    if (JS_IsUndefined(s->repl_fn)) {
        ...
        /* The callback receives the matched text and the pattern's index --
         * the same two fields allIn reports, so callers can switch on the
         * pattern without re-matching. Its result is ToString'd, matching
         * String.prototype.replace conventions. */
        JSValue args[2], r;
...
/* replaceAllIn(text, repl): parity with Matcher.replaceAllIn's rule --
 * NON-overlapping, left to right, the only sane rule once bytes are being
 * removed -- but over the whole pattern set in one pass. `repl` is a
 * replacement string or repl(match, patternIndex). The subject is snapshotted
 * to C bytes BEFORE the handle is resolved and the scan begins: the callback
 * runs JS mid-scan, and dyn_m_text's borrowed narrow-string pointer is only
 * valid until JS runs, so replacers never get it. */
                                     ...
                                     int argc, JSValueConst *argv)
{
    ...
    /* Coerce BOTH arguments before resolving the handle (dyna-nat.h). */
    ...
    if (!a) {
        JS_FreeCString(ctx, s.text);
        if (s.repl_str)
            JS_FreeCString(ctx, (void *)s.repl_str);
    ...
    /* the tail after the last replacement */
    if (!s.failed && !s.oom &&
        dyn_m_buf_put(&s.out, s.text + s.last_end, s.tlen - s.last_end) < 0)
        s.oom = 1;
...
static const JSCFunctionListEntry dyn_matcher_funcs[] = {
    ...
    JS_CFUNC_DEF("JaroWinkler", 2, dyn_jaro_winkler),          /*new API, plan-spelled */
