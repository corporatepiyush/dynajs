 ...
 * each match, so k matches over an n-byte haystack cost up to k*n scanned
 * bytes: needle "aa" over "a".repeat(2e6) is ~4e12 byte-steps on the loop
 * thread. Matcher's sibling kernels got budgets (Levenshtein's cell cap,
 * diff's DYN_DIFF_MAX_DIAGONAL_STEPS); this scan got nothing. Same shape as
 * those caps: a count/enumeration that would exceed this many total advanced
 * byte-steps refuses with a clean RangeError instead of hanging. Normal
 * inputs are untouched -- a scan must already match roughly every other
 * byte of a multi-hundred-kilobyte haystack to trip it. */
