                          ...
                          JSValueConst *argv, int want_rows)
{
    ...
    sql_cache_ent_t *ent;
    int cached = 0;
    char *key = NULL;       /* snapshot of the caller's exact query text */
    ...
    if (!cached) {
        /* The cache KEY must be the text the caller passed, byte for byte:
           sqlite3_sql(st) can be shorter than the query text (a prepared
           statement's text), and copying from it under `sqllen` over-read
           the heap and stored a key no later lookup could match. */
    ...
    if (cached) {
        ...
        if (tail && sql_tail_empty(tail))
            sql_cache_put(s, key, sqllen, hash, st);  /* key = the caller's exact text */
        else
            sqlite3_finalize(st);
    }
    free(key);
