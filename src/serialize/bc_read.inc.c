                                       ...
                                       int flags, int kind)
{
    ...
    if (JS_IsException(obj))
        return JS_EXCEPTION;
    p = JS_VALUE_GET_OBJ(obj);
    /* kind == KEY runs no user code anywhere in the loop, so the GPN
       snapshot (flags kept, incl. JS_GPN_ENUM_ONLY: it filters on
       enumerability and fills JSPropertyEnum.is_enumerable) is
       authoritative and the per-key JS_GetOwnPropertyInternal re-check is
       redundant.  kind == VALUE / KEY_AND_VALUE call getters -- user code
       -- and a getter may delete a future key or flip its enumerability
       between OwnPropertyKeys and its visit (test262
       Object/values/getter-removing-future-key.js): those kinds keep the
       per-key re-check and therefore need the FULL own-key list. */
    trust_snapshot = kind == JS_ITERATOR_KIND_KEY
        || !(flags & JS_GPN_ENUM_ONLY);
    if (JS_GetOwnPropertyNamesInternal(ctx, &atoms, &len, p,
                                       trust_snapshot ? flags
                                                      : (flags & ~JS_GPN_ENUM_ONLY)))
        goto exception;
    r = JS_NewArray(ctx);
    if (JS_IsException(r))
        goto exception;
    rp = JS_VALUE_GET_OBJ(r);
    /* Bulk build: presize the result once and store the elements straight
       into its fast-array slots (house pattern: js_append_enumerate).
       The per-element JS_CreateDataPropertyUint32 ran the full define
       machinery (index boxing, atom conversion, shape update) per key.
       u.array.count is bumped right after each store so that the free
       path on exception (kind == VALUE / KEY_AND_VALUE run getters, i.e.
       user code) never walks an unwritten slot. */
    bulk = rp->fast_array && rp->class_id == JS_CLASS_ARRAY
        && (uint64_t)len <= 0x7ffffffeU;
    if (bulk && len > 0 && expand_fast_array(ctx, rp, len))
        goto exception;
    for(j = i = 0; i < len; i++) {
        JSAtom atom = atoms[i].atom;
        if (trust_snapshot) {
            if ((flags & JS_GPN_ENUM_ONLY) && !atoms[i].is_enumerable)
                continue;
        } else {
            JSPropertyDescriptor desc;
            int res;

            /* Check if property is still enumerable: getters for earlier
               keys may have run user code */
        ...
        case JS_ITERATOR_KIND_KEY_AND_VALUE:
            ...
            if (JS_IsException(key))
                goto exception1;
            if (bulk) {
                JSObject *vp = JS_VALUE_GET_OBJ(val);
                if (expand_fast_array(ctx, vp, 2)) {
                    /* key is still live here and not yet stored into vp
                       (u.array.count is 0, so exception1's free of val
                       releases no slots) -- free it explicitly; every
                       later failure path hands key over to val or to
                       JS_CreateDataPropertyUint32, which consumes it. */
                    JS_FreeValue(ctx, key);
                    goto exception1;
                }
                vp->u.array.u.values[0] = key;
                vp->u.array.count = 1;
                value = JS_GetProperty(ctx, obj, atom);
                if (JS_IsException(value))
                    goto exception1;
                vp->u.array.u.values[1] = value;
                vp->u.array.count = 2;
                if (likely(JS_VALUE_GET_TAG(vp->prop[0].u.value) == JS_TAG_INT))
                    vp->prop[0].u.value = JS_NewInt32(ctx, 2);
            } else {
    ...
    if (bulk
    &&  likely(JS_VALUE_GET_TAG(rp->prop[0].u.value) == JS_TAG_INT)
    &&  (uint32_t)JS_VALUE_GET_INT(rp->prop[0].u.value) < rp->u.array.count)
        rp->prop[0].u.value = JS_NewInt32(ctx, rp->u.array.count);
    goto done;
...
/* --- non-ES static Object utilities. Installed non-enumerable on the Object
 * CONSTRUCTOR — NEVER on
...
/* Type tag from the internal class (does NOT consult
 * Symbol.toStringTag — a deliberate, cheaper divergence). Number/Boolean/String
 * wrapper objects report the same tag as their primitives. */
...
/* isNil / isNotNil — magic 0=isNil, 1=isNotNil. */
...
/* defaultTo(d, v) -> v unless v is null/undefined/NaN, then d. */
...
/* size / isEmpty — count of own enumerable string keys. */
...
/* invert / invertObj — swap keys and values (last value wins).
...
/* pick(keys, o) -> new object with the listed keys that exist in o
...
/* omit(keys, o) -> own enumerable string props of o not in keys. */
...
/* pickBy(pred, o) -> props where pred(value, key) is truthy. */
...
/* toPairs(o) -> [[k, v], ...] (own enumerable string entries). */
...
/* fromPairs(pairs) -> object built from an array of [k, v] pairs. */
...
/* assoc(k, v, o) -> shallow copy of o with k set to v. */
...
/* dissoc(k, o) -> shallow copy of o without k. */
...
/* tap(fn, x) -> call fn(x) for its side effect, return x. */
...
/* clone: deep-copies plain objects and arrays (recursively), and Date /
 * RegExp via their constructors. Primitives, functions and other exotics are
 * returned by reference (documented divergence). */
...
/* equals: deep structural equality for primitives (SameValueZero, so
...
static JSValue js_object_path_get(JSContext *ctx, JSValueConst obj, JSValueConst path)
{
    ...
    if (JS_IsString(path)) {
        ...
        p = s;
        while (*p) {
            /* One pass: strchr already walks to the terminator when there is
               no dot, and strlen then walked the same bytes again. */
            const char *e = p;
            const char *dot;
            while (*e && *e != '.') e++;
            dot = (*e == '.') ? e : NULL;
...
/* has(prop, obj) — own property. hasIn(prop, obj) — the `in` operator. */
...
/* Shallow merge: both keep the LEFT object's key order, only the
 ...
 * created as plain objects, never as arrays for integer keys (documented
 * divergence). */
...
/* get(obj, path, default?) — deep read (path = dotted string or array). */
...
/* set(obj, path, value) — deep set, MUTATES obj (creating intermediate
...
/* is(Ctor, val) — type check (instanceof, or primitive constructor). */
...
/* Lens(getter, setter) / new Lens(getter, setter) — a custom lens (same as
 * Lens.lens). Being a real constructor makes `x instanceof Lens` work; the
 * prop/index/path/lens/view/set/over statics ride on the function object. */
static JSValue js_lens_constructor(JSContext *ctx, JSValueConst new_target,
                                   int argc, JSValueConst *argv)
{
    (void)new_target;
    return js_lens_make(ctx, LENS_CUSTOM, JS_UNDEFINED,
...
int JS_AddIntrinsicLens(JSContext *ctx)
{
    ...
    /* Lens is a real constructor function; the statics are properties on it,
     * and JS_SetConstructor wires up Lens.prototype / proto.constructor so
     * `lens instanceof Lens` holds. */
    ctor = JS_NewCFunction2(ctx, js_lens_constructor, "Lens", 2,
                            JS_CFUNC_constructor_or_func, 0);
    if (JS_IsException(ctor)) { JS_FreeValue(ctx, proto); return -1; }
    JS_SetPropertyFunctionList(ctx, ctor, js_lens_static_funcs, countof(js_lens_static_funcs));
    JS_SetConstructor(ctx, ctor, proto);   /* borrows both; dups into props */
    JS_FreeValue(ctx, proto);
    if (JS_DefinePropertyValueStr(ctx, ctx->global_obj, "Lens", ctor,
                                  JS_PROP_WRITABLE | JS_PROP_CONFIGURABLE) < 0)
        return -1;
    return 0;
}
...
/* ===== non-ES Function.prototype combinators. Each method returns a NEW
 * function (a JS_NewCFunctionData closure) that captures the participating
 * functions in its data[]. Registered non-enumerable on Function.prototype.
 * Names are non-colliding with ES (call/apply/bind/toString/length/name).
 * No placeholder-argument support. ===== */
...
/* ===== timed function wrappers =============================================
 *
 * The backlog item was "throttle debounce delay memoize lazy -- need event-loop
 * integration and cancellation". Re-scoped as the note asked:
 *
 *   throttle / debounce / delay  need a scheduler, and get one
 *   memoize                      needs no scheduler at all
 *   lazy                         DECLINED: it is `once`. A lazily-initialised
 *                                value is `(() => expensive()).once()`, which
 *                                already computes at most once and returns the
 *                                same result forever. A second spelling would
 *                                be a second name for one behaviour.
 *
 * The scheduler is looked up as globalThis.setTimeout at USE time, not at wrap
 * time: an embedding with no event loop can still BUILD a debounced function,
 * and only calling it needs somewhere to run. The error when there is none
 * says which of the two is missing rather than "undefined is not a function".
 *
 * State lives in a JS Array cell inside the closure data, the same shape
 * `once` uses:  [timerId, pendingArgs, pendingThis, lastInvokeMs, lastResult].
 * Every wrapper carries `.cancel()`; debounce also carries `.flush()`, because
 * "run the pending call now" is a different request from "forget it".
 */

static int64_t date_now(void);          /* src/builtins/date.inc.c */

enum { JS_FNT_TIMER = 0, JS_FNT_ARGS, JS_FNT_THIS, JS_FNT_LAST, JS_FNT_RESULT };

static JSValue js_fn_timer_global(JSContext *ctx, const char *name)
{
    JSValue g = JS_GetGlobalObject(ctx), f;
    f = JS_GetPropertyStr(ctx, g, name);
    JS_FreeValue(ctx, g);
    if (JS_IsException(f))
        return f;
    if (!JS_IsFunction(ctx, f)) {
        ...
        return JS_ThrowTypeError(ctx,
            "globalThis.%s is not a function: debounce, throttle and delay need "
            "an event loop to schedule against", name);
    }
    return f;
}

/* clearTimeout(cell[TIMER]) and forget it. Silent when nothing is pending. */
static int js_fn_timer_clear(JSContext *ctx, JSValueConst cell)
{
    JSValue id = JS_GetPropertyInt64(ctx, cell, JS_FNT_TIMER), clr, r;
    if (JS_IsException(id))
        return -1;
    if (JS_IsUndefined(id) || JS_IsNull(id)) {
        JS_FreeValue(ctx, id);
        return 0;
    }
    clr = js_fn_timer_global(ctx, "clearTimeout");
    if (JS_IsException(clr)) { JS_FreeValue(ctx, id); return -1; }
    r = JS_Call(ctx, clr, JS_UNDEFINED, 1, (JSValueConst *)&id);
    JS_FreeValue(ctx, clr);
    JS_FreeValue(ctx, id);
    if (JS_IsException(r))
        return -1;
    JS_FreeValue(ctx, r);
    return JS_SetPropertyInt64(ctx, cell, JS_FNT_TIMER, JS_UNDEFINED) < 0 ? -1 : 0;
}

/* setTimeout(cb, ms) -> cell[TIMER] */
static int js_fn_timer_arm(JSContext *ctx, JSValueConst cell, JSValueConst cb,
                           double ms)
{
    JSValue st = js_fn_timer_global(ctx, "setTimeout"), id;
    JSValueConst args[2];
    JSValue msv;
    if (JS_IsException(st))
        return -1;
    msv = JS_NewFloat64(ctx, ms);
    args[0] = cb;
    args[1] = msv;
    id = JS_Call(ctx, st, JS_UNDEFINED, 2, args);
    JS_FreeValue(ctx, st);
    JS_FreeValue(ctx, msv);
    if (JS_IsException(id))
        return -1;
    return JS_SetPropertyInt64(ctx, cell, JS_FNT_TIMER, id) < 0 ? -1 : 0;
}

static int js_fn_timer_save(JSContext *ctx, JSValueConst cell,
                            JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    if (JS_IsException(a))
        return -1;
    for (i = 0; i < argc; i++)
        if (JS_SetPropertyInt64(ctx, a, i, JS_DupValue(ctx, argv[i])) < 0) {
            JS_FreeValue(ctx, a);
            return -1;
        }
    if (JS_SetPropertyInt64(ctx, cell, JS_FNT_ARGS, a) < 0)
        return -1;
    return JS_SetPropertyInt64(ctx, cell, JS_FNT_THIS,
                               JS_DupValue(ctx, this_val)) < 0 ? -1 : 0;
}

/* Invoke f with the saved this/args, clear them, and record the result. */
static JSValue js_fn_timer_invoke(JSContext *ctx, JSValueConst f,
                                  JSValueConst cell)
{
    JSValue a, thisv, r = JS_UNDEFINED;
    int64_t n = 0, i;
    JSValue *argv = NULL;

    a = JS_GetPropertyInt64(ctx, cell, JS_FNT_ARGS);
    if (JS_IsException(a))
        return JS_EXCEPTION;
    if (JS_IsUndefined(a)) {
        JS_FreeValue(ctx, a);
        return JS_UNDEFINED;            /* nothing pending */
    }
    thisv = JS_GetPropertyInt64(ctx, cell, JS_FNT_THIS);
    if (JS_IsException(thisv)) { JS_FreeValue(ctx, a); return JS_EXCEPTION; }
    if (js_get_length64(ctx, &n, a)) goto fail;
    if (n > 0) {
        argv = js_malloc(ctx, sizeof(JSValue) * (size_t)n);
        if (!argv) goto fail;
        for (i = 0; i < n; i++) {
            argv[i] = JS_GetPropertyInt64(ctx, a, i);
    ...
    /* The pending call is cleared BEFORE f runs: f can call the wrapper again,
     * and it must then be scheduling a new call rather than re-arming this one. */
    if (JS_SetPropertyInt64(ctx, cell, JS_FNT_ARGS, JS_UNDEFINED) < 0 ||
        JS_SetPropertyInt64(ctx, cell, JS_FNT_THIS, JS_UNDEFINED) < 0)
        goto fail;
    r = JS_Call(ctx, f, thisv, (int)n, (JSValueConst *)argv);
    for (i = 0; i < n; i++) JS_FreeValue(ctx, argv[i]);
    js_free(ctx, argv);
    JS_FreeValue(ctx, a);
    JS_FreeValue(ctx, thisv);
    if (JS_IsException(r))
        return JS_EXCEPTION;
    if (JS_SetPropertyInt64(ctx, cell, JS_FNT_RESULT, JS_DupValue(ctx, r)) < 0) {
        JS_FreeValue(ctx, r);
        return JS_EXCEPTION;
    }
    return r;
...
/* the timer callback for both debounce and throttle */
static JSValue js_fn_timer_fire(JSContext *ctx, JSValueConst this_val,
                                int argc, JSValueConst *argv, int magic,
                                JSValue *fd)
{
    JSValue r;
    (void)this_val; (void)argc; (void)argv; (void)magic;
    if (JS_SetPropertyInt64(ctx, fd[1], JS_FNT_TIMER, JS_UNDEFINED) < 0)
        return JS_EXCEPTION;
    if (JS_SetPropertyInt64(ctx, fd[1], JS_FNT_LAST,
                            JS_NewFloat64(ctx, (double)date_now())) < 0)
        return JS_EXCEPTION;
    r = js_fn_timer_invoke(ctx, fd[0], fd[1]);
    if (JS_IsException(r))
        return JS_EXCEPTION;
    JS_FreeValue(ctx, r);
    return JS_UNDEFINED;
...
static JSValue js_fn_debounce_call(JSContext *ctx, JSValueConst this_val,
                                   int argc, JSValueConst *argv, int magic,
                                   JSValue *fd)
{
    JSValue cb, msv;
    double ms;
    (void)magic;
    if (js_fn_timer_clear(ctx, fd[1]))
        return JS_EXCEPTION;
    if (js_fn_timer_save(ctx, fd[1], this_val, argc, argv))
        return JS_EXCEPTION;
    msv = JS_GetPropertyInt64(ctx, fd[2], 0);
    if (JS_ToFloat64(ctx, &ms, msv)) { JS_FreeValue(ctx, msv); return JS_EXCEPTION; }
    JS_FreeValue(ctx, msv);
    cb = JS_NewCFunctionData(ctx, js_fn_timer_fire, 0, 0, 2, (JSValueConst *)fd);
    if (JS_IsException(cb))
        return JS_EXCEPTION;
    if (js_fn_timer_arm(ctx, fd[1], cb, ms)) { JS_FreeValue(ctx, cb); return JS_EXCEPTION; }
    JS_FreeValue(ctx, cb);
    return JS_UNDEFINED;
}

static JSValue js_fn_throttle_call(JSContext *ctx, JSValueConst this_val,
                                   int argc, JSValueConst *argv, int magic,
                                   JSValue *fd)
{
    JSValue lastv, msv, timer, cb, r;
    double ms, last, now = (double)date_now();
    (void)magic;

    msv = JS_GetPropertyInt64(ctx, fd[2], 0);
    if (JS_ToFloat64(ctx, &ms, msv)) { JS_FreeValue(ctx, msv); return JS_EXCEPTION; }
    JS_FreeValue(ctx, msv);
    lastv = JS_GetPropertyInt64(ctx, fd[1], JS_FNT_LAST);
    if (JS_IsException(lastv)) return JS_EXCEPTION;
    if (JS_ToFloat64(ctx, &last, lastv)) { JS_FreeValue(ctx, lastv); return JS_EXCEPTION; }
    JS_FreeValue(ctx, lastv);

    if (now - last >= ms) {
        /* leading edge: run it now */
        if (JS_SetPropertyInt64(ctx, fd[1], JS_FNT_LAST, JS_NewFloat64(ctx, now)) < 0)
            return JS_EXCEPTION;
        if (js_fn_timer_save(ctx, fd[1], this_val, argc, argv))
            return JS_EXCEPTION;
        return js_fn_timer_invoke(ctx, fd[0], fd[1]);
    }
    /* inside the window: remember the newest call and make sure a trailing one
     * is scheduled. Dropping it instead would lose the LAST value, which is the
     * one a caller is usually throttling towards. */
    if (js_fn_timer_save(ctx, fd[1], this_val, argc, argv))
        return JS_EXCEPTION;
    timer = JS_GetPropertyInt64(ctx, fd[1], JS_FNT_TIMER);
    if (JS_IsException(timer)) return JS_EXCEPTION;
    if (JS_IsUndefined(timer)) {
        cb = JS_NewCFunctionData(ctx, js_fn_timer_fire, 0, 0, 2, (JSValueConst *)fd);
        if (JS_IsException(cb)) { JS_FreeValue(ctx, timer); return JS_EXCEPTION; }
        if (js_fn_timer_arm(ctx, fd[1], cb, ms - (now - last))) {
            JS_FreeValue(ctx, cb);
            JS_FreeValue(ctx, timer);
            return JS_EXCEPTION;
        }
        JS_FreeValue(ctx, cb);
    }
    JS_FreeValue(ctx, timer);
    r = JS_GetPropertyInt64(ctx, fd[1], JS_FNT_RESULT);
    return r;
}

static JSValue js_fn_timer_cancel(JSContext *ctx, JSValueConst this_val,
                                  int argc, JSValueConst *argv, int magic,
                                  JSValue *fd)
{
    (void)this_val; (void)argc; (void)argv; (void)magic;
    if (js_fn_timer_clear(ctx, fd[0]))
        return JS_EXCEPTION;
    if (JS_SetPropertyInt64(ctx, fd[0], JS_FNT_ARGS, JS_UNDEFINED) < 0 ||
        JS_SetPropertyInt64(ctx, fd[0], JS_FNT_THIS, JS_UNDEFINED) < 0)
        return JS_EXCEPTION;
    return JS_UNDEFINED;
}

static JSValue js_fn_timer_flush(JSContext *ctx, JSValueConst this_val,
                                 int argc, JSValueConst *argv, int magic,
                                 JSValue *fd)
{
    (void)this_val; (void)argc; (void)argv; (void)magic;
...
/* Build the [timer, args, this, last, result] cell. */
static JSValue js_fn_timer_cell(JSContext *ctx, double last)
{
    JSValue cell = JS_NewArray(ctx);
    if (JS_IsException(cell))
        return cell;
    if (JS_SetPropertyInt64(ctx, cell, JS_FNT_TIMER, JS_UNDEFINED) < 0 ||
        JS_SetPropertyInt64(ctx, cell, JS_FNT_ARGS, JS_UNDEFINED) < 0 ||
        JS_SetPropertyInt64(ctx, cell, JS_FNT_THIS, JS_UNDEFINED) < 0 ||
        JS_SetPropertyInt64(ctx, cell, JS_FNT_LAST, JS_NewFloat64(ctx, last)) < 0 ||
        JS_SetPropertyInt64(ctx, cell, JS_FNT_RESULT, JS_UNDEFINED) < 0) {
        JS_FreeValue(ctx, cell);
        return JS_EXCEPTION;
    }
    return cell;
...
/* magic 0 = debounce (trailing edge), 1 = throttle (leading + trailing). */
static JSValue js_function_ext_timed(JSContext *ctx, JSValueConst this_val,
                                     int argc, JSValueConst *argv, int magic)
{
    JSValue cell, msbox, wrapper, m;
    JSValueConst nd[3], cd[2];
    double ms = 0;

    if (argc > 0 && JS_ToFloat64(ctx, &ms, argv[0]))
        return JS_EXCEPTION;
    if (!(ms >= 0))
        return JS_ThrowRangeError(ctx, "the delay must be a non-negative number");
    /* A throttle starts ready to fire; a debounce has no such notion. */
    cell = js_fn_timer_cell(ctx, magic ? -1e18 : 0);
    if (JS_IsException(cell))
        return cell;
    msbox = JS_NewArray(ctx);
    if (JS_IsException(msbox)) { JS_FreeValue(ctx, cell); return msbox; }
    if (JS_SetPropertyInt64(ctx, msbox, 0, JS_NewFloat64(ctx, ms)) < 0) {
        JS_FreeValue(ctx, cell); JS_FreeValue(ctx, msbox);
        return JS_EXCEPTION;
    }
    nd[0] = this_val;
    nd[1] = cell;
    nd[2] = msbox;
    wrapper = JS_NewCFunctionData(ctx,
        magic ? js_fn_throttle_call : js_fn_debounce_call, 0, 0, 3, nd);
    if (JS_IsException(wrapper)) goto done;

    cd[0] = cell;
    m = JS_NewCFunctionData(ctx, js_fn_timer_cancel, 0, 0, 1, cd);
    if (JS_IsException(m)) { JS_FreeValue(ctx, wrapper); wrapper = JS_EXCEPTION; goto done; }
    JS_SetPropertyStr(ctx, wrapper, "cancel", m);

    cd[0] = this_val;
    cd[1] = cell;
    m = JS_NewCFunctionData(ctx, js_fn_timer_flush, 0, 0, 2, cd);
    if (JS_IsException(m)) { JS_FreeValue(ctx, wrapper); wrapper = JS_EXCEPTION; goto done; }
    JS_SetPropertyStr(ctx, wrapper, "flush", m);
 done:
    JS_FreeValue(ctx, cell);
    JS_FreeValue(ctx, msbox);
    return wrapper;
}

/* delay(ms, ...args): schedule ONE call and return a handle with cancel(). It
 * is not a wrapper -- the call is already committed -- so it returns the handle
 * rather than a function, which is what makes the two impossible to confuse. */
static JSValue js_function_ext_delay(JSContext *ctx, JSValueConst this_val,
                                     int argc, JSValueConst *argv, int magic)
{
    JSValue cell, cb, handle, m;
    JSValueConst nd[2];
    double ms = 0;
    int i;
    (void)magic;
    ...
    if (!(ms >= 0))
        return JS_ThrowRangeError(ctx, "the delay must be a non-negative number");
    cell = js_fn_timer_cell(ctx, 0);
    if (JS_IsException(cell))
        return cell;
    {   /* the trailing arguments become the call's arguments */
        JSValue a = JS_NewArray(ctx);
        if (JS_IsException(a)) { JS_FreeValue(ctx, cell); return a; }
        for (i = 1; i < argc; i++)
            if (JS_SetPropertyInt64(ctx, a, i - 1, JS_DupValue(ctx, argv[i])) < 0) {
                JS_FreeValue(ctx, a); JS_FreeValue(ctx, cell);
                return JS_EXCEPTION;
            }
        if (JS_SetPropertyInt64(ctx, cell, JS_FNT_ARGS, a) < 0) {
            JS_FreeValue(ctx, cell);
    ...
    nd[0] = this_val;
    nd[1] = cell;
    cb = JS_NewCFunctionData(ctx, js_fn_timer_fire, 0, 0, 2, nd);
    if (JS_IsException(cb)) { JS_FreeValue(ctx, cell); return cb; }
    if (js_fn_timer_arm(ctx, cell, cb, ms)) {
        JS_FreeValue(ctx, cb); JS_FreeValue(ctx, cell);
        return JS_EXCEPTION;
    }
    JS_FreeValue(ctx, cb);
    handle = JS_NewObject(ctx);
    if (JS_IsException(handle)) { JS_FreeValue(ctx, cell); return handle; }
    m = JS_NewCFunctionData(ctx, js_fn_timer_cancel, 0, 0, 1, (JSValueConst *)&cell);
    if (JS_IsException(m)) {
        JS_FreeValue(ctx, handle); JS_FreeValue(ctx, cell);
        return JS_EXCEPTION;
    }
    JS_SetPropertyStr(ctx, handle, "cancel", m);
    JS_FreeValue(ctx, cell);
    return handle;
}

/* memoize(keyFn?): a Map from key to result.
 *
 * The DEFAULT key is the first argument, used as a Map key -- so it is exact
 * (SameValueZero), never a stringification that collides 1 with "1". Calling a
 * default-keyed memo with more than one argument THROWS and names memoize(fn),
 * because any single-argument key for a multi-argument call is a silent
 * collision waiting for the caller. */
static JSValue js_fn_memoize_call(JSContext *ctx, JSValueConst this_val,
                                  int argc, JSValueConst *argv, int magic,
                                  JSValue *fd)
{
    JSValueConst f = fd[0], cache = fd[1], keyfn = fd[2];
    JSValueConst mget = fd[3], mset = fd[4], mhas = fd[5];
    JSValue key, got, has, r;
    (void)magic;

    if (JS_IsFunction(ctx, keyfn)) {
        key = JS_Call(ctx, keyfn, this_val, argc, argv);
        if (JS_IsException(key))
            return JS_EXCEPTION;
    } else {
        if (argc > 1)
            return JS_ThrowTypeError(ctx,
                "memoize() keys on the first argument only; pass a key function "
                "-- memoize(fn) -- for a call with %d arguments", argc);
        key = JS_DupValue(ctx, argc > 0 ? argv[0] : JS_UNDEFINED);
    }
    /* GET first, HAS only to disambiguate. A hit is the common case and the
     * naive has()-then-get() pays TWO map lookups for it; get() alone
     * distinguishes every hit except one whose cached value is undefined, and
     * that is the only case that needs the second call. */
    got = JS_Call(ctx, mget, cache, 1, (JSValueConst *)&key);
    if (JS_IsException(got)) { JS_FreeValue(ctx, key); return JS_EXCEPTION; }
    if (!JS_IsUndefined(got)) {
        JS_FreeValue(ctx, key);
        return got;
    }
    JS_FreeValue(ctx, got);
    has = JS_Call(ctx, mhas, cache, 1, (JSValueConst *)&key);
    if (JS_IsException(has)) { JS_FreeValue(ctx, key); return JS_EXCEPTION; }
    if (JS_ToBool(ctx, has)) {
        JS_FreeValue(ctx, has);
        JS_FreeValue(ctx, key);
        return JS_UNDEFINED;          /* a cached undefined is still a hit */
    }
    JS_FreeValue(ctx, has);
    /* A THROWING call is not cached: the next call gets to try again, which is
     * what a caller expects of a memo over something that can fail. */
    r = JS_Call(ctx, f, this_val, argc, argv);
    if (JS_IsException(r)) { JS_FreeValue(ctx, key); return JS_EXCEPTION; }
    {
        JSValueConst sa[2];
        JSValue sr;
        sa[0] = key;
        sa[1] = r;
        sr = JS_Call(ctx, mset, cache, 2, sa);
        JS_FreeValue(ctx, key);
        if (JS_IsException(sr)) { JS_FreeValue(ctx, r); return JS_EXCEPTION; }
        JS_FreeValue(ctx, sr);
    }
    return r;
}

static JSValue js_function_ext_memoize(JSContext *ctx, JSValueConst this_val,
                                       int argc, JSValueConst *argv, int magic)
{
    JSValue g, mapctor, cache, wrapper;
    JSValue mget = JS_UNDEFINED, mset = JS_UNDEFINED, mhas = JS_UNDEFINED;
    JSValueConst nd[6];
    (void)magic;

    /* The cache is a real Map, so keys compare by SameValueZero and 1 never
     * collides with "1". Its three methods are resolved ONCE here rather than
     * per call -- a memo exists to avoid work, so it may not add a property
     * lookup to every hit. */
    g = JS_GetGlobalObject(ctx);
    mapctor = JS_GetPropertyStr(ctx, g, "Map");
    JS_FreeValue(ctx, g);
    if (JS_IsException(mapctor))
        return mapctor;
    cache = JS_CallConstructor(ctx, mapctor, 0, NULL);
    JS_FreeValue(ctx, mapctor);
    if (JS_IsException(cache))
        return cache;
    mget = JS_GetPropertyStr(ctx, cache, "get");
    mset = JS_GetPropertyStr(ctx, cache, "set");
    mhas = JS_GetPropertyStr(ctx, cache, "has");
    if (JS_IsException(mget) || JS_IsException(mset) || JS_IsException(mhas)) {
        JS_FreeValue(ctx, cache); JS_FreeValue(ctx, mget);
        JS_FreeValue(ctx, mset); JS_FreeValue(ctx, mhas);
        return JS_EXCEPTION;
    }
    nd[0] = this_val;
    nd[1] = cache;
    nd[2] = argc > 0 ? argv[0] : JS_UNDEFINED;
    nd[3] = mget;
    nd[4] = mset;
    nd[5] = mhas;
    wrapper = JS_NewCFunctionData(ctx, js_fn_memoize_call, 0, 0, 6, nd);
    JS_FreeValue(ctx, cache);
    JS_FreeValue(ctx, mget);
    JS_FreeValue(ctx, mset);
    JS_FreeValue(ctx, mhas);
    return wrapper;
}
...
static const JSCFunctionListEntry js_function_ext_funcs[] = {
    ...
    /* timed wrappers */
    JS_CFUNC_MAGIC_DEF("debounce",     1, js_function_ext_timed, 0 ),
    JS_CFUNC_MAGIC_DEF("throttle",     1, js_function_ext_timed, 1 ),
    JS_CFUNC_MAGIC_DEF("delay",        1, js_function_ext_delay, 0 ),
    JS_CFUNC_MAGIC_DEF("memoize",      0, js_function_ext_memoize, 0 ),
};

/* ===== non-ES Function STATICS — installed non-enumerable on the Function
 * CONSTRUCTOR (never the prototype). `this_val`
 ...
 * to slice, and passing all remaining args at the final depth. */
 ...
 * of its list arguments; lift = liftN(fn.length, fn). */
 ...
 * concat of map(f, applyX) over each f in applyF. */
...
/* ==========================================================================
   Slow-array (dictionary/holey) element fast paths.

   A JS_CLASS_ARRAY in slow mode (!p->fast_array) stores its elements as shape
   properties with tagged-int atoms (__JS_AtomFromUInt32), value at
   p->prop[i].u.value; a hole is simply an absent property. One hole demotes
   an array, and every builtin below used to fall back to a per-INDEX loop
   over JS_GetPropertyInt64/JS_TryGetPropertyInt64 (atom + hash lookup +
   prototype walk per index), so a 1e6-hole array cost 1e6 property walks to
   answer a MISS. These helpers enumerate the OCCUPIED entries instead -- the
   way JS_GetOwnPropertyNamesInternal walks the shape table.

   Shared semantic contract, documented at each call site:

   - Holes are prototype-visible: spec Get(O,k)/HasProperty(O,k) consult the
     prototype chain (Array.prototype[5] = 'x' shows through a hole at 5), and
     the generic loops DO consult it (TryGetPropertyInt64 -> JS_HasProperty).
     Every fast path here therefore requires js_proto_chain_no_indexed();
     under that guard a hole is genuinely absent/undefined and may be skipped
     or matched against `undefined` without observable difference.
   - Index positions holding JS_PROP_TMASK entries (accessors, var-refs) are
     refused: reading u.value raw would bypass the getter the generic path
     calls (and the getter is observable).
   - Mutating fast paths additionally require every element to have default
     attributes (JS_PROP_C_W_E): frozen/sealed/partially-writable arrays keep
     the generic loop, which implements the spec's exact partial-mutation and
     throw order. Read-only scans do not care about attributes.
   - Once the guards pass, no user code can run during the operation (strict
     equality never calls out; writes go to plain own data properties), so
     shape-table positions recorded at entry stay valid.
   ========================================================================== */

typedef struct JSSlowArrEnt {
    uint32_t idx; /* element index */
    uint32_t pos; /* shape/prop slot of the element property */
    JSValue val;  /* dup of the element value, consumed by the move */
} JSSlowArrEnt;

static int js_slow_arr_ent_cmp(const void *a, const void *b, void *arg)
{
    uint32_t ia = ((const JSSlowArrEnt *)a)->idx;
    uint32_t ib = ((const JSSlowArrEnt *)b)->idx;
    (void)arg;
    return ia < ib ? -1 : ia > ib ? +1 : 0;
}

/* binary search for an exact index in a sorted entry list, with an O(1)
   neighbor hint (the list position adjacent to the one being processed) */
static int js_slow_arr_find(JSSlowArrEnt *ents, int occ, int64_t tgt, int hint)
{
    int lo, hi;
    if (hint >= 0 && hint < occ && (int64_t)ents[hint].idx == tgt)
        return hint;
    lo = 0;
    hi = occ - 1;
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        if ((int64_t)ents[mid].idx == tgt)
            return mid;
        if ((int64_t)ents[mid].idx < tgt)
            lo = mid + 1;
        else
...
/* Sort the entry list by index. Nearly-sorted lists (each operation on the
   same object leaves a couple of appended entries at the shape tail) cost
   O(occ + shifts) with insertion sort; arbitrary orders fall back to rqsort
   once the shift counter shows the insertion pass would go quadratic. */
static void js_slow_arr_sort(JSSlowArrEnt *ents, int occ)
{
    int k;
    int64_t shifts = 0;
    for(k = 1; k < occ; k++) {
        JSSlowArrEnt t = ents[k];
        int j = k - 1;
        if (ents[j].idx <= t.idx)
            continue;
        ...
        ents[j + 1] = t;
        if (shifts > (int64_t)4 * occ) {
            rqsort(ents, occ, sizeof(*ents), js_slow_arr_ent_cmp, NULL);
            return;
        }
    }
}

/* TRUE if no object on p's prototype chain has an own integer-index property
   (and none may synthesize one: only plain objects/arrays may appear on the
   chain; a Proxy there could answer index reads from its target). Covers both
   index representations: tagged-int atoms (indices < 2^31) and array-index
   string atoms (2^31 <= index < 2^32), since Array.prototype[4000000000]
   shows through a hole exactly like Array.prototype[5] does. */
static BOOL js_proto_chain_no_indexed(JSContext *ctx, JSObject *p)
{
    JSObject *proto = p->shape->proto;
    for (; proto != NULL; proto = proto->shape->proto) {
        JSShape *sh = proto->shape;
        JSShapeProperty *prs = get_shape_prop(sh);
        int i;
        if (unlikely(proto->class_id != JS_CLASS_OBJECT &&
                     proto->class_id != JS_CLASS_ARRAY))
            return FALSE;
        for(i = 0; i < sh->prop_count; i++) {
            JSAtom a = prs[i].atom;
            if (a == JS_ATOM_NULL)
                continue;
            if (__JS_AtomIsTaggedInt(a))
                return FALSE;
            if (unlikely(JS_AtomIsArrayIndex(ctx, &(uint32_t){0}, a)))
                return FALSE;
        }
    }
    return TRUE;
...
/* A slow JS_CLASS_ARRAY whose occupied entries may be scanned directly by the
   element fast paths (see the contract above). Refuses arrays holding an
   integer index stored as a STRING atom: tagged-int atoms only reach
   2^31-1, so e.g. a[4294967294] lives in the shape as "4294967294" and the
   tagged-int walk would silently miss it. Such arrays keep the generic
   paths, which map every int64 index to its atom. */
static BOOL js_slow_array_scan_start(JSContext *ctx, JSObject *p)
{
    JSShape *sh;
    JSShapeProperty *prs;
    int i;
    if (p->class_id != JS_CLASS_ARRAY || p->fast_array)
        return FALSE;
    if (!js_proto_chain_no_indexed(ctx, p))
        return FALSE;
    sh = p->shape;
    prs = get_shape_prop(sh);
    for(i = 0; i < sh->prop_count; i++) {
        JSAtom a = prs[i].atom;
        if (a == JS_ATOM_NULL || __JS_AtomIsTaggedInt(a))
            continue;
        {
            uint32_t dummy;
            if (JS_AtomIsArrayIndex(ctx, &dummy, a))
                return FALSE;
        }
    }
    return TRUE;
...
/* full eligibility check for paths that MUST know eligibility before touching
   any value (the read-only scans below instead check JS_PROP_TMASK lazily per
   entry and restart generically -- they are stateless, so a restart is
   invisible) */
static BOOL js_slow_array_scan_ok(JSContext *ctx, JSObject *p)
{
    JSShape *sh;
    JSShapeProperty *prs;
    int i;
    if (!js_slow_array_scan_start(ctx, p))
        return FALSE;
    sh = p->shape;
    prs = get_shape_prop(sh);
    for(i = 0; i < sh->prop_count; i++) {
        if (prs[i].atom != JS_ATOM_NULL && __JS_AtomIsTaggedInt(prs[i].atom) &&
            (prs[i].flags & JS_PROP_TMASK) != 0)
            return FALSE;
    }
    return TRUE;
}

/* Best-effort inverse of convert_fast_array_to_array(): a slow array with no
   holes (occupied == length) and default element attributes is rebuilt as a
   fast array so the dense memmove paths can run. Element entries are removed
   exactly the way delete_property() removes them (value freed, atom cleared,
   deleted_prop_count++), then compact_properties() rebuilds the shape and its
   hash in one O(n) pass -- no per-element hash walks. Returns FALSE if any
   precondition fails (callers silently use the generic path; OOM surfaces
   there too). */
static no_inline BOOL js_slow_array_try_convert_fast(JSContext *ctx,
                                                     JSObject *p)
{
    JSShape *sh;
    JSShapeProperty *prs;
    ...
    if (p->class_id != JS_CLASS_ARRAY || p->fast_array)
        return FALSE;
    /* Array.prototype itself manages is_std_array_prototype via add_property;
       it is never converted back to fast. */
    ...
    sh = p->shape;
    prs = get_shape_prop(sh);
    for(i = 0; i < sh->prop_count; i++, prs++) {
        JSAtom a = prs->atom;
        if (a == JS_ATOM_NULL)
            continue;
        if (__JS_AtomIsTaggedInt(a)) {
            if (prs->flags != JS_PROP_C_W_E)
                return FALSE;
            occ++;
        } else if (a == JS_ATOM_length) {
            if (!(prs->flags & JS_PROP_WRITABLE))
                return FALSE;
        }
        /* named properties are kept: fast arrays hold them in the shape too */
    }
    if (occ == 0 || occ > INT32_MAX)
        return FALSE;
    prs = get_shape_prop(sh);
    if (prs[0].atom != JS_ATOM_length ||
    ...
    vals = js_malloc(ctx, sizeof(JSValue) * occ);
    if (!vals)
        return FALSE;
    ...
    for(i = 0; i < sh->prop_count; i++, prs++) {
        ...
        if (a == JS_ATOM_NULL || !__JS_AtomIsTaggedInt(a))
            continue;
        vals[__JS_AtomToUInt32(a)] = JS_DupValue(ctx, p->prop[i].u.value);
        /* the removal half of delete_property(), minus the hash-chain walk */
        free_property(ctx->rt, &p->prop[i], prs->flags);
        JS_FreeAtom(ctx, a);
        prs->flags = 0;
        prs->atom = JS_ATOM_NULL;
        p->prop[i].u.value = JS_UNDEFINED;
        sh->deleted_prop_count++;
    }
    /* On OOM this leaves a valid slow shape: cleared entries are skipped by
       every lookup and enumeration; the values are already safe in vals[]. */
    compact_properties(ctx, p);

    p->u.array.u.values = vals;
    p->u.array.u1.size = occ;
    p->u.array.count = occ;
    p->fast_array = TRUE;
    return TRUE;
}

/* shift/unshift element move on a slow array with holes. delta = -1 (shift)
   or +argc (unshift); every occupied element i moves to i+delta, holes are
   preserved, the entries nobody moves into are deleted.
   Returns 1 when the move was performed (the caller skips JS_CopySubArray;
   unshift still writes argv at [0..argc) and both callers fix up the length,
   exactly as they do after JS_CopySubArray), 0 to fall back to
   JS_CopySubArray (which gets a dense-conversion chance first), -1 on OOM.

   Spec: Array.prototype.shift steps 5-8 / unshift steps 5-8 move each PRESENT
   element via HasProperty+Get+Set and delete the vacated slot; with
   js_slow_array_scan_ok() (no indexed proto props, no accessors) plus default
   attributes, moving the occupied entries directly and deleting
   { j : j-delta not occupied } is indistinguishable from the generic loop:
   holes stay holes, every value lands on its new index, and no index in
   [0,delta) or [len+delta, ...) survives. */
static no_inline int js_slow_array_shift_unshift(JSContext *ctx, JSObject *p,
                                                 int64_t len, int64_t delta)
{
    JSShape *sh;
    JSShapeProperty *prs;
    JSSlowArrEnt *ents;
    JSValue *rval;
    int *ridx;
    int nring, head, tail, count;
    int occ = 0, cap = 0, k, kk, kk2, asc, ret = 0;
    int64_t i, t;

    if (len < 2 || len > UINT32_MAX)
        return 0;
    /* targets must stay valid index atoms */
    if (delta > 0 && len + delta > UINT32_MAX)
        return 0;
    /* dense arrays are worth more converted to fast (memmove from now on) */
    if (js_slow_array_try_convert_fast(ctx, p))
        return 0;
    if (!js_slow_array_scan_start(ctx, p))
        return 0;
    sh = p->shape;
    /* one walk collects (index, slot); values stay in their slots */
    occ = 0;
    cap = 0;
    ents = NULL;
    asc = 1;
    ...
    for(i = 0; i < sh->prop_count; i++, prs++) {
        ...
        if (occ == cap) {
            int newcap = cap < 16 ? 16 : cap * 2;
            JSSlowArrEnt *nents = js_realloc(ctx, ents,
                                             sizeof(*ents) * newcap);
            if (!nents) {
                js_free(ctx, ents);
                return -1;
        ...
        ents[occ].idx = __JS_AtomToUInt32(a);
        ents[occ].pos = (uint32_t)i;
        ents[occ].val = JS_UNDEFINED;
        if ((int64_t)ents[occ].idx >= len) {
            /* defense in depth: an element at/after `length` is never
               visited by the generic path (CopySubArray reads [1, len) for
               shift and [0, len) for unshift), so it must not be moved or
               deleted here either. Unreachable via engine paths (every
               length shrink deletes the covered indices), but cheap. */
            js_free(ctx, ents);
            return 0;
        }
        if (occ > 0 && ents[occ - 1].idx >= ents[occ].idx)
            asc = 0;
        occ++;
    }
    if (occ == 0) {
        js_free(ctx, ents);
        /* all holes: move and delete pass are no-ops, and the prototype
           guard proved the generic loop's per-index lookups would all miss,
           so skipping JS_CopySubArray entirely is invisible. The caller
           still runs Delete(newLen) (shift), the argv writes and the
           SetLength, exactly as after JS_CopySubArray. */
        return 1;
    }
    if (!asc)
        js_slow_arr_sort(ents, occ);
    /* destinations must stay representable as tagged-int atoms: indices
       above INT32_MAX are stored as string atoms and __JS_AtomFromUInt32
       would wrap them (only reachable for unshift: shift destinations are
       src-1 < 2^31 because scan_start refused string-atom indices). */
    if (delta > 0 && (int64_t)ents[occ - 1].idx + delta > INT32_MAX) {
        js_free(ctx, ents);
        return 0;
    }

    /* new[i] = old[i+delta] is a pure shift of the value stream. Grab each
       slot's own value (the slot is left JS_UNDEFINED), then forward it to
       its destination a+delta: a still-unprocessed occupied slot (hold it in
       a FIFO of at most delta in-flight values and deposit when that slot's
       turn comes) or a hole (new property, right away). Each value is moved
       as a pointer exactly once; there is no refcount traffic.
       Spec: shift steps 5-8 / unshift steps 5-8 write each PRESENT element
       to its new index with the same result. */
    nring = delta > 0 ? (delta < occ ? delta : occ) : 1;
    ridx = js_malloc(ctx, sizeof(*ridx) * nring);
    if (!ridx) {
        js_free(ctx, ents);
        return -1;
    }
    rval = js_malloc(ctx, sizeof(*rval) * nring);
    if (!rval) {
        js_free(ctx, ridx);
        js_free(ctx, ents);
        return -1;
    ...
    head = tail = count = 0;
    for(kk = 0; kk < occ; kk++) {
        int k = delta < 0 ? occ - 1 - kk : kk;
        int64_t a = (int64_t)ents[k].idx;
        JSProperty *pslot = &p->prop[ents[k].pos];
        JSValue own = pslot->u.value;
        pslot->u.value = JS_UNDEFINED;
        if (count > 0 && (int64_t)ents[ridx[head]].idx == a) {
            /* this slot's new value just arrived */
            pslot->u.value = rval[head];
            head = (head + 1) % nring;
            count--;
        }
        t = a + delta;
        if (t < 0) {
            /* shift drops index 0; the caller already read it as the result
               (holding its own reference), so release this one */
            JS_FreeValue(ctx, own);
            continue;
        }
        kk2 = js_slow_arr_find(ents, occ, t, k + (int)delta);
        if (kk2 >= 0) {
            rval[tail] = own;
            ridx[tail] = kk2;
            tail = (tail + 1) % nring;
            ...
            /* destination is a hole */
            JSProperty *pr;
            pr = add_property(ctx, p, __JS_AtomFromUInt32((uint32_t)t),
                              JS_PROP_C_W_E);
            if (!pr) {
                JS_FreeValue(ctx, own);
                ret = -1;
    ...
    /* delete the sources nobody moved into:
       shift: j with j+1 not occupied (includes j == len-1);
       unshift: j with j-delta not occupied (includes j < delta).
       Their values already moved out above, so the deletes free nothing. */
    for(k = 0; k < occ; k++) {
        int64_t src = (int64_t)ents[k].idx - delta;
        int hint = (delta == -1) ? k + 1 : (delta == 1 ? k - 1 : -1);
        if (src < 0 || js_slow_arr_find(ents, occ, src, hint) < 0)
            delete_property(ctx, p, __JS_AtomFromUInt32(ents[k].idx));
    }
    ret = 1;

 drain:
    while (count > 0) {
        JS_FreeValue(ctx, rval[head]);
        head = (head + 1) % nring;
        count--;
    }
    js_free(ctx, ridx);
...
/* slice/concat read side: define only the OCCUPIED entries of the slow array
   `sp` that fall in [start, final) on the (plain, freshly created) result
   array, using the exact property-set call the generic loop uses, so the
   result is identical -- holes simply stay holes. Entries are visited in
   shape order; if that order is not ascending the walk bails (returning 0)
   after having defined a prefix: the generic loop then defines the rest,
   and re-defining a data property with the same value and default attributes
   is unobservable. Returns 1 filled, 0 not applicable, -1 exception. */
static no_inline int js_slow_array_fill_range(JSContext *ctx, JSObject *sp,
                                              JSValueConst arr,
                                              int64_t start, int64_t final,
                                              int64_t base, BOOL is_slice)
{
    JSShape *sh;
    JSShapeProperty *prs;
    int64_t i, prev;
    ...
    if (!js_slow_array_scan_ok(ctx, sp))
        return 0;
    sh = sp->shape;
    prs = get_shape_prop(sh);
    prev = -1;
    for(i = 0; i < sh->prop_count; i++, prs++) {
        ...
        if (a == JS_ATOM_NULL || !__JS_AtomIsTaggedInt(a))
            continue;
        idx = __JS_AtomToUInt32(a);
        v = (int64_t)idx;
        if (!first && v <= prev)
            return 0; /* shape not in index order: let the generic loop run */
        prev = v;
        first = 0;
        if (v < start || v >= final)
            continue;
        val = JS_DupValue(ctx, sp->prop[i].u.value);
        if (is_slice) {
            if (JS_CreateDataPropertyUint32(ctx, arr,
                                            (uint32_t)(v - start),
                                            val, JS_PROP_THROW) < 0)
                return -1;
        } else {
            if (JS_DefinePropertyValueInt64(ctx, arr, base + v, val,
                                            JS_PROP_C_W_E |
                                            JS_PROP_THROW) < 0)
                return -1;
        }
                           ...
                           int64_t from_pos, int64_t count, int dir)
{
    ...
    if (JS_VALUE_GET_TAG(obj) == JS_TAG_OBJECT) {
        p = JS_VALUE_GET_OBJ(obj);
        if (p->class_id != JS_CLASS_ARRAY || !p->fast_array) {
            /* one hole (or a defineProperty) demoted this array: the generic
               loop below pays JS_TryGetPropertyInt64 + JS_SetPropertyInt64
               per element. A dense slow array (occupied == length, default
               attributes) is rebuilt as a fast array so the memmove path
               applies; with holes the generic loop stays correct. */
            js_slow_array_try_convert_fast(ctx, p);
        }
        if (p->class_id != JS_CLASS_ARRAY || !p->fast_array) {
            p = NULL;
        }
...
/* ── Array.from / %TypedArray%.from fast paths ───────────
   js_array_from() runs the full iterator protocol per element
   (JS_IteratorNext builds a {value, done} result object per step, then
   JS_DefinePropertyValueInt64 re-parses the index atom and walks the
   whole define machinery, growing the result array one realloc at a
   time); %TypedArray%.from additionally materializes an intermediate
   JSArray (js_array_from_iterator).  For dense fast arrays
   (count == length: every index below length reads an own slot) and
   typed arrays whose @@iterator is still a built-in values(), the
   elements can be read directly and stored into the pre-sized result.
   Guards, so the observable semantics stay exactly the generic ones:
   - @@iterator must be the built-in Array.prototype.values /
     %TypedArray%.prototype.values C function: custom iterators,
     keys() and entries() keep the generic loop;
   - %ArrayIteratorPrototype%.next must still be the built-in (the
     generic loop resolves 'next' off the iterator's prototype, so an
     impostor would run per step);
   - with a mapfn, the per-step loop keeps js_array_iterator_next()'s
     observable behavior: the length is re-read per step (shrinking it
     stops the iteration, growing it extends it through prototype
     reads) and a detached typed array throws per step; without a
     mapfn no user code can run mid-copy, so the entry-time check
     cannot go stale;
   - IteratorClose on an abnormal exit looks up 'return' on the
     iterator's prototype chain: if one is visible, the generic loop
     runs so it is invoked.  (js_array_from_iterator, used by
     %TypedArray%.from, never closes its iterator, so that branch
     passes need_iter_close = FALSE to mirror it.)
   All direct stores follow the store-then-bump-count discipline: GC
   marks fast-array slots only below u.array.count (js_array_mark), so
   a collection mid-fill never sees uninitialized slots. */
...
/* Return 1 with the source object, its element count and typed-array
   flag set when the source iterates element-for-element identically to
   the built-in values() iterator, 0 to fall back to the generic loop,
   -1 when an exception is pending. */
static int js_from_fast_probe(JSContext *ctx, JSValueConst items,
                              JSValueConst iter, BOOL need_iter_close,
                              JSObject **pp, uint32_t *pcount, BOOL *pis_ta)
{
    JSObject *p;
    JSCFunctionType ft;
    uint32_t count;
    ...
    *pis_ta = FALSE;
    ...
    /* the 'next' the generic loop would call per step (resolved off
       the iterator object's prototype) must still be the built-in */
    ft.iterator_next = js_array_iterator_next;
    v = JS_GetProperty(ctx, ctx->class_proto[JS_CLASS_ARRAY_ITERATOR],
                       JS_ATOM_next);
    if (JS_IsException(v))
        return -1;
    if (!JS_IsCFunction(ctx, v, ft.generic, 0)) {
        JS_FreeValue(ctx, v);
        return 0;
    }
    JS_FreeValue(ctx, v);
    ...
    /* the source's @@iterator must be a built-in values() */
    ft.generic_magic = js_create_array_iterator;
    is_ta_iter = 0;
    if (!JS_IsCFunction(ctx, iter, ft.generic, JS_ITERATOR_KIND_VALUE)) {
        ft.generic_magic = js_create_typed_array_iterator;
        if (!JS_IsCFunction(ctx, iter, ft.generic, JS_ITERATOR_KIND_VALUE))
            return 0;
        /* the generic loop would call this iterator ctor on the source,
           which validates its 'this' and throws for a non-typed-array */
        is_ta_iter = 1;
    }
    ...
    if (p->class_id == JS_CLASS_ARRAY) {
        ...
        if (!p->fast_array)
            return 0;
        /* dense only: holes would be read from the prototype chain,
           and a length above count iterates through those reads too */
        if (js_get_length32(ctx, &len32, items))
            return -1;
        ...
        *pis_ta = FALSE;
        ...
        /* a detached buffer must keep taking the generic path, whose
           first next() raises the OOB TypeError */
    ...
    /* the same bound the OP_append bulk path uses: a fast-array target
       cannot hold more elements */
    if (count > 0x7ffffffe)
        return 0;
    *pp = p;
    *pcount = count;
...
/* A result usable by the direct-store loops: a fresh, empty,
   extensible fast array (JS_NewArray(), or a constructor that returned
   one) distinct from the source. */
static BOOL js_from_target_fast(JSValueConst r, JSValueConst src,
                                JSObject **prp)
{
    JSObject *rp;
    if (JS_VALUE_GET_TAG(r) != JS_TAG_OBJECT)
        return FALSE;
    rp = JS_VALUE_GET_OBJ(r);
    if (rp->class_id != JS_CLASS_ARRAY || !rp->fast_array ||
        !rp->extensible || rp->u.array.count != 0 ||
        JS_VALUE_GET_TAG(rp->prop[0].u.value) != JS_TAG_INT ||
        JS_VALUE_GET_INT(rp->prop[0].u.value) != 0)
        return FALSE;
    if (JS_VALUE_GET_TAG(src) == JS_TAG_OBJECT &&
        rp == JS_VALUE_GET_OBJ(src))
        return FALSE;
    *prp = rp;
    return TRUE;
}

/* One store into the from() result with the same observable semantics
   as the generic loop's JS_DefinePropertyValueInt64(): existing slot →
   plain overwrite (a C_W_E define on an existing fast-array element
   does exactly that); next slot on an extendable fast array →
   add_fast_array_element, the same append JS_CreateProperty performs;
   anything else (a captured-and-mutated result, non-extensible,
   subclass prototype) → the exact generic define.  Consumes 'v'.
   Returns < 0 on exception. */
static int js_from_store_element(JSContext *ctx, JSValueConst r,
                                 JSObject *rp, int64_t k, JSValue v)
{
    if (likely(rp->class_id == JS_CLASS_ARRAY && rp->fast_array &&
               ((uint64_t)k < (uint64_t)rp->u.array.count ||
                can_extend_fast_array(rp)))) {
        return JS_SetPropertyValue(ctx, r, JS_NewInt32(ctx, (int32_t)k), v,
                                   JS_PROP_C_W_E | JS_PROP_THROW);
    }
    return JS_DefinePropertyValueInt64(ctx, r, k, v,
                                       JS_PROP_C_W_E | JS_PROP_THROW);
}

/* Element read for the per-step loops: same value as the generic
   JS_GetPropertyInt64() (whose own fast path already returns the dense
   slot directly), one dispatch earlier. */
static JSValue js_from_read_element(JSContext *ctx, JSValueConst obj,
                                    JSObject *p, int64_t k)
{
    if (p->class_id == JS_CLASS_ARRAY && p->fast_array &&
        (uint64_t)k < (uint64_t)p->u.array.count)
        return JS_DupValue(ctx, p->u.array.u.values[k]);
    return JS_GetPropertyInt64(ctx, obj, k);
}
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    if (!JS_IsUndefined(iter) && !JS_IsNull(iter)) {
        ...
        if (JS_IsException(r))
            goto exception;
        /* probed here, after the constructor call: a constructor with
           side effects may have converted the source to a slow array,
           shrunk it or detached its buffer, and the bulk copy below
           must only run on the source state that the generic loop
           would have iterated */
        fast = js_from_fast_probe(ctx, items, iter, mapping, &sp, &count,
                                  &is_ta);
        if (fast < 0)
            goto exception;
        if (fast && js_from_target_fast(r, items, &rp)) {
            if (!mapping) {
                /* bulk copy: no user code can run mid-loop, so the
                   entry-time dense/oob checks cannot go stale */
                if (count != 0 && expand_fast_array(ctx, rp, count))
                    goto exception;
                if (is_ta) {
                    for (i = 0; i < count; i++) {
                        v = JS_GetPropertyInt64(ctx, items, i);
                        if (JS_IsException(v))
                            goto exception;
                        rp->u.array.u.values[i] = v;
                    }
                } else {
                    JSValue *sv = sp->u.array.u.values;
                    for (i = 0; i < count; i++)
                        rp->u.array.u.values[i] = JS_DupValue(ctx, sv[i]);
                }
                rp->u.array.count = count;
                k = count;
            } else {
                /* mapped loop: js_array_iterator_next()'s per-step
                   observable behavior, without the {value, done}
                   result objects */
                for (k = 0;; k++) {
                    uint32_t len32;
                    if (is_ta) {
                        if (typed_array_is_oob(sp)) {
                            JS_ThrowTypeErrorArrayBufferOOB(ctx);
                            goto exception_close;
                        }
                        len32 = sp->u.array.count;
                    } else {
                        /* the length is re-read per step exactly like
                           the built-in iterator does */
                        if (js_get_length32(ctx, &len32, items))
                            goto exception_close;
                    }
                    if (k >= len32)
                        break;
                    v = js_from_read_element(ctx, items, sp, k);
                    if (JS_IsException(v))
                        goto exception_close;
                    args[0] = v;
                    ...
                    if (JS_IsException(v))
                        goto exception_close;
                    if (js_from_store_element(ctx, r, rp, k, v) < 0)
                        goto exception_close;
                }
            }
        } else {
        ...
        if (js_get_length64(ctx, &len, arrayLike) < 0)
            goto exception;
        /* the length is captured once here, before the constructor
           runs, exactly as the spec and the generic loop read it */
        ...
        if (JS_IsException(r))
            goto exception;
        /* array-like fast source, probed after the constructor call
           (which may have converted the source to a slow array,
           shrunk it or detached its buffer): every index below the
           captured length must still read a dense own slot, i.e. the
           current element count must cover the captured length.  A
           larger count is fine: both paths read live values for the
           first len indexes only. */
        fast = FALSE;
        sp = NULL;
        count = 0;
        is_ta = FALSE;
        if (len <= 0x7ffffffe && JS_VALUE_GET_TAG(arrayLike) == JS_TAG_OBJECT) {
            JSObject *ap = JS_VALUE_GET_OBJ(arrayLike);
            if (ap->class_id == JS_CLASS_ARRAY && ap->fast_array) {
                if ((uint64_t)ap->u.array.count >= (uint64_t)len) {
                    sp = ap;
                    count = (uint32_t)len;
                    fast = TRUE;
                }
            } else if (ap->class_id >= JS_CLASS_UINT8C_ARRAY &&
                       ap->class_id <= JS_CLASS_FLOAT64_ARRAY &&
                       !typed_array_is_oob(ap) &&
                       (uint64_t)ap->u.array.count >= (uint64_t)len) {
                sp = ap;
                count = (uint32_t)len;
                is_ta = TRUE;
                fast = TRUE;
        ...
        if (fast && js_from_target_fast(r, arrayLike, &rp)) {
            if (!mapping) {
                if (count != 0 && expand_fast_array(ctx, rp, count))
                    goto exception;
                if (is_ta) {
                    for (i = 0; i < count; i++) {
                        v = JS_GetPropertyInt64(ctx, arrayLike, i);
                        if (JS_IsException(v))
                            goto exception;
                        rp->u.array.u.values[i] = v;
                    }
                ...
                /* length was read once at the top; the elements are
                   read live per step, exactly like the generic loop */
                for (k = 0; k < len; k++) {
                    v = js_from_read_element(ctx, arrayLike, sp, k);
                    if (JS_IsException(v))
                        goto exception;
                    args[0] = v;
                    args[1] = JS_NewInt32(ctx, k);
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    if (JS_IsException(arr))
        goto exception;
    /* defines into `arr` must not run user code (a species Proxy result
       keeps the generic loop below) */
    arr_plain = JS_VALUE_GET_TAG(arr) == JS_TAG_OBJECT &&
        JS_VALUE_GET_OBJ(arr)->class_id == JS_CLASS_ARRAY;
    n = 0;
    for (i = -1; i < argc; i++) {
        if (i < 0)
            e = obj;
        ...
        if (res) {
            ...
            /* slow-array source: define the OCCUPIED entries only (spec
               concat step 8: HasProperty+Get+CreateDataProperty per index;
               holes produce no property, and js_slow_array_fill_range's
               prototype guard makes the skipped holes genuinely absent) */
            if (arr_plain && len > 0 &&
                JS_VALUE_GET_TAG(e) == JS_TAG_OBJECT) {
                JSObject *sp = JS_VALUE_GET_OBJ(e);
                if (sp->class_id == JS_CLASS_ARRAY && !sp->fast_array) {
                    /* a dense slow source is worth converting first: the
                       generic loop below then reads it with the dense
                       shortcut instead of one hash lookup per index */
                    js_slow_array_try_convert_fast(ctx, sp);
                    res = js_slow_array_fill_range(ctx, sp, arr,
                                                   0, len, n, FALSE);
                    if (res < 0)
                        goto exception;
                    if (res > 0) {
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    /* Fast path for a dense array, as indexOf/includes/lastIndexOf/copyWithin
     * in this file already do. The generic form is two non-inlined calls and a
     * class-id switch per element to perform one 16-byte store and a refcount
     * bump.
     *
     * The bounds test is taken AFTER all three JS_ToInt64Clamp calls above,
     * because coercing argv[1]/argv[2] can run user code that shrinks the
     * array; `end <= count` is therefore checked against the count as it is
     * now. set_value frees the old element, and JS_FreeValue cannot re-enter
     * JS or reallocate u.array.u.values, so holding `arrp` across the loop is
     * safe -- the adjacent bug class is holding a pointer into a table a
     * callback can grow, and this loop invokes no callback. */
    {
        JSValue *arrp;
        uint32_t count;
        if (js_get_fast_array(ctx, obj, &arrp, &count) && end <= count) {
            for (; start < end; start++)
                set_value(ctx, &arrp[start], JS_DupValue(ctx, argv[0]));
            return obj;
        }
    }
    while (start < end) {
                                 ...
                                 int argc, JSValueConst *argv)
{
    ...
    if (len > 0) {
        ...
        if (js_get_fast_array(ctx, obj, &arrp, &count)) {
            /* see js_array_indexOf: the needle's tag is loop-invariant, and an
               int needle against an int element is one compare. SameValueZero
               agrees with == for two int32s (neither can be NaN or -0). */
            if (JS_VALUE_GET_TAG(argv[0]) == JS_TAG_INT) {
                int v = JS_VALUE_GET_INT(argv[0]);
                for (; n < count; n++) {
                    JSValueConst e = arrp[n];
                    if (JS_VALUE_GET_TAG(e) == JS_TAG_INT) {
                        if (JS_VALUE_GET_INT(e) == v) {
                            res = TRUE;
                            goto done;
                        }
            ...
            if (js_slow_array_scan_start(ctx, p)) {
                /* slow array: scan the OCCUPIED entries only. The guard
                   proved the prototype chain has no indexed properties, so
                   a hole is genuinely `undefined`: SameValueZero(undefined,
                   undefined) matches, and any other needle can only match
                   an own element. Spec: Array.prototype.includes step 7
                   (Get(O,k) with SameValueZero), holes handled explicitly
                   for the `undefined` needle below. An accessor element is
                   only met mid-walk: the scan is stateless, so falling
                   through to the generic loop then is invisible. */
                JSShape *sh = p->shape;
                JSShapeProperty *prs = get_shape_prop(sh);
                int64_t i;
                int eligible = 1;
                if (JS_IsUndefined(argv[0])) {
                    /* includes(undefined) matches every hole: TRUE iff an
                       own undefined >= n exists or a hole >= n exists
                       (i.e. fewer than len-n occupied entries in range). */
                    int64_t occ = 0;
                    for(i = 0; eligible && i < sh->prop_count; i++, prs++) {
                        JSAtom a = prs->atom;
                        if (a == JS_ATOM_NULL || !__JS_AtomIsTaggedInt(a))
                            continue;
                        ...
                        if ((int64_t)__JS_AtomToUInt32(a) < n)
                            continue;
                        occ++;
                        if (JS_IsUndefined(p->prop[i].u.value)) {
                            res = TRUE;
                            break;
                        }
                    }
                    if (eligible && !res && occ < len - n)
                        res = TRUE;
                } else if (JS_VALUE_GET_TAG(argv[0]) == JS_TAG_INT) {
                    int v = JS_VALUE_GET_INT(argv[0]);
                    for(i = 0; eligible && i < sh->prop_count; i++, prs++) {
                        JSAtom a = prs->atom;
                        JSValueConst e;
                    ...
                    for(i = 0; eligible && i < sh->prop_count; i++, prs++) {
                        ...
                        if ((int64_t)__JS_AtomToUInt32(a) < n)
                            continue;
                        if (js_strict_eq2(ctx, argv[0], p->prop[i].u.value,
                                          JS_EQ_SAME_VALUE_ZERO)) {
                            res = TRUE;
                            goto done;
                        }
                ...
                if (eligible)
                    goto done;
                /* bailed mid-walk on an accessor: restart generically; the
                   partial result is discarded so the generic loop recomputes
                   everything, getters included, exactly like node */
                res = FALSE;
            }
        }
        for (; n < len; n++) {
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    if (len > 0) {
        ...
        if (js_get_fast_array(ctx, obj, &arrp, &count)) {
            /* The needle's tag is loop-invariant. js_strict_eq2() is too large
               for clang to inline, so searching for an int in a dense numeric
               array pays a real call per element for what is one compare.
               Elements of any other tag still go through it -- a float64
               element can equal an int needle (1 === 1.0). */
            if (JS_VALUE_GET_TAG(argv[0]) == JS_TAG_INT) {
                int v = JS_VALUE_GET_INT(argv[0]);
                for (; n < count; n++) {
                    JSValueConst e = arrp[n];
            ...
            JSObject *p = JS_VALUE_GET_OBJ(obj);
            if (js_slow_array_scan_start(ctx, p)) {
                /* slow array: scan the OCCUPIED entries only; with no indexed
                   properties on the prototype chain a miss costs O(occupied)
                   instead of O(len) property walks. The full entry list is
                   walked (shape order is not index order) tracking the lowest
                   matching index >= n -- spec Array.prototype.indexOf step 5
                   (k ascending, StrictEquals). A `undefined` needle needs the
                   FIRST hole's index and falls through to the generic loop
                   below, which is exact. StrictEquals(NaN, *) is false, so a
                   NaN needle simply finds nothing, like the generic loop.
                   An accessor element met mid-walk falls through too (the
                   scan is stateless, and any match already found is real --
                   only non-accessor entries were compared). */
                JSShape *sh = p->shape;
                JSShapeProperty *prs = get_shape_prop(sh);
                int64_t i;
                int eligible = 1;
                if (!JS_IsUndefined(argv[0])) {
                    if (JS_VALUE_GET_TAG(argv[0]) == JS_TAG_INT) {
                        /* StrictEquals agrees with == for two int32s */
                        int v = JS_VALUE_GET_INT(argv[0]);
                        for(i = 0; eligible && i < sh->prop_count;
                            i++, prs++) {
                            JSAtom a = prs->atom;
                            JSValueConst e;
                            uint32_t idx;
                    ...
                    if (eligible)
                        goto done;
                    /* accessor met mid-walk: discard the partial match and
                       restart generically (the generic loop assumes res==-1) */
                    res = -1;
                }
            }
        }
                                    ...
                                    int argc, JSValueConst *argv)
{
    ...
    if (len > 0) {
        ...
        /* fast arrays: read the element directly, and specialise on the
           needle's tag the same way js_array_indexOf does. Every index in
           0..n is below count here, so the scan is complete on its own. */
        if (js_get_fast_array(ctx, obj, &arrp, &count) && n < count) {
            if (JS_VALUE_GET_TAG(argv[0]) == JS_TAG_INT) {
                int v = JS_VALUE_GET_INT(argv[0]);
                for (; n >= 0; n--) {
                    JSValueConst e = arrp[n];
        ...
        }
        {
            ...
            if (js_slow_array_scan_start(ctx, p) &&
                ...
                /* slow array: occupied entries only, tracking the HIGHEST
                   matching index <= n (spec Array.prototype.lastIndexOf step
                   6: k descending). A `undefined` needle needs the LAST hole
                   and keeps the generic loop below, which is exact. An
                   accessor element met mid-walk falls through (stateless
                   scan; matches found so far are real). */
                JSShape *sh = p->shape;
                JSShapeProperty *prs = get_shape_prop(sh);
                int64_t i;
                int eligible = 1;
                if (JS_VALUE_GET_TAG(argv[0]) == JS_TAG_INT) {
                    ...
                    for(i = 0; eligible && i < sh->prop_count; i++, prs++) {
                        ...
                        e = p->prop[i].u.value;
                        if (JS_VALUE_GET_TAG(e) == JS_TAG_INT) {
                            if (JS_VALUE_GET_INT(e) == v && idx > res)
                                res = idx;
                        } else if (js_strict_eq2(ctx, argv[0], e,
                                                 JS_EQ_STRICT) &&
                                   idx > res) {
                             ...
                             int argc, JSValueConst *argv, int toLocaleString)
{
    ...
    /* Starting from 0 makes every join pay an initial 0-size alloc plus a
       realloc before it holds anything. Seed it instead -- but keep the seed
       SMALL: a large seed measurably hurts big joins (it front-loads a
       large-block allocation, and for a wide result it makes the first
       string_buffer_widen() realloc the whole seed instead of 16 bytes). */
    string_buffer_init(ctx, b, (int)min_int64(max_int64(n * 2, 16), 128));

    for(i = 0; i < n; i++) {
        /* separator first for i > 0; peeled out of the loop body below */
        if (i > 0) {
            if (c >= 0) {
                            ...
                            int argc, JSValueConst *argv, int shift)
{
    ...
    if (len > 0) {
        ...
        if (js_get_fast_array(ctx, obj, &arrp, &count32) && count32 == len) {
            ...
            if (shift) {
                int r = 0;
                JSObject *ap = JS_VALUE_GET_OBJ(obj);
                res = JS_GetPropertyInt64(ctx, obj, 0);
                if (JS_IsException(res))
                    goto exception;
                if (ap->class_id == JS_CLASS_ARRAY && !ap->fast_array)
                    r = js_slow_array_shift_unshift(ctx, ap, len, -1);
                if (r < 0)
                    goto exception;
                if (!r && JS_CopySubArray(ctx, obj, 0, 1, len - 1, +1))
                    goto exception;
            } else {
                             ...
                             int argc, JSValueConst *argv, int unshift)
{
    ...
    if (unshift && argc > 0) {
        ...
        JSObject *ap = JS_VALUE_GET_OBJ(obj);
        if (ap->class_id == JS_CLASS_ARRAY && !ap->fast_array)
            r = js_slow_array_shift_unshift(ctx, ap, len, argc);
        if (r < 0)
            goto exception;
        if (!r && JS_CopySubArray(ctx, obj, argc, 0, len, -1))
            goto exception;
        from = 0;
    }
    for(i = 0; i < argc; i++) {
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    if (JS_VALUE_GET_TAG(obj) == JS_TAG_OBJECT &&
        ...
        !JS_VALUE_GET_OBJ(obj)->fast_array) {
        /* dense slow array: rebuild fast and take the memmove path below */
        js_slow_array_try_convert_fast(ctx, JS_VALUE_GET_OBJ(obj));
    }
    if (js_get_fast_array(ctx, obj, &arrp, &count32) && count32 == len) {
        uint32_t ll, hh;
    ...
    if (len > 1 && len <= UINT32_MAX && JS_VALUE_GET_TAG(obj) == JS_TAG_OBJECT) {
        JSObject *p = JS_VALUE_GET_OBJ(obj);
        if (js_slow_array_scan_ok(ctx, p)) {
            /* slow array with holes: mirror the OCCUPIED entries only.
               Spec: Array.prototype.reverse steps 5-8 move each present
               element to len-1-k with Set/Delete; with no indexed proto
               props and default attributes a hole at k mirrors to a hole at
               len-1-k, so swapping/moving the occupied entries directly is
               unobservable. Elements with non-default attributes keep the
               generic loop below (exact partial-mutation/throw order).
               Adds run before deletes so no property-array compaction can
               invalidate the recorded slot positions. */
            JSShape *sh = p->shape;
            JSShapeProperty *prs = get_shape_prop(sh);
            JSSlowArrEnt *ents, *moves = NULL;
            int occ = 0, k, nmoves = 0, ncap = 0, asc = 1, r = 1;
            int64_t i;
            for(i = 0; i < sh->prop_count; i++, prs++) {
                if (prs->atom != JS_ATOM_NULL &&
                    __JS_AtomIsTaggedInt(prs->atom)) {
                    if (prs->flags != JS_PROP_C_W_E)
                        goto generic_reverse;
                    occ++;
                }
            }
            if (occ == 0)
                return obj; /* all holes: nothing to move */
            ents = js_malloc(ctx, sizeof(*ents) * occ);
            if (!ents)
                goto exception;
            k = 0;
            prs = get_shape_prop(sh);
            for(i = 0; i < sh->prop_count; i++, prs++) {
                ...
                if (a == JS_ATOM_NULL || !__JS_AtomIsTaggedInt(a))
                    continue;
                ents[k].idx = __JS_AtomToUInt32(a);
                ents[k].pos = (uint32_t)i;
                ents[k].val = JS_UNDEFINED; /* swap slot, not an owner */
                if ((int64_t)ents[k].idx >= len) {
                    /* defense in depth: the generic loop only ever touches
                       indices < len, so an element at/after `length` must
                       not be moved either. Unreachable via engine paths. */
                    js_free(ctx, ents);
                    goto generic_reverse;
                }
                if (k > 0 && ents[k - 1].idx >= ents[k].idx)
                    asc = 0;
                k++;
            }
            if (!asc)
                js_slow_arr_sort(ents, occ);
            /* mirror destinations must stay tagged-int atoms: indices above
               INT32_MAX live in the shape as string atoms and
               __JS_AtomFromUInt32 would wrap them (huge-length arrays like
               new Array(3e9) with a couple of low-index elements) */
            if ((int64_t)len - 1 - ents[0].idx > INT32_MAX) {
                js_free(ctx, ents);
                goto generic_reverse;
            }
            /* pass 1: pairs whose smaller member is occupied */
            for(k = 0; k < occ; k++) {
                int64_t m = (int64_t)len - 1 - ents[k].idx;
                int kk;
                if (m < ents[k].idx)
                    break; /* pairs are handled from their smaller side */
                if (m == ents[k].idx)
                    continue; /* middle of an odd length mirrors itself */
                kk = js_slow_arr_find(ents, occ, m, occ - 1 - k);
                if (kk >= 0) {
                    /* symmetric pair: swap the two property values */
                    JSProperty *pa = &p->prop[ents[k].pos];
                    JSProperty *pb = &p->prop[ents[kk].pos];
                    JSValue t = pa->u.value;
                    pa->u.value = pb->u.value;
                    pb->u.value = t;
                } else {
                    /* mirror is a hole: value moves up, source becomes a hole */
                    if (nmoves == ncap) {
                        int ncap2 = ncap < 8 ? 8 : ncap * 2;
                        JSSlowArrEnt *nmv =
                            js_realloc(ctx, moves, sizeof(*nmv) * ncap2);
                        if (!nmv) {
                            r = -1;
                            break;
                        }
                    ...
                    moves[nmoves].idx = ents[k].idx;
                    moves[nmoves].pos = 0;
                    moves[nmoves].val =
                        JS_DupValue(ctx, p->prop[ents[k].pos].u.value);
                    nmoves++;
                }
            }
            /* pass 2: occupied second-half entries whose mirror is a hole
               (their pair has no occupied smaller side) */
            if (r > 0) {
                for(k = 0; k < occ; k++) {
                    int64_t m = (int64_t)len - 1 - ents[k].idx;
                    if (m >= ents[k].idx)
                        continue; /* first half / middle: handled above */
                    if (js_slow_arr_find(ents, occ, m, occ - 1 - k) < 0) {
                        if (nmoves == ncap) {
                            int ncap2 = ncap < 8 ? 8 : ncap * 2;
                            JSSlowArrEnt *nmv =
                                js_realloc(ctx, moves,
                                           sizeof(*nmv) * ncap2);
                            if (!nmv) {
                                r = -1;
                                break;
                        ...
                        moves[nmoves].val =
                            JS_DupValue(ctx,
                                        p->prop[ents[k].pos].u.value);
                        nmoves++;
                    }
                }
            }
            /* apply the one-sided moves: add at the (snapshot) hole, then
               delete the source */
            for(k = 0; r > 0 && k < nmoves; k++) {
                int64_t m = (int64_t)len - 1 - moves[k].idx;
                JSProperty *pr;
                pr = add_property(ctx, p, __JS_AtomFromUInt32((uint32_t)m),
                                  JS_PROP_C_W_E);
                if (!pr) {
                    r = -1;
                    break;
                }
                pr->u.value = moves[k].val;
                moves[k].val = JS_UNDEFINED;
            }
            for(k = 0; k < nmoves; k++) {
                if (!JS_IsUndefined(moves[k].val))
                    JS_FreeValue(ctx, moves[k].val);
                else
                    delete_property(ctx, p,
                                    __JS_AtomFromUInt32(moves[k].idx));
            }
            js_free(ctx, moves);
            js_free(ctx, ents);
            if (r < 0)
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    /* a dense slow array is rebuilt fast so the bulk-dup fast case below
       applies (best effort; species resolution may still take the slow one) */
    if (JS_VALUE_GET_TAG(obj) == JS_TAG_OBJECT &&
        JS_VALUE_GET_OBJ(obj)->class_id == JS_CLASS_ARRAY &&
        !JS_VALUE_GET_OBJ(obj)->fast_array) {
        js_slow_array_try_convert_fast(ctx, JS_VALUE_GET_OBJ(obj));
    ...
    if (JS_IsUndefined(ctor) &&
        ...
        /* slow source: define just the occupied entries in [start, final)
           (spec slice steps 8-10: HasProperty+Get per index, CreateData-
           Property only when present; skipping holes needs the prototype
           guard inside js_slow_array_fill_range). Holes stay holes. */
        if (JS_VALUE_GET_TAG(obj) == JS_TAG_OBJECT && count > 0) {
            JSObject *sp = JS_VALUE_GET_OBJ(obj);
            if (sp->class_id == JS_CLASS_ARRAY && !sp->fast_array) {
                filled = js_slow_array_fill_range(ctx, sp, arr,
                                                  start, final, 0, TRUE);
                if (filled < 0)
                    goto exception;
            }
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    /* x6: write back EVERY slot unconditionally (spec: Set for all
       k < j). The previous fast path skipped the Set when a value
       sorted back to its original index, assuming it was still sitting
       there -- wrong whenever the slot was not an own element (a hole
       resolved through the prototype was never materialized) or when
       the comparator mutated the array mid-sort (deleted the slot).
       Both now take the ordinary Set path, exactly like node. */
    while (n < pos) {
        if (array[n].str)
                                      ...
                                      BOOL *pdone, int magic)
{
    ...
    if (p->class_id >= JS_CLASS_UINT8C_ARRAY &&
        ...
        len = p->u.array.count;
    } else if (p->class_id == JS_CLASS_ARRAY && p->fast_array &&
               JS_VALUE_GET_TAG(p->prop[0].u.value) == JS_TAG_INT) {
        /* fast array: the length is prop[0] itself -- js_get_length32()
           would run a full property get that resolves the 'length' atom
           to this same slot, then ToUint32 the same INT */
        len = JS_VALUE_GET_INT(p->prop[0].u.value);
    } else {
    ...
    *pdone = FALSE;
    if (it->kind == JS_ITERATOR_KIND_KEY) {
        ...
        if (p->class_id == JS_CLASS_ARRAY && p->fast_array &&
            idx < p->u.array.count) {
            /* dense own slot: identical to the JS_GetPropertyUint32()
               below (JS_GetPropertyValue's array fast path is this
               same slot read); indexes at or beyond 'count' keep the
               generic read so prototype-chain holes behave the same */
...
/* Fast path for OP_for_of_next: serve one step of a for-of loop inline
   when all of the following hold:
   - the iterator object (sp[offset]) is the built-in array iterator
     (JS_CLASS_ARRAY_ITERATOR),
   - the 'next' method (sp[offset+1]) captured by OP_for_of_start is the
     built-in %ArrayIteratorPrototype%.next C function, so no user code
     can interpose on this step,
   - the iteration kind is "value",
   - the iterated object currently has fast (dense) array elements
     (JS_CLASS_ARRAY with fast_array, or a typed array).
   The step mirrors js_array_iterator_next() above exactly:
   - for fast arrays every index below u.array.count holds a present
     value (fast arrays are dense: convert_fast_array_to_array() zeroes
     count when leaving that mode and set_array_length() shrinks count
     when the length shrinks), so duplicating u.array.u.values[idx]
     yields the same value as the JS_GetPropertyUint32() the builtin
     performs; 'length' is re-read from prop[0] only to detect the end
     of the iteration, exactly like the builtin's js_get_length32();
   - for typed arrays the length is u.array.count after the same
     typed_array_is_oob() check and the element is read with the same
     JS_GetPropertyUint32() call as the builtin;
   - out of bounds or an already exhausted iterator performs the same
     state transition as the builtin (it->obj released and set to
     undefined, it->idx unchanged) so subsequent .next() calls on a
     user-held reference behave identically;
   - anything else (slow/sparse arrays, class mismatch, kind != value,
     tampered 'next', string/arguments iterators, ...) returns FALSE so
   the generic iterator protocol runs.
   When returning TRUE, *pdone is 0 with *pvalue set, 1 with
   *pvalue = JS_UNDEFINED (iteration finished), or -1 with an exception
   pending (*pvalue is JS_EXCEPTION). The caller must release the
   iterator slot (sp[offset]) on *pdone != 0, as the generic path does. */
static BOOL js_for_of_next_fast_array(JSContext *ctx, JSValue *sp,
                                      int offset, JSValue *pvalue,
                                      int *pdone)
{
    JSValueConst iter, arr;
    JSArrayIteratorData *it;
    JSCFunctionType ft;
    JSObject *p, *arrp;
    uint32_t idx;
    ...
    iter = sp[offset];
    if (unlikely(JS_VALUE_GET_TAG(iter) != JS_TAG_OBJECT))
        return FALSE;
    p = JS_VALUE_GET_OBJ(iter);
    if (unlikely(p->class_id != JS_CLASS_ARRAY_ITERATOR))
        return FALSE;
    it = p->u.array_iterator_data;
    if (unlikely(!it))
        return FALSE;
    /* the 'next' method captured at OP_for_of_start must be the built-in
       one; then this step cannot run user code */
    ft.iterator_next = js_array_iterator_next;
    if (unlikely(!JS_IsCFunction(ctx, sp[offset + 1], ft.generic, 0)))
        return FALSE;
    if (unlikely(it->kind != JS_ITERATOR_KIND_VALUE))
        return FALSE;
    arr = it->obj;
    if (unlikely(JS_VALUE_GET_TAG(arr) != JS_TAG_OBJECT)) {
        if (likely(JS_IsUndefined(arr))) {
            /* exhausted iterator: same as 'done:' in the builtin */
            *pvalue = JS_UNDEFINED;
            *pdone = 1;
            return TRUE;
        }
    ...
    arrp = JS_VALUE_GET_OBJ(arr);
    if (arrp->class_id == JS_CLASS_ARRAY) {
        uint32_t len;
        JSValueConst len_val;

        if (unlikely(!arrp->fast_array))
            return FALSE;
        idx = it->idx;
        if (likely(idx < arrp->u.array.count)) {
            /* present element: fast arrays are dense below count */
            it->idx = idx + 1;
            *pvalue = JS_DupValue(ctx, arrp->u.array.u.values[idx]);
            *pdone = 0;
            return TRUE;
        }
        /* length may exceed count (e.g. a.length = 10): only the 'length'
           property (always prop[0] of a JS_CLASS_ARRAY) tells whether we
           are done; indices in [count, length) must go through the
           generic path (prototype lookup) */
        len_val = arrp->prop[0].u.value;
        switch(JS_VALUE_GET_NORM_TAG(len_val)) {
        case JS_TAG_INT:
            len = (uint32_t)JS_VALUE_GET_INT(len_val);
            break;
        case JS_TAG_FLOAT64:
            len = (uint32_t)JS_VALUE_GET_FLOAT64(len_val);
            break;
        default:
            /* cannot happen: array 'length' is a uint32 */
            return FALSE;
        }
        if (unlikely(idx < len))
            return FALSE; /* hole/prototype region */
        goto done_iter;
    } else if (arrp->class_id >= JS_CLASS_UINT8C_ARRAY &&
               arrp->class_id <= JS_CLASS_FLOAT64_ARRAY) {
        if (unlikely(typed_array_is_oob(arrp)))
            return FALSE; /* the generic path raises the TypeError */
        idx = it->idx;
        if (unlikely(idx >= arrp->u.array.count))
            goto done_iter;
        it->idx = idx + 1;
        /* same reads as JS_GetPropertyValue() for typed arrays */
        switch(arrp->class_id) {
        case JS_CLASS_INT8_ARRAY:
            *pvalue = JS_NewInt32(ctx, arrp->u.array.u.int8_ptr[idx]);
            break;
        case JS_CLASS_UINT8C_ARRAY:
        case JS_CLASS_UINT8_ARRAY:
            *pvalue = JS_NewInt32(ctx, arrp->u.array.u.uint8_ptr[idx]);
            break;
        case JS_CLASS_INT16_ARRAY:
            *pvalue = JS_NewInt32(ctx, arrp->u.array.u.int16_ptr[idx]);
            break;
        case JS_CLASS_UINT16_ARRAY:
            *pvalue = JS_NewInt32(ctx, arrp->u.array.u.uint16_ptr[idx]);
            break;
        case JS_CLASS_INT32_ARRAY:
            *pvalue = JS_NewInt32(ctx, arrp->u.array.u.int32_ptr[idx]);
            break;
        case JS_CLASS_UINT32_ARRAY:
            *pvalue = JS_NewUint32(ctx, arrp->u.array.u.uint32_ptr[idx]);
            break;
        case JS_CLASS_BIG_INT64_ARRAY:
            *pvalue = JS_NewBigInt64(ctx, arrp->u.array.u.int64_ptr[idx]);
            break;
        case JS_CLASS_BIG_UINT64_ARRAY:
            *pvalue = JS_NewBigUint64(ctx, arrp->u.array.u.uint64_ptr[idx]);
            break;
        case JS_CLASS_FLOAT16_ARRAY:
            *pvalue = __JS_NewFloat64(ctx, fromfp16(arrp->u.array.u.fp16_ptr[idx]));
            break;
        case JS_CLASS_FLOAT32_ARRAY:
            *pvalue = __JS_NewFloat64(ctx, arrp->u.array.u.float_ptr[idx]);
            break;
        case JS_CLASS_FLOAT64_ARRAY:
            *pvalue = __JS_NewFloat64(ctx, arrp->u.array.u.double_ptr[idx]);
            break;
        default:
        ...
        if (unlikely(JS_IsException(*pvalue))) {
            /* allocation failure: same state the builtin would leave */
            *pdone = -1;
            return TRUE;
        }
        *pdone = 0;
    ...
    return FALSE;
 done_iter:
    /* same state transition as the builtin on idx >= len */
    JS_FreeValue(ctx, it->obj);
    it->obj = JS_UNDEFINED;
    *pvalue = JS_UNDEFINED;
    *pdone = 1;
