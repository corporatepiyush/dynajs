    ...
    while (pos < bc_len) {
        ...
        if (op == OP_ext) {
            ...
            /* loc2: two u16 local indices, indexed by the interpreter with no
               re-check (see OP2_READ_LOC_LOC) -- except the ERM shard, whose
               ops carry a small opcode parameter in the first slot
               (using_add: dispose hint 0..2; using_dispose: flags 0..3) and a
               reserved zero in the second. Both are read by the handlers as
               parameters, never as var_buf indices. */
            if (opcode_info2[op2].fmt == OP_FMT_loc2 &&
                (op2 == OP2_using_add || op2 == OP2_using_dispose)) {
                if (get_u16(bc_buf + pos + 2) > 3 ||
                    get_u16(bc_buf + pos + 4) != 0)
                    goto invalid_operand;
            } else if (opcode_info2[op2].fmt == OP_FMT_loc2 &&
                (get_u16(bc_buf + pos + 2) >= b->var_count ||
                 get_u16(bc_buf + pos + 4) >= b->var_count))
                goto invalid_operand;
            /* label_const8 (BRANCH shard): the u8 at pos+6 indexes cpool with
               no runtime re-check (mirrors OP_FMT_const8) */
            if (opcode_info2[op2].fmt == OP_FMT_label_const8 &&
                bc_buf[pos + 6] >= (uint32_t)b->cpool_count)
                goto invalid_operand;
            /* label_var_ref_atom (BRANCH shard): the u16 at pos+6 indexes the
               closure's var_refs (mirrors OP_FMT_var_ref) and the u32 at
               pos+8 is a raw ATOM to fix up through the blob's atom table
               (mirrors OP_FMT_atom; must happen before execution) */
            if (opcode_info2[op2].fmt == OP_FMT_label_var_ref_atom) {
                if (get_u16(bc_buf + pos + 6) >=
                    (uint32_t)b->closure_var_count)
                    goto invalid_operand;
                ...
                if (!s->is_rom_data) {
                    ...
                    /* ROM data: atoms are already direct; just validate */
                    if (bc_idx_to_atom(s, &atom, idx)) {
                        b->byte_code_len = pos;
                        return -1;
                    }
                ...
                /* CALL shard (fused method calls): the method-name atom at
                   pos+2 fixes up through the blob's atom table exactly like
                   bank-1 atom operands; the indexed operands (form byte,
                   local/arg idx, cpool index) are validated here because the
                   interpreter never re-checks them. */
                if (!s->is_rom_data) {
                ...
                case OP_FMT_atom_loc:
                    /* form: 0=get_loc 1=get_loc_check 2=get_arg 3=push_i16 */
                    if (bc_buf[pos + 6] > 3)
                        goto invalid_operand;
                    if (bc_buf[pos + 6] == 2) {
                        if (get_u16(bc_buf + pos + 7) >=
                            (uint32_t)b->arg_count)
                            goto invalid_operand;
                    } else if (bc_buf[pos + 6] <= 1) {
                        if (get_u16(bc_buf + pos + 7) >=
                            (uint32_t)b->var_count)
                            goto invalid_operand;
                    }
                    /* form 3 is a raw i16 immediate: nothing to check */
                    break;
                case OP_FMT_atom_i8:
                    /* form: 0=push_i8 1=push_const8 */
                    if (bc_buf[pos + 6] > 1)
                        goto invalid_operand;
                    if (bc_buf[pos + 6] == 1 &&
                        bc_buf[pos + 7] >= (uint32_t)b->cpool_count)
                        goto invalid_operand;
                    break;
                case OP_FMT_atom_const:
                    if (get_u32(bc_buf + pos + 6) >=
                        (uint32_t)b->cpool_count)
                        goto invalid_operand;
                    break;
                default:
                    break;
...
#define CHECK_LABEL_TARGET(operand_off, val)                                  \
    ...
    while (pos < bc_len) {
        ...
        if (op == OP_ext) {
            if (bc_len - pos < 2 || bc[pos + 1] >= OP2_COUNT)
                goto fail;
            /* BRANCH shard: the u32 at pos+2 is an OP_if_false-style relative
               label offset — same instruction-start requirement as bank-1 */
...
static int JS_ValidateFunctionStackSize(BCReaderState *s, JSFunctionBytecode *b)
{
    ...
    while (ss->pc_stack_len > 0) {
        ...
        switch(op) {
        case OP_ext:
            /* bank-2 branch ops (BRANCH shard): the label target must be
               explored at the same stack level (net 0; pass 1 already
               bounded op2 and the encoding) */
            if (bc[pos + 1] == OP2_streq_const_if_false ||
                bc[pos + 1] == OP2_streq_varprop_if_false) {
...
static JSValue JS_ReadModule(BCReaderState *s)
{
    ...
    return obj;
 fail:
    /* Two references must be released here: 'm' holds the reference
     * js_new_module_def created (the loaded_modules base reference) and 'obj'
     * holds the reference JS_NewModuleValue took (JS_DupValue). Freeing only
     * 'm' left the module alive with ref_count==1 that js_free_modules()'s
     * single base-reference release could never drop, so a rejected module
     * blob leaked the JSModuleDef AND every GC object only it referenced
     * (req_module_entries[].attributes already read before the failure);
     * JS_FreeRuntime then asserted on the non-empty gc_obj_list (fuzz_bytecode
     * finding F-1). obj is JS_UNDEFINED (freed as a no-op) until JS_NewModuleValue. */
    if (m)
...
static JSValue JS_ReadObjectTag(BCReaderState *s)
{
    ...
    for(i = 0; i < prop_count; i++) {
        if (bc_get_atom(s, &atom))
            goto fail;
        /* bc_get_atom maps stream value 0 to JS_ATOM_NULL, but a property
         * stored under a NULL atom is invisible to the GC: mark_children
         * and gc_decref skip prs->atom == JS_ATOM_NULL. A hostile blob
         * pairs such a key with an OBJECT_REFERENCE back-ref to this very
         * object (only readable under JS_READ_OBJ_REFERENCE), so a read
         * that fails later leaves 'obj' alive with its own reference as
         * the last one -- a self-cycle refcounting cannot drop and the
         * cycle collector cannot see. JS_FreeRuntime then aborted on
         * assert(list_empty(&rt->gc_obj_list)) (fuzz finding F-1, bc2
         * dedupe group). The writer never emits a NULL-atom property key
         * (live JS objects cannot own one), so every such blob is invalid
         * bytecode and must be refused. JS_FreeAtom(JS_ATOM_NULL) is a
         * no-op, so the shared fail path needs no extra cleanup here. */
