...
static void *dyn_ml_r_kmeans(dyn_de_t *r)
{
    ...
    if (!dyn_de_ok(r) || dyn_ml_de_dims(r, &n, k, nf) < 0)
        return NULL;
...
static void *dyn_ml_r_nb(dyn_de_t *r)
{
    ...
    if (!dyn_de_ok(r) || dyn_ml_de_dims(r, &n, ncl, nf) < 0)
        return NULL;
...
static void *dyn_ml_r_svm(dyn_de_t *r)
{
    ...
    if (!dyn_de_ok(r) || !dyn_ml_fits(r, ncl, sizeof(double)) ||
        !dyn_ml_fits(r, nbin, 1))
        return NULL;
    classes = dyn_ml_de_doubles(r, (size_t)ncl);
    if (!classes)
        return NULL;
...
static void *dyn_ml_r_gmm(dyn_de_t *r)
{
    ...
    if (!dyn_de_ok(r) || dyn_ml_de_dims(r, &n, k, cols) < 0)
        return NULL;
...
static void *dyn_ml_r_forest(dyn_de_t *r)
{
    ...
    if (!dyn_de_ok(r) || !dyn_ml_fits(r, n_trees, 8))
        return NULL;
    ...
    for (t = 0; t < m->n_trees; t++) {
        ...
        dyn_tnode_t *nodes;
        if (!dyn_de_ok(r) || !dyn_ml_fits(r, nn, DYN_ML_NODE_BYTES))
            goto fail;
                           ...
                           int argc, JSValueConst *argv, int magic)
{
    ...
    f = fopen(path, "wb");
    if (!f) {
        JSValue e = JS_ThrowTypeError(ctx, "cannot open '%s' for writing", path);
        /* borrowed from a Path: nothing to free */
        free(buf);
        return e;
    }
