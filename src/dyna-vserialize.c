 ...
 * losing every byte), and an ArrayBuffer comes back as an ArrayBuffer.
 * A DataView clones as a Uint8Array over its bytes.
 * Returns JS_UNDEFINED when `v` is not byte-backed. */
static JSValue vs_clone_bytes(JSContext *ctx, JSValueConst v)
{
    static const struct { const char *name; JSTypedArrayEnum t; } ta_map[] = {
        { "Uint8ClampedArray", JS_TYPED_ARRAY_UINT8C },
        { "Int8Array", JS_TYPED_ARRAY_INT8 },
        { "Uint8Array", JS_TYPED_ARRAY_UINT8 },
        { "Int16Array", JS_TYPED_ARRAY_INT16 },
        { "Uint16Array", JS_TYPED_ARRAY_UINT16 },
        { "Int32Array", JS_TYPED_ARRAY_INT32 },
        { "Uint32Array", JS_TYPED_ARRAY_UINT32 },
        { "BigInt64Array", JS_TYPED_ARRAY_BIG_INT64 },
        { "BigUint64Array", JS_TYPED_ARRAY_BIG_UINT64 },
        { "Float16Array", JS_TYPED_ARRAY_FLOAT16 },
        { "Float32Array", JS_TYPED_ARRAY_FLOAT32 },
        { "Float64Array", JS_TYPED_ARRAY_FLOAT64 },
    };
    size_t off, len, bpe, ab;
    JSValue buf, copy, ctor, global, r;
    int i, matched = -1;
    ...
    /* identify the view's constructor among the typed arrays */
    ctor = JS_GetPropertyStr(ctx, v, "constructor");
    ...
    if (!JS_IsException(global)) {
        for (i = 0; i < (int)countof(ta_map) && matched < 0; i++) {
            JSValue g = JS_GetPropertyStr(ctx, global, ta_map[i].name);
            if (!JS_IsException(g)) {
                if (JS_SameValue(ctx, ctor, g) == 1)
                    matched = ta_map[i].t;
                JS_FreeValue(ctx, g);
            }
        }
...

    {
        ...
        r = JS_NewTypedArray(ctx, 3, ta,
                             matched >= 0 ? (JSTypedArrayEnum)matched
