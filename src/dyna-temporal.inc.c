...
/* The exact epoch-day span of the supported year range, as day numbers. The
   old round +/-100000000 bound was tighter than the year range on BOTH ends
   (year-min Jan 1 is day -100000109, year-max Dec 31 is +100000109), so
   dateFromEpochDay refused days for PlainDates the constructor could build,
   and add()/subtract() refused day shifts that stay inside valid years. */
#define TP_EPOCH_DAY_MIN (tp_days_from_civil(TP_MIN_YEAR, 1, 1))    /* -100000109 */
#define TP_EPOCH_DAY_MAX (tp_days_from_civil(TP_MAX_YEAR, 12, 31))  /*  100000109 */
...
static JSValue tp_new_date(JSContext *ctx, JSValueConst new_target,
                           int64_t y, int m, int d)
{
    tp_date_t *p;
    JSValue obj;
    ...
    p->y = y; p->m = m; p->d = d;
    obj = dyn_plain_wrap(ctx, new_target, dyn_pdate_class_id, p, dyn_pdate_free);
    return obj;
}

static JSValue tp_new_dur3(JSContext *ctx, JSValueConst new_target,
                           int64_t months, int64_t days, int64_t ms_part)
{
    ...
    return dyn_plain_wrap(ctx, new_target, dyn_dur_class_id, p, dyn_dur_free);
}

static JSValue tp_new_dur(JSContext *ctx, int64_t months, int64_t days)
{
    return tp_new_dur3(ctx, JS_UNDEFINED, months, days, 0);
}
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    if (JS_ToInt64(ctx, &y, argv[0]) || JS_ToInt32(ctx, &m, argv[1])
        ...
        return JS_EXCEPTION;
    return tp_new_date(ctx, new_target, y, m, d);
}
                        ...
                        int64_t days)
{
    ...
    if (days) {
        ...
        int mm, dd;
        if (ed < TP_EPOCH_DAY_MIN || ed > TP_EPOCH_DAY_MAX)
            return JS_ThrowRangeError(ctx, "PlainDate: the result leaves the "
                                           "supported range");
        tp_civil_from_days(ed, &y, &mm, &dd);
        m = mm; d = dd;
    }
    return tp_new_date(ctx, JS_UNDEFINED, y, m, d);
}
...
#undef FIELD
    ...
    return tp_new_dur3(ctx, new_target, years * 12 + months, weeks * 7 + days,
...
static JSValue dyn_dur_get(JSContext *ctx, JSValueConst this_val, int magic)
{
    ...
    if (!p)
        return JS_EXCEPTION;
    switch (magic) {
    case 0: return JS_NewInt64(ctx, p->months % 12); /* the REST after years:
        the reference documents years/months as "months / 12 and the rest";
        returning the folded total made years*12 + months != the value the
        Duration was built from */
...
/* One integer, so a PlainTime is a register. If this grows, the claim in
   the module header that comparison is an integer compare stops being true. */
...
static JSValue tp_new_time(JSContext *ctx, JSValueConst new_target, int32_t ms)
{
    ...
    if (!p)
        return JS_ThrowOutOfMemory(ctx);
    p->ms = ms;
    return dyn_plain_wrap(ctx, new_target, dyn_ptime_class_id, p, dyn_ptime_free);
}
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    return tp_new_time(ctx, new_target, ((f[0] * 60 + f[1]) * 60 + f[2]) * 1000 + f[3]);
}
                             ...
                             int argc, JSValueConst *argv, int magic)
{
    ...
    if (total < 0)
        total += TP_DAY_MS;
    return tp_new_time(ctx, JS_UNDEFINED, (int32_t)total);
}
...
#undef TWO
    ...
    JS_FreeCString(ctx, s);
    return tp_new_time(ctx, JS_UNDEFINED, ((f[0] * 60 + f[1]) * 60 + f[2]) * 1000 + f[3]);
bad:
...
static JSValue tp_new_dt(JSContext *ctx, JSValueConst new_target,
                         int64_t y, int m, int d, int32_t ms)
{
    tp_dt_t *p;
    ...
    return dyn_plain_wrap(ctx, new_target, dyn_pdt_class_id, p, dyn_pdt_free);
}
    ...
    for (i = 2; i < 6; i++)                /* the date fields are tp_check's */
        if (f[i] < 0 || f[i] > LIM[i])
            return JS_ThrowRangeError(ctx, "PlainDateTime: %s %d is not 0..%d",
                                      NM[i], f[i], LIM[i]);
    return tp_new_dt(ctx, new_target, y, f[0], f[1],
                           ...
                           int argc, JSValueConst *argv, int magic)
{
    ...
    JS_FreeValue(ctx, nd);
    (void)ed;
    return tp_new_dt(ctx, JS_UNDEFINED, y, m, d, (int32_t)total);
}
                            ...
                            int argc, JSValueConst *argv, int magic)
{
    ...
    if (!p)
        return JS_EXCEPTION;
    return magic ? tp_new_time(ctx, JS_UNDEFINED, p->ms)
                 : tp_new_date(ctx, JS_UNDEFINED, p->d.y, p->d.m, p->d.d);
}
