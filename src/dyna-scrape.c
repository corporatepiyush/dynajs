...
static int fe_load_robots(JSContext *ctx, fe_t *f, JSValueConst client,
    ...
    for (hop = 0; ; hop++) {
        ...
        if (f->robots_on) {
            ...
            if (!h->robots_tried || now_ms >= h->robots_next_ms)
                { if (fe_load_robots(ctx, f, client, uh, h, https) < 0) goto out; }
                           ...
                           JSValueConst *argv)
{
...

    {
        ...
        int v_priv, v_reval, v_ttl, v_insec;
                           ...
                           JSValueConst *argv)
{
    ...
    const char *s;
    int v_maxpages, v_maxdepth, v_samehost;
    (void)nt;
