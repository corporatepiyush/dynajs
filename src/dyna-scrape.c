...
typedef struct {
    ...
    rb_t  *robots;       /* NULL = allow all (unfetched or unreachable) */
    int    robots_tried;
} fe_host_t;
...
/* robots.txt, once per host. Unreachable robots means ALLOW (RFC 9309), and
   `tried` stops a 404 being re-fetched for every URL on the host. */
                           ...
                           fe_host_t *h, int https)
{
    ...
    if (JS_IsException(res)) { JS_FreeValue(ctx, JS_GetException(ctx)); return; }
    if (fe_status(ctx, res) == 200) {
        ...
        if (txt) {
            ...
            if (r) {
                r->delay = -1;
                if (rb_parse(r, txt, bn, f->agent) < 0) { rb_free(r); r = NULL; }
                h->robots = r;
            }
            JS_FreeCString(ctx, txt);
        }
                          ...
                          JSValueConst *argv)
{
    ...
    for (hop = 0; ; hop++) {
        ...
        if (f->robots_on) {
            ...
            if (h->robots && !rb_allows_path(h->robots, path, strlen(path))) {
                f->skipped_robots++;
...
static const JSCFunctionListEntry dyn_cr_proto[] = {
    JS_CFUNC_DEF("start", 2, dyn_cr_start),
