 ...
 * instead of being silently ignored -- `{weight: true}` used to read as an
 * UNWEIGHTED graph and every later addEdge with a weight silently stored a
 * 1.0. Symbol keys and non-enumerable properties are not options and stay
 * invisible (JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY). Getters are NOT
 * invoked here; the values are read afterwards, so a throwing getter on a
 * KNOWN key still propagates. Returns 0 when the bag is clean, -1 with the
 * TypeError pending. */
static int dyn_graph_opts_check(JSContext *ctx, JSValueConst o,
...
static const char *const dyn_graph_keys[] = { "directed", "weighted" };
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    if (argc >= 1 && JS_IsObject(argv[0])) {
        /*unknown keys are refused BEFORE any value is read; a
         * non-object first arg keeps its historic "no options" reading. */
        if (dyn_graph_opts_check(ctx, argv[0], dyn_graph_keys,
                                 countof(dyn_graph_keys)))
            return JS_EXCEPTION;
        ...
        int bd = JS_ToBool(ctx, d);
        int bw = JS_ToBool(ctx, w);
        JS_FreeValue(ctx, d); JS_FreeValue(ctx, w);
        /* a throwing getter on a KNOWN key now STOPS the ctor instead of
         * leaving a pending exception while the graph is built */
        if (bd < 0 || bw < 0)
            return JS_EXCEPTION;
        directed = bd;
        weighted = bw;
    ...
    g->directed = directed;
    g->weighted = weighted;
    obj = dyn_plain_wrap(ctx, new_target, dyn_graph_class_id, g, dyn_graph_free);
    return obj;
}
...
/* ---- BFS / DFS ---- */
/* The visitor forms live below the array forms (shared emit helper); declared
 * here because the entry points dispatch on the second argument. */
static int dyn_graph_visit_opt(JSContext *ctx, JSValueConst opts,
                               const char *op, JSValue *pvisit);
static int dyn_graph_bfs_visit(JSContext *ctx, dyn_graph_t *g, uint32_t src,
                               JSValueConst visit, uint32_t *pvisited);
static int dyn_graph_dfs_visit(JSContext *ctx, dyn_graph_t *g, uint32_t src,
                               JSValueConst visit, uint32_t *pvisited);

/* bfs(src) -> number[]  |  bfs(src, {visit(node)}) -> number (visited count).
 * The visitor form early-exits when visit returns true and allocates nothing
 * per node; a visit() throw unwinds the traversal and propagates. */
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    if (argc >= 2 && !JS_IsUndefined(argv[1]) && !JS_IsNull(argv[1])) {
        JSValue visit = JS_UNDEFINED;
        uint32_t visited;
        int rc;
        if (dyn_graph_visit_opt(ctx, argv[1], "bfs", &visit))
            return JS_EXCEPTION;
        rc = dyn_graph_bfs_visit(ctx, g, src, visit, &visited);
        JS_FreeValue(ctx, visit);
        return rc < 0 ? JS_EXCEPTION : JS_NewInt64(ctx, visited);
    }
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    if (argc >= 2 && !JS_IsUndefined(argv[1]) && !JS_IsNull(argv[1])) {
        ...
        uint32_t visited;
        int rc;
        if (dyn_graph_visit_opt(ctx, argv[1], "dfs", &visit))
            return JS_EXCEPTION;
        rc = dyn_graph_dfs_visit(ctx, g, src, visit, &visited);
        JS_FreeValue(ctx, visit);
        return rc < 0 ? JS_EXCEPTION : JS_NewInt64(ctx, visited);
    }
...
/* ----: CSR export + neighborsInto;: visitor traversals ----
 *
 * Allocation contracts:
 *   - exportCSR() COPIES ONCE into two fresh Int32Arrays and never aliases the
 *     native adjacency; the {offsets, edges} pair is node-ordered, i.e.
 *     edges[offsets[u] .. offsets[u+1]] are exactly `neighbors(u)`.
 *   - neighborsInto(u, out) NEVER allocates: it writes straight through the
 *     caller's Int32Array and returns the degree. A buffer shorter than the
 *     degree throws RangeError BEFORE anything is written; extra capacity is
 *     left untouched.
 *   - the visitor forms of bfs/dfs/topologicalSort allocate only the traversal
 *     working set (queue + seen bits) once per call -- nothing per node -- and
 *     stop the walk the moment `visit` returns true. */

/* The one Int32Array class id, captured at registration by asking the engine
 * about a fresh 1-element array (the same trick dyna:dataframe uses). */
static JSClassID dyn_graph_i32_class_id;

/* Bind a caller's Int32Array as (pointer, element count). Strict on the type:
 * a Uint32Array or BigInt64Array would silently misrender large node ids.
 * Returns 0, or -1 (throwing). The pointer aliases the JS buffer and is valid
 * only while no JS runs. */
static int dyn_graph_get_i32(JSContext *ctx, JSValueConst v, int32_t **pp,
                             uint32_t *pn)
{
    JSValue buf;
    uint8_t *base;
    ...
    if (JS_GetClassID(v) != dyn_graph_i32_class_id) {
        JS_ThrowTypeError(ctx, "dyna:structures expected an Int32Array");
        return -1;
    }
    ...
    if (off > ab || len > ab - off) {
        JS_ThrowRangeError(ctx, "dyna:structures Int32Array out of bounds");
        return -1;
    }
    *pp = (int32_t *)(base + off);
    *pn = (uint32_t)(len / 4);
...
/* JS_NewArrayBufferCopy must not see NULL for an empty array. */
static const int32_t dyn_graph_i32_zero;

/* Fresh Int32Array copied from a native buffer (never aliases it). */
static JSValue dyn_graph_i32_array(JSContext *ctx, const int32_t *a, size_t n)
{
    JSValue ab, out;
    JSValueConst ta_args[3];

    if (n == 0)
        a = &dyn_graph_i32_zero;
    ab = JS_NewArrayBufferCopy(ctx, (const uint8_t *)a, n * sizeof(int32_t));
    if (JS_IsException(ab))
        return ab;
    ta_args[0] = ab;
    ta_args[1] = JS_UNDEFINED;
    ta_args[2] = JS_UNDEFINED;
    out = JS_NewTypedArray(ctx, 3, ta_args, JS_TYPED_ARRAY_INT32);
    JS_FreeValue(ctx, ab);
    return out;
}

/* exportCSR() -> { offsets: Int32Array(n+1), edges: Int32Array(total degree) }. */
static JSValue dyn_graph_export_csr(JSContext *ctx, JSValueConst this_val,
                                    int argc, JSValueConst *argv)
{
    dyn_graph_t *g;
    int32_t *off = NULL, *ed = NULL;
    uint64_t total = 0;
    uint32_t u, i, at = 0;
    JSValue joff, jed, res;
    ...
    for (u = 0; u < g->n; u++)
        total += g->adj[u].n;
    /* Node ids are int32 by construction, and an edges array this large could
     * not be indexed by an Int32Array offsets entry anyway. */
    if (total > (uint64_t)INT32_MAX)
        return JS_ThrowRangeError(ctx, "exportCSR: too many edges (%llu)",
                                  (unsigned long long)total);
    off = (int32_t *)malloc(((size_t)g->n + 1) * sizeof(int32_t));
    ed = (int32_t *)malloc((total ? (size_t)total : 1) * sizeof(int32_t));
    if (!off || !ed) {
        free(off); free(ed);
        return JS_ThrowOutOfMemory(ctx);
    }
    for (u = 0; u < g->n; u++) {
        off[u] = (int32_t)at;
        for (i = 0; i < g->adj[u].n; i++)
            ed[at++] = g->adj[u].e[i].to;
    }
    off[g->n] = (int32_t)at;
    joff = dyn_graph_i32_array(ctx, off, (size_t)g->n + 1);
    free(off);
    if (JS_IsException(joff)) { free(ed); return joff; }
    jed = dyn_graph_i32_array(ctx, ed, (size_t)total);
    free(ed);
    if (JS_IsException(jed)) {
        JS_FreeValue(ctx, joff);
        return jed;
    }
    /* JS_DefinePropertyValueStr consumes `val` on success AND on failure
     * (property_set_convert), so each branch below frees only what the
     * defines did not take over. */
    res = JS_NewObject(ctx);
    if (JS_IsException(res)) {
        JS_FreeValue(ctx, jed);
        JS_FreeValue(ctx, joff);
        return JS_EXCEPTION;
    }
    if (JS_DefinePropertyValueStr(ctx, res, "offsets", joff, JS_PROP_C_W_E) < 0) {
        JS_FreeValue(ctx, jed);
        JS_FreeValue(ctx, res);
        return JS_EXCEPTION;
    }
    if (JS_DefinePropertyValueStr(ctx, res, "edges", jed, JS_PROP_C_W_E) < 0) {
        JS_FreeValue(ctx, res);
        return JS_EXCEPTION;
    }
    return res;
...
/* neighborsInto(u, out): write u's neighbors into the caller's Int32Array,
 * return the count (the degree). No allocation. */
static JSValue dyn_graph_neighbors_into(JSContext *ctx, JSValueConst this_val,
                                        int argc, JSValueConst *argv)
{
    dyn_graph_t *g;
    uint32_t u, n, i;
    ...
    if (!g)
        return JS_EXCEPTION;
    if (u >= g->n)
        return JS_ThrowRangeError(ctx, "neighborsInto: node out of range");
    if (dyn_graph_get_i32(ctx, argv[1], &out, &n))
        return JS_EXCEPTION;
    deg = (int64_t)g->adj[u].n;
    if ((uint64_t)deg > (uint64_t)n)
        return JS_ThrowRangeError(ctx,
            "neighborsInto: out has %u slots, node %u has %lld neighbors",
            n, u, (long long)deg);
    /* no JS may run between the bind and these stores */
    for (i = 0; i < g->adj[u].n; i++)
        out[i] = g->adj[u].e[i].to;
    return JS_NewInt64(ctx, deg);
}

/* Parse a {visit} options bag into *pvisit (a JSfunction). The bag is strict:
 * `visit` is its only key. Returns 0, or -1 (throwing). */
static int dyn_graph_visit_opt(JSContext *ctx, JSValueConst opts,
                               const char *op, JSValue *pvisit)
{
    static const char *const keys[] = { "visit" };

    if (!JS_IsObject(opts)) {
        JS_ThrowTypeError(ctx, "%s: expected an options object with a visit "
                               "callback", op);
        return -1;
    }
    if (dyn_graph_opts_check(ctx, opts, keys, 1))
        return -1;
    *pvisit = JS_GetPropertyStr(ctx, opts, "visit");
    if (JS_IsException(*pvisit))
        return -1;
    if (!JS_IsFunction(ctx, *pvisit)) {
        JS_FreeValue(ctx, *pvisit);
        *pvisit = JS_UNDEFINED;
        JS_ThrowTypeError(ctx, "%s: options.visit must be a function", op);
        return -1;
    }
    return 0;
}

/* Call visit(node); 1 = continue, 0 = stop, -1 = it threw (exception pending). */
static int dyn_graph_emit(JSContext *ctx, JSValueConst visit, int node)
{
    JSValue arg, r;
    int stop;

    arg = JS_NewInt64(ctx, node);
    r = JS_Call(ctx, visit, JS_UNDEFINED, 1, (JSValueConst *)&arg);
    JS_FreeValue(ctx, arg);
    if (JS_IsException(r))
        return -1;
    stop = JS_ToBool(ctx, r);
    JS_FreeValue(ctx, r);
    return stop < 0 ? -1 : !stop;
}

/* bfs(src, {visit}): early exit when visit returns true. Allocates only the
 * queue + seen bits, once. Returns the number of nodes visited (counting the
 * one visit returned true for); -1 (throwing). */
static int dyn_graph_bfs_visit(JSContext *ctx, dyn_graph_t *g, uint32_t src,
                               JSValueConst visit, uint32_t *pvisited)
{
    int *q;
    char *seen;
    uint32_t head = 0, tail = 0, visited = 0, i;
    int ev;
    /* the working set is sized ONCE, so a visit() that calls addNode/addEdge
     * is refused the same way aStar refuses a mutating heuristic */
    const uint32_t n0 = g->n;
    const uint64_t m0 = g->m;

    q = (int *)malloc((size_t)(g->n ? g->n : 1) * sizeof(int));
    seen = (char *)calloc(g->n ? g->n : 1, 1);
    if (!q || !seen) {
        free(q); free(seen);
        JS_ThrowOutOfMemory(ctx);
        return -1;
    }
    q[tail++] = (int)src; seen[src] = 1;
    while (head < tail) {
        int x = q[head++];
        visited++;
        ev = dyn_graph_emit(ctx, visit, x);
        if (ev < 0) { free(q); free(seen); return -1; }
        if (g->n != n0 || g->m != m0) {
            free(q); free(seen);
            JS_ThrowTypeError(ctx, "bfs: the visitor mutated the graph");
            return -1;
        }
        if (ev == 0) break;
...
/* dfs(src, {visit}): the array form's walk (mark on pop), with a visit call in
 * the same position the order entry is written. The stack is sized by the edge
 * count for the same reason the array form's is. */
static int dyn_graph_dfs_visit(JSContext *ctx, dyn_graph_t *g, uint32_t src,
                               JSValueConst visit, uint32_t *pvisited)
{
    int *stack;
    char *seen;
    uint32_t sp = 0, visited = 0, i;
    int ev;
    const uint32_t n0 = g->n;
    const uint64_t m0 = g->m;
    uint64_t cap = 1;
    ...
    for (i = 0; i < g->n; i++)
        cap += g->adj[i].n;
    if (cap > (uint64_t)UINT32_MAX) {
        JS_ThrowRangeError(ctx, "dfs: graph too large to traverse");
        return -1;
    }
    stack = (int *)malloc((size_t)cap * sizeof(int));
    seen = (char *)calloc(g->n ? g->n : 1, 1);
    ...
    while (sp) {
        ...
        visited++;
        ev = dyn_graph_emit(ctx, visit, x);
        if (ev < 0) { free(stack); free(seen); return -1; }
        if (g->n != n0 || g->m != m0) {
            free(stack); free(seen);
            JS_ThrowTypeError(ctx, "dfs: the visitor mutated the graph");
            return -1;
        }
        if (ev == 0) break;
        /* push in reverse so smaller ids are visited first, as in the array form */
        for (i = g->adj[x].n; i-- > 0; ) {
            int v = g->adj[x].e[i].to;
            if (!seen[v]) stack[sp++] = v;
        }
...
/* toposort({visit}): Kahn's order with a visit callback per node; early exit
 * on true. A cycle encountered before the walk stops still throws. */
static int dyn_graph_topo_visit(JSContext *ctx, dyn_graph_t *g,
                                JSValueConst visit, uint32_t *pvisited)
{
    int *indeg, *q;
    uint32_t head = 0, tail = 0, visited = 0, u, i;
    int ev;
    const uint32_t n0 = g->n;
    const uint64_t m0 = g->m;

    indeg = (int *)calloc(g->n ? g->n : 1, sizeof(int));
    q = (int *)malloc((size_t)(g->n ? g->n : 1) * sizeof(int));
    if (!indeg || !q) {
        free(indeg); free(q);
        JS_ThrowOutOfMemory(ctx);
    ...
    for (u = 0; u < g->n; u++) if (indeg[u] == 0) q[tail++] = (int)u;
    while (head < tail) {
        int x = q[head++];
        visited++;
        ev = dyn_graph_emit(ctx, visit, x);
        if (ev < 0) { free(indeg); free(q); return -1; }
        if (g->n != n0 || g->m != m0) {
            free(indeg); free(q);
            JS_ThrowTypeError(ctx, "topologicalSort: the visitor mutated the graph");
            return -1;
        }
        if (ev == 0) {
            /* stopped by visit: an incomplete order is the caller's choice,
             * not a cycle -- skip the drained-queue check */
            free(indeg); free(q);
            *pvisited = visited;
            return 0;
        }
    ...
    free(indeg); free(q);
    if (visited < g->n)
        return -2;          /* cycle: the queue drained before every node */
    *pvisited = visited;
    return 0;
}
 ...
 * topologicalSort() -> number[]  |  topologicalSort({visit(node)}) -> number
 * (visited count). The visitor form early-exits when visit returns true; a
 * cycle reached before that still throws. */
                                   ...
                                   int argc, JSValueConst *argv)
{
    ...
    if (argc >= 1 && !JS_IsUndefined(argv[0]) && !JS_IsNull(argv[0])) {
        ...
        uint32_t visited;
        int rc;
        if (dyn_graph_visit_opt(ctx, argv[0], "topologicalSort", &visit))
            return JS_EXCEPTION;
        rc = dyn_graph_topo_visit(ctx, g, visit, &visited);
        JS_FreeValue(ctx, visit);
        if (rc == -2)
...
/* connectedComponents: comp[i] = component id (0-based). Deliberately no
 * directed guard (unlike mst/topoSort): on a directed graph this answers WEAK
 * components -- the connected parts of the underlying undirected graph --
 * which is the useful reading of "connected" there. Strong components are a
 * different algorithm. Weak means every edge is walked in BOTH directions:
 * the old outgoing-only walk answered reachability classes and silently split
 * {a->b, c->b} into two components. */
static JSValue dyn_graph_components(JSContext *ctx, JSValueConst this_val,
                                    int argc, JSValueConst *argv)
{
    dyn_graph_t *g;
    int *comp;
    uint32_t *off = NULL, *tgt = NULL, *fill = NULL;
    int *stack;
    uint32_t sp, i, u, s, n, ncomp = 0;
    uint64_t m2 = 0;
    JSValue res;
    (void)argc; (void)argv;
    g = graph_of(ctx, this_val);
    if (!g)
        return JS_EXCEPTION;
    n = g->n;
    comp = (int *)malloc((size_t)(n ? n : 1) * sizeof(int));
    stack = (int *)malloc((size_t)(n ? n : 1) * sizeof(int));
    off = (uint32_t *)malloc((size_t)(n + 1) * sizeof(uint32_t));
    if (!comp || !stack || !off) { free(comp); free(stack); free(off);
                                   return JS_ThrowOutOfMemory(ctx); }
    /* Mirror the adjacency once: each edge appears under BOTH endpoints, so
     * the walk below sees the underlying undirected graph. O(n + m). Pass 1
     * sizes every node's mirrored degree (out + in), pass 2 scatters. */
    fill = (uint32_t *)malloc((size_t)(n ? n : 1) * sizeof(uint32_t));
    if (!fill) { free(comp); free(stack); free(off);
                 return JS_ThrowOutOfMemory(ctx); }
    for (u = 0; u < n; u++) fill[u] = g->adj[u].n;
    for (u = 0; u < n; u++)
        for (i = 0; i < g->adj[u].n; i++) {
            int v = g->adj[u].e[i].to;
            fill[(uint32_t)v]++;
    ...
    tgt = (uint32_t *)malloc((size_t)(m2 ? m2 : 1) * sizeof(uint32_t));
    if (!tgt) { free(comp); free(stack); free(off); free(fill);
                return JS_ThrowOutOfMemory(ctx); }
    for (u = 0; u < n; u++) fill[u] = off[u];
    for (u = 0; u < n; u++)
        for (i = 0; i < g->adj[u].n; i++) {
            int v = g->adj[u].e[i].to;
            tgt[fill[u]++] = (uint32_t)v;
            tgt[fill[v]++] = u;
        }
    for (i = 0; i < n; i++) comp[i] = -1;
    for (s = 0; s < n; s++) {
        if (comp[s] != -1) continue;
        sp = 0; stack[sp++] = (int)s; comp[s] = (int)ncomp;
        while (sp) {
            int x = stack[--sp];
            uint32_t a = off[x], e = off[x + 1];
            for (; a < e; a++) {
                uint32_t v = tgt[a];
                if (comp[v] == -1) { comp[v] = (int)ncomp; stack[sp++] = (int)v; }
            }
        }
        ncomp++;
    }
    res = int_array(ctx, comp, n);
    free(comp); free(stack); free(off); free(tgt); free(fill);
    return res;
}
...
static const JSCFunctionListEntry dyn_graph_proto[] = {
    ...
    JS_CFUNC_DEF("neighborsInto", 2, dyn_graph_neighbors_into),
    JS_CFUNC_DEF("exportCSR", 0, dyn_graph_export_csr),
...
static JSValue dyn_graph_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
    ...
    return dyn_plain_wrap(ctx, JS_UNDEFINED, dyn_graph_class_id, g, dyn_graph_free);
}
...
int dyn_graph_register(JSContext *ctx, JSModuleDef *m)
{
    ...
    if (dyn_register_plain_class(ctx, m, &dyn_graph_class_id, &dyn_graph_class,
        ...
        return -1;
    /* Capture the Int32Array class id for exportCSR/neighborsInto (one fresh
     * 1-element array, then ask the engine). */
    {
        JSValueConst ta_args[1];
        JSValue ta, zero = JS_NewInt32(ctx, 0);
        ta_args[0] = zero;
        ta = JS_NewTypedArray(ctx, 1, ta_args, JS_TYPED_ARRAY_INT32);
        if (JS_IsException(ta))
            return -1;
        dyn_graph_i32_class_id = JS_GetClassID(ta);
        JS_FreeValue(ctx, ta);
