...
{
    if (a->n == a->cap) {
        uint32_t nc = a->cap ? a->cap * 2 : 4;
        dyn_edge_t *ne = (dyn_edge_t *)realloc(a->e, (size_t)nc * sizeof(dyn_edge_t));
        if (!ne)
            return -1;
        a->e = ne;
        a->cap = nc;
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (!g)
        return JS_EXCEPTION;
    if (u < 0 || v < 0)
        return JS_ThrowRangeError(ctx, "node id must be >= 0");
    if (!g->weighted)
        w = 1.0;
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    JSValue res;
    (void)argc;
    if (JS_ToInt32(ctx, &src, argv[0]))
        return JS_EXCEPTION;
    g = graph_of(ctx, this_val);
    if (!g)
        return JS_EXCEPTION;
    if (src < 0 || (uint32_t)src >= g->n)
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (src < 0 || (uint32_t)src >= g->n)
        return JS_ThrowRangeError(ctx, "source out of range");
    if (have_dst && (dst < 0 || (uint32_t)dst >= g->n))
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (JS_IsException(row) ||
            JS_DefinePropertyValueUint32(ctx, res, i, row, JS_PROP_C_W_E) < 0) {
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    for (u = 0; u < g->n; u++)
        for (i = 0; i < g->adj[u].n; i++) {
            ...
            if (ne == cap) {
                kedge *n2 = (kedge *)realloc(es, (cap *= 2) * sizeof(kedge));
    ...
    for (i = 0; i < ne; i++) {
        ...
        total += es[i].w;
        {
            ...
            if (JS_IsException(e) ||
                JS_DefinePropertyValueUint32(ctx, e, 0, JS_NewInt64(ctx, a), JS_PROP_C_W_E) < 0 ||
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    if (!g)
        return JS_EXCEPTION;
    if (src < 0 || (uint32_t)src >= g->n || dst < 0 || (uint32_t)dst >= g->n)
    ...
    /* re-resolve: the heuristic may have mutated `this` */
    g = graph_of(ctx, this_val);
    ...
    while (heap_pop(&heap, &it)) {
        ...
        for (i = 0; i < g->adj[u].n; i++) {
            ...
            if (w < 0) { free(h); free(gscore); free(prev); free(closed); free(heap.a);
...
static const JSCFunctionListEntry dyn_graph_proto[] = {
    ...
    JS_CFUNC_DEF("addEdge", 3, dyn_graph_add_edge),
...
static JSValue dyn_graph_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (varint_form) {
             ...
             * forged degree still cannot outrun what remains. */
            if (dyn_de_uvarint(r, &d) < 0 ||
