...
static JSValue dyn_rrule_build(JSContext *ctx, JSValueConst new_target,
                               const DynRRuleSpec *s)
{
    DynRRule *rr;
    int64_t ds_sec;
    int no_by;
    ...
    if (rr->ds_y < 1 || rr->ds_y > DYN_RRULE_MAX_YEAR) {
        /* Copy the value out BEFORE free(): the error text reads it after the
           region is released -- ASAN: heap-use-after-free (audit F3). */
        const int64_t bad_y = rr->ds_y;
        free(rr);
    ...
    return dyn_plain_wrap(ctx, new_target, dyn_rrule_class_id, rr, dyn_rrule_dispose);
}
...
typedef struct DynRRuleGen {
    ...
    uint32_t steps;
    uint8_t exhausted;           /* set when the period budget was exhausted */
} DynRRuleGen;
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    /* A budget-exhausted break means the scan stopped early: the result is
     * silently partial whether or not an external limit was given (a natural
     * end -- COUNT/UNTIL/year-9999 or collect_cb's own limit -- never sets
     * the exhausted flag). */
    if (!c.failed && g.exhausted)
        return JS_FreeValue(ctx, c.arr),
            JS_ThrowRangeError(ctx,
                "RRule.all(): recurrence budget exhausted (%u periods); "
                "collected %lld of %lld results -- narrow the rule or raise "
                "the limit", (unsigned)DYN_RRULE_MAX_STEPS, (long long)c.n,
                (long long)(limit < 0 ? c.n : limit));
    return c.arr;
}
...
typedef struct {
    ...
    int64_t n;
    int failed;
    int done;                /* natural completion: the window end was reached */
} DynRRuleWindow;
                                 ...
                                 int argc, JSValueConst *argv)
{
    ...
    /* A budget-exhausted break leaves the window partially scanned only if
     * the loop never reached the window end; a natural end is complete. */
    if (g.exhausted && !w.done) {
        JS_FreeValue(ctx, w.arr);
        return JS_ThrowRangeError(ctx,
            "RRule.between(): recurrence budget exhausted (%u periods); "
            "the window is incomplete -- narrow it or raise the limit",
            (unsigned)DYN_RRULE_MAX_STEPS);
    }
    return w.arr;
}
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    /* If the budget was exhausted before any occurrence past fromDate was
     * found, the result is unknown, not "none": throw rather than lie. */
    if (g.exhausted)
        return JS_ThrowRangeError(ctx,
            "RRule.next(): recurrence budget exhausted (%u periods); the "
            "search is incomplete -- narrow the query or raise the limit",
            (unsigned)DYN_RRULE_MAX_STEPS);
    return JS_NULL;
}
...
typedef struct {
    ...
    int have;
    int stop;
    int done;                /* natural completion: fromDate was passed */
} DynRRulePrev;
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    /* Budget exhausted before the cursor passed fromDate (or, without a
     * fromDate, before the rule's true end): pv.res is the best-so-far, not
     * the true answer -- throw rather than return a too-early date. */
    if (g.exhausted && !pv.done)
        return JS_ThrowRangeError(ctx,
            "RRule.prev(): recurrence budget exhausted (%u periods); the "
            "search is incomplete -- narrow the query or raise the limit",
            (unsigned)DYN_RRULE_MAX_STEPS);
    if (pv.have)
                                     ...
                                     int argc, JSValueConst *argv)
{
    ...
    if (argc > 1 && JS_IsObject(argv[1])) {
        /*{dtstart} is the whole bag. A typo'd key used to be dropped
           and the rule anchored to its own default start. */
        static const char *const rr_keys[] = { "dtstart" };
        if (dyn_time_opts_strict(ctx, argv[1], rr_keys, 1)) {
            JS_FreeCString(ctx, s);
            return JS_EXCEPTION;
        }
    ...
    return dyn_rrule_build(ctx, JS_UNDEFINED, &spec);
}
                                       ...
                                       DynRRuleSpec *s)
{
    ...
    if (!JS_IsUndefined(v)) {
        ...
        if (JS_IsNumber(v)) {
            int32_t n;
            if (JS_ToInt32(ctx, &n, v) == 0 && n >= 0 && n <= 6)
                w = n;
        } else {
            size_t len;
        ...
        if (w < 0) {
            JS_FreeValue(ctx, v);
            JS_ThrowRangeError(ctx, "RRule: wkst must be 0..6 or MO..SU");
            return -1;
        }
        s->wkst = w;
        s->orig |= DYN_RR_ORIG_WKST;
    ...
    /* BYHOUR/BYMINUTE/BYSECOND/BYEASTER are refused, not silently ignored:
     * the string-form parser (dyn_rrule_spec_parse_part) already rejects
     * them, but the object-form constructor read past them unchecked -- so
     * new RRule({freq:"DAILY", byhour:[9]}) silently produced a rule that
     * ignores the option. Fail the same way the RRULE string form does. */
    {
        static const char *const refused[] = { "byhour", "byminute",
                                               "bysecond", "byeaster" };
        int ri;
        for (ri = 0; ri < 4; ri++) {
            v = JS_GetPropertyStr(ctx, obj, refused[ri]);
            if (JS_IsException(v))
                return -1;
            if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
                JS_FreeValue(ctx, v);
                JS_ThrowSyntaxError(ctx,
                    "dyna:time: unsupported RRULE option (%s)", refused[ri]);
                return -1;
            }
            JS_FreeValue(ctx, v);
        }
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    if (dyn_rrule_spec_from_options(ctx, argv[0], &spec))
        return JS_EXCEPTION;
    return dyn_rrule_build(ctx, new_target, &spec);
}
