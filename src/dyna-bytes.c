...
/* Clamp an optional fromIndex-style scalar: String.prototype clamping
 * (negative -> 0), NOT JS_ToIndex, whose RangeError on a negative would make
 * `indexOf(needle, -1)` throw where every String/Buffer peer searches.
 * UNSET is reported as the sentinel SIZE_MAX and each caller substitutes its
 * own default (0 for the forward scans, the buffer length for lastIndexOf),
 * because the length is only known after the buffer is resolved. Coercion
 * goes through JS_ToInt64 (ToInteger), which is why every caller resolves
 * these BEFORE the needle and the buffer -- a valueOf can run arbitrary JS,
 * and no argument may be resolved before that can happen (the standing order
 * from dyn_bh_slice). The sentinel is unreachable from the coercion itself:
 * anything above INT64_MAX wraps negative in JS_ToInt64 and clamps to 0. */
...
/* indexOf(buf, needle, fromIndex?): the search STARTS at fromIndex (clamped
 * String.prototype.indexOf-style); the motivating case is a successive-match
 * loop, which no longer allocates a slice() per hit. The empty needle matches
 * at the clamped start -- the default fromIndex 0 preserves the historical
 * "bytes.Index(s, []) == 0". */
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (start == DYN_BYTES_IDX_UNSET)
        start = 0;               /* forward scans default to the buffer start */
    if (start > n)
        start = n;

    if (plen == 0)
        return JS_NewInt64(ctx, (int64_t)start); /* bytes.Index(s, []) == start */
...
/* lastIndexOf(buf, needle, fromIndex?): searches BACKWARD from fromIndex
 * (clamped into [0, len], default len = the whole buffer); a match must START
 * at or before the window edge, so the search region is the prefix
 * buf[0 .. fromIndex + plen). The empty needle matches at the clamped
 * fromIndex -- the default (len) preserves the historical
 * "bytes.LastIndex(s, []) == len(s)". */
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    if (plen > n)
        return JS_NewInt32(ctx, -1);
    /* Last match at index <= hi is exactly dyn_bytes_last_find over the
     * prefix window buf[0 .. hi+plen): same textbook scan, bounded. */
    hi = n - plen;
    if (start < hi)
        hi = start;
...
/* count(buf, needle, fromIndex?): NON-overlapping occurrences, unlike
 * dyna:strings' indexOfAll (which is deliberately overlapping). The scan
 * starts at fromIndex (clamped), so the empty needle counts the positions of
 * the remaining window: n - start + 1 (the default fromIndex 0 preserves the
 * historical n + 1). See the module doc comment for the empty-needle
 * convention. */
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    if (dyn_bytes_view(ctx, argv[0], &buf, &n))
        return JS_EXCEPTION;
    if (start == DYN_BYTES_IDX_UNSET)
        start = 0;               /* the scan defaults to the buffer start */
    if (start > n)
        start = n;
...
/* startsWith(buf, needle, fromIndex=0) / endsWith(buf, needle,
 * end=buf.length): the String.prototype predicates over bytes. fromIndex is
 * where the prefix must begin; `end` is the exclusive position the suffix
 * must end at -- a match must END there, not merely fit before it. Both
 * scalars are coerced before the needle and the buffer are resolved (a
 * valueOf can run JS). */
                                     ...
                                     int argc, JSValueConst *argv)
{
    ...
    if (dyn_bytes_view(ctx, argv[0], &buf, &n))
        return JS_EXCEPTION;
    if (start == DYN_BYTES_IDX_UNSET)
        start = 0;               /* the prefix begins at the buffer start */
    if (start > n)
        start = n;
                                   ...
                                   int argc, JSValueConst *argv)
{
    ...
    if (end == DYN_BYTES_IDX_UNSET)
        end = n;                 /* the whole buffer is the default window */
    if (end > n)
        end = n;
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    /* list must be an Array, matching Bytes.concat and Node's Buffer.concat:
     * the length-getter alone accepted any object (concat(5) -> empty buffer,
     * since ToUint32(undefined) is 0) where every peer throws TypeError. */
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    dyn_bh_dirty_handle(argv[0]);  /* a Bytes handle owns this buffer */
...
/* WHATWG lossy UTF-8 decode (Encoding Standard §UTF-8 decode, the U+FFFD
 * substitution policy TextDecoder implements). JS_NewStringLen alone has two
 * divergences from that contract wording ("invalid sequences become U+FFFD"):
 * a surrogate-encoded sequence (ED A0 80, WTF-8 for a lone surrogate)
 * round-trips to a LONE SURROGATE instead of U+FFFD, and an overlong or
 * out-of-range sequence collapses to a single U+FFFD where the spec replaces
 * each maximally-consumed subpart -- C0 AF is two replacements, not one, and
 * F4 90 80 80 is four. Valid input never reaches the re-encoder: the strict
 * kernel answers first and the bytes go straight to JS_NewStringLen. */
static JSValue dyn_utf8_lossy_string(JSContext *ctx, const uint8_t *p, size_t n)
{
    ...
    /* ASCII prefix first, SWAR: a buffer with no high bit is valid UTF-8 by
     * definition, so the overwhelming case pays one cheap scan and never
     * touches the validator (the unconditional validate_utf8 added ~30% to
     * toUtf8 of a 1 MB ASCII buffer -- audit A/B numbers). */
    while (ascii + 8 <= n) {
        uint64_t word;
...
multibyte_tail:
    if (simd.validate_utf8(p, n) == n)
        return JS_NewStringLen(ctx, (const char *)p, n); /* valid, no copy */

    /* worst case: one 3-byte U+FFFD per input byte */
    ...
    while (i < n) {
        ...
        if (c < 0xC2)
            bad = 1;                 /* stray continuation, or overlong lead C0/C1 */
        else if (c < 0xE0) {
            len = 2;                 /* C2..DF: second byte 80..BF */
            bad = 0;
        } else if (c < 0xF0) {
            len = 3;                 /* E0: A0.., ED: ..9F (surrogate block fenced) */
            lo = (c == 0xE0) ? 0xA0 : 0x80;
            hi = (c == 0xED) ? 0x9F : 0xBF;
            bad = 0;
        } else if (c < 0xF5) {
            len = 4;                 /* F0: 90.., F4: ..8F (past U+10FFFF fenced) */
            lo = (c == 0xF0) ? 0x90 : 0x80;
            hi = (c == 0xF4) ? 0x8F : 0xBF;
            bad = 0;
        } else
            bad = 1;                 /* F5..FF: no valid sequence starts here */
        if (!bad) {
            /* collect the maximal subpart: the longest prefix that could still
             * complete; the first byte outside this lead's bounds ends it */
            for (j = 1; j < len; j++) {
        ...
        if (!bad) {
            /* the bound table already excludes overlong, surrogate and
             * >U+10FFFF forms, so a complete bounded sequence is a scalar */
            memcpy(out + o, p + i, len);
            o += len;
            i += len;
        } else {
            out[o++] = 0xEF;         /* U+FFFD for the consumed subpart */
            out[o++] = 0xBF;
            out[o++] = 0xBD;
            i += bad;
        }
 ...
 *  computed once at construction and RE-COMPUTED after any write that goes
 *  through this handle: every mutating path (fill, write*, handing out
 *  .array) clears them to UNKNOWN and the next read rescans. Recomputing
 *  eagerly per write would make a write loop O(n) per call; clearing makes
 *  any number of writes cost one rescan.
 *
 *  SCOPE of the invariant: the flags describe the buffer as last seen
 *  through THIS handle. A write through a DIFFERENT handle sharing the
 *  buffer (a slice's owner or sibling) is not observed -- the class has no
 *  back-pointer to co-own flags across views, and hooking the engine's
 *  typed-array stores is not available.
 *
...
/* the cached-summary sentinel: the flags are invalid and must be rescanned */
#define DYN_BH_UNKNOWN (-1)

typedef struct {
    JSValue u8;        /* the Uint8Array this handle owns */
    int is_ascii;      /* 1 yes, 0 no, DYN_BH_UNKNOWN = rescan before use */
    int is_valid_utf8;
} dyn_bh_t;
...
/* Method/static-created handles (slice, alloc, concat, ...): never under a
 * [[Construct]], so always the class's own prototype. */
static JSValue dyn_bh_wrap(JSContext *ctx, JSValue u8)
{
    return dyn_bh_wrap_flags(ctx, u8, 0, JS_UNDEFINED);
}

/* The constructor routes through here too, passing its new_target, which is
 * what makes `class B extends Bytes {}` produce B instances
 * (OrdinaryCreateFromConstructor via dyn_ctor_proto). The proto is resolved
 * BEFORE the buffer is consumed or the handle allocated: a throwing
 * "prototype" getter propagates with nothing to dispose. */
static JSValue dyn_bh_wrap_flags(JSContext *ctx, JSValue u8, int inherit_ascii,
                                 JSValueConst new_target)
{
    dyn_bh_t *b;
...
/* A write through THIS handle (fill, write*, handing out .array) may have
 * changed the buffer, so both cached summaries drop to UNKNOWN and the next
 * read rescans (see the class comment for why lazily, and for the scope). */
...
/* Recompute UNKNOWN flags from the handle's own buffer. 0 / -1 (throws). */
...
/* The free mutators (fill/copy/write on any view) clear the summaries of a
 * Bytes HANDLE argument too -- the handle owns the buffer they just wrote.
 * A forward declaration is needed because they are defined above the class;
 * dyn_bh_class_id is forward-declared at the top of the file for the same
 * reason. */
static void dyn_bh_dirty_handle(JSValueConst v)
{
    dyn_bh_t *b = (dyn_bh_t *)JS_GetOpaque(v, dyn_bh_class_id);
                             ...
                             JSValueConst *argv, int magic)
{
    ...
    if (magic == 0) { /* alloc(n) */
        ...
        /* INT32_MAX is the engine's real ArrayBuffer ceiling (2^31 itself is
         * rejected there as "invalid array buffer length"); enforcing it here
         * keeps this module's own error the one the caller sees. */
    ...
    /* concat(list [, ...more]) --: the ARRAY form and a VARIADIC form,
     * plus the mixed one (a leading array and further positional views).
     * Views come from list's elements first (when argv[0] is an Array) and
     * then from any remaining arguments. One allocation, sized in a first
     * pass; the pass-2 re-resolve keeps the "list changed" contract for the
     * array part (positional arguments are fixed JSValues, so only array
     * elements can move). */
    {
        ...
        /* NOT an error when argv[0] is not an Array: that is the VARIADIC
         * form, and junk arguments (5, "nope", ...) fail one door later in
         * the view resolve with the module's byte-view TypeError -- the
         * contract the existing tests pin. A lone view concats itself. */
        if (is_list) {
        ...
        /* fetch element `idx` of the logical sequence: the array's items
         * first, then the positional arguments (the latter dup'd for the
         * uniform free path). The extras begin at argv[1] in the mixed/list
         * form and at argv[0] in the pure-variadic form. */
#define DYN_CONCAT_ELEM(idx)                                                  \
    ((idx) < alen ? JS_GetPropertyUint32(ctx, list, (uint32_t)(idx))          \
        ...
        out = (uint8_t *)malloc(total_bytes ? total_bytes : 1);
        if (!out)
            return JS_ThrowOutOfMemory(ctx);
        for (i = 0; i < total; i++) {
            JSValue e = DYN_CONCAT_ELEM(i);
            ...
            if (o + n > total_bytes) { JS_FreeValue(ctx, e); free(out);
...
static JSValue dyn_bh_get(JSContext *ctx, JSValueConst this_val, int magic)
{
    ...
    default:
        /* .array is the sanctioned escape hatch, and it is also how ANY JS
         * write to the buffer must enter: through this getter. Invalidating
         * here is what keeps the cached summaries honest against mutations
         * the class itself never sees. */
        dyn_bh_dirty(b);
        return JS_DupValue(ctx, b->u8);   /* .array -- the owned view */
    }
}
                            ...
                            JSValueConst *argv)
{
    ...
    if (start < 0) start = 0;
    /* A start past the end clamps to an EMPTY view (subarray/Buffer parity),
     * not an error: without this the typed-array constructor below is handed
     * off > byteLength and throws the engine's generic "invalid length". */
    ...
    /* The child must not inherit a STALE parent flag: resolve any UNKNOWN
     * (invalidated) parent summaries first. Known-ASCII stays inheritable
     * because ASCII is closed under subranges; the non-ASCII case scans the
     * child itself (a slice can cut a multi-byte sequence in half). */
    if (dyn_bh_resolve(ctx, b) < 0) {
...
static JSValue dyn_bh_fill(JSContext *ctx, JSValueConst this_val, int argc,
    ...
    a[0] = b->u8;
    for (i = 0; i < 3; i++) a[i + 1] = (i < argc) ? argv[i] : JS_UNDEFINED;
    r = dyn_bytes_fill(ctx, JS_UNDEFINED, 4, a);
    if (JS_IsException(r)) return r;
    JS_FreeValue(ctx, r);   /* the free fill's result: the underlying Uint8Array */
    dyn_bh_dirty(b);
    /* BEHAVIOR CHANGE: the method returns the HANDLE (`this`), chainable
     * with indexOf/fill/write* in one expression. The FREE fill(buf, ...)
     * still returns `buf` (the underlying Uint8Array) -- that signature is
     * legacy and unchanged. The old method contract returned the same
     * Uint8Array as the free function; nothing in tests/, examples/ or the
     * docs consumed it (checked when this changed), and `this` is strictly
     * more useful: `.array` recovers the Uint8Array in one hop. */
...
/* ----: bulk read/write without dropping to .array + free copy ------- */

/* readBytes(off, len): a FRESH Uint8Array copying buf[off .. off+len).
 * A copy, deliberately: the class is a copied byte buffer (its constructor
 * copies for exactly the cached-flag reason documented there), so a bulk
 * read that aliased would be the one method handing out a view under a
 * copy-flavored contract -- .slice() is the sanctioned view, and it says so
 * in its own name and doc. */
                                 ...
                                 int argc, JSValueConst *argv)
{
    ...
    /* JS_ToIndex sets its own exception (RangeError on negative/too-large);
     * propagate it rather than masking with a TypeError */
...
/* writeBytes(off, src): copies src's bytes at off, overlap-safe (memmove --
 * src may alias this very buffer, e.g. a subarray of .array), and returns
 * the number of bytes written. Any part of the write that would land past
 * the buffer throws RangeError; nothing is written partially. */
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (JS_ToIndex(ctx, &off, argv[0]))
        return JS_EXCEPTION;   /* ToIndex's own RangeError, not a TypeError */
    if (dyn_bytes_view(ctx, argv[1], &src, &sn))
        return JS_EXCEPTION;
    if (dyn_bytes_view(ctx, b->u8, &p, &n) < 0)
        return JS_EXCEPTION;
                          ...
                          size_t *len, const char **owned)
{
    *owned = NULL;
    ...
    }
    {
         ...
         *
         * Same rule as dyn_bytes_view one function over: a WIDER-element view
         * (Uint16Array raw bytes etc) is a TypeError, not a silent
         * reinterpretation -- use bytesOf() to state it. And a view whose
         * buffer fails the fetch is DETACHED: JS_GetArrayBuffer has already
         * thrown the engine's "ArrayBuffer is detached" TypeError, so it is
         * propagated instead of falling through to string coercion (a
         * detached Uint8Array used to become the text "[object Uint8Array]",
         * which validates, counts and transcodes without error). */
        size_t off = 0, blen = 0, bpe = 0;
        ...
        if (!JS_IsException(buf)) {
            ...
            JS_FreeValue(ctx, buf);
            if (!ab)
                return -1;   /* detached: the engine's TypeError is pending */
            *data = ab + off;
            *len = blen;
            return 0;
        }
    ...
    }
    {
        ...
        /* A genuine ArrayBuffer that fails the fetch was detached mid-resolve
         * and the engine's TypeError is pending: propagate it. Any OTHER
         * object is not byte data either, and must not fall through to
         * "[object ...]" -- so objects stop here, and only primitives reach
         * the string coercion below. */
        if (JS_IsObject(v))
            return -1;
        JS_FreeValue(ctx, JS_GetException(ctx));
    }
...
/* indexOfAny(text, chars, fromIndex?): first byte index >= fromIndex holding
 * any char in `chars`, or -1. The set is resolved first (as before), then the
 * fromIndex scalar, then the text -- matching the module's coercion order. */
static JSValue dyn_text_index_of_any(JSContext *ctx, JSValueConst this_val,
                                     int argc, JSValueConst *argv)
{
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    /* The kernel collapses both failure causes into one -1: distinguish them
     * by re-validating the input (BEFORE `owned` is released -- on that path
     * `data` points into it). Invalid UTF-8 is the rejected decode;
     * well-formed UTF-8 that failed means some code point does not fit
     * latin1. */
    if (rc != 0)
 ...
 * An ODD byte length is ill-formed, matching what isValidUtf16 (false) and
 * utf16ToUtf8 (RangeError, same message) do with it: the trailing byte is a
 * truncated unit, and silently counting the whole units would bless input the
 * next function in the family rejects. */
                            ...
                            JSValueConst *argv)
{
     ...
     * TSan would find it. */
    {
        ...
        if (!cs) { JS_FreeValue(ctx, proto); JS_FreeValue(ctx, str); free(t); return JS_EXCEPTION; }
        t->is_wide = 0;
...
/* (dyn_txt_m_valid_utf8 -- the isValidUtf8 method shim -- retired by:
   the member is a getter now; dyn_txt_get magic 4 calls the same helper.) */
/* The UTF-16-input family is served by dyn_txt_u16_scan (below the class
   comments there): forwarding t->str to the FREE utf-16 functions handed
   them UTF-8 bytes -- the wrong encoding: Text("abc").countUtf16() answered
   1, isValidUtf16() answered FALSE for plain ASCII, and utf16ToUtf8() threw
   "byte length must be even" on any odd-length string. The free functions
   also gave right answers BY ACCIDENT on the obvious probes (a lone
   surrogate really did come back invalid), so a test written against those
   would have passed and pinned the wrong encoding in place. */

/* UTF-16 predicates over a Text's own character data, with no per-unit JS
   call (the charCodeAt loop this replaced cost one JS_Invoke per unit:
   3250 us for 120k units against 320 us for countUtf8 -- ws-bytes audit A/B).

   A NARROW (Latin-1) string's bytes ARE its code units: every unit is
   <= 0xFF, so no surrogate can exist and both predicates are trivial
   (JS_GetNarrowStringBytes borrows the storage, never allocates).

   A WIDE string is encoded once with JS_ToCStringLen2(cesu8=1): CESU-8
   gives EVERY code unit its own <=3-byte sequence -- unlike the default
   WTF-8, a surrogate pair is not merged into 4 bytes but stays as the two
   adjacent encodings ED[A0-AF]xx (high) and ED[B0-BF]xx (low). Each lead
   byte is one code point (a pair's low lead is skipped inside the pair
   step), and ill-formedness is exactly "a
   high-surrogate encoding not immediately followed by a low-surrogate
   encoding, or a bare low-surrogate encoding". */
static size_t dyn_txt_u16_scan(JSContext *ctx, dyn_txt_t *t, int *valid)
/* shared walker; `valid` may be NULL (counting only). Returns code points. */
{
    const uint8_t *p;
    const char *cs;
    size_t n, i, units = 0;

    {
        ...
        if (p) {
            if (valid)
                *valid = 1;
            return bn;               /* <= 0xFF per unit: no surrogate exists */
        }
    }
    ...
    while (i < n) {
        if (p[i] < 0x80) {
            ...
            units++;                 /* 2-byte BMP unit */
            i += 2;
        } else if (p[i] == 0xED && i + 2 < n &&
                   p[i + 1] >= 0xA0 && p[i + 1] <= 0xAF) {
            /* high-surrogate encoding: well-formed only when the very next
             * unit is the low-surrogate encoding of the pair */
            if (i + 5 < n && p[i + 3] == 0xED &&
                p[i + 4] >= 0xB0 && p[i + 4] <= 0xBF) {
                units++;             /* ONE code point: the low lead is skipped */
                i += 6;
            } else {
                units++;
                if (valid) {
                ...
                i += 3;
                   ...
                   p[i + 1] >= 0xB0 && p[i + 1] <= 0xBF) {
            units++;
            if (valid) {             /* low surrogate with no high before it */
                *valid = 0;
                JS_FreeCString(ctx, cs);
                return 0;
            }
            ...
            units++;                 /* 3-byte BMP unit (or defensive skip) */
            i += ((p[i] & 0xF0) == 0xE0) ? 3 : 1;
        }
    }
    JS_FreeCString(ctx, cs);
    if (valid)
        *valid = 1;
    return units;                /* a pair was ONE unit here: low lead skipped */
}
...
static JSValue dyn_txt_get(JSContext *ctx, JSValueConst this_val, int magic)
{
    ...
    if (magic == 4) {
        /*the isValidUtf8 GETTER. Same predicate the method used to
         * forward to (dyn_text_is_valid_utf8 over the wrapped string -- the
         * helper behind the free isValidUtf8 and Bytes.isValidUtf8 too), now
         * behind an accessor so `text.isValidUtf8` reads exactly like
         * `bytes.isValidUtf8`. A JS property is one slot per name, so the
         * historical METHOD spelling `text.isValidUtf8()` is no longer a
         * function call -- the same way `bytes.isValidUtf8()` has always
         * thrown. (The forward shim dyn_txt_m_valid_utf8 went with it.) */
        a[0] = t->str;
...
/* toBytes() IS toUtf8() plus the handle wrap: one encoding, one copy of the
   body. The first version duplicated the cstring->u8array body here, which is
   how the two spellings drift (this module shipped exactly that once). */
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    if (JS_IsException(u8))
        return u8;
    return dyn_bh_wrap(ctx, u8);   /* consumes u8 on every path */
}

/* utf16ToUtf8() on a Text is STRICT, like the free function: it throws
   RangeError on an ill-formed surrogate (the only ill-formedness a JS string
   can hold). Mapping it to plain toUtf8() silently emitted the WTF-8 bytes
   of a lone surrogate where the contract -- and utf16ToUtf8 over BYTES --
   both throw. A well-formed string encodes identically either way, so only
   the ill-formed path changes behavior. */
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    if (!t)
        return JS_EXCEPTION;
    dyn_txt_u16_scan(ctx, t, &valid);
    /* a scan that failed to encode the string has a pending exception
     * (OOM); only a clean scan answers "ill-formed" */
    if (!valid) {
...
static const JSCFunctionListEntry dyn_txt_proto[] = {
    ...
    /*getter, matching Bytes.isValidUtf8 -- same shared helper. */
    ...
    /* On a Text this IS toUtf8 when the string is well-formed; the strict
       wrapper first rejects the one ill-formedness a JS string can carry
       (a lone surrogate), matching the free utf16ToUtf8 over bytes. */
...
static const JSCFunctionListEntry dyn_bh_proto[] = {
    ...
    /* (needle, fromIndex): the forward macros build their own padded argv,
     * so the declared length is arity metadata here -- unlike the FREE
     * functions below, where it gates real argv bounds. */
...
static const JSCFunctionListEntry dyn_bytes_funcs[] = {
    ...
    /*the optional fromIndex is argv[2], read unconditionally
     * (JS_IsUndefined first) -- the declared length MUST be 3 so the engine
     * pads argv when the caller omits it (see the registration note above). */
