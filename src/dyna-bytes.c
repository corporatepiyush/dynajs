                            ...
                            JSValueConst *argv)
{
     ...
     * TSan would find it. */
    {
        ...
        if (!cs) { JS_FreeValue(ctx, str); free(t); return JS_EXCEPTION; }
        t->is_wide = 0;
...
static uint16_t *dyn_txt_u16_of(JSContext *ctx, dyn_txt_t *t, size_t *punits)
{
    ...
    if (JS_IsException(lv)) return NULL;
    if (JS_ToUint32(ctx, &n, lv)) { JS_FreeValue(ctx, lv); return NULL; }
    JS_FreeValue(ctx, lv);
       ...
       "a\u{1F600}" did not. */
    {
        ...
        if (cca == JS_ATOM_NULL) { free(u16); return NULL; }
        for (i = 0; i < n; i++) {
            JSValue idx = JS_NewUint32(ctx, (uint32_t)i);
            JSValue c = JS_Invoke(ctx, t->str, cca, 1, (JSValueConst *)&idx);
            uint32_t v = 0;
            if (JS_IsException(c) || JS_ToUint32(ctx, &v, c)) {
                JS_FreeValue(ctx, c); JS_FreeValue(ctx, idx);
                JS_FreeAtom(ctx, cca); free(u16); return NULL;
            }
            JS_FreeValue(ctx, c);
