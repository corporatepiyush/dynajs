...
static JSValue dyn_bh_fill(JSContext *ctx, JSValueConst this_val, int argc,
    ...
    dyn_bh_dirty(b);
    /* The free fill(buf, ...) returns `buf` (the underlying Uint8Array), and
     * the handle method's documented contract is the SAME return -- "returns
     * the underlying Uint8Array, not the handle". Return the free fill's
     * result instead of dupping `this`. */
    return r;
}
...
static const JSCFunctionListEntry dyn_bh_proto[] = {
    ...
    JS_CFUNC_DEF("indexOf", 1, dyn_bh_index_of),
    JS_CFUNC_DEF("lastIndexOf", 1, dyn_bh_last_index_of),
