...
struct dyn_evloop {
    ...
    /* Set by dyn_evloop_poll when the periodic timer fired in the batch just
     * returned, cleared on every call. A caller whose periodic work exists to
     * cover a LOST kernel wakeup (the XNU listen-socket case) must run it on
     * that tick, not on every readiness batch: doing it per batch costs a
     * syscall per event to discover "nothing pending". */
    int timer_fired;
#ifdef DYN_EV_POLL
...
int dyn_evloop_poll(dyn_evloop_t *lp, int timeout_ms)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        int mask = 0;
        dyn_ev_reg *r;
        if ((int)(uintptr_t)evs[i].ident == DYN_EV_TIMER_IDENT) {
            lp->timer_fired = 1;
            continue;               /* an ident, not a descriptor */
        }
        if (fd < 0 || fd >= lp->cap)
            continue;
        r = &lp->regs[fd];
...
int dyn_evloop_poll(dyn_evloop_t *lp, int timeout_ms)
{
    ...
    lp->timer_fired = 0;   /* poll(2) has no timer primitive: never a tick */
