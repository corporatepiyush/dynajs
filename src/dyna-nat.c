...
/* The prototype an object of `class_id` must get when the [[Construct]] it is
 * being created for runs with new.target = `new_target`
 * (OrdinaryCreateFromConstructor): Get(new_target, "prototype") when that is
 * an object, else the class's own registered proto. This is the same rule the
 * engine's builtins follow (js_resolve_ctor_proto / js_create_from_ctor,
 * vm/interpreter.inc.c) and what makes `class R extends Random {}` produce
 * R instances at all. JS_UNDEFINED (method-created wrappers: no construction
 * is running) and a non-object "prototype" both take the fallback; a THROWING
 * "prototype" getter propagates -- the wrap helpers dispose their native and
 * return JS_EXCEPTION, and since this runs BEFORE any allocation there is
 * nothing to leak. The realm refinement of js_resolve_ctor_proto
 * (JS_GetFunctionRealm) is approximated with the current context: it is not
 * public API, and for the C-function new_targets these classes see it is the
 * same realm anyway. Returns an owned value. */
/* Shared: classes whose object shape keeps them OUTSIDE dyn_res_wrap /
 * dyn_plain_wrap (custom finalizers, multi-value opaques) call this directly
 * to get the same new_target semantics. */
