...
/* A minimal growable byte buffer: DynBuf is engine-internal and a native module
   only gets the public API -- which is why this wraps the PURE-C shared core
   (core/dyn-sb.h, no engine headers) rather than reaching for DynBuf. The
   struct name, the specialized single-byte put and the seed (64) stay. */
...
{
    if (b->n == b->cap
        && !dyn_sb_reserve((void **)&b->p, &b->cap, b->n + 1, 64)) {
        b->oom = 1;
        return;
    }
    b->p[b->n++] = c;
...
/* The multi-byte half of JSON5 whitespace: NBSP (U+00A0), ZWNBSP (U+FEFF) and
 * the Unicode Zs separators (U+1680, U+2000..U+200A, U+202F, U+205F, U+3000).
 * The subject is UTF-8; anything that is one of these is consumed and 1 is
 * returned. Malformed UTF-8 never matches -- the value parser reports it. */
static int dyn_j5_ws_multibyte(dyn_j5_t *p)
{
    const unsigned char *s = (const unsigned char *)p->s;
    size_t n = p->n, i = p->i, left = n - i;

    if (left >= 2 && s[i] == 0xC2 && s[i + 1] == 0xA0) {          /* NBSP */
        p->i += 2; return 1;
    }
    if (left >= 3 && s[i] == 0xEF && s[i + 1] == 0xBB && s[i + 2] == 0xBF) {
        p->i += 3; return 1;                                       /* ZWNBSP */
    }
    if (left >= 3 && s[i] == 0xE1 && s[i + 1] == 0x9A && s[i + 2] == 0x80) {
        p->i += 3; return 1;                                       /* U+1680 */
    }
    if (left >= 3 && s[i] == 0xE2 && s[i + 1] == 0x80) {
        if (s[i + 2] >= 0x80 && s[i + 2] <= 0x8A) {                /* U+2000..A */
            p->i += 3; return 1;
        }
        if (s[i + 2] == 0xAF) {                                    /* U+202F */
            p->i += 3; return 1;
        }
    }
    if (left >= 3 && s[i] == 0xE2 && s[i + 1] == 0x81 && s[i + 2] == 0x9F) {
        p->i += 3; return 1;                                       /* U+205F */
    }
    if (left >= 3 && s[i] == 0xE3 && s[i + 1] == 0x80 && s[i + 2] == 0x80) {
        p->i += 3; return 1;                                       /* U+3000 */
    }
    return 0;
}
...
static int dyn_j5_ws(dyn_j5_t *p)
{
    while (p->i < p->n) {
        ...
        if (c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\f'
            || c == '\v') {
            p->i++;
        } else if ((unsigned char)c >= 0x80 && dyn_j5_ws_multibyte(p)) {
            continue;
...
static JSValue dyn_j5_string(dyn_j5_t *p, char quote)
{
    ...
    while (p->i < p->n && p->s[p->i] != quote) {
        ...
        case '0':
            /* JSON5: \0 is an escape only when NOT followed by a decimal
             * digit -- "\01" is malformed, not a NUL plus a 1. */
            if (p->i < p->n && p->s[p->i] >= '0' && p->s[p->i] <= '9') {
                dyn_buf_free(&b);
                dyn_j5_err(p, "bad escape (\\0 followed by a digit)");
                return JS_EXCEPTION;
            }
            dyn_buf_put(&b, '\0');
            break;
        ...
        case 'x': case 'u':
            ...
            break;
        default:
            /* \1..\9 are not escapes in JSON5 (DecimalDigit is an
             * EscapeCharacter, so it can never be a NonEscapeCharacter). */
            if (c >= '1' && c <= '9') {
                dyn_buf_free(&b);
                dyn_j5_err(p, "bad escape");
                return JS_EXCEPTION;
            }
            dyn_buf_put(&b, c); break;       /* \\ \" \' \/ and the rest */
        }
    }
    if (p->i >= p->n) {
        dyn_buf_free(&b);
...
/* 0x-prefixed integer. JSON5 has no hex FLOAT, so this is exact up to 2^53 --
   and the cap below is what MAKES that comment true: `v = v * 16` used to wrap
   silently past 16 digits, returning garbage for a long hex literal. */
static JSValue dyn_j5_hexnum(dyn_j5_t *p, int sign)
{
    uint64_t v = 0, lim = (uint64_t)1 << 53;
    int any = 0;
    p->i += 2;
    while (p->i < p->n) {
        ...
        if (h < 0)
            break;
        /* v <= 2^53 holds on entry, so v * 16 cannot overflow uint64 here */
        if (v * 16 + (uint64_t)h > lim) {
            dyn_j5_err(p, "hex number exceeds 2^53");
            return JS_EXCEPTION;
        }
        v = v * 16 + (uint64_t)h;
        any = 1;
...
static JSValue dyn_j5_number(dyn_j5_t *p)
{
    ...
    {
        /* The token goes to ToNumber straight from the subject string: the old
           buf[64] staging copy turned every literal of 64+ characters -- a
           perfectly valid long decimal -- into "bad number". ToNumber over a
           string is the engine's own correctly-rounded, locale-independent
           parser; strtod would read LC_NUMERIC for the radix. */
        JSValue sv = JS_NewStringLen(p->ctx, p->s + start, len);
        double d;
        int r;
...
/* One key, decoded to its UTF-16 code units ONCE. RFC 8785 orders keys by
   UTF-16 units: the old comparator re-did this decode in EVERY comparison
   (two mallocs and a full UTF-8 walk per pair), which is quadratic-with-
   allocation on exactly the objects JCS signs. Lone surrogates pass through
   as themselves -- they are valid UTF-16 units even though they are not
   valid Unicode scalar values. */
typedef struct { uint16_t *u; size_t n; } dyn_key16_t;

static int dyn_key_decode(JSContext *ctx, JSAtom a, dyn_key16_t *out)
{
    JSValue v = JS_AtomToString(ctx, a);
    const char *s;
    size_t n = 0, i = 0;
    ...
    while (i < n) {
        uint32_t cp = dyn_u8_next(s, n, &i);     /* advances i */
        if (cp < 0x10000)
            out->u[out->n++] = (uint16_t)cp;
        else {
            cp -= 0x10000;
            out->u[out->n++] = (uint16_t)(0xD800 | (cp >> 10));
            out->u[out->n++] = (uint16_t)(0xDC00 | (cp & 0x3FF));
        }
    }
    JS_FreeCString(ctx, s);
    return 0;
...
static int dyn_key16_less(const dyn_key16_t *a, const dyn_key16_t *b)
{
    size_t k = 0, m = a->n < b->n ? a->n : b->n;
    while (k < m) {
        if (a->u[k] != b->u[k])
            return a->u[k] < b->u[k];
        k++;
    }
    return a->n < b->n;                          /* shorter key sorts first */
}
...
static int dyn_ser_object(dyn_ser_t *w, JSValueConst v)
{
    ...
    if (JS_GetOwnPropertyNames(w->ctx, &tab, &len, v,
        ...
        return -1;
    if (w->stable && len > 1) {            /* insertion sort: key sets are small */
        dyn_key16_t *ks = (dyn_key16_t *)malloc((size_t)len * sizeof *ks);
        if (!ks) {
            JS_ThrowOutOfMemory(w->ctx);
            JS_FreePropertyEnum(w->ctx, tab, len);
            return -1;
        }
        for (i = 0; i < len; i++) {
            if (dyn_key_decode(w->ctx, tab[i].atom, &ks[i]) == 0)
                continue;
            for (j = 0; j < i; j++)
                free(ks[j].u);
            free(ks);
        ...
        for (i = 1; i < len; i++) {
            ...
            dyn_key16_t k = ks[i];
            j = i;
            while (j > 0 && dyn_key16_less(&k, &ks[j - 1])) {
                tab[j] = tab[j - 1];
                ks[j] = ks[j - 1];
                j--;
            }
    ...
    for (i = 0; i < len; i++) {
        ...
        if (JS_IsException(pv))
            goto done;
        if (JS_IsUndefined(pv) || JS_IsFunction(w->ctx, pv)) {
            /* omitted, as JSON.stringify does: undefined AND function/symbol
             * members vanish from objects (arrays still print them as null). */
...
static int dyn_ser_value(dyn_ser_t *w, JSValueConst v)
{
    ...
    if (JS_IsBigInt(w->ctx, v)) {
        /* JSON.stringify and the JSON5 reference both throw on BigInt; printing
         * "null" here would silently corrupt a canonical document. */
        JS_ThrowTypeError(w->ctx, "%s: cannot serialize a BigInt",
