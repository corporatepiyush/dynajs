                                                      ...
                                                      JSObject *p, int flags)
{
    ...
    *plen = 0;
    ...
    /* FAST PATH for the shape every ordinary caller asks for: own enumerable
       string keys, in insertion order. Object.keys/values/entries/assign,
       JSON.stringify, for-in and spread all land here, and the general path
       below costs two shape passes, an array-index test and a kind lookup per
       property, and a sort -- 30 ns per key before anything is returned.
       Refuses (falls through) wherever that work is load-bearing: an exotic
       object, any integer key (which the general path sorts FIRST), or a
       symbol (which needs the kind classification). */
    if (flags == (JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY)
        && !p->is_exotic && !p->fast_array) {
        JSShape *fsh = p->shape;
        JSShapeProperty *fprs = get_shape_prop(fsh);
        int fi, ok = 1, fn = 0;
        for (fi = 0; fi < fsh->prop_count; fi++, fprs++) {
            JSAtom a = fprs->atom;
            if (a == JS_ATOM_NULL)
                continue;
            if (__JS_AtomIsTaggedInt(a)
                || (fprs->flags & JS_PROP_TMASK) == JS_PROP_VARREF
                || JS_AtomGetKind(ctx, a) != JS_ATOM_KIND_STRING) {
                ok = 0;
                break;
            }
            if (fprs->flags & JS_PROP_ENUMERABLE)
                fn++;
        }
        if (ok) {
            JSPropertyEnum *ft = js_malloc(ctx, sizeof(*ft) * max_int(fn, 1));
            int fk = 0;
            if (!ft)
                return -1;
            fprs = get_shape_prop(fsh);
            for (fi = 0; fi < fsh->prop_count; fi++, fprs++) {
                if (fprs->atom == JS_ATOM_NULL
                    || !(fprs->flags & JS_PROP_ENUMERABLE))
                    continue;
                ft[fk].is_enumerable = TRUE;
                ft[fk++].atom = JS_DupAtom(ctx, fprs->atom);
            }
            *ptab = ft;
            *plen = (uint32_t)fk;
            return 0;
...
/* Own enumerable STRING keys of a plain object, in insertion order, into a
   caller buffer: one shape pass, no table malloc, no sort. Refuses (-1) for an
   exotic object or any integer key, where JS_GetOwnPropertyNames reorders.
   Atoms are DUP'd exactly as that function does -- the caller frees each.
   -2 means the buffer is short and *plen is the count to allocate. */
                       ...
                       uint32_t buf_size, uint32_t *plen)
{
    ...
    *plen = 0;
    ...
    if (p->is_exotic || p->fast_array)
        return -1;
    sh = p->shape;
    if ((uint32_t)sh->prop_count > buf_size) {
        *plen = sh->prop_count;         /* an upper bound: the caller allocates */
        return -2;                      /* and retries -- NOT a refusal */
    }
    for (i = 0, prs = get_shape_prop(sh); i < sh->prop_count; i++, prs++) {
...
static int JS_TryGetPropertyInt64(JSContext *ctx, JSValueConst obj, int64_t idx, JSValue *pval)
{
    ...
    /* Fast path: a dense element of a fast array. In fast_array mode every
       index below u.array.count holds a present value (convert_fast_array_to_-
       array() zeroes count when leaving that mode), so the JS_HasProperty walk
       below is redundant -- this is the same shortcut JS_GetPropertyValue()
       takes. Without it every element of map/filter/forEach/every/some pays two
       full property-machinery walks instead of one array index. */
    if (likely(JS_VALUE_GET_TAG(obj) == JS_TAG_OBJECT)) {
        JSObject *p = JS_VALUE_GET_OBJ(obj);
        if (likely(p->class_id == JS_CLASS_ARRAY &&
                   (uint64_t)idx < p->u.array.count)) {
            *pval = JS_DupValue(ctx, p->u.array.u.values[idx]);
            return TRUE;
        }
    }
                                                             ...
                                                             JSObject *p)
{
    ...
    js_array_pool_free(ctx->rt, p); /* class-gated: only JS_CLASS_ARRAY pools */
#else
...
static int delete_property(JSContext *ctx, JSObject *p, JSAtom atom)
{
 ...
 redo:
    sh = p->shape;
    h1 = prop_hash_bucket(atom, sh->prop_hash_mask);
    h = sh->hash_table[h1];
...
static int expand_fast_array(JSContext *ctx, JSObject *p, uint32_t new_len)
{
    ...
    if (unlikely(new_size == 0)) {
        /* new_len == 0 and nothing reserved yet: nothing to grow into.
           Must be a no-op SUCCESS: js_realloc2(ptr, 0) frees the buffer
           and returns NULL without throwing, so falling through would
           return -1 with no pending exception -- the caller reports
           failure and the VM raises a stale JS_TAG_UNINITIALIZED value
           (seen as `[...[], ...[], 1]` throwing "uninitialized" after
           the js_append_enumerate bulk path started calling this with
           pos + count32 == 0). -1 now implies a pending exception for
           every caller. */
        return 0;
    }
#ifdef CONFIG_OBJ_POOL
    if (p->u.array.u.values == NULL) {
        /* initial growth: serve from the recycle cache when a bucket
           fits (see js_array_pool_alloc); the bucket becomes u1.size */
        uint32_t pooled_size;
        JSValue *pooled = js_array_pool_alloc(ctx->rt, new_size, &pooled_size);
        if (pooled) {
            p->u.array.u.values = pooled;
            p->u.array.u1.size = pooled_size;
            return 0;
        }
    }
#endif
