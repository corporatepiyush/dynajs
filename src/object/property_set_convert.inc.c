                                   ...
                                   JSShapeProperty **pprs)
{
    ...
    /* an in-place update may re-flag any slot (e.g. a value property
       converted to a getter, or flag-lowering freeze/seal walks); the
       all-plain teardown guarantee is dropped conservatively */
    sh->all_plain = FALSE;
#endif
    if (sh->is_hashed) {
...
static int JS_ToBoolFree(JSContext *ctx, JSValue val)
{
    ...
    switch(tag) {
    /* Tags ordered hottest-first. Every arm returns or breaks. */
    case JS_TAG_INT:
    ...
    case JS_TAG_OBJECT:
        ...
        break;
    /* bool, null and undefined share their int payload */
    case JS_TAG_BOOL:
    case JS_TAG_NULL:
    case JS_TAG_UNDEFINED:
...
/* below this size (in limbs) the schoolbook multiplication
   (mp_mul_basecase) is faster than the Karatsuba multiplication */
#define MP_MUL_KARA_THRESHOLD 32

/* Karatsuba multiplication recursion guard. The maximum bigint size is
   JS_BIGINT_MAX_SIZE limbs so this depth is never reached in
   practice. When it is reached (it cannot happen), the schoolbook
   multiplication is used, so the result stays correct. */
#define MP_MUL_KARA_MAX_DEPTH 64

static js_limb_t mp_max_limb(js_limb_t a, js_limb_t b)
{
    return a > b ? a : b;
}

/* return the scratch size (in limbs) needed by mp_mul_karatsuba() for
   a multiplication of 'na' by 'nb' limbs */
static js_limb_t mp_mul_kara_scratch_size(js_limb_t na, js_limb_t nb)
{
    js_limb_t m, s1, s2, s3, slen1;
    ...
    if (nb <= MP_MUL_KARA_THRESHOLD)
        return 0;
    if (na >= 2 * nb) {
        /* 'op1' is cut into chunks of 'nb' limbs: one temporary of
           2*nb limbs is needed for the chunk products */
        s1 = mp_mul_kara_scratch_size(nb, nb);
        if (na % nb != 0)
            s2 = mp_mul_kara_scratch_size(nb, na % nb);
        else
            s2 = 0;
        return 2 * nb + mp_max_limb(s1, s2);
    }
    m = (nb + 1) / 2;
    /* sum1 needs max(na-m, m)+1 limbs, sum2 needs m+1 limbs and the
       product of the sums needs slen1+(m+1) limbs (see
       mp_mul_karatsuba) */
    slen1 = mp_max_limb(na - m, m) + 1;
    s1 = mp_mul_kara_scratch_size(m, m);
    s2 = mp_mul_kara_scratch_size(na - m, nb - m);
    s3 = mp_mul_kara_scratch_size(slen1, m + 1);
    return 2 * (slen1 + m + 1) + mp_max_limb(s1, mp_max_limb(s2, s3));
}

/* result[0..na+nb-1] = op1[0..na-1] * op2[0..nb-1] (exact product).
   1 <= nb <= na. 'scratch' must contain at least
   mp_mul_kara_scratch_size(na, nb) limbs and is clobbered. */
static void mp_mul_karatsuba(js_limb_t *result,
                             const js_limb_t *op1, js_limb_t na,
                             const js_limb_t *op2, js_limb_t nb,
                             js_limb_t *scratch, int depth)
{
    js_limb_t m, nh1, nh2, carry;
    
    if (na < nb) {
        const js_limb_t *tmp_op = op1;
        js_limb_t tmp_n = na;
        op1 = op2;
        na = nb;
        op2 = tmp_op;
    ...
    if (nb <= MP_MUL_KARA_THRESHOLD || depth >= MP_MUL_KARA_MAX_DEPTH) {
        mp_mul_basecase(result, op1, na, op2, nb);
        return;
    }
    if (na >= 2 * nb) {
        /* unbalanced multiplication: cut 'op1' into 'nb'-sized chunks
           and multiply each chunk by 'op2' */
        js_limb_t *tmp = scratch; /* 2*nb limbs */
        js_limb_t nchunks = na / nb, rem = na % nb;
        js_limb_t i, off, off2;
        js_limb_t *srec = scratch + 2 * nb;
        
        /* the chunk products are added with overlapping windows: the
           limbs of 'result' above the first chunk product are read
           (by mp_add) before being written, so they must be zeroed */
        memset(result + 2 * nb, 0, (na - nb) * sizeof(js_limb_t));
        mp_mul_karatsuba(tmp, op1, nb, op2, nb, srec, depth + 1);
        memcpy(result, tmp, 2 * nb * sizeof(js_limb_t));
        for(i = 1; i < nchunks; i++) {
            off = i * nb;
            mp_mul_karatsuba(tmp, op1 + off, nb, op2, nb, srec, depth + 1);
            carry = mp_add(result + off, result + off, tmp, 2 * nb, 0);
            /* the carry may propagate into the chunks not yet added */
            for(off2 = off + 2 * nb; carry != 0 && off2 < na + nb; off2++) {
                result[off2]++;
                carry = (result[off2] == 0);
            }
        }
        if (rem != 0) {
            off = nchunks * nb;
            mp_mul_karatsuba(tmp, op2, nb, op1 + off, rem, srec, depth + 1);
            carry = mp_add(result + off, result + off, tmp, rem + nb, 0);
            for(off2 = off + rem + nb; carry != 0 && off2 < na + nb; off2++) {
                result[off2]++;
                carry = (result[off2] == 0);
            }
        }
    ...
    /* balanced multiplication: na < 2*nb. Writing op1 = h1*B^m + l1
       and op2 = h2*B^m + l2 (m = ceil(nb/2), l1 = m limbs, h2 = nb-m
       <= m limbs):
       op1*op2 = z0 + (z1 << m) + (z2 << 2m)
         z0 = l1*l2, z2 = h1*h2, z1 = (l1+h1)*(l2+h2) - z0 - z2
       which needs 3 multiplications of half-size operands. */
    {
        js_limb_t slen1, slen2, plen, i;
        js_limb_t *sum1, *sum2, *prod, *srec;
        
        m = (nb + 1) / 2;
        nh1 = na - m;
        nh2 = nb - m;
        slen1 = mp_max_limb(nh1, m) + 1;
        slen2 = m + 1;
        plen = slen1 + slen2;
        sum1 = scratch;
        sum2 = sum1 + slen1;
        ...
        /* z0 = l1*l2 */
        mp_mul_karatsuba(result, op1, m, op2, m, srec, depth + 1);
        /* z2 = h1*h2 */
        mp_mul_karatsuba(result + 2 * m, op1 + m, nh1, op2 + m, nh2,
                         srec, depth + 1);
        /* sum1 = l1 + h1 (slen1 limbs), sum2 = l2 + h2 (slen2 limbs) */
        memset(sum1, 0, slen1 * sizeof(js_limb_t));
        memcpy(sum1, op1 + m, nh1 * sizeof(js_limb_t));
        carry = mp_add(sum1, sum1, op1, m, 0);
        if (nh1 > m) {
            for(i = m; carry != 0 && i < nh1; i++) {
                sum1[i]++;
                carry = (sum1[i] == 0);
            }
        }
        sum1[slen1 - 1] = carry;
        memset(sum2, 0, slen2 * sizeof(js_limb_t));
        memcpy(sum2, op2 + m, nh2 * sizeof(js_limb_t));
        carry = mp_add(sum2, sum2, op2, m, 0);
        /* nh2 <= m and slen2 = m+1 so exactly one carry limb */
        sum2[slen2 - 1] = carry;
        /* prod = (l1+h1)*(l2+h2), plen = slen1+slen2 limbs */
        mp_mul_karatsuba(prod, sum1, slen1, sum2, slen2, srec, depth + 1);
        /* prod -= z0 (2m limbs) and z2 (nh1+nh2 limbs). The result is
           non-negative and fits in plen limbs, however the low limbs
           of prod can be smaller than z0 (resp. z2) so the borrow
           must propagate through the high limbs of prod */
        carry = mp_sub(prod, prod, result, 2 * m, 0);
        for(i = 2 * m; carry != 0 && i < plen; i++) {
            prod[i]--;
            carry = (prod[i] == (js_limb_t)-1);
        }
        carry = mp_sub(prod, prod, result + 2 * m, nh1 + nh2, 0);
        for(i = nh1 + nh2; carry != 0 && i < plen; i++) {
            prod[i]--;
            carry = (prod[i] == (js_limb_t)-1);
        }
        /* result += prod << m; the carry may propagate into the high
           limbs of z2 (the final result fits in na+nb limbs so it
           terminates inside the result) */
        carry = mp_add(result + m, result + m, prod, plen, 0);
        for(i = m + plen; carry != 0 && i < na + nb; i++) {
            result[i]++;
            carry = (result[i] == 0);
        }
    }
...
/* multiply 'op1' (n1 limbs) by 'op2' (n2 limbs) into 'result'
   (n1+n2 limbs), choosing between the schoolbook and the Karatsuba
   multiplication. Return FALSE in case of memory allocation failure. */
static BOOL js_mp_mul(JSContext *ctx, js_limb_t *result,
                      const js_limb_t *op1, js_limb_t n1,
                      const js_limb_t *op2, js_limb_t n2)
{
    js_limb_t *scratch, s;
    ...
    s = mp_mul_kara_scratch_size(n1, n2);
    if (s == 0) {
        mp_mul_basecase(result, op1, n1, op2, n2);
        return TRUE;
    }
    scratch = js_malloc(ctx, s * sizeof(js_limb_t));
    if (!scratch)
        return FALSE;
    mp_mul_karatsuba(result, op1, n1, op2, n2, scratch, 0);
    js_free(ctx, scratch);
    return TRUE;
}
...
{
    JSBigInt *r;
    const JSBigInt *a1, *b1;
    js_limb_t na, nb;
    
    r = js_bigint_new(ctx, a->len + b->len);
    if (!r)
        return NULL;
    /* mp_mul_karatsuba() expects na >= nb; the schoolbook product of
       the raw limb patterns is the same regardless of the operand
       order */
    if (a->len >= b->len) {
        a1 = a;
        b1 = b;
    ...
    na = a1->len;
    nb = b1->len;
    if (!js_mp_mul(ctx, r->tab, a1->tab, na, b1->tab, nb)) {
        js_free(ctx, r);
        return NULL;
    }
...
/* below this size (in limbs) the repeated division by radix_base is
   faster than the divide-and-conquer conversion */
#define JS_BIGINT_TOSTR_DC_THRESHOLD 64
/* divide-and-conquer conversion recursion guard (the recursion depth
   is at most log2(JS_BIGINT_MAX_SIZE*digits_per_limb) so it is never
   reached; when it is, the quadratic loop is used, so the result
   stays correct) */
#define JS_BIGINT_TOSTR_MAX_DEPTH 64
/* powers of radix_base: radix_base^(2^s) for s = 0..14 covers
   2^14 chunks of digits_per_limb digits, more than the maximum bigint
   size in every radix */
#define JS_BIGINT_TOSTR_POW_MAX 16

typedef struct JSToStrPow {
    js_limb_t *tab;
    int len;
} JSToStrPow;

/* compare the integers taba[0..na) and tabb[0..nb) (leading zero
   limbs allowed). Return -1, 0 or 1. */
static int mp_cmp_lc(const js_limb_t *taba, int na,
                     const js_limb_t *tabb, int nb)
{
    int i;
    
    while (na > 1 && taba[na - 1] == 0)
        na--;
    while (nb > 1 && tabb[nb - 1] == 0)
        nb--;
    if (na != nb)
    ...
    for(i = na - 1; i >= 0; i--) {
        if (taba[i] != tabb[i])
            return taba[i] < tabb[i] ? -1 : 1;
    }
    return 0;
}

/* quotient/remainder of a multi-limb division: q = a / b (qlen
   limbs), r = a mod b (rlen limbs), both stripped (at least 1
   limb). 'taba' is not modified. Returns FALSE in case of memory
   allocation failure. */
static BOOL js_bigint_dc_divmod(JSContext *ctx,
                                js_limb_t **ptabq, int *plenq,
                                js_limb_t **ptabr, int *plenr,
                                const js_limb_t *taba, int na,
                                const js_limb_t *tabb, int nb)
{
    js_limb_t *r, *q, *b2, h, shift;
    int i;
    
    r = js_malloc(ctx, (na + 2) * sizeof(js_limb_t));
    if (!r)
        return FALSE;
    memcpy(r, taba, na * sizeof(js_limb_t));
    while (na > 1 && r[na - 1] == 0)
        na--;
    while (nb > 1 && tabb[nb - 1] == 0)
        nb--;
    if (na < nb) {
        /* q = 0, r = a */
        q = js_malloc(ctx, sizeof(js_limb_t));
        if (!q) {
            js_free(ctx, r);
            return FALSE;
        }
    ...
    /* normalize the divisor so that its top limb has its high bit set */
    shift = js_limb_clz(tabb[nb - 1]);
    b2 = (js_limb_t *)tabb;
    if (shift != 0) {
        b2 = js_malloc(ctx, nb * sizeof(js_limb_t));
        if (!b2) {
            js_free(ctx, r);
            return FALSE;
        }
        mp_shl(b2, tabb, nb, shift);
        h = mp_shl(r, r, na, shift);
        if (h != 0)
            r[na++] = h;
    }
    q = js_malloc(ctx, (na - nb + 2) * sizeof(js_limb_t));
    if (!q) {
        if (b2 != tabb)
            js_free(ctx, b2);
        js_free(ctx, r);
...
/* free the powers of radix_base */
static void js_bigint_tostr_free_pows(JSContext *ctx, JSToStrPow *powers,
                                      int npows)
{
    int i;
    for(i = 0; i < npows; i++)
        js_free(ctx, powers[i].tab);
}

/* write the digits of 'tab' (destroyed but not freed) right-to-left
   at '*ppq' using a divide-and-conquer split by powers of radix_base
   ('n_chunks' chunks of 'dpl' digits; if 'is_top' is TRUE the leading
   zero chunks are skipped instead of being written as zeros). If a
   memory allocation fails, the quadratic conversion is used for the
   current node instead (so the output is always correct and this
   function never fails). */
static void js_bigint_tostr_dc(JSContext *ctx, char **ppq, js_limb_t *tab,
                               int len, js_limb_t n_chunks, BOOL is_top,
                               unsigned int radix, js_limb_t radix_base,
                               int dpl, const JSToStrPow *powers, int npows,
                               int depth)
{
    char *q = *ppq;
    
    if (depth >= JS_BIGINT_TOSTR_MAX_DEPTH ||
        len < JS_BIGINT_TOSTR_DC_THRESHOLD || n_chunks <= 2) {
        js_limb_t v;
        if (is_top) {
            for(;;) {
                /* remove leading zero limbs */
                while (len > 1 && tab[len - 1] == 0)
                    len--;
                if (len == 1 && tab[0] < radix_base) {
                    v = tab[0];
                    if (v != 0) {
                    ...
                    v = mp_div1(tab, tab, len, radix_base, 0);
                    q = limb_to_a(q, v, radix, dpl);
                }
            }
        } else {
            for(; n_chunks > 0; n_chunks--) {
                v = mp_div1(tab, tab, len, radix_base, 0);
                q = limb_to_a(q, v, radix, dpl);
            }
        }
    ...
    /* split n_chunks = (n_chunks - h) + h with h the largest power of
       two < n_chunks and divide by radix_base^h: the quotient has
       n_chunks-h chunks and the remainder exactly h chunks */
    {
        int s, qlen, rlen;
        js_limb_t h;
        js_limb_t *tabq, *tabr;
        ...
        s = 0;
        while (s + 1 < npows && (2 << s) < n_chunks)
            s++;
        h = (js_limb_t)1 << s;
        if (!js_bigint_dc_divmod(ctx, &tabq, &qlen, &tabr, &rlen,
                                 tab, len, powers[s].tab, powers[s].len)) {
            /* memory allocation failure: fall back to the quadratic
               conversion ('tab' is still intact, nothing was
               written) */
            js_bigint_tostr_dc(ctx, ppq, tab, len, n_chunks, is_top, radix,
                               radix_base, dpl, powers, npows,
                               JS_BIGINT_TOSTR_MAX_DEPTH);
            return;
        }
        /* 'tab' is not freed here: the caller owns it (it is either
           the buffer allocated in js_bigint_tostr_dc_init or the
           tabq/tabr buffer allocated by the caller's divmod) */
        /* the low digits are written first (the buffer is filled
           right-to-left) */
        js_bigint_tostr_dc(ctx, ppq, tabr, rlen, h, FALSE, radix,
                           radix_base, dpl, powers, npows, depth + 1);
        js_free(ctx, tabr);
        js_bigint_tostr_dc(ctx, ppq, tabq, qlen, n_chunks - h, is_top,
                           radix, radix_base, dpl, powers, npows, depth + 1);
        js_free(ctx, tabq);
    }
}

/* convert 'r' (len limbs, not modified, must be != 0) to digits in the
   given radix using a divide-and-conquer conversion. Return FALSE if
   a memory allocation failed before any digit was written (the caller
   then falls back to the quadratic conversion). */
static BOOL js_bigint_tostr_dc_init(JSContext *ctx, char **ppq,
                                    const js_limb_t *tab, int len,
                                    js_limb_t n_chunks, unsigned int radix,
                                    js_limb_t radix_base, int dpl)
{
    JSToStrPow powers[JS_BIGINT_TOSTR_POW_MAX];
    js_limb_t *tmp, *P, *cand;
    int npows, i, l, Plen, clen, s;
    js_limb_t k;
    
    tmp = js_malloc(ctx, len * sizeof(js_limb_t));
    if (!tmp)
        return FALSE;
    memcpy(tmp, tab, len * sizeof(js_limb_t));
    /* build the powers radix_base^(2^s) by successive squaring */
    npows = 1;
    while ((1 << npows) < n_chunks && npows < JS_BIGINT_TOSTR_POW_MAX)
        npows++;
    powers[0].tab = js_malloc(ctx, sizeof(js_limb_t));
    if (!powers[0].tab) {
        js_free(ctx, tmp);
        return FALSE;
    }
    powers[0].tab[0] = radix_base;
    powers[0].len = 1;
    for(i = 1; i < npows; i++) {
        l = powers[i - 1].len;
        powers[i].tab = js_malloc(ctx, 2 * l * sizeof(js_limb_t));
        if (!powers[i].tab) {
            js_bigint_tostr_free_pows(ctx, powers, i);
            js_free(ctx, tmp);
            return FALSE;
        }
        if (!js_mp_mul(ctx, powers[i].tab, powers[i - 1].tab, l,
                       powers[i - 1].tab, l)) {
            js_free(ctx, powers[i].tab);
            js_bigint_tostr_free_pows(ctx, powers, i);
            js_free(ctx, tmp);
        ...
        powers[i].len = 2 * l;
        while (powers[i].len > 1 && powers[i].tab[powers[i].len - 1] == 0)
            powers[i].len--;
    }
    /* 'n_chunks' was computed from 'n_digits' which is only an upper
       bound of the number of digits when the radix is not a power of
       two. Compute the exact number of chunks: the divide-and-conquer
       split needs it to put exactly 'h' chunks in the low part,
       otherwise the leading digits of the result are wrong. The
       exact value is k+1 where k = floor(log_radix_base(value)),
       computed by binary decomposition using the powers table. */
    P = js_malloc(ctx, (len + 1) * sizeof(js_limb_t));
    if (!P) {
        js_bigint_tostr_free_pows(ctx, powers, npows);
        js_free(ctx, tmp);
        return FALSE;
    }
    cand = js_malloc(ctx, (2 * len + 2) * sizeof(js_limb_t));
    if (!cand) {
        js_free(ctx, P);
        js_bigint_tostr_free_pows(ctx, powers, npows);
        js_free(ctx, tmp);
    ...
    k = 0;
    for(s = npows - 1; s >= 0; s--) {
        /* cand = P * powers[s] = radix_base^(k + 2^s) */
        if (!js_mp_mul(ctx, cand, P, Plen,
                       powers[s].tab, powers[s].len)) {
            js_free(ctx, P);
            js_free(ctx, cand);
            js_bigint_tostr_free_pows(ctx, powers, npows);
            js_free(ctx, tmp);
        ...
        clen = Plen + powers[s].len;
        while (clen > 1 && cand[clen - 1] == 0)
            clen--;
        if (mp_cmp_lc(tmp, len, cand, clen) >= 0) {
            memcpy(P, cand, clen * sizeof(js_limb_t));
            Plen = clen;
            k += (js_limb_t)1 << s;
        }
    }
    js_free(ctx, P);
    js_free(ctx, cand);
    /* the value has exactly k+1 chunks (k+1 <= n_chunks since
       n_chunks was an upper bound) */
    n_chunks = k + 1;
    js_bigint_tostr_dc(ctx, ppq, tmp, len, n_chunks, TRUE, radix,
                       radix_base, dpl, powers, npows, 0);
    js_bigint_tostr_free_pows(ctx, powers, npows);
    js_free(ctx, tmp);
    return TRUE;
}
...
static JSValue js_bigint_to_string1(JSContext *ctx, JSValueConst val, int radix)
{
    if (JS_VALUE_GET_TAG(val) == JS_TAG_SHORT_BIG_INT) {
        ...
        *--q = '\0';
        ...
        if (!is_binary_radix) {
            int len, dpl;
            js_limb_t radix_base, v, n_chunks;
            radix_base = radix_base_table[radix - 2];
            dpl = digits_per_limb_table[radix - 2];
            len = r->len;
            n_chunks = ((js_limb_t)n_digits + dpl - 1) / dpl;
            if (len >= JS_BIGINT_TOSTR_DC_THRESHOLD && n_chunks > 2 &&
                js_bigint_tostr_dc_init(ctx, &q, r->tab, len, n_chunks,
                                        radix, radix_base, dpl)) {
                /* done (divide and conquer conversion) */
            } else {
            for(;;) {
                /* remove leading zero limbs */
                                   ...
                                   JSToNumberHintEnum flag)
{
 ...
 redo:
    ...
    switch(tag) {
    /* Tags ordered hottest-first; bigint led this switch and is the rarest
     * operand of the lot. Every arm breaks or returns. */
    /* already numeric: the whole point of the fast path */
    case JS_TAG_FLOAT64:
    case JS_TAG_INT:
    case JS_TAG_EXCEPTION:
        ret = val;
        break;
    /* valueOf/Symbol.toPrimitive, then retry */
    case JS_TAG_OBJECT:
    ...
    case JS_TAG_STRING_ROPE:
        ...
        break;
    /* bool and null both convert via their int payload */
    case JS_TAG_BOOL:
    case JS_TAG_NULL:
    ...
    case JS_TAG_UNDEFINED:
        ret = JS_NAN;
        break;
    /* bigint: rejected unless the caller asked for numeric */
    case JS_TAG_BIG_INT:
    case JS_TAG_SHORT_BIG_INT:
...
static void js_print_object(JSPrintValueState *s, JSObject *p)
{
    ...
    if (p->class_id == JS_CLASS_ARRAY) {
        ...
        if (p->fast_array) {
            ...
            for(i = 0; i < len1; i++) {
                /* js_print_value() runs arbitrary user code (an element's
                   toString), which can resize the array (reallocating u.values)
                   or convert it to a normal object (freeing u.values and
                   clearing fast_array). Re-validate the fast-array invariant
                   on every iteration (CVE-2025-62490). */
                if (unlikely(!p->fast_array || i >= p->u.array.count))
                    break;
                js_print_comma(s, &comma_state);
                js_print_value(s, p->u.array.u.values[i]);
            }
            if (p->fast_array && i < p->u.array.count)
                js_print_more_items(s, &comma_state, p->u.array.count - i);
            if (p->fast_array && p->u.array.count < len) {
        ...
        if (!s->options.raw_dump && s->ctx) {
            ...
            js_putc(s, ' ');
            /* OWN-property shape read, NOT get_prop_string: console.log must
               never execute a `name` GETTER (re-entrancy into user JS from a
               diagnostic path) */
            {
                JSProperty *pr;
                JSValue self = JS_MKPTR(JS_TAG_OBJECT, p);
                JSShapeProperty *prs = find_own_property(&pr,
                    JS_VALUE_GET_OBJ(self), JS_ATOM_name);
                if (prs && (prs->flags & JS_PROP_TMASK) == JS_PROP_NORMAL
                    && JS_IsString(pr->u.value)) {
                    func_name_str = JS_ToCString(s->ctx, pr->u.value);
                } else {
                    func_name_str = NULL;
                }
            }
                          ...
                          JSStrictEqModeEnum eq_mode)
{
    ...
    tag2 = JS_VALUE_GET_NORM_TAG(op2);
    switch(tag1) {
    /* Tags ordered hottest-first: numbers and object identity dominate ===.
     * Every arm breaks or gotos, so the order is free. */
    /* int + float64: the numeric fast path. INT gotos number_test, whose label lives in the FLOAT64 block -- keep these two adjacent */
    case JS_TAG_INT:
        d1 = JS_VALUE_GET_INT(op1);
        if (tag2 == JS_TAG_INT) {
            d2 = JS_VALUE_GET_INT(op2);
    ...
    number_test:
        ...
        break;
    /* reference identity -- the other dominant === operand */
    case JS_TAG_OBJECT:
        if (tag1 != tag2)
            res = FALSE;
        else
...
static __exception int js_operator_typeof(JSContext *ctx, JSValueConst op1)
{
    ...
    switch(tag) {
    /* Tags ordered hottest-first. Every arm breaks, so the order is free. */
...
static __exception int js_for_of_next(JSContext *ctx, JSValue *sp, int offset)
{
    ...
    if (likely(!JS_IsUndefined(sp[offset]))) {
        /* fast path: built-in array iterator over an object with fast
           array elements (mirrors js_array_iterator_next()) */
        if (unlikely(!js_for_of_next_fast_array(ctx, sp, offset,
                                                &value, &done))) {
            value = JS_IteratorNext(ctx, sp[offset], sp[offset + 1],
                                    0, NULL, &done);
...
static __exception int js_for_await_of_next(JSContext *ctx, JSValue *sp)
{
    ...
    /* x3/fix1: while the iteration machinery runs, the loop's catch offset
       (sp[-1]) holds -1, which the unwinder treats as "next()-originated
       abrupt completion": set [[Done]], NO AsyncIteratorClose, rethrow
       immediately (ForIn/OfBodyEvaluation: a throw from IteratorStepValue
       only marks the iterator done). The parser restores the async-close
       handler pos via OP_for_await_catch_body right after the machinery.
       (The x3 code before this pointed the marker at the async-close
       handler for machinery throws too - one extra awaited-close hop and a
       spurious .return() on a rejecting next().) */
    iter = sp[-3];
    next = sp[-2];
    sp[-1] = JS_NewCatchOffset(ctx, -1);
...
static __exception int js_iterator_get_value_done(JSContext *ctx, JSValue *sp)
{
    ...
    if (JS_IsException(value))
        return -1;
    JS_FreeValue(ctx, obj);
    /* x3/fix1: the catch-offset slot (now at sp[-2], still holding -1 from
       js_for_await_of_next) is left untouched here - a throw from this
       opcode is still machinery-originated (no iterator close; node does
       not call .return() when next() resolves to a non-object). The parser
       emits OP_for_await_catch_body right after this opcode to point the
       marker back at the async-close handler for body-originated throws. */
    sp[-1] = value;
    sp[0] = JS_NewBool(ctx, done);
    return 0;
}

/* Fresh {value, done} iterator result at the per-context cached shape
   ctx->iterator_result_shape (regexp_result_shape pattern): identical
   observable object to a JS_NewObject() + two C_W_E defines -- plain
   object, Object.prototype, own "value" then "done" in that order --
   without the per-call shape transition and property-hash machinery
   (~65% of .next() time; see lldb_before.txt).  Fresh identity per
   call is preserved: a new JSObject every time, and callers may freely
   mutate or extend a result afterwards (a first mutation just copies
   the shared shape, ref_count COW).  The property slots created from
   the shape are uninitialized; both are stored immediately below and
   no allocation happens in between, so the GC cannot observe garbage
   (js_object_presize_new() uses the same post-alloc seed pattern). */
static JSValue js_new_iterator_result(JSContext *ctx, JSValue val,
                                      BOOL done)
{
    JSValue obj;
    JSObject *p;

    if (likely(ctx->iterator_result_shape)) {
        obj = JS_NewObjectFromShape(ctx,
                                    js_dup_shape(ctx->iterator_result_shape),
                                    JS_CLASS_OBJECT, NULL);
        if (likely(!JS_IsException(obj))) {
            p = JS_VALUE_GET_OBJ(obj);
            p->prop[0].u.value = val;
            p->prop[1].u.value = JS_NewBool(ctx, done);
            return obj;
        }
        JS_FreeValue(ctx, val);
        return obj;
    }
    /* no cached shape (raw context without intrinsics): build the
...
static JSValue js_create_iterator_result(JSContext *ctx,
                                         ...
                                         BOOL done)
{
    return js_new_iterator_result(ctx, val, done);
}
...
static __exception int js_append_enumerate(JSContext *ctx, JSValue *sp)
{
     ...
     */
    /* x5: GetMethod(@@iterator) must be observed exactly ONCE (spec
       GetIterator / IterableToArrayCreate). The method fetched here is
       both the builtin-check input and the one invoked, so a getter on
       @@iterator runs once and the FIRST returned iterator is the one
       iterated (previously JS_GetIterator() below re-fetched the
       property and iterated the second result). */
    ...
    if (unlikely(!JS_IsFunction(ctx, iterator))) {
        JS_FreeValue(ctx, iterator);
        JS_ThrowTypeError(ctx, "value is not iterable");
        return -1;
    }

    enumobj = JS_GetIterator2(ctx, sp[-1], iterator);
    JS_FreeValue(ctx, iterator);
    ...
    &&  js_get_fast_array(ctx, sp[-1], &arrp, &count32)) {
        ...
        if (len != count32)
            goto general_case;
        /* Bulk append: presize the target once and store the dup'd
           elements straight into its fast-array slots.  The per-element
           JS_DefinePropertyValueUint32 ran the full define machinery
           (index boxing, atom conversion, shape update) per element --
           62% of spread time.  The target of
           OP_append is always a fresh fast array, but guard anyway and
           fall back to the per-element define if it is not.  pos must
           equal u.array.count: array-literal ELISIONS ([, ...x]) bump
           pos past count, and the direct-store path would raise count
           over uninitialized slots (GC-scanned garbage JSValues) --
           the define path converts to a slow array, giving correct
           holes.  Also falls back for self-spread (expanding the
           source would realloc the buffer arrp points at). */
        {
            JSObject *tobj = JS_VALUE_GET_OBJ(sp[-3]);
            if (count32 != 0 /* empty spread: nothing to append */
            &&  tobj->fast_array && tobj->class_id == JS_CLASS_ARRAY
            &&  JS_VALUE_GET_OBJ(sp[-3]) != JS_VALUE_GET_OBJ(sp[-1])
            &&  (uint64_t)pos + count32 <= 0x7ffffffeU
            &&  (uint32_t)pos == tobj->u.array.count) {
                if (expand_fast_array(ctx, tobj, pos + count32))
                    goto exception;
                for (i = 0; i < count32; i++)
                    tobj->u.array.u.values[pos + i] = JS_DupValue(ctx, arrp[i]);
                tobj->u.array.count = pos + count32;
                /* keep prop[0] (the length) in sync, as
                   add_fast_array_element does on every push */
                if (likely(JS_VALUE_GET_TAG(tobj->prop[0].u.value) == JS_TAG_INT)
                &&  (uint32_t)JS_VALUE_GET_INT(tobj->prop[0].u.value) < tobj->u.array.count)
                    tobj->prop[0].u.value = JS_NewInt32(ctx, tobj->u.array.count);
                pos += count32;
            } else {
                for (i = 0; i < count32; i++) {
                             ...
                             BOOL is_arg)
{
    ...
    /* JS_EvalFunction on a bare bytecode value passes sf == NULL: there is
     * no parent frame, so there is nothing to close over -- fail closed
     * (sweep finding F1: the crafted-blob harness SIGSEGV'd right here). */
    if (!sf)
        return NULL;
    p = JS_VALUE_GET_OBJ(sf->cur_func);
    b = p->u.func.function_bytecode;
    
    /* var_idx arrives from deserialized closure vars, so the asserts the
     * upstream code relied on are the ONLY defense under a release build --
     * and an OOB vardefs read plus an sf->var_refs[...] WRITE at :7426 is
     * the result (sweep finding F1). REAL bounds, fail closed: the callers
     * treat NULL as an error. */
    if (var_idx < 0 ||
        var_idx >= (is_arg ? (int)b->arg_count : (int)b->var_count))
        return NULL;
    if (is_arg) {
    ...
    if (var_ref_idx >= b->var_ref_count)
        return NULL;
...
#if CONFIG_FAST_PROP_TEARDOWN
                    p->shape->all_plain = FALSE; /* slot gains a masked flag */
#endif
                           ...
                           BOOL is_eval, JSModuleDef *m)
{
    JSObject *p;
    JSVarRef **var_refs;
    int i = 0; /* read by the fail-path error message */
    ...
    if (b->closure_var_count) {
        ...
        /* cur_var_refs is the parent's CLOSURE array -- the one js_closure2
         * allocated for the parent with b->closure_var_count entries (and
         * which JS_CallInternal hands back to OP_fclosure as
         * p->u.func.var_refs, see interpreter.inc.c). closure_var_count is
         * therefore both the memory-safety bound for a hostile blob's
         * REF/GLOBAL_REF var_idx (sweep finding F1) and the only correct
         * one: bounding by var_ref_count instead rejected legitimate
         * closures whenever the parent carries GLOBAL/GLOBAL_REF entries
         * (any program-mode parent whose nested functions reference a
         * global -- print, Error, ...), because closure_var_count exceeds
         * var_ref_count there. sf is NULL exactly when this frame has no
         * parent to close over, and then a REF closure is bogus. */
        int parent_var_refs_len = 0;
        if (sf) {
            JSObject *pf = JS_VALUE_GET_OBJ(sf->cur_func);
            parent_var_refs_len =
                pf->u.func.function_bytecode->closure_var_count;
        }
        for(i = 0; i < b->closure_var_count; i++) {
            JSClosureVar *cv = &b->closure_var[i];
            JSVarRef *var_ref;
            ...
            case JS_CLOSURE_ARG:
                ...
                break;
            case JS_CLOSURE_REF:
                if (cv->var_idx >= parent_var_refs_len)
                    goto fail;
 ...
 fail:
    ...
    /* A bare JS_EXCEPTION with nothing pending is dumped by js_std_dump_error
     * as the no-exception sentinel -- the user sees a bare "uninitialized"
     * and exit 1. Say what actually failed instead. */
    if (!JS_HasException(ctx))
        JS_ThrowInternalError(ctx, "could not resolve closure variable #%d",
                              i);
    return JS_EXCEPTION;
}
