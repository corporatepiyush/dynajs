...
/* Bucket of 'atom' in a shape's property hash table.

   The mask alone is not a hash. An integer-index atom is `index |
   JS_ATOM_TAG_INT` (src/value/atoms.inc.c:26), so `atom & prop_hash_mask` keeps
   only the low bits of the index, and every key whose index is a multiple of
   the table size lands in bucket 0. That is reachable from untrusted input --
   JSON.parse of {"65536":1,"131072":1,...} builds exactly such an object -- and
   it turns the chain walks in find_own_property() and add_shape_property() into
   O(k^2). Measured before the fold: 4.0x per doubling of k, 1394 ms to build
   32000 keys against 1 ms for the same count of consecutive keys, and 272x on
   JSON.parse against an equal-sized payload with sequential keys.

   The single xor-shift is what the *cost* allows, not what a hash textbook
   would pick. Two multiplicative alternatives were built and measured on this
   machine: Fibonacci multiply-shift (a per-shape shift byte, so a second
   dependent load) and the high half of the product masked (one load, as here).
   Both fixed the input and both cost 3-6% on prop_read/prop_write/prop_update/
   prop_create -- the bucket sits on the dependency chain feeding the
   hash_table[] load, and a multiply there is not free however it is arranged.
   Shift-xor-and is three ALU ops on a value already in a register and measured
   flat on those same rows, while scattering every power-of-two stride from 2^6
   to 2^16 down to control speed (tests/bench_prop_hash.js sweeps them; testing
   only the stride the constant was chosen against would prove nothing).

   This is not a keyed hash and does not claim to be: an adversary who reads
   this line can solve for colliding indices. It stays bounded because the two
   ways an atom gets its value are both constrained -- string atoms are handed
   out sequentially by intern order, which the caller does not choose, and
   integer atoms cannot exceed JS_ATOM_MAX_INT, which caps how many indices can
   be made to agree after the fold. A keyed hash is the answer if that ever
   stops being true; it costs the 3-6% measured above.

   Folding at 18 rather than a smaller shift is deliberate: below 2^18 atoms
   the fold is the identity, so a program with fewer than 262144 distinct atoms
   -- which is every ordinary one -- gets byte-identical bucket assignments to
   the unmixed build. The behaviour change is confined to the large and
   integer-tagged atoms, which is exactly the range a caller can choose.

   ON BY DEFAULT (CONFIG_PROP_HASH_MIX, audit 2.3). The mixing is two ALU ops
   on the dependency chain feeding the hash_table[] load, and measurement says
   that is not free: a threshold-0 control -- this same helper with the fold
   removed, every call site otherwise identical -- was flat on every microbench
   row, while the fold costs ~0.3 ns per lookup. That is invisible on
   prop_read (7.2 ns) and +7% on global_write (4.3 ns). Opt out with
   CONFIG_PROP_HASH_MIX=n for maximum lookup speed. */
static force_inline uint32_t prop_hash_bucket(JSAtom atom, uint32_t mask)
{
    uint32_t h = (uint32_t)atom;
#ifdef CONFIG_PROP_HASH_MIX
    h ^= h >> 18;
#endif
    return h & mask;
}

/* prop_hash_bucket() ends in a mask, so the table size must stay a power of
   two for the mask to describe a contiguous bucket range. */
static inline void js_shape_set_hash_size(JSShape *sh, uint32_t hash_size)
{
    assert(hash_size >= 2 && (hash_size & (hash_size - 1)) == 0);
...
static int init_shape_hash(JSRuntime *rt)
{
    ...
    /* Per-runtime salt (audit 2.3b): every hashed shape's hash descends from
       shape_initial_hash(), so randomising that root makes the whole shape
       hash table a per-runtime function. Fail closed like JS_InitAtoms. */
    if (dyn_os_entropy(&rt->shape_hash_seed, sizeof(rt->shape_hash_seed)) < 0)
        return -1;
...
static uint32_t shape_initial_hash(JSRuntime *rt, JSObject *proto)
{
    uint32_t h;
    h = shape_hash((uint32_t)rt->shape_hash_seed, (uintptr_t)proto);
    if (sizeof(proto) > 4)
                                           ...
                                           int hash_size, int prop_size)
{
    ...
    js_shape_set_hash_size(sh, hash_size);
    sh->prop_size = prop_size;
    sh->prop_count = 0;
    sh->deleted_prop_count = 0;
    sh->is_hashed = FALSE;
#if CONFIG_FAST_PROP_TEARDOWN
    sh->all_plain = TRUE; /* vacuously true: no properties yet */
#endif
    return sh;
}
                                        ...
                                        int hash_size, int prop_size)
{
    ...
    sh->hash = shape_initial_hash(ctx->rt, proto);
    sh->is_hashed = TRUE;
                                       ...
                                       JSObject *p, uint32_t count)
{
    ...
    if (new_hash_size != (sh->prop_hash_mask + 1)) {
        ...
        js_shape_set_hash_size(sh, new_hash_size);
        ...
        for(i = 0, pr = get_shape_prop(sh); i < sh->prop_count; i++, pr++) {
            if (pr->atom != JS_ATOM_NULL) {
                h = prop_hash_bucket(pr->atom, sh->prop_hash_mask);
...
static int compact_properties(JSContext *ctx, JSObject *p)
{
    JSShape *sh, *old_sh;
    intptr_t h;
    uint32_t new_hash_size, i, j, new_size;
    ...
    for(i = 0; i < sh->prop_count; i++) {
        if (old_pr->atom != JS_ATOM_NULL) {
            ...
            h = prop_hash_bucket(old_pr->atom, sh->prop_hash_mask);
            pr->hash_next = sh->hash_table[h];
            sh->hash_table[h] = j + 1;
            prop[j] = prop[i];
            j++;
                              ...
                              JSObject *p, JSAtom atom, int prop_flags)
{
    ...
    pr->flags = prop_flags;
#if CONFIG_FAST_PROP_TEARDOWN
    if (prop_flags & JS_PROP_TMASK)
        sh->all_plain = FALSE; /* free_property must dispatch per slot */
#endif
    /* add in hash table */
    h = prop_hash_bucket(atom, sh->prop_hash_mask);
    pr->hash_next = sh->hash_table[h];
                                       ...
                                       JSAtom atom, int prop_flags)
{
    ...
    for(sh1 = rt->shape_hash[h1]; sh1 != NULL; sh1 = sh1->shape_hash_next) {
        ...
        if (sh1->hash == h &&
            ...
            /* Check the ADDED property before walking the shared prefix. Both
               orders are equally correct, but the prefix walk is O(prop_count)
               and runs on every candidate whose 32-bit hash collides, so with
               the added property checked last a colliding atom set makes each
               property add O(k) -- i.e. O(k^2) to build one object -- on
               attacker-shaped keys (JSON.parse of untrusted input reaches
               here). The added property is the most selective field and is
               O(1), so testing it first bounds the damage: a collision that is
               not a real match now costs one compare, not prop_count. */
...
/* Recycled allocation for JSObject header structs (house pattern: see
   js_date_cache_alloc/free). Every JSObject is js_malloc'd as
   sizeof(JSObject) regardless of class -- the class payload lives in the
   u union -- so a single LIFO serves all classes.

   On pop the struct is used exactly like a js_malloc'd (i.e. garbage)
   struct: JS_NewObjectFromShape below writes every field it reads --
   flag bits, class_id, weakref_count, u.opaque, shape, prop, the
   class-specific u arm, ref_count -- and add_gc_object() rewrites
   js_rc()->mark, js_rc()->gc_obj_type and header.link. The allocator's
   JSMallocBlockHeader (which travels in front of user_data and holds
   ref_count/mark/gc_obj_type) is never touched by push/pop, so a pooled
   block stays indistinguishable from a live allocation for the arena
   allocator and can be js_free_rt()'d on eviction or at teardown. */
static JSObject *js_obj_pool_alloc(JSContext *ctx)
{
    JSRuntime *rt = ctx->rt;
    JSObject *p = rt->obj_pool_free_list;
    if (likely(p != NULL)) {
        rt->obj_pool_free_list = *(JSObject **)p; /* header.link.next */
        rt->obj_pool_count--;
        return p;
    }
    return js_malloc(ctx, sizeof(JSObject));
}

static void js_obj_pool_free(JSRuntime *rt, JSObject *p)
{
    if (rt->obj_pool_count < JS_OBJ_POOL_MAX_OBJECTS) {
        *(JSObject **)p = rt->obj_pool_free_list;
        rt->obj_pool_free_list = p;
        rt->obj_pool_count++;
    } else {
        js_free_rt(rt, p);
    }
...
/* Recycled allocation for JS_CLASS_ARRAY fast-array values buffers,
   pooled by capacity bucket (exact powers of two, 1..1024 slots).
   u1.size is the true slot capacity for pooled arrays (js_realloc2's
   slack is folded into it by expand_fast_array), so a pushed buffer is
   always at least as large as its bucket. Pop returns the smallest
   bucket >= min_slots and reports the bucket as the new capacity; the
   freelist link is stashed in values[0], which is unreachable garbage
   for a freshly grown array until the caller writes it. */
static JSValue *js_array_pool_alloc(JSRuntime *rt, uint32_t min_slots,
                                    uint32_t *psize)
{
    int b = 32 - clz32(min_slots - 1); /* ceil(log2(min_slots)) */
    JSValue *tab;
    if (b >= JS_ARRAY_POOL_NBUCKETS)
        return NULL;
    tab = (JSValue *)rt->array_pool_free_list[b];
    if (likely(tab != NULL)) {
        rt->array_pool_free_list[b] = *(JSValue **)tab;
        rt->array_pool_count--;
        rt->array_pool_slots -= 1 << b;
        *psize = 1 << b;
    }
    return tab;
}

/* free p->u.array.u.values, pooling it when it is a JS_CLASS_ARRAY
   buffer whose capacity is an exact pooled bucket. Only JS_CLASS_ARRAY
   maintains u1.size as a capacity (js_build_arguments leaves it stale
   for JS_CLASS_ARGUMENTS), hence the class gate. */
static void js_array_pool_free(JSRuntime *rt, JSObject *p)
{
    JSValue *tab = p->u.array.u.values;
    if (tab == NULL)
        return; /* matches js_free_rt(NULL): no-op */
    if (p->class_id == JS_CLASS_ARRAY) {
        uint32_t size = p->u.array.u1.size;
        if (size != 0 && size <= JS_ARRAY_POOL_MAX_BUCKET &&
            (size & (size - 1)) == 0 &&
            rt->array_pool_slots + (int)size <= JS_ARRAY_POOL_MAX_SLOTS) {
            int b = 31 - clz32(size); /* log2(size), size is a power of two */
            *(JSValue **)tab = (JSValue *)rt->array_pool_free_list[b];
            rt->array_pool_free_list[b] = tab;
            rt->array_pool_count++;
            rt->array_pool_slots += size;
            return;
        }
    }
    js_free_rt(rt, tab);
}
#endif /* CONFIG_OBJ_POOL */
                                     ...
                                     JSProperty *props)
{
...
#ifdef CONFIG_OBJ_POOL
    p = js_obj_pool_alloc(ctx);
#elif defined(CONFIG_NURSERY_PROBE)
    /* diagnostic nursery probe: bump-allocate the header (falls back to
       the normal allocator when the probe is out of room -- see the
       CONFIG_NURSERY_PROBE note in the Makefile) */
    p = js_nursery_probe_alloc_rt(ctx->rt, sizeof(JSObject));
    if (unlikely(!p))
    ...
    p->prop = js_nursery_probe_alloc_rt(ctx->rt, sizeof(JSProperty) * sh->prop_size);
    if (unlikely(!p->prop))
    ...
    case JS_CLASS_DATE:
        p->u.object_data = JS_UNDEFINED;
        p->u.date.fields_cache = NULL; /* object memory is js_malloc'd,
                                          not zeroed: without this a
                                          recycled JSObject would carry
...
/* Create an object literal already at its final shape.
 *
 * No prototype-safety check is needed here, unlike the constructor case:
 * OP_define_field is [[DefineOwnProperty]], which never consults the prototype
 * chain, so a proto accessor or a non-writable proto property cannot change
 * what the stores do. The shape is still keyed on the proto so a realm with a
 * different Object.prototype rebuilds rather than reusing.
 *
 * Falls back to a plain JS_NewObject on any failure -- the table is a hint, and
 * an object built the slow way is indistinguishable from one built this way
 * (same transition-tree shape, same insertion order, same attributes). */
static JSValue js_object_presize_new(JSContext *ctx, JSObjectPresizeSite *st)
{
    JSObject *proto = get_proto_obj(ctx->class_proto[JS_CLASS_OBJECT]);
    JSValue obj;
    JSObject *p;
    int i;

    if (!st->cached_shape || st->cached_shape->proto != proto) {
        ...
        for (i = 0; i < st->field_count; i++) {
            JSProperty *pr = add_property(ctx, tp, st->fields[i], JS_PROP_C_W_E);
            if (!pr) {
                JS_FreeValue(ctx, tmp);
                return JS_NewObject(ctx);   /* hint failed: plain path */
            }
            /* add_property leaves the slot uninitialized; seed it so freeing
               the throwaway object does not free garbage */
            pr->u.value = JS_UNDEFINED;
        }
        if (st->cached_shape)
            js_free_shape(ctx->rt, st->cached_shape);
        st->cached_shape = js_dup_shape(tp->shape);
        JS_FreeValue(ctx, tmp);
    }

    obj = JS_NewObjectFromShape(ctx, js_dup_shape(st->cached_shape),
...
/* Runtime-recorded literal boilerplate (CONFIG_LITERAL_BOILERPLATE).
 *
 * The interpreter's OP_object/OP_define_field handlers watch each literal
 * evaluation: OP_object starts watching (holding one reference to the fresh
 * object so the watched pointer can never be recycled), and OP_define_field
 * calls js_object_presize_record_site() after each define with the running
 * define count. When the run is eligible, the site's shape is created (first
 * evaluations) or upgraded in place (later evaluations) to describe the
 * literal's first 'nf' fields. Later evaluations of the same site replay it
 * through js_object_presize_new(), which creates the object directly at the
 * recorded shape so each OP_define_field of the prefix becomes a plain slot
 * overwrite instead of a shape transition; fields beyond the recorded prefix
 * transition exactly as before and grow the site until it covers the whole
 * literal. This is the classic "boilerplate" trick driven by execution
 * instead of a static bytecode pattern, so literals with computed values
 * (get_field chains, calls, arithmetic between OP_object and the defines)
 * are covered -- the parse-time CONFIG_PRESIZE_LITERAL analyzer must reject
 * those because it cannot safely attribute arbitrary opcode runs.
 *
 * Eligibility is exact (checked incrementally per define by the caller and
 * re-verified wholesale here):
 *   - the object is a plain, non-exotic JS_CLASS_OBJECT;
 *   - the shape's proto is the current Object.prototype (a literal whose run
 *     contains an OP_set_proto to a different proto never records);
 *   - every described property is plain data with exactly C_W_E flags and a
 *     non-array-index atom;
 *   - the shape has exactly 'nf' properties and no deleted ones, so a prop
 *     added by any other opcode in the run disqualifies it;
 *   - 2..JS_PRESIZE_MAX_FIELDS fields, and the site table has room.
 * On any mismatch the recording is simply dropped: the hint is absent and
 * the literal builds exactly as it did before this mechanism existed.
 *
 * Returns the site now covering 'bc_offset' (created, upgraded, or already
 * current), NULL if nothing is recorded. Never throws. */
static JSShape *js_object_presize_site_shape(JSContext *ctx, JSObject *p,
                                             int nf)
{
    JSShape *sh;
    JSShapeProperty *pr;
    ...
    if (nf < 2 || nf > JS_PRESIZE_MAX_FIELDS)
        return NULL;
    if (p->class_id != JS_CLASS_OBJECT || p->is_exotic || p->fast_array)
        return NULL;
    sh = p->shape;
    if (sh->proto != get_proto_obj(ctx->class_proto[JS_CLASS_OBJECT]))
        return NULL;
    if (sh->prop_count != nf || sh->deleted_prop_count != 0)
        return NULL;
    pr = get_shape_prop(sh);
    for (i = 0; i < nf; i++) {
        if (pr->flags != JS_PROP_C_W_E ||  /* exact: no TMASK, no LENGTH,
                                              no missing C/W/E bit */
            __JS_AtomIsTaggedInt(pr->atom))
            return NULL;
        pr++;
    }
    return sh;
...
static JSObjectPresizeSite *js_object_presize_site_fill(
    JSContext *ctx, JSObjectPresizeSite *st, uint32_t bc_offset,
    JSShape *sh, int nf)
{
    JSAtom *fields;
    int i;

    fields = js_malloc(ctx, sizeof(JSAtom) * nf);
    if (!fields)
        return NULL;
    for (i = 0; i < nf; i++)
        fields[i] = JS_DupAtom(ctx, get_shape_prop(sh)[i].atom);
    if (st->fields) {
        for (i = 0; i < st->field_count; i++)
            JS_FreeAtom(ctx, st->fields[i]);
        js_free(ctx, st->fields);
    }
    if (st->cached_shape)
        js_free_shape(ctx->rt, st->cached_shape);
    st->fields = fields;
    st->field_count = nf;
    st->cached_shape = js_dup_shape(sh);
    return st;
}

static JSObjectPresizeSite *js_object_presize_record_site(
    JSContext *ctx, JSFunctionBytecode *b, JSObjectPresizeSite *st,
    uint32_t bc_offset, JSObject *p, int nf)
{
    JSObjectPresize *ps;
    JSShape *sh;

    if (st != NULL && st->cached_shape == p->shape)
        return st;  /* already current (typical full-shape replay); the
                       cached shape was fully validated when the site was
                       (re)filled, so no re-walk is needed */

    sh = js_object_presize_site_shape(ctx, p, nf);
    if (!sh)
        return NULL;

    if (st != NULL) {
        /* upgrade in place (same site, run defined more fields) */
        return js_object_presize_site_fill(ctx, st, bc_offset, sh, nf);
    }
    ...
    if (ps != NULL) {
        ...
        for (si = 0; si < ps->site_count; si++)
            if (ps->sites[si].bc_offset == bc_offset)
                return &ps->sites[si];  /* a parse-time site covers it */
        if (ps->site_count >= ps->site_cap ||
            ps->site_count >= JS_PRESIZE_MAX_SITES)
            return NULL;
    } else {
        ps = js_mallocz(ctx, sizeof(*ps) + sizeof(JSObjectPresizeSite) * 4);
        if (!ps)
            return NULL;
        ps->site_count = 0;
        ps->site_cap = 4;
    ...
    st = &ps->sites[ps->site_count];
    memset(st, 0, sizeof(*st));
    st->bc_offset = bc_offset;
    st = js_object_presize_site_fill(ctx, st, bc_offset, sh, nf);
    if (st)
        ps->site_count++;
    return st;
}
#endif /* CONFIG_LITERAL_BOILERPLATE */
...
static int JS_SetObjectData(JSContext *ctx, JSValueConst obj, JSValue val)
{
    ...
    if (JS_VALUE_GET_TAG(obj) == JS_TAG_OBJECT) {
        ...
        case JS_CLASS_DATE:
            JS_FreeValue(ctx, p->u.object_data);
            p->u.object_data = val;
            /* construction-time store of the time value: a fresh Date
               has fields_cache == NULL, but be defensive -- any store
               through here invalidates the cached field breakdown
               (JS_SetThisTimeValue does the same for mutations) */
            if (unlikely(p->u.date.fields_cache != NULL)) {
                p->u.date.fields_cache->e[0].valid = 0;
                p->u.date.fields_cache->e[1].valid = 0;
            }
            return 0;
        }
    }
...
static void js_object_data_finalizer(JSRuntime *rt, JSValue val)
{
    ...
    if (p->class_id == JS_CLASS_DATE) {
        /* recycle the per-object cached field breakdown (plain data:
           no mark, nothing inside for the GC to trace) */
        if (p->u.date.fields_cache != NULL) {
            js_date_cache_free(rt, p->u.date.fields_cache);
            p->u.date.fields_cache = NULL;
        }
    }
}
...
static void free_object(JSRuntime *rt, JSObject *p)
{
    ...
    pr = get_shape_prop(sh);
#if CONFIG_FAST_PROP_TEARDOWN
    if (unlikely(sh->all_plain)) {
        /* CONFIG_FAST_PROP_TEARDOWN: every slot of this shape is a plain
           JSValue (no getter/setter, var_ref or autoinit anywhere -- the
           JSShape.all_plain bit is maintained by add_shape_property and
           conservatively cleared by js_shape_prepare_update() and the
           global getset->varref conversion). One dense sweep of value
           frees; the shape's prop table is not touched at all. Deleted
           slots are plain (flags 0) with a value of JS_UNDEFINED. */
        JSProperty *pp = p->prop;
        for(i = 0; i < sh->prop_count; i++)
            JS_FreeValueRT(rt, pp[i].u.value);
    } else
#endif
    for(i = 0; i < sh->prop_count; i++) {
...
void __JS_FreeValueRT(JSRuntime *rt, JSValue v)
{
    ...
    switch(tag) {
    /* Tags ordered hottest-first. Every arm breaks. */
    /* objects dominate refcounted frees */
    case JS_TAG_OBJECT:
    ...
    case JS_TAG_STRING:
        {
            ...
            if (p->atom_type) {
                ...
                /* release the slice's reference on its flat parent
                   (mirror of js_free_string; recursion is one level).
                   Constant atom parents (hash_next bit 0 clear) took no
                   reference at creation, so no decrement here. */
                JSString *parent = js_slice_parent(p);
                int parent_holds_ref = p->hash_next & 1;
#ifdef DUMP_LEAKS
                list_del(&p->link);
#endif
                js_free_rt(rt, (uint8_t *)p - JS_SLICE_PREFIX_SIZE);
                if (parent_holds_ref)
...
#endif
#if CONFIG_PRESIZE_LITERAL
            /* same: each cached literal shape owns a ref pinning its proto */
            if (b->obj_presize) {
                int si;
                for (si = 0; si < b->obj_presize->site_count; si++)
                    if (b->obj_presize->sites[si].cached_shape)
                        mark_func(rt,
                            &b->obj_presize->sites[si].cached_shape->header);
            }
#endif
        }
        break;
...
static void gc_free_cycles(JSRuntime *rt)
{
    ...
    list_for_each_safe(el, el1, &rt->gc_zero_ref_count_list) {
        ...
        if (js_rc(p)->gc_obj_type == JS_GC_OBJ_TYPE_JS_OBJECT &&
            ...
            js_rc(p)->mark = 0;
#ifdef CONFIG_OBJ_POOL
        } else if (js_rc(p)->gc_obj_type == JS_GC_OBJ_TYPE_JS_OBJECT) {
            /* the JSObject struct was already dismembered by
               free_object() in the REMOVE_CYCLES phase; only the struct
               itself is released here */
            js_obj_pool_free(rt, (JSObject *)p);
#endif
        } else {
            js_free_rt(rt, p);
        }
...
static void compute_jsstring_size(JSString *str, JSMemoryUsage_helper *hp)
{
    if (!str->atom_type) {  /* atoms are handled separately */
        ...
        if (unlikely(str->is_slice)) {
            /* A slice owns only its prefix block (JS_SLICE_PREFIX_SIZE
               payload + JSString header): the character bytes live in the
               flat parent, which the slice keeps alive with exactly one
               reference. The old accounting charged the slice
               sizeof(JSString)+len as if the window were its own
               allocation, so N live slices of one parent overcounted N
               windows of parent bytes. Now the slice is charged its block,
               then the parent's shared cost is charged through the same
               ref-count division every other reference uses: each
               reference (value slot or slice) contributes
               cost/ref_count, so a parent held only by its N slices sums
               to exactly one allocation. */
            hp->str_size += ((JS_SLICE_PREFIX_SIZE + sizeof(*str)) /
                             s_ref_count);
            str = js_slice_parent(str); /* flat; atom since wave B is OK */
            if (str->atom_type) {
                /* the parent's bytes live in the intern table, accounted
                   under `atoms`; charge only the slice's own block above */
                return;
            }
            s_ref_count = js_rc(str)->ref_count;
        }
...
{
    /* node/V8 parity (LONG-standing ticket, aligned 2026-09-11): recursion
       overflow is a RangeError named "Maximum call stack size exceeded".
       The old InternalError("stack overflow") was a quickjs-ism no other
       engine shares; the blast radius was audited before the change:
       - test262: 0 of the 60 pinned fails involve stack overflow, and no
         corpus test pins a constructor class for recursion overflow, so
         the fail list is unaffected;
       - the committed suites that mention it pin the MESSAGE, not the
         class (tests/test_json.js, tests/bench_parse_frameworks.js), and
         the bbreview tail-call probes pin the divergence per-engine -- all
...
static JSValueConst JS_GetPrototypePrimitive(JSContext *ctx, JSValueConst val)
{
    ...
    /* Tags ordered hottest-first. Every arm breaks. */
    /* string and rope: primitive property access is mostly this */
    case JS_TAG_STRING:
    case JS_TAG_STRING_ROPE:
...
/* const: read-only dispatch table, never written. Keeps a function-pointer
   array out of writable memory and off the mutable-file-scope race surface. */
