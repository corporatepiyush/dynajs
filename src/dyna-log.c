...
/* Level metadata precomputed ONCE, so the emit path never calls strlen or a
   ...
   Lower-case is the MAINTAINED contract (tests/test_log.js pins "info  api:"
   in the text format); the API.md example line showing "WARN  api:" is a
   stale doc example -- ticketed for a doc fix, not an engine change. */
static const struct {
...
typedef struct {
    ...
    dyn_log_sink_t *sink;   /* NULL => stderr (or fn dest) */
    JSValue dest_fn;        /*function dest (line: string) => void;
                               JS_UNDEFINED when dest is stderr/a path */
    /*{sample: N} emit sampling -- every Nth emit attempt that passes
       the level gate is written (N = 1, the default, is every attempt).
       The counter is PER LOGGER: a child starts counting at zero again. */
    int64_t emit_count;
    int64_t sample_n;
...
/* Frees the C half. dest_fn (a JSValue) is freed by the CALLER, which must
 * know the runtime: dyn_logger_free_rt below for the finalizer/ctor paths. */
...
/* The function dest is the class's only JSValue: it must be traced or
 * a collection cycle that reaches only through the logger would free it. */
...
static JSClassDef dyn_logger_class = {
    "Logger", .finalizer = dyn_logger_finalizer, .gc_mark = dyn_logger_mark,
};
...
/* Hex-digit test for the partial-escape check in the repair pass below. */
...
/* One byte of JSON punctuation (outside a string): the repair pass uses it to
   tell a completed literal from a half-written one. */
...
/* Turn a truncated JSON line back into ONE parseable record. The cut can
   land on any byte: mid-value-string, mid-KEY, mid-number, on a dangling
   comma or colon, inside a nested object or array -- and the `}\n` close
   itself can be the write that got refused, which used to drop the newline
   and fuse this record with the next one. Truncation is a cold path, so the
   emitted bytes are scanned ONCE (bounded by the line cap) to recover the
   structure: string state, the open-container stack, and the longest prefix
   that is still a valid record. The close then either keeps and closes a cut
   VALUE string (the common huge-msg case, `..."msg":"...`), or rolls back to
   that prefix when the cut left the line unable to close cleanly, and in
   both cases closes every open container in reverse order and terminates
   with a newline. Whatever the payload, the record parses. */
static void dyn_line_json_repair(dyn_line_t *b)
{
    size_t safe = 1;                /* right after the opening '{' */
    char cstack[32];                /* open containers; nesting is bounded by
                                       DYN_LOG_MAX_DEPTH (+ the err frame),
                                       so 32 can never overflow */
    int sp = 0;
    ...
    for (size_t i = 0; i < b->n; i++) {
        ...
        if (in_str) {
            ...
            if (c == '"') {
                ...
                in_str = in_key = 0;
                if (!was_key)
                    safe = i + 1;   /* a value string completed here */
            }
            continue;
        }
        switch (c) {
        ...
        case ',':
            if (i && !dyn_json_punct(b->p[i - 1]))
                safe = i;           /* a literal value completed before it */
            expect_key = sp > 0 && cstack[sp - 1] == '{';
            break;
        case '{':
            if (sp < (int)countof(cstack)) {
                cstack[sp] = '{';
                sp++;               /* clamp keeps k below the written slots */
            }
            expect_key = 1;
            safe = i + 1;
            break;
        ...
        case ']':
            if (sp > 0) sp--;
            expect_key = sp > 0 && cstack[sp - 1] == '{';
            safe = i + 1;           /* a closed container is a valid prefix */
            break;
        default:                    /* literal byte: nothing to decide */
            break;
        }
    }

    if (in_str && !in_key) {
        /* The cut is inside a VALUE string: keep the content. First drop a
           partial \uXXXX escape or a dangling backslash the cut split -- a
           `\u00` tail would fuse with the marker into an invalid escape --
           then a UTF-8 sequence split mid-codepoint. */
        size_t hex = 0;
        while (hex < 4 && nn > hex && dyn_json_hex(b->p[nn - 1 - hex]))
            hex++;
        if (hex > 0 && hex < 4 && nn >= hex + 2 &&
            b->p[nn - 1 - hex] == 'u' && b->p[nn - 2 - hex] == '\\')
            nn -= hex + 2;
        {
            size_t bs = 0;
            while (nn > bs && b->p[nn - 1 - bs] == '\\')
                bs++;
            if (bs & 1)
                nn--;               /* a dangling backslash would eat the close */
        }
        while (nn > safe && ((unsigned char)b->p[nn - 1] & 0xC0) == 0x80)
            nn--;                   /* orphan continuation bytes */
        if (nn > safe && (unsigned char)b->p[nn - 1] >= 0xC0)
            nn--;                   /* a lead byte that lost its tail */
        b->n = nn;
        dyn_line_force(b, "...", 3);
        dyn_line_force(b, "\"", 1);
    } else {
        /* Mid-key, mid-literal or on punctuation: roll back to the last
           point where the line was a valid record prefix. */
        b->n = safe;
    }
    for (k = sp - 1; k >= 0; k--)
...
/* The per-emit gate: rotation is lazy, checked before the line lands. A
   line never straddles two files -- the size check counts the INCOMING line
   (`written + n > max`), so a line that would overflow the cap OPENS the
   next file whole instead of pushing the active file past its size. An
   empty active file takes any single line, however oversized, without
   churning one file per line. */
static void dyn_sink_maybe_rotate(dyn_log_sink_t *s, int64_t now, size_t n)
{
    ...
    if ((s->freq_ms && now >= s->period_ms + s->freq_ms) ||
        (s->max_size && s->written > 0 && s->written + n > s->max_size))
        dyn_sink_rotate(s, now);
}
                          ...
                          JSValueConst fields_v, const char *msg, size_t msg_n)
{
    ...
    if (b->truncated) {
        ...
        /* force, not put: the newline must never be the byte the cap
           refuses -- a text line without it fuses with the next one */
        dyn_line_force(b, "\n", 1);
    }
}
                            ...
                            int argc, JSValueConst *argv, int magic)
{
    ...
    if (magic < L->level)                    /* the gate, before any work */
        return JS_UNDEFINED;
    /*{sample: N} -- a DETERMINISTIC per-logger counter: every Nth
     * level-passing attempt is written (N=1 writes everything). The counter
     * counts attempts, not wall time, so a sampled logger's output is
     * reproducible run to run given the same call sequence. */
    ...
    if (L->fmt == DYN_FMT_TEXT) {
        ...
        if (!b.truncated) {
            dyn_line_puts(&b, "}\n");
            /* The close write ITSELF can be the one that hits the cap (the
               content landed at exactly limit-1 or limit): the put above
               then silently drops the `}` or the newline and the record
               fuses with the next line. The repair pass below handles that
               state like any other cut. */
        }
        if (b.truncated)
...
a function dest receives the exact formatted line (trailing
       newline included) the instant it is built -- nothing is batched, so
       flush() is a no-op for it and exit-time flushing has nothing to do.
       A throwing dest propagates the throw out of the emit: a sink that
       raises is a caller bug, unlike a failing fd which the sink machinery
       tracks and drops for. */
 ...
 * pilots): every options bag below is checked against its call site's
 * valid-key table, and an unknown own enumerable string key throws
 *   TypeError: unknown option "X" (valid: a, b, c)
 * instead of being silently ignored -- `{host: "p2"}` used to "work" and
 * just drop the field, the worst failure mode for a log config. Symbol keys
 * and non-enumerable properties are not options and stay invisible
 * (JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY, the for-in sweep). null/undefined
 * and primitive bags keep their historic "no options" reading. Getters are
 * NOT invoked here; the option values are read afterwards, so a throwing
 * getter on a KNOWN key propagates exactly as before. The table and its
 * memory live at each call site; JSPropertyEnum + atoms are engine
 * allocations, key text goes back through JS_FreeCString. Returns 0 when
 * the bag is clean, -1 with the TypeError pending. */
...
/* The valid-key tables, one per options bag (keep in sync with
 * dyn_logger_opts / dyn_logger_child and dynajs.d.ts). */
static const char *const dyn_logger_ctor_keys[] = {
    "level", "name", "timestamp", "format", "pid", "hostname", "base",
    "mkdir", "buffer", "sample", "rollover", "dest",
};
static const char *const dyn_logger_rollover_keys[] = {
    "size", "frequency", "count", "symlink",
};
...
static int dyn_logger_opts(JSContext *ctx, JSValueConst opts, dyn_logger_t *L)
{
    ...
    if (!JS_IsObject(opts))
        return 0;
    /*unknown keys are refused BEFORE any option value is read, so a
       typo can never half-configure a logger. */
    if (dyn_opts_check(ctx, opts, dyn_logger_ctor_keys,
                       countof(dyn_logger_ctor_keys)))
        return -1;

    /* The SETTER (dyn_logger_set_level) is this class's level validator: a
       present level value is coerced and parsed exactly like `L.level = v`
       (audit X-2 -- the ctor used to accept any non-string silently as
       "info" while the same value threw from the setter). Unset stays the
       default. A throwing getter or to-string PROPAGATES. */
    v = JS_GetPropertyStr(ctx, opts, "level");
    if (JS_IsException(v)) {
        JS_FreeValue(ctx, v);   /* no-op on the exception tag: house style */
        return -1;
    }
    if (JS_IsUndefined(v)) {
        JS_FreeValue(ctx, v);
        ...
        JS_FreeValue(ctx, v);
        if (!s)
            return -1;          /* the coercion's own error is pending */
        lv = dyn_level_of(s);
    ...
    if (JS_IsString(v)) {
        ...
        if (s) {
            ...
            if (strcmp(s, "iso") == 0)
                ...
                L->ts_mode = DYN_TS_EPOCH;   /* "epoch" is the declared
                                                spelling (dynajs.d.ts);
                                                "epochMs" stays as the
                                                historical alias */
            else {
                JS_FreeCString(ctx, s);
                JS_FreeValue(ctx, v);
    ...
    /* ---- destination ---- */
    {
        ...
        if (JS_IsException(v)) {
            JS_FreeValue(ctx, v);   /* no-op on the exception tag */
            return -1;          /* a throwing getter PROPAGATES (audit X-2) */
        }
        if (JS_IsBool(v)) {
            if (JS_ToBool(ctx, v))
                bufcap = 64u * 1024u;
        } else if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
            /* numbers and coercing objects alike: the conversion runs user
               JS whose throw must PROPAGATE with the original error (the
               level setter's rule), and only a value that CONVERTS but is
               out of range (NaN included) is refused. The old code ran
               neither check: a throwing buffer getter was swallowed and a
               wrong-typed buffer silently meant "no batching". */
            double d;
            if (JS_ToFloat64(ctx, &d, v)) {
                JS_FreeValue(ctx, v);
                return -1;
        ...
        /*{sample: N} -- every Nth emit attempt (that passes the level
         * gate) is written. Deterministic counter mod N, per logger. */
        ...
        if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
            ...
            if (!JS_IsNumber(v) || d != (double)(int64_t)d || d < 1 ||
                d > 4294967296.0) {
                JS_FreeValue(ctx, v);
        ...
        if (JS_IsObject(v)) {
            JSValue r;
            /*the sub-bag is strict too -- {rollover:{sise:1e6}} used
             * to silently disable size-based rollover. */
        ...
        if (JS_IsString(v)) {
            ...
            /*a function dest writes every formatted line itself.
             * Rollover is a file-shape feature and refuses it (before the
             * dup, so this throw has nothing to clean). */
            if (has_rollover) {
                JS_FreeValue(ctx, v);
                JS_ThrowTypeError(ctx,
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    L->dest_fn = JS_UNDEFINED;
    L->sample_n = 1;                       /* default: emit everything */
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    if (argc > 1 && !JS_IsUndefined(argv[1]) && !JS_IsNull(argv[1])) {
        ...
        /*the child bag carries exactly one option. `level` as a FIELD
         * stays a reserved frame key (see the function comment), so a stray
         * {levle: "debug"} here must not read as silence. */
    ...
    if (C->sink)
        C->sink->refs++;
    /*a child shares the function dest (a duped reference) and
     * the sample divisor, but the sample COUNTER restarts at zero: it is
     * per logger, so a child's first sampled emit is deterministic within
     * the child regardless of how much the parent already logged. */
                               ...
                               JSValue *data)
{
        ...
        return JS_EXCEPTION;
    {
        ...
        dyn_line_force(&b, "\n", 1);       /* never let the cap eat the newline */
