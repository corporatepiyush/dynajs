                        ...
                        int *clen)
{
    ...
    if (JS_IsBigInt(ctx, v)) {
         ...
         * as a negative int (found by audit). Round-trip the conversion and
         * refuse anything that is not exact. */
        ...
        if (same < 0)
            return -1;
        if (!same) {
            JS_ThrowRangeError(ctx, "ASN.1: INTEGER BigInt is outside the "
                                    "int64 range and would wrap");
            return -1;
        }
