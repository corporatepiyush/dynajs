/* dyna-asn1.c -- ASN.1 DER codec, wired into the dyna:serialize module.
 ...
 NULL plus context tagging. adds exact INTEGERs beyond int64 (BigInt,
 both ways, 64-byte content cap); adds IA5String/BMPString/
   UniversalString.
...
#define ASN1_MAX_INT_BYTES  8      /* a DER INTEGER this wide still fits int64 */
#define ASN1_MAX_BIGINT_BYTES 64   /*the INTEGER content cap, both ways */
...
/*an INTEGER whose content is wider than int64 still decodes exactly,
 * as a BigInt. The magnitude is converted to decimal by repeated division by
 * 10 (a 64-byte magnitude is at most 155 digits, so fixed stack buffers
 * bound every step) and the global BigInt() constructor -- the only
 * arbitrary-precision entry point -- builds the value (the dyna-net-redis
 * RESP3 bignum idiom). Returns the value, or an exception naming the reason. */
static JSValue asn1_bigint_from_digits(JSContext *ctx, int neg,
                                       const char *digits, int nd)
{
    char buf[ASN1_MAX_BIGINT_BYTES * 3 + 2];
    JSValue global, ctor, arg, out;
    JSValueConst a1[1];
    int pos = 0;

    if (neg)
        buf[pos++] = '-';
    memcpy(buf + pos, digits, (size_t)nd);
    pos += nd;
    buf[pos] = '\0';

    global = JS_GetGlobalObject(ctx);
    ctor = JS_GetPropertyStr(ctx, global, "BigInt");
    JS_FreeValue(ctx, global);
    if (!JS_IsFunction(ctx, ctor)) {
        JS_FreeValue(ctx, ctor);
        return JS_ThrowInternalError(ctx,
            "ASN.1: the BigInt constructor is unavailable");
    }
    arg = JS_NewStringLen(ctx, buf, (size_t)pos);
    ...
    JS_FreeValue(ctx, ctor);
    /* a decimal integer string cannot fail to parse; if it ever does, surface
     * the constructor's own reason rather than inventing one */
...
/* content[start..start+len) is a minimal two's-complement INTEGER wider than
 * int64. Answer the exact BigInt. */
static JSValue asn1_decode_wide_int(dr_t *d, size_t start, size_t len)
{
    uint8_t mag[ASN1_MAX_BIGINT_BYTES];     /* little-endian magnitude */
    char digits[ASN1_MAX_BIGINT_BYTES * 3 + 1];
    int neg = (d->p[start] & 0x80) != 0;
    int n = (int)len, nd = 0, i;

    for (i = 0; i < n; i++)
        mag[i] = d->p[start + (size_t)(n - 1 - i)];   /* BE input -> LE */
    if (neg) {
        /* magnitude = 2^(8n) - u: two's complement of the LE bytes */
        unsigned carry = 1;
        for (i = 0; i < n; i++) {
            unsigned x = (unsigned)(uint8_t)~mag[i] + carry;
            mag[i] = (uint8_t)(x & 0xFF);
            carry = x >> 8;
        }
    }
    while (n > 0 && mag[n - 1] == 0)
    ...
    while (n > 0) {
        ...
        int k;
        for (k = n - 1; k >= 0; k--) {
            unsigned x = ((unsigned)rem << 8) | mag[k];
            mag[k] = (uint8_t)(x / 10);
            rem = x % 10;
        }
    ...
    for (i = 0; i < nd / 2; i++) {          /* remainders came out backwards */
        char t = digits[i];
        digits[i] = digits[nd - 1 - i];
        digits[nd - 1 - i] = t;
    }
    return asn1_bigint_from_digits(d->ctx, neg, digits, nd);
}
...
static JSValue asn1_decode_int(dr_t *d, size_t start, size_t end)
{
    ...
    if (len > ASN1_MAX_BIGINT_BYTES)
        return JS_ThrowSyntaxError(d->ctx,
            "ASN.1: INTEGER content of %u bytes exceeds the %d-byte cap",
            (unsigned)len, ASN1_MAX_BIGINT_BYTES);
    ...
    if (len > ASN1_MAX_INT_BYTES)
        return asn1_decode_wide_int(d, start, len);   /*exact BigInt */
    for (k = 0; k < len; k++)
        u = (u << 8) | d->p[start + k];
    if (d->p[start] & 0x80) {
        if (len == 8)
...
/* ---: IA5String (22), UniversalString (28), BMPString (30) -----------
 * The decoder interprets them (ASCII / UCS-4 big-endian / UCS-2 big-endian)
 * so a decoded tree re-encodes byte-identically, the same invariant the
 * UTF8String/PrintableString decoders carry. Every refusal names its reason. */

static JSValue asn1_decode_ia5(dr_t *d, size_t start, size_t end)
{
    const uint8_t *p = d->p + start;
    ...
    for (i = 0; i < n; i++)
        if (p[i] > 0x7F)
            return JS_ThrowSyntaxError(d->ctx,
                "ASN.1: IA5String byte 0x%02X at offset %u is outside ASCII",
                p[i], (unsigned)i);
    return JS_NewStringLen(d->ctx, (const char *)p, n);
}

/* One code point (already validated non-surrogate, <= 0x10FFFF) to UTF-8.
 * Returns the number of bytes written (1..4). */
static int utf8_put(uint8_t *q, uint32_t cp)
{
    if (cp < 0x80) {
        q[0] = (uint8_t)cp;
        return 1;
    ...
    if (cp < 0x800) {
        q[0] = (uint8_t)(0xC0 | (cp >> 6));
        q[1] = (uint8_t)(0x80 | (cp & 0x3F));
        return 2;
    }
    if (cp < 0x10000) {
        q[0] = (uint8_t)(0xE0 | (cp >> 12));
        q[1] = (uint8_t)(0x80 | ((cp >> 6) & 0x3F));
        q[2] = (uint8_t)(0x80 | (cp & 0x3F));
        return 3;
    }
    q[0] = (uint8_t)(0xF0 | (cp >> 18));
    q[1] = (uint8_t)(0x80 | ((cp >> 12) & 0x3F));
    q[2] = (uint8_t)(0x80 | ((cp >> 6) & 0x3F));
    q[3] = (uint8_t)(0x80 | (cp & 0x3F));
    return 4;
}

/* UCS-2 (bmp) / UCS-4 big-endian content to a JS string. Every refusal names
 * its reason: a ragged tail, a surrogate (reserved, not a character), or a
 * code point above the Unicode range. */
static JSValue asn1_decode_ucs(dr_t *d, size_t start, size_t end, int bmp)
{
    const uint8_t *p = d->p + start;
    size_t n = end - start, i, unit = bmp ? 2 : 4, w = 0, need;
    uint8_t *out;
    JSValue r;

    if (n % unit != 0)
        return JS_ThrowSyntaxError(d->ctx,
            bmp ? "ASN.1: BMPString content is %u octets, not a whole number "
                  "of 2-octet groups"
                : "ASN.1: UniversalString content is %u octets, not a whole "
                  "number of 4-octet groups",
            (unsigned)n);
    /* overflow guard (codegraph D21; idiom from dyna-crypto.c): the UTF-8
     * re-encode is at most 4 bytes per group plus the NUL, so capping
     * n/unit at (SIZE_MAX-1)/4 keeps the product and the sum inside
     * size_t. Real content is bounded by the DER buffer, so this is
     * belt-and-braces. */
    if (n / unit > (SIZE_MAX - 1) / 4)
        return JS_ThrowRangeError(d->ctx, "ASN.1: %s content too large to "
                                  "re-encode",
                                  bmp ? "BMPString" : "UniversalString");
    need = n / unit * 4 + 1;
    out = (uint8_t *)malloc(need);
    if (!out)
        return JS_ThrowOutOfMemory(d->ctx);
    for (i = 0; i < n; i += unit) {
        uint32_t cp = 0;
        size_t k;
        for (k = 0; k < unit; k++)
        ...
        if (cp >= 0xD800 && cp <= 0xDFFF) {
            free(out);
            return JS_ThrowSyntaxError(d->ctx,
                "ASN.1: %s content contains surrogate 0x%04X, which is "
                "reserved and not a character",
                bmp ? "BMPString" : "UniversalString", cp);
        }
        if (cp > 0x10FFFF) {
            free(out);
            return JS_ThrowSyntaxError(d->ctx,
                "ASN.1: UniversalString code point 0x%X is above U+10FFFF",
                cp);
        }
        w += (size_t)utf8_put(out + w, cp);
    }
    r = JS_NewStringLen(d->ctx, (const char *)out, w);
    free(out);
    return r;
}
...
static JSValue asn1_decode_tlv(dr_t *d, size_t limit)
{
    ...
    if (cls == 0 && (tag == 16 || tag == 17)) {
        /* X.690 8.9.1/8.12.1: SEQUENCE and SET are always constructed; DER
         * (10.1) has no primitive form. A primitive 10/11 used to fall into
         * the children loop and parse its content octets as TLVs (found by
         * audit). */
        JSValue kids;
        if (!constructed)
            return JS_ThrowSyntaxError(d->ctx,
                "ASN.1: a primitive SEQUENCE/SET is not DER; the encoding of "
                "a SEQUENCE or SET is constructed");
        kids = asn1_decode_children(d, end);
    ...
    d->i = end;                 /* simple content: consume the span */
    {
        ...
        if (cls == 0) {
            ...
            case 22: v = asn1_decode_ia5(d, start, end); break;
            case 28: v = asn1_decode_ucs(d, start, end, 0); break;
            case 30: v = asn1_decode_ucs(d, start, end, 1); break;
            case 23:
...
/*minimal two's-complement content octets for a BigInt beyond int64.
 * The value arrives as the decimal string JS_ToCStringLen produces (digits,
 * optional leading '-', no "n" suffix); the magnitude is built little-endian
 * by repeated *10+digit, then placed as the minimal big-endian two's
 * complement (X.690 8.3.2). Returns 0, or -1 with an exception set: a
 * TypeError for a string that is not plain digits, a RangeError past the
 * ASN1_MAX_BIGINT_BYTES content cap. */
static int der_int_from_decimal(JSContext *ctx, const char *s,
                                uint8_t *content, int *clen)
{
    uint8_t mag[ASN1_MAX_BIGINT_BYTES];     /* little-endian magnitude */
    uint8_t be[ASN1_MAX_BIGINT_BYTES];      /* big-endian magnitude */
    int neg = 0, n = 0, i, m;
    ...
    for (; *s; s++) {
        unsigned carry, d = (unsigned)(*s - '0');
        if (d > 9) {
            JS_ThrowTypeError(ctx, "ASN.1: INTEGER BigInt did not stringify "
                                    "to decimal digits");
            return -1;
        }
        carry = d;
        for (i = 0; i < n; i++) {
            unsigned x = (unsigned)mag[i] * 10 + carry;
            mag[i] = (uint8_t)(x & 0xFF);
            carry = x >> 8;
        }
        while (carry) {
            if (n >= ASN1_MAX_BIGINT_BYTES) {
                JS_ThrowRangeError(ctx, "ASN.1: INTEGER BigInt needs more "
                                        "than the %d-byte content cap",
                                   ASN1_MAX_BIGINT_BYTES);
                return -1;
            }
            mag[n++] = (uint8_t)(carry & 0xFF);
            carry >>= 8;
        }
    }
    if (n == 0) {                           /* "0", "-0": DER writes +0 */
        content[0] = 0x00;
        *clen = 1;
        return 0;
    }
    for (i = 0; i < n; i++)
        be[i] = mag[n - 1 - i];
    /* minimal width m: the top bit must say "sign", not "padding" */
    m = n + ((be[0] & 0x80) ? 1 : 0);
    if (m > ASN1_MAX_BIGINT_BYTES) {
        JS_ThrowRangeError(ctx, "ASN.1: INTEGER BigInt needs more than the "
                                "%d-byte content cap", ASN1_MAX_BIGINT_BYTES);
        return -1;
    }
    if (!neg) {
        if (m != n)
            content[0] = 0x00;              /* positive: a 0x00 sign octet */
        memcpy(content + (m - n), be, (size_t)n);
    } else {
        /* two's complement in m bytes: invert, add one -- content is
         * BIG-ENDIAN, so the +1 starts at the least significant octet
         * (content[m-1]); m was chosen so the result's top bit is set */
        unsigned carry = 1;
        memset(content, 0xFF, (size_t)m);
        memcpy(content + (m - n), be, (size_t)n);
        for (i = 0; i < m; i++)
            content[i] = (uint8_t)~content[i];
        for (i = m - 1; i >= 0 && carry; i--) {
            unsigned x = (unsigned)content[i] + carry;
            content[i] = (uint8_t)(x & 0xFF);
            carry = x >> 8;
        }
        if (!(content[0] & 0x80)) {         /* cannot happen for minimal m */
            JS_ThrowInternalError(ctx, "ASN.1: INTEGER two's complement is "
                                        "not minimal");
            return -1;
        }
    }
                        ...
                        int *clen)
{
    ...
    if (JS_IsBigInt(ctx, v)) {
        JSValue back;
        int same;
        /* JS_ToBigInt64 WRAPS modulo 2^64: 12345678901234567890n would encode
         * as a negative int (found by audit). Round-trip the conversion:
         * anything exact in int64 takes the fast path; anything beyond takes
         * the exact decimal path, up to the shared content cap. */
        if (JS_ToBigInt64(ctx, &iv, v) < 0)
            return -1;
        back = JS_NewBigInt64(ctx, iv);
        if (JS_IsException(back))
            return -1;
        same = JS_SameValue(ctx, back, v);
        ...
        }
        {
            ...
            if (!s)
                return -1;
            rc = der_int_from_decimal(ctx, s, content, clen);
...
static int asn1_enc_string(JSContext *ctx, JSValueConst v, int tag, ab_t *out)
{
    ...
    if (tag == 19) {
        ...
        size_t i;
        for (i = 0; i < n; i++)
            if (((const uint8_t *)s)[i] > 0x7F) {
                /* the offending byte must be read BEFORE the free: the error
                 * message quotes it, and quoting freed memory is the
                 * freed-region access ASan aborts on */
                unsigned bad = ((const uint8_t *)s)[i];
                JS_FreeCString(ctx, s);
                JS_ThrowTypeError(ctx, "ASN.1: IA5String is ASCII; byte 0x%02X "
                                        "at offset %u is outside it",
                                  bad, (unsigned)i);
                return -1;
            }
    } else if (tag == 12) {
        /*adjacent fix: the engine's string->UTF-8 boundary writes a
         * lone surrogate as WTF-8, which is invalid DER (X.690 8.21) and
         * exactly what the UTF8String DECODER refuses -- so the encoder
         * refuses it too, by name, instead of emitting an undecodable TLV. */
        if (!utf8_ok((const uint8_t *)s, n)) {
            JS_FreeCString(ctx, s);
            JS_ThrowTypeError(ctx, "ASN.1: UTF8String content would not be "
                                "well-formed UTF-8 (a lone surrogate?)");
            return -1;
        }
    } else if (tag == 28 || tag == 30) {
        /*BMPString is UCS-2 big-endian, UniversalString UCS-4
         * big-endian (X.690 8.23/8.25). Re-encode through code points rather
         * than writing the UTF-8 bytes. The strict utf8_ok check first: a
         * lone surrogate arrives from the engine as WTF-8 and is refused by
         * name, never silently rewritten. */
        uint8_t *b;
        size_t i = 0, w = 0, unit = (tag == 30) ? 2 : 4, need;
        if (!utf8_ok((const uint8_t *)s, n)) {
            JS_FreeCString(ctx, s);
            JS_ThrowTypeError(ctx, "ASN.1: %s value would carry an ill-formed "
                                "sequence (a lone surrogate?)",
                              tag == 30 ? "BMPString" : "UniversalString");
            return -1;
        }
        /* overflow guard (codegraph D21; idiom from dyna-crypto.c): at
         * most 4 UTF-8 bytes per code point plus 4 slack bytes, so capping
         * n at (SIZE_MAX-4)/4 keeps the product inside size_t. Real values
         * are bounded far below this. */
        if (n > (SIZE_MAX - 4) / 4) {
            JS_FreeCString(ctx, s);
            JS_ThrowRangeError(ctx, "ASN.1: %s value too large to re-encode",
                               tag == 30 ? "BMPString" : "UniversalString");
            return -1;
        }
        need = n * 4 + 4;
        ...
        while (i < n) {
            uint32_t cp;
            size_t k, width;
            uint8_t first = ((const uint8_t *)s)[i];
            if (first < 0x80) { width = 1; cp = first; }
            else if (first < 0xE0) { width = 2; cp = first & 0x1F; }
            else if (first < 0xF0) { width = 3; cp = first & 0x0F; }
            else { width = 4; cp = first & 0x07; }
            for (k = 1; k < width; k++)
                cp = (cp << 6) | (((const uint8_t *)s)[i + k] & 0x3F);
            i += width;
            if (tag == 30 && cp > 0xFFFF) {
                free(b);
                JS_FreeCString(ctx, s);
                JS_ThrowTypeError(ctx, "ASN.1: BMPString cannot represent "
                                    "U+%X (above U+FFFF); use utf8String or "
                                    "universalString", cp);
                return -1;
            }
            for (k = unit; k > 0; k--) {   /* big-endian, low octet last */
                b[w + k - 1] = (uint8_t)(cp & 0xFF);
                cp >>= 8;
            }
            w += unit;
        }
                         ...
                         int depth)
{
    ...
    if (isa == 1) {
        ...
        /* a constructed flag on a primitive-only universal type used to be
         * silently ignored (found by audit); every other misuse refuses */
        if (constructed) {
            JS_ThrowTypeError(ctx,
                "ASN.1: universal tag %u is primitive in DER; constructed "
                "form is not available", (unsigned)tag);
            goto fail;
        }
        switch (tag) {
        case 1: {   /* BOOLEAN */
        ...
        case 2: {   /* INTEGER */
            /*the content cap, not the int64 width -- a BigInt past
             * int64 encodes through the decimal path into the same buffer. */
            uint8_t content[ASN1_MAX_BIGINT_BYTES];
            int clen;
        ...
        case 12: case 19: case 22: case 23: case 24:
        case 28: case 30: {  /* string types */
            ab_t tmp;
            if (!JS_IsString(vval)) {
...
/*IA5String (ASCII), BMPString (UCS-2 big-endian), UniversalString
 * (UCS-4 big-endian). Deliberately shallow like every constructor here --
 * asn1_enc_string validates at ENCODE, the single choke point, and the
 * decoders interpret the same tags so a decode re-encodes byte-identically. */
static JSValue asn1_ia5(JSContext *ctx, JSValueConst this_val,
                        int argc, JSValueConst *argv)
{
    (void)this_val;
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "ASN1.ia5String(str): a string is "
                                    "required");
    return asn1_str_node(ctx, 22, argv[0]);
}

static JSValue asn1_bmp(JSContext *ctx, JSValueConst this_val,
                        int argc, JSValueConst *argv)
{
    (void)this_val;
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "ASN1.bmpString(str): a string is "
                                    "required");
    return asn1_str_node(ctx, 30, argv[0]);
}

static JSValue asn1_universal(JSContext *ctx, JSValueConst this_val,
                              int argc, JSValueConst *argv)
{
    (void)this_val;
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "ASN1.universalString(str): a string is "
                                    "required");
    return asn1_str_node(ctx, 28, argv[0]);
}
...
static const JSCFunctionListEntry asn1_methods[] = {
    ...
    JS_CFUNC_DEF("ia5String", 1, asn1_ia5),
    JS_CFUNC_DEF("bmpString", 1, asn1_bmp),
    JS_CFUNC_DEF("universalString", 1, asn1_universal),
