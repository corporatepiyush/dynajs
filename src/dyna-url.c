...
typedef struct {
    ...
    uint32_t query_version;   /* bumped on every query-slot write: bound
                                 URLSearchParams instances use it to detect
                                 that their parsed cache went stale (another
                                 bound instance wrote through) */
    JSValue sp_wrapper;       /* memoized bound URLSearchParams (JS_UNDEFINED
                                 until the first searchParams access). WHATWG
                                 URL sec.6.1 declares searchParams [SameObject]
                                 and models it as the URL's "query object",
                                 created once and associated with the URL --
   so returning one wrapper per access was not
   just O(n^2) per append loop (each fresh
   wrapper's first mutation reparsed the whole
   query), it was non-conformant. The slot owns
   one JS ref: released by dyn_url_finalizer
   only, traced by dyn_url_gc_mark. Not a leak
   cycle: the wrapper's dyn_sp_t keeps the URL's
   C state alive with a raw ref, but never a JS
   edge back to the URL object. */
} dyn_url_t;
...
static void dyn_url_finalizer(JSRuntime *rt, JSValue val)
{
    ...
    /* refs counts our own JS object plus every live bound dyn_sp_t, so refs
       landing on 0 here means the memo slot is empty and the C state dies
       with this JS object. Otherwise release the slot's JS ref: if that
       wrapper was the last raw-ref holder, dyn_sp_free frees u -- so u is
       dead past the JS_FreeValueRT and must not be touched again. */
    if (--u->refs == 0) {
        dyn_url_free(u);
        return;
    }
    JS_FreeValueRT(rt, u->sp_wrapper);
}

static void dyn_url_gc_mark(JSRuntime *rt, JSValueConst val, JS_MarkFunc *mark)
{
    dyn_url_t *u = (dyn_url_t *)JS_GetOpaque(val, dyn_url_class_id);
    if (u)
        JS_MarkValue(rt, u->sp_wrapper, mark);
}

static JSClassDef dyn_url_class = { "URL", .finalizer = dyn_url_finalizer,
                                    .gc_mark = dyn_url_gc_mark };
                            ...
                            int file_drive, int special)
{
    ...
    while (i <= n) {
            ...
            i++;
        {
            ...
            if (dyn_is_dot_seg(p + b, len, 0)) {
                ...
                /* ".." ending the input keeps the trailing "/" only when it
                   had something to pop or the scheme is special; a ".."
                   over an already-empty path on a non-special scheme leaves
                   the path empty (node pins new URL("..", "foo://a/b") ->
                   "foo://a"). The <d/../..> family matches node too; only
                   the pure-dot ../.. over segments keeps node's one-off
                   shorter result -- a documented micro-deviation. */
                if (i >= n && (special || before > 0))
                    dyn_sb_putc(&seg, '/');
            } else {
                dyn_sb_putc(&seg, '/');
    ...
    /* a path emptied by dot-popping stays empty on a non-special scheme and
       carries the "/" for a special one (its empty segment IS the slash) */
    if (seg.n == 0) {
        if (!special)
            goto done;
        dyn_sb_putc(out, '/');
                               ...
                               int *port_colon)
{
    ...
    if (hb < n && s[hb] == '[') {            /* IPv6 literal */
        ...
        if (close + 1 < n && s[close + 1] == ':') {
            free(u->port); /* a re-assignment (file: authority fall-through, base inheritance) must not drop the previous string */
    ...
    free(u->host); /* a re-assignment (file: authority fall-through, base inheritance) must not drop the previous string */
                         ...
                         const dyn_url_t *base)
{
    ...
    }
    {
        ...
        if (ab <= n) {
            ...
            u->has_authority = base->has_authority;
            /* free() first at every slot: the file: branches above may
               already have filled these (base host inheritance runs twice
               on file:/ over a file base), and a bare re-assignment leaked
               the first string -- the conformance teardown's residue */
            if (base->host)     { free(u->host);     u->host = dyn_strndup(base->host, strlen(base->host)); }
            if (base->port)     { free(u->port);     u->port = dyn_strndup(base->port, strlen(base->port)); }
            if (base->username) { free(u->username); u->username = dyn_strndup(base->username, strlen(base->username)); }
            if (base->password) { free(u->password); u->password = dyn_strndup(base->password, strlen(base->password)); }
        ...
        if (i < n && dyn_authority_end(ps[i], special)) {
            ...
            if (is_file && rel && base && base->path &&
                ...
                if (!ref_is_drive) {
                    ...
                    dyn_remove_dots(&out, t.p, t.n, 1, special);
                    dyn_sb_free(&t);
                } else {
                    dyn_remove_dots(&out, ps + i + 1, n - i - 1, is_file, special);
                }
            } else {
                dyn_remove_dots(&out, ps + i + 1, n - i - 1, is_file, special);
            }
                ...
                out.p[2] = ':';
            {
                dyn_sb_t t;
                dyn_sb_init(&t);
                dyn_remove_dots(&t, out.p + 1, out.n - 1, 1, special);
                dyn_sb_free(&out);
                out = t;
            }
            ...
            dyn_remove_dots(&out, merged.p + skip,
                            merged.n ? merged.n - skip : 0, is_file, special);
            dyn_sb_free(&merged);
        } else {
            dyn_remove_dots(&out, ps + i, n - i, is_file, special);
        }
...
/* The URL serializer into an existing buffer. Split out of dyn_url_href so
 the setters can build a candidate URL with one field substituted and
   reparse it -- the percent-encoding, IDNA and port rules then live in the
   parser alone. */
static void dyn_url_serialize(dyn_sb_t *b, const dyn_url_t *u)
{
...
/* The href string: the serializer above, nothing else. */
...
/* The full constructor pipeline, shared by the constructor itself and the
 URL.parse/canParse/join statics. With quiet = 0 every failure throws
   exactly as the constructor always has; with quiet = 1 every failure the
   constructor would THROW for (bad base, over-long input, unparsable input)
   yields JS_NULL instead -- only OOM and a throwing coercion surface as
   exceptions, so URL.parse never throws for a parse failure. new_target is
   the constructor's subclass hook; the statics pass JS_UNDEFINED (the plain
   class prototype). */
static JSValue dyn_url_build(JSContext *ctx, JSValueConst new_target,
                             int argc, JSValueConst *argv, int quiet)
{
    ...
    free(base.drive);
    u->refs = 1;
    /* Wrap with new_target's prototype so `class MyURL extends URL` sees
       MyURL's methods (the plain class proto used to swallow them). */
    {
        JSValue proto, obj;
        if (JS_IsUndefined(new_target)) {
            proto = JS_GetClassProto(ctx, dyn_url_class_id);
        } else {
        ...
        obj = JS_NewObjectProtoClass(ctx, proto, dyn_url_class_id);
        JS_FreeValue(ctx, proto);
...
{
    return dyn_url_build(ctx, new_target, argc, argv, 0);
}

/* URL.parse(input, base?): the constructor's pipeline with quiet=1 -- JS_NULL
 * for every failure the constructor would throw for. A URL instance otherwise,
 * with the plain class prototype (WHATWG URL.parse returns a base URL, never
 * a subclass of the receiver). */
static JSValue js_url_parse_static(JSContext *ctx, JSValueConst this_val,
                                   int argc, JSValueConst *argv)
{
    (void)this_val;
    return dyn_url_build(ctx, JS_UNDEFINED, argc, argv, 1);
}

/* URL.canParse(input, base?): parse() as a predicate. */
static JSValue js_url_canparse_static(JSContext *ctx, JSValueConst this_val,
                                      int argc, JSValueConst *argv)
{
    JSValue r;
    (void)this_val;
    r = dyn_url_build(ctx, JS_UNDEFINED, argc, argv, 1);
    if (JS_IsException(r))
        return r;
    JS_FreeValue(ctx, r);
    return JS_IsNull(r) ? JS_FALSE : JS_TRUE;
}

/* URL.join(base, rel): resolved href of <rel> against <base>. Argument order
 * is (base, reference) as in the plan; the parse itself is new URL(rel,
 * base). Both failure modes THROW (TypeError, constructor texts): an
 * unparseable base or reference in a join is a caller bug, and a null return
 * would be one silent string-surgery bug away from a wrong fetch. */
static JSValue js_url_join_static(JSContext *ctx, JSValueConst this_val,
                                  int argc, JSValueConst *argv)
{
    JSValue args[2], obj, r;
    dyn_url_t *u;
    (void)this_val;
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "URL.join(base, rel): base and rel are required");
    args[0] = argv[1];              /* rel: the input */
    args[1] = argv[0];              /* base */
    obj = dyn_url_build(ctx, JS_UNDEFINED, 2, args, 0);
    if (JS_IsException(obj))
        return obj;
    u = dyn_url_of(ctx, obj);
    if (!u) {
        JS_FreeValue(ctx, obj);
        return JS_EXCEPTION;
    }
    r = dyn_url_href(ctx, u);
    JS_FreeValue(ctx, obj);
    return r;
}
...
typedef struct {
    ...
    dyn_url_t *host;          /* borrowed; the raw ref keeps the URL's C state alive */
    dyn_sp_list_t list;       /* parsed cache of the current query; valid when
                                 list_valid and cached_version matches */
    int list_valid;
    int list_canonical;       /* the query string IS the list's serialization
                                 (only meaningful when list_valid): concat
                                 append is exact only over such a query.
                                 INVARIANT: 1 only while the current query
                                 is canonical -- ref() recomputes it on every
                                 (re)parse, and every mutator's serialized
                                 output is canonical by construction, so it
                                 never goes stale behind a sibling's write. */
    uint32_t cached_version;  /* the version `list` was parsed from */
    uint32_t query_version;   /* standalone mode's own version counter */
} dyn_sp_t;
...
static JSValue dyn_url_get(JSContext *ctx, JSValueConst this_val, int magic)
{
    ...
    case U_HASH:
        ...
        break;
    case U_SEARCHPARAMS: {
        /* [SameObject] (WHATWG URL sec.6.1): the getter returns THE URL's
           query object, one per URL, created lazily on first access. A fresh
           wrapper per access made the natural `for (..) u.searchParams
           .append(..)` idiom O(n^2): every new wrapper's first mutation
           reparsed the entire query. The memo lives in dyn_url_t (one JS ref,
           traced by dyn_url_gc_mark, released by dyn_url_finalizer); writes
           through other bound instances -- none exist per URL any more, but
           the version counter still guards the general case -- invalidate its
           parsed cache exactly as before. */
        if (JS_IsUndefined(u->sp_wrapper)) {
            dyn_sp_t *sp = (dyn_sp_t *)calloc(1, sizeof(*sp));
            if (!sp)
                return JS_ThrowOutOfMemory(ctx);
            sp->host = u;
            u->refs++;
            u->sp_wrapper = dyn_plain_wrap(ctx, JS_UNDEFINED, dyn_sp_class_id, sp, dyn_sp_free);
            if (JS_IsException(u->sp_wrapper)) {
                /* dyn_plain_wrap ran dyn_sp_free on failure, which already
                   dropped the raw ref we took */
                u->sp_wrapper = JS_UNDEFINED;
                return JS_EXCEPTION;
            }
        }
        return JS_DupValue(ctx, u->sp_wrapper);
    }
...
/* ----: the WHATWG setters --------------------------------------------
 *
 * Each setter builds the URL text with ONE field substituted by the caller's
 * raw value and reparses it through this module's own parser, so the
 * normalisation rules (percent-encoding, IDNA, lowercasing, default-port
 * elision, path resolution) exist in exactly one place. A value the parser
 * refuses is a NO-OP, which is the specification's own rule for a setter it
 * cannot represent. Delimiters inside the value move the components the
 * spec's state override moves: a '?' in `pathname` starts the query, a '#'
 * starts the fragment, and a `host` value may carry `:port`. */

enum { URLF_PATH, URLF_QUERY, URLF_FRAGMENT, URLF_HOST, URLF_PORT };

static JSValue dyn_sp_ctor(JSContext *ctx, JSValueConst new_target,
                           int argc, JSValueConst *argv);   /* for searchParams= */

/* The URL text with `field` swapped for `raw` (an empty raw clears the
   component). The PATH and QUERY values are inserted RAW and every untouched
   component after them is serialized, so a delimiter inside the value starts
   the component the spec's state override would start there; the rest of the
   URL is encoded exactly as its own serializer would. */
static char *dyn_url_candidate(const dyn_url_t *u, int field, const char *raw,
                               size_t *outn)
{
    dyn_sb_t b;
    dyn_url_t tmp = *u;
    char *r;
    const char *v = (raw && *raw) ? raw : NULL;

    switch (field) {
    case URLF_PATH:
    case URLF_QUERY:
        /* prefix: everything up to the component being replaced */
        tmp.path = field == URLF_PATH ? NULL : tmp.path;
        tmp.query = NULL;
        tmp.fragment = NULL;
        dyn_sb_init(&b);
        dyn_url_serialize(&b, &tmp);
        if (field == URLF_QUERY && v)
            dyn_sb_putc(&b, '?');
        if (v)
            dyn_sb_puts(&b, v);
        break;
    case URLF_FRAGMENT: tmp.fragment = (char *)v; break;
    case URLF_HOST:     /* NOT built here: the host setter's value carries
                           its own host:port split (the WHATWG state
                           override's, which differs observably from this
                           parser's) -- see dyn_url_apply_host. */
                        return NULL;
    case URLF_PORT:     tmp.port = (char *)v; break;
    default:            return NULL;
    }
    if (field == URLF_FRAGMENT || field == URLF_PORT) {
        dyn_sb_init(&b);
        dyn_url_serialize(&b, &tmp);
    }
    if (b.oom) { dyn_sb_free(&b); return NULL; }
...
/* Copy an optional field over, freeing the old one. */
static int dyn_url_put(char **slot, const char *v)
{
    char *n = v ? dyn_strndup(v, strlen(v)) : NULL;
    if (v && !n)
        return -1;
    free(*slot);
    *slot = n;
...
/* Reparse `text` and adopt what the parser produced. `move_query`/`move_frag`
 * carry the delimiters the caller's value contained: only then does the parse
 * get to replace those components (the spec's state override leaves the rest
 * of the URL alone). Returns 1 changed, 0 a no-op, -1 on OOM. */
static int dyn_url_adopt(dyn_url_t *u, const char *text, size_t n, int field,
                         int move_query, int move_frag)
{
    dyn_url_t fresh;
    int rc = 1;

    memset(&fresh, 0, sizeof fresh);
    if (dyn_url_parse(&fresh, text, n, NULL) != 0) {
        /* A REFUSED parse still leaves the partial fields the parser had
           already allocated in `fresh` (it never frees on its failure
           paths -- its callers do). Free them here: every refused setter
           call used to leak exactly this set (the host = "n:81" shape ran
           the whole thing 2k times and stood ~7 allocations per call). */
        free(fresh.scheme); free(fresh.username); free(fresh.password);
        free(fresh.host); free(fresh.port); free(fresh.path);
        free(fresh.query); free(fresh.fragment); free(fresh.drive);
        return 0;                       /* unrepresentable: no-op */
    }
    if (dyn_url_put(&u->path, fresh.path) < 0
        || ((field == URLF_PATH || field == URLF_QUERY)
            && move_query && dyn_url_put(&u->query, fresh.query) < 0)
        || ((field == URLF_PATH || field == URLF_QUERY || field == URLF_FRAGMENT)
            && move_frag && dyn_url_put(&u->fragment, fresh.fragment) < 0)
        || ((field == URLF_HOST || field == URLF_PORT)
            && (dyn_url_put(&u->host, fresh.host) < 0
                || dyn_url_put(&u->port, fresh.port) < 0)))
        rc = -1;
    if (rc == 1)
        u->query_version++;             /* bound searchParams revalidate */
    free(fresh.scheme); free(fresh.username); free(fresh.password);
    free(fresh.host); free(fresh.port); free(fresh.path);
    free(fresh.query); free(fresh.fragment); free(fresh.drive);
    return rc;
...
/* The setter body: `raw` is the value after the per-property input rules
 * (a leading '#'/'?' stripped, the path rooted). */
static JSValue dyn_url_apply(JSContext *ctx, dyn_url_t *u, int field,
                             const char *raw, int move_query, int move_frag)
{
    dyn_sb_t b;
    char *cand;
    size_t cn = 0;
    ...
    dyn_sb_init(&b);
    {
        char *core;                    /* candidate + untouched trailing parts */
        dyn_sb_t head;
        dyn_sb_init(&head);
        core = dyn_url_candidate(u, field, raw, &cn);
        if (!core) { dyn_sb_free(&head); return JS_ThrowOutOfMemory(ctx); }
        if (field == URLF_PATH) {
            /* A '#' in the value starts the fragment, so the URL's own query
               belongs BEFORE it (the parse never reaches query state). */
            const char *h = strchr(core, '#');
            if (h)
                dyn_sb_put(&head, core, (size_t)(h - core));
            else
                dyn_sb_put(&head, core, cn);
            if (!move_query && u->query) {
                dyn_sb_putc(&head, '?');
                dyn_href_component(&head, u->query, DYN_PCT_QUERY);
            }
            if (h)
                dyn_sb_putc(&head, '#'), dyn_sb_puts(&head, h + 1);
        } else {
            dyn_sb_put(&head, core, cn);
            if (!move_frag && u->fragment) {
                dyn_sb_putc(&head, '#');
                dyn_href_component(&head, u->fragment, DYN_PCT_FRAGMENT);
            }
        }
        free(core);
        if (head.oom) { dyn_sb_free(&head); return JS_ThrowOutOfMemory(ctx); }
        cand = dyn_strndup(head.p ? head.p : "", head.n);
        cn = head.n;
        dyn_sb_free(&head);
    }
    dyn_sb_free(&b);
    if (!cand)
        return JS_ThrowOutOfMemory(ctx);
    rc = dyn_url_adopt(u, cand, cn, field, move_query, move_frag);
    free(cand);
    if (rc < 0)
        return JS_ThrowOutOfMemory(ctx);
    return JS_UNDEFINED;
...
/* The WHATWG host/hostname setter body. `raw` is the setter value;
 * `allow_port` is 1 for `host` and 0 for `hostname` (whose value must not
 * name a port at all -- even an empty one).
 *
 * The value's split differs from this parser's and the difference is
 * observable (every row verified against the reference implementation):
 * the port is the run of ASCII digits that STARTS the port text -- anything
 * after the digits ends the state-override parse and is silently ignored
 * (host = "n:81:8080" and host = "n:81x" both set port 81) -- and a port
 * text that does not fit (no leading digit, or a number above 65535:
 * host = "n:x", host = "n:65536") is IGNORED while the host still applies
 * and the old port stays. A value with no port at all, including a bare
 * trailing colon (host = "n:"), keeps the old port; an empty host or a
 * host the parser refuses (forbidden code points: host = "n m") makes the
 * whole setter a no-op. The candidate text must carry EXACTLY the new
 * authority: building it with both the value's port and the old one (the
 * scheme-prefixed "n:81:8080/a" shape) is what made host = "n:81" a silent
 * no-op. */
static JSValue dyn_url_apply_host(JSContext *ctx, dyn_url_t *u, const char *raw,
                                  int allow_port)
{
    dyn_sb_t b;
    dyn_url_t tmp;
    ...
    if (raw[0] == '[') {
        size_t close = 0;
        while (close < n && raw[close] != ']')
            close++;
        if (close >= n)
            return JS_UNDEFINED;        /* unterminated literal: no-op */
        he = close + 1;
        if (he < n) {
            if (raw[he] != ':')
                return JS_UNDEFINED;    /* junk after ']': no-op */
            port = raw + he + 1;
        }
    } else {
        for (i = 0; i < n; i++)
            if (raw[i] == ':') { he = i; port = raw + i + 1; break; }
    }
    if (he == 0)
        return JS_UNDEFINED;            /* empty host: no-op */
    if (raw[0] != '[') {
        /* Forbidden host code points refuse the WHOLE setter before any
           split can reinterpret them (an '@' would otherwise be read as
           userinfo by the re-parse and set the WRONG host): the same set
           for special and non-special schemes (reference-verified). */
        size_t j;
        for (j = 0; j < he; j++) {
    ...
    if (port && !allow_port)
        return JS_UNDEFINED;            /* hostname= naming a port: no-op */
    newport = u->port;                  /* no port in the value: it stays */
    if (port && port[0]) {
        size_t d = 0;
        unsigned long long v = 0;
        int bad = 0;
        while (port[d] >= '0' && port[d] <= '9') {
            v = v * 10 + (unsigned long long)(port[d] - '0');
            if (v > 65535) {
                bad = 1;                /* out of range: ignore the port */
                break;
            }
            d++;
        }
        if (!bad && d > 0) {
            snprintf(portbuf, sizeof portbuf, "%llu", v);
            newport = portbuf;
        }
        /* else: the port text is not a fitting digit run and is IGNORED
           (host = "n:x" / host = "n:65536" set the host, keep the port) */
    }
    hostpart = dyn_strndup(raw, he);
    if (!hostpart)
        return JS_ThrowOutOfMemory(ctx);
    tmp = *u;
    tmp.host = hostpart;
    tmp.port = (char *)newport;
    dyn_sb_init(&b);
    dyn_url_serialize(&b, &tmp);
    free(hostpart);
    if (b.oom) { dyn_sb_free(&b); return JS_ThrowOutOfMemory(ctx); }
    cand = dyn_strndup(b.p ? b.p : "", b.n);
    cn = b.n;
    dyn_sb_free(&b);
    if (!cand)
        return JS_ThrowOutOfMemory(ctx);
    rc = dyn_url_adopt(u, cand, cn, URLF_HOST, 0, 0);
    free(cand);
    if (rc < 0)
        return JS_ThrowOutOfMemory(ctx);
    return JS_UNDEFINED;
...
static int dyn_url_has_byte(const char *s, size_t n, char c)
{
    return memchr(s, c, n) != NULL;
}

static JSValue dyn_url_set(JSContext *ctx, JSValueConst this_val,
                           JSValueConst val, int magic)
{
    ...
    size_t n = 0;
    JSValue r;
    int move_q = 0, move_f = 0;

    if (!u)
        return JS_EXCEPTION;
    /* `s` may advance past a leading delimiter; `base` is what FreeCString
       must see -- an interior pointer scribbles the atom table. */
    base = JS_ToCStringLen(ctx, &n, val);   /* the spec's ToString coercion */
    if (!base)
        return JS_EXCEPTION;
    s = base;
    switch (magic) {
    case U_HASH:
        if (n && s[0] == '#') { s++; n--; }
        move_f = 1;                     /* the value IS the fragment */

        if (n == 0) { free(u->fragment); u->fragment = NULL; }
        raw = dyn_strndup(s, n);
        break;
    case U_SEARCH:
        if (n && s[0] == '?') { s++; n--; }
        move_q = 1;
        move_f = dyn_url_has_byte(s, n, '#');
        if (n == 0) { free(u->query); u->query = NULL; u->query_version++; }
        raw = dyn_strndup(s, n);
        break;
    case U_PATHNAME: {
        dyn_sb_t b;
        if (n == 0 && u->has_authority) {
            raw = dyn_strndup("/", 1);
        } else if (u->has_authority && s[0] != '/') {
            dyn_sb_init(&b);
            dyn_sb_putc(&b, '/');
            dyn_sb_put(&b, s, n);
            if (b.oom) { dyn_sb_free(&b); JS_FreeCString(ctx, base);
                         return JS_ThrowOutOfMemory(ctx); }
            raw = dyn_strndup(b.p, b.n);
            dyn_sb_free(&b);
        } else {
            raw = dyn_strndup(s, n);
        }
        {
            size_t rl = raw ? strlen(raw) : 0;
            move_q = dyn_url_has_byte(raw ? raw : "", rl, '?');
            move_f = dyn_url_has_byte(raw ? raw : "", rl, '#');
        }
        break;
    }
    case U_HOSTNAME:
        /* a forbidden host code point makes the setter a no-op */
        if (!u->has_authority || !u->host || n == 0
            || dyn_url_has_byte(s, n, '/') || dyn_url_has_byte(s, n, '?')
            || dyn_url_has_byte(s, n, '#')) {
    ...
    case U_HOST:
        ...
        raw = dyn_strndup(s, n);        /* host, or host:port (split later) */
        break;
    case U_PORT: {
        /* an empty value clears the port; anything non-numeric is a no-op */
        size_t i;
        if (!u->has_authority || !u->host) {
            JS_FreeCString(ctx, base);
            return JS_UNDEFINED;
        }
        if (n == 0) {
    ...
    case U_SEARCHPARAMS: {
        /* replace the query with the serialization of new URLSearchParams(v),
           which is what the spec's setter is defined to do */
        JSValue spv = dyn_sp_ctor(ctx, JS_UNDEFINED, 1, &val);
        dyn_sp_t *sp;
        if (JS_IsException(spv)) { JS_FreeCString(ctx, base); return JS_EXCEPTION; }
        sp = (dyn_sp_t *)JS_GetOpaque(spv, dyn_sp_class_id);
        if (!sp) { JS_FreeValue(ctx, spv); JS_FreeCString(ctx, base);
                   return JS_EXCEPTION; }
        if (dyn_url_put(&u->query, sp->query && *sp->query ? sp->query : NULL) < 0) {
            JS_FreeValue(ctx, spv);
            JS_FreeCString(ctx, base);
            return JS_ThrowOutOfMemory(ctx);
        }
        u->query_version++;
    ...
    if (!raw)
        return JS_ThrowOutOfMemory(ctx);
    if (magic == U_HOST || magic == U_HOSTNAME) {
        /* the host setters carry their own value split (the WHATWG state
           override's) -- see dyn_url_apply_host */
        r = dyn_url_apply_host(ctx, u, raw, magic == U_HOST);
        free(raw);
        return r;
    }
    r = dyn_url_apply(ctx, u, magic == U_PORT ? URLF_PORT
                          : magic == U_PATHNAME ? URLF_PATH
                          : magic == U_SEARCH ? URLF_QUERY : URLF_FRAGMENT,
                      raw, move_q, move_f);
    free(raw);
    return r;
}
...
static const JSCFunctionListEntry dyn_url_proto[] = {
    ...
    JS_CGETSET_MAGIC_DEF("host", dyn_url_get, dyn_url_set, U_HOST),
    JS_CGETSET_MAGIC_DEF("hostname", dyn_url_get, dyn_url_set, U_HOSTNAME),
    JS_CGETSET_MAGIC_DEF("port", dyn_url_get, dyn_url_set, U_PORT),
    JS_CGETSET_MAGIC_DEF("pathname", dyn_url_get, dyn_url_set, U_PATHNAME),
    JS_CGETSET_MAGIC_DEF("search", dyn_url_get, dyn_url_set, U_SEARCH),
    JS_CGETSET_MAGIC_DEF("hash", dyn_url_get, dyn_url_set, U_HASH),
    JS_CGETSET_MAGIC_DEF("origin", dyn_url_get, NULL, U_ORIGIN),
    JS_CGETSET_MAGIC_DEF("searchParams", dyn_url_get, dyn_url_set, U_SEARCHPARAMS),
...
/* URLSearchParams (WHATWG): the query list. Two modes: a BOUND instance came
   ...
   query. The spec binds ONE URLSearchParams per URL ([SameObject]
   searchParams): url.searchParams hands back the memoized wrapper in
   dyn_url_t, and every URL has at most one bound instance.
 entries/keys/values return IterableIterator objects (WHATWG
   shape) over a live index into the current pair list; the former ARRAY
   returns are kept as entriesArray()/keysArray()/valuesArray(). Instance
   [Symbol.iterator] IS the entries iterator, so spread, for..of, Array.from
   and new Map work directly on the object. sort() orders by the DECODED key's
   byte order, which for astral code
   points can diverge from the UTF-16 code-unit order the spec requires.
   READS AND WRITES ARE AMORTIZED-O(1) IN THE QUERY LENGTH beyond one parse:
   the parsed pair list is cached in the instance and revalidated against the
   owning query's version counter, so an append loop no longer reparses the
   whole query per call (it was O(n^2) bytes: 20000 appends took 25 s). */
...
/* The owning query's version: bound instances track the URL's counter (a
   sibling bound instance may have written through), standalone ones their
   own. */
static uint32_t dyn_sp_version(const dyn_sp_t *sp)
{
    return sp->host ? sp->host->query_version : sp->query_version;
}
...
static int dyn_sp_set_query(dyn_sp_t *sp, const char *q, size_t n)
{
    ...
    free(*slot);
    *slot = nq;
    /* the write invalidates every parsed cache over this slot, including
       this instance's (rebuilt lazily by dyn_sp_list_ref) */
    if (sp->host)
        sp->host->query_version++;
    else
        sp->query_version++;
    return 0;
}
...
/* The parsed view of the current query. Reads used to reparse the whole
   query string per call (500 gets over 500 pairs = 500 full reparses); the
   cached list is revalidated against the version instead. Mutators that
   rebuild the list in place resync cached_version after dyn_sp_apply so the
   next read does not reparse; append takes a concat fast path below and
   leaves the cache stale on purpose. */
/* Does `q` already read as its own urlencoded serialization? A query is a
   canonical fixed point iff every '&' segment is non-empty, carries its '=',
   and is made only of bytes that both decode verbatim and re-encode
   verbatim under the form-2 set (no '%', no '+', nothing the set escapes).
   Over such a query, append-by-concatenation is byte-identical to the
   parse+append+serialize round trip the other mutators run; a query like
   "a=1&&b=2" (empty segment) or "a?b=c" (escapable byte) is not. */
static int dyn_sp_query_canonical(const char *q, size_t n)
{
    size_t i = 0, seg = 0;
    int has_eq = 0;
    for (i = 0; i <= n; i++) {
        unsigned char c = (i < n) ? (unsigned char)q[i] : '&';
        if (c == '&') {
            if (i == seg || !has_eq)
                return 0;        /* empty segment, or a bare key with no '=' */
            seg = i + 1;
            has_eq = 0;
        } else {
            if (c == '=')
                has_eq = 1;
                     ...
                     c == '#' || c == '<' || c == '>' ||
                     c <= 0x1F || c >= 0x7F)
                return 0;        /* decodes or re-encodes to other bytes */
        }
    }
    return 1;
}

static dyn_sp_list_t *dyn_sp_list_ref(dyn_sp_t *sp)
{
    uint32_t ver = dyn_sp_version(sp);
    if (!sp->list_valid || ver != sp->cached_version) {
        const char *q = dyn_sp_query_str(sp);
        dyn_sp_list_free(&sp->list);
        dyn_sp_list_init(&sp->list);
        dyn_sp_parse(q, strlen(q), &sp->list);
        sp->list_valid = 1;
        sp->list_canonical = dyn_sp_query_canonical(q, strlen(q));
        sp->cached_version = ver;
    }
    return &sp->list;
}

/* Mark the cached list unusable: a mutator failed midway and the list may no
   longer agree with the serialized query. */
static void dyn_sp_cache_drop(dyn_sp_t *sp)
{
    sp->list_valid = 0;
}
...
static void dyn_sp_parse(const char *q, size_t n, dyn_sp_list_t *l)
{
    ...
    while (i < n) {
        ...
        if (i == b) {              /* empty segment ("&&", trailing &):
                                       WHATWG skips it, node has 0 pairs */
            i++;
            continue;
        }
        dyn_sb_init(&k);
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    if (dyn_sp_kv(ctx, argc, argv, &ks, &kn, &vs, &vn) < 0)
        return JS_EXCEPTION;
    /* Append over a CANONICAL query is exact by concatenation (the '&'
       separator and the form-2 encoding of k/v are already the canonical
       spellings), so an append loop costs the appended bytes only. Anything
       else -- an unparsed cache, a URL-inherited query with empty segments
       or escapable bytes -- takes the round trip, whose output IS canonical,
       so at most the first append pays. The version bump in
       dyn_sp_set_query invalidates the parsed cache for the next read. */
    if (sp->list_valid && sp->list_canonical) {
        const char *q = dyn_sp_query_str(sp);
        size_t ql = strlen(q);
        dyn_sb_t out;
        dyn_sb_init(&out);
        ...
        dyn_sb_putc(&out, '=');
        dyn_pct_encode(&out, vs ? vs : "", vn, 2);
        if (out.oom || dyn_sp_set_query(sp, out.p ? out.p : "", out.n) < 0) {
            dyn_sb_free(&out);
            JS_FreeCString(ctx, ks);
        ...
        dyn_sp_list_t *l = dyn_sp_list_ref(sp);
        if (dyn_sp_list_put(l, ks, kn, vs ? vs : "", vn) < 0 ||
            dyn_sp_apply(sp, l) < 0) {
            dyn_sp_cache_drop(sp);
            JS_FreeCString(ctx, ks);
            if (vs) JS_FreeCString(ctx, vs);
            return JS_ThrowOutOfMemory(ctx);
        }
        sp->cached_version = dyn_sp_version(sp);   /* l IS the new state */
        sp->list_canonical = 1;                    /* serialize output is */
    }
    JS_FreeCString(ctx, ks);
    if (vs) JS_FreeCString(ctx, vs);
    return JS_UNDEFINED;
                          ...
                          int argc, JSValueConst *argv)
{
    ...
    if (!sp)
        return JS_EXCEPTION;
    /* The spec's has(name, value) form: with a second argument only pairs
       matching BOTH count (delete() had the two-arg form first; has got
       parity). Missing/undefined value keeps the name-only check. */
    if (dyn_sp_kv(ctx, argc, argv, &ks, &kn, &vs, &vn) < 0)
        return JS_EXCEPTION;
    l = dyn_sp_list_ref(sp);
    for (i = 0; i < l->n; i++)
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    for (i = 0; i < l->n; i++)
        if (strcmp(l->a[i].k, ks) != 0 ||
            ...
            /* ASan LeakSanitizer (gate): the removed pair's strings
               used to be orphaned here -- positions past w are never read
               again, and dyn_sp_list_free only walks i < l->n, so every
               delete of a matching pair permanently lost its k/v mallocs. */
            free(l->a[i].k);
            free(l->a[i].v);
        }
    l->n = w;
    if (dyn_sp_apply(sp, l) < 0) {
        rc = -1;
        dyn_sp_cache_drop(sp);   /* l may disagree with the slot now */
    } else {
        sp->cached_version = dyn_sp_version(sp);   /* l IS the new state */
        sp->list_canonical = 1;
    }
                          ...
                          int argc, JSValueConst *argv)
{
    ...
    for (i = 0; i < l->n; i++) {
        if (!replaced && strcmp(l->a[i].k, ks) == 0) {
            ...
            /* a later duplicate of the matched key: dropped by compaction.
 ASan LeakSanitizer (gate): its strings used to be
               orphaned exactly like delete's (same walk, same list_free). */
            free(l->a[i].k);
            free(l->a[i].v);
        }
    }
    ...
    /* l is the borrowed cache: drop it on the failure paths (it may no
       longer agree with the serialized query), never free it here */
    if (rc)
        dyn_sp_cache_drop(sp);
    JS_FreeCString(ctx, ks);
    if (vs) JS_FreeCString(ctx, vs);
...
/* ---------------- iteration ----------------
 * entries()/keys()/values() return IterableIterator objects (WHATWG shape)
 * over a LIVE index into the current pair list -- the list is re-resolved on
 * every next(), so a mutation between steps is observed exactly as the spec's
 * index-based iteration observes it. The former ARRAY returns moved to
 * entriesArray/keysArray/valuesArray ('s toArray twins) -- that is
 * a documented BEHAVIOR CHANGE for code that indexed or measured the old
 * arrays. [Symbol.iterator] on the instance is the entries iterator, so
 * for..of / spread / Array.from / new Map work on the instance directly. */

/* mode: 0 keys, 1 values, 2 entries. The iterator holds one JS ref to the
   URLSearchParams instance (traced by gc_mark -- a bound instance's URL C
   state stays alive through it), plus a read cursor. The C list pointer is
   NEVER cached: dyn_sp_list_ref re-resolves the parsed cache on every next(),
   so a concurrent mutation can never leave the iterator holding a dropped
   cache. */
typedef struct {
    JSValue sp;               /* the URLSearchParams instance (JS ref) */
    size_t idx;               /* read cursor */
    int mode;
    int finished;             /* WHATWG: once the walk exhausts, the iterator
                                 stays done forever -- pairs appended after
                                 exhaustion are NOT resurrected by it */
} dyn_spi_t;

static JSClassID dyn_spi_class_id;

static void dyn_spi_finalizer(JSRuntime *rt, JSValue val)
{
    dyn_spi_t *it = (dyn_spi_t *)JS_GetOpaque(val, dyn_spi_class_id);
    if (!it)
        return;
    JS_FreeValueRT(rt, it->sp);
    free(it);
}

static void dyn_spi_gc_mark(JSRuntime *rt, JSValueConst val, JS_MarkFunc *mark)
{
    dyn_spi_t *it = (dyn_spi_t *)JS_GetOpaque(val, dyn_spi_class_id);
    if (it)
        JS_MarkValue(rt, it->sp, mark);
}

static JSClassDef dyn_spi_class = { "URLSearchParams Iterator",
                                    .finalizer = dyn_spi_finalizer,
                                    .gc_mark = dyn_spi_gc_mark };

static JSValue dyn_spi_next(JSContext *ctx, JSValueConst this_val,
                            int argc, JSValueConst *argv)
{
    dyn_spi_t *it = (dyn_spi_t *)dyn_plain_get(ctx, this_val, dyn_spi_class_id);
    dyn_sp_t *sp;
    dyn_sp_list_t *l;
    JSValue res, val;
    (void)argc; (void)argv;
    if (!it)
        return JS_EXCEPTION;
    sp = (dyn_sp_t *)dyn_plain_get(ctx, it->sp, dyn_sp_class_id);
    if (!sp)
        return JS_EXCEPTION;    /* unreachable: the iterator holds a JS ref */
    l = dyn_sp_list_ref(sp);
    if (it->finished || it->idx >= l->n) {
        JSValue res;
        /* Exhausted: {value: undefined, done: true} -- the spec-literal
           shape, with `value` PRESENT and `done` PRESENT and truthy (an
           unset done is falsy and the spread/for-of protocol would iterate
           forever -- that exact bug hung the consumer once). The finished
           flag latches: WHATWG iterators do not resume when pairs are
           appended after exhaustion (live-vs-finished split: mutation is
           observed BETWEEN steps, never after the walk ends). */
        it->finished = 1;
        res = JS_NewObject(ctx);
        if (JS_IsException(res))
            return res;
        JS_SetPropertyStr(ctx, res, "value", JS_UNDEFINED);
        JS_SetPropertyStr(ctx, res, "done", JS_TRUE);
        return res;
    }
    res = JS_NewObject(ctx);
    if (JS_IsException(res))
    ...
    if (it->mode == 2) {
        ...
        JS_SetPropertyUint32(ctx, pair, 0, JS_NewString(ctx, l->a[it->idx].k));
        JS_SetPropertyUint32(ctx, pair, 1, JS_NewString(ctx, l->a[it->idx].v));
        val = pair;
    } else if (it->mode == 1) {
        val = JS_NewString(ctx, l->a[it->idx].v);
    } else {
        val = JS_NewString(ctx, l->a[it->idx].k);
    }
...
/* The iterator protocol's self-return: [Symbol.iterator]() -> the iterator. */
static JSValue dyn_spi_self(JSContext *ctx, JSValueConst this_val,
                            int argc, JSValueConst *argv)
{
    (void)argc; (void)argv;
...
static const JSCFunctionListEntry dyn_spi_proto[] = {
    JS_CFUNC_DEF("next", 0, dyn_spi_next),
    JS_CFUNC_DEF("[Symbol.iterator]", 0, dyn_spi_self),
};

/* Build an iterator over `this` (the URLSearchParams instance). */
static JSValue dyn_sp_iter_new(JSContext *ctx, JSValueConst this_val, int mode)
{
    dyn_sp_t *sp = dyn_sp_of(ctx, this_val);
    dyn_spi_t *it;
    JSValue proto, obj;
    if (!sp)
        return JS_EXCEPTION;
    it = (dyn_spi_t *)calloc(1, sizeof(*it));
    if (!it)
        return JS_ThrowOutOfMemory(ctx);
    it->sp = JS_DupValue(ctx, this_val);
    it->mode = mode;
    proto = JS_GetClassProto(ctx, dyn_spi_class_id);
    obj = JS_NewObjectProtoClass(ctx, proto, dyn_spi_class_id);
    JS_FreeValue(ctx, proto);
    if (JS_IsException(obj)) {
        JS_FreeValueRT(JS_GetRuntime(ctx), it->sp);
        free(it);
        return obj;
    }
    JS_SetOpaque(obj, it);
...
{
    (void)argc; (void)argv;
    return dyn_sp_iter_new(ctx, this_val, 0);
}

static JSValue dyn_sp_values_iter(JSContext *ctx, JSValueConst this_val,
                                  int argc, JSValueConst *argv)
{
    (void)argc; (void)argv;
    return dyn_sp_iter_new(ctx, this_val, 1);
}
...
{
    (void)argc; (void)argv;
    return dyn_sp_iter_new(ctx, this_val, 2);
}

/* [Symbol.iterator] over the INSTANCE: the entries iterator. */
...
/* toArray twins: the pre-UL-3 array forms, for code that indexed or
   measured entries()/keys()/values() before they grew iterator forms. */
static JSValue dyn_sp_keys_array(JSContext *ctx, JSValueConst this_val,
                                 int argc, JSValueConst *argv)
{
    dyn_sp_t *sp = dyn_sp_of(ctx, this_val);
    (void)argc; (void)argv;
...
static JSValue dyn_sp_values_array(JSContext *ctx, JSValueConst this_val,
                                   int argc, JSValueConst *argv)
{
    dyn_sp_t *sp = dyn_sp_of(ctx, this_val);
    (void)argc; (void)argv;
...
static JSValue dyn_sp_entries_array(JSContext *ctx, JSValueConst this_val,
                                    int argc, JSValueConst *argv)
{
    dyn_sp_t *sp = dyn_sp_of(ctx, this_val);
    (void)argc; (void)argv;
                           ...
                           int argc, JSValueConst *argv)
{
        ...
        if (JS_IsArray(ctx, argv[0])) {
            ...
            for (i = 0; i < len; i++) {
                ...
                {   JSValue lv = JS_GetPropertyStr(ctx, pr, "length");
                    ...
                    JS_FreeValue(ctx, lv); }
                if (prlen == 0) {
                    /* an empty pair contributes nothing; reading index 0 of
                       [] would stringify the hole into the literal
                       "undefined" key */
                    JS_FreeValue(ctx, pr);
                    continue;
                }
    ...
    /* Wrap with new_target's prototype so `class X extends URLSearchParams`
       instances carry X's methods (the plain class proto otherwise swallows
       them). Mirrors the dyna-libc worker/textdecoder ctor shape. */
    {
        JSValue proto, obj;
        if (JS_IsUndefined(new_target)) {
            proto = JS_GetClassProto(ctx, dyn_sp_class_id);
        } else {
            proto = JS_GetPropertyStr(ctx, new_target, "prototype");
            if (JS_IsException(proto)) {
                dyn_sp_free(sp);
        ...
        obj = JS_NewObjectProtoClass(ctx, proto, dyn_sp_class_id);
        JS_FreeValue(ctx, proto);
        if (JS_IsException(obj)) {
            dyn_sp_free(sp);
            return obj;
...
static const JSCFunctionListEntry dyn_sp_proto[] = {
    ...
    /*iterator forms (WHATWG shape); the array returns live on as the
       *Array() twins. [Symbol.iterator] is the entries iterator. */
    JS_CFUNC_DEF("keys", 0, dyn_sp_keys),
    JS_CFUNC_DEF("values", 0, dyn_sp_values_iter),
    JS_CFUNC_DEF("entries", 0, dyn_sp_entries),
    JS_CFUNC_DEF("[Symbol.iterator]", 0, dyn_sp_symbol_iterator),
    JS_CFUNC_DEF("keysArray", 0, dyn_sp_keys_array),
    JS_CFUNC_DEF("valuesArray", 0, dyn_sp_values_array),
    JS_CFUNC_DEF("entriesArray", 0, dyn_sp_entries_array),
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    while (i < n) {
        ...
        if (i > b) {
            ...
            /* Decoding can only SHRINK, so pre-size each builder from its
               segment length: seed = need makes the curve's first allocation
               the exact bound, and the per-sink capacity branch never fires
               its grow path -- one malloc per segment instead of the
               seed->grow realloc chain (sampling put ~19% of formDecode in
               tiny allocator calls from exactly this churn). */
            if (eq == (size_t)-1) {
                dyn_sb_reserve((void **)&k.p, &k.cap, i - b, i - b);
                dyn_pct_decode(&k, s + b, i - b, 1);
            } else {
                dyn_sb_reserve((void **)&k.p, &k.cap, eq - b, eq - b);
                dyn_sb_reserve((void **)&v.p, &v.cap, i - eq - 1, i - eq - 1);
...
/* WHATWG "domain to ASCII" is run beStrict on URL hosts, so an xn-- (A-)
   label must decode as punycode AND re-encode to the exact same body, and
   the decoded label may not be empty (VerifyDnsLength min 1). Node pins the
   observable behavior: http://xn--a-ecp/ and http://xn--/ throw, while
   http://xn--mnchen-3ya/ and http://xn--zzz/ keep their hosts. Without this
   check the ASCII fast path accepted hosts the engine's own domainToASCII
   refuses (a differential-parsing hole: user agents reject, we accept). */
static int dyn_host_alabel_roundtrip(const char *h, size_t n)
{
    size_t i, ls = 0;
    for (i = 0; i <= n; i++) {
        if (i == n || h[i] == '.') {
            size_t ll = i - ls;
            if (ll >= 4 && (h[ls] | 0x20) == 'x' && (h[ls + 1] | 0x20) == 'n' &&
                h[ls + 2] == '-' && h[ls + 3] == '-') {
                dyn_cp_t dec, enc;
                size_t blen = ll - 4, k;
                /* the bare A-label "xn--" is VALID here: the URL standard's
                   domain-to-ASCII runs VerifyDnsLength=false (WPT pins
                   https://xn--/ valid; node/ICU's rejection is CheckHyphens
                   overreach), so an empty punycode body is fine */
                if (blen == 0) {
                    ls = i + 1;         /* keep the scan in step with the
                                           label boundary: a bare xn-- label
                                           must not merge with the next one */
                    continue;
                }
                dyn_cp_init(&dec);
                dyn_cp_init(&enc);
                if (pc_decode(h + ls + 4, blen, &dec) < 0 || dec.n == 0 ||
                    pc_encode(dec.p, (int)dec.n, &enc) < 0 || enc.n != blen) {
                    dyn_cp_free(&dec);
                    dyn_cp_free(&enc);
                    return -1;
                }
                /* the decoded label must itself be a legal domain label:
                   U+2478-style punycode look-alikes decode from valid
                   punycode but are UTS #46 disallowed (node throws for
                   http://xn--a-ecp/ -> "a<circled-1>") */
                for (k = 0; k < dec.n; k++) {
                    if (dec.p[k] > 0x7F) {
                        int ri = dyn_tab3(idna_status_ranges,
                                          countof(idna_status_ranges),
                                          dec.p[k]);
                        if (ri >= 0 && idna_status_ranges[ri][2] == 3) {
                            dyn_cp_free(&dec);
                            dyn_cp_free(&enc);
                ...
                for (k = 0; k < blen; k++) {
                    if ((uint32_t)(uint8_t)h[ls + 4 + k] != enc.p[k]) {
                        dyn_cp_free(&dec);
                        dyn_cp_free(&enc);
                        return -1;
                    }
...
static int dyn_host_validate(const char *h, size_t n)
{
    ...
    return dyn_host_alabel_roundtrip(h, n);
}
...
static int dyn_url_init_module(JSContext *ctx, JSModuleDef *m)
{
    ...
    if (dyn_register_plain_class(ctx, m, &dyn_sp_class_id, &dyn_sp_class,
        ...
        return -1;
    /*the pair iterator class. Internal -- constructed only through
       keys()/values()/entries()/[Symbol.iterator], never exported. */
    JS_NewClassID(&dyn_spi_class_id);
    if (JS_NewClass(JS_GetRuntime(ctx), dyn_spi_class_id, &dyn_spi_class) < 0)
        return -1;
    proto = JS_NewObject(ctx);
    if (JS_IsException(proto))
        return -1;
    JS_SetPropertyFunctionList(ctx, proto, dyn_spi_proto,
                               countof(dyn_spi_proto));
    /* %IteratorPrototype% chaining + Symbol.toStringTag (WHATWG iterator
       shape): the walk Array.prototype.values() -> its prototype -> ITS
       prototype reaches %IteratorPrototype% through builtins only (no user
       code), which makes the pair iterators real Iterators for
       Iterator.from. A failed link is non-fatal: the proto stays a plain
       object chain and only the cosmetic instanceof/Iterator.from surface
       is lost. */
    {
        JSValue arr = JS_NewArray(ctx);
        JSValue arrproto = JS_GetPrototype(ctx, arr);
        JSValue values = JS_GetPropertyStr(ctx, arrproto, "values");
        JSValue arriter = JS_Call(ctx, values, arr, 0, NULL);
        JSValue aip = JS_GetPrototype(ctx, arriter);
        JSValue iterproto = JS_GetPrototype(ctx, aip);
        if (JS_IsObject(iterproto))
            JS_SetPrototype(ctx, proto, iterproto);
        JS_FreeValue(ctx, iterproto);
        JS_FreeValue(ctx, aip);
        JS_FreeValue(ctx, arriter);
    ...
    }
    {
        ...
        JSValue tagatom = JS_GetPropertyStr(ctx, sym, "toStringTag");
        JSValue tag = JS_NewString(ctx, "URLSearchParams Iterator");
        JSAtom katom = JS_ValueToAtom(ctx, tagatom);
        /* {writable: false, enumerable: false, configurable: true} */
        JS_DefinePropertyValue(ctx, proto, katom, tag, JS_PROP_CONFIGURABLE);
        JS_FreeAtom(ctx, katom);
        JS_FreeValue(ctx, tagatom);
        JS_FreeValue(ctx, sym);
        JS_FreeValue(ctx, g);
    }
    /* JS_SetClassProto TAKES the reference (same transfer as in
       dyn_register_plain_class) -- do not free it here. */
    JS_SetClassProto(ctx, dyn_spi_class_id, proto);
    /*the parse statics live on the constructor. Reach it through the
       registered prototype's constructor back-link (the same route
       dyn_register_plain_class's re-registration fallback uses). */
    proto = JS_GetClassProto(ctx, dyn_url_class_id);
    if (JS_IsException(proto))
        return -1;
    ...
    if (JS_SetPropertyStr(ctx, ctor, "parse",
                          JS_NewCFunction(ctx, js_url_parse_static,
                                          "parse", 2)) < 0 ||
        JS_SetPropertyStr(ctx, ctor, "canParse",
                          JS_NewCFunction(ctx, js_url_canparse_static,
                                          "canParse", 2)) < 0 ||
        JS_SetPropertyStr(ctx, ctor, "join",
                          JS_NewCFunction(ctx, js_url_join_static,
                                          "join", 2)) < 0) {
        JS_FreeValue(ctx, ctor);
        return -1;
    }
