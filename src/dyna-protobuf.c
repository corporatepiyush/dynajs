...
#include "dyna-simd-kernels.h"  /* simd.validate_utf8: proto3 string fields */
...
/* Thin wrapper over core/dyn-sb.h: the reserve-then-raw split (reserve takes
   EXTRA, not an absolute need) and the seed (128) are this codec's. */
...
static void pb_schema_free(JSContext *ctx, pb_schema_t *s)
{
    ...
    for (k = 0; k < s->n; k++) {
        ...
        if (s->f[k].sub) {
            /* the child NODE itself is heap-allocated (calloc in
             * pb_field_parse) and used to leak 16 bytes per nested message
             * schema on every encode/decode call (ASan, audit) */
                          ...
                          pb_field_t *f, int depth)
{
    ...
    if (JS_IsException(pv))
        goto out;
    {
        /* through a local: pb_type_of writes an int, and &f->type would alias
         * the four adjacent uint8_t fields (type/flags/ktype/vtype) -- benign
         * only by coincidence today (found by audit) */
        int ttype;
    ...
    if (JS_IsException(pv))
        goto out;
    /* JS_ToBool yields -1 on error (a Symbol property would throw): -1 is
     * truthy in C, so the error case has to be separated or the field is
     * silently marked repeated with an exception left pending. */
        ...
        goto out;
    {
        ...
        JS_FreeValue(ctx, pv);      /* freed BEFORE the overwrite: this value
                                       used to leak on every map field (audit) */
        if (ismap < 0)
            goto out;
        if (!ismap)
            goto no_map;
                          ...
                          JSValueConst v)
{
    ...
    case PB_T_DOUBLE: {
        double d;
        /* STRICT, like the integer types: JS_ToFloat64 would silently coerce
         * "1.5" or {valueOf} here (found by audit). */
        if (!JS_IsNumber(v)) {
            JS_ThrowTypeError(ctx, "encode: a %s field requires a number", what);
            return -1;
        }
...
static int pb_enc_map_key(pb_enc_t *e, int ktype, const char *s, size_t n)
{
    ...
    return 0;
badkey:
    /* s/n are the key's own bytes (borrowed from the caller, freed at its
       out: label) and not NUL-terminated at n, hence the %.*s. */
    JS_ThrowRangeError(e->ctx, "encode: map key \"%.*s\" is out of range for "
                               "its key type", (int)n, s);
    return -1;
}
...
static JSValue pb_decode_value(pb_rd_t *r, int type, const pb_schema_t *sub)
{
    ...
    case PB_T_STRING: {
        ...
        if (pb_rd_len(r, &len) < 0)
            return JS_EXCEPTION;
        /* proto3 strings ARE UTF-8 on the wire: JS_NewStringLen would paper
         * over bad bytes with replacement characters, so a forged field
         * would decode to text that was never sent. Refuse instead. */
...
static JSValue pb_decode_message(pb_rd_t *r, const pb_schema_t *s)
{
    ...
    while (r->i < r->n) {
        ...
        /* Check the full 64-bit tag before truncating: a high tag like
           ((1ULL<<32 | 1)<<3) would otherwise alias to field 1 after the
           (uint32_t) cast, letting a crafted message inject a low field. */
        if (tag > (((uint64_t)PB_MAX_FIELD << 3) | 7)) {
            JS_ThrowSyntaxError(r->ctx, "decode: field number %llu exceeds the "
                                        "2^29-1 limit",
                                (unsigned long long)(tag >> 3));
