 ...
 * A single leading 'v' is allowed (API contract for parse/isValid, and the
 * same leniency the comparator parser and clean() already apply): without
                            ...
                            const char ***pcstrs, SemVer **psvs, uint32_t *pcount)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        if (semver_parse(cstrs[i], slen, &svs[i])) {
            ...
            JS_ThrowTypeError(ctx, "dyna:semver: invalid version in array at "
                                   "index %u", i);
            goto fail;
        }
    }
...
static int sv_sort_set(JSContext *ctx, JSValueConst obj, uint32_t k,
                       JSValue val);
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    /* The API contract is "returns the same array, sorted ascending in
     * place" (like Array.prototype.sort), so the sorted elements are written
     * back into the caller's array and THAT array is returned -- not a fresh
     * one. `elems` holds owned dups of the original elements, so rewriting
     * the array does not alias anything we free below. */
    out = JS_DupValue(ctx, argv[0]);
    if (count > 0) {
        ...
        for (i = 0; i < count; i++) {
            if (sv_sort_set(ctx, out, i, JS_DupValue(ctx, elems[idx[i]]))) {
                js_free(ctx, idx); js_free(ctx, tmp);
                JS_FreeValue(ctx, out);
                sv_free_versions(ctx, elems, cstrs, svs, count);
                return JS_EXCEPTION;
...
/* Array.prototype.sort's Set(O, k, v, throw=true) semantics for the
 * in-place write-back: a non-writable element (frozen) or an accessor
 * without a setter throws a TypeError; anything else is written.
 * JS_DefineProperty cannot express this -- JS_DefinePropertyValue always
 * redefines the attributes too, and the engine refuses ANY attribute
 * application on a non-configurable property ("property is not
 * configurable"), which wrongly rejected sealed arrays and wrongly accepted
 * writable:false-but-configurable ones. So the writability check goes
 * through Reflect.getOwnPropertyDescriptor and the write through
 * JS_SetPropertyUint32 (which honors the existing property: data write or
 * setter call, exactly Set semantics). Returns 0 on success, -1 with an
 * exception pending on failure. Takes ownership of `val`. */
static int sv_sort_set(JSContext *ctx, JSValueConst obj, uint32_t k,
                       JSValue val)
{
    JSValue global, reflect, fn, desc = JS_UNDEFINED;
    JSValueConst args[2];
    int r, throw_ro = 0;

    global = JS_GetGlobalObject(ctx);
    reflect = JS_GetPropertyStr(ctx, global, "Reflect");
    fn = JS_GetPropertyStr(ctx, reflect, "getOwnPropertyDescriptor");
    if (JS_IsException(fn) || !JS_IsFunction(ctx, fn)) {
        JS_FreeValue(ctx, global);
        JS_FreeValue(ctx, reflect);
        /* no Reflect available: fall back to a plain Set */
        r = JS_SetPropertyUint32(ctx, obj, k, val);
        JS_FreeValue(ctx, fn);
        return (r < 0) ? -1 : 0;
    }
    JS_FreeValue(ctx, global);
    JS_FreeValue(ctx, reflect);
    args[0] = obj;
    args[1] = JS_NewUint32(ctx, k);
    desc = JS_Call(ctx, fn, JS_UNDEFINED, 2, args);
    JS_FreeValue(ctx, fn);
    ...
    if (JS_IsObject(desc)) {
        JSValue w = JS_GetPropertyStr(ctx, desc, "writable");
        JSValue g = JS_GetPropertyStr(ctx, desc, "get");
        JSValue s = JS_GetPropertyStr(ctx, desc, "set");
        int has_get = !JS_IsUndefined(g) && !JS_IsNull(g);
        int has_set = !JS_IsUndefined(s) && !JS_IsNull(s);
        /* The writability path is for DATA descriptors only: an accessor
           descriptor has no `writable` key, so treating absent-writable as
           read-only made sort throw on getter+setter elements that native
           sort writes through (the setter is called). Getter-only stays a
           TypeError; setter-only cannot reach this point because the read
           phase already rejects non-string elements. */
        if (!has_get && !has_set) {
            if (JS_ToBool(ctx, w) == 0)
                throw_ro = 1;                   /* data property, writable:false */
        } else if (!has_set) {
            throw_ro = 1;                       /* accessor without a setter */
        }
        JS_FreeValue(ctx, w);
        JS_FreeValue(ctx, g);
        JS_FreeValue(ctx, s);
    ...
    if (throw_ro) {
        JS_FreeValue(ctx, val);
        JS_ThrowTypeError(ctx,
            "dyna:semver: sort cannot write back a non-writable element");
        return -1;
    }
    r = JS_SetPropertyUint32(ctx, obj, k, val);
    return (r < 0) ? -1 : 0;
                                  ...
                                  int argc, JSValueConst *argv, int want_max)
{
    ...
    if (parse_range(rs, rn, rg)) {
        js_free(ctx, rg);
        sv_free_versions(ctx, elems, cstrs, svs, count);
        /* the offending text (bounded, like parse's 80-char bound): the error
           is built while `rs` is still alive, freed after it */
                                    ...
                                    int argc, JSValueConst *argv)
{
    ...
    if (parse_range(rs, rn, rg)) {
        ...
        JSValue e = JS_ThrowTypeError(ctx, "dyna:semver: invalid range \"%.*s\"",
                                      (int)(rn > 64 ? 64 : rn), rs);
        JS_FreeCString(ctx, vs);
        JS_FreeCString(ctx, rs);   /* `rs` is IN the error text: free after it */
        return e;
    }
    res = range_test(&v, rg);
    js_free(ctx, rg);
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    if (parse_range(s, n, &r->rg)) {
        JSValue e;
        free(r);
        /* the offending text (bounded): built while `s` is alive */
        e = JS_ThrowTypeError(ctx, "dyna:semver: invalid range \"%.*s\"",
                              (int)(n > 64 ? 64 : n), s);
        JS_FreeCString(ctx, s);   /* `s` is IN the error text: free after it */
        return e;
    }
    ...
    return dyn_plain_wrap(ctx, dyn_range_class_id, r, dyn_range_dispose);
}
