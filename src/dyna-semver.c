/*
 * dyna:semver -- semver 2.0.0 parsing, comparison, and range matching.
 *
 * `Range` is a compiled capability: parse once, match many. Measured crossover
 * is N=10 uses of one instance versus N free calls (5.9x at N=1000).
 * Full API: see the dyna:* module in dyna-libc.h.
 */
#include "dyna-nat.h"
#include "dtoa.h"                 /* u64toa: the engine's pair-digit number formatter */

#if defined(CONFIG_NATIVE_MODULES) && defined(CONFIG_NATIVE_MODULE_SEMVER)

#include <stddef.h>
#include <stdint.h>
#include <stdlib.h>   /* malloc/free: the Range capability owns its compiled state */
#include <string.h>
...
/* First bytes that can begin an ECMA-262 whitespace character (UTF-8 lead
 * bytes plus the single-byte members). A lookup keeps the per-version trim
 * cost at one load+branch on the hot Range.test path. */
static const unsigned char SV_WS1[256] = {
    [0x09] = 1, [0x0A] = 1, [0x0B] = 1, [0x0C] = 1, [0x0D] = 1, [0x20] = 1,
    [0xC2] = 1, [0xE1] = 1, [0xE2] = 1, [0xE3] = 1, [0xEF] = 1,
};

/* One ECMA-262 WhiteSpace or LineTerminator code point at the start of s
 * (UTF-8): U+0009-000D, U+0020, U+00A0, U+1680, U+2000-200A, U+2028, U+2029,
 * U+202F, U+205F, U+3000, U+FEFF. Returns its byte length, 0 if none. This is
 * exactly the set String.prototype.trim strips, which is what node-semver's
 * parse applies before its grammar ("version.trim().match(FULL)"). */
static size_t sv_ws_char(const char *s, size_t n)
{
    ...
    if (c == 0xE1 && n >= 3 && (unsigned char)s[1] == 0x9A &&
    ...
    if (c == 0xE2 && n >= 3 && (unsigned char)s[1] == 0x81 &&
        (unsigned char)s[2] == 0x9F)
        return 3;                                  /* U+205F */
    if (c == 0xE3 && n >= 3 && (unsigned char)s[1] == 0x80 &&
        (unsigned char)s[2] == 0x80)
        return 3;                                  /* U+3000 */
    if (c == 0xEF && n >= 3 && (unsigned char)s[1] == 0xBB &&
...
static size_t sv_ws_trail(const char *s, size_t n)
{
    ...
    for (;;) {
        ...
        if (n == 0)
            break;
        /* find the byte length of the whitespace char ending at s[n-1]; the
         * n>=3 guards inside sv_ws_char make partial matches impossible, so
         * at most one k validates */
        for (k = 1; k <= 3 && k <= n; k++) {
...
{
    return (int)u64toa(buf, v);           /* engine formatter, byte-identical */
}
 ...
 * (is_build==0) an all-digit identifier must have no leading zero and is a
 * NUMBER, capped at MAX_SAFE like major/minor/patch; build identifiers are
 * opaque per SemVer 2.0.0 sec.10 ("build metadata MUST be ignored"), so no
 * leading-zero rule and no numeric cap applies to them. 0 / -1. */
    ...
    for (i = 0; i <= n; i++) {
        if (i == n || s[i] == '.') {
            ...
            if (!is_build && alldig && len >= 16) {
                /* A numeric identifier is a NUMBER, and npm's cap on any
                 * numeric field/identifier is MAX_SAFE -- the same cap
                 * sv_parse_num enforces on major/minor/patch. Shorter runs
                 * are below the cap by construction (< 10^15). The compare
                 * inside the loop saturates: v <= 10*MAX_SAFE + 9 before the
                 * exit, so no u64 overflow. */
                uint64_t v = 0;
                for (k = 0; k < len; k++) {
                    v = v * 10 + (uint64_t)(id[k] - '0');
                    if (v > SV_MAX_SAFE)
                        return -1;               /* numeric id > MAX_SAFE */
                }
            }
            ids[count].s = id;
            ids[count].len = len;
 ...
 * Leading/trailing ECMA-262 whitespace is skipped (node-semver parity: its
 * parse is "version.trim().match(FULL)", so satisfies(" 1.2.3 ",...) must
 * hold). A single leading 'v' is allowed (API contract for parse/isValid, and
 * the same leniency the comparator parser and clean() already apply): without
 * it, satisfies("1.2.3","v1.2.3") worked while satisfies("v1.2.3","1.2.3")
 * threw -- one grammar for the range side and a different one for the
 * version side. The trimmed view is what the returned identifier spans point
 * into, which is fine: it stays within the caller's buffer. */
 ...
 * caret/tilde/hyphen/x-range shorthand have all been resolved away). */
typedef struct {
...
static int range_test(const SemVer *v, const Range *rg)
{
    int i;
    if (v->n_pre < 0)
        return 0;                     /* invalid element (tolerant read) */
    for (i = 0; i < rg->n; i++)
...
/* A numeric prerelease identifier as a JS Number. node-semver keeps the
 * identifier a STRING when its value reaches MAX_SAFE_INTEGER (classes/
 * semver.js: "num >= 0 && num < MAX_SAFE_INTEGER" -- strict), so 1.0.0-
 * 9007199254740991 has prerelease ["9007199254740991"], not [2^53-1]. The
 * parse-side cap means values are never ABOVE MAX_SAFE, so "<" is the only
 * test needed. */
static JSValue sv_id_number(JSContext *ctx, const SemVerId *id)
{
    ...
    {   /* unreachable today (parse caps numeric ids at 16 digits), kept as a
         * value-exact fallback for future cap changes */
        double d = 0;
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    if (semver_parse(s, n, &v)) {
        JSValue e = JS_ThrowTypeError(ctx, "dyna:semver: invalid version \"%.*s\"",
                                      (int)(n > 80 ? 80 : n), s);
        JS_FreeCString(ctx, s);   /* `s` is IN the error text: free after it */
        return e;
    }
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    s = sv_arg_str(ctx, argc, argv, &n);
    if (!s) return JS_EXCEPTION;
    /* node parity: clean is "version.trim().replace(/^[=v]+/,'')" then parse;
     * trim is the full ECMA-262 set, so the strip loop must run on the
     * trimmed view (whitespace after the '='/'v' run is left for
     * semver_parse's own trim, exactly like node). */
    {
        size_t lead = sv_ws_lead(s, n), trail;
        p = s + lead;
        n -= lead;
                                ...
                                int argc, JSValueConst *argv, int magic)
{
    ...
    default:
        /* prerelease(v) -> array | null: "or null when none" per the API
         * contract (matching node-semver). parse(v).prerelease stays [] --
         * there it is the object-field form. */
...
/* ==================================================================== *
 *  JS boundary: diff / compareBuild *
 * ==================================================================== */

/* Parse two version args and keep both C strings ALIVE in the pas/pbs outs
 * (the SemVers' prerelease/build spans borrow them). Throws unless both args
 * are strings that parse strictly. Returns 0 / -1. */
...
/* diff(a, b): the highest-precedence component that differs:
 * "major" | "minor" | "patch" | "prerelease" | "build", or null when the two
 * versions are identical. This is node-semver's DOCUMENTED contract; note
 * modern node-semver's implementation adds 'pre' to the tuple names when the
 * higher version carries a prerelease -- we always return the plain name.
 * Prerelease comparison is by precedence, build comparison is by string. */
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    if (a.major != b.major)
        ...
        /* tuple equal, prerelease differs. node-semver 7.8.5 pins the
         * release-vs-prerelease case ("1.2.3" vs "1.2.3-alpha") to "patch"
         * -- moving to a release is a patch-level change -- and keeps
         * "prerelease" only when BOTH sides carry one. */
        which = (a.n_pre == 0 || b.n_pre == 0) ? "patch" : "prerelease";
    }
    else {
        int i;
        int build_eq = a.n_build == b.n_build;
        for (i = 0; build_eq && i < a.n_build; i++)
            build_eq = a.build[i].len == b.build[i].len &&
                       memcmp(a.build[i].s, b.build[i].s, a.build[i].len) == 0;
        if (!build_eq)
            which = "build";
    }
    JS_FreeCString(ctx, as);
...
/* compareBuild(a, b): node-semver semantics -- "the same as compare
 * but considers build when two versions are equal": precedence first, and a
 * precedence tie is broken by the build identifiers (absent build sorts
 * BELOW a present one; identifiers compare numerically when both are
 * numeric, numeric < alphanumeric, else lexically). -1 | 0 | 1. */
...
/* ==================================================================== *
 *  JS boundary: intersects *
 * ==================================================================== */

/* Does the exact version `eq` (node operator-less comparator) satisfy c?
 * Node answers with Range(comp.value).test(version); the extra rule beyond
 * the comparison is the prerelease gate: a prerelease version satisfies a
 * comparator only when the comparator's own version carries a prerelease ON
 * THE SAME major.minor.patch (node's testSet allow-list). */
static int sv_eq_intersects(const Comparator *eq, const Comparator *c)
{
    if (eq->v.n_pre > 0) {
        if (c->v.n_pre == 0)
            return 0;
        if (c->v.major != eq->v.major || c->v.minor != eq->v.minor ||
            c->v.patch != eq->v.patch)
            return 0;
    }
    return cmp_test(&eq->v, c);
}

/* "<0.0.0" and the synthetic "<0.0.0-0": below the lowest release and
 * excluding prereleases -- such a comparator matches nothing. */
static int sv_comp_matches_nothing(const Comparator *c)
{
    if (c->op != OP_LT || c->v.major != 0 || c->v.minor != 0 || c->v.patch != 0)
        return 0;
    return c->v.n_pre == 0 ||
           (c->v.n_pre == 1 && c->v.pre[0].numeric && c->v.pre[0].len == 1 &&
            c->v.pre[0].s[0] == '0');
}

/* Node-semver Comparator.intersects, over our resolved comparators: the
 * conservative pairwise rule (a false negative is possible only for
 * unsatisfiable ranges, which is node's behavior too). */
/* node-semver normalises ">=0.0.0" (and "*"/empty) to its ANY comparator;
 * ours stays a real GTE(0.0.0), so mirror the normalization here: ANY
 * intersects anything, checked BEFORE the matches-nothing rule (node's ANY
 * branch returns first, which is why (">=0.0.0","<0.0.0") is true there). */
static int sv_comp_is_any(const Comparator *c)
{
    return c->op == OP_GTE && c->v.major == 0 && c->v.minor == 0 &&
           c->v.patch == 0 && c->v.n_pre == 0;
}
...
static int sv_comp_intersects(const Comparator *a, const Comparator *b)
{
    ...
    /* exact-version comparators answer by testing the version */
    ...
    if (a_up && b_up) return 1;              /* same direction increasing */
    if (a_dn && b_dn) return 1;              /* same direction decreasing */

    /* same version, both bounds inclusive (>=, <=, =) */
    a_inc = a->op == OP_GTE || a->op == OP_LTE || a->op == OP_EQ;
    b_inc = b->op == OP_GTE || b->op == OP_LTE || b->op == OP_EQ;
    sc = semver_compare(&a->v, &b->v);
    if (sc == 0 && a_inc && b_inc)
        return 1;
    if (a_up && b_dn && sc < 0) return 1;
    if (a_dn && b_up && sc > 0) return 1;
    return 0;
}

/* One comparator set is satisfiable iff every pair of its comparators
 * intersects (node's isSatisfiable). */
...
/* Read one range argument: a range STRING (parsed into *out; the comparators
 * borrow the returned C string, which the caller must JS_FreeCString) or an
 * existing Range instance (its Range is shallow-copied into *out and borrows
 * the instance's source; the caller must NOT free). *powned tells which.
 * Throws TypeError otherwise. 0 / -1.  (Both defined below, after the Range
 * class machinery they need.) */
static int sv_range_arg(JSContext *ctx, JSValueConst v, Range *out,
                        const char **powned, int *powned_flag);
 ...
 * and `svs` (parsed). allow_invalid: 0 = an unparseable element throws a
 * TypeError naming the index (sort's contract: sort() is a total order, a
 * junk element is a caller bug); 1 = the element is kept but marked
 * svs[i].n_pre = -1, which range_test refuses (node parity: maxSatisfying,
 * minSatisfying and filter treat junk entries as non-matching instead of
 * failing the whole call). On any error frees everything filled so far and
 * throws. On success the caller owns all three and must free via
 * sv_free_versions. */
                            ...
                            int allow_invalid)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        if (semver_parse(cstrs[i], slen, &svs[i])) {
            if (!allow_invalid) {
                /* the index: an 80-entry feed with one typo is otherwise
                   undiagnosable from the message alone */
                JS_ThrowTypeError(ctx, "dyna:semver: invalid version in array "
                                       "at index %u", i);
                goto fail;
            }
            svs[i].n_pre = -1;              /* invalid marker: range_test 0 */
        }
    }
...
/* sort(arr) -> NEW array sorted by ascending precedence (d.ts: "does not
 * mutate the input"). Deliberately NOT node-semver's in-place semver.sort:
 * the documented contract here is a pure function. Element references are
 * MOVED out of `elems` into the output (each exactly once), so the cleanup
 * below frees only JS_UNDEFINED slots for moved entries. */
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    if (count > 0) {
        ...
        for (i = 0; i < count; i++) {
            /* steal the element reference: it is consumed exactly once */
                                  ...
                                  int argc, JSValueConst *argv, int want_max)
{
    ...
    if (parse_range(rs, rn, rg)) {
        ...
        sv_free_versions(ctx, elems, cstrs, svs, count);
        JS_FreeCString(ctx, rs);
        /* node parity (ranges/max-satisfying.js): an invalid range yields
         * null, not a throw -- "no version satisfies" is the right answer to
         * an unsatisfiable range for a best-pick. */
        return JS_NULL;
    }
                                    ...
                                    int argc, JSValueConst *argv)
{
    ...
    if (parse_range(rs, rn, rg)) {
        ...
        JS_FreeCString(ctx, vs);
        JS_FreeCString(ctx, rs);
        /* node parity (functions/satisfies.js): an invalid RANGE is answered
         * with false, not a throw -- only an invalid VERSION throws here
         * (matching node, where satisfies propagates the SemVer ctor error
         * but catches the Range ctor error). */
        return JS_NewBool(ctx, 0);
    }
    res = range_test(&v, rg);
    js_free(ctx, rg);
...
static void inc_apply(IncVer *v, int rel, const char *id, size_t idn)
{
    ...
    case REL_PRERELEASE:
        ...
        break;
    case REL_RELEASE:
        /* Drop the prerelease (and build metadata, which inc never emits).
         * The no-prerelease case is answered in dyn_semver_inc, matching
         * node-semver's `if (this.prerelease.length === 0) return null`. */
        v->n_pre = 0;
        break;
    }
}
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    /* node-semver pins: inc(v, 'release') with NO prerelease on the input
     * returns null (semver@7.8.5: inc('1.0.0','release') === null), while an
     * input WITH a prerelease returns the stripped X.Y.Z. Capture the input
     * state before inc_apply clears it. */
    had_pre = iv.n_pre > 0;
    ...
    /* inc must never emit a version its own parser rejects: sv_parse_num caps
     * major/minor/patch at MAX_SAFE and sv_parse_ids caps numeric identifiers
     * the same way, so a bump that would cross the cap is refused here rather
     * than returned as an unparseable string. */
    if (iv.major > SV_MAX_SAFE || iv.minor > SV_MAX_SAFE ||
        iv.patch > SV_MAX_SAFE) {
...
/* ==================================================================== *
 *  class Range -- a COMPILED range: parse once, test many versions      *
 * ==================================================================== *
 *
 * The free satisfies()/maxSatisfying()/minSatisfying() js_malloc a Range,
 * parse_range() it and free it again on EVERY call. A dependency solver asks
 * "does this version satisfy this range?" in an inner loop over thousands of
 * versions, so that parse is pure repeated work.
 *
 * new Range(">=1.2.3 <2") parses once; .test(v) is then a comparison against
 * the compiled comparator sets. Same relationship as new RegExp to a literal
 * match. A plain GC object: the compiled state is a fixed-size POD struct plus
 * the source string, so there is nothing scarce to close.
 *
 * The compiled state is READ-ONLY, so a Range is freely reusable and needs no
 * busy flag -- unlike Hasher, no method mutates it.
 */

typedef struct {
    Range rg;
    char *source;   /* the range as given, for .source */
} dyn_range_t;
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    /* coerce first: nothing is allocated yet, so a throwing coercion strands
     * nothing. */
    s = JS_ToCStringLen(ctx, &n, argv[0]);
    if (!s)
        return JS_EXCEPTION;
    ...
    /* OWN the range text FIRST and parse from the owned copy. parse_range
     * leaves every prerelease identifier span pointing INTO the string it
     * was given, and a compiled Range outlives this call, so the spans must
     * live in r->source (freed only by dyn_range_dispose, together with
     * r->rg). Parsing directly from `s` left the comparators' prerelease
     * pointers dangling after JS_FreeCString: any later .test() of a
     * prerelease version compared against freed memory. */
    ...
    if (parse_range(r->source, n, &r->rg)) {
        /* the offending text (bounded): built while r->source is alive */
...
/* .test(version) -> boolean. An unparseable version is `false`, not a throw:
 * "does this satisfy" has a defensible answer for junk, and a solver filtering
 * a list should not have to pre-validate every entry. */
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    (void)argc;
    /* coerce the argument to C locals BEFORE resolving the native handle */
    vs = JS_ToCStringLen(ctx, &vn, argv[0]);
    if (!vs)
        return JS_EXCEPTION;
...
/* magic: 1 = maxSatisfying, 0 = minSatisfying. Returns the ORIGINAL array
 * element (not a re-serialised version), matching the free functions. */
                                 ...
                                 int argc, JSValueConst *argv, int want_max)
{
    ...
    (void)argc;
    /* the array read runs user JS (length/index getters), so it happens before
...
/* .filter(versions) -> every satisfying element, input order preserved. */
...
/* Read one range argument (intersects): a range STRING (parsed into
 * *out; the comparators borrow the returned C string, which the caller must
 * JS_FreeCString) or an existing Range instance (shallow copy into *out,
 * borrowing the instance's source; the caller must NOT free). *powned_flag
 * says which. Throws TypeError otherwise. 0 / -1. */
static int sv_range_arg(JSContext *ctx, JSValueConst v, Range *out,
                        const char **powned, int *powned_flag)
{
    const char *s;
...
/* intersects(rangeA, rangeB) -> boolean (node-semver semantics: true when
 * the comparators of some pair of ||-sets pairwise intersect). */
static JSValue dyn_semver_intersects(JSContext *ctx, JSValueConst this_val,
                                     int argc, JSValueConst *argv)
{
    Range ra, rb;
    ...
    if (sv_range_arg(ctx, argc > 1 ? argv[1] : JS_UNDEFINED,
                     &rb, &own_b, &flag_b)) {
        if (flag_a) JS_FreeCString(ctx, own_a);
        return JS_EXCEPTION;
    }
    res = sv_range_intersects(&ra, &rb);
    if (flag_a) JS_FreeCString(ctx, own_a);
    if (flag_b) JS_FreeCString(ctx, own_b);
    return JS_NewBool(ctx, res);
}

/* .intersects(other): other is a range string or a Range instance;
 * node-semver comparator-pairwise semantics (see the free intersects). */
...
/* Number of ||-separated comparator sets the range compiled to. Exposed
 * because it is the one observable measure of compilation having happened. */
