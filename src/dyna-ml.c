...
static int dyn_ml_refuse_csr(JSContext *ctx, JSValueConst v)
{
    if (JS_GetClassID(v) != dyn_csr_class_id)
        return 0;
    JS_ThrowTypeError(ctx,
        "dyna:ml this estimator has no sparse path, so it cannot take a CSR. "
                            ...
                            int argc, JSValueConst *argv)
{
    ...
    for (i = 0; i < nnz; i++) {
        if (!(colf[i] >= 0.0) || colf[i] >= (double)s->cols ||
            ...
            dyn_csr_dispose(s);
            return JS_ThrowRangeError(ctx,
                "ml.CSR: columns[%u] must be an integer in [0, %u) and its value finite",
                (unsigned)i, ncols);
        }
                           ...
                           int argc, JSValueConst *argv)
{
    ...
    if (iv < 0 || (uint64_t)iv >= (uint64_t)s->rows)
        return JS_ThrowRangeError(ctx, "ml.row: row %lld is outside [0, %u)",
 ...
 * arms bypass that ingest, and a CSR is allowed to CARRY non-finite values
 * (fromDense keeps a NaN by design, because its job is to preserve data, not
 * to validate it -- pinned by test_ml_sparse.js). Without this check a NaN
 * slips into A^T A / the gradient and the model still "fits" -- silently NaN
                            ...
                            const double *sw, const dyn_csr_t *S)
{
    ...
    if (dyn_solve(AtA, Aty, p)) {
        free(AtA); free(Aty); free(coef);
        JS_ThrowInternalError(ctx, "ml.fit: LinearRegression: singular system");
        return -1;
    }
    for (a = 0; a < cols; a++)
        coef[a] = Aty[a];
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (mx.cols != m->n_features) {
        dyn_matrix_free(&mx);
        return JS_ThrowTypeError(ctx,
            "ml.predict: X has %u features, model expects %u",
                                  ...
                                  JSClassID class_id)
{
    ...
    if (mx.cols != m->n_features) {
        dyn_matrix_free(&mx);
        return JS_ThrowTypeError(ctx, "ml.predict: X has %u features, model expects %u",
                                 (unsigned)mx.cols, (unsigned)m->n_features);
    }
    rows = mx.rows;
                                        ...
                                        JSClassID class_id)
{
    ...
    if (mx.cols != m->n_features) {
        dyn_matrix_free(&mx);
        return JS_ThrowTypeError(ctx, "ml.predictProba: X has %u features, model expects %u",
                                 (unsigned)mx.cols, (unsigned)m->n_features);
    }
    rows = mx.rows;
    k = m->n_classes;
                                ...
                                JSClassID class_id)
{
    ...
    if (mx.cols != m->n_features) {
        dyn_matrix_free(&mx);
        return JS_ThrowTypeError(ctx, "ml.apply: X has %u features, model expects %u",
                                 (unsigned)mx.cols, (unsigned)m->n_features);
    }
    rows = mx.rows;
