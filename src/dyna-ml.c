 ...
 * (Gaussian elimination with partial pivoting; a tiny ridge is tried only
 * when the elimination reports a numerically singular system -- an unusable
 * pivot, judged against the system's own rounding noise -- see
 * dyn_linreg_solve);
 ...
 *
 * Error-message convention (docs/ERRORS.md): every thrown message carries a
 * prefix identifying where it came from. A throw inside a function that serves
 * exactly one JS-visible entry point uses "ml.<op>:" (ctors use the class
 * name, getters the property name); a shared validator that many entry points
 * route through cannot honestly name one op, so it uses the bare module tag
 * "dyna:ml ". JS_ThrowOutOfMemory stays unprefixed (it has no module context).
 */
#include "dyna-nat.h"
 ...
 * AND the full valid set, instead of being silently ignored. Same shape as
 * the pilot dyn_opts_strict in dyna-file.c: a non-object bag counts as
 * absent; own ENUMERABLE STRING keys only (symbol and inherited keys are
 * invisible); a Proxy ownKeys trap that throws propagates; an array bag's
 * "0","1",... keys are unknown -> throws. */
...
/* The valid-key tables, one per options bag (see each call site). */
...
static const char *const ml_tree_keys[] = { "nEstimators", "maxDepth",
                                            ...
                                            "subsample" };
/* The XGB models also read these; the plain trees' own targeted refusals for
 * validationFraction/earlyStoppingRounds stay intact because listing them
 * here keeps them OUT of the generic unknown-key error. */
...
#define DYN_RIDGE           1e-9    /* diagonal load, SINGULAR fallback only,
                                       in ABSOLUTE units of the WEIGHT-
                                       NORMALIZED A^T A: the system is divided
                                       by the mean sample weight BEFORE the
                                       load is added, so the load is the
                                       constant DYN_RIDGE in those units and
                                       scales with the mean weight in raw ones
                                       (see dyn_linreg_solve) */

/* The un-ridged solve's conditioning gate: the smallest pivot ratio (pivot /
 * largest diagonal entry of A^T A) at which the un-ridged normal equations
 * are still trusted. The gate separates two regimes that the elimination
 * cannot tell apart on its own -- both end in a tiny last pivot:
 *
 *   - ELIMINATION RESIDUE. An exactly-singular design whose cancellation is
 *     not dyadic leaves a pivot at the elimination's own rounding level:
 *     measured at <= ~2.4e-17 RELATIVE across the whole singular battery
 *     (tests/test_ml_linreg_exact.js), well below eps itself. Such a system
 *     has no un-ridged answer -- dividing by the residue amplifies it into
 *     1/residue garbage -- and the ridge fallback is the right solver: the
 *     residue IS rank deficiency.
 *
 *   - A WELL-POSED but near-singular design. Its small pivot is a genuine
 *     (small) singular value, not a rounding artifact; the normal equations
 *     deliver its OLS solution to rounding, and ridge-biasing it would BE
 *     the error.
 *
 * The cut sits at 1000*eps (~2.2e-13) relative: four orders of magnitude
 * above the measured residue band, so every rounding-level residue is routed
 * to the ridge, while well-posed designs still solve exactly -- un-ridged,
 * to rounding -- down to pivots ~1e-13 of A^T A's scale. Below the cut the
 * ridge fallback takes over, and its identifiable predictions stay pinned
 * (see dyn_linreg_solve and the docs). */
 ...
 * sequential loop would use. So results can differ from a strictly sequential
 * computation in the last ULP or so: a fitted coefficient can move by an ULP,
 * and a KMeans point almost exactly equidistant from two centroids may land in
 * either. That is the ONLY axis on which results are equivalent-but-not-bit-
 * equal: the SIMD build versus the -DDYN_ML_NO_SIMD oracle build (and, across
 * targets, the same build in its own last ULP). On every other axis the forms
 * are bit-equal BY CONSTRUCTION and the tests pin them with Object.is: a dense
 * X and a CSR X of the same reading run ONE computation (dyn_ml_row_at feeds
 * the same kernel), and the LinearRegression sparse fit reads the same
 * normalized reading with the same rounding as the dense arm (see
 * dyn_linreg_solve). Multiply-add contraction cannot break that: dyn_ml_axpy/
 * dyn_ml_scale multiply through a temp (never fused to one FMA), and in the
 * one corner where a `t += a * b` shape does survive -- the unweighted scalar
 * A^T y updates -- bit-equality holds by IEEE exactness of multiplying by 1.0
 * (fma(1, y, t) is exactly t + y), not by expression-shape identity: a fusing
 * target fuses only the sparse side there. Values compared ACROSS builds
 * that only share an equivalence class are compared with a tolerance; values
 * that are one computation are compared bit-for-bit.
 *
...
/* The one Float64Array class id, captured at module init by asking the engine
 * about a fresh 1-element array. Width alone cannot type a view: a
 * BigInt64Array/BigUint64Array shares the 8-byte element size, and reinterpreting
 * int64 bytes as doubles is silent garbage on every into-API. */
static JSClassID dyn_ml_f64_class_id;

/* Resolve a Float64Array to its backing double* and element count. Returns 0, or
 * -1 (throwing) for anything that is not exactly a Float64Array (a BigInt
 * array shares the 8-byte width and is refused BY IDENTITY, not by width), a
 * detached buffer, or an out-of-bounds view. The pointer aliases the JS buffer
 * and is valid only while no JS runs. */
                                 ...
                                 int from, size_t rows, double **pw)
{
    ...
    *pw = NULL;
    if (JS_IsUndefined(opts))
        return 0;
    /*fit's trailing bag carries sampleWeight and nothing else. */
                                 ...
                                 int from, const char *what)
{
    ...
    if (JS_IsUndefined(opts))
        return 0;
    /*strict even where sampleWeight itself would be refused. */
    if (dyn_opts_strict(ctx, opts, ml_weights_keys, 1))
        return -1;
    wv = JS_GetPropertyStr(ctx, opts, "sampleWeight");
    if (JS_IsException(wv))
                               ...
                               int missing_ok)
{
    ...
    if (JS_IsArray(ctx, xv)) {
        ...
        /* The Array-of-rows copy is already owned, so the weights (and their
         * getters) can run afterwards without any dangling alias. */
        if (pw && dyn_ml_ingest_weights(ctx, argc, argv, opt_from,
                                        mx->rows, pw)) {
            dyn_matrix_free(mx);
            free(*py);
            *py = NULL;
        ...
        if (dyn_ml_ingest_vector(ctx, yv, (size_t)rows64, py))
            return -1;
        /* sampleWeight (and its getters) is read into an OWNED copy BEFORE the
         * zero-copy X alias. A getter that ArrayBuffer.transfer()s the caller's
         * X buffer therefore makes the alias attempt below fail cleanly with
         * "ArrayBuffer is detached" instead of leaving the trainer reading
         * detached memory. */
        if (pw && dyn_ml_ingest_weights(ctx, argc, argv, opt_from,
                                        (size_t)rows64, pw)) {
            free(*py);
        ...
        /* The X alias is the last JS-touching step: y and the weights are
         * already owned copies, so no JS can run while the alias is held. */
...
/* Ingest X and y, and (when `pw` is non-NULL) the trailing sampleWeight option,
 * into C buffers. Weights are read into an OWNED copy before any zero-copy X
 * alias is taken, so a getter that detaches the caller's X buffer cannot leave a
 * single-owner alias held across user JS -- it fails cleanly instead. `opt_from`
 * is the first argv index the options object may occupy. */
...
/* The y-only variant (no sampleWeight): used by the search drivers, which take
 * a plain X/y and define weights through the per-fold estimator options. Keeps
 * the historical argument shape but still routes through the same order. */
...
/* Ingest X and the trailing sampleWeight option (for an estimator whose fit
 * takes X but no y, e.g. KMeans/StandardScaler). The weights are read into an
 * OWNED copy BEFORE the zero-copy X alias, so a getter that detaches the
 * caller's X buffer fails cleanly ("ArrayBuffer is detached") instead of leaving
 * the alias held across user JS. `opt_from` is the first argv index the options
 * object may occupy; `pw` receives the weights (caller frees) or NULL. */
                             ...
                             dyn_matrix_t *mx, double **pw)
{
    ...
    if (JS_IsArray(ctx, xv)) {
        ...
        if (dyn_ml_ingest_weights(ctx, argc, argv, opt_from, mx->rows, pw)) {
            /* the caller treats -1 as "nothing owned": drop the matrix here */
            dyn_matrix_free(mx);
            return -1;
        }
        return 0;
...
/* ----: the into-family and {as: "f64"} ----------------------- */

/* The trailing predict option bag carries `as` and nothing else. */
static const char *const ml_as_keys[] = { "as" };

/* Parse {as: "f64"} from the trailing options object (dyn_ml_opts finds it
 * past `from`). Absent/undefined/null keeps the historic Array result; "f64"
 * asks for a Float64Array; anything else is a TypeError. 0 ok, -1 throw. */
...
/* Bind the predictInto/transformInto out buffer: a Float64Array with room for
 * `expect` elements. SHORT is a RangeError before anything is written; extra
 * capacity is left untouched. The pointer aliases the caller's buffer and is
 * only held across C loops (never across a JS call). 0 ok, -1 throw. */
                           ...
                           double **pp)
{
    ...
    if (n < expect) {
        JS_ThrowRangeError(ctx,
            ...
            "nothing was written", (unsigned)n, (unsigned)expect);
        return -1;
    }
    *pp = p;
    return 0;
...
/* The {as: "f64"} result emitter: same doubles, Float64Array instead of a
 * fresh JS Array when asked. */
...
/* The public predict/predictInto arg split. `into` mode shifts every X
 * argument right by one (argv[0] is the out buffer) and reads no bag. */
 ...
 * Returns 0, or -1 if a pivot is unusable (not positive, not finite, or below
 * the caller's `min_pivot`). The caller passes a scale-relative minimum so the
 * same function detects both an exactly zero pivot and one that elimination
 * rounding itself could have produced -- a residue below the system's own
 * rounding noise is indistinguishable from zero however the design is scaled
 * (see the conditioning gate in dyn_linreg_solve). */
static int dyn_solve(double *A, double *b, size_t p, double min_pivot)
{
    ...
    for (col = 0; col < p; col++) {
        ...
        /* `!(maxv > 0)` rather than `maxv <= 0`: a NaN pivot must refuse,
           not divide by itself. An +Inf pivot refuses the same way -- it is
           not usable either, and letting it through divided the system into
           NaN coefficients. */
 ...
 * accumulates its normal equations over nonzero PAIRS of each row's READING
 * (see dyn_csr_row_reading), which is O(sum m_i^2) with m_i the row's distinct
 * columns, against O(rows * cols^2); LogisticRegression's dot products and
 * gradient updates run over nonzeros, which is O(nnz) per iteration against
 * O(rows * cols). Every other fit REFUSES a CSR and names `.toDense()`.
 * Every predict and predictProba takes one: a predict kernel walks one
 * dense row at a time, so a sparse row is materialised into an O(cols) scratch
 * and fed to the SAME kernel -- never the whole matrix (see dyn_ml_row_src).
 *
 * Refusing a fit is the honest answer rather than densifying behind the
 * caller's back: expanding it silently is the 40 GB the sparse form existed to
 * avoid, and the caller should be able to point at the line where that
 * happened. It is the same rule W9.4 applied to imputation. */
...
/* ---------- the READING of a row: one matrix for every consumer ----------
 *
 * A CSR row can store its entries in any column order and repeat a column
 * (both constructor-legal), but the MATRIX it denotes is always the same one
 * row()/toDense()/dyn_ml_row_at read: implicit zeros everywhere, and each cell
 * the STORAGE-ORDER SUM of its column's entries. dyn_csr_row_reading computes
 * exactly that reading -- distinct (column, value) cells, ascending by column,
 * duplicates summed in the same left-to-right order `x[col] += val` accumulates
 * them -- so every consumer sees ONE matrix and one value per cell, bit for
 * bit, however the row happened to be stored. The LinearRegression sparse fit
 * consumes rows through this expansion (see dyn_linreg_solve) and the finite
 * checks run on it (a duplicate sum can overflow while every stored value is
 * finite -- [1e308, 1e308] at one column READS Infinity), so which form a
 * matrix arrived in can change neither the model fitted nor what is refused.
 *
 * A run whose sum is exactly zero is dropped: it reads as an implicit zero,
 * and a zero contributes nothing to any product. Returns the cell count, or
 * -1 (without throwing) when some summed cell is non-finite -- stored
 * non-finite or duplicate-sum overflow alike -- with *bad_nan classifying it;
 * the caller throws its own message. `ent` must hold one cell per entry in
 * the largest row (dyn_csr_max_row). */
typedef struct {
    uint32_t col;
    double val;
    size_t pos;             /* storage position: the sum-order tiebreak */
} dyn_csr_ent_t;
                                ...
                                int *bad_nan)
{
    ...
    /* Only a row with a descending pair needs sorting at all; fromDense emits
     * strictly ascending columns and canonical duplicate stacks stay grouped,
     * so the common case is the linear pass above. The sort is by (column,
     * storage position) -- qsort is not stable, and the sum below must run in
     * storage order to be bitwise the direct expansion. */
    if (descending)
    ...
    while (r < m) {
        ...
        double s = 0.0;             /* storage-order sum: the cell's reading */
                            ...
                            int argc, JSValueConst *argv)
{
    ...
    for (i = 0; i < nnz; i++) {
        if (!(colf[i] >= 0.0) || colf[i] >= (double)s->cols ||
             ...
             * was noticed. Printed 64-bit: cols is caller-declared and can
             * exceed what `(unsigned)` can hold. */
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (dyn_ml_ingest_X(ctx, argv[0], rows_arg, cols_arg, &mx))
        return JS_EXCEPTION;
    /* The ctor refuses a non-finite stored value, and so does this -- a CSR is
     * an immutable value handle and BOTH ways of building one draw the line at
     * the same place. The asymmetry this replaces (fromDense kept a NaN "by
     * design") made the finite-check surface reachable through only one of the
     * two constructors; a matrix that cannot carry a non-finite value in any
     * form says the same thing at both declarations. Checked before anything
     * is allocated, and naming the exact cell like every other input refusal. */
    for (i = 0; i < mx.rows; i++) {
        for (j = 0; j < mx.cols; j++) {
...
/* Can a dense expansion of `rows x cols` doubles be materialised at all?
 *
 * The dense form of a CSR is handed back as JS Arrays -- row() is one Array of
 * `cols` numbers, toDense() an Array of `rows` of them -- or, on the predict
 * side, an O(cols) C scratch row (see dyn_ml_row_src). `cols` is
 * CALLER-DECLARED metadata: `new CSR([1], [0], [0, 1], 2**40)` is a legal
 * 1-nonzero matrix whose expansion is pure metadata arithmetic, so admission
 * here is what stands between one nonzero and a dead process on a path whose
 * docs promise a clean throw.
 *
 * THE BOUND IS MEASURED, not guessed. The JS Array build is linear in
 * elements at ~24 B of live memory per number (a JSValue plus growth slack)
 * plus ~280 B per row Array -- 2^22: 31 ms/103 MB, 2^24: 110 ms/390 MB,
 * 2^26: 439 ms/1.53 GB, 2^27: 875 ms/3.05 GB for one row; 2^20/2^22/2^24
 * one-column rows: 143 ms/230 MB, 477 ms/896 MB, 2189 ms/3.6 GB -- and it
 * STOPS BEING VIABLE at 2^31 elements: rows=1, cols=2^31 is SIGSEGV inside
 * the Array build after multi-second grinding, cols=2^32-1 dies at rc=139,
 * and even 2^30, which COMPLETES, needs ~23 GB to build one row. The old
 * bound here (dims and total <= 2^32-1) admitted every one of those kills,
 * so an admitted size could die instead of complete.
 *
 * Admission is therefore capped at what completes comfortably: at most 1 GiB
 * of live materialisation by the measured model (24 B * rows * cols + 280 B
 * * rows). That keeps any one Array under 2^26 elements (~48x below the
 * structural death), bounds the predict-side C scratch (8 B/cell) at ~335 MB,
 * and an admitted expansion completes in well under a second (pinned at the
 * boundary by tests: the admitted maximum completes, one past it refuses with
 * the documented RangeError, and no request takes the process down). */
static int dyn_ml_dense_fits(size_t rows, size_t cols)
{
    const size_t budget = (size_t)1 << 30;      /* 1 GiB of live materialisation */
    const size_t per_cell = 24;                 /* measured: JSValue + slack */
    const size_t per_row = 280;                 /* measured: one row Array */

    if (!cols)
        cols = 1;
    if (!rows)
        rows = 1;
    if (cols > (budget - per_row) / per_cell)   /* one row alone is over budget */
        return 0;
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    for (i = 0; i < s->rows; i++)
        for (k = s->ptr[i]; k < s->ptr[i + 1]; k++)
            out[i * s->cols + s->col[k]] += s->val[k];  /* duplicates SUM */
                           ...
                           int argc, JSValueConst *argv)
{
    ...
    for (k = s->ptr[iv]; k < s->ptr[iv + 1]; k++)
        out[s->col[k]] += s->val[k];   /* duplicate indices SUM (scipy) */
...
/* The sparse-path twin of dyn_ml_check_finite, for the two estimators that
 * fit a CSR directly. The dense ingest checks X and y in one place; the CSR
 * arms bypass that ingest. The check runs on each row's READING -- the summed
 * cells dyn_csr_row_reading computes, the very values row()/toDense()/every
 * predict consume -- and NOT on the stored values: every stored value can be
 * finite while a duplicate sum overflows ([1e308, 1e308] at one column READS
 * Infinity), and refusing the CSR form where the dense form refuses (its
 * cells ARE the reading) is the parity "identical whichever form X arrived in"
 * rests on. Both construction paths now refuse a non-finite stored value, so
 * one cannot reach here; a reading check catches one anyway, since a stored
 * NaN sums to NaN. Without this check a NaN or an overflowed sum slips into
 * A^T A / the gradient and the model still "fits" -- silently NaN
 * coefficients, exactly what W9.4 exists to prevent. */
...
/* ---- the predict-side sparse path ---------------------------------- *
 *
 * Every predict kernel below walks X one dense row at a time, so a CSR X is
 * expanded ONE ROW AT A TIME into a cols-double scratch and handed to the
 * SAME kernel the dense form runs: a sparse row IS a dense row with implicit
 * zeros, so the two forms cannot compute different answers because there is
 * only one computation. This is not the whole-matrix expansion the sparse
 * form exists to avoid -- the scratch is O(cols), transient per call, and no
 * predict ever materialises the matrix.
 *
 * The scratch is allocated after the last throwing check of each predict and
 * freed before any JS runs, so no exit path leaks and no user code can detach
 * a buffer while a row pointer is held. */
typedef struct {
    const double *dense;    /* rows*cols for a dense X, else NULL */
    const dyn_csr_t *S;     /* for a CSR X, else NULL */
    double *scratch;        /* cols doubles for a CSR X, else NULL */
    size_t rows, cols;
} dyn_ml_row_src;

/* Bind the source; for a CSR allocate the row scratch. The scratch is one
 * dense row, so it is subject to the same "does not fit in memory" bound as
 * row()/toDense() -- a caller-declared width is metadata, not memory. -1 +
 * RangeError past that bound, -1 + ThrowOutOfMemory on allocation failure
 * (a dense source never allocates). */
...
/* Row i as a dense row: `dense + i * cols` for a dense X; for a CSR the
 * scratch, zero-filled first -- the implicit zeros are the point -- then
 * accumulated from the row's nonzeros. Duplicate column indices within a row
 * SUM (scipy's rule) in storage order, the identical accumulation toDense()
 * and row() use (and the identical sums dyn_csr_row_reading computes), so the
 * expanded row is BITWISE the row() reading of the same
 * CSR and every predict agrees with them by construction. (On the FIT side,
 * LinearRegression now consumes rows through that same reading --
 * dyn_csr_row_reading -- with the same rounding as its dense arm, so a sparse
 * fit is BIT-EQUAL to the dense fit of the same matrix. LogisticRegression's
 * fit gradient is the one walk left applying a duplicate per nonzero through
 * dyn_csr_dot/dyn_csr_axpy: the same sum in a different floating-point order,
 * equivalence-class equal but not guaranteed bit-equal; see the module header.)
 * The returned pointer is invalidated by the next call on a CSR source. */
...
/* The predict-side finite check for a CSR X, run on the READING (the summed
 * cells dyn_csr_row_reading computes) exactly like the fit-side check above:
 * the implicit zeros trivially read finite, but a duplicate sum can overflow
 * while every stored value is finite, and the dense form refuses its cells --
 * which ARE the reading -- so this must refuse the same rows. Same policy and
 * same message as dyn_ml_check_finite_fast, so which form X arrived in does
 * not change what a predict refuses. */
                            ...
                            const double *sw, const dyn_csr_t *S)
{
    ...
    if (p > (SIZE_MAX / sizeof(double)) / p) {
        JS_ThrowRangeError(ctx, "ml.fit: too many features for LinearRegression");
        return -1;
    }
    ...
    if (S) {
         ...
         * the accumulation runs over nonzero PAIRS: O(sum m_i^2) over the
         * row's m_i distinct columns rather than O(rows * cols^2).
         *
         * Each row is consumed through its READING (dyn_csr_row_reading):
         * duplicate column indices summed in storage order and an unsorted row
         * re-sorted -- exactly the matrix row()/toDense()/dyn_ml_row_at serve
         * and the dense arms below compute, however the row was stored. The
         * old loop walked the STORED pairs and assumed one entry per column in
         * ascending order (which only fromDense guarantees): same-column
         * duplicates landed their cross terms on the diagonal and an unsorted
         * row scattered lower-triangle terms where the mirror below overwrote
         * them -- constructor-legal rows silently fitted a different matrix.
         * Only the upper triangle is built (mirrored below); the reading is
         * ascending by column, so the pair loop needs no ordering assumption
         * of its own.
         *
         * The rounding is the dense arm's rounding, product by product: each
         * product goes through a temp (like dyn_ml_axpy -- never contracted
         * into one FMA), and each cell of A^T A / A^T y receives one product
         * per row in row order on both arms, so a sparse fit is BIT-EQUAL to
         * the dense fit of the same matrix. The dense arms' exact-zero
         * contributions (a zero cell multiplies to +-0.0, which adds nothing)
         * are the only skipped terms. */
        for (i = 0; i < S->rows; i++) {
            int bad_nan = 0;
            ...
            long ka, kb;
            if (n < 0) {
                /* unreachable: dyn_ml_check_csr_finite ran over these same
                 * readings first -- kept as the belt on a future caller */
                free(AtA); free(Aty); free(coef); free(ent);
                JS_ThrowRangeError(ctx,
                    "ml.fit: X contains a %s value; fit rejects missing data. Use "
                    "imputeMean(X) or dropMissing(X, y) first.",
                    bad_nan ? "NaN" : "infinite");
    ...
    /* Mirror the symmetric upper triangle. The ridge is NOT added here: the
     * un-ridged system is solved first and the ridge is only tried when the
     * elimination reports a singular matrix (see below).
     *
     * The ridge, when it is needed at all, is scaled by the MEAN WEIGHT and
     * by NOTHING else:
     *   - by the MEAN WEIGHT, so that multiplying every weight by a constant
     *     leaves the fit unchanged. Weighted least squares is homogeneous in
     *     w -- X^T(cW)X b = X^T(cW)y has the same solution as c = 1 -- but a
     *     ridge of fixed absolute size would not be. The system is NORMALIZED
     *     by that mean before the load is formed, so the homogeneity is exact
     *     and no intermediate product can overflow or underflow (see the
     *     fallback below); in normalized units the load is the constant
     *     DYN_RIDGE.
     *   - NOT by the system scale. A load proportional to the largest
     *     diagonal entry of A^T A grows with the units the features happen
     *     to be measured in and zeroes every direction whose diagonal sits
     *     far enough below the largest one -- a unit-scale feature beside a
     *     1e6-scale constant column would be fitted as ~zero. The load must
     *     be blind to the design's internal scales; the mean weight (a
     *     property of the weights alone) is the only permitted multiplier.
     *   - the zero system (an empty fit) gets the same constant, so its
     *     answer is the zero model rather than a second refusal.
     *
     * Which system is solved is a CONDITIONING decision, not a preference:
     * the un-ridged one is taken only while its pivots clear
     * DYN_SOLVE_MIN_PIVOT x its own scale (the gate above separates
     * rounding-level elimination residue from genuine rank deficiency). */
    {
        ...
        /* Un-ridged first: the normal equations ARE ordinary least squares,
         * and for any design whose elimination completes the coefficients are
         * the exact OLS solution to rounding -- the ridge contributes nothing
         * but bias there. The keep-a-copy is the price of falling back:
         * elimination destroys its input, and the fallback needs the original
         * system to add the ridge to. */
        Asave = (double *)malloc((size_t)p * p * sizeof(double));
        bsave = (double *)malloc(p * sizeof(double));
        if (!Asave || !bsave) {
            free(Asave); free(bsave);
            free(AtA); free(Aty); free(coef); free(ent);
            JS_ThrowOutOfMemory(ctx);
            return -1;
        }
        ...
        if (dyn_solve(AtA, Aty, p, ascale * DYN_SOLVE_MIN_PIVOT) != 0) {
            /* Singular normal equations: an exactly zero pivot, a residue at
             * or below the elimination's own rounding noise (the gate above),
             * or an empty fit. Restore the system and solve the ridge-
             * stabilized one, which is what keeps a singular design solvable
             * instead of throwing.
             *
             * The load is formed in NORMALIZED units. Holding a raw
             * "DYN_RIDGE x mean weight x system scale" product is what made
             * the fallback fall over at both ends of the finite weight range:
             * 1e300 weights sent the product to +Inf (the diagonal became Inf,
             * the solve refused its own load), and subnormal 1e-320 weights
             * multiplied it down to 0 (the singular system stayed singular and
             * the caller got "singular error" for a design it was told stays
             * solvable). Dividing the SYSTEM by the weight scale first removes
             * both: the load is then the constant DYN_RIDGE in those units --
             * DYN_RIDGE x the mean weight in raw ones, and nothing more. In
             * particular the load carries NO factor of the system's own scale:
             * scaling any FEATURE column by c must leave the fit's accuracy
             * unchanged, and a scale-proportional load would instead grow
             * with c until it swallowed the directions whose diagonals sit
             * below the largest column's.
             *
             * The solution is unchanged by the normalization -- both sides are
             * divided -- and the fit is exactly homogeneous in the weights:
             * scaling every weight by c divides the system by c and leaves
             * the load alone, which is the property the mean-weight scaling
             * was there for.
             *
             * Weights are finite and positive by the time the solver runs (the
             * ingest refuses otherwise), so the division cannot hit zero or a
             * NaN. The mean is taken MAX-SCALED so that summing it cannot
             * overflow either: w/wmax <= 1, so the sum is at most the row
             * count. For equal weights this is bit-identical to the plain mean
             * (every term is exactly 1).
             *
             * STATED FINITE-INPUT LIMIT: this normalization runs AFTER A^T A
             * is accumulated, and the accumulation multiplies the raw
             * products w*x^2 and w*x*y -- a single product past DBL_MAX
             * (1e300 weights on 1e6-scale features, say) reaches the solve as
             * +Inf, and no load can repair an Inf diagonal. The caller gets
             * the typed "singular system" refusal, not a model. Closing that
             * gap means pre-scaling the columns AND the targets (and the
             * weights) before the accumulation, which would reorder every
             * product the solver's pinned answers are stated in. */
            double wscale = 1.0;
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    rows_arg = argc > 2 ? argv[2] : JS_UNDEFINED;
    cols_arg = argc > 3 ? argv[3] : JS_UNDEFINED;
    /* Coerce ALL args to C buffers BEFORE resolving the handle. The X alias
     * (and any zero-copy y/x view) is taken AFTER the weights getters have run,
     * inside dyn_ml_ingest_Xy, so no JS can run while it is held. */
...
/* predict(X[, rows, cols][, {as: "f64"}]) -> number[] | Float64Array
 * predictInto(out, X[, rows, cols]) -> number of rows written (
 * writes through the caller's Float64Array, never allocates; RangeError on a
 * short out BEFORE anything is written). X may also be a CSR: each row is
 * expanded one row at a time into the cols-sized scratch (duplicate column
 * indices summed at expansion) and fed to the SAME dyn_ml_dot the dense loop
 * runs -- one computation for both forms, so a sparse and a dense call on the
 * same data return bit-identical answers, and the matrix is never expanded. */
                                       ...
                                       int argc, JSValueConst *argv, int into)
{
    ...
    if (into) {
        /* the out alias is taken here, after every JS-running step */
    ...
    /* the one-row scratch is allocated after the last throwing check */
    if (dyn_ml_row_src_init(ctx, &rs, &mx, S)) {
        if (!into) free(yout);
        dyn_matrix_free(&mx);
        return JS_EXCEPTION;
    }
    /* ONE computation for both forms: dyn_ml_row_at yields the dense row --
     * `mx.data + i * cols` for a dense X, the scratch with duplicates summed
     * for a CSR -- and the same dyn_ml_dot reads it. */
    for (i = 0; i < rows; i++)
        yout[i] = m->intercept + dyn_ml_dot(m->coef, dyn_ml_row_at(&rs, i),
                                            m->n_features);
                              ...
                              const double *sw, const dyn_csr_t *S)
{
    ...
    for (it = 0; it < m->max_iter; it++) {
        ...
        for (i = 0; i < rows; i++) {
            ...
            if (S) {
                /* The gradient over the nonzeros only: O(nnz) per iteration
                 * rather than O(rows * cols). A zero feature contributes
                 * nothing to a dot product and nothing to a gradient. (This is
                 * the same objective as the dense arm, accumulated in a
                 * different floating-point order -- a sparse FIT is equivalent
                 * to a dense fit, not bit-equal to it. The PREDICT path has no
                 * such latitude and shares one kernel; see dyn_ml_row_src.) */
...
/* Raw scores for one SPARSE row were computed by a separate nonzero walk
 * (dyn_csr_dot) until it was measured to disagree with the dense kernel by
 * 2-7 ulp -- more under duplicate indices -- which made the "same answers"
 * contract false. Now the sparse form is expanded by dyn_ml_row_at and
 * dyn_logreg_scores runs for both; see dyn_ml_row_src. */

/* Shared body: predict class labels (proba=0) or probabilities (proba=1).
 * For a flat Float64Array X, predict(X, rows, cols).
 * predictInto(out, X[, rows, cols]) in the label (proba=0) arm writes
 * through the caller's Float64Array and returns the row count; a short out
 * is a RangeError before anything is written, and no JS-side allocation
 * happens on the whole path.
 * X may also be a CSR: each row is expanded one row at a time into the shared
 * scratch (duplicate column indices summed at expansion) and scored by the
 * SAME dyn_logreg_scores the dense form runs, so predict and predictProba are
 * bit-identical across forms (this is one of the two estimators whose fit
 * takes a CSR; every other fit still names .toDense()). */
                                       ...
                                       int into)
{
    ...
    K = m->n_classes;
    if (into) {
        /* a short out is refused here, before any write */
        if (dyn_ml_bind_out(ctx, argv[0], rows, &yout)) {
            dyn_matrix_free(&mx);
            return JS_EXCEPTION;
        }
    ...
    /* ONE computation for both forms: the expanded row is scored by the same
     * dyn_logreg_scores the dense form runs. */
    for (i = 0; i < rows; i++) {
    ...
    dyn_matrix_free(&mx); /* done reading X (incl. any alias) before any JS */
    if (into) {
        /* yout aliases the caller's buffer: nothing to free */
        return JS_NewInt64(ctx, (int64_t)rows);
    }
...
static JSValue dyn_logreg_get_classes(JSContext *ctx, JSValueConst t)
{
    ...
    if (!m->fitted) return JS_ThrowInternalError(ctx, "ml.classes: classes before fit");
...
static JSValue dyn_logreg_get_coef(JSContext *ctx, JSValueConst t)
{
    dyn_logreg_t *m = (dyn_logreg_t *)dyn_res_native(ctx, t, dyn_logreg_class_id);
    if (!m) return JS_EXCEPTION;
    if (!m->fitted) return JS_ThrowInternalError(ctx, "ml.coef: coef before fit");
...
static JSValue dyn_logreg_get_intercept(JSContext *ctx, JSValueConst t)
{
    dyn_logreg_t *m = (dyn_logreg_t *)dyn_res_native(ctx, t, dyn_logreg_class_id);
    if (!m) return JS_EXCEPTION;
    if (!m->fitted) return JS_ThrowInternalError(ctx, "ml.intercept: intercept before fit");
                            ...
                            size_t rows, size_t cols, const double *w)
{
    ...
    if (rows < k)
        return JS_ThrowRangeError(ctx,
            "ml.fit: KMeans needs at least nClusters rows"), -1;
    ...
    if (k > 64 && k > rows / 2)
        return JS_ThrowRangeError(ctx,
            "ml.fit: KMeans nClusters too close to the number of rows"), -1;
...
/* predict(X[, rows, cols][, {as: "f64"}]) -> number[] | Float64Array
 * predictInto(out, X[, rows, cols]) -> rows written (no allocation on
 * the JS side; RangeError on a short out before anything is written). */
                                       ...
                                       int argc, JSValueConst *argv, int into)
{
    ...
    if (!into && dyn_ml_parse_as_opt(ctx, argc, argv, 1, &as_f64))
        return JS_EXCEPTION;
    /* a CSR X walks its rows through the shared row source */
    if (JS_GetClassID(xa) == dyn_csr_class_id) {
        S = dyn_ml_as_csr(ctx, xa);
        if (!S)
            return JS_EXCEPTION;
    ...
    /* rows=1 calls, one per row: dyn_km_assign is row-independent, so this is
     * exactly the one whole-matrix call the dense form used to make, and the
     * sparse and dense paths share it */
    for (i = 0; i < rows; i++)
                              ...
                              int newton)
{
    ...
    if (argc > 0 && JS_IsObject(argv[0])) {
        JSValueConst o = argv[0];
        /*the bag is strict. On the plain trees validationFraction/
         * earlyStoppingRounds stay in the table so their targeted "only
         * supported by the XGB models" refusals below keep firing instead
         * of the generic unknown-key error. */
        const char *ks[16];
        int nk = 0, kt;
        for (kt = 0; kt < 9; kt++) ks[nk++] = ml_tree_keys[kt];
        if (newton) {
            for (kt = 0; kt < 7; kt++) ks[nk++] = ml_tree_xgb_extra_keys[kt];
        } else {
    ...
    if (opt.newton) {
        ...
        for (ti = 0; ti < rounds; ti++) {
            if (rounds == 1) {
                ...
                /* bootstrap: rows draws WITH replacement. dyn_fastmod is an
                   exact n % d (see dyn-prng.h), so the draw->index mapping --
                   and with it the seeded reproducibility of every recorded
                   model -- is bit-identical to the division it replaces;
                   rows == 1 keeps taking the draws and maps them to 0. */
...
/* predict(X[, rows, cols][, {as: "f64"}]) / predictInto(out, X[, rows, cols]).
 * Shared by every tree family model (DecisionTree, RandomForest, XGB -- both
 * regressor and classifier variants). */
                                       ...
                                       JSClassID class_id, int into)
{
    ...
    if (!into && dyn_ml_parse_as_opt(ctx, argc, argv, 1, &as_f64))
        return JS_EXCEPTION;
    /* a CSR X walks its rows through the shared row source. No finite
     * check here, same as the dense arm: a tree routes a missing value down
     * its missing branch, sparse or not. */
    if (JS_GetClassID(xa) == dyn_csr_class_id) {
        S = dyn_ml_as_csr(ctx, xa);
        if (!S)
            return JS_EXCEPTION;
    ...
    if (!out || (m->classifier && !votes)) {
        /* in into-mode `out` aliases the caller's buffer: never freed here */
        if (!into) free(out);
        free(votes);
        dyn_matrix_free(&mx);
        return JS_ThrowOutOfMemory(ctx);
                                        ...
                                        JSClassID class_id)
{
    ...
    /* a CSR X walks its rows through the shared row source (no finite
     * check, same as the dense arm -- trees route missing values) */
    if (JS_GetClassID(argv[0]) == dyn_csr_class_id) {
        S = dyn_ml_as_csr(ctx, argv[0]);
        if (!S)
            return JS_EXCEPTION;
                            ...
                            JSValueConst *argv)
{
    ...
    if (argc > 1) {
        ...
        /* -1 = the property get threw (getter ran user JS): refuse rather
         * than silently enable whitening (no-swallow rule). */
        if (m->whiten < 0)
            return JS_EXCEPTION;
    }
                                   ...
                                   int into)
{
    ...
    rows = S ? S->rows : mx.rows;
    nc = m->n_classes;
    /* shape in = shape out; a CSR is neither flat nor row-arrays, and answers
     * in the row-arrays form like an array X does */
    flat = !S && !mx.owned;
    if (into) {
        /* label arm only: probabilities are a rows x nc matrix */
        if (proba) {
            dyn_matrix_free(&mx);
            return JS_ThrowTypeError(ctx,
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    if (argc > 0 && !JS_IsUndefined(argv[0])) {
        if (JS_ToFloat64(ctx, &eps, argv[0]))
            return JS_EXCEPTION;
        if (!(eps > 0.0))
            return JS_ThrowRangeError(ctx, "ml.DBSCAN: eps must be positive");
        if (!isfinite(eps) || !isfinite(eps * eps))
            return JS_ThrowRangeError(ctx,
                "ml.DBSCAN: eps must be finite and its square must not overflow");
    }
    if (argc > 1 && !JS_IsUndefined(argv[1])) {
                                ...
                                JSClassID class_id, int mode, int into)
{
    ...
    if (mode == DYN_SCALE_FIT_FWD) {
        /* fitTransform is a fit: it needs the same missing-data refusal
         * fit() has (W9.4). StandardScaler's sumsq arm happened to trip its
         * overflow guard on a NaN, with a misleading message; MinMaxScaler
         * silently produced NaN min/max -- the inconsistency this check
         * removes. */
        if (dyn_ml_check_finite(ctx, &mx, NULL)) {
            dyn_matrix_free(&mx);
            return JS_EXCEPTION;
    ...
    if (into) {
        ...
        /* rows*cols cannot wrap here: ingest already bounded rows*cols */
                                    ...
                                    int into)
{
    ...
    rows = S ? S->rows : mx.rows;
    /* shape in = shape out; a CSR answers in the row-arrays form like an
     * array X does */
    flat = !S && !mx.owned;
    ...
    if (into) {
        /* labels are a single column: predictInto is label-arm only */
        if (decision) {
            dyn_matrix_free(&mx);
            return JS_ThrowTypeError(ctx,
...
#define DYN_SVM_REFUSE_NONFINITE(dv)                                           \
        do { if (!isfinite(dv)) {                                              \
            if (!into) free(out); /* into-mode out aliases the caller's buffer */ \
            dyn_ml_row_src_free(&rs);                                          \
            dyn_matrix_free(&mx);                                              \
            return JS_ThrowRangeError(ctx,                                     \
                         ...
                         size_t rows, size_t cols)
{
    ...
    /* `ll` was measured by the E step BEFORE the final M step, so it is the
     * likelihood of the PREVIOUS parameters, not of the ones stored above.
     * Report the likelihood of the stored parameters: it is what predictProba
     * answers per row, and what scikit-learn's lower_bound_ reports. One
     * extra O(rows*k*cols) pass against the EM loop that just ran. */
                                    ...
                                    int into)
{
    ...
    if (proba) {
        /* rows=1 calls, one per row: dyn_gmm_estep is row-independent, so
         * this is exactly the one whole-matrix call the dense form used to
         * make, and the sparse and dense paths share it */
        for (i = 0; i < rows; i++)
            dyn_gmm_estep(m, dyn_ml_row_at(&rs, i), 1, out + i * m->k);
    } else {
        double *sc = (double *)malloc(m->k * sizeof(double));
        if (!sc) {
            /* into-mode `out` aliases the caller's buffer: never freed here
             * (same guard as the logreg/NB/KNN/forest/SVC into arms) */
            if (!into)
                free(out);
            dyn_ml_row_src_free(&rs);
            dyn_matrix_free(&mx);
                              ...
                              int argc, JSValueConst *argv, int want_pr)
{
    ...
    if (!want_pr) {
         ...
         *
         * TIES ARE ONE THRESHOLD. A run of equal scores is a single PR point:
         * its precision is read at the END of the run and weighted by the
         * positives the run adds, which is the grouping
         * precision_recall_curve applies. Scoring each item of the run
         * individually over-credited mixed tie groups (measured:
         * AP([1,0,1],[.9,.9,.1]) answered 0.833 where the threshold
         * definition -- and sklearn -- answer 1/2*1/2 + 1/2*2/3 = 7/12). */
...
{
    JSValue v;
    /* NO strict check here: this is an internal helper, and the visible
     * entry points (trainTestSplit / kFold / stratifiedKFold / crossValScore
     * / gridSearch / randomSearch) each validate against their OWN union
     * table before delegating -- a table shared here would either reject a
     * legal key of the caller ({scoring}) or accept-and-ignore one. */
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    /*this split's bag reads {seed, shuffle, testSize} only. */
...
/* The kFold BODY, without the bag check: dyn_ml_folds delegates here
 * on behalf of crossValScore/gridSearch/randomSearch, whose own union tables
 * already validated the bag against their SUPERSET (they admit {scoring}).
 * The public kFold wrapper below adds its own strict check. */
                                 ...
                                 int argc, JSValueConst *argv)
{
    ...
    /* (the bag check lives in the public dyn_ml_kfold wrapper below --
     * this impl is also entered by dyn_ml_folds for crossValScore/
     * gridSearch/randomSearch, whose union tables admit {scoring} too). */
    if (JS_IsObject(argv[0])) {
        JSValue lv = JS_GetPropertyStr(ctx, argv[0], "length");
        if (JS_IsException(lv))
            return JS_EXCEPTION;
...
static JSValue dyn_ml_kfold(JSContext *ctx, JSValueConst this_val,
                            int argc, JSValueConst *argv)
{
    /*kFold's own bag reads {seed, shuffle, k, folds} only. */
    if (dyn_opts_strict(ctx, argc > 1 ? argv[1] : JS_UNDEFINED,
                        ml_sel_fold_keys, 4))
        return JS_EXCEPTION;
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    /*this fold's bag reads {seed, shuffle, k, folds} only. */
    if (dyn_opts_strict(ctx, argc > 1 ? argv[1] : JS_UNDEFINED,
                        ml_sel_fold_keys, 4))
        return JS_EXCEPTION;
    if (dyn_ml_sel_opts(ctx, argc > 1 ? argv[1] : JS_UNDEFINED,
     ...
     * The index tie-break keeps the assignment identical to encounter order. */
    {
        ...
        while (run < n) {
            ...
            uint32_t foldno = 0;
            cnt = 0;
            /* a wrapping counter replaces cnt % k per row: cnt starts at 0
               and advances by one, so the two are equal by induction */
            for (pos = run; pos < n && kv[pos].key == k0; pos++, cnt++) {
                fold[kv[pos].row] = foldno + 1u;               /* 1-based */
                if (++foldno == (uint32_t)k) foldno = 0;
            }
            /* A class with fewer members than k cannot appear in every fold,
             * so some folds have ZERO of it -- the exact failure stratified
             * folding exists to prevent. scikit-learn refuses for the same
             * reason; do the same instead of handing back a misleading split. */
            if (cnt < (size_t)k) {
                JS_ThrowRangeError(ctx,
...
/* Ingest X and y for the model-selection drivers (crossValScore/gridSearch/
 * randomSearch), guaranteeing an OWNED X matrix before any per-fold callback.
 *
 * These drivers call the factory, fit(), predict() and the scorer -- arbitrary
 * user JS -- and then rebuild each fold's subset from mx->data via
 * dyn_ml_rows_subset. Array-of-rows X is already copied by the ingest (owned=1);
 * a flat Float64Array X is aliased ZERO-COPY (owned=0) and this API has no
 * (rows, cols) argument, so we infer rows = len(y), cols = X.length / rows and
 * memcpy it into owned memory. A per-fold callback that ArrayBuffer.transfer()s
 * the caller's X buffer therefore cannot invalidate the features the remaining
 * folds read. The copy is ONCE per search (not per fold), so it is negligible
 * against the fits that follow. y is always an owned copy. -1 (throwing) on err. */
                                ...
                                dyn_matrix_t *mx, double **py)
{
    if (JS_IsArray(ctx, xv))
        return dyn_ml_ingest_Xy_no_w(ctx, xv, yv, JS_UNDEFINED, JS_UNDEFINED,
                                     mx, py);
    /* flat Float64Array X: infer (rows, cols). rows is len(y); cols is the rest
     * of the buffer, so a supervised search can accept a flat target alongside
     * a flat feature matrix with no explicit shape arguments. */
    {
        ...
        if (ret)
            return -1;
        /* the flat ingest aliased X (owned=0); make an owned copy so a per-fold
         * callback cannot detach it under the later dyn_ml_rows_subset reads */
        {
...
/*{nJobs, onProgress}. HONEST CONTRACT: nJobs is VALIDATED and then
 * run SEQUENTIALLY. The estimator factory is a JS function and an os.Worker
 * cannot receive functions (only structured-cloneable data), so per-fold
 * parallelism would need the caller to ship the whole estimator build as a
 * worker script -- not expressible for the general factory this API takes.
 * Rather than silently ignoring the key (the defect class W9.4 exists to
 * prevent) or densifying a fake pool, nJobs>1 runs the identical sequential
 * path and `onProgress` reports fold completion through the same hook a
 * future Worker scheduler would drive. nJobs=1 is the identical fast path:
 * the callback is simply not invoked when absent. 0 ok, -1 throw. */
...
/* Fire {done, total} into the onProgress hook, if any. 0 ok, -1 throw. */
 ...
 *
 * {nJobs, onProgress} -- nJobs is validated (>= 1) and the folds run
 * SEQUENTIALLY (an os.Worker cannot receive the estimator factory; see
 * dyn_ml_parse_jobs). onProgress({done, total}) fires after each fold. */
                                      ...
                                      int argc, JSValueConst *argv)
{
    ...
    /*unknown keys are refused before any fold runs. */
                                  ...
                                  int argc, JSValueConst *argv, int random)
{
    ...
    if (argc < 4 || !JS_IsFunction(ctx, argv[0]) || !JS_IsObject(argv[3]))
        return JS_ThrowTypeError(ctx, random
            ? "ml.randomSearch(estimatorFactory, X, y, grid[, options])"
            : "ml.gridSearch(estimatorFactory, X, y, grid[, options])");
    /*unknown keys are refused before any search work starts. */
    ...
    for (combo = 0; combo < total; combo++) {
        ...
        for (f = 0; f < nfold; f++) {
            ...
            if (bad) { free(tr); free(te); JS_FreeValue(ctx, params); goto done; }
            {
                ...
                /*fold-level progress across the WHOLE search; nJobs is
                 * validated but the walk is sequential (see dyn_ml_parse_jobs) */
 ...
 *
 *  A compiled capability in the sense: the STAGES are the configuration
 ...
 *
 *  OWNERSHIP: by REFERENCE, not cascade. dispose() releases the stage
 *  references the Pipeline holds; it does NOT call close() on the stages.
 *  (An earlier close()-closes-stages version broke stages shared between two
 *  Pipelines and ran JS from a teardown path -- a destructor must not run
 *  arbitrary JS.) A caller who wants a stage gone closes it explicitly or
 *  lets the collector finalise it, which for a resource-backed stage
 *  releases the native memory just the same.
 * ==================================================================== */
...
{
    /* capture the Float64Array class id before any estimator can be built */
    {
