 ...
 * built and timed are in bench/ML_REPORT.md; the summary:
 *
 ...
 * documented in API.md: a fitted coefficient can move by an ULP, and a KMeans
                                 ...
                                 int from, size_t rows, double **pw)
{
    ...
    *pw = NULL;
    ...
    if (rows && sum <= 0.0) {
        free(w);
        JS_ThrowRangeError(ctx, "sampleWeight sums to zero; there is nothing to fit");
        return -1;
    }
    *pw = w;
    return 0;
 ...
 * or 0 on OOM (throwing). Insertion sort: the class count is small. */
 ...
 *
 * bench/ML_PRODUCTION_PLAN.md gap 9: every fit took a dense matrix, so data
    ...
    for (it = 0; it < m->max_iter; it++) {
        ...
        for (k = 0; k < n_out; k++) {
            for (j = 0; j < cols; j++) {
                ...
                if (m->l1 != 0.0) {          /* soft-threshold: exact zeros */
                    double t = m->lr * m->l1;
                            ...
                            size_t rows, size_t cols, const double *w)
{
    ...
    for (it = 0; it < DYN_KMEANS_MAX_ITER; it++) {
        int changed = 0;
        dyn_km_assign(X, rows, cols, cent, k, labels, NULL);
        for (i = 0; i < rows; i++)
            if (labels[i] != prev[i]) {
                changed = 1;
                break;
    ...
    m->inertia = dyn_km_assign(X, rows, cols, cent, k, labels, w);
    free(m->centroids);
    m->centroids = cent;
    m->n_features = cols;
    m->fitted = 1;
...
static void dyn_tree_sort_kv(dyn_tree_kv_t *a, size_t n)
{
    ...
    }
    {
        ...
        if (hi + 1 > 1)
            dyn_tree_sort_kv(a, hi + 1);
        if (lo < n)
            dyn_tree_sort_kv(a + lo, n - lo);
    }
}
    ...
    if (argc > 0 && JS_IsObject(argv[0])) {
        ...
        if (newton &&
            (dyn_opt_double(ctx, o, "lambda", &opt.lambda, 0.0, 1e12) ||
             ...
             dyn_opt_double(ctx, o, "validationFraction", &val_frac, 0.0, 0.9) ||
                            ...
                            const double *w)
{
    ...
    if (opt.max_bins) {
        ...
        hnext = (uint32_t *)malloc((hist.max_bins + 1) * sizeof(uint32_t));
        if (!hbuf || !hnext) {
            JS_ThrowOutOfMemory(ctx);
            goto done;
        }
        ...
        for (r = 0; r < rounds; r++) {
            ...
            for (k = 0; k < n_out; k++) {
                ...
                if (m->classifier) {
                     ...
                     * the value inside it comes from the real loss */
                    double *nn = (double *)realloc(lnum, tr->n_nodes * sizeof(double));
                    double *dd;
                    if (!nn) { JS_ThrowOutOfMemory(ctx); goto done; }
                    lnum = nn;
                    dd = (double *)realloc(lden, tr->n_nodes * sizeof(double));
    ...
    for (sweep = 0; sweep < DYN_JACOBI_SWEEPS; sweep++) {
        ...
        for (i = 0; i < p; i++)
            for (j = i + 1; j < p; j++)
                off += A[i * p + j] * A[i * p + j];
        if (off <= 1e-300)
            break;                    /* already diagonal to full precision */
        for (i = 0; i < p; i++) {
            for (j = i + 1; j < p; j++) {
                double aij = A[i * p + j];
                double theta, t, c, s;
 ...
 *
 * No spatial index, so a region query is a linear scan and fit is O(rows^2 *
 * cols) -- the same complexity scikit-learn falls back to for high-dimensional
 * data, where a tree stops helping. Memory stays O(rows): the neighbour lists are
 * recomputed on demand into one scratch buffer per query rather than materialised
 * as an O(rows^2) matrix, which is what would actually make large inputs fail.
 *
                          ...
                          size_t rows, size_t cols, int *labels)
{
    ...
    for (i = 0; i < rows; i++) {
        ...
        nn = dyn_dbscan_region(X, rows, cols, i, m->eps2, neigh);
        if (nn < m->min_pts)
        ...
        for (qi = 0; qi < qn; qi++) {
            ...
            if (labels[q] == -1)
                labels[q] = cluster;
            sn = dyn_dbscan_region(X, rows, cols, q, m->eps2, seed);
            if (sn < m->min_pts)
                            ...
                            size_t rows, size_t cols, const double *w)
{
    ...
    if (s->minmax) {
        ...
        /* centre = mean, spread = population std (ddof = 0) */
        ...
        for (j = 0; j < cols; j++) {
            double mean = centre[j] / wtot;
            /* var = E[x^2] - mean^2 can go slightly negative on a constant
             * column; clamp rather than hand sqrt() a negative. */
            double var = sumsq[j] / wtot - mean * mean;
            double sd;
            if (var < 0.0)
                var = 0.0;
            sd = sqrt(var);
                        ...
                        size_t d, double gamma)
{
    ...
    case DYN_SVM_POLY: {
        ...
        for (e = 0; e < m->degree; e++)
            r *= base;              /* integer power: exact, and degree is small */
        return r;
    }
    default:
                                    ...
                                    int argc, JSValueConst *argv, int decision)
{
    ...
    for (i = 0; i < rows; i++) {
        ...
        if (decision) {
            ...
            out[i] = (dyn_svm_decide(m, &m->bin[0], xi, m->gamma) >= 0.0)
                         ? m->classes[1] : m->classes[0];
        } else {
            double best = -HUGE_VAL;
            size_t bestk = 0;
            for (k = 0; k < m->n_bin; k++) {
                           ...
                           JSValueConst *argv)
{
    ...
    /* a refit must not leak the previous parameters */
    free(m->weight); free(m->mean); free(m->var);
    free(m->inv_var); free(m->lognorm);
    m->weight = NULL; m->mean = NULL; m->var = NULL;
    m->inv_var = NULL; m->lognorm = NULL;
    m->fitted = 0;
...
/* ==================================================================== *
 *  classification metrics beyond accuracy (STDLIB_OOP_PLAN W9.6)
 * ==================================================================== *
 *
                              ...
                              int argc, JSValueConst *argv, int want_pr)
{
    ...
    if (!want_pr) {
        ...
        qsort(sorted, n, sizeof(double), dyn_auc_cmp);
        for (i = 0; i < n; i++) {
            if (yt[i] != pos)
                continue;
         ...
         * makes, because the trapezoid is optimistically biased here. */
        size_t *idx = (size_t *)malloc(n * sizeof(size_t));
        double tp = 0.0, fp = 0.0, ap = 0.0;
        if (!idx) { free(yt); free(ys); return JS_ThrowOutOfMemory(ctx); }
        for (i = 0; i < n; i++) idx[i] = i;
        /* insertion-free: simple selection by repeated max is O(n^2); use a
         * sort on a paired key array instead */
        for (i = 0; i < n; i++)
            for (j = i + 1; j < n; j++)
                if (ys[idx[j]] > ys[idx[i]]) { size_t t = idx[i]; idx[i] = idx[j]; idx[j] = t; }
        for (i = 0; i < n; i++) {
            if (yt[idx[i]] == pos) { tp += 1.0; ap += tp / (tp + fp); }
            else fp += 1.0;
        }
        free(idx);
        r = ap / npos;
...
#define OPT_GET(name, body) \
    ...
    OPT_GET("seed", { double d; if (JS_ToFloat64(ctx, &d, v)) { JS_FreeValue(ctx, v); return -1; }
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    /* Round-robin within each class. A class's members are handed folds
     * 0,1,..,k-1,0,1,.. in encounter order, so every fold gets that class's
     * count divided as evenly as it divides. */
    {
        size_t assigned = 0;
        for (i = 0; i < n; i++) {
            size_t j, cnt;
            if (fold[idx[i]] != 0u) continue;      /* already placed */
            cnt = 0;
            for (j = i; j < n; j++) {
                if (y[idx[j]] != y[idx[i]]) continue;
                fold[idx[j]] = (uint32_t)(cnt % (size_t)k) + 1u;  /* 1-based */
                cnt++;
                assigned++;
            }
        }
                                  ...
                                  int argc, JSValueConst *argv, int random)
{
    ...
    size_t nfold = 0, nkey = 0, total = 1, combo, k, f;
    ...
    for (combo = 0; combo < total; combo++) {
        JSValue params = JS_NewObject(ctx), rec, sarr, model;
        double mean = 0.0;
        size_t rest = combo;
        ...
        for (f = 0; f < nfold; f++) {
            ...
            /* bind the parameters: the factory is called with them */
            bound = JS_NewCFunctionData(ctx, NULL, 0, 0, 0, NULL);
...
static const JSCFunctionListEntry dyn_ml_funcs[] = {
    ...
    JS_CFUNC_DEF("dropMissing", 2, dyn_ml_drop_missing),
...
/* dispose() deliberately frees NOTHING. It is called by the framework's
 * close(), which has no JSRuntime, and releasing this object's stage
 * references needs one. The custom finalizer below does the whole teardown
 * instead, so there is exactly one place that owns it and no path where the
 * array is freed while the JSValues in it are not.
 *
 * OWNERSHIP IS BY REFERENCE, NOT BY CASCADE. An earlier version had close()
 * call close() on each stage. That is the wrong shape twice over: it would
 * have to run JS from a teardown path, and it makes a stage shared with
 * anything else unusable the moment one Pipeline is closed. Releasing the
 * reference is what ownership means in a collected heap -- a stage nothing
 * else holds is finalized promptly, and one that IS still held keeps working.
 * The docs say so rather than leaving a caller to find out. */
