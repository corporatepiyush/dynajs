                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (!f)
        return JS_EXCEPTION;
    if (i < 0 || (uint32_t)i >= f->n)
        return JS_ThrowRangeError(ctx, "index out of range");
    for (idx = (uint32_t)i + 1; idx <= f->n; idx += idx & (~idx + 1))
        f->tree[idx] += delta;
    return JS_DupValue(ctx, this_val);
}
                                      ...
                                      int argc, JSValueConst *argv)
{
    ...
    if (i < 0 || (uint32_t)i >= f->n)
        return JS_ThrowRangeError(ctx, "index out of range");
    for (idx = (uint32_t)i + 1; idx > 0; idx -= idx & (~idx + 1))
        s += f->tree[idx];
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    double s = 0.0;
    (void)argc;
    if (JS_ToInt32(ctx, &lo, argv[0]))
        return JS_EXCEPTION;
    if (JS_ToInt32(ctx, &hi, argv[1]))
        return JS_EXCEPTION;
    f = (dyn_fenwick_t *)dyn_plain_get(ctx, this_val, dyn_fenwick_class_id);
    if (!f)
        return JS_EXCEPTION;
    if (lo < 0 || hi < 0 || (uint32_t)lo >= f->n || (uint32_t)hi >= f->n)
        return JS_ThrowRangeError(ctx, "index out of range");
    if (lo > hi)
        return JS_NewFloat64(ctx, 0.0); /* empty range */
    for (idx = (uint32_t)hi + 1; idx > 0; idx -= idx & (~idx + 1))
        s += f->tree[idx];
    for (idx = (uint32_t)lo; idx > 0; idx -= idx & (~idx + 1))
        s -= f->tree[idx];
    return JS_NewFloat64(ctx, s);
}
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    if (!s)
        return JS_EXCEPTION;
    if (lo < 0 || hi < 0 || (uint32_t)lo >= s->n || (uint32_t)hi >= s->n)
        return JS_ThrowRangeError(ctx, "index out of range");
    if (lo > hi)
...
static JSValue dyn_bitset_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
    ...
    if (chunked) {
        n = dyn_de_u32(r);
        if (!dyn_de_ok(r) || n > (1u << 26))     /* 4 Gbit cap */
            return dyn_codec_throw(ctx, DYN_DE_TRUNCATED);
    } else {
        n = first;
    ...
    {
        uint32_t nchunks = dyn_de_u32(r), c;
        uint32_t want = (n + DS_BITSET_CHUNKW - 1) / DS_BITSET_CHUNKW;
        if (!dyn_de_ok(r) || nchunks != want) {
            JS_FreeValue(ctx, obj);
            return dyn_codec_throw(ctx, DYN_DE_TRUNCATED);
        }
        for (c = 0; c < nchunks; c++) {
                                  ...
                                  uint32_t *count, uint32_t *elems)
{
    ...
    if (first == DS_NUM_EXT) {
        ...
        e = n * per;
        kind = DS_NUM_RAW;
        if ((uint64_t)e * 8 > (uint64_t)dyn_de_left(r))
            return NULL;
    }
...
static int dyn_ringbuf_write(JSContext *ctx, dyn_ser_t *w, JSValueConst obj)
{
    ...
    if (dyn_ser_u32(w, rb->cap) < 0 || dyn_ser_u32(w, rb->count) < 0)
        return -1;
                            ...
                            int argc, JSValueConst *argv, int magic)
{
    ...
    ds2_str_release(ctx, &k);
    return JS_NewInt64(ctx, (int64_t)out);
}
...
static JSValue ds2_mset_has(JSContext *ctx, JSValueConst this_val,
                            int argc, JSValueConst *argv)
{
    JSValue c = ds2_mset_count(ctx, this_val, argc, argv);
    int64_t n;
    if (JS_IsException(c))
        return c;
    if (JS_ToInt64(ctx, &n, c)) {
        JS_FreeValue(ctx, c);
        return JS_EXCEPTION;
    }
    JS_FreeValue(ctx, c);
...
static JSValue ds2_mset_total(JSContext *ctx, JSValueConst this_val)
{
    ...
    if (!w)
        return JS_EXCEPTION;
    return JS_NewInt64(ctx, (int64_t)dyn_mset_total(w->c));
}
...
static const JSCFunctionListEntry ds2_mset_proto[] = {
    ...
    JS_CFUNC_MAGIC_DEF("add", 2, ds2_mset_add, 0),
    JS_CFUNC_MAGIC_DEF("remove", 2, ds2_mset_add, 1),
...
static const JSCFunctionListEntry ds2_mmheap_proto[] = {
    JS_CFUNC_DEF("push", 2, ds2_mmheap_push),
...
static JSValue ds2_cms_getter(JSContext *ctx, JSValueConst this_val, int magic)
{
    ...
    default: return JS_NewInt64(ctx, (int64_t)dyn_cms_total(w->c));
    }
}

static const JSCFunctionListEntry ds2_cms_proto[] = {
    JS_CFUNC_DEF("add", 2, ds2_cms_add),
...
static JSValue ds2_mmap_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
    ...
    for (i = 0; i < nk; i++) {
        ...
        /* A per-key count is a length too: it must fit the payload. */
    ...
    vals = ds2_read_cells(ctx, r, total);
    if (JS_IsException(vals)) {
        free(keys); free(klens); free(counts);
        return vals;
    }
    ...
    for (i = 0; i < nk; i++)
        for (j = 0; j < counts[i]; j++) {
            JSValue v = JS_GetPropertyUint32(ctx, vals, total++);
...
static JSValue ds2_btree_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
    ...
    if (!k)
        return dyn_codec_throw(ctx, DYN_DE_TRUNCATED);
    vals = dyn_codec_read_values(ctx, r);
    if (JS_IsException(vals)) {
        free(k);
        return vals;
    }
