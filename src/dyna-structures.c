...
#include "core/dyn-prng.h"      /* dyn_os_entropy: seeds the string hashes */
 ...
 *   SortedMap    has entries(); handing it [k,v] iteration too is a deliberate
 *                follow-up decision, not a drive-by (the pin lives in
 *                tests/test_structures_iter.js).
 */
...
/* Per-process seed for the module's string hashes, same discipline as
 * dyn-ds.c's index hash (audit 13.8.1): a fixed hash is collision-floodable
 * with precomputed keys -- for the LRU that means every hostile key landing
 * in one bucket chain, O(n) per op. Seeded once per process; the first caller
 * wins and later ones no-op, so a table grown under one seed is never
 * re-hashed under another. Atomic because the first caller can be a worker
 * thread. Until entropy arrives the fallback is the golden-ratio constant:
 * dyn_os_entropy fails closed (writes nothing on refusal), and a fixed seed
 * beats reading uninitialized bytes. */
...
/* Two independent 64-bit hashes of the key (FNV-1a + a second seed), both
 * starting from the per-process seed. NOTE: because the hashes are seeded, a
 * filter's bits only mean anything within the process that set them -- a
 * filter moved across processes (DYNS reload in a fresh runtime) must be
 * re-populated, not queried, or it would answer with false negatives. */
static void dyn_bloom_hashes(const char *s, size_t n, uint64_t *h1, uint64_t *h2)
{
    /* FNV-1a offset basis. The old constant here read ...6603 -- the real
     * basis ends ...6037; the typo halved the avalanche in the low bits. */
    uint64_t a = 14695981039346656037ULL ^ dyn_str_seed();
    uint64_t c = ~a; /* second, independent starting point */
    size_t i;
    for (i = 0; i < n; i++) {
 ...
 * cannot overflow the C stack (cf. the libscl fixed-4096-on-stack bug).
 * ===================================================================== */
...
static DynTrieNode *dyn_trie_node(dyn_trie_t *t)
{
    ...
    if (!c || c->used == c->cap) {
        uint32_t cap = c ? (c->cap < DYN_TRIE_CHUNK_MAX ? c->cap * 2
                                                        : DYN_TRIE_CHUNK_MAX)
                         : 32;
        c = (DynTrieChunk *)dyn_nat_malloc(sizeof(*c) +
                                         ...
                                         int argc, JSValueConst *argv)
{
    ...
    if (base->child) {
        if (scap < 8) {
            ...
            if (!nn || !dd) { dyn_nat_free(nn ? nn : nstk); dyn_nat_free(dd ? dd : dstk);
    ...
    dyn_nat_free(kb); dyn_nat_free(nstk); dyn_nat_free(dstk);
    return arr;
fail:
    JS_FreeCString(ctx, pfx);
    dyn_nat_free(kb); dyn_nat_free(nstk); dyn_nat_free(dstk);
...
/* FNV-1a over the key. The offset basis is XORed with the per-process seed
 * (dyn_str_seed, same discipline as dyn-ds.c's index hash): the constant
 * alone is public knowledge, so an unseeded table is collision-floodable --
 * every precomputed key into one bucket chain, O(n) per op. The old basis
 * constant read ...6603, a corrupted FNV basis (the real one ends ...6037). */
 ...
 * pilots): an unknown own enumerable string key in LRU's options bag throws
 *   TypeError: unknown option "X" (valid: a, b, c)
 * instead of being silently ignored -- `{ttlMs: 30000, onEvict: fn}` is the
 * eviction-audit pair, and `{ttlmMs: 30000}` (or `{ttl: 30000}`) used to
 * build a cache with NO expiry while the caller believed otherwise. This is
 * the ONLY options bag in dyna-structures.c (every other constructor takes
 * positional args). Symbol keys and non-enumerable properties are not
 * options and stay invisible (JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY).
 * Getters are NOT invoked here; values are read afterwards, so a throwing
 * getter on a KNOWN key still propagates. Returns 0 when the bag is clean,
 * -1 with the TypeError pending. */
                            ...
                            int argc, JSValueConst *argv)
{
    ...
    if (!L->buckets) { dyn_nat_free(L); return JS_ThrowOutOfMemory(ctx); }
    if (argc > 1 && JS_IsObject(argv[1])) {
        /*unknown keys are refused BEFORE anything is allocated or
         * read; a non-object second arg keeps the historic "no options"
         * reading. */
        ...
        if (!JS_IsUndefined(v)) {
            ...
            if (ms <= 0) {
                dyn_nat_free(L->buckets); dyn_nat_free(L);
...
/* entries(): a fresh Array of [key, value] pairs, most-recently-used first
   (the recency list's own order). Expired entries are absent, matching
   get()/has(). The Array is a snapshot, which is also what makes the shared
   [Symbol.iterator] below safe: it walks this Array's own iterator, never a
   live cursor into the bucket table. */
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    if (JS_IsException(arr))
        return arr;
    now = dyn_lru_now_ms();     /* one clock read per snapshot, not per node */
    for (n = L->head; n; n = n->next) {
        JSValue pair;
        if (dyn_lru_expired(n, now))
            continue;
        ...
        if (JS_DefinePropertyValueUint32(ctx, pair, 0,
                JS_NewStringLen(ctx, n->key, n->klen), JS_PROP_C_W_E) < 0 ||
            JS_DefinePropertyValueUint32(ctx, pair, 1,
                JS_DupValue(ctx, n->value), JS_PROP_C_W_E) < 0) {
            JS_FreeValue(ctx, pair);    /* not yet stored in arr */
            JS_FreeValue(ctx, arr);
            return JS_EXCEPTION;
        }
        if (JS_DefinePropertyValueUint32(ctx, arr, out++, pair,
                                         JS_PROP_C_W_E) < 0) {
            JS_FreeValue(ctx, arr);     /* pair consumed by the define */
            return JS_EXCEPTION;
        }
    }
    return arr;
...
/* [Symbol.iterator] delegates through the shared ds2 snapshot helper; the
   definition (DS2_ITERATOR below) sits with that helper near the ds2
   bindings, which are compiled after this Gen-1 class. */
...
/* =====================================================================
 * Sorted containers -- SortedSet (keys only) and SortedMap (key -> JS
 * value). Both sit on the SAME dyn-ds B+tree core as BTree (M30-04: the
 * skiplist that predated it carried worse constants on every point op --
 * 1.4-4x slower set/get/has/delete/churn at 1e6 keys -- and ~13% more
 * memory per entry; only whole-structure snapshots were faster, and the
 * core's dyn_btree_get_keys bulk copy closes that). O(log n) for
 * add/get/delete/floor/ceil. NaN keys are refused at the binding, below.
 * ===================================================================== */

typedef struct {
    JSRuntime *rt;
    dyn_btree_t *b;              /* the shared dyn-ds B+tree core */
} dyn_skip_t;
...
/* Insert/update key. `val` is dup'd; the old value (if any) is freed.
 * Returns 0, or -1 on OOM (exception NOT set here; caller throws). The
 * core rolls its leaf insert back on a failed split (E10-01), so -1
...
/* Remove `key`; frees the stored cell. Returns 1 if it was there. */
...
/* shared: the first (magic 0) or last (magic 1) key as a number, or undefined
...
/* shared: fresh ascending Array of every key. The per-LEAF walk (one callback
 * per order-32 leaf) hands over contiguous key runs, so the per-key cost is
 * just JS_NewFloat64 + the array append -- no per-element call across the
 * module boundary, and no intermediate key buffer. */
typedef struct {
    JSContext *ctx;
    JSValue arr;
    uint32_t i;
                          ...
                          const dyn_cell_t *vals, uint32_t n)
{
    ...
    for (j = 0; j < n; j++) {
        if (JS_DefinePropertyValueUint32(st->ctx, st->arr, st->i++,
                JS_NewFloat64(st->ctx, keys[j]), JS_PROP_C_W_E) < 0) {
            st->failed = 1;
            return 0;
        }
    }
    ...
    if (dyn_btree_iter_seek(s->b, lo, &it)) {
        ...
        do {
            if (!dyn_btree_iter_get(&it, &k, &v) || k > hi)
                break;
            if (JS_DefinePropertyValueUint32(ctx, arr, out++,
                    JS_NewFloat64(ctx, k), JS_PROP_C_W_E) < 0) {
                JS_FreeValue(ctx, arr);
                return JS_EXCEPTION;
            }
        ...
        do {
            ...
            if (JS_IsException(pair) ||
                JS_DefinePropertyValueUint32(ctx, pair, 0,
                    JS_NewFloat64(ctx, k), JS_PROP_C_W_E) < 0 ||
                JS_DefinePropertyValueUint32(ctx, pair, 1,
                    JS_DupValue(ctx, ds2_value(v)), JS_PROP_C_W_E) < 0 ||
...
/* entries(): [key, value] pairs in the same ascending-key order keys() walks
   (the snapshot shape of SortedSet's toArray). Same per-leaf walk: the pair
   construction is pure JS work, the keys/values arrive as contiguous runs. */
typedef struct {
    JSContext *ctx;
    JSValue arr;
    uint32_t i;
    ...
    for (j = 0; j < n; j++) {
        ...
        if (JS_DefinePropertyValueUint32(st->ctx, pair, 0,
            ...
            JS_DefinePropertyValueUint32(st->ctx, pair, 1,
                JS_DupValue(st->ctx, ds2_value(&vals[j])), JS_PROP_C_W_E) < 0) {
            JS_FreeValue(st->ctx, pair);    /* not yet stored in arr */
            st->failed = 1;
            return 0;
        }
        if (JS_DefinePropertyValueUint32(st->ctx, st->arr, st->i++, pair,
                                         JS_PROP_C_W_E) < 0) {
            JS_FreeValue(st->ctx, st->arr);  /* pair consumed by the define */
            st->failed = 2;
            return 0;
        }
    }
                                     ...
                                     int argc, JSValueConst *argv)
{
    ...
    st.failed = 0;
    st.arr = JS_NewArray(ctx);
    if (JS_IsException(st.arr)) return st.arr;
    dyn_btree_walk_leaves(s->b, skip_pairs_leaf, &st);
    if (st.failed == 2)
        return JS_EXCEPTION;            /* st.arr freed by the callback */
    if (st.failed) {
        JS_FreeValue(ctx, st.arr);
        return JS_EXCEPTION;
    }
...
static JSValue dyn_lru_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
    ...
    if (!keys || !klens) {
        dyn_nat_free(keys); dyn_nat_free(klens);
        return JS_ThrowOutOfMemory(ctx);
    }
    for (i = 0; i < n; i++)
                          ...
                          JSClassID id, int with_values)
{
    ...
    if (!s)
        return -1;
    /* One bulk copy of the ascending key stream; every arm below classifies
     * and writes from the buffer. Same wire format as when the walk was a
     * live level-0 chain traversal. */
    ...
    /* Classify: whether every key is an exact integer, and whether every
     * delta is the same. Identical arithmetic to the old chain walk: step is
     * the FIRST delta, and any later delta that differs drops the arm. */
    {
        int all_int = 1, arithmetic = 1;
        double step = 0;
        for (i = 0; i < n; i++) {
            if (!dyn_sk_is_int(keys[i])) { all_int = 0; arithmetic = 0; break; }
            if (i == 1) step = keys[1] - keys[0];
            else if (i > 1 && keys[i] - keys[i - 1] != step) arithmetic = 0;
        }
        if (all_int && n >= 2 && arithmetic) {
            if (dyn_ser_u32(w, DS_SKIP_EXT) < 0 ||
                dyn_ser_u8(w, DS_SK_ARITHMETIC) < 0 ||
                dyn_ser_u32(w, n) < 0 ||
                dyn_ser_f64(w, keys[0]) < 0 ||
    ...
    if (!with_values)
        return 0;
    /* one ordered walk collects every value: the previous per-index cursor
                             ...
                             const char *method, int with_values)
{
...

    {
        ...
        case DS_SK_ARITHMETIC: {
            ...
            if (!dyn_de_ok(r)) { dyn_nat_free(keys); return dyn_codec_throw(ctx, DYN_DE_TRUNCATED); }
...
/* The core stores engine values in an opaque 16-byte cell. The cell helpers
 * (ds2_cell / ds2_value / ds2_free_cell / ds2_dup_cell) live ABOVE the
 * SortedSet/SortedMap section: the consolidated sorted containers store
 * JSValues in dyn_btree cells too, so the conversion pair must be visible to
 * both halves of the module. (M30-04) */
...
/* LRU's [Symbol.iterator] shares the ds2 snapshot machinery: entries() builds
 * a fresh Array of [key, value] pairs and the iterator is that Array's own
 * values() iterator. Forward-declared at the Gen-1 proto table above. */
...
static JSValue ds2_mmap_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
    ...
    if (!keys || !klens || !counts) {
        dyn_nat_free(keys); dyn_nat_free(klens); dyn_nat_free(counts);
        return ds2_oom(ctx);
    }
    for (i = 0; i < nk; i++) {
        keys[i] = dyn_de_blob(r, &klens[i]);
...
static int ds2_table_write(JSContext *ctx, dyn_ser_t *w, JSValueConst obj)
{
    ...
    dyn_nat_free(rows); dyn_nat_free(cols); dyn_nat_free(tmp); dyn_nat_free(rid); dyn_nat_free(cid);
    if (rc < 0)
        return -1;
    return ds2_write_cells(ctx, w, o->c, n, ds2_table_cell);
}
                             ...
                             size_t **off, size_t **len)
{
    ...
    *blob = b; *off = o; *len = l;
    return 0;
fail:
    dyn_nat_free(b); dyn_nat_free(o); dyn_nat_free(l);
...
static JSValue ds2_table_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
    ...
    if (!rk || !ck || !rl || !cl) {
        dyn_nat_free(rk); dyn_nat_free(ck); dyn_nat_free(rl); dyn_nat_free(cl);
        return ds2_oom(ctx);
    }
    if (first != DS_TABLE_EXT) {
        for (i = 0; i < n; i++) {
        ...
        if (ds2_tbl_read_dict(r, &rn, &rblob, &roff, &rlen) < 0 ||
            ...
            dyn_nat_free(rk); dyn_nat_free(ck); dyn_nat_free(rl); dyn_nat_free(cl);
            dyn_nat_free(rblob); dyn_nat_free(roff); dyn_nat_free(rlen);
            dyn_nat_free(cblob); dyn_nat_free(coff); dyn_nat_free(clen);
            return dyn_codec_throw(ctx, DYN_DE_TRUNCATED);
        }
        for (k = 0; k < n; k++) {
            uint64_t a, b;
...
static JSValue ds2_rmap_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
...

    {
        ...
        if (!lo || !hi) {
            dyn_nat_free(lo); dyn_nat_free(hi); dyn_nat_free(b);
            return ds2_oom(ctx);
        }
    ...
    vals = ds2_read_cells(ctx, r, n);
    if (JS_IsException(vals)) {
        dyn_nat_free(lo); dyn_nat_free(hi);
        return vals;
    }
    m = dyn_rmap_new();
    if (!m) {
...
static JSValue ds2_itree_read(JSContext *ctx, dyn_de_t *r, JSValueConst opts)
{
    ...
    for (i = 0; i < n; i++) {
        JSValue v = JS_GetPropertyUint32(ctx, vals, i);
        dyn_cell_t cell = ds2_cell(v);
        /* A malformed record's bounds must not poison the sort: a NaN bound
         * makes the comparator non-total -- every later query silently
         * mis-answers. NaN is the ONLY thing skipped: a point span (hi ==
         * lo) is a legal insert() and a query-visible member -- dropping it
         * here made deserialize lose data the live tree answers for -- and
         * an inverted span clamps to a point in dyn_itree_insert, exactly
         * what the same (lo, hi, v) through the JS API would have stored. */
