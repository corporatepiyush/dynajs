...
static void file_job_work(void *arg)
{
    ...
    if (j->op == FILE_OP_READ) {
         ...
         * even opened -- including the overwrite-onto-self case where the
         * O_TRUNC then empties the shared inode. Mirror that exactly. */
        j->len = (size_t)st.st_size;
        flags = O_WRONLY | O_CREAT | O_CLOEXEC | (j->overwrite ? O_TRUNC : O_EXCL);
...
static void file_job_done(void *arg)
{
    ...
    if (j->err || (j->op == FILE_OP_COPY && j->is_dir)) {
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    res = dyn_res_wrap(ctx, dyn_flock_class_id, l, dyn_flock_dispose);
    return res;
}
                                   ...
                                   int argc, JSValueConst *argv)
{
    ...
    l = (dyn_flock_t *)dyn_res_native(ctx, this_val, dyn_flock_class_id);
    if (!l)
        return JS_EXCEPTION;
    ret = JS_Call(ctx, argv[0], JS_UNDEFINED, 0, NULL);
    /* release no matter what the callback did */
    dyn_flock_dispose(l);
