            ...
            if (acc_len + take + 1 > acc_cap) {
                ...
                if (nc > DYN_MAX_INPUT) {
                    ...
                    return JS_ThrowRangeError(ctx,
                        "FileReader.read: accumulated read exceeds %u bytes"
                                  ...
                                  int argc, JSValueConst *argv)
{
       ...
       and would follow a symlink component. */
    {
        ...
        if (rfd < 0) {
            int e = errno;
            JSValue ex = dyn_fs_throw(ctx, e, "readFile", path);
            dyn_path_unborrow(ctx, path);
            return ex;
        }
        ...
        if (dyn_io_slurp_fd(rfd, &src, 0, DYN_MAX_INPUT) < 0) {
            ...
            if (se == EFBIG)
                return JS_ThrowRangeError(ctx,
                    "readFile: file exceeds %u bytes (DYN_MAX_INPUT)",
                    (unsigned)DYN_MAX_INPUT);
            /* read-leg failures (EISDIR on a directory, EACCES, ...) carry
             * .code/.errno like the open leg -- a bare InternalError made
             * callers branch blind. */
            return dyn_fs_throw(ctx, se ? se : EIO, "readFile", path);
        }
    }
    dyn_path_unborrow(ctx, path);
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    if (argc > 1 && JS_IsObject(argv[1])) {
        JSValue v = JS_GetPropertyStr(ctx, argv[1], "retry");
        if (!JS_IsUndefined(v)) {
                                 ...
                                 int argc, JSValueConst *argv)
{
        ...
        return JS_EXCEPTION;
    {
        ...
        if (rfd < 0) {
            int e = errno;
            JSValue ex = dyn_fs_throw(ctx, e, "readBytes", path);
            dyn_path_unborrow(ctx, path);
            return ex;
        }
        /* dyn_io_slurp_fd owns (closes) rfd on every path. */
        if (dyn_io_slurp_fd(rfd, &src, 0, DYN_MAX_INPUT) < 0) {
            ...
            if (se == EFBIG)
                return JS_ThrowRangeError(ctx,
                    "readBytes: file exceeds %u bytes (DYN_MAX_INPUT)",
                    (unsigned)DYN_MAX_INPUT);
            return dyn_fs_throw(ctx, se ? se : EIO, "readBytes", path);
        }
    }
    dyn_path_unborrow(ctx, path);
                              ...
                              JSValueConst *argv)
{
    ...
    a[0] = f->path;
    a[1] = argv[0];
    JSValue r = dyn_fs_copy_file(ctx, JS_UNDEFINED, 2, a);
