 ...
 *
 * Only the ASYNC one-shots (readFileAsync/writeFileAsync/syncAsync) offload
 * through the dyna-aio adapter (io_uring on Linux, kqueue + thread pool
 * elsewhere); FileReader, FileWriter and the sync writeFile are plain
 * read(2)/write(2) over a buffer, and must never add a copy or a syscall the
 * raw OS API would not. Handles are value handles: constructed with a path,
 * closed
...
/* ----: STRICT OPTIONS BAGS -----------------------------------------
 *
 * Every options object this module parses goes through here. An unknown key
 * is a TypeError naming the key and the valid ones
 * (`unknown option "X" (valid: a, b, c)`), never a silent skip: before this
 * check, `readFile(p, {bytes: true})` "worked" and returned a string, which
 * is the worst failure mode a caller can hit -- the call succeeds and does
 * the slow/wrong thing. dyna:file follows the pilot convention set by
 * dyna:csv, and applies it to EVERY bag in the module, not just the new
 * ones, so the rule is learnable.
 *
 * Enumerated STRING keys only (JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY, the
 * same sweep that drives for-in): symbol keys and non-enumerable properties
 * are not options and stay invisible. null/undefined and primitives are not
 * bags and keep the historic "no options" reading. The message names the key
 * in full for realistic keys; a pathologically long key is truncated by the
 * engine's fixed 256-byte error-message buffer, which is QuickJS formatting,
 * not this check. Returns 0 when the bag is clean, -1 with the exception
 * pending. */
        ...
        }
        {
            ...
            next = openat(dirfd, comp,
                          ...
                          ((comp[0] == '.' && (comp[1] == '\0' ||
                           ...
                           ? 0 : O_NOFOLLOW));
            if (next < 0) {
                /* The documented refusal for a symlink component is ELOOP,
                   and that is what the LEAF open reports everywhere. But with
                   O_DIRECTORY also set, macOS reports ENOTDIR here, which
                   would make e.code wrong for exactly the case the strict
                   walk exists to catch. Only a symlink component is mapped
                   to ELOOP; a genuine file-in-path ENOTDIR is kept. */
                int saved = errno;
                struct stat lst;
 ...
 *
 * (Hoisted above FileReader/FileWriter/readFile/writeFile: those entry points
 * go through here too -- a failed open() reports ENOENT/EACCES/ELOOP like
 * every other fs op, and callers branch on .code (see FileLock's EWOULDBLOCK
 * example in the API reference). They used to throw a bare InternalError that
 * lost the errno entirely.) */
...
/* Forward decl: the fresh-Uint8Array builder is defined with the File handle
 * class further down; the reader's byte paths need it now. */
static JSValue dyn_file_bytes_to_u8(JSContext *ctx, const uint8_t *data,
                                    size_t n);

/* Shared body of read()/readAll()/readBytes(): pull up to `want` raw bytes
 * (want < 0 = to EOF) through the fill buffer into one owned block. The
 * string and the bytes variants differ ONLY in how the block is wrapped, so
 * there is one accumulation loop and one DYN_MAX_INPUT refusal point -- the
 * cap binds the byte path too, because a Uint8Array is memory exactly like a
 * string is and neither may grow without bound on purpose. `op` names the
 * entry point in the messages. */
        ...
        for (;;) {
            ...
            if (acc_len + take + 1 > acc_cap) {
                ...
                while (nc < acc_len + take + 1)
                    nc *= 2;
                /* readFile caps whole-file reads at DYN_MAX_INPUT with a clean
                 * RangeError; an omitted-n read()/readAll() is the same
                 * whole-file operation and gets the same bound, so a huge
                 * stream OOMs nowhere -- it is refused here instead. */
                if (nc > DYN_MAX_INPUT) {
                    free(acc);
                    return JS_ThrowRangeError(ctx,
                        "%s: accumulated read exceeds %u bytes"
                        " (DYN_MAX_INPUT)", op, (unsigned)DYN_MAX_INPUT);
                }
 ...
 * STRING-y by contract: invalid UTF-8 becomes U+FFFD. The byte paths are
 * readInto()/readBytes() below. */
...
/* readBytes([n]) -> the byte twin of read([n]): a fresh Uint8Array of up to
 * n raw bytes, the whole rest at EOF-ward omission. No UTF-8 step touches
 * the bytes, so what went in is what comes out. */
...
/* readInto(buf) -> bytes copied, 0 = EOF. The BYTES primitive: memcpy straight
 * out of the fill buffer into the caller's typed array, bypassing the string
 * accumulator entirely. It is a raw-byte move, NOT a text read -- no UTF-8
 * repair happens here, so a multi-byte sequence split across the edge of a
 * caller's buffer arrives split, exactly as it sits on disk; reassemble at
 * the text layer if text is what you want. A buffer larger than the fill
 * buffer simply loops fills.
 *
 * Coerce-before-bind (the UAF class a sibling lane's reviewer caught in a
 * stats kernel): the view is resolved and every JS-facing step is done
 * BEFORE the backing pointer is bound, and the intermediate ArrayBuffer
 * reference is held to the end of the call so nothing the GC does to the
 * caller's view can strand the pointer mid-copy. The loop body is memcpy and
 * read(2) only -- no JS can run between copies. */
                                     ...
                                     int argc, JSValueConst *argv)
{
    ...
    if (JS_IsException(ab))
        return JS_EXCEPTION; /* not a view: QuickJS's message names it */
    base = JS_GetArrayBuffer(ctx, &ab_size, ab);
    if (!base) {
        JS_FreeValue(ctx, ab);
        return JS_EXCEPTION; /* detached mid-resolve; already threw */
    }
    ...
    while (total < len) {
        ...
        if (f == 0)
            break; /* EOF: return what we have (0 when nothing) */
        avail = r->end - r->start;
                                     ...
                                     int argc, JSValueConst *argv)
{
    ...
    for (;;) {
        ...
        if (acc_len + take + 1 > acc_cap) {
            ...
            while (nc < acc_len + take + 1)
                nc *= 2;
            /* A line is bounded by the whole-read cap for the same reason as
             * read()/readAll() above: a newline-less multi-GB stream would
             * otherwise accumulate without any refusal point. */
            if (nc > DYN_MAX_INPUT) {
                free(acc);
                return JS_ThrowRangeError(ctx,
...
static const JSCFunctionListEntry dyn_freader_proto[] = {
    ...
    /* The byte paths. readInto is the primitive (caller owns the buffer);
     * readBytes allocates. read/readLine/readAll stay string-y. */
...
/* Jobs SUBMITTED to the pool whose completion has not run yet. The reaper may
 * only unhook when this is zero TOO: with two jobs whose completions straddle
 * a drain, a pending-only check unhooks after the first release while the
 * second job is still running -- its release is then counted but never
 * performed, the ref sticks, and the hook's wakeup holds the loop open forever
 * (the arg2_async_inflight shape in dyna-crypto.c, found there as a hang).
 * The completion-time re-arm below stays as the backstop for the case
 * dyn_net_on_drain FAILED at submit: then no hook exists to keep armed. */
static _Thread_local int file_async_inflight;
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    if (argc > 1 && JS_IsObject(argv[1])) {
        ...
        {
            int b = JS_ToBool(ctx, v); /* -1 = the property get threw (getter ran user JS): refuse */
            JS_FreeValue(ctx, v);
            if (b < 0)
                return JS_EXCEPTION;
            append = b;
    ...
    *powned = NULL;
    ...
    }
    {
        ...
        /* GetArrayBufferView, not GetTypedArrayBuffer: the latter rejects a
           DataView, which then fell through to ToString and wrote the literal
           text "[object DataView]" -- silent corruption with a plausible byte
           count. dyn_crypto_data fixed this exact defect with the same one-
           line change; the d.ts promises DataView here too. */
...
/* defined below, next to dyn_fs_throw: the async completions share its
   {message, errno, code, op} shape */
static JSValue dyn_fs_error_obj(JSContext *ctx, int e, const char *op,
                                const char *path);
...
/* ---- shutdown sweeps: parked async file ops fail at teardown ----------
 *
 * A file job parks its resolve/reject across JS -- readFileAsync on a FIFO
 * with no writer parks FOREVER -- and those pins keep the promise and its
 * forest alive past the last JS_FreeContext, tripping JS_FreeRuntime's
 * gc_obj_list assertion (exit 134) IF the teardown is reached. Each job
 * registers a shutdown sweep at submit and unregisters at settle; the sweep
 * FAILS the park (a rejection naming engine shutdown, delivered while JS
 * is legal) and releases the pins. The JOB STRUCT stays with its worker: a
 * completion that still lands sees the released pins (UNDEFINED) and only
 * does the C bookkeeping.
 *
 * LIVENESS POLICY (the same rule the stream and scrape parks follow, one
 * paragraph at both declarations): a parked native op never holds the
 * event loop BY ITSELF -- a live event source does. A file job holds the
 * shared I/O reactor while its worker runs, because the completion is an
 * EVENT that can arrive from outside the JS heap: a readFileAsync keeps
 * the process alive until the kernel answers -- a FIFO with no writer
 * keeps it alive forever (Node's fs does the same), so that park reaches
 * the sweeps only when something else ends the process (an uncaught throw,
 * an unhandled rejection). A park whose only settler is future JS (the
 * stream pipelines, lines next(), codec ops, crawl waiters) holds nothing:
 * when JS is done and no event source remains, the process exits QUIETLY
 * and the sweeps fail those parks. The distinction is the completer, not
 * the family: whichever op still has a completer outside the JS heap keeps
 * the loop; the park itself never does. */
static void file_park_fail(JSContext *ctx, JSRuntime *rt,
...
static void fsync_job_sweep(JSContext *ctx, JSRuntime *rt, void *opaque)
{
    fsync_job_t *j = (fsync_job_t *)opaque;
    file_park_fail(ctx, rt, &j->resolve, &j->reject,
                   "FileWriter.syncAsync: aborted at engine shutdown");
    if (!ctx) {
        JS_FreeValueRT(rt, j->self);
        j->self = JS_UNDEFINED;
    } else {
...
static void fsync_job_done(void *arg)      /* LOOP: JS is safe here */
{
    ...
    JS_RemoveShutdownSweep(JS_GetRuntime(ctx), fsync_job_sweep, j);
    if (JS_IsUndefined(j->resolve)) {
        /* the shutdown sweep already failed this park and released the
           pins: only the reactor bookkeeping is left */
        if (j->offloaded) {
            file_async_pending_release++;
            if (!file_async_hooked &&
                dyn_net_on_drain(file_async_reap, &file_async_hooked) >= 0) {
                file_async_hooked = 1;
                file_async_ctx = ctx;
            }
            file_async_inflight--;
    ...
    if (j->rc < 0) {
        v = dyn_fs_error_obj(ctx, j->err ? j->err : EIO, "sync", NULL);
        a1[0] = v;
    ...
    if (j->offloaded) {
        ...
        /* Same re-arm as file_job_done: syncAsync overlapping another
         * offload must not strand its release on an unhooked reap. */
        if (!file_async_hooked &&
            dyn_net_on_drain(file_async_reap, &file_async_hooked) >= 0) {
            file_async_hooked = 1;
            file_async_ctx = ctx;
                                      ...
                                      int argc, JSValueConst *argv)
{
    ...
    JS_AddShutdownSweep(JS_GetRuntime(ctx), fsync_job_sweep, j);
    ...
    j->offloaded = 1;
    file_async_inflight++;              /* the completion brings it back down */
...
/* Strict open + whole-file slurp behind readFile() and readBytes(). One
 * implementation, one DYN_MAX_INPUT bound, one error shape; `op` only names
 * the entry point in the messages. Returns 0 on success (*src owned, release
 * with dyn_iobuf_free), -1 with the exception pending. */
                              ...
                              const char *op, dyn_iobuf_t *src)
{
       ...
       and would follow a symlink component. */
    {
        ...
        if (rfd < 0) {
            int e = errno;
            dyn_fs_throw(ctx, e, op, path); /* exception now pending */
            dyn_path_unborrow(ctx, path);
            return -1;
        }
        /* dyn_io_slurp_fd owns (closes) rfd on every path. */
        if (dyn_io_slurp_fd(rfd, src, 0, DYN_MAX_INPUT) < 0) {
            ...
            else /* read-leg failures (EISDIR on a directory, EACCES, ...)
                    carry .code/.errno like the open leg -- a bare
                    InternalError made callers branch blind. */
...
/* readFile(path[, {bytes}]) -> string, or a Uint8Array with {bytes: true}.
 * The bytes flag mirrors readFileAsync's: a STRING read decodes UTF-8, so it
 * is lossy for binary data (invalid bytes become U+FFFD and cannot be
 * recovered); the bytes path wraps the raw slurp in a fresh Uint8Array with
 * nothing decoded. */
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (argc > 1 && JS_IsObject(argv[1])) {
        /* Coerce the options BEFORE touching the file system: JS_GetPropertyStr
         * can run a getter, which must not observe a half-opened read. */
        ...
        b = JS_ToBool(ctx, v); /* -1 = the property get threw (getter ran user JS): refuse */
        JS_FreeValue(ctx, v);
        if (b < 0)
            return JS_EXCEPTION;
        as_bytes = b;
...
/* readBytes(path) -> Uint8Array: the free-function alias of
 * readFile(path, {bytes: true}), named for what it returns. Same strict open,
 * same slurp, same cap; identical to File.readBytes, which forwards here. */
                                   ...
                                   int argc, JSValueConst *argv)
{
    ...
    if (argc > 2 && JS_IsObject(argv[2])) {
        ...
        if (dyn_opts_strict(ctx, argv[2], keys, 1)) {
            dyn_path_unborrow(ctx, path);
            if (str)
                JS_FreeCString(ctx, str); /* the payload is already owned */
            return JS_EXCEPTION;
        }
...
/* dyn_fs_errno_code / dyn_fs_throw now live directly above dyn_stat_strict
 * (they are used by FileReader/FileWriter/readFile/writeFile, which are
 * defined BEFORE this point). */

/* The same error WITHOUT the throw, for the async completions: they reject a
   promise rather than unwind, so they need the object itself. The message
   stays the bare strerror (what the completions have always put there) and
   `path`, when given, is its own property as before; `code` and `op` are the
   fields this adds so an async failure carries what a sync dyn_fs_throw
   carries. Returns JS_EXCEPTION on OOM (with the exception pending). */
static JSValue dyn_fs_error_obj(JSContext *ctx, int e, const char *op,
                                const char *path)
{
    JSValue err;
    ...
    if (code)
        JS_DefinePropertyValueStr(ctx, err, "code", JS_NewString(ctx, code),
                                  JS_PROP_C_W_E);
    if (op)
        JS_DefinePropertyValueStr(ctx, err, "op", JS_NewString(ctx, op),
                                  JS_PROP_C_W_E);
    if (path)
        JS_DefinePropertyValueStr(ctx, err, "path", JS_NewString(ctx, path),
                                  JS_PROP_C_W_E);
    return err;
}
 ...
 *
 * DELIBERATE: every errno maps to false, so EACCES (or ELOOP, or EIO) is
 * indistinguishable from ENOENT. An exists() that threw on EACCES would make
 * "does this path exist?" unusable inside permission boundaries a caller
 * cannot inspect, and the affirmative answer is what callers branch on --
 * callers that must tell missing from unreadable have stat(), which throws
 * with errno/code. Keep the coarse contract here. */
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        /* P8 (audit): fdopendir inside rmrf_children TAKES the fd (closedir
           closes it), so the identity check needs a private dup taken BEFORE
           the walk. If the NAME was swapped for another directory mid-walk
           (TOCTOU), stat(path) below names the impostor while pin_fd names
           the directory we actually emptied — refuse rather than remove it. */
        {
            int pin_fd = dup(fd);
            if (pin_fd < 0) {
                close(fd);
...
static int dyn_glob_match(const char *p, const char *s)
{
    ...
    backtrack:
        ...
        /* The resume point is where the star's tail match currently starts.
           If it already sits at the terminator, the star has consumed the
           whole subject and there is nothing left to swallow: advancing
           star_s here walked past the NUL and read out of bounds (pattern
           "*b" vs subject "a" reached it deterministically). */
        if (*star_s == '\0')
            return 0;
                            ...
                            JSValueConst *argv)
{
    ...
    if (argc > 1 && JS_IsObject(argv[1])) {
        ...
        JSValue v;
        if (dyn_opts_strict(ctx, argv[1], keys, 1)) {
            JS_FreeCString(ctx, pattern); /* every exit owns its borrow */
            return JS_EXCEPTION;
        }
                                      ...
                                      int argc, JSValueConst *argv)
{
    ...
    /* same one-time boundary resolve as makeTempDir: the system temp dir
       sits under /var → /private/var on macOS and every Path operation is
       strict (O_NOFOLLOW). */
    {
        char rp[PATH_MAX];
        if (realpath(tpl, rp)) {
...
#endif
/* The copy gate reads the SAME budget off the same clock: a copy is a read
 * and a write back to back, and the kernel-path copy of a page-cache-resident
 * file costs about what reading it does. Defaulting to the read threshold
 * keeps one 1 MiB loop-block budget across the family instead of three that
 * could drift. */
#ifndef DYN_FILE_ASYNC_COPY_MIN
#define DYN_FILE_ASYNC_COPY_MIN   DYN_FILE_ASYNC_READ_MIN
#endif
...
static void file_async_reap(void *unused)
{
    ...
    /* `inflight` gates the unhook: a pending-only check unhooks while a
     * SECOND offloaded job is still running, stranding that job's release
     * with no hook ever to run it -- a stuck reactor ref, i.e. a loop that
     * never exits. */
    if (file_async_pending_release == 0 && file_async_inflight == 0 &&
        file_async_hooked) {
        file_async_hooked = 0;
...
typedef struct {
    ...
    char *path2;                /* copy: the destination, same ownership rule */
    uint8_t *buf;               /* write: payload (owned). read: unused. */
    dyn_iobuf_t src;            /* read: the slurped file, freed on the loop */
    int have_src;
    size_t len;
    int op, append, as_bytes;
    int overwrite;              /* copy: O_TRUNC instead of O_EXCL, as sync */
    int is_dir;                 /* copy: the source was a directory (a
                                   TypeError case, not an errno case) */
    int same_file;              /* copy: overwrite onto the same inode (a
                                   TypeError case, mirrored from sync) */
    int err_at;                 /* copy: which path failed -- 0 source, 1 dest */
...
/* Defined in dyna-filecopy.inc.c below: the async copy runs the SAME
   kernel-first primitive as the sync one, so there is one copy implementation
   and the two arms cannot disagree about bytes or refusals. */
...
static void file_job_work(void *arg)
{
    ...
    if (j->op == FILE_OP_READ) {
        ...
        /* Strict open FIRST, then slurp from the fd -- dyn_io_slurp resolves
           by name and would follow a symlink (audit 13.7.3), same as the
           sync readFile. */
        {
            int rfd = dyn_open_strict(j->path, O_RDONLY | O_CLOEXEC, 0);
            if (rfd < 0 || dyn_io_slurp_fd(rfd, &j->src, 0, DYN_MAX_INPUT) < 0) {
    ...
    } else if (j->op == FILE_OP_WRITE) {
        int flags = O_WRONLY | O_CREAT | (j->append ? O_APPEND : O_TRUNC);
        int fd = dyn_open_strict(j->path, flags, 0644);
        size_t off = 0;
        ...
        close(fd);
    } else if (j->op == FILE_OP_COPY) {
        /* The sync copyFile sequence (dyna-filecopy.inc.c), VERBATIM: the
         * strict opens, the directory refusal, the O_EXCL/O_TRUNC choice and
         * the source's mode bits are what the sync path does, and an async
         * variant that differed in any of them would make the promise settle
         * differently from the call it mirrors. The only error HANDLING that
         * differs is where the failure is reported: here it is captured and
         * the promise rejects on the loop. errno is captured before any close
         * so a failure is never relabelled by a succeeding close. */
        int in = -1, out = -1, flags;
        struct stat st;
        in = dyn_open_strict(j->path, O_RDONLY | O_CLOEXEC, 0);
        if (in < 0) { j->err = errno ? errno : EIO; j->err_at = 0; return; }
        if (fstat(in, &st) != 0) {
            j->err = errno ? errno : EIO; j->err_at = 0; goto copy_done;
        }
        if (S_ISDIR(st.st_mode)) { j->is_dir = 1; j->err_at = 0; goto copy_done; }
        /* Sync resolves with the SOURCE size seen before the destination is
         * even opened. The overwrite arm opens WITHOUT O_TRUNC and only
         * truncates after an fstat fstat same-inode refusal -- mirroring the
         * sync fix that keeps copyFile(p, p, {overwrite:true}) from emptying
         * the file it was asked to copy. */
        j->len = (size_t)st.st_size;
        flags = O_WRONLY | O_CREAT | O_CLOEXEC | (j->overwrite ? 0 : O_EXCL);
        out = dyn_open_strict(j->path2, flags, st.st_mode & 0777);
        if (out < 0) { j->err = errno ? errno : EIO; j->err_at = 1; goto copy_done; }
        if (j->overwrite) {
            struct stat dst;
            if (fstat(out, &dst) == 0 && dst.st_dev == st.st_dev &&
                dst.st_ino == st.st_ino) {
                j->same_file = 1; j->err_at = 1; goto copy_done;
            }
            if (ftruncate(out, 0) != 0) {
                j->err = errno ? errno : EIO; j->err_at = 1; goto copy_done;
            }
        }
        if (dyn_copy_fds(in, out) != 0) {
            j->err = errno ? errno : EIO; j->err_at = 1;
        }
copy_done:
        if (in >= 0) close(in);
        if (out >= 0) close(out);
...
/* The file-job shutdown sweep (see file_park_fail): fails the parked
   read/write/copy at teardown. The job struct itself is left to its worker
   -- a completion that still lands does the bookkeeping and frees it. */
static void file_job_sweep(JSContext *ctx, JSRuntime *rt, void *opaque)
{
    file_job_t *j = (file_job_t *)opaque;
    const char *what = j->op == FILE_OP_READ ?
        "readFileAsync: aborted at engine shutdown" :
        j->op == FILE_OP_WRITE ?
        "writeFileAsync: aborted at engine shutdown" :
        "copyFileAsync: aborted at engine shutdown";
    file_park_fail(ctx, rt, &j->resolve, &j->reject, what);
}
...
static void file_job_done(void *arg)
{
    ...
    JS_RemoveShutdownSweep(JS_GetRuntime(ctx), file_job_sweep, j);
    if (JS_IsUndefined(j->resolve)) {
        /* the shutdown sweep already failed this park and released the
           pins: only the reactor bookkeeping is left */
        if (j->offloaded) {
    ...
    if (j->err || (j->op == FILE_OP_COPY && (j->is_dir || j->same_file))) {
        if (j->op == FILE_OP_COPY) {
            const char *at = j->err_at ? j->path2 : j->path;
            /* Reject with the SAME error the sync path throws: same type,
             * same message text, same errno/code fields. The throw helpers
             * leave the error in the pending-exception slot, so it is
             * detached with JS_GetException and handed to the rejection
             * instead of unwinding. The family's `op`/`path` fields go on
             * top, as dyn_fs_error_obj adds for the read/write arms. */
            if (j->is_dir)
                JS_ThrowTypeError(ctx, "copyFile: '%s' is a directory", at);
            else if (j->same_file)
                JS_ThrowTypeError(ctx,
                    "copyFile: '%s' and '%s' are the same file",
                    j->path, j->path2);
            else
                dyn_fs_throw(ctx, j->err, "copyFile", at);
            v = JS_GetException(ctx);
            if (!JS_IsException(v)) {
                JS_DefinePropertyValueStr(ctx, v, "op",
                                          JS_NewString(ctx, "copyFile"),
                                          JS_PROP_C_W_E);
                JS_DefinePropertyValueStr(ctx, v, "path",
                                          JS_NewString(ctx, at),
                                          JS_PROP_C_W_E);
            }
        } else {
            v = dyn_fs_error_obj(ctx, j->err,
                                 j->op == FILE_OP_READ ? "readFile" : "writeFile",
                                 j->path);
        }
        a1[0] = v;
        r = JS_Call(ctx, j->reject, JS_UNDEFINED, 1, a1);
        ...
        if (j->op == FILE_OP_WRITE || j->op == FILE_OP_COPY)
    ...
    if (j->offloaded) {
        ...
        /* The reap can have run and UNHOOKED between this job's submit and
         * its completion: two overlapping offloads whose completions drain
         * in separate ticks -- the first tick's reap releases its ref and
         * unhooks, and this release would be orphaned, with the loop held
         * open forever. Re-arm here, so every counted release has a hook to
         * run it; the release itself stays in the hook, which is the one
         * point the reactor is not iterating anything. (Exposed by the first
         * test that ran TWO offloaded copies at once; reads never split a
         * drain because they complete inside one wake.) */
        if (!file_async_hooked &&
            dyn_net_on_drain(file_async_reap, &file_async_hooked) >= 0) {
            file_async_hooked = 1;
            file_async_ctx = ctx;
...
/* {inline, offloaded, readMin, writeMin, copyMin} -- the counters and the
 * thresholds that produced them, so a test can assert WHICH arm ran. */
                                    ...
                                    int argc, JSValueConst *argv)
{
    ...
    JS_SetPropertyStr(ctx, o, "copyMin", JS_NewInt64(ctx, DYN_FILE_ASYNC_COPY_MIN));
    return o;
}
...
static JSValue file_async_submit(JSContext *ctx, file_job_t *j, int offload)
{
    ...
    j->resolve = funcs[0];
    j->reject = funcs[1];
    JS_AddShutdownSweep(JS_GetRuntime(ctx), file_job_sweep, j);
                                        ...
                                        int argc, JSValueConst *argv)
{
    ...
    if (argc > 1 && JS_IsObject(argv[1])) {
        ...
        int b;
        if (dyn_opts_strict(ctx, argv[1], keys, 1)) {
            file_job_free(j); /* j->path is already set: free the whole job */
            return JS_EXCEPTION;
        }
        v = JS_GetPropertyStr(ctx, argv[1], "bytes");
        b = JS_ToBool(ctx, v); /* -1 = the property get threw (getter ran user JS): refuse */
...
/* ==================================================================== *
 *  platform dirs (plan 3.8): dataDir/configDir/cacheDir + Site variants *
 *                                                                       *
 *  XDG on Linux, ~/Library on macOS, %LOCALAPPDATA% on Windows.         *
 *  Lookup NEVER creates the directory (XDG spec). Env override wins     *
 *  over the default; a relative env value is invalid and ignored.       *
 * ==================================================================== */

/* kind: 0 data, 1 config, 2 cache; site: 1 = system-wide. Returns a
 * malloc'd NUL-terminated string, or NULL on OOM. */
static char *dyn_pdir_base(int kind, int site)
{
    ...
    }
    {
        ...
        if (home[strlen(home) - 1] == '/')
            snprintf(buf, sizeof buf, "%s%s", home, s[kind]);
        else
            snprintf(buf, sizeof buf, "%s/%s", home, s[kind]);
        return strdup(buf);
    }
    ...
    if (site) {
        switch (kind) {
        case 0: {   /* first entry of $XDG_DATA_DIRS, else /usr/local/share */
        ...
        case 1: {   /* first entry of $XDG_CONFIG_DIRS, else /etc/xdg */
        ...
        default:    /* no XDG_CACHE_DIRS; platformdirs uses /var/cache */
    ...
    }
    {
        ...
        case 0:
            ...
            if (e && *e && e[0] == '/')   /* relative must be ignored (spec) */
                return strdup(e);
            if (home[strlen(home) - 1] == '/')
...
/* magic: kind | site<<2. Optional `app` is a single path segment appended
 * to the base; it is validated so a hostile name cannot escape the dir. */
                            ...
                            JSValueConst *argv, int magic)
{
    ...
    if (argc > 0 && !JS_IsUndefined(argv[0])) {
        ...
        if (!app)
            return JS_EXCEPTION;
        /* a single path segment: no '/', '\\', NUL, not "." or ".." */
        if (app_len == 0 || strchr(app, '/') || strchr(app, '\\') ||
            strlen(app) != app_len || strcmp(app, ".") == 0 ||
            strcmp(app, "..") == 0) {
 ...
 *  FileLock (plan 3.9)                                                  *
 *                                                                       *
 *  flock(2) on Linux+macOS, LockFileEx on Windows, and an O_CREAT|O_EXCL*
 *  lock file as fallback for filesystems without flock.                *
 *                                                                       *
 *  flock locks belong to the OPEN FILE DESCRIPTION, not the process:    *
 *  two FileLock objects on one path conflict even in ONE process, and   *
 *  a fork()ed child INHERITS the fd, so it holds the lock until EVERY   *
 *  copy is closed. Stale locks are never auto-broken: a dead holder's   *
 *  flock dies with its fd, and an O_EXCL lock file left behind stays    *
 *  until a human removes it.                                            *
 * ==================================================================== */

typedef struct {
    int fd;           /* the lock fd; -1 once released */
    int excl;         /* 1 when the O_EXCL fallback is in use */
    char *lockfile;   /* malloc'd "<path>.lock" path in excl mode */
} dyn_flock_t;
...
/* One non-blocking acquisition attempt. Returns 0 (held), 1 (contended),
 * or -1 (error, errno set). */
static int dyn_flock_try(dyn_flock_t *l, const char *path)
{
...
#else
    if (l->excl) {
        /* O_EXCL fallback: the lock IS the file's existence */
    ...
    if (l->fd < 0) {
        ...
        if (l->fd < 0 && errno == EACCES) {
            /* A read-only file cannot be opened O_RDWR, but flock(2) does not
             * require a writable fd -- the lock belongs to the open file
             * description, not the access mode. Retry read-only, without
             * O_CREAT (the file exists; we just cannot write it). */
    ...
    if (errno == EOPNOTSUPP || errno == ENOLCK || errno == ENOSYS) {
        /* flock unsupported on this filesystem: switch to O_EXCL mode */
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    /* coerce options BEFORE resolving the handle (repo rule). retry/retryMs
       must be NUMBERS: JS_ToInt64 would coerce a string, and a typo'd "x"
       silently reading as 0 is the atoi bug class. */
    if (argc > 1 && JS_IsObject(argv[1])) {
    ...
    /* path: Path or string (constructor convention) */
                                   ...
                                   int argc, JSValueConst *argv)
{
    ...
    /* release no matter what the callback did -- but at most ONCE: fn may
     * have closed the lock itself (directly or via any path that reaches
     * close()), and dyn_res_release already ran the dispose then. Disposing
     * the captured native a second time was a double free: a probe that
     * called close() inside fn aborted with heap corruption. */
    if (!box->closed)
        dyn_flock_dispose(l);
    dyn_res_mark_closed(JS_GetRuntime(ctx), this_val, dyn_flock_class_id);
    return ret;   /* fn's value, or JS_EXCEPTION if fn threw */
}
...
static const JSCFunctionListEntry dyn_file_funcs[] = {
    ...
    JS_CFUNC_DEF("copyFileAsync", 2, dyn_fs_copy_file_async),
                           ...
                           JSValueConst *argv)
{
    ...
    /* Proto FIRST (OrdinaryCreateFromConstructor via dyn_ctor_proto): a
     * throwing "prototype" getter propagates before anything is allocated. */
...
/* readBytes: the binary half, forwarded to the free readBytes. A dedicated
 * byte read is not a style preference -- readText builds a JS STRING, so any
 * byte that is not valid UTF-8 becomes U+FFFD and cannot be recovered.
 * Measured: writing 1,2,255,0,3 and reading it back through the string path
 * returned SEVEN bytes (1,2,239,191,189,0,3), silently corrupting the
 * payload. That is defect D-1's shape all over again, in the opposite
 * direction. */
                              ...
                              JSValueConst *argv)
{
    ...
    /* delegate to the single hardened copy primitive: O_EXCL by default,
       mode from source, kernel clone where available, no in-memory whole-file
       string. {overwrite: true} relaxes the refusal, exactly as for the free
 copyFile -- parity was the point: the handle form used to be
       strictly weaker than the free form for no reason. */
    a[0] = f->path;
    a[1] = argv[0];
    ...
    JSValueConst one[1]; one[0] = argv[0];
...
/* ----: Glob.match(path, pattern) ------------------------------------
 * The one-shot spelling of `new Glob(pattern).matches(path)` for callers who
 * have the two strings and no instance: a pure LEXICAL test over the same
 * dyn_glob_match the class wraps (one implementation, two spellings -- the
 * differential in tests/test_glob_match.js holds them identical). Same
 * reading as `matches`: whole-path string match, '/' an ordinary byte to the
 * wildcards, NO leading-dot rule. `path` is a Path or a plain string;
...
static int dyn_file_init_module(JSContext *ctx, JSModuleDef *m)
{
    ...
    if (dyn_register_plain_class(ctx, m, &dyn_gl_class_id, &dyn_gl_class,
                                 ...
                                 dyn_gl_ctor, "Glob") < 0)
        return -1;
    {   /*Glob.match(path, pattern) hangs off the CONSTRUCTOR --
           same recovery the shared registrar itself uses. */
