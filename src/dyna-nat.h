 ...
 *
 * `new_target` is the new.target the CALLER's [[Construct]] runs with: a
 * constructor passes its own new_target argument through, which is what makes
 * `class R extends C {}` produce instances of R (the prototype comes from
 * Get(new_target, "prototype"), OrdinaryCreateFromConstructor -- a non-object
 * result falls back to the class's own proto, a throwing getter propagates).
 * Any wrapper created OUTSIDE a construction -- method and factory results --
 * passes JS_UNDEFINED and always gets the class's own prototype, with no
 * property lookup at all. */
 ...
 *
 * Modules allocate their payloads with libc malloc, which the ENGINE's
 * JS_SetMemoryLimit and gc thresholds cannot see: a module holding 500 MB of
 * native data is invisible to a configured 256 MB JS limit. This is the
 * accounting layer for that memory: the same malloc underneath, wrapped with
 * a process-wide live-bytes ledger and an optional cap.
 *
 * Scope, stated honestly: the ledger sees the allocations routed through it --
 * the DynResource box every dyna:* resource carries (dyn_res_wrap, tracked
 * alongside its engine allocation) and the module TUs converted to these
 * entry points (dyna:structures end-to-end). Payload buffers in modules not
 * yet converted (dyna:dataframe, dyna:ml, dyna:http, ...) are NOT counted;
 * sys.memoryUsage() documents this. Adopting a site is mechanical: swap
 * malloc/calloc/realloc/strdup/free for the dyn_nat_* form -- sizes, semantics
 * and return conventions are identical (NULL means "refused or failed", which
 * every converted call site must answer with JS_ThrowOutOfMemory).
 *
 * dyn_nat_free may be called with NULL. Thread-safe: the ledger and cap are
 * C11 atomics (relaxed ordering) because modules like the HTTP server
 * allocate from worker threads concurrently. */
void *dyn_nat_malloc(size_t size);
void *dyn_nat_calloc(size_t nmemb, size_t size);
/* Like realloc(3): NULL ptr allocates; on refusal/failure returns NULL and
...
/* Ledger-only tracking for native bytes held by the ENGINE allocator -- the
 * DynResource box, which js_mallocz allocates (so JS_SetMemoryLimit keeps
 * seeing it) but which is module-native memory, not JS heap. dyn_nat_track
 * reserves the bytes against the cap (0, or -1 when over: the caller refuses
 * and reports out-of-memory); dyn_nat_untrack releases them. */
int dyn_nat_track(size_t size);
void dyn_nat_untrack(size_t size);

/* Live module-native bytes on the ledger (net of frees, not churn). */
uint64_t dyn_nat_bytes(void);
/* The module-native cap in bytes: 0 (the default) is uncapped; a positive
 * value makes dyn_nat_malloc/calloc/realloc/strdup and dyn_nat_track refuse
 * past it. Setting a limit NEVER frees anything -- it only gates new
 * allocations. */
 ...
 * `new_target` means exactly what it does for dyn_res_wrap: constructors pass
 * it through (subclasses of the class get the subclass's prototype); creators
 * that are not under a [[Construct]] pass JS_UNDEFINED. */
JSValue dyn_plain_wrap(JSContext *ctx, JSValueConst new_target,
                       JSClassID class_id, void *native,
                       DynDisposeFunc dispose);

/* The prototype an object of `class_id` must get under [[Construct]] with
 * new.target = `new_target` (OrdinaryCreateFromConstructor: Get(new_target,
 * "prototype") when that is an object, else the class's own registered proto;
 * a throwing "prototype" getter propagates). For classes that fit the
 * dyn_res_wrap / dyn_plain_wrap shapes this is an implementation detail of
 * the wrap helpers; classes whose object shape keeps them outside those
 * helpers (custom finalizers, opaques that are not a single malloc'd native)
 * call it DIRECTLY in their constructor and then JS_NewObjectProtoClass with
 * the result -- same contract as the sweep in commit 026c2045: constructors
 * pass their new_target through, creators not under a [[Construct]] pass
 * JS_UNDEFINED (pure class-proto fallback, no property lookup). Resolve it
 * BEFORE allocating any native state: the throwing-getter path must not have
 * anything to dispose. Returns an owned value. */
...
/* The form a FINALIZER must call: during JS_FreeRuntime the contexts are
   freed by the cycle sweep while the runtime is not, so a shutdown teardown
 must not route through a stored ctx (ASan heap-use-after-free). */
...
/* The boundary-only form: same table, same (fn, udata) dedupe, but NEVER arms
   the backend's periodic wakeup and never fails with -2 -- for hooks that need
   a pass boundary, not a clock (the UDP graveyard flush; the shutdown sweeps
   cover a process that goes quiet before the next pass). */
...
/* fix3: swallowed net-handler throws. Every exception a TCP/UDP event handler
 * throws is counted (the product doctrine keeps the event loop alive);
 * dyn_net_handler_throw_count backs net.swallowedHandlerThrows() and the -d
 * dump, dyn_net_set_debug arms the once-per-kind stderr diagnostic. */
...
/* Canonical CBOR + xxh64 fingerprint of a JS value (from dyna:vserialize).
 * 0 on success; -1 with an exception pending. */
 ...
 * Declared here historically, but only dyna:uring itself calls it today --
 * dyna:file has its own reader. */
...
int js_nat_init_stream(JSContext *ctx); /* dyna:stream pull-based byte streams */
#endif
...
int js_nat_init_async(JSContext *ctx); /* dyna:async: embedded-JS concurrency utils */
int js_nat_init_scrape(JSContext *ctx); /* dyna:scrape robots/politeness policy */
#endif
#ifdef CONFIG_NATIVE_MODULE_BENCH
int js_nat_init_bench(JSContext *ctx); /* dyna:bench: native bench harness */
#endif
#ifdef CONFIG_NATIVE_MODULE_OAUTH2
int js_nat_init_oauth2(JSContext *ctx); /* dyna:oauth2 OAuth 2.0 + PKCE + Bearer helpers (optional, no JWT dep) */
#endif
