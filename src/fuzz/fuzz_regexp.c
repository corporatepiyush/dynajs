...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // reset per input (audit 13.4.2): lre_check_timeout() trips after ~100
    // interrupt calls, so an unreset counter makes the executor permanently
    // dead after one slow input. Same idiom as fuzz_regexp_compile.c.
    nbinterrupts = 0;
