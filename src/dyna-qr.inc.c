 ...
 * Returns 0, -1 (payload does not fit), or -2 (out of memory). */
static int qr_build(const uint8_t *in, size_t n, int ecl, int want_ver,
                    int want_mask, qr_grid_t *out)
{
    uint8_t cw[QR_MAX_CW];
    /* Each grid is 31 KB at version 40: three of them plus the codeword
     * buffers are ~100 KB, too much to nest inside whatever native frame
     * called us (see the comment in dyn_qr_encode). They live on the heap. */
    qr_grid_t *base = malloc(sizeof *base);
    qr_grid_t *best = malloc(sizeof *best);
    qr_grid_t *cur = malloc(sizeof *cur);
    int ver, ncw, mask, have_best = 0, rc = -2;
    long bestscore = 0;

    if (!base || !best || !cur)
        goto done;
    ver = qr_pick_version(n, ecl, want_ver);
    if (ver < 0) { rc = -1; goto done; }
    ...
    for (mask = 0; mask < 8; mask++) {
        ...
        *cur = *base;
        ...
        sc = qr_penalty(cur);
        /* keep the FIRST strictly-lower score: with mask pinning every scored
         * copy wins, otherwise ties keep the earlier (spec: lowest, ties
         * unspecified but determinism demands a stable pick). */
        if (!have_best || sc < bestscore) {
            bestscore = sc;
            *best = *cur;
            have_best = 1;
        }
                             ...
                             int argc, JSValueConst *argv, int magic)
{
    ...
    {
        int brc = qr_build((const uint8_t *)text, n, ecl, ver, mask, q);
        if (brc == -2) {
            JS_FreeCString(ctx, text);
            free(q);
            return JS_ThrowOutOfMemory(ctx);
