...
static void scalar_softmax_impl(float *restrict out,
  ...
  if (do_log) {
    /* Analytic log-sum-exp: out = x - (max + log(sum)). Every ISA kernel (NEON/SSE4.2/
     * AVX2/AVX-512/SVE, head and tail) uses exactly this form; the old logf(e_i)+log_inv
     * composition saturated to -Inf once x-max < ~-88 (fast_exp underflows to 0), so the
     * scalar reference disagreed with every accelerated path by an -Inf-vs-finite class.
     * One rounding order everywhere: offset = maxv + log_s (one add), then one subtract.
     * NaN propagates: fast_exp(NaN) = NaN poisons sum, inv_sum, off and every output. */
    float log_s = logf(inv_sum);
    float off = maxv + log_s;
    for (size_t i = 0; i < n; i++)
      out[i] = in[i] - off;
  } else {
    for (size_t i = 0; i < n; i++)
...
void simd_scalar_gemv(float *restrict y, const float *restrict a,
  ...
  for (size_t i = 0; i < m; i++) {
    ...
    /* BLAS beta=0 semantics: y may hold NaN/Inf garbage and must not be read
     * (0 * NaN = NaN would otherwise poison the overwrite). */
...
void simd_scalar_gemv_t(float *restrict y,
  ...
  if (beta == 0.0f) {
    for (size_t j = 0; j < n; j++)
      y[j] = 0.0f; /* beta=0: never read y (NaN-poison-safe), just zero it */
  } else {
    for (size_t j = 0; j < n; j++)
      y[j] *= beta;
  }
...
void simd_scalar_gemm(float *restrict c, const float *restrict a,
  ...
  const size_t T = 32;
  /* BLAS beta=0 semantics: C := alpha*A*B, prior c never read (0*NaN = NaN). */
  if (beta == 0.0f) {
    for (size_t i = 0; i < m; i++)
      for (size_t j = 0; j < n; j++)
        c[i * n + j] = 0.0f;
...
static size_t simd_scalar_find_first_of_u16(const uint16_t *restrict p,
                                             ...
                                             size_t setlen) {
  /* Wide-string membership with a SMALL set (the anchored-scan terminators
   * are four): linear set scan per unit, no 64K table. */
...
/* Decode table, COMPILE-TIME. It used to be memset and refilled on every
 * call -- 256 bytes cleared plus 64 stores before decoding a single
 * character, which for a short input is the entire cost. A static const
 * table also has no initialiser to race on. */
...
/* Portable membership scan: one shift, one index, one bit test per byte. */
