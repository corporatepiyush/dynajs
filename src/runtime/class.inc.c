...
{
    if (c < 0x100) {
        /* single latin1 characters are immutable and are requested in bulk
           by split(""), the string iterator, charAt and string index reads;
           serve them from the per-runtime cache instead of allocating a
           fresh JSString each time. The cache itself holds one reference,
           so while any JSValue refers to the string its ref_count is >= 2
           and in-place mutation (JS_ConcatStringInPlace requires
           ref_count == 1) can never apply to it. */
        JSRuntime *rt = ctx->rt;
        ...
        if (likely(str != NULL)) {
            ...
            str->u.str8[0] = c;
            str->u.str8[1] = '\0';
            rt->latin1_char_cache[c] = str; /* the creation reference is
                                               kept by the cache */
            js_rc(str)->ref_count++;        /* the returned reference */
        }
        return JS_MKPTR(JS_TAG_STRING, str);
    } else {
        uint16_t ch16 = c;
...
/* Minimum substring length that becomes a lazy slice instead of an eager
   copy. Below it the copy is cheaper than a slice header (and short
   substrings — split/match results — are usually consumed repeatedly).
   DYNA_SLICE_MIN_LEN overrides for testing/exposure runs. Read once. */
static int js_slice_min_len(void)
{
    static int cached = -1;
    if (unlikely(cached < 0)) {
        const char *s = getenv("DYNA_SLICE_MIN_LEN");
        cached = JS_SLICE_MIN_LEN_DEFAULT;
        if (s && *s) {
            long v = strtol(s, NULL, 10);
            if (v >= 1 && v <= JS_STRING_LEN_MAX)
                cached = (int)v;
        }
    }
    return cached;
...
/* Allocate a slice value referencing [start, end) of p's characters.
   p may be a value string, an atom (string literals as values ARE the
   interned JSString, atom_type != 0), or a slice. The slice keeps the flat
   root (or the atom itself) alive with one reference; parents are always
   flat and depth is exactly 1.

   Atom parents and the intern table: the table holds WEAK entries —
   __JS_FreeAtom unlinks the entry (JS_FreeAtomStruct) the instant its
   refcount hits zero — so a slice must hold a STRONG reference to keep the
   atom's characters alive; dropping it unlinks the entry normally. The one
   exception is CONSTANT atoms (index < JS_ATOM_END, i.e. the predefined
   ones): JS_DupAtom is a no-op for them and JS_FreeAtom never decrements
   them, so the slice mirrors that and takes NO reference; it records the
   fact in its otherwise-unused hash_next bit 0 ("parent ref taken") so the
   free path (js_free_string / __JS_FreeValueRT) knows whether to
   decrement. */
static JSValue js_new_string_slice(JSContext *ctx, JSString *p, int start, int end)
{
    ...
    if (parent->atom_type) {
        /* never JS_AtomIsConst-tagged-int here: parents are JSStrings */
    ...
    str->atom_type = 0;
    str->hash = 0;
    str->hash_next = parent_holds_ref; /* bit 0: free path must decrement */
    sl->offset = offset;
    sl->parent = parent; /* the slice keeps the parent alive (see above) */
#ifdef DUMP_LEAKS
...
static JSValue js_sub_string(JSContext *ctx, JSString *p, int start, int end)
{
    ...
    if (len >= js_slice_min_len()) {
        /* long substring: make a lazy slice over the flat root
           (js_new_string_slice resolves slice parents and, since wave B,
           accepts atom parents by holding a strong reference on the
           interned string — the weak intern table then keeps the entry as
           long as the slice lives). Wide parents stay wide — the old
           O(len) narrowability scan + copy is exactly what made big
           substrings quadratic-ish; narrowing still happens in the copy
           paths below whenever they run. */
...
/* return pointer into a JSString with a live ref_count (for a sliced
   string the pointer is interior to the flat parent, which the slice
   value keeps alive; the same borrow contract applies: the caller must
   keep 'val' reachable while it uses the bytes and must not free them) */
...
const char *JS_ToCStringLen2(JSContext *ctx, size_t *plen, JSValueConst val1, BOOL cesu8)
{
    ...
    len = str->len;
    if (!str->is_wide_char) {
        /* JS_FreeCString recovers the owning JSString with
           container_of(ptr, JSString, u), so the zero-copy fast path must
           not hand out a pointer interior to the slice's parent. */
...
/* TRUE if 'p' is narrow or its units are all latin1 (<= 0xFF). Width
 * normalization is hash-consistent: hash_string16 and hash_string8 share
 * the h*263+c recurrence, so all-latin1 content hashes identically at
 * either width (the same invariant __JS_NewAtom's slice narrowing relies
 * on). SWAR probe -- four units per test via a 64-bit word, memcpy rather
 * than a pointer cast because a cast is UB and traps on strict-alignment
 * targets (same pattern as str8_is_ascii in string.inc.c): the scan must
 * stay near memcpy speed or the narrow copy it buys costs more than it
 * saves. */
...
/* Copy p's characters into a NARROW dst (p must be narrowable). A SWAR
 * 4-units-per-word packer measured 2x SLOWER than this plain loop here
 * (store-forwarding stalls on the 8-byte load + 4-byte store), so the
 * simple per-unit loop stays; the compiler vectorizes it to NEON on
 * arm64. Measured, not assumed -- see bench b4_wide_narrow.js. */
                                ...
                                const JSString *p1, const JSString *p2)
{
    ...
    /* Narrow-at-flatten (wave B T4): a wide SLICE whose content is all
     * latin1 would materialize a 2x-wide flat result that no read needs.
     * The narrowing scan runs only when a wide slice participates, so
     * genuinely-wide flat concats (the common case) keep their exact old
     * layout and cost. */
    alloc_wide = is_wide_char;
    if (is_wide_char &&
        ((p1->is_slice && p1->is_wide_char) ||
         (p2->is_slice && p2->is_wide_char))) {
...
static BOOL JS_ConcatStringInPlace(JSContext *ctx, JSString *p1, JSValueConst op2) {
    if (JS_VALUE_GET_TAG(op2) == JS_TAG_STRING) {
        ...
        if (p2->len == 0)
            return TRUE;
        /* never append in place into a slice: that would write past
           slice->len into the parent's characters */
        if (p1->is_slice)
            return FALSE;
