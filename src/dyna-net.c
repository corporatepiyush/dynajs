...
/* The RUNTIME, not a context: dyn_net_reactor_release_rt runs from finalizers
   during JS_FreeRuntime, where every context of the runtime is already freed
   but rt is not. net_reactor_free only needs the rt for the thread-state
   bookkeeping (js_std_set_io_reactor_rt), and a dead ctx here was exactly
 the ASan abort. */
...
/* The DIRECT-WAIT twin of dyn_net_drain: instead of poll()ing the reactor's
 * backend fd and then draining, we block IN the backend (kevent/epoll_wait
 * with the JS loop's own timeout) and dispatch what it reports -- one kernel
 * entry where the drain path needs two. Hook semantics are identical to
 * dyn_net_drain's, so a caller cannot tell which shape served its wakeup:
 * the same release-deferral guard, the same post-pass. The XNU listen-sweep
 * and the disk-pool wake fd are registered IN the backend, so both still
 * arrive here (dyn_aio_run performs the same post-poll sweep as the drain). */
...
{
    int i;
    /* Registration is set MEMBERSHIP, not a counter: the reactor is torn down
       (net_reactor_free) with every net resource closed, which resets the
       table, and a registrant that re-arms on its next event must register
       again -- against a stale "already hooked" latch its hook would be lost
       for the life of the NEW reactor. Dedupe by (fn, udata) identity makes
       every re-arm a no-op while the hook is still in the table, so callers
       can register unconditionally instead of tracking a latch of their own. */
    for (i = 0; i < net_n_hooks; i++)
...
/* The no-clock form, for hooks that are NOT time-driven: they need a pass
 * BOUNDARY (the point after dyn_aio_drain, when every completion loop has
 * returned), never a wakeup. dyn_net_on_drain arms the backend's 250ms tick
 * for the FIRST registrant so quiet loops still run their timeouts; a
 * boundary-only hook must not pin that permanent wakeup on the reactor -- a
 * graveyard flush runs when traffic runs a pass anyway, and teardown covers
 * the process that goes quiet. Registration therefore never fails with -2:
 * a backend that cannot arm a clock loses nothing the hook needed. Same
 * table, same (fn, udata) dedupe, same reset when the reactor is freed --
 * re-arm at every event, exactly like the timed form. */
...
dyn_aio_t *dyn_net_reactor_acquire(JSContext *ctx)
{
    ...
    if (!net_aio) {
        ...
        js_std_set_io_reactor(ctx, dyn_aio_backend_fd(net_aio), dyn_net_drain,
                              net_aio);
        /* the direct wait is available from day one: a reactor-only loop
           (every dyna:net server) then blocks in ONE kernel entry */
...
/* rt-based core: the form a FINALIZER must call. A shutdown teardown runs
   after JS_FreeContext has freed every context but before JS_FreeRuntime is
   done, so going through the ctx form handed net_rt a freed pointer
 . Live-context callers keep the wrapper below; the acquire path
   still records the rt here. */
