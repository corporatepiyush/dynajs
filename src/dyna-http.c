                              ...
                              size_t outsz)
{
    ...
    if (plen == 0)
        return -1;
    /* RFC 9112 3: the request line is printable ASCII with single-SP
     * delimiters. The scan above stops at SP/CR/'?' but admitted every other
     * CTL -- notably a bare LF, which the proxy re-emitted VERBATIM into the
     * upstream request line ("GET /a\nHost: HTTP/1.1"): a request-smuggling
     * primitive against whatever sits behind us (CWE-444), and one that
     * defeats the proxy's re-serialisation defence. A malformed target is a
     * 400, not a 404. */
    for (q = 0; q < plen; q++) {
        unsigned char ch = (unsigned char)buf[start + q];
        if (ch < 0x21 || ch == 0x7f)
            return -1;
    }
    if (plen >= outsz)
...
/* One pass over the header block for the framing decision: the first
   Content-Length value, how many times it appears, whether any
   Transfer-Encoding is present, and how many Host lines there are (RFC 9110
   7.2: none or two+ on an HTTP/1.1 request is a 400). Replaces three separate
   header scans on the hot request path. Header lines are CRLF-preceded (the
   request line is not). */
typedef struct { const char *cl; size_t cl_len; int cl_count; int has_te; int host_count; } dyn_frame_t;

static void dyn_req_frame(const char *buf, size_t len, dyn_frame_t *f)
{
    size_t i;
    f->cl = NULL; f->cl_len = 0; f->cl_count = 0; f->has_te = 0; f->host_count = 0;
    for (i = 0; i + 2 < len; i++) {
        const char *n;
        size_t nlen;
        if (buf[i] != '\r' || buf[i + 1] != '\n')
        ...
        if (nlen > 15 && dyn_ci_eq(n, "content-length", 14) && n[14] == ':') {
            ...
            f->has_te = 1;
        } else if (nlen > 5 && dyn_ci_eq(n, "host", 4) && n[4] == ':') {
            f->host_count++;
        }
    }
}
...
#define HANDLE_DEADLINE() do { \
    ...
    for (;;) {
        ...
        }
        {
             ...
             * with more than one, MUST be answered 400. Both used to be
             * served (first-wins), letting one request read as two. */
            if (http11 && fr.host_count != 1) {
                dyn_http_respond(fd, 400, "text/plain", "Bad Request", 11, 0);
                return;
            }
        }
         ...
         * but the IDLE wait between requests is capped at 5 s, NOT the full
         * request budget. A keep-alive connection otherwise holds one thread-
         * pool worker for the whole 30 s deadline; a handful of idle browser
         * tabs could pin every worker and stall the server for everyone. */
        if (s->req_timeout_ms) {
            uint64_t now = dyn_timer_now_ms();
            uint64_t idle = DYN_HTTP_IDLE_TIMEOUT_MS_DEFAULT;
            uint64_t budget = (uint64_t)s->req_timeout_ms;
            if (idle > budget)
                idle = budget;
            deadline = now + idle;
        }
...
static void *dyn_http_acceptor_main(void *arg)
{
    ...
    while (!atomic_load_explicit(&s->stop_flag, memory_order_relaxed)) {
        ...
        if (s->q.count < DYN_HTTP_CONN_QUEUE_CAP && !s->q.shutdown) {
            ...
            pthread_mutex_unlock(&s->q.mutex);
            /* Answer before dropping (audit M10-06): a peer told nothing
               retries blind against a silent close. A 503 with
               Connection: close is the protocol's own "try later".
               MSG_DONTWAIT or the acceptor BLOCKS: the accepted fd has no
               O_NONBLOCK yet, this thread serves every accept, and one
               slow peer's full buffer would stall them all. Best-effort
               either way -- the close below is the actual shed. Skipped
               entirely when shutting down: teardown makes no traffic. */
            if (!s->q.shutdown) {
                static const char busy[] =
                    ...
                    "Connection: close\r\n"
                    "\r\n";
                ssize_t w = send(fd, busy, sizeof(busy) - 1,
...
/* Number of field lines in the header block [buf, buf+len): every CRLF that
   starts a non-empty line, one per well-formed field line (the request line is
   not CRLF-preceded and the terminator's CRLFCRLF starts an empty line). On an
   incomplete block it counts the lines received so far, which is all the cap
   on that path needs: an over-count refuses early, an under-count is caught by
   the complete-block check below. */
...
static void dyn_app_process(dyn_app_conn_t *c)
{
    ...
    for (;;) {
        ...
        if (head_len > DYN_APP_MAX_HEADER ||
            dyn_req_header_count(base, head_len) > DYN_APP_MAX_HEADER_COUNT) {
            dyn_app_send_err(c, 431, "{\"error\":\"header too large\"}");
            dyn_app_conn_close(c);
            return;
        }
        ...
        {
            int http11 = dyn_req_is_http11(base, head_len);
            dyn_frame_t fr;
            dyn_req_frame(base, head_len, &fr);
            if (fr.has_te) {
        ...
        {
            static const char *const dup_first_wins[] = {
                "cookie", "set-cookie",                /* special */
                "connection", "accept", "accept-encoding",
                "accept-language", "cache-control", "vary",
            };
            size_t a;
            for (a = 0; a + 2 < head_len; a++) {
                const char *na;
                size_t nalen = 0, k;
                if (base[a] != '\r' || base[a + 1] != '\n')
                    continue;
                na = base + a + 2;
                if (na[0] == '\r')
                    break;                     /* empty line: end of block */
                while (na + nalen < base + head_len && na[nalen] != ':')
                    nalen++;
                for (k = 0; k < countof(dup_first_wins); k++)
                    if (strlen(dup_first_wins[k]) == nalen &&
                        dyn_ci_eq(na, dup_first_wins[k], nalen))
                        break;
                if (k < countof(dup_first_wins))
                    continue;
                for (k = 0; k + 1 < a; k++) {  /* earlier CRLF-preceded lines */
                    const char *nb;
                    size_t nblen = 0;
                    if (base[k] != '\r' || base[k + 1] != '\n')
                        continue;
                    nb = base + k + 2;
                    if (nb[0] == '\r')
                        break;
                    while (nb + nblen < na && nb[nblen] != ':')
                        nblen++;
                    if (nblen == nalen && dyn_ci_eq(na, nb, nalen)) {
                        dyn_app_send_err(c, 400,
        ...
        if (!route) {
             ...
             * must name application/json, ignoring parameters (charset). */
            {
                ...
                size_t rctlen = 0;
                char typebuf[64];
                rct = dyn_req_header(base, head_len, "content-type", &rctlen);
                if (rct) {
...
static void wsc_hs_work(void *arg)
{
    ...
    if (hs->tls) {
        ...
        while (off < hs->req.len) {
            n = dyn_tls_write(hs->tls, hs->req.data + off,
                              hs->req.len - off);
            if (n <= 0) {
                hs->err = DYN_HTTP_ERR_SEND;
                return;
