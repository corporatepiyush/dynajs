...
/* ── Max ─────────────────────────────────────────────────────── */
/* NaN handling mirrors the scalar reference exactly: NaN never wins (MAXNM
 * returns the numeric operand), an all-NaN input yields the -FLT_MAX seed,
 * and a mixed input yields the largest finite value. */
static float simd_neon_max(const float *restrict x, size_t n) {
  ...
  if (n >= 4) { /* only seed a 4-wide vector when >=4 elements exist (OOB else) */
    float32x4_t vmax = vmaxnmq_f32(vld1q_f32(x), vdupq_n_f32(-FLT_MAX));
    for (i = 4; i + 4 <= n; i += 4)
      vmax = vmaxnmq_f32(vmax, vld1q_f32(&x[i]));
    result = vmaxnmvq_f32(vmax);
  } else {
    result = x[0];
    i = 1;
  }
...
static float simd_neon_min(const float *restrict x, size_t n) {
  ...
  float result;
  if (n >= 4) { /* only seed a 4-wide vector when >=4 elements exist (OOB else) */
    float32x4_t vmin = vminnmq_f32(vld1q_f32(x), vdupq_n_f32(FLT_MAX));
    for (i = 4; i + 4 <= n; i += 4)
      vmin = vminnmq_f32(vmin, vld1q_f32(&x[i]));
    result = vminnmvq_f32(vmin);
  } else {
    result = x[0];
    i = 1;
  }
...
static size_t simd_neon_argmax(const float *restrict x, size_t n) {
  ...
  size_t best_idx;
  if (n >= 4) { /* only seed a 4-wide vector when >=4 elements exist (OOB else) */
    /* Scalar parity: the reference seeds m = x[0], so a NaN FIRST element
     * poisons the whole scan (strict > never beats NaN) and it returns 0. */
    if (isnan(x[0]))
      return 0;
    ...
    for (i = 4; i + 4 <= n; i += 4) {
      ...
      /* Winner mask: vi > vmax, OR vmax is NaN and vi is finite (a NaN lane
       * value must not swallow a later real maximum — the carrier is MAXNM so
       * the VALUE advances; without the repair the INDEX would stay stale). */
      uint32x4_t old_finite = vceqq_f32(vmax, vmax);
      uint32x4_t vi_finite = vceqq_f32(vi, vi);
      /* (vmax is NaN) AND (vi is finite) */
      uint32x4_t nanwin = vandq_u32(vmvnq_u32(old_finite), vi_finite);
      uint32x4_t mask = vorrq_u32(vcgtq_f32(vi, vmax), nanwin);
      vmax = vmaxnmq_f32(vmax, vi);
...
static size_t simd_neon_argmin(const float *restrict x, size_t n) {
  ...
  size_t best_idx;
  if (n >= 4) { /* only seed a 4-wide vector when >=4 elements exist (OOB else) */
    if (isnan(x[0])) /* scalar parity: NaN seed poisons the scan -> 0 */
      return 0;
    ...
    for (i = 4; i + 4 <= n; i += 4) {
      ...
      uint32x4_t step = {0, 1, 2, 3};
      vidxi = vaddq_u32(vidxi, step);
      uint32x4_t old_finite = vceqq_f32(vmin, vmin);
      uint32x4_t vi_finite = vceqq_f32(vi, vi);
      uint32x4_t nanwin = vandq_u32(vmvnq_u32(old_finite), vi_finite);
      uint32x4_t mask = vorrq_u32(vcltq_f32(vi, vmin), nanwin);
      vmin = vminnmq_f32(vmin, vi);
...
static void simd_neon_argminmax(const float *restrict x, size_t n,
  ...
  size_t imin, imax;
  if (n >= 4) { /* only seed a 4-wide vector when >=4 elements exist (OOB else) */
    if (isnan(x[0])) { /* scalar parity: NaN seed poisons both scans -> 0 */
      *argmin_out = *argmax_out = 0;
      return;
    }
    float32x4_t vmin = vld1q_f32(x);
    ...
    for (i = 4; i + 4 <= n; i += 4) {
      ...
      uint32x4_t step = {0, 1, 2, 3};
      vidxi = vaddq_u32(vidxi, step);
      /* NaN-repair winner masks, as in argmax/argmin. */
      uint32x4_t old_finite = vceqq_f32(vmin, vmin);
      uint32x4_t vi_finite = vceqq_f32(vi, vi);
      uint32x4_t nanwin = vandq_u32(vmvnq_u32(old_finite), vi_finite);
      uint32x4_t mask_lt = vorrq_u32(vcltq_f32(vi, vmin), nanwin);
      uint32x4_t mask_gt = vorrq_u32(vcgtq_f32(vi, vmax), nanwin);
      vmin = vminnmq_f32(vmin, vi);
      vmax = vmaxnmq_f32(vmax, vi);
...
static void simd_neon_div(float *restrict z,
  ...
  size_t i = 0;
  for (; i + 4 <= n; i += 4)
    /* vdivq_f32, not vrecpeq+2xNR: on Apple M1 Pro (clang -O2, 2^20 random
     * floats spanning 2^-30..2^30, median of 21) vdivq measured 0.107-0.137
     * ms vs 0.142-0.178 ms for the reciprocal chain (~1.1-1.3x faster), AND
     * it is bit-identical to scalar a/b (0/2^20 diffs) where the chain
     * differed on 1,048,548/2^20 pairs, up to 47,393 ulp near the subnormal
     * floor -- head lanes and scalar tail can no longer disagree. */
    vst1q_f32(&z[i], vdivq_f32(vld1q_f32(&a[i]), vld1q_f32(&b[i])));
  for (; i < n; i++)
    z[i] = a[i] / b[i];
}
...
/* ── Activations ──────────────────────────────────────────────── */
/* Bitwise mirror of fast_sigmoid: fast_exp(-x) computed with the same mul ->
 * add -> truncate rounding (the +30 early-out comes out of the arithmetic;
 * see the loop), the -30 early-out as a select, NaN passthrough. The old
 * clamp pipeline gave 8.9e-14 at x=-31, ~1-2e-7 at x=31 and 0.5 at NaN in the
 * vector head — none of those matched the scalar reference. */
static void simd_neon_sigmoid(float *restrict out,
  ...
  for (; i + 4 <= n; i += 4) {
    float32x4_t vi = vld1q_f32(&in[i]);
    /* Unclamped fast_exp(-x), pipeline identical to the scalar (mul, add,
     * truncate). The x > 30 early-out is arithmetically free: for every
     * x >= 30 the computed 1 + e rounds to exactly 1.0f (e <= 9.4e-14, well
     * under the 6e-8 half-ulp at 1.0), so 1/(1+e) IS fast_sigmoid's 1.0. The
     * int clamps reproduce fast_exp's own guards for x >= 88 (0 / +Inf).
     * Only the -30 side needs a select (1/(1+e^30) is ~7e-31, not exactly 0),
     * plus the NaN passthrough the d.ts contract requires. */
    float32x4_t p = vaddq_f32(vmulq_f32(vi, magic), bias);
    int32x4_t bits = vcvtq_s32_f32(p);
    ...
    float32x4_t r = vdivq_f32(one, ve);
    r = vbslq_f32(vcltq_f32(vi, lo), vzero, r);      /* x < -30: exactly 0 */
    vst1q_f32(&out[i], vbslq_f32(vceqq_f32(vi, vi), r, vi)); /* NaN -> NaN */
  }
  for (; i < n; i++)
    out[i] = fast_sigmoid(in[i]);
}

/* tanh via fast exp: tanh(x) = 2*sigmoid(2x) - 1, mirroring fast_tanh: the
 * |x|>10 early-outs are exact ±1 selects, NaN propagates, and the reciprocal
 * is taken BEFORE doubling (2*(1/(1+e))) because that is the order the scalar
 * `2.0f * fast_sigmoid(2x) - 1` rounds in (the old head divided 2/(1+e)). */
static void simd_neon_tanh_fast(float *restrict out,
  ...
  for (; i + 4 <= n; i += 4) {
    float32x4_t vi = vld1q_f32(&in[i]);
    /* Same unclamped trick as sigmoid at double the magic (2x exact): the
     * computed 2/(1+e)-1 already lands on exactly ±1.0f for every x >= 10
     * (and, via the int-clamp saturation, for x <= -88 too), so only the
     * x < -10 band needs a select; NaN propagates by passthrough. */
    float32x4_t p = vaddq_f32(vmulq_f32(vi, magic), bias);
    int32x4_t bits = vcvtq_s32_f32(p);
    bits = vmaxq_s32(bits, vreinterpretq_s32_u32(zero));
    bits = vminq_s32(bits, vreinterpretq_s32_u32(top));
    float32x4_t ve = vsubq_f32(
        vmulq_f32(two, vdivq_f32(one, vaddq_f32(one, vreinterpretq_f32_s32(bits)))),
        one);
    ve = vbslq_f32(vcltq_f32(vi, lo), vdupq_n_f32(-1.0f), ve); /* x < -10: exactly -1 */
    vst1q_f32(&out[i], vbslq_f32(vceqq_f32(vi, vi), ve, vi));  /* NaN -> NaN */
  }
  for (; i < n; i++)
...
static void simd_neon_silu(float *restrict out,
  ...
  for (; i + 4 <= n; i += 4) {
    ...
    /* SATURATING add: the plain vaddq_s32 wrapped for x <= ~-89 (exp(+|x|)
     * pushed the bits int past INT32_MAX, wrapped negative, clamped to 0, and
     * silu(-100) returned -100 where the scalar tail gives ~-0. Saturation
     * lands on +Inf bits -> sigmoid 0 -> x*0, matching the scalar. */
...
static void simd_neon_relu(float *restrict out,
  ...
  for (; i + 4 <= n; i += 4) {
    float32x4_t vi = vld1q_f32(&in[i]);
    /* compare+select, not vmaxq: scalar is `x > 0 ? x : 0.0f`, so NaN -> 0.0
     * (vmaxq would keep NaN) and -0.0 -> the +0.0 literal (vmaxq may keep -0). */
    uint32x4_t mask = vcgtq_f32(vi, zero);
    vst1q_f32(&out[i], vbslq_f32(mask, vi, zero));
  }
  for (; i < n; i++)
...
static inline float32x4_t neon_fast_exp_f32(float32x4_t x) {
  /* Split mul/add — NOT vmlaq: the scalar fast_exp is statement-split
   * (contraction-proof), so the vector mirror must round mul then add too.
   * The old fused vmlaq disagreed with the scalar tail by one int step on
   * some inputs (softmax head vs tail). */
  int32x4_t bits = vcvtq_s32_f32(
      vaddq_f32(vmulq_f32(x, vdupq_n_f32(12102203.0f)), vdupq_n_f32(1065353216.0f)));
...
static void simd_neon_log_softmax(float *restrict out,
  ...
  for (; i < n; i++)
    out[i] = in[i] - (maxv + log_s); /* same rounding order as the vector head */
}

static void simd_neon_vexp(float *restrict out,
                                  const float *restrict in, size_t n) {
  float32x4_t magic = vdupq_n_f32(12102203.0f);
  float32x4_t bias = vdupq_n_f32(1065353216.0f); /* the fast_exp bias, as a float add */
  float32x4_t vzero = vdupq_n_f32(0.0f);
  uint32x4_t top = vdupq_n_u32(0x7F800000);
  size_t i = 0;
  float32x4_t vlo = vdupq_n_f32(-88.0f);
  float32x4_t vhi = vdupq_n_f32(88.0f);
  float32x4_t vinf = vdupq_n_f32(INFINITY);
  for (; i + 4 <= n; i += 4) {
    float32x4_t vi = vld1q_f32(&in[i]);
    /* Mirror fast_exp statement-for-statement so head, tail and the scalar
     * reference round identically: p = 12102203*x (mul), p += bias (add),
     * truncate. fast_exp's guards become selects: x < -88 -> 0, x > 88 -> +Inf,
     * NaN -> the input NaN (the d.ts contract says unary math propagates NaN;
     * the old int pipeline turned NaN into 1.0 in the head only). */
    float32x4_t p = vaddq_f32(vmulq_f32(vi, magic), bias);
    int32x4_t bits = vcvtq_s32_f32(p);
    bits = vmaxq_s32(bits, vreinterpretq_s32_u32(vdupq_n_u32(0)));
    bits = vminq_s32(bits, vreinterpretq_s32_u32(top));
    float32x4_t ve = vreinterpretq_f32_s32(bits);
    /* Guard selects only fire for |x| >= 88 or NaN; the in-range common case
     * (the clamp-free block) skips them entirely. */
    /* fast_exp's ±88 guards as selects: the trunc pipeline alone can differ
     * from the guards inside (88, 88.03) (finite vs +Inf), so keep both
     * selects unconditional — cheaper than a per-block domain branch. */
    ve = vbslq_f32(vcltq_f32(vi, vlo), vzero, ve);
    ve = vbslq_f32(vcgtq_f32(vi, vhi), vinf, ve);
    vst1q_f32(&out[i], vbslq_f32(vceqq_f32(vi, vi), ve, vi)); /* NaN passthrough */
  }
  for (; i < n; i++)
    out[i] = fast_exp(in[i]);
}
...
static void simd_neon_vrsqrt(float *restrict out,
  ...
  for (; i + 4 <= n; i += 4) {
    ...
    vs = vmulq_f32(vrsqrtsq_f32(vmulq_f32(vi, vs), vs), vs);
    vs = vmulq_f32(vrsqrtsq_f32(vmulq_f32(vi, vs), vs), vs);
    /* Mask like the scalar tail (`v > 0 ? 1/sqrt(v) : 0`): the raw estimate
     * chain returns NaN/-Inf/-huge for 0, negatives and NaN. */
    vst1q_f32(&out[i], vbslq_f32(vcgtq_f32(vi, vzero), vs, vzero));
  }
  for (; i < n; i++) {
    float v = in[i];
...
static float simd_neon_dist_cos(const float *restrict a,
  ...
  /* na and nb are float accumulators: forming their product in float
   * overflows to inf once each reaches ~1.8e19 even though the true
   * denominator (and sqrt of it) is exactly representable as a double.
   * Promote BEFORE the multiply -- T-CODEQL-6. */
...
static void simd_neon_gemv(float *restrict y,
  ...
  for (size_t i = 0; i < m; i++) {
    ...
    for (; j < n; j++)
      result += a[i * n + j] * x[j];
    /* BLAS beta=0 semantics: never read y (0 * NaN = NaN would poison it). */
...
static void simd_neon_gemv_t(float *restrict y,
  ...
  if (beta == 0.0f) {
    for (size_t j = 0; j < n; j++)
      y[j] = 0.0f; /* beta=0: never read y (NaN-poison-safe) */
  } else {
    for (size_t j = 0; j < n; j++)
      y[j] *= beta;
  }
...
static void simd_neon_gemm(float *restrict c,
  ...
  const size_t T = 32;
  /* BLAS beta=0 semantics: C := alpha*A*B — prior c is never read (0 * NaN
   * would keep the poison), the tile is zeroed instead. */
  if (beta == 0.0f) {
    for (size_t i = 0; i < m; i++)
      for (size_t j = 0; j < n; j++)
        c[i * n + j] = 0.0f;
  ...
  for (size_t i0 = 0; i0 < m; i0 += T) {
    ...
    for (size_t j0 = 0; j0 < n; j0 += T) {
      ...
      for (size_t k0 = 0; k0 < k; k0 += T) {
        ...
        /* The C tile is held in REGISTERS across the whole k range. It used
         * to be loaded and stored on every k iteration, so a T-deep tile did
         * T round trips to memory per output element -- two memory ops per
         * FMA, where a register-blocked kernel has none. j is stepped 8 wide
         * so two accumulators are live, which also gives the FMA unit two
         * independent chains instead of one.
         *
         * The summation order over kk is unchanged, so the result is
         * bit-identical. */
        for (size_t i = i0; i < imax; i++) {
          size_t j = j0;
          for (; j + 8 <= jmax; j += 8) {
            float32x4_t c0 = vld1q_f32(&c[i * n + j]);
            float32x4_t c1 = vld1q_f32(&c[i * n + j + 4]);
            for (size_t kk = k0; kk < kmax; kk++) {
              float32x4_t vaik = vdupq_n_f32(alpha * a[i * k + kk]);
              c0 = vmlaq_f32(c0, vaik, vld1q_f32(&b[kk * n + j]));
              c1 = vmlaq_f32(c1, vaik, vld1q_f32(&b[kk * n + j + 4]));
            }
          ...
          for (; j + 4 <= jmax; j += 4) {
            float32x4_t c0 = vld1q_f32(&c[i * n + j]);
            for (size_t kk = k0; kk < kmax; kk++)
              c0 = vmlaq_f32(c0, vdupq_n_f32(alpha * a[i * k + kk]),
                             vld1q_f32(&b[kk * n + j]));
            vst1q_f32(&c[i * n + j], c0);
          }
          for (; j < jmax; j++) {
...
/* ── Hamming ──────────────────────────────────────────────────── */
/* CNT (vcntq_u8, per-byte popcount) is mandatory base Armv8.0-A ASIMD
 * (Arm DDI0487) — no SHA3 or other feature gate is needed on any aarch64
 * host; the old in-code claim that vector popcount requires ARMv8.2-A +
 * SHA3 was wrong (audit E13-03). The XOR'd 4-word group is reinterpreted
 * as 16 bytes, CNT'd, and drained into a u32 each iteration: 16 lanes x
 * max 8 set bits = 128, so the u8 horizontal add cannot wrap.
 * Measured vs the old scalar-popcount loop (Apple M1 Pro, arm64, clang
 * -O2, /tmp/bench_hamming.c, 1<<20 words, 25 reps, median-of-runs):
 * 250us -> 195us per call, ~1.28x; results verified bit-identical over
 * 1M random words and exhaustive small cases. */
static float simd_neon_hamming(const uint32_t *restrict a,
  ...
  size_t i = 0;
  for (; i + 4 <= n_words; i += 4) {
    uint32x4_t xored = veorq_u32(vld1q_u32(&a[i]), vld1q_u32(&b[i]));
    result += vaddvq_u8(vcntq_u8(vreinterpretq_u8_u32(xored)));
  }
  for (; i < n_words; i++)
...
static double simd_neon_f64_max(const double *restrict x, size_t n) {
  ...
  if (n >= 2) { /* seed a 2-wide vector only when >=2 elements exist (OOB else) */
    /* MAXNM + -DBL_MAX seed: NaN ignored, all-NaN -> -DBL_MAX, like the scalar. */
    float64x2_t vmax = vmaxnmq_f64(vld1q_f64(x), vdupq_n_f64(-DBL_MAX));
    for (i = 2; i + 2 <= n; i += 2)
      vmax = vmaxnmq_f64(vmax, vld1q_f64(&x[i]));
    result = vmaxnmvq_f64(vmax);
  } else {
    result = x[0];
    i = 1;
  }
...
static double simd_neon_f64_min(const double *restrict x, size_t n) {
  ...
  size_t i;
  if (n >= 2) { /* seed a 2-wide vector only when >=2 elements exist (OOB else) */
    float64x2_t vmin = vminnmq_f64(vld1q_f64(x), vdupq_n_f64(DBL_MAX));
    for (i = 2; i + 2 <= n; i += 2)
      vmin = vminnmq_f64(vmin, vld1q_f64(&x[i]));
    result = vminnmvq_f64(vmin);
  } else {
    result = x[0];
    i = 1;
  }
...
static void simd_neon_f32_cummax(float *out, const float *x, size_t n) {
  ...
  for (; i + 4 <= n; i += 4) {
    float32x4_t v = vld1q_f32(&x[i]);
    /* MAXNM: a NaN never becomes the running max — the scalar oracle carries
     * the last finite max straight past NaN (`x[i] > acc` is false for NaN),
     * vmaxq would splatter defaultNaN over the rest of the scan. */
    v = vmaxnmq_f32(v, vextq_f32(ninf, v, 3)); /* fill -inf so lane0 keeps its own */
    v = vmaxnmq_f32(v, vextq_f32(ninf, v, 2));
    v = vmaxnmq_f32(v, vdupq_n_f32(carry));
    vst1q_f32(&out[i], v);
    carry = vgetq_lane_f32(v, 3);
  }
  for (; i < n; i++) {
 ...
 * a 256-entry membership table drives the tail and larger sets. The table is
 * built only when the tail actually runs: the vector loop gets first crack at
 * answering so a hit (or a whole-range miss) never pays the 256-byte setup. */
static size_t simd_neon_find_first_of_u16(const uint16_t *restrict p,
                                            size_t n,
  ...
  if (setlen <= 4) {
    /* The anchored-scan terminators are four units; OR the equality masks
     * and find the first set nibble. Same shape as the 8-bit kernel, half
     * the lanes. */
    for (k = 0; k < setlen; k++) vset[k] = vdupq_n_u16(set[k]);
    for (; i + 8 <= n; i += 8) {
      uint16x8_t blk = vld1q_u16(p + i);
      uint16x8_t m = vceqq_u16(blk, vset[0]);
      for (k = 1; k < setlen; k++)
        m = vorrq_u16(m, vceqq_u16(blk, vset[k]));
      /* vshrn #8 narrows 8 u16 lanes to 8 bytes, 0x00/0xFF per lane; the
       * first set BYTE sits at bit 8*lane, so ctz>>3 recovers the lane.
       * (The 8-bit kernel's nibble_mask maps byte lanes and would
       * mis-index u16 lanes; a double vshrn shifts twice and zeroes the
       * mask -- caught by the wide-anchor scan test, which the scalar
       * tail's <8-unit remainder used to half-hide.) */
      uint64_t mask = (uint64_t)vget_lane_u64(
          vreinterpret_u64_u8(vshrn_n_u16(m, 8)), 0);
      if (mask) return i + (__builtin_ctzll(mask) >> 3);
    }
    if (i >= n) return SIZE_MAX;
  }
  /* small-set loop stopped short, or a >4 set: scalar to the end */
  for (; i < n; i++)
    for (k = 0; k < setlen; k++)
      if (p[i] == set[k]) return i;
  return SIZE_MAX;
}
...
static size_t simd_neon_find_first_of(const uint8_t *restrict p, size_t n,
  ...
  if (setlen <= 8) {
    ...
    if (i >= n) return SIZE_MAX; /* vector loop covered the whole range */
  }
  /* Only reachable when a scalar tail remains (setlen > 8, or the vector
   * loop stopped short of n): NOW the table is needed. */
...
/* 256-bit membership by two table lookups per 16 bytes.
 *
 *   sel  = bitmap[(v >> 3) & 15] chosen from the low or high half by v >> 7
 *   bit  = 1 << (v & 7), itself a table lookup
 *   member = (sel & bit) != 0
 *
 * vqtbl1q_u8 indexes a 16-byte table, so the 32-byte bitmap is split in half
 * and the halves blended on the top bit of v. No per-call setup: the bitmap
 * arrives ready, which is the point of this kernel over find_first_of. */
static size_t simd_neon_find_bitmap(const uint8_t *restrict p, size_t n,
                                    const uint8_t *restrict bitmap) {
  const uint8x16_t lo_tbl = vld1q_u8(bitmap);
  const uint8x16_t hi_tbl = vld1q_u8(bitmap + 16);
  static const uint8_t pow2_init[16] = {1,2,4,8,16,32,64,128, 0,0,0,0,0,0,0,0};
  const uint8x16_t pow2 = vld1q_u8(pow2_init);
  size_t i = 0;
  for (; i + 16 <= n; i += 16) {
    uint8x16_t v = vld1q_u8(p + i);
    uint8x16_t idx = vandq_u8(vshrq_n_u8(v, 3), vdupq_n_u8(0x0f));
    uint8x16_t ml = vqtbl1q_u8(lo_tbl, idx);
    uint8x16_t mh = vqtbl1q_u8(hi_tbl, idx);
    /* top bit of v selects the high half of the bitmap */
    uint8x16_t hi_sel = vcgeq_u8(v, vdupq_n_u8(0x80));
    uint8x16_t sel = vbslq_u8(hi_sel, mh, ml);
    uint8x16_t bit = vqtbl1q_u8(pow2, vandq_u8(v, vdupq_n_u8(0x07)));
    uint8x16_t hit = vtstq_u8(sel, bit);
    uint64_t mask = simd_neon_nibble_mask(hit);
    if (mask) return i + (__builtin_ctzll(mask) >> 2);
  }
  for (; i < n; i++)
    if (bitmap[p[i] >> 3] & (uint8_t)(1u << (p[i] & 7))) return i;
  return SIZE_MAX;
}
...
/* Nibble value and validity in 9 vector ops instead of 19.
 *
 * The three separate range tests ('0'-'9', 'a'-'f', 'A'-'F') cost six compares
 * and eight selects. Folding case with `| 0x20` collapses the two letter
 * ranges into one, and UNSIGNED subtraction does the range test for free: a
 * character below the range wraps to a large value and fails the <= compare,
 * so `c - '0' <= 9` is the whole digit test. ':' (0x3A) becomes 10 and is
 * correctly rejected, which is the case a naive `>= '0'` test lets through. */
static inline uint8x16_t simd_neon_hex_nibbles(uint8x16_t c, uint8x16_t *valid) {
  uint8x16_t dv = vsubq_u8(c, vdupq_n_u8('0'));            /* '0'-'9' -> 0..9  */
  uint8x16_t lo = vorrq_u8(c, vdupq_n_u8(0x20));           /* fold 'A'-'F'     */
  uint8x16_t av = vsubq_u8(lo, vdupq_n_u8('a' - 10));      /* 'a'-'f' -> 10..15 */
  uint8x16_t isd = vcleq_u8(dv, vdupq_n_u8(9));
  uint8x16_t isa = vandq_u8(vcgeq_u8(av, vdupq_n_u8(10)),
                            vcleq_u8(av, vdupq_n_u8(15)));
  *valid = vorrq_u8(isd, isa);
  return vbslq_u8(isd, dv, av);
}
...
static size_t simd_neon_hex_decode(const char *restrict src, size_t n,
  ...
  size_t i = 0, o = 0;
  /* Validity is ACCUMULATED and tested once after the loop, not per block.
     A cross-lane reduce (vminvq) plus an unpredictable branch every 32 bytes
     serialises the whole loop, which is why decode ran at a quarter of
     encode's rate at every size. A malformed digit is the rare case, so the
     bypass is "the whole buffer was clean" -- and on failure the caller sees
     the same SIZE_MAX it always did, having already been unable to trust dst
     on a partial decode. */
  ...
  for (; i + 32 <= n; i += 32) {
    ...
    acc = vandq_u8(acc, vandq_u8(hvalid, lvalid));
    vst1q_u8(dst + o, vorrq_u8(vshlq_n_u8(hn, 4), ln));
    o += 16;
  }
  if (vminvq_u8(acc) != 0xFF) return SIZE_MAX;
  for (; i < n; i += 2) {
...
/* ── Override table ─────────────────────────────────────────────── */
/* base64 encode, 48 bytes -> 64 chars per iteration.
 *
 * vld3q_u8 de-interleaves the 3-byte groups for free, which is exactly the
 * shape base64 wants: val[0]/val[1]/val[2] are the first/second/third byte of
 * every triple. The 6-bit fields then need only shifts and masks, and the
 * 64-entry alphabet is one vqtbl4q_u8 -- a single instruction for 16 lookups,
 * where the scalar loop does one indexed load per character.
 *
 * The scalar tail handles the remainder and the '=' padding, which is the only
 * part with a branch. */
static size_t simd_neon_base64_encode(const uint8_t *restrict src, size_t n,
                                      char *restrict dst) {
  static const char E[] =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
  uint8x16x4_t T;
  size_t i = 0, o = 0;
  T.val[0] = vld1q_u8((const uint8_t *)E);
  T.val[1] = vld1q_u8((const uint8_t *)E + 16);
  T.val[2] = vld1q_u8((const uint8_t *)E + 32);
  T.val[3] = vld1q_u8((const uint8_t *)E + 48);
  for (; i + 48 <= n; i += 48) {
    uint8x16x3_t v = vld3q_u8(src + i);
    uint8x16x4_t out;
    uint8x16_t a = v.val[0], b = v.val[1], c = v.val[2];
    out.val[0] = vshrq_n_u8(a, 2);
    out.val[1] = vorrq_u8(vshlq_n_u8(vandq_u8(a, vdupq_n_u8(0x03)), 4),
                          vshrq_n_u8(b, 4));
    out.val[2] = vorrq_u8(vshlq_n_u8(vandq_u8(b, vdupq_n_u8(0x0F)), 2),
                          vshrq_n_u8(c, 6));
    out.val[3] = vandq_u8(c, vdupq_n_u8(0x3F));
    out.val[0] = vqtbl4q_u8(T, out.val[0]);
    out.val[1] = vqtbl4q_u8(T, out.val[1]);
    out.val[2] = vqtbl4q_u8(T, out.val[2]);
    out.val[3] = vqtbl4q_u8(T, out.val[3]);
    vst4q_u8((uint8_t *)dst + o, out);
    o += 64;
  }
  for (; i + 3 <= n; i += 3) {
    uint32_t x = ((uint32_t)src[i] << 16) | ((uint32_t)src[i + 1] << 8) | src[i + 2];
    dst[o++] = E[(x >> 18) & 0x3F]; dst[o++] = E[(x >> 12) & 0x3F];
    dst[o++] = E[(x >> 6) & 0x3F];  dst[o++] = E[x & 0x3F];
  }
  if (n - i == 1) {
...
void simd_override_neon(simd_t *t) {
  ...
  t->base64_encode = simd_neon_base64_encode;
  ...
  t->find_first_of_u16 = simd_neon_find_first_of_u16;
  t->find_bitmap = simd_neon_find_bitmap;
