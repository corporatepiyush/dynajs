...
/* Read options.max into *out (-1 = absent). Coercion can run user JS, so it
   happens before any operand is bound. `fname` names the caller in the error
   (Levenshtein and DamerauLevenshtein share the option and its contract). */
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    if (dyn_lev_read_max(ctx, argc, argv, &maxv, "Levenshtein") < 0)  /* coerce options FIRST */
        return JS_EXCEPTION;
    if (argc < 2)
    ...
    /* Levenshtein is O(n*m) by definition, so the caller controls the CPU cost
       ...
       than burn it. `max` narrows the band, but the DP branch below enforces
       the SAME cell budget on it -- a max at or past the pattern length
       degenerates to the full matrix, so max must not bypass the guard. */
    ...
    if (m == 0) {
        ...
        size_t need, width;
        /* The band is final HERE, so this is the only place the cell budget
           can be enforced: the guard above fires only when maxv < 0, and a
           { max } at or past the pattern length degenerates the band to the
           full matrix -- O(n*m) again, whatever `max` claimed. Each DP row
           visits at most min(m, 2*band+1) cells (lo = max(1, i-band),
           hi = min(m, i+band)), so that times n is the work a caller can
           demand. (band < m <= DYN_APPROX_MAX_BYTES here, so 2*band+1
           cannot overflow.) */
        if (band == (size_t)-1)
            band = m;
...
/* The Jaro similarity of two code-point arrays: matched pairs may sit at most
   floor(max(|a|,|b|)/2) - 1 positions apart, t counts half-transpositions of
   the matched sequences, and the score is the mean of the three ratios. The
   caller handles the degenerate empty cases and has already applied the
   byte-equality fast path, so m >= 1 is guaranteed here. */
                              ...
                              const uint32_t *b, size_t nb)
{
    ...
    if (!bflag || !amatch) {
        free(bflag);
        free(amatch);
        return -1.0;                    /* caller turns this into an OOM */
    }
    for (i = 0; i < na; i++) {
    ...
    /* Half-transpositions: positions where the two matched sequences, both
       read left to right, disagree. */
    for (j = 0; j < nb; j++) {
        if (!bflag[j])
            continue;
        if (b[j] != amatch[k])
    ...
    jaro = ((double)m / (double)na + (double)m / (double)nb
            + ((double)m - (double)t) / (double)m) / 3.0;
    return jaro;
}

/* JaroWinkler(a, b) -> [0, 1]. The Jaro similarity with Winkler's
   common-prefix boost: l = min(prefix, 4), scale p = 0.1, applied when the
   Jaro score exceeds the 0.7 boost threshold -- the standard definition
   (Winkler 1990), threshold included. Code-point operands, like
   Levenshtein. */
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    if (dyn_operand_get(ctx, argv[1], "b", &B) < 0)
        goto done;
    /* Identical input scores 1 (byte equality IS code-point equality for
       valid UTF-8), answered before any kernel work -- the same bypass
       DiceCoefficient takes. */
#ifndef DYN_NO_BYPASS
    if (A.bytes == B.bytes && memcmp(A.cs, B.cs, A.bytes) == 0) {
        ret = JS_NewFloat64(ctx, 1.0);
        goto done;
...
#endif
    if (A.n == 0 || B.n == 0) {         /* one side empty: nothing can match */
    ...
    /* The window walk is O(n * window), so a caller controls the CPU cost
       through the input lengths; the same cell budget Levenshtein enforces
       bounds it here too. (The memcmp bypass above is why the identical-input
    ...
    if (jaro < 0.0) {                   /* malloc failure inside the kernel */
        JS_ThrowOutOfMemory(ctx);
        goto done;
    }
    if (jaro > 0.7) {
...
/* Restricted ("Optimal String Alignment") Damerau-Levenshtein: a transposition
   counts as one edit, but a transposed pair may not take part in another edit.
   Three DP rows (the transposition reads the row TWO steps back); the same
   |i-j| <= max band discipline as DYN_LEV_DP_BODY. `r0/r1/r2` are the i-2 /
   i-1 / i rows. */
#define DYN_OSA_DP_BODY                                                        \
    ...
    for (j = 0; j <= m; j++)                                                   \
        r1[j] = j;                        /* the i = 0 row */                  \
    for (j = 0; j <= m; j++)                                                   \
        r0[j] = DYN_LEV_INF;              /* no i = -1 row: never read */      \
    for (i = 1; i <= n; i++) {                                                 \
        lo = (i > band) ? i - band : 1;                                        \
        hi = (i + band < m) ? i + band : m;                                    \
        ...
        for (j = lo; j <= hi; j++) {                                           \
            ...
            if (v > sub)                                                       \
                v = sub;                                                       \
            if (i >= 2 && j >= 2 && A[i - 1] == B[j - 2] &&                    \
                A[i - 2] == B[j - 1] && r0[j - 2] + 1 < v)                     \
                v = r0[j - 2] + 1;                                             \
            r2[j] = v;                                                         \
        }                                                                      \
        if (hi < m)                                                            \
...
/* DamerauLevenshtein(a, b[, { max }]) -> the OSA edit distance in CODE POINTS.
   Same { max } contract as Levenshtein: exact while <= max, max + 1 once the
   distance exceeds it, and a negative max is a RangeError. */
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    if (dyn_lev_read_max(ctx, argc, argv, &maxv, "DamerauLevenshtein") < 0)
        return JS_EXCEPTION;           /* coerce options FIRST */
    if (argc < 2)
    ...
    /* The shorter side is the pattern, exactly as in Levenshtein: the band
       arithmetic and the budget use m = min(|a|,|b|). */
    if (A.n <= B.n) { pat = &A; txt = &B; } else { pat = &B; txt = &A; }
    m = pat->n; n = txt->n;
    diff = n - m;
    if (maxv >= 0 && (uint64_t)diff > (uint64_t)maxv) {
        ret = JS_NewInt64(ctx, maxv + 1);   /* length alone already exceeds max */
        goto done;
    }
    band = maxv >= 0 ? (size_t)maxv : n;    /* no max: the band covers all. A
       band past n is a no-band query (|i - j| <= n always), and clamping it
       there keeps 2*band+1 inside size_t even for a huge { max }. */
    if (band > n)
        band = n;
    if (m == 0) {
        dist = n;
    } else {
        /* Each row visits at most min(m, 2*band+1) + 2 cells; same budget as
           Levenshtein, enforced at the same place. (band <= n <= 16 MiB of
           code points, so 2*band+1 cannot overflow.) */
        width = band >= m ? m + 2
        ...
        if (!rows) { JS_ThrowOutOfMemory(ctx); goto done; }
        if (pat->ascii && txt->ascii) {
