...
/* For the node/index array growth: this fragment used to reach json5.inc.c's
   static dyn_grow_cap across the include order in dyna-encoding.c -- an
   invisible coupling the buffer consolidation made explicit. */
...
typedef struct {
    ...
    int        first_only;          /* first(): stop the frontier at one match */
} jp_eval_t;
...
{
    if (*n == *cap) {
        size_t nc = dyn_sb_grow_cap(*cap, *n + 1, 32);
                            ...
                            int64_t index)
{
    ...
    if (e->nn == e->ncap) {
        size_t nc = dyn_sb_grow_cap(e->ncap, e->nn + 1, 32);
 ...
 * Arrays enumerate their PRESENT index properties rather than walking
 * 0..length-1: an attacker-shaped { b: new Array(4294967295) } is empty but
 * its length alone used to cost ~106 s of atom churn per wildcard segment.
 * Enumerating the element table is O(elements). Integer keys come back in
 * ascending order, so document order is preserved; "length" is not an index
 * atom and is filtered, and non-index members of an array are outside the
 * RFC 9535 data model exactly as before. */
                       ...
                       uint32_t *cap)
{
    ...
    if (len >= 0) {
        ...
        for (k = 0; k < len32; k++) {
            ...
            JSValue cv;
            int r;
            if (!JS_AtomIsArrayIndex(e->ctx, &idx, tab[k].atom))
                continue;
            r = jp_own(e->ctx, v, tab[k].atom, &cv);
            if (r < 0) { e->err = 1; break; }
            if (r == 0) continue;
            {
                int32_t idxn = jp_push_node(e, cv, (int32_t)node, JS_ATOM_NULL,
                                            (int64_t)idx);
                if (idxn < 0 || jp_push_idx(e, a, n, cap, (uint32_t)idxn) < 0)
                    break;
            }
        }
...
/* RFC 9535 s2.3.5.2.2: objects/arrays are compared STRUCTURALLY (same
   names/values, same length/elements), not by identity. JSON data is acyclic,
   so a depth cap of 256 -- the module's other caps -- is generous; beyond it
   the comparison answers "not equal" rather than risking the native stack. */
#define JP_CMP_MAX_DEPTH 256

static int jp_deep_equal(JSContext *ctx, JSValueConst a, JSValueConst b,
                         int depth);

/* A member of `a` looked up in `b`; both member values freed here. */
static int jp_member_equal(JSContext *ctx, JSValueConst a, JSAtom name,
                           JSValueConst b, int depth)
{
    JSPropertyDescriptor da, db;
    int eq = 0, ra, rb;

    ra = JS_GetOwnProperty(ctx, &da, a, name);
    if (ra < 0)
        return -1;
    if (ra == 0)
        return 0;                            /* absent: names differ */
    rb = JS_GetOwnProperty(ctx, &db, b, name);
    if (rb < 0) {
        JS_FreeValue(ctx, da.value);
        return -1;
    }
    if (rb == 0) {
        JS_FreeValue(ctx, da.value);
    ...
    eq = jp_deep_equal(ctx, da.value, db.value, depth);
    JS_FreeValue(ctx, da.value);
    JS_FreeValue(ctx, db.value);
    return eq;
}
...
static int jp_deep_equal(JSContext *ctx, JSValueConst a, JSValueConst b,
                         int depth)
{
    if (depth > JP_CMP_MAX_DEPTH)
        return 0;
    if (JS_VALUE_GET_PTR(a) == JS_VALUE_GET_PTR(b))
        return 1;
    {
        int aa = JS_IsArray(ctx, a) == 1, ab = JS_IsArray(ctx, b) == 1;
        if (aa || ab) {
            JSValue la, lb;
            int64_t na = -1, nb = -1, i;
            if (!aa || !ab)
                return 0;
            la = JS_GetPropertyStr(ctx, a, "length");
            lb = JS_GetPropertyStr(ctx, b, "length");
            if (!JS_IsException(la) && !JS_IsException(lb)) {
                JS_ToInt64(ctx, &na, la);
                JS_ToInt64(ctx, &nb, lb);
            }
            JS_FreeValue(ctx, la);
            JS_FreeValue(ctx, lb);
            if (na != nb)
            ...
            for (i = 0; i < na; i++) {
                ...
                eq = jp_deep_equal(ctx, ea, eb, depth + 1);
                JS_FreeValue(ctx, ea);
                JS_FreeValue(ctx, eb);
                if (eq <= 0)
                    return eq;
    ...
    if (JS_IsObject(a) && JS_IsObject(b)) {
        JSPropertyEnum *tab = NULL, *tabb = NULL;
        uint32_t len = 0, lenb = 0, k;
        int eq = 1;
        /* same collection of names: equal cardinality, then every name of a
         * must exist in b with an equal value. */
        if (JS_GetOwnPropertyNames(ctx, &tab, &len, a,
                                   JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY) < 0)
            return -1;
        if (JS_GetOwnPropertyNames(ctx, &tabb, &lenb, b,
                                   JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY) < 0) {
        ...
        if (len != lenb)
            eq = 0;
        for (k = 0; k < len && eq > 0; k++)
            eq = jp_member_equal(ctx, a, tab[k].atom, b, depth + 1);
        JS_FreePropertyEnum(ctx, tab, len);
        JS_FreePropertyEnum(ctx, tabb, lenb);
        return eq;
    }
    return 0;                                /* leaf/type mismatch */
}

static int jp_compare(JSContext *ctx, uint8_t op, const jp_val_t *a,
                      const jp_val_t *b)
{
    int eq = 0, lt = 0, ord = 0;
    ...
    if (a->kind == b->kind) {
        ...
        default:
            /* RFC 9535 s2.3.5.2.2: structural equality for objects/arrays.
             * -1 (an engine error) propagates, never reads as "equal". */
            eq = jp_deep_equal(ctx, a->owned, b->owned, 0);
            if (eq < 0)
                return -1;
            break;
        }
...
static int jp_run(jp_eval_t *e, JSValueConst value)
{
    ...
    for (g = 0; g < e->p->nseg; g++) {
        ...
        /* THE first() EARLY EXIT, and the only one: once the next frontier has
           a node, later `from` nodes can only append AFTER it -- matches come
           out in document order -- so cur[0] is already final and the query
           stops walking. all() and paths() leave first_only 0 and see every
           node. A descendant segment's preorder walk above is not pruned; the
           stop is here, in the selector application itself. */
        e->nnxt = 0;
        for (i = 0; i < nfrom; i++) {
            for (k = 0; k < seg->n; k++) {
                if (jp_apply_sel(e, from[i], &e->p->sels[seg->first + k]) < 0)
                    return -1;
                if (e->first_only && e->nnxt)
                    break;
            }
            if (e->first_only && e->nnxt)
                break;
                           ...
                           int argc, JSValueConst *argv)
{
    ...
    return dyn_plain_wrap(ctx, new_target, dyn_jp_class_id, p, jp_free_v);
}
