                           ...
                           size_t bufsz, int skip_empty)
{
    ...
    while (*path) {
        ...
        if (!*sep) sep = NULL;
        if (!dlen) {
            /* S9: an empty PATH element is POSIX-execvp cwd search. It is a
               cwd-injection vector when PATH is env-controlled, so callers
               may skip empty elements entirely (skip_empty=1). Default 0
...
static int dyn_envv_at(JSContext *ctx, JSValueConst obj, JSAtom atom, char **slot)
{
    ...
    /* The same NUL refusal as every other dyna:sys boundary: execve would act
       on the truncated prefix of "NAME\0junk" or "value\0junk" -- an env gate
       that approved the full JS string must not be bypassed by a prefix. */
...
typedef struct {
    ...
    const uint8_t *input;
    size_t   inlen;
    int      no_path_cwd;               /* S9: skip empty PATH elements */
} dyn_exec_t;
                       ...
                       int64_t floor_, const char *what, int64_t *out)
{
    ...
    /* NaN coerces to 0, which for timeoutMs means "no deadline": an option
       whose coercion fails open must refuse non-finite values, the same rule
       setNativeMemoryLimit enforces. */
                           ...
                           int64_t *out)
{
    ...
    /* NaN would coerce to 0, which here means uid/gid 0 (root); Infinity has
       no integer form at all. The option must fail CLOSED on a non-finite
       value, like the other numeric options. */
    if (!JS_IsNumber(v) || JS_ToFloat64(ctx, &d, v) != 0 || !isfinite(d)) {
        JS_FreeValue(ctx, v);
                         ...
                         JSValue *input_ref, int *want_bytes)
{
    ...
    if (!JS_IsUndefined(v)) {
        ...
        JS_FreeValue(ctx, v);
        if (b < 0) return -1;
        /* allowPathCwd:false skips empty PATH elements (the execvp
         * cwd search); default (absent) preserves POSIX parity. */
        e->no_path_cwd = !b;
                          ...
                          dyn_exec_t *e, JSValue *input_ref, int *want_bytes)
{
    ...
    /* Refusing NUL here matters more than anywhere else: execve on the
       truncated prefix runs a DIFFERENT program than the string the caller
       sees ("/bin/echo\0-evil" would exec /bin/echo under a name a validator
       already approved). */
