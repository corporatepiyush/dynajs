/* Number.prototype (standard plus non-ES extensions) and Boolean.prototype.
 *
 ...
 * Defined below with the other range builders. */
...
/* --- non-ECMAScript Number.prototype methods ---
...
/* isMultipleOf(n) -> this % n === 0. n==0 or NaN -> false via fmod. */
...
/* mathMod(n) -> NaN unless both are integers and n >= 1, else a
...
/* clamp(min, max) -> `this` clamped into [min, max]. */
...
/* log(base=e) -> change-of-base logarithm. */
...
/* round/ceil/floor(precision=0) -> round to `precision` decimal places
...
/* chr() -> the single character for this code (fromCharCode semantics). */
...
/* ===== Number formatting + iteration — batch 2 =====
                                 ...
                                 int argc, JSValueConst *argv, int magic)
{
    ...
    if (place < 0) place = 0;
    /* Bound the width: place feeds string_buffer_init directly, and an
       unbounded one is a single-call multi-GB allocation. format() caps
       at 20; pad is a layout helper, so 1 MiB of padding is the ceiling
       -- generous for every real use and trivially small as memory. */
    if (place > (1 << 20)) place = 1 << 20;
    if (!isfinite(d))          /* Infinity / NaN: no integer form to pad */
        return JS_ToStringFree(ctx, __JS_NewFloat64(ctx, d));
    /* 1e308 is finite but outside int64: the cast is UB. Clamp so a huge
       number pads as the nearest representable edge, not as garbage. The
       upper clamp must land BELOW 2^63: 2^63-1 is not representable as a
       double (it rounds back up to 2^63), so the nearest valid double is
       2^63-1024. */
                                    ...
                                    int argc, JSValueConst *argv, int magic)
{
    ...
    dot = num;                        /* one pass, not strchr then strlen */
    while (*dot && *dot != '.') dot++;
    intlen = (int)(dot - num);
    if (!*dot) dot = NULL;
    first = 0;
                                        ...
                                        int argc, JSValueConst *argv)
{
    ...
    if (!isfinite(d))
        return JS_ToStringFree(ctx, __JS_NewFloat64(ctx, d));
    /* 1e308 is finite but outside int64: the cast is UB; clamp as in pad() */
    if (d >= 9223372036854775808.0)
        d = 9223372036854775808.0 - 1024.0;
    else if (d < -9223372036854775808.0)
        d = -9223372036854775808.0;
                                      ...
                                      int argc, JSValueConst *argv)
{
    ...
    if (!isfinite(d))
        return JS_ToStringFree(ctx, __JS_NewFloat64(ctx, d));
    av = fabs(d);
    /* clamp to int64 range before the long long cast (see pad()) */
                                   ...
                                   int argc, JSValueConst *argv)
{
    ...
    /* huge finite: clamp before the cast (see pad()) -- the > NUM_RANGE_MAX
       check below rejects it cleanly once it is in range */
    else if (d >= 9223372036854775808.0) n = 9223372036854775807LL;
    else n = (int64_t)d;
                                  ...
                                  int argc, JSValueConst *argv, int magic)
{
    ...
    if (span < 0) return JS_NewArray(ctx);         /* wrong direction -> empty */
    {
        ...
        /* a quotient past NUM_RANGE_MAX (including +Inf from 1e308/1e-308)
           must be rejected BEFORE the cast, which is UB for it */
                                      ...
                                      int argc, JSValueConst *argv)
{
    ...
    {
        double q = ceil(span / step);               /* end-exclusive */
        /* reject past NUM_RANGE_MAX BEFORE the cast (see upto) */
        if (!(q < (double)NUM_RANGE_MAX))
            return JS_ThrowRangeError(ctx, "range too large");
...
static const JSCFunctionListEntry js_number_ext_funcs[] = {
    ...
    /* mathMod / clamp, log / precision round */
                           ...
                           int argc, JSValueConst *argv)
{
    ...
    /* Identity fast path.  parseInt(str) is ToInt32-equivalent to
       parsing ToString(v): for a number v that is an integer with
       |v| <= 2^53, ToString(v) is plain digits (ES2024 §6.1.6.1.20
       uses positional notation for k <= n <= 21; 2^53 ~= 9.0e15 <
       1e21), so the generic ToString->parse round trip returns v
       itself.  The radix conversion runs FIRST (a throwing radix
       object must throw even when arg0 qualifies; ToString of a
       number cannot throw, so the reorder is unobservable) and only
       the decimal radixes (undefined/0/10) take the path. */
        ...
        if (radix0 == 0 || radix0 == 10) {
                ...
                return JS_DupValue(ctx, argv[0]);
            {
                ...
                if (isfinite(d) && d == floor(d) && fabs(d) <= 9007199254740992.0) {
                    if (d == 0)
                        return JS_NewInt32(ctx, 0); /* -0 -> +0 (ToString) */
                    return JS_NewFloat64(ctx, d);
                }
            }
        }
        /* non-qualifying: fall through to the generic ToString path
           (its radix conversion already ran and cannot throw twice) */
