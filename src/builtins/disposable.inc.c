...
/* GetDisposeMethod step 1.b.ii: the internal closure an async hint wraps a
   sync @@dispose fallback in. Calls the sync method with the resource value
   as `this`, DISCARDS its return value (spec note: "a Promise returned from
   a synchronous @@dispose method will not be awaited"), and converts a
   thrown error into a rejected promise instead of a synchronous throw.
   func_data[0] = the sync method. */
...
/* Wrap a sync @@dispose fallback method for an async hint. Consumes
   'method' (the closure dups its own reference). */
static JSValue js_make_sync_dispose_wrapper(JSContext *ctx, JSValue method)
{
    ...
    JS_FreeValue(ctx, method); /* the closure holds its own dup */
    return f;
}
                                       ...
                                       int argc, JSValueConst *argv, int magic)
{
    ...
    if (JS_IsUndefined(method)) {
        /* GetDisposeMethod fallback to the sync @@dispose method; for an
           async hint the fallback is wrapped so the async DisposeResources
           driver awaits the wrapper's (immediately settled) promise rather
           than a Promise the sync method returned (step 1.b.ii) */
