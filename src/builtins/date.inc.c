...
static JSValue JS_SetThisTimeValue(JSContext *ctx, JSValueConst this_val, double v)
{
    if (JS_VALUE_GET_TAG(this_val) == JS_TAG_OBJECT) {
        ...
        if (p->class_id == JS_CLASS_DATE) {
            JSValue nv = JS_NewFloat64(ctx, v);
            /* The slot always holds a number (or UNDEFINED before the
               constructor's first store), and int/float64 JSValues are
               immediates in this engine (refcounted tags are negative,
               numbers are not): storing in place IS the slot reuse --
               only free if something refcounted is somehow there
               (defensive; never taken in practice). */
            if (unlikely(JS_VALUE_HAS_REF_COUNT(p->u.object_data)))
                JS_FreeValue(ctx, p->u.object_data);
            /* single mutation choke point: every setter funnels here
               (setTime, set_date_field for all setX/setUTCX, setYear),
               so this is the one place the cached field breakdown is
               dropped on mutation (JS_SetObjectData does the same for
               the construction stores) */
            if (unlikely(p->u.date.fields_cache != NULL)) {
                p->u.date.fields_cache->e[0].valid = 0;
                p->u.date.fields_cache->e[1].valid = 0;
            }
            p->u.object_data = nv;
            return nv; /* numbers are uncounted: JS_DupValue(nv) == nv */
        }
    }
...
/* Fixed-width decimal emitters for get_date_string(). snprintf costs hundreds
   of ns per call and every date string needs 3-5 of them; these are a few
   stores each. Callers guarantee the value ranges and the buffer headroom. */
static char const date_digit_pairs[201] =
...
/* 0 <= v <= 99 */
static inline int date_put_u2(char *p, int v)
{
    p[0] = date_digit_pairs[v * 2];
    p[1] = date_digit_pairs[v * 2 + 1];
    return 2;
}

/* 0 <= v <= 999 */
static inline int date_put_u3(char *p, int v)
{
    p[0] = '0' + v / 100;
    date_put_u2(p + 1, v % 100);
    return 3;
}

/* 0 <= v <= 9999 */
static inline int date_put_u4(char *p, int v)
{
    date_put_u2(p, v / 100);
    date_put_u2(p + 2, v % 100);
    return 4;
}

/* equivalent to snprintf(p, avail, "%0*d", 4 + (y < 0), y) */
static int date_put_year(char *p, size_t avail, int y)
{
    if (likely((unsigned)y <= 9999))
        return date_put_u4(p, y);
    return snprintf(p, avail, "%0*d", 4 + (y < 0), y);
}

/* Compute (and cache) the field breakdown of a Date.  The per-object
 * cache (u.date.fields_cache) turns repeat getters -- the audit's
 * dominant pattern, 11.8x vs node -- into one double compare (plus, for
 * local time, one tz-epoch compare).  Invalidation rule:
 *  - any store of a new time value (JS_SetThisTimeValue for mutations,
 *    JS_SetObjectData for construction) clears valid on both slots;
 *  - a local breakdown is additionally only served while the tz offset
 *    for its UTC second is unchanged: tz-epoch equal (no memo refill
 *    since the computation) or, after refills, one re-resolve of the
 *    same second still returning the cached offset.  This matches the
 *    uncached path exactly, including the tz memo's documented
 *    <=1-second TZ-change staleness window, and never extends it.
 * UTC breakdowns have tz == 0 by definition and need no tz check.
 * The cache is best-effort: allocation failure just answers uncached. */
                                       ...
                                       double fields[minimum_length(9)], int is_local, int force)
{
    ...
    JSDateFieldsCacheEntry *c = NULL; /* cache entry on the hit path */

    if (JS_ThisTimeValue(ctx, &dval, obj))
        return -1;

    if (likely(!isnan(dval))) {
        /* JS_ThisTimeValue only succeeds for JS_CLASS_DATE */
        JSDateFieldsCache *fc = JS_VALUE_GET_OBJ(obj)->u.date.fields_cache;
        if (fc) {
            c = &fc->e[is_local & 1];
            if (c->valid && c->time == dval) {
                if (!is_local || js_date_tz_epoch() == c->tz_epoch)
                    goto hit;
                /* the memo refilled since we computed this breakdown:
                 * the offset for our second may have moved (DST start,
                 * TZ change).  Re-resolve it -- one memo lookup -- and
                 * only serve the cache if it is still the same. */
                if ((int32_t)-getTimezoneOffset((int64_t)dval) == c->fields[8]) {
                    c->tz_epoch = js_date_tz_epoch();
                    goto hit;
                }
                /* offset changed: fall through and recompute */
            }
            c = NULL;
        }
    }
    ...
    if (likely(!isnan(dval))) {
        /* refill the cache (all nine values fit int32: |y| <= 275760,
         * |tz| <= 1440, the rest by construction) */
        JSObject *p = JS_VALUE_GET_OBJ(obj);
        JSDateFieldsCache *fc = p->u.date.fields_cache;
        JSDateFieldsCacheEntry *c;
        if (unlikely(fc == NULL)) {
            fc = js_date_cache_alloc(ctx);
            if (unlikely(!fc))
                return TRUE; /* cache is an optimization: answer anyway */
            p->u.date.fields_cache = fc;
        }
        c = &fc->e[is_local & 1];
        c->time = dval;
        c->tz_epoch = is_local ? js_date_tz_epoch() : 0;
        c->fields[0] = (int32_t)y;
        c->fields[1] = (int32_t)i;
        c->fields[2] = (int32_t)(days + 1);
        c->fields[3] = (int32_t)h;
        c->fields[4] = (int32_t)m;
        c->fields[5] = (int32_t)s;
        c->fields[6] = (int32_t)ms;
        c->fields[7] = (int32_t)wd;
        c->fields[8] = (int32_t)tz;
        c->valid = 1;
    }
    return TRUE;

hit:
    /* serve the cached breakdown (values are exact: every field fits
     * int32 losslessly) */
    for(i = 0; i < 9; i++)
        fields[i] = c->fields[i];
    return TRUE;
...
static double set_date_fields(double fields[minimum_length(7)], int is_local) {
    ...
    if (mn < 0)
        mn += 12;
    if (!(ym >= -271821 && ym <= 275760))   /* NaN fails both bounds */
        return NAN;
                               ...
                               int argc, JSValueConst *argv, int magic)
{
    ...
    if (part & 1) { /* date part */
        switch(fmt) {
        case 0:
            memcpy(buf + pos, day_names + wd * 3, 3); pos += 3;
            buf[pos++] = ',';
            buf[pos++] = ' ';
            pos += date_put_u2(buf + pos, d);
            buf[pos++] = ' ';
            memcpy(buf + pos, month_names + mon * 3, 3); pos += 3;
            buf[pos++] = ' ';
            pos += date_put_year(buf + pos, sizeof(buf) - pos, y);
            buf[pos++] = ' ';
            break;
        case 1:
            memcpy(buf + pos, day_names + wd * 3, 3); pos += 3;
        ...
        case 2:
            if (likely((unsigned)y <= 9999)) {
                pos += date_put_u4(buf + pos, y);
            } else {
                pos += snprintf(buf + pos, sizeof(buf) - pos,
                                "%+07d", y);
            }
            buf[pos++] = '-';
            pos += date_put_u2(buf + pos, mon + 1);
            buf[pos++] = '-';
            pos += date_put_u2(buf + pos, d);
            buf[pos++] = 'T';
            break;
    ...
    if (part & 2) { /* time part */
        ...
        case 0:
        case 1:
            pos += date_put_u2(buf + pos, h);
            buf[pos++] = ':';
            pos += date_put_u2(buf + pos, m);
            buf[pos++] = ':';
            pos += date_put_u2(buf + pos, s);
            memcpy(buf + pos, " GMT", 4); pos += 4;
            if (fmt == 1) {
                if (tz < 0) {
                    buf[pos++] = '-';
                    tz = -tz;
        ...
        case 2:
            ...
            pos += date_put_u2(buf + pos, s);
            buf[pos++] = '.';
            pos += date_put_u3(buf + pos, ms);
            buf[pos++] = 'Z';
            break;
        case 3:
            pos += date_put_u2(buf + pos, (h + 11) % 12 + 1);
            buf[pos++] = ':';
            pos += date_put_u2(buf + pos, m);
            buf[pos++] = ':';
            pos += date_put_u2(buf + pos, s);
...
/* ============================================================================
 * non-ES Date.prototype extensions. English/ISO, local-time, IMMUTABLE
 * (producers return a NEW Date, never mutating the receiver -- a documented
 * divergence). No untrusted-string parsing here (Date.create's NLP parser +
 * per-locale formatting are deferred); every arg is coerced to a C local first
 * (reentrancy-safe, no native handle held); no DoS loops.
 * ========================================================================== */
...
int JS_AddIntrinsicDate(JSContext *ctx)
{
    ...
    if (JS_IsException(obj))
        return -1;
    /* non-ES Date.prototype extensions, non-enumerable. (skipped when
     * the prototype-extension opt-out is armed.) */
