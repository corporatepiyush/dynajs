...
static JSValue js_async_from_sync_iterator_close_wrap(JSContext *ctx,
                                                      ...
                                                      int magic, JSValue *func_data)
{
    /* x3: func_data[0] is the wrapper itself. After this close the
       wrapper's sync iterator is cleared (the [[Done]]-flag semantics of
       syncIteratorRecord), so a second close - e.g. the loop-level
       AsyncIteratorClose after the rejected value propagated - resolves
       with { done: true } without calling the sync `return` again. */
    JSAsyncFromSyncIteratorData *s =
        JS_GetOpaque(func_data[0], JS_CLASS_ASYNC_FROM_SYNC_ITERATOR);
    JS_Throw(ctx, JS_DupValue(ctx, argv[0]));
    if (s && !JS_IsUndefined(s->sync_iter)) {
        JS_IteratorClose(ctx, s->sync_iter, TRUE);
        JS_FreeValue(ctx, s->sync_iter);
        s->sync_iter = JS_UNDEFINED;
    }
    return JS_EXCEPTION;
}

static JSValue js_async_from_sync_iterator_close_wrap_func_create(JSContext *ctx, JSValueConst wrapper)
{
                                                ...
                                                int magic)
{
    ...
    if (JS_IsUndefined(s->sync_iter)) {
        /* x3: the sync iterator was already closed (syncIteratorRecord
           [[Done]]): resolve with { done: true } for next/return/throw. */
        err = js_create_iterator_result(ctx, JS_DupValue(ctx, JS_UNDEFINED), TRUE);
        ...
        goto reject;
    {
        ...
        if (done || magic == GEN_MAGIC_RETURN) {
            ...
            resolve_reject[1] =
                js_async_from_sync_iterator_close_wrap_func_create(ctx, this_val);
