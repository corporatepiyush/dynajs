...
static const char *const pxy_ctor_keys[] =
    { "port", "maxConns", "idleTimeoutMs", "connectTimeoutMs", "upstream" };
...
struct dyn_pxy {
    ...
    /* The RUNTIME, for teardown: a dispose reached from the runtime's final
       ...
       so the reactor release there must go through the rt form. */
    JSRuntime *rt;
    dyn_aio_t *aio;
    int listen_fd;
    uint16_t port;
    ...
    uint16_t *up_port;
    struct sockaddr_storage *up_sa;   /* pre-resolved at construction */
    socklen_t *up_salen;
    int *up_fam;
    int n_up, next_up;
                          ...
                          unsigned len, void *udata)
{
    ...
    /* round-robin over PRE-RESOLVED upstreams: no getaddrinfo on accept */
    idx2 = p->next_up;
...
static void dyn_pxy_dispose(void *native)
{
    ...
    if (p->aio) {
        ...
        /* rt, not ctx: a dispose from the runtime's final sweep runs after
           JS_FreeContext has freed the context. */
                             ...
                             int idx)
{
    ...
    /*each upstream entry accepts {host, port} only. Nothing is owned
     * here yet, so the refusal is a plain -1 to the caller's cleanup. */
    ...
    /* PRE-RESOLVE here: upstreams are static configuration, so a blocking
       getaddrinfo belongs at construction, not on the reactor inside every
       accepted connection. A bad name fails the constructor loudly. */
    {
        char errbuf[160];
        if (dyn_aio_resolve(p->up_host[idx], p->up_port[idx],
            ...
            /* The host was strdup'd before the resolve; dispose only frees
               slots below n_up, and the failing slot never joins that range,
...
static int pxy_read_opts(JSContext *ctx, dyn_pxy_t *p, JSValueConst o)
{
    ...
    /*reject unknown keys BEFORE any option is read or upstream is
     * resolved (the caller disposes the half-built proxy on -1). */
    ...
    /* Conversion failures propagate (like tcp_read_bounds): the old
       `JS_ToInt32(...) == 0 && n >= 0` silently DROPPED the option when the
       value threw (a Symbol, a throwing valueOf), so the proxy ran on the
       default with no error. The value/range gating stays as it was. */
