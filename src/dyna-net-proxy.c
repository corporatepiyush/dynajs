 ...
 * the portable path buffers in userspace. See NET_PLAN.md.
 */
#include "dyna-nat.h"
#include "dyna-aio.h"
                           ...
                           unsigned len, void *udata)
{
    ...
    pr->deadline_ms = 0;
    pr->last_ms = dyn_timer_now_ms();
    pr->refs += 2;                     /* before arming: a recv can complete inline */
    if (dyn_aio_recv(p->aio, pr->cfd, 0, 1, pxy_recv_client, pr) < 0 ||
        dyn_aio_recv(p->aio, pr->ufd, 0, 1, pxy_recv_upstream, pr) < 0) {
        pr->refs -= 2;
        pxy_pair_close(pr);
        pxy_pair_unref(pr);
        return;
