/* typeof result strings are pre-interned per runtime: OP_typeof otherwise
   allocates a fresh string per execution. Unbuilt slots (tag != STRING) are
   filled on first use; the zeroed runtime also triggers the build. */
static JSValue js_typeof_string(JSContext *ctx, JSAtom atom)
{
    ...
    switch(atom) {
    case JS_ATOM_undefined: idx = 0; break;
    case JS_ATOM_function:  idx = 1; break;
    case JS_ATOM_object:    idx = 2; break;
    case JS_ATOM_string:    idx = 3; break;
    case JS_ATOM_number:    idx = 4; break;
    case JS_ATOM_boolean:   idx = 5; break;
    case JS_ATOM_symbol:    idx = 6; break;
    case JS_ATOM_bigint:    idx = 7; break;
    default:
...
#define CASE(op)        case_ ## op: __asm__ volatile("label_" #op ":\n.globl label_" #op); dummy_case_ ## op
#else
 ...
 restart:
    for(;;) {
        ...
        CASE(OP_object):
            ...
            /* A literal whose whole key set is statically known is created
               directly at its final shape, turning each following
               OP_define_field into a plain slot overwrite instead of a shape
               transition + property-array growth. The table is keyed by the
               offset of this very opcode; functions have a handful of literal
               sites, so the scan is shorter than the transition it saves, and
               the common case (no table at all) is one predictable branch. */
            if (unlikely(b->obj_presize != NULL)) {
                ...
                for (si = 0; si < ps->site_count; si++) {
                    if (ps->sites[si].bc_offset == off) {
                        *sp++ = js_object_presize_new(ctx, &ps->sites[si]);
                        if (unlikely(JS_IsException(sp[-1])))
                            goto exception;
                        BREAK;
                    }
                ...
                if (likely(JS_VALUE_GET_TAG(obj) == JS_TAG_OBJECT)) {   \
                    ...
                    for(;;) {                                           \
                        ...
                        if (unlikely(p->is_exotic)) {                   \
                            /* A plain Array's exotic own-property method \
                               covers integer indices only, so a non-index \
                               miss is genuine: keep walking the proto   \
                               chain (arr.map no longer pays the slow    \
                               path). Other exotics keep the slow path. */ \
                        ...
                        if (!p) {                                       \
                           ...
                           JS_VALUE_GET_TAG(obj) == JS_TAG_STRING) {    \
                    val = JS_NewInt32(ctx, JS_VALUE_GET_STRING(obj)->len); \
                } else if (is_length &&                                 \
                           JS_VALUE_GET_TAG(obj) == JS_TAG_STRING_ROPE) { \
                    val = JS_NewInt32(ctx,                              \
                                      JS_VALUE_GET_STRING_ROPE(obj)->len); \
                } else {                                                \
                name ## _slow_path:                                     \
                    sf->cur_pc = pc;                                    \
        ...
        CASE(OP_put_array_el):
            {
                ...
                if (likely(JS_VALUE_GET_TAG(sp[-3]) == JS_TAG_OBJECT &&
                    ...
                    /* Plain Array first: one compare vs the TypedArray range's
                     * two, and it is the common case. Mutually exclusive. */
        ...
        CASE(OP_div):
            {
                ...
                if (likely(JS_VALUE_IS_BOTH_INT(op1, op2))) {
                    ...
                    /* __JS_NewFloat64, not JS_NewFloat64: the float64 arm of
                       js_binary_arith_slow does not normalize an exact result
                       back to an int, and the two must agree exactly. */
                    sp[-2] = __JS_NewFloat64(ctx, d1 / d2);
                    sp--;
                } else {
                    goto binary_arith_slow;
...
static void async_func_free_frame(JSRuntime *rt, JSAsyncFunctionState *s)
{
    ...
    /* sf->cur_sp is NULL while the function is running. It is also left NULL
     * when a body completed via a SYNC return under the async machinery
     * (done: already freed the arg/var/stack values and returned without
     * restoring cur_sp -- see async_func_resume, which fails closed on it):
     * there are then no stack values left to free. */
    if (sf->cur_sp != NULL) {
...
static JSValue async_func_resume(JSContext *ctx, JSAsyncFunctionState *s)
{
    ...
    if (JS_IsException(ret) || JS_IsUndefined(ret)) {
        ...
        if (JS_IsUndefined(ret)) {
            /* A completed generator/async body must exit through
               OP_return_async/await/yield (done_generator), which stores
               sf->cur_sp. A body that reaches a SYNC return (OP_return /
               OP_return_undef / tail call) instead returns via done:, which
               leaves sf->cur_sp NULL -- reading cur_sp[-1] here is then a
               NULL[-1] SEGV (fuzz_bceval sustained run #2). The reader pins
               the module function's kind and the body terminator, but a
               REACHABLE sync return is statically indistinguishable from a
               dead one (the compiler emits dead sync returns in async
               bodies), so this side fails closed instead of dereferencing. */
            if (unlikely(sf->cur_sp == NULL)) {
                /* keep the normal completion path below: is_completed,
                   close_var_refs and async_func_free_frame must all run */
