...
/* `using` declaration runtime support (defined in src/runtime/using.inc.c,
   included after disposable.inc.c). */
                               ...
                               int argc, JSValue *argv, int flags)
{
    ...
    JSVarRef **var_refs;
    size_t alloca_size;
    /* Proper tail calls: instead of C-recursing into the callee (the
       audit's "no TCO": tail recursion overflows the stack at ~2600
       frames), a tail call whose callee fits the already-allocated frame
       tears this frame down and re-enters the setup below, so the C stack
       stays constant. tc_owner_* hold the one reference we own for the
       currently running function/'this' (the caller's borrowed reference
       is never freed here); they are released once the next frame has
       been rebound, so no live frame or backtrace can dangle. */
    ...
    /* Object-literal boilerplate recorder (CONFIG_LITERAL_BOILERPLATE):
       from a literal's OP_object until the next OP_object / foreign define /
       cancel, bb_rec_obj is the literal object created at bb_rec_off,
       bb_rec_site is the presize site covering it (NULL until one is
       recorded) and bb_rec_nf counts the defines applied this run.
       bb_rec_obj is UNOWNED and only ever compared (never dereferenced):
       every OP_define_field run in generated bytecode is preceded by its
       own OP_object (literals, the destructuring excludeList target, the
       hoisted-var object; the sparse-array variant uses index atoms and is
       excluded), so the state is always reset by the run's own OP_object
       before a define can observe it -- a stale pointer can neither be
       dereferenced nor false-match. bb_rec_site points into this frame's
       bytecode obj_presize table, which never moves (sites are appended in
       place, capacity-bounded) and outlives the frame. */
    JSObject *bb_rec_obj = NULL;
    JSObjectPresizeSite *bb_rec_site = NULL;
    uint32_t bb_rec_off;
    int bb_rec_nf;
#define BB_REC_START(val, off_) do { \
        bb_rec_obj = JS_VALUE_GET_OBJ(val); \
        bb_rec_off = (off_); \
        bb_rec_site = NULL; \
        bb_rec_nf = 0; \
    } while (0)
#define BB_REC_CLEAR() do { \
        bb_rec_obj = NULL; \
        bb_rec_site = NULL; \
    } while (0)
#endif
...
#define BREAK           SWITCH(pc)
 ...
 restart_call: /* tail calls re-enter here with func_obj/this_obj/argc/argv
                  (tc_argv) and b/p rebound by the trampoline in done: */
    ...
    if (unlikely(tc_entry)) {
        ...
        if (arg_allocated_size == 0) {
            /* enough arguments: borrow the heap buffer like a caller's
               argv; the values and the buffer are released at this
               frame's teardown (see done:) */
            arg_buf = tc_argv;
            tc_argv_live = TRUE;
        } else {
            /* missing arguments: move the values into the frame and pad */
            arg_buf = local_buf;
            for(i = 0; i < argc; i++)
                arg_buf[i] = argv[i];
        ...
        CASE(OP_object):
                        ...
                        if (ps->sites[si].bc_offset == off) {
                            *sp++ = js_object_presize_new(ctx,
                            ...
                            if (unlikely(JS_IsException(sp[-1])))
                                goto exception;
#if CONFIG_LITERAL_BOILERPLATE
                            /* keep watching a replayed site: a run with
                               more fields than recorded grows it */
            ...
            *sp++ = JS_NewObject(ctx);
            if (unlikely(JS_IsException(sp[-1])))
                goto exception;
#if CONFIG_LITERAL_BOILERPLATE
            /* CONFIG_LITERAL_BOILERPLATE: watch this literal's define run;
               an eligible run records its shape as the site's boilerplate
               (see OP_define_field). */
        ...
        CASE(OP_tail_call):
            ...
            has_call_argc:
                ...
                if (unlikely(opcode == OP_tail_call) && b->func_kind == JS_FUNC_NORMAL) {
                    /* proper tail call: reuse this C frame when the callee
                       is a plain bytecode function whose frame fits the
                       already-allocated region; anything else falls back
                       to the recursive call below */
                    JSObject *np;
                    JSFunctionBytecode *nb;
                    tc_func = call_argv[-1];
                    if (JS_VALUE_GET_TAG(tc_func) == JS_TAG_OBJECT &&
                        (np = JS_VALUE_GET_OBJ(tc_func))->class_id == JS_CLASS_BYTECODE_FUNCTION &&
                        (nb = np->u.func.function_bytecode)->func_kind == JS_FUNC_NORMAL) {
                        /* pad only when arguments are missing; otherwise the
                           heap tc_argv stays borrowed like a caller's argv
                           (freed at this frame's teardown, after any mapped-
                           arguments var_refs have been closed) */
                        tc_arg_alloc = (call_argc < nb->arg_count) ?
                            nb->arg_count : 0;
                        if (sizeof(JSValue) * (tc_arg_alloc + nb->var_count +
                                               nb->stack_size) +
                            sizeof(JSVarRef *) * nb->var_ref_count <= alloca_capacity) {
                            tc_argv = js_malloc(ctx, sizeof(JSValue) * call_argc);
                            if (tc_argv) {
                                int j;
                                memcpy(tc_argv, call_argv,
                                       sizeof(JSValue) * call_argc);
                                /* the moved values are owned by tc_argv now:
                                   neutralize the slots so the teardown free
                                   loop cannot free them twice */
        ...
        CASE(OP_tail_call_method):
            {
                ...
                if (unlikely(opcode == OP_tail_call_method) && b->func_kind == JS_FUNC_NORMAL) {
                    /* proper tail call through a method: 'this' travels
                       with the call (see OP_tail_call) */
                    JSObject *np;
                    JSFunctionBytecode *nb;
                    tc_func = call_argv[-1];
                    if (JS_VALUE_GET_TAG(tc_func) == JS_TAG_OBJECT &&
                        ...
                        if (sizeof(JSValue) * (tc_arg_alloc + nb->var_count +
                            ...
                            if (tc_argv) {
                                ...
                                tc_argc = call_argc;
                                tc_this = call_argv[-2];
                                call_argv[-2] = JS_UNDEFINED; /* this */
                                call_argv[-1] = JS_UNDEFINED; /* func */
                                for(j = 0; j < call_argc; j++)
                                    call_argv[j] = JS_UNDEFINED;
                                tc_pending = TRUE;
                                goto done;
...
#define JS_THROW_ERROR_ITERATOR_THROW 4
        ...
        CASE(OP_for_await_catch_body):
            /* async for-await loop: the iteration machinery (next/await/
               get_value_done) is over, so an exception from here on is
               body-originated and must await the iterator close: restore
               the loop catch offset (now three slots down, below the
               value/done pair) to the body handler (x3 fix1: while the
               machinery ran, the marker held -1, which the unwinder treats
               as "next()-originated: done, no close"). */
            {
                int32_t diff;
                diff = get_u32(pc);
...
#if CONFIG_FIELD_IC
            if (b->field_ic) {
                /* monomorphic inline cache: identical shape => the same
                   plain-data own property at the same offset */
                uint32_t ic_site = (uint32_t)(pc - b->byte_code_buf) + 1;
                JSFieldIC *ic = &b->field_ic[(ic_site * 2654435761u) &
                                              b->field_ic_mask];
                if (ic->site == ic_site &&
                    likely(JS_VALUE_GET_TAG(sp[-1]) == JS_TAG_OBJECT)) {
                    JSObject *p2 = JS_VALUE_GET_OBJ(sp[-1]);
                    if (p2->shape == ic->shape) {
                        JSValue val2 = JS_DupValue(ctx,
                                                   p2->prop[ic->off].u.value);
                        JS_FreeValue(ctx, sp[-1]);
                        sp[-1] = val2;
                        pc += 4;
                        BREAK;
                ...
                /* populate/retire: own plain-data property only */
                if (likely(JS_VALUE_GET_TAG(sp[-1]) == JS_TAG_OBJECT)) {
                    JSObject *p2 = JS_VALUE_GET_OBJ(sp[-1]);
                    JSProperty *pr2;
                    JSShapeProperty *prs2;
                    if (!p2->is_exotic &&
                        (prs2 = find_own_property(&pr2, p2, get_u32(pc))) &&
                        !(prs2->flags & JS_PROP_TMASK)) {
                        if (ic->shape)
                            js_free_shape(rt, ic->shape);
                        ic->shape = js_dup_shape(p2->shape);
                        ic->off = (uint32_t)(pr2 - p2->prop);
                        ic->site = ic_site;
                    } else if (ic->site == ic_site) {
                        if (ic->shape) {
                            js_free_shape(rt, ic->shape);
                            ic->shape = NULL;
                        }
        ...
        CASE(OP_define_field):
            {
                ...
                if (unlikely(bb_rec_obj != NULL)) {
                    /* sp[-1] is the object the define just landed on. While
                       it is the watched literal, track the run: the shape
                       must grow by exactly this define (or land on the
                       recorded prefix), with this atom as a plain C_W_E
                       property, staying a plain object on Object.prototype.
                       Any foreign define (nested literal, destructuring
                       target) cancels the recording. */
                    if (JS_VALUE_GET_OBJ(sp[-1]) == bb_rec_obj &&
                        !__JS_AtomIsTaggedInt(atom)) {
                        JSObject *po = bb_rec_obj;
                        JSShape *sh = po->shape;
                        int prefix = bb_rec_site ?
                            bb_rec_site->field_count : 0;
                        int k = bb_rec_nf + 1; /* defines incl. this one */
                        int expect = k > prefix ? k : prefix;

                        if (sh->prop_count == expect &&
                            po->class_id == JS_CLASS_OBJECT &&
                            !po->is_exotic && !po->fast_array &&
                            sh->proto ==
                              get_proto_obj(ctx->class_proto[JS_CLASS_OBJECT])) {
                            JSShapeProperty *pr = get_shape_prop(sh);
                            if (pr[k - 1].atom == atom &&
                                pr[k - 1].flags == JS_PROP_C_W_E) {
...
#define OP2_READ_LOC_LOC(v1, v2)                                              \
            ...
            CASE2(OP2_mul_loc_loc):
                ...
                BREAK;
            /* ── BRANCH shard: fused switch-probe superinstructions ──
               Shared strict-eq ladder: computes `res` for two BORROWED
               operands (identical tag ladder to OP_STRICT_EQ_BRANCH / OP_CMP_
               STRICT_EQ minus the frees — the probe leaves v on the stack and
               the cpool entry is immortal; the varprop variant frees its OWN
               loaded value separately). js_strict_eq2 borrows both. */
#define OP2_PROBE_STRICT_EQ(o1, o2)                                      \
                ...
                if (likely(tag1 == JS_TAG_INT)) {                        \
                    if (tag2 == JS_TAG_INT) {                            \
                        ...
                        res = (JS_VALUE_GET_INT(o1) ==                   \
                               JS_VALUE_GET_FLOAT64(o2));                \
                    } else {                                             \
                        res = FALSE;                                     \
                    }                                                    \
                } else if (JS_TAG_IS_FLOAT64(tag1)) {                    \
                    if (tag2 == JS_TAG_INT) {                            \
                        res = (JS_VALUE_GET_FLOAT64(o1) ==               \
                               JS_VALUE_GET_INT(o2));                    \
                    } else if (JS_TAG_IS_FLOAT64(tag2)) {                \
                        res = (JS_VALUE_GET_FLOAT64(o1) ==               \
                               JS_VALUE_GET_FLOAT64(o2));                \
                    ...
                    res = (tag2 == JS_TAG_OBJECT &&                      \
                           JS_VALUE_GET_OBJ(o1) == JS_VALUE_GET_OBJ(o2)); \
                } else if (tag1 == JS_TAG_NULL ||                        \
                           tag1 == JS_TAG_UNDEFINED) {                   \
                    res = (tag1 == tag2);                                \
            ...
            CASE2(OP2_streq_const_if_false):
                {
                    /* peek v, strict-compare against cpool[k], branch when
                       NOT equal; v stays on the stack (net effect of the
                       folded dup/push/strict_eq_if_false chain). Operands:
                       [label:u32 (rel. to itself)][cpool idx:u8]. */
                    JSValue op1 = sp[-1];
            ...
            CASE2(OP2_streq_varprop_if_false):
                {
                    /* peek v; read var_refs[n] exactly like OP_get_var (TDZ
                       ReferenceError for lexical closure vars, undeclared-
                       global fallback); load .atom exactly like OP_get_field
                       (inline proto walk, getter receiver = the object, may
                       throw -> exception); strict-compare v against it, free
                       the loaded value, branch when NOT equal; v stays.
                       Operands: [label:u32][var_ref:u16][atom:u32]. */
                    ...
                    int32_t diff = (int32_t)get_u32(pc);
                    {
                        ...
                        obj_owned = TRUE; /* getter/proxy may rebind the cell */
            ...
            /* ── CALL shard: fused method-call superinstructions ──
               `get_field2 <atom>; [<one hot arg op>]; call_method` folded to
               ONE dispatch (resolve_labels matched BACKWARD at the call site,
               audit §19). The body performs the property load exactly like
               GET_FIELD_INLINE(get_field2), materializes the one argument
               with the EXACT semantics of its source op, then calls exactly
               like OP_call_method; the C fast path dispatches plain
               JS_CLASS_C_FUNCTION callees straight through js_call_c_function
               (identical to JS_CallInternal's non-bytecode routing). Stack
               footprint is [this] -> [result] (n_pop 1, n_push 1): func and
               arg live in C locals, never on the interpreter stack, so the
               frame allocation from the static n_pop/n_push is exact. */
#define OP2_CALL_METHOD(argc_, kind_, load_d_, adv_)                        \
            {                                                               \
                ...
                int call_argc = (argc_);                                    \
                JSValue mf_argv1[2]; /* [0]: the argument — the epilogue    \
                                        releases its POST-call content      \
                                        (borrow-argv contract, F1);         \
                                        [1]: defined padding, because       \
                                        js_call_c_function's f_f/f_f_f      \
                                        bodies read arg_buf[1] even when    \
                                        argc == length (sqrt et al) and     \
                                        the arity guard then skips the pad  \
                                        alloca on the fallback path */      \
                mf_atom = get_u32(pc);                                      \
                mf_this = sp[-1]; /* stays on the stack: GC-scanned until   \
                                     the op replaces it with the result */  \
                mf_obj = mf_this;                                           \
                mf_loaded = FALSE;                                          \
                if (likely(JS_VALUE_GET_TAG(mf_this) == JS_TAG_OBJECT)) {   \
                    ...
                    for(;;) {                                               \
                        ...
                        if (unlikely(mf_p->is_exotic)) {                    \
                            /* a plain Array's exotic own-property method   \
                               covers integer indices only (see the array   \
                               miss note in GET_FIELD_INLINE) */            \
                            if (mf_p->class_id != JS_CLASS_ARRAY ||         \
                                __JS_AtomIsTaggedInt(mf_atom)) {            \
                ...
                if (!mf_loaded) {                                           \
                    ...
                    if (unlikely(JS_IsException(mf_func)))                  \
                        goto exception; /* nothing owned yet */             \
                }                                                           \
                /* the get_field2 value, held in a C local: the op's stack  \
                   footprint is exactly [this] -> [result] (n_pop 1,        \
                   n_push 1), matching the static n_pop/n_push the stack    \
                   simulators used for the frame allocation */              \
                mf_arg = JS_UNDEFINED;                                      \
                switch ((kind_)) {                                          \
                case 1: /* <get_loc|get_loc_check|get_arg|push_i16> */      \
                    mf_form = pc[4];                                        \
                    mf_idx = get_u16(pc + 5);                               \
                    if (mf_form == 3) { /* push_i16 */                      \
                        mf_arg = JS_NewInt32(ctx, get_i16(pc + 5));         \
                        break;                                              \
                    }                                                       \
                    if (mf_form == 1) { /* get_loc_check: TDZ first */      \
                        if (unlikely(JS_IsUninitialized(var_buf[mf_idx]))) {\
                            JS_FreeValue(ctx, mf_func); /* owned */         \
                    ...
                    mf_arg = JS_DupValue(ctx, mf_form == 2                  \
                                               ? arg_buf[mf_idx]            \
                                               : var_buf[mf_idx]);          \
                    break;                                                  \
                case 2: /* <push_i8|push_const8> imm:i8 */                  \
                    mf_form = pc[4];                                        \
                    if (mf_form == 0)                                       \
                    ...
                    break;                                                  \
                case 3: /* push_const cpool:u32 */                          \
                    mf_arg = JS_DupValue(ctx, b->cpool[get_u32(pc + 4)]);   \
                    break;                                                  \
                default: /* method with no argument */                      \
                    break;                                                  \
                }                                                           \
                /* OP_call_method from here on (cur_pc = past the call) */  \
                sf->cur_pc = pc + (adv_);                                   \
                mf_argv1[0] = mf_arg;                                       \
                mf_argv1[1] = JS_UNDEFINED; /* readable pad: see above */   \
                if (likely(JS_VALUE_GET_TAG(mf_func) == JS_TAG_OBJECT) &&   \
                    JS_VALUE_GET_OBJ(mf_func)->class_id ==                  \
                        JS_CLASS_C_FUNCTION) {                              \
                    /* T3 lean C-callee dispatch: for the two cprotos that  \
                       dominate fused method calls (plain generic and       \
                       generic_magic) with enough args to satisfy the C     \
                       arity, replicate js_call_c_function's preamble       \
                       EXACTLY (overflow check, system-frame push, realm)   \
                       and skip its generic machinery (the cproto switch,   \
                       the arity-pad alloca). f_f/f_f_f stay on the         \
                       fallback: their bodies read arg_buf[1] past argc;    \
                       slot [1] is defined padding here (so the fallback    \
                       stays ASan-clean), and the unfused generic stream    \
                       carries the func slot there — both convert to NaN    \
                       without throwing, results identical. */              \
                    JSObject *mf_cf = JS_VALUE_GET_OBJ(mf_func);            \
                    JSCFunctionEnum mf_cp = mf_cf->u.cfunc.cproto;          \
                    if (unlikely(call_argc < mf_cf->u.cfunc.length) ||      \
                        unlikely(mf_cp != JS_CFUNC_generic &&               \
                                 mf_cp != JS_CFUNC_generic_magic)) {        \
                        ...
                        /* pre-built system frame: same fields and same    \
                           values js_call_c_function sets (nothing more,   \
                           nothing less -- backtraces and GC must not      \
                           tell the difference) */                          \
                        ...
                        mf_csf.arg_count = call_argc;                       \
                        mf_csf.arg_buf = mf_argv1;                          \
                        if (mf_cp == JS_CFUNC_generic)                      \
                            ret_val = mf_cf->u.cfunc.c_function.generic(    \
                                        mf_realm, mf_this, call_argc,       \
                                        (JSValueConst *)mf_argv1);          \
                        else                                                \
                            ret_val = mf_cf->u.cfunc.c_function.            \
                                generic_magic(mf_realm, mf_this,            \
                                              call_argc,                    \
                                              (JSValueConst *)mf_argv1,     \
                                              mf_cf->u.cfunc.magic);        \
                    ...
                    /* JS_CallInternal's argv is JSValue * (non-const):     \
                       with flags=0 the callee OWNS-AND-MUTATES the slots */\
                    ret_val = JS_CallInternal(ctx, mf_func, mf_this,        \
                                              JS_UNDEFINED, call_argc,      \
                                              mf_argv1, 0);                 \
                }                                                           \
                if (unlikely(JS_IsException(ret_val))) {                    \
                    /* BORROW-ARGV CONTRACT (F1 fix): flags=0 means         \
                       JS_CallInternal treats mf_argv1 as the callee's arg  \
                       slots (arg_allocated_size=0, arg_buf=argv): a JS     \
                       callee's put_arg may have CONSUMED the pre-call      \
                       ticket (set_value frees the old slot content) and    \
                       stored a different value. Exactly like the generic   \
                       OP_call_method -- which frees call_argv[i] AFTER the \
                       call, and whose in-stack argv the exception unwinder \
                       releases -- we must free the POST-CALL slot content, \
                       never the pre-call mf_arg (a C array is invisible    \
                       to the unwinder). argv slots are not part of the     \
                       callee's local_buf, so its teardown never frees      \
                       them; one release here is exact on every path        \
                       (bytecode callee w/ and w/o param writes, argc <     \
                       arg_count copy path, generator dup path, C callee    \
                       who never consumes). */                              \
                ...
                /* same free order as OP_call_method: this, func, args     \
                   (args = post-call slot content, see the F1 note above) */\
                JS_FreeValue(ctx, sp[-1]); /* this */                       \
                JS_FreeValue(ctx, mf_func);                                 \
                if (call_argc)                                              \
                    JS_FreeValue(ctx, mf_argv1[0]);                         \
                sp--;                                                       \
...
#undef OP2_PROBE_STRICT_EQ
            ...
            CASE2(OP2_using_add):
                {
                    int hint = get_u16(pc);
                    pc += 4; /* loc2 form: two raw u16 slots (second unused) */
                    sf->cur_pc = pc;
            ...
            CASE2(OP2_using_dispose):
                {
                    int flags = get_u16(pc); /* bit0: async, bit1: pending */
                    JSValue ret;
                    pc += 4; /* loc2 form: two raw u16 slots (second unused) */
                    sf->cur_pc = pc;
                    ...
                    JS_FreeValue(ctx, sp[-2]);
                    if (!(flags & 2))
                        JS_FreeValue(ctx, sp[-1]); /* dummy, not a pending */
                    sp[-2] = ret;
                    sp--;
                    if (JS_IsException(ret))
                        goto exception;
...
#define OP_CMP_STRICT_EQ(opcode, inv)                                   \
 ...
 exception:
#if CONFIG_LITERAL_BOILERPLATE
    BB_REC_CLEAR(); /* a half-recorded literal run dies with the frame */
#endif
        ...
        while (sp > stack_buf) {
            ...
            if (JS_VALUE_GET_TAG(val) == JS_TAG_CATCH_OFFSET) {
                ...
                if (pos == 0) {
                    ...
                    /* fix1: async for-await loop's iteration machinery
                       (next()/await/get_value_done). A next()-originated
                       abrupt completion means the iterator signalled done
                       (ForIn/OfBodyEvaluation sets [[Done]] and returns
                       WITHOUT AsyncIteratorClose): drop the loop slots and
                       keep unwinding - no close, no extra microtask hop
                       (node-matching). */
                    JS_FreeValue(ctx, sp[-1]); /* drop the next method */
                    sp--;
                    JS_FreeValue(ctx, sp[-1]); /* drop the iterator */
                    sp--;
                } else {
...
#if CONFIG_LITERAL_BOILERPLATE
    /* release the recorder's reference (covers normal return, generator
       suspension and the tail-call re-entry below, whose restart_call
       does NOT re-run the C local initializers) */
    BB_REC_CLEAR();
#endif
    if (unlikely(tc_argv_live)) {
        /* borrowed heap argv of the torn-down frame: any mapped-arguments
           var_refs were closed above, so the values can be released */
        for(i = 0; i < argc; i++)
    ...
    if (unlikely(tc_pending)) {
        /* enter the pending tail call in this same C frame. The torn-down
           frame is unlinked, so its function/'this' references (owned by
           tc_owner_*, JS_UNDEFINED = none) can be released now; the
           pending tc_func/tc_this refs take their place. Freeing before
           the sf->cur_func rebind is safe: nothing can observe the frame
           in between (no allocation or code runs until restart_call). */
        tc_pending = FALSE;
        ...
        p = JS_VALUE_GET_OBJ(func_obj);
        b = p->u.func.function_bytecode;
        ctx = b->realm; /* may differ from the torn-down frame's realm */
        tc_entry = TRUE;
        goto restart_call;
    }
    JS_FreeValue(ctx, tc_owner_func); /* no-op unless a hop ran here */
    JS_FreeValue(ctx, tc_owner_this);
    return ret_val;
}
                                           ...
                                           JSAsyncGeneratorData *s)
{
    ...
    for(;;) {
        ...
        case JS_ASYNC_GENERATOR_STATE_COMPLETED:
            ...
            /* the generator is done: keep draining the remaining queued
               requests (concurrent next() consumers otherwise wait on
               promises that are never resolved and the process exits
               with the job queue empty); only return() pauses here for
               its awaited completion value */
            break;
                                                   ...
                                                   int magic, JSValue *func_data)
{
    ...
    if (magic >= 2) {
        ...
        /* the return() request is settled: keep draining the remaining
           queued requests (spec AsyncGeneratorAwaitReturn resumes
           AsyncGeneratorResumeNext after the awaited completion value
           settles); otherwise requests queued behind return() on a
           generator that completed while they were queued are never
           serviced and their consumers hang */
