/* Markdown for dyna:html (design 07): CommonMark's core plus the GFM pieces
   ...
   input verbatim is an XSS hole with a nice API. Full API: API.md. */
...
static int md_url_ok(const char *s, size_t n)
{
    ...
    for (; i < n; i++) {
        ...
        if (k + 1 >= sizeof scheme)
            return 1;                   /* far too long to be a scheme, so it
                                           is a relative path, not one */
        scheme[k++] = (char)ht_lower(c);
    }
    if (i >= n)
        return 1;                       /* no colon at all */
    scheme[k] = 0;
