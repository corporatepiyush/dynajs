...
typedef struct {
    ...
    uint32_t seg_off, nseg;             /* cached dotted path atoms */
    uint8_t  is_dot;                    /* path is single "." */
} tpl_node_t;
...
typedef struct {
    ...
    tpl_node_t *node;
    uint32_t    n, cap;
    JSAtom     *atoms;                  /* pooled path atoms */
    uint32_t    natoms, cap_atoms;
    JSRuntime  *rt;                     /* for freeing atoms */
...
/* Cache dotted path as atoms for fast lookup without re-parsing or re-atomizing. */
static int tpl_cache_path(JSContext *ctx, tpl_t *t, uint32_t idx)
{
    tpl_node_t *nd = &t->node[idx];
    const char *p = t->src + nd->a;
    size_t n = nd->b - nd->a;
    size_t segs, st, i;
    ...
    for (i = 0; i < n; i++)
        if (p[i] == '.')
            segs++;
    if (t->natoms + segs > t->cap_atoms) {
        size_t nc = t->cap_atoms ? t->cap_atoms * 2 : 16;
        JSAtom *np;
        while (nc < t->natoms + segs)
            nc *= 2;
        np = (JSAtom *)realloc(t->atoms, nc * sizeof *np);
        if (!np)
            return -1;
        t->atoms = np;
        t->cap_atoms = (uint32_t)nc;
        if (!t->rt)
            t->rt = JS_GetRuntime(ctx);
    }
    nd->seg_off = t->natoms;
    nd->nseg = (uint32_t)segs;
    st = 0;
    for (i = 0; i <= n; i++) {
        if (i == n || p[i] == '.') {
            JSAtom a = JS_NewAtomLen(ctx, p + st, i - st);
            if (a == JS_ATOM_NULL)
                return -1;
            t->atoms[t->natoms++] = a;
            st = i + 1;
...
static int tpl_compile(JSContext *ctx, tpl_t *t, char *err, size_t errsz)
{
    ...
    while (i < t->slen) {
        ...
        {
            uint32_t at;
            if (tpl_emit(t, raw || sigil == '&' ? TN_RAW : TN_VAR, a, b, &at) < 0)
                goto oom;
            if (tpl_cache_path(ctx, t, at) < 0)
                goto oom;
        }
...
static const uint8_t TPL_ESC_SET[5] = { '<', '>', '&', '"', '\'' };
#define TPL_SIMD_MIN 64
...
static void tpl_escape(hb_t *b, const char *s, size_t n)
{
    ...
    while (i < n) {
        run = i;
        /* Same window rule as ht_escape: the SIMD fallback cannot be gated
           on the tail length -- when the scalar probe filled and the tail is
           short, s[i] was never examined, and the switch below rewrites that
           unexamined byte. Scan whatever the tail is. */
        { size_t probe = i + TPL_SIMD_MIN;
          if (probe > n) probe = n;
          while (i < probe && !TPL_ESC[(unsigned char)s[i]]) i++;
          if (i == probe && i < n) {
              size_t k = simd.find_first_of((const uint8_t *)s + i, n - i,
                                            TPL_ESC_SET, sizeof TPL_ESC_SET);
              i = (k == (size_t)-1) ? n : i + k;
          } }
...
/* One dotted path, resolved against the innermost scope that HAS its head --
   ...
   Own data properties only: a getter would run code. Uses cached atoms. */
static JSValue tpl_lookup(tpl_r_t *r, const tpl_node_t *nd)
{
    int d;
    ...
    if (nd->nseg == 0) {
        /* Fallback for nodes without cached atoms (should not happen). */
        const char *path = r->t->src + nd->a;
        size_t n = nd->b - nd->a;
        if (n == 1 && path[0] == '.')
            return JS_DupValue(r->ctx, r->scope[r->depth]);
        for (d = r->depth; d >= 0; d--) {
    ...
    for (d = r->depth; d >= 0; d--) {
        ...
        for (s = 0; s < nd->nseg; s++) {
            JSPropertyDescriptor desc;
            JSAtom key = r->t->atoms[nd->seg_off + s];
            int got;
            if (!JS_IsObject(cur)) { ok = 0; break; }
            got = JS_GetOwnProperty(r->ctx, &desc, cur, key);
            if (got <= 0 || (desc.flags & JS_PROP_GETSET)) {
...
static int tpl_section(tpl_r_t *r, const tpl_node_t *nd)
{
    ...
    if (len >= 0) {                     /* an array renders its body per item */
        int64_t i;
        /* Pre-reserve output to avoid repeated reallocs for large arrays. */
        if (len > 16) {
            size_t est = (size_t)len * 32;
            size_t body_text = 0;
            uint32_t k;
            for (k = nd->body; k < nd->end; k++) {
                if (r->t->node[k].kind == TN_TEXT)
                    body_text += r->t->node[k].b - r->t->node[k].a;
            }
            if (body_text)
                est = (size_t)len * (body_text + 16);
            if (est > r->out.cap && est < (16u << 20))
                hb_reserve(&r->out, est);
            else if (len > 1000)
                hb_reserve(&r->out, r->out.n + est);
        }
                            ...
                            int argc, JSValueConst *argv)
{
    ...
    return dyn_plain_wrap(ctx, new_target, dyn_tpl_class_id, t, tpl_free_v);
}
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    r.t = t;
    hb_init(&r.out);
    /* Pre-reserve for large renders: template size + a guess for variables. */
    if (t->slen > 1024) {
        size_t est = t->slen * 2;
        if (est > 256 && est < (8u << 20))
            hb_reserve(&r.out, est);
    }
    r.depth = 0;
