        ...
        if (i < len && buf[i] == '"') {           /* quoted field */
            ...
            while (i < len) {
                ...
                while (i + 8 <= len) {
                    ...
                    while (buf[i] != '"') {       /* hit block: byte-wise up to
                ...
                while (i < len && buf[i] != '"') { /* scalar tail: <8 B left */
                    if (buf_putc(&fld, (char)buf[i])) goto oom;
                    i++;
                }
                if (i >= len) break;              /* no closing quote before EOF */
            quote:
                if (i + 1 < len && buf[i + 1] == '"') {      /* escaped quote */
                    if (buf_putc(&fld, '"')) goto oom;
                    i += 2;
                    continue;
                }
                i++;
...
static int csv_load(JSContext *ctx, const char *path, Table *t, int strict) {
    ...
    int errrow = 0;
    int r = csv_parse_ex(data, dlen, t, strict, &errrow);
    dyn_iobuf_free(&src);
 ...
 * like every other method-originated error ("csv.selectColumnRange: requested
 * window ... exceeds ..."). */
static int range_window(JSContext *ctx, JSValueConst obj, size_t total, size_t maxspan,
                        const char *pfx, size_t *ps, size_t *pe) {
    int64_t start = 0, end = 0; int has_end;
    ...
    if (opt_int(ctx, obj, "start", 0, &start, NULL)) return -1;
    ...
    if (has_end && end > start && (uint64_t)(end - start) > (uint64_t)maxspan) {
        JS_ThrowRangeError(ctx, "%s: requested window %lld exceeds the maximum of %zu rows", pfx, (long long)(end - start), maxspan);
        return -1;
    }
...
static JSValue js_csv_read_column_values_range(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    size_t s, e;
    if (range_window(ctx, OPTS, t.n - 1, 1000, "csv.readColumnValuesRange", &s, &e)) goto done;
    JSValue a = JS_NewArray(ctx);
    uint32_t k = 0;
...
static JSValue js_csv_read_row_range(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    int64_t start = 0; int has_end; int64_t end = 0;
    if (opt_int(ctx, OPTS, "start", 0, &start, NULL)) goto done;
    if (opt_int(ctx, OPTS, "end", 0, &end, &has_end)) goto done;
    if (start < 0) start = 0;
...
static JSValue js_csv_select_column_range(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    size_t s, e;
    if (range_window(ctx, OPTS, t.n - 1, 100, "csv.selectColumnRange", &s, &e)) { free(idx); goto done; }
