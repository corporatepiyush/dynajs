...
/* Saturating add: a counter must never wrap. A Multiset count that wrapped
 * to a small number, or a CMS cell that wrapped, would silently break the
 * "never under-count" contract -- a lower number for a bigger input. */
static uint64_t u64_sat_add(uint64_t a, uint64_t b)
{
    uint64_t s = a + b;
...
static int ds_reserve(void **p, uint32_t *cap, uint32_t need, size_t esz)
{
    ...
    if (need <= *cap)
        return 0;
    /* The multiply below must not wrap on 32-bit hosts. */
    if ((size_t)need > SIZE_MAX / esz)
        return -1;
...
/* The per-process hash seed. Until dyn_ds_hash_seed() is called the default
   is the same compile-time constant as before, so pure-C callers that never
   seed keep today's deterministic behaviour; js_nat_init_all() seeds from
   kernel entropy once per process (audit 13.8.1). */
static atomic_uint_least64_t ds_seed0 = 0x9E3779B97F4A7C15ULL;
static atomic_uint_least64_t ds_seed1 = 0xC2B2AE3D27D4EB4FULL;

/* Seed both words exactly once per process. Every runtime (main and each
   worker context) calls this at its own init, so the first caller wins and
   later ones are no-ops: a table grown under one seed must never be re-hashed
   under another. Atomic because the first call can come from a worker thread
   (js_worker_main -> JS_NewCustomContext -> js_nat_init_all); a lazily
   initialised static is a data race even when every writer stores the same
   value. */
void dyn_ds_hash_seed(uint64_t seed0, uint64_t seed1)
{
    ...
    if (atomic_flag_test_and_set_explicit(&seeded, memory_order_relaxed))
        return;
    atomic_store_explicit(&ds_seed0, seed0, memory_order_relaxed);
    atomic_store_explicit(&ds_seed1, seed1, memory_order_relaxed);
}
...
static uint64_t ix_hash(const char *k, size_t n)
{
    ...
    uint64_t a, b, h;
    /* Relaxed loads compile to a single load on x86-64/aarch64 -- the seed
       must never turn the hot path into a call or a syscall. */
    uint64_t s0 = atomic_load_explicit(&ds_seed0, memory_order_relaxed);
    uint64_t s1 = atomic_load_explicit(&ds_seed1, memory_order_relaxed);
...
static int ix_grow(dyn_ix *ix)
{
    ...
    dyn_ixslot *ns;
    uint32_t i;
    /* Same ceiling as ds_reserve: doubling past 2^30 wraps and would hand
     * back a one-slot table with a full-width mask. */
    if (ix->cap > 0x40000000u)
        return -1;
    ns = (dyn_ixslot *)ds_calloc(nc, sizeof(*ns));
    if (!ns)
        return -1;
    for (i = 0; i < ix->cap; i++)
        if (ix->slot[i].h)
                 ...
                 uint64_t *out)
{
    ...
    if (pos != UINT32_MAX) {
        uint32_t idx = m->ix.slot[pos].idx;
        uint64_t nc = u64_sat_add(cur, (uint64_t)delta);
        m->total = u64_sat_add(m->total, nc - cur);
        m->rec[idx].count = nc;
        if (out)
            *out = nc;
        return 0;
    ...
    m->len++;
    m->total = u64_sat_add(m->total, (uint64_t)delta);
    if (out)
        *out = (uint64_t)delta;
    return 0;
}
...
int dyn_mset_set_count(dyn_mset_t *m, const char *k, size_t n, uint64_t c)
{
    ...
    if (pos != UINT32_MAX) {
        uint32_t idx = m->ix.slot[pos].idx;
        m->total = u64_sat_add(m->total - m->rec[idx].count, c);
        if (c == 0)
            mset_erase(m, idx, pos);
        else
            m->rec[idx].count = c;
...
int dyn_mmap_put(dyn_mmap_t *m, const char *k, size_t n, const dyn_cell_t *v)
{
    ...
    if (pos == UINT32_MAX) {
        /* Every failing step BEFORE the key is installed: a key whose value
         * array then failed used to stay behind as a phantom empty record,
         * violating the "container unchanged on -1" contract. */
        ...
        if (key_init(&r->key, k, n, h) < 0)
            return -1;
        if (ds_reserve((void **)&r->val, &r->cap, 1, sizeof(*r->val)) < 0) {
            free(r->key.p);
            return -1;
        }
        ...
        if (ds_reserve((void **)&r->val, &r->cap, r->len + 1,
                       sizeof(*r->val)) < 0)
            return -1;
    }
    r->val[r->len++] = *v;
                  ...
                  const char *v, size_t vn, int force)
{
    ...
    if (vp != UINT32_MAX) {
        ...
        /* force: the value is taken. Take EVERY failing step BEFORE the
         * eviction -- an OOM after the evict used to lose (key2, v) and
         * insert nothing. ix_reserve on both indexes makes the later
         * ix_insert calls infallible. */
        if (kp != UINT32_MAX) {
            ds_key nv;
            ...
            if (key_init(&nv, v, vn, vh) < 0)
                return -1;
            vp = ix_find(&b->rev, vh, v, vn);   /* reserve may have rehashed */
            bimap_erase(b, b->rev.slot[vp].idx);
            kp = ix_find(&b->fwd, kh, k, kn);      /* indices may have moved */
            {
                uint32_t idx = b->fwd.slot[kp].idx;
                uint32_t op = ix_find(&b->rev, b->rec[idx].val.h,
                                      b->rec[idx].val.p, b->rec[idx].val.n);
                if (op != UINT32_MAX)
                    ix_erase(&b->rev, op);
                free(b->rec[idx].val.p);
                b->rec[idx].val = nv;
        ...
        r = NULL;
        if (ds_reserve((void **)&b->rec, &b->cap, b->len + 1,
                       sizeof(*b->rec)) < 0)
            return -1;
        /* Build into a LOCAL, not the last dense slot: bimap_erase below
         * swaps the evicted record with the last slot, and a pre-built
         * record sitting there is silently clobbered into the hole. */
        {
            bimap_rec tmp;
            tmp.key.p = NULL;
            tmp.val.p = NULL;
            if (key_init(&tmp.key, k, kn, kh) < 0)
                return -1;
            if (key_init(&tmp.val, v, vn, vh) < 0) {
                free(tmp.key.p);
                return -1;
            }
            if (ix_reserve(&b->fwd) < 0 || ix_reserve(&b->rev) < 0) {
                free(tmp.key.p);
                free(tmp.val.p);
                return -1;
            }
            vp = ix_find(&b->rev, vh, v, vn);   /* reserves may have rehashed */
            bimap_erase(b, b->rev.slot[vp].idx);
            b->rec[b->len] = tmp;
            ix_insert(&b->fwd, kh, tmp.key.p, kn, b->len);
            ix_insert(&b->rev, vh, tmp.val.p, vn, b->len);
            b->len++;
        }
        return 0;
    }
                           ...
                           size_t rn, const char *c, size_t cn)
{
    ...
    char *p;
    int i;
    /* 8-byte prefix + both lengths: a 32-bit wrap here would hand the two
     * memcpys a tiny buffer while claiming the full size. */
    if (rn > SIZE_MAX - 8 - cn)
        return -1;
    n = 8 + rn + cn;
    p = (inl && n <= TABLE_PAIR_INLINE) ? inl : (char *)malloc(n);
    if (!p)
        return -1;
    for (i = 0; i < 8; i++)
...
int dyn_rset_add(dyn_rset_t *s, double lo, double hi)
{
    ...
    merged.lo = lo;
    merged.hi = hi;
    /* The bound is the GROWING merge, not the original hi: with the original
     * hi the loop can stop short of a span the merge already covers. Today
     * every export coalesces eagerly so the case is unreachable, but the
     * invariant belongs in the loop, not in the callers. */
    while (j < s->len && s->s[j].lo <= merged.hi) {
...
int dyn_rset_remove(dyn_rset_t *s, double lo, double hi)
{
    ...
    if (!(hi > lo))
        return 0;
    /* First span whose hi > lo -- the same predicate rset_lower() bisects;
     * the scan here was O(n) while add() got the binary search (a RangeSet
     * deleting across a large set paid it per call). */
    i = rset_lower(s, lo);
    j = i;
...
struct dyn_itree {
    ...
    uint32_t built;
    int dirty;
    /* Queries run since the last insert. 2+ with a dirty tail rebuilds it:
     * without this, a burst of inserts just under ITREE_PENDING_MAX followed
     * by a query storm made EVERY query of the storm linearly scan the tail
     * (measured 43.5 us vs 0.5 us clean at 1M entries, 1M queries = 43 s),
     * because the insert-side bound is what lets the tail sit unbuilt. */
    uint32_t qsince;
};
...
/* Below this tail size the scan is sub-microsecond and a rebuild would be
 * pure waste: the burst trigger (second query after inserts) fires only on a
 * tail that is actually expensive to scan. */
#define ITREE_BURST_MIN 1024u
...
static int itree_build(dyn_itree_t *t)
{
    ...
    /* Leave a short tail unindexed; the query scans it. The first query of a
     * read burst still scans, but the SECOND rebuilds -- the burst amortises
     * one sort against all its remaining scans. A tiny tail is not rebuilt:
     * scanning it costs less than the sort it would trigger. */
    t->qsince++;
    if (t->built && (t->qsince < 2 || t->len - t->built < ITREE_BURST_MIN) &&
        t->len - t->built <= ITREE_PENDING_MAX(t->built))
        return 0;
                         ...
                         uint32_t *out, uint32_t cap)
{
    ...
    /* k == 0 (no built interval has lo <= hi) must NOT discard the tail
     * matches collected above: a small unindexed tail sits there precisely
     * when the tail holds the smallest los, and returning 0 silently hid
     * those intervals from every query whose hi was below the built prefix. */
    if (k == 0)
        return found;
...
/* Depth of node i (0-based index): floor(log2(i+1)). Even = min level. One
 * clz instruction on both major ISAs; the loop is the portable fallback. */
static int mm_is_min_level(uint32_t i)
{
#if defined(__GNUC__) || defined(__clang__)
    return (31 - __builtin_clz(i + 1)) % 2 == 0;
#else
    int d = 0;
    uint32_t n = i + 1;
    while (n > 1) { n >>= 1; d++; }
...
dyn_cms_t *dyn_cms_new(uint32_t width, uint32_t depth)
{
    ...
    if (width == 0 || depth == 0)
        return NULL;
    /* The module ceiling is 2^24 entries (DYN_MAX_CAPACITY in the binding
     * layer); the old 2^28 bound let `new CountMinSketch(2^28, 1)` request a
     * single 2 GiB calloc. 2^24 counters is 128 MB: still generous. */
    if ((uint64_t)width * depth > (uint64_t)1 << 24)
        return NULL;
...
{
    (void)s;
    /* Seed 0, like dyn_hll_add: a serialized sketch must probe the same
     * cells in the process that reads it as in the one that filled it, or
     * counts() silently answers under the wrong hash. HLL already chose
     * portability over per-process hardening here; CMS makes the same
     * choice for the same reason (the seeded form made cross-process
     * merge/count garbage -- audit E10-03). */
...
void dyn_cms_add(dyn_cms_t *s, const char *k, size_t n, uint64_t count)
{
    ...
    for (i = 0; i < s->depth; i++) {
        ...
        s->c[(size_t)i * s->width + col] =
            u64_sat_add(s->c[(size_t)i * s->width + col], count);
    }
    s->total = u64_sat_add(s->total, count);
}
...
uint64_t dyn_cms_count(const dyn_cms_t *s, const char *k, size_t n)
{
    ...
    /* depth >= 1 is enforced at construction, so best is always a real cell.
     * The old `best == UINT64_MAX ? 0` sentinel path is unreachable -- and it
     * became WRONG once saturation made UINT64_MAX a legitimate cell value
     * (every probe saturated -> count answered 0). */
    return best;
}
...
int dyn_cms_merge(dyn_cms_t *a, const dyn_cms_t *b)
{
    ...
    for (i = 0; i < n; i++)
        a->c[i] = u64_sat_add(a->c[i], b->c[i]);
    a->total = u64_sat_add(a->total, b->total);
    return 0;
}
 ...
 * read away. 1ULL<<64 is UB so entries past 63 are zeroed manually -- a
 * subnormal exact for r in 64..1073 would differ, but no valid register ever
 * holds a rank that high (max is 64-p+1), so the zero tail is only a fence. */
...
/* Free one subtree recursively. Height is log_16(count) -- a dozen frames at
 * the uint32 ceiling -- so this needs no explicit stack. Used when the
 * iterative free's own stack cannot grow: teardown must still release
 * everything rather than leak the unvisited subtrees. */
static void dyn_btree_free_kids(btree_node *nd, dyn_cell_free_fn fn, void *ud)
{
    if (nd->leaf) {
        uint32_t i;
        if (fn)
        ...
        for (i = 0; i <= nd->n; i++)
            if (nd->u.i.kid[i])
                dyn_btree_free_kids(nd->u.i.kid[i], fn, ud);
    }
    free(nd);
}
...
{
    /* len/cap are uint32_t because ds_reserve speaks uint32_t caps: passing a
     * size_t* cast to uint32_t* only works while the upper half stays zero.
     * A DFS stack on a height<=8, order-16 tree holds <150 entries, so the
     * narrower type cannot overflow. */
    btree_node **st = NULL;
    uint32_t len = 0, cap = 0;
    if (!t)
        return;
    if (t->root && ds_reserve((void **)&st, &cap, 1, sizeof(*st)) == 0)
        st[len++] = t->root;
    while (len) {
        ...
        if (nd->leaf) {
            ...
            for (i = 0; i <= nd->n; i++) {
                if (!nd->u.i.kid[i])
                    continue;
                if (ds_reserve((void **)&st, &cap, len + 1,
                               sizeof(*st)) < 0) {
                    /* The stack cannot grow, so free the subtree INLINE
                     * rather than dropping it: an OOM during teardown must
                     * not leak everything under this node. */
                    dyn_btree_free_kids(nd->u.i.kid[i], fn, ud);
                    continue;
                }
                st[len++] = nd->u.i.kid[i];
...
/* Split `nd` (entered with n == ORDER), propagating up. Returns -1 on
 * allocation failure. Every node the cascade can need is PRE-ALLOCATED before
 * the first mutation, so a failure returns -1 with the tree untouched.
 *
 * The cascade is exactly: `nd`, then each ancestor holding ORDER-1 keys (it
 * reaches ORDER when it absorbs a separator), plus a new root if the cascade
 * tops out at the root. Height is log_16(count) <= 8 at the uint32 ceiling,
 * so MAX_CHAIN slots cannot be exhausted by a consistent tree; the bound
 * exists so a corrupted one fails closed instead of overrunning the array. */
#define DYN_BTREE_MAX_CHAIN 16

static int btree_split(dyn_btree_t *t, btree_node *nd)
{
    btree_node *pre[DYN_BTREE_MAX_CHAIN];
    btree_node *new_root = NULL;
    uint32_t mid = DYN_BTREE_ORDER / 2, j, nn = 0, next_right = 0;
    btree_node *p;

    if (nd->n != DYN_BTREE_ORDER)
        return 0;                      /* not full: nothing to split, no alloc */
    p = nd;
    for (;;) {
        if (nn >= DYN_BTREE_MAX_CHAIN)
            return -1;                  /* impossible depth: fail closed */
        pre[nn] = btree_node_new(p->leaf);
        if (!pre[nn])
            goto oom;
        nn++;
        if (!p->parent)
            break;                      /* the root splits: need a new one too */
        p = p->parent;
        if (p->n != DYN_BTREE_ORDER - 1)
            break;                      /* absorbs the separator: no split */
    }
    if (!p->parent) {
        new_root = btree_node_new(0);
        if (!new_root)
            goto oom;
    }

    while (nd->n == DYN_BTREE_ORDER) {
        btree_node *right = pre[next_right++], *par;
        double up;
        if (nd->leaf) {
        ...
        /* `nd->parent` is NULL exactly when `nd` was the ROOT, and the new
         * root was pre-allocated with the siblings above: the mutation below
         * cannot fail, which is the whole point of the pre-staging. */
        par = nd->parent;
        if (par == NULL) {
            par = new_root;
            par->n = 1;
    ...
    /* The walk above counts the splitting set exactly, so no pre-allocated
     * node survives unused; the loop frees defensively anyway so a corrupted
     * tree cannot turn a consistency bug into a leak. */
    while (next_right < nn)
        free(pre[next_right++]);
    free(new_root);                     /* provably NULL here */
    return 0;
oom:
    while (nn > 0)
        free(pre[--nn]);
                  ...
                  dyn_cell_t *old, int *replaced)
{
    ...
    *replaced = 0;
    ...
    nd->n++;
    t->count++;
    if (btree_split(t, nd) < 0) {
        /* The split allocates before it mutates, so on -1 the tree is exactly
         * as it was before this call: undo the leaf insertion and honour the
         * documented "container unchanged on -1" contract. Without this, the
         * leaf rests at n == ORDER and the NEXT set() shifts key[32]/val[32]
         * out of bounds -- val[32] lands on the leaf's next pointer. */
        for (j = i; j + 1 < nd->n; j++) {
            nd->key[j] = nd->key[j + 1];
            nd->u.l.val[j] = nd->u.l.val[j + 1];
        }
...
uint32_t dyn_btree_get_keys(const dyn_btree_t *t, double *out)
{
    btree_node *nd = btree_leftmost(t);
    uint32_t n = 0;
    /* Deletes leave empty leaves in the chain (no rebalance), so skip them
     * rather than trusting every node to hold keys. */
    while (nd) {
        if (nd->n) {
            memcpy(out + n, nd->key, (size_t)nd->n * sizeof(double));
            n += nd->n;
        }
        nd = nd->u.l.next;
    }
                               ...
                               void *ud)
{
    ...
    while (nd) {
        if (nd->n) {
            if (!fn(ud, nd->key, nd->u.l.val, nd->n))
                break;
            n += nd->n;
        }
        nd = nd->u.l.next;
