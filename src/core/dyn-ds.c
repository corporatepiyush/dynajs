...
{
    (void)s;
    *h1 = dyn_xxh64((const uint8_t *)k, n,
                    atomic_load_explicit(&ds_seed1, memory_order_relaxed));
...
static int btree_split(dyn_btree_t *t, btree_node *nd)
{
    while (nd->n == DYN_BTREE_ORDER) {
        ...
        if (!right)
            return -1;
        /* A root split needs a new parent: allocate it BEFORE the split
         * mutates `nd`, so an OOM here leaves the tree untouched instead of
         * a half-split leaf with 16 keys dropped and a dangling chain link
         * (the old code split first, then returned -1 with `right` freed). */
        par = nd->parent;
        if (!par) {
        ...
        /* `nd->parent` is still NULL exactly when `nd` was the ROOT: the
         * split never rewrites it, and `par` above was pre-allocated for
         * that case. Checking `!par` would be wrong -- par is never NULL
         * here -- and the old code then left t->root pointing at the old
         * leaf, silently un-linking half the tree. */
        if (nd->parent == NULL) {
            par->n = 1;
            par->key[0] = up;
            par->u.i.kid[0] = nd;
