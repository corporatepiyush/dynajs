...
#include <sys/eventfd.h>   /* the wake channel: eventfd on Linux, pipe elsewhere */
#else
...
struct dyn_pool_chan {
    ...
    /* The wake channel (audit E10-07): a self-pipe everywhere except Linux,
     * where it is a single eventfd carrying the 8-byte counter -- one fd, no
     * 64 KiB kernel pipe buffer per channel, and the exact same coalescing
     * protocol. On Linux wake_r == wake_w, which the close paths know. */
    int wake_r, wake_w;
    int wake_pending;
...
/* 0 means INLINE-ONLY -- the T1 control the pool notes measure T2 against, not
...
/* Coalesced wakeup: one signal in flight at a time, so a burst of
 * completions costs one write, not one per job. On Linux the signal is the
 * eventfd's 8-byte counter add (write of exactly 8 bytes -- any other length
 * is EINVAL); elsewhere it is one byte into the pipe. Caller holds the pool
 * lock. */
...
dyn_pool_t *dyn_pool_new(unsigned nthreads, unsigned queue_cap)
{
    ...
    for (i = 0; i < nthreads; i++) {
        if (pthread_create(&p->threads[i], NULL, pool_worker, p) != 0) {
            ...
            /* Same teardown order as dyn_pool_free: every initialized
             * primitive must go, or a failed pool construction leaks it. */
...
void dyn_pool_free(dyn_pool_t *p)
{
    ...
    while (p->chans) {
        ...
        if (ch->wake_w >= 0 && ch->wake_w != ch->wake_r) close(ch->wake_w);
        free(ch->cdone);
        free(ch);
    }
    pthread_cond_destroy(&p->quiesce);
...
dyn_pool_chan_t *dyn_pool_chan_new(dyn_pool_t *p)
{
    ...
    if (!ch->cdone)
        goto fail;
#if defined(__linux__)
    /* eventfd(EFD_SEMAPHORE off): the counter accumulates adds and a single
     * read returns-and-clears it, which is the coalescing the byte-pipe
     * protocol implemented by hand. Pollable POLLIN, so every existing
     * consumer (epoll, io_uring poll_multishot, poll(2)) is unchanged. */
...
int dyn_pool_drain(dyn_pool_chan_t *ch)
{
    ...
    p = ch->pool;
    pthread_mutex_lock(&p->mu);
    /* Clear the wake channel under the lock so a completion arriving now
     * re-signals rather than being lost against a half-cleared flag. The
     * 8-byte read is the eventfd counter (returns-and-clears it); on the
     * pipe leg a short read of the single coalesced byte works the same. */
    {
        uint64_t v;
