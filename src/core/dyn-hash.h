...
/* Incremental XXH32: the SAME function fed in pieces (LZ4 frame content
 * checksums arrive per block). Defined by xxHash spec v0.7.3; the digest of
 * any chunking of a message equals dyn_xxh32 of the whole message. */
typedef struct {
    uint32_t v1, v2, v3, v4;   /* the four lane accumulators */
    uint64_t total;            /* bytes fed so far */
    uint8_t mem[16];           /* the trailing partial stripe */
    size_t memn;
} dyn_xxh32_ctx_t;

void dyn_xxh32_init(dyn_xxh32_ctx_t *c, uint32_t seed);
void dyn_xxh32_update(dyn_xxh32_ctx_t *c, const uint8_t *data, size_t len);
/* Does not consume or reset `c`: the caller can keep feeding after. */
