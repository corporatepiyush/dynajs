...
const char *dyn_scram_strerror(int code)
{
    ...
    case DYN_SCRAM_E_ENTROPY:return "OS entropy unavailable";
 ...
 * dyn_ct_equal (dyn-hash) carries the compiler barrier that stops the
 * optimiser proving a non-zero difference can never return to zero and
 * inserting exactly that early exit; a bare accumulate loop here does not. */
