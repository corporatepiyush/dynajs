...
/* The id packs idx+1 into 20 bits, so idx+1 must stay <= 0xfffff: at most
 * 2^20-1 slots can ever exist before ID_MAKE would spill the index into the
 * generation and two live timers would share one id (a cancel of one kills
 * the other). Slots are recycled and nslots never shrinks, so refusing the
 * add beats silent aliasing. In-tree callers hold a handful of timers; this
 * bound exists because the core is a public API. */
#define DYN_TIMER_MAX_SLOTS ((1u << 20) - 1u)
...
static uint32_t slot_alloc(dyn_timers_t *t)
{
    ...
    /* id-width bound: the next slot's index+1 must fit 20 bits (see
     * DYN_TIMER_MAX_SLOTS). NO_POS makes dyn_timer_add return
     * DYN_TIMER_NONE rather than mint an aliasing id. */
    if (t->nslots >= DYN_TIMER_MAX_SLOTS)
        return NO_POS;
