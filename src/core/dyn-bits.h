...
/* Fixed-width rotates, for the hash and compression cores. Like
 * dyn_bits_rotate_left above, n is reduced modulo the width first -- the
 * bare `(x << n) | (x >> (width - n))` form shifts by the full width,
 * which is UB, at n == 0 and n == width. Valid n (1..width-1) is unchanged. */
