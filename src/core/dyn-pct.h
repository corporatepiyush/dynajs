...
/* Encode forms:
 *  0 -- unreserved + *-._ + !'()~  (encodeURIComponent's set), space -> %20
 *  1 -- unreserved + *-._, space -> '+'  (x-www-form-urlencoded): the
 *       encodeURIComponentStrict set (tests/test_url.js pins space -> '+')
 *  2 -- the WHATWG urlencoded PERCENT-ENCODE set (C0/DEL/non-ASCII/space/
 *       "#<> escape, space literal): the URLSearchParams serializer set the
 *       project pins (tests/test_url.js: ~ ' ! * ( ) + & = : / stay literal)
