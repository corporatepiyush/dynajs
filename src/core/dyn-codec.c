...
{
    uint64_t v = 0;    /* wide: a full group reaches 85^5-1 > 2^32, and the
                        * overflow test below must see the true value */
    unsigned nb = 0;
    size_t i, o = 0;
    ...
    for (i = 0; i < slen; i++) {
        ...
        nb++;
        if (nb == 5) {
            /* A full group encodes at most 2^32-1 ("s8W-!"): anything above
             * is a group no encoder emits, and Python's a85decode -- the
             * reference this codec is pinned against -- raises "Ascii85
             * overflow" for it. Wrapping mod 2^32 (the old uint32_t
             * accumulator) made the two implementations disagree on exactly
             * this input ("uuuuu" decoded instead of being refused). */
            if (v > 0xFFFFFFFFu)
                return DYN_CODEC_BAD;
            out[o++] = (uint8_t)(v >> 24);
            out[o++] = (uint8_t)(v >> 16);
    ...
    if (nb > 1) {
        ...
        /* Padding can push a partial group past 2^32-1 exactly as a full group
         * can ("s8" = 82,23,u,u,u overflows): Python's a85decode -- the
         * reference this codec is pinned against -- raises "Ascii85 overflow"
         * there. Without this check the (uint8_t)(v >> 24) extraction below
         * silently truncated to a wrong byte. */
        if (v > 0xFFFFFFFFu)
            return DYN_CODEC_BAD;
        for (k = 0; k < nb - 1; k++)
