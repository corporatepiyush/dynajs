 ...
 *
 *   encode: RFC 1952 framing over a real DEFLATE stream (LZ77 hash-chain
 *           match finding), fixed-Huffman codes (sec.3.2.6) at low levels and
 *           dynamic-Huffman blocks (sec.3.2.7, built from the input's own
 *           symbol statistics) from level 6 up -- whichever of the two is
 *           smaller, by exact comparison -- falling back to STORED blocks when
 *           compressing would not shrink the input, so the output never
 *           expands. Always valid DEFLATE that any standard decoder accepts,
 *           plus the CRC-32 and ISIZE trailer.
...
/* As dyn_gzip_build, reusing the caller's scratch, with the level selecting
 * the entropy stage: < 6 emits the fixed-Huffman fast path, >= 6 the
 * dynamic-Huffman encoder (which still compares both and keeps the smaller,
 * so a higher level can size-regress nothing and fail nothing a lower level
 * survives). */
...
/* ---- raw DEFLATE -----------------------------------------------------------
 * The same encoder/decoder the gzip framing wraps, without the 10-byte
 * header or the 8-byte trailer. `dyn_raw_deflate` picks the entropy stage
 * by the same level rule as dyn_gzip_build_ctx (< 6 fixed, >= 6 dynamic
 * with exact size comparison, stored fallback so output never expands
 * beyond the stored ceiling). `dyn_raw_inflate` requires the whole input
 * to be one DEFLATE stream (trailing bytes are refused) and caps output
 * at DYN_MAX_OUTPUT. UNTRUSTED SURFACE on inflate. */
 ...
 * UNTRUSTED SURFACE. Returns 0 on success; every refusal is a DISTINCT
 * negative code below, so the JS layer can name the class it hit (the old
 * contract was one anonymous -1 for every failure). Callers that only test
 * `< 0` keep their exact behaviour. */
int dyn_lz4_frame_decode(const uint8_t *src, size_t len, dyn_outbuf_t *o);

/* The refusal codes of dyn_lz4_frame_decode. */
#define DYN_LZ4F_OK             0
#define DYN_LZ4F_ERR_TRUNC     (-1)  /* input ends before the frame does */
#define DYN_LZ4F_ERR_MAGIC     (-2)  /* not 0x184D2204 */
#define DYN_LZ4F_ERR_VERSION   (-3)  /* FLG version bits not 01 */
#define DYN_LZ4F_ERR_BLOCK_MAX (-4)  /* BD names a reserved/absurd block size */
#define DYN_LZ4F_ERR_LINKED    (-5)  /* linked blocks (FLG bit 5 clear) */
#define DYN_LZ4F_ERR_HDR_SUM   (-6)  /* descriptor checksum mismatch */
#define DYN_LZ4F_ERR_BLOCK_CAP (-7)  /* block longer than the declared cap */
#define DYN_LZ4F_ERR_BLOCK     (-8)  /* a compressed block does not decode */
#define DYN_LZ4F_ERR_BLOCK_SUM (-9)  /* block checksum mismatch */
#define DYN_LZ4F_ERR_CONT_SUM  (-10) /* content checksum mismatch */
#define DYN_LZ4F_ERR_CONT_SIZE (-11) /* declared content size disagrees */
#define DYN_LZ4F_ERR_OOM       (-12) /* allocation failure, not corruption */

/* A stable, lowercase reason phrase for a refusal code ("corrupt LZ4
 * block", ...). Never NULL. */
