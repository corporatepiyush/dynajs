 ...
 *
 * An earlier sketch proposed a `dyn_elem_ops_t {dup,free,mark,cmp}`
 ...
 *
 * ---- Numeric keys and priorities ----
 *
 * NaN is NOT guarded here: every NaN input is the BINDING's to refuse (the JS
 * layer throws RangeError for NaN keys, priorities and interval bounds). A
 * NaN reaching the core lands somewhere a search can never find it (BTree:
 * inserted and unretrievable) or orders arbitrarily (mmheap). The deserializer
 * paths filter the shapes a wire record could smuggle.
 */
#ifndef DYN_DS_H
#define DYN_DS_H
...
/* Seed the string-key index hash (audit 13.8.1). Call once at startup with
 * kernel entropy; the first call per process wins and later calls are no-ops.
 * Until called, the compile-time default (the former fixed secrets) applies. */
 ...
 * `out` must be a DIFFERENT set: it is cleared before `s` is read. */
 ...
 * that stops being reachable is handed to `fn`. `dup` is REQUIRED (not
 * optional): a split that encloses [lo,hi) duplicates its cell, and a NULL
 * dup dereferences through NULL on exactly that path. */
...
/* count() caches its estimate in the struct, so it is not const-safe under
 * concurrency. This file's containers are SINGLE-THREADED: one HLL must not
 * be counted while another thread adds or merges into it. */
...
/* Bulk-copy every key in ascending order into `out`, which must have room for
 * dyn_btree_size(t) doubles; returns the count written. Snapshot builders
 * (keys()/toArray(), the DYNS key stream) use this instead of stepping the
 * iterator one extern call per element: the walk is a straight memcpy per
 * leaf with no per-key call overhead. */
uint32_t dyn_btree_get_keys(const dyn_btree_t *t, double *out);
/* Walk every non-empty leaf left to right, invoking fn once per leaf with the
 * leaf's OWN contiguous key (and value) arrays; do not retain the pointers.
 * fn returns 0 to abort the walk. Returns the number of keys visited. One
 * call per ORDER-32 leaf amortizes the cross-module call to nothing, which is
 * what lets the sorted-container bindings build snapshots at skiplist-chain
 * speed or better. */
 ...
 * MUTATION invalidates a live iterator: a set/delete shifts keys within the
 * leaf the iterator is in, so it may skip or duplicate entries with no way
 * to detect it. Finish iterating before mutating. */
