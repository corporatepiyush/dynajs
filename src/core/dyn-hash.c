...
#include <stdatomic.h>   /* crc32c_hw_ok / sha_ni_ok: runtime CPU probes are
                         * shared statics; see those functions */
#endif
...
/* DCE-proof zeroisation for keyed scratch (audit S10). */
...
/* Hardware SHA-256 (Intel SHA extensions, x86-64). The same sixteen groups of
 * four rounds as the ARMv8 path above, with the two-operations-per-group
 * asymmetry the x86 instructions have: each sha256rnds2 does two rounds
 * reading the message from implicit XMM0 (dwords 0 and 2), the 0x0E dword
 * rotation brings the group's other two message dwords into those lanes for
 * the second sha256rnds2, and sha256msg1/msg2 fold in the next four schedule
 * words.
 * The endianness split is the same as ARM's -- the message block arrives
 * big-endian and is reversed per dword by one pshufb, the state is host order
 * and is NOT -- but the register layout is not: sha256rnds2 works on ABEF and
 * CDGH halves, so the state is shuffled to that form once at load and back
 * once at store. Getting that juggling wrong is how this kernel produces
 * plausible-looking wrong digests; the differential against the scalar path
 * is what catches it.
 *
 * Unlike __ARM_FEATURE_SHA2, SHA-NI is not a compile-time property of any
 * -march this file is built with, so the kernel carries
 * target("sha,sse4.1") -- pshufb/blend need SSE4.1-class encoding and every
 * shipping SHA-NI CPU has it -- and the selector below probes before calling.
 * Verified against the FIPS 180-4 vectors and 10,000 pseudorandom blocks
 * (deterministic PRNG), kernel called directly and via the probe, against
 * this file's scalar path; see the selector comment for how it was run. */
#if defined(__x86_64__) && !defined(DYN_SHA256_NO_HW)
#define DYN_SHA256_X86 1
#include <immintrin.h>

/* Four rounds. `a` holds the four message words these rounds consume, `p` the
 * previous group's four (dead after the alignr reads them), `n` the partial
 * words the NEXT group needs. The msg1 that starts the vector sixteen rounds
 * ahead writes `p`, not `a`: W[t] needs W[t-16] and W[t-15], so the pair is
 * (previous group, this group), and `a` itself is still live -- it is the
 * alignr operand of the group AFTER this one. Writing msg1 into `a` was this
 * kernel's first draft and the differential caught it on block zero. `sched`
 * is 0 for the last three groups: the msg1 here prepares the words twelve
 * rounds ahead (one group later than the ARM path's sixteen, because the
 * source pair is the previous group's vector), so the cutoff is rounds < 52,
 * not the ARM path's rounds < 48. The K addend and the alignr/msg2 work sit
 * BETWEEN the two rnds2 calls on purpose: they depend on the message vectors,
 * not the state, so they fill the shadow of the first rnds2's latency -- the
 * published Intel sample's ordering. */
#define DYN_SHA256_NI_QUAD(a, p, n, kidx, sched)                               \
    do {                                                                       \
        msg = _mm_add_epi32((a), _mm_loadu_si128((const __m128i *)             \
                                                 &sha256_K[(kidx)]));          \
        state1 = _mm_sha256rnds2_epu32(state1, state0, msg);                   \
        tmp = _mm_alignr_epi8((a), (p), 4);                                    \
        (n) = _mm_add_epi32((n), tmp);                                         \
        (n) = _mm_sha256msg2_epu32((n), (a));                                  \
        msg = _mm_shuffle_epi32(msg, 0x0E);                                    \
        state0 = _mm_sha256rnds2_epu32(state0, state1, msg);                   \
        if (sched) (p) = _mm_sha256msg1_epu32((p), (a));                       \
    } while (0)

__attribute__((target("sha,sse4.1")))
static void sha256_compress_sha_ni(dyn_hash_state_t *s, const uint8_t *block)
{
    ...
    /* sha256rnds2 wants {a,b,e,f} / {c,d,g,h}, not {a,b,c,d} / {e,f,g,h}. */
    tmp    = _mm_loadu_si128((const __m128i *)&s->w32[0]);
    state1 = _mm_loadu_si128((const __m128i *)&s->w32[4]);
    ...
    state1 = _mm_blend_epi16(state1, tmp, 0xF0);  /* CDGH */
    {
        ...
        /* Rounds 0-3: message words straight off the block, big-endian. */
        msg0 = _mm_shuffle_epi8(_mm_loadu_si128((const __m128i *)(block +  0)),
                                bswap);
        msg = _mm_add_epi32(msg0, _mm_loadu_si128((const __m128i *)
                                                  &sha256_K[0]));
        state1 = _mm_sha256rnds2_epu32(state1, state0, msg);
        msg = _mm_shuffle_epi32(msg, 0x0E);
        state0 = _mm_sha256rnds2_epu32(state0, state1, msg);

        /* Rounds 4-7: first msg1 -- W[16..19] can start once W[4..7] exist. */
        msg1 = _mm_shuffle_epi8(_mm_loadu_si128((const __m128i *)(block + 16)),
                                bswap);
        msg = _mm_add_epi32(msg1, _mm_loadu_si128((const __m128i *)
                                                  &sha256_K[4]));
        state1 = _mm_sha256rnds2_epu32(state1, state0, msg);
        msg = _mm_shuffle_epi32(msg, 0x0E);
        state0 = _mm_sha256rnds2_epu32(state0, state1, msg);
        ...
        /* Rounds 8-11. */
        msg2 = _mm_shuffle_epi8(_mm_loadu_si128((const __m128i *)(block + 32)),
                                bswap);
        msg = _mm_add_epi32(msg2, _mm_loadu_si128((const __m128i *)
                                                  &sha256_K[8]));
        state1 = _mm_sha256rnds2_epu32(state1, state0, msg);
        msg = _mm_shuffle_epi32(msg, 0x0E);
        state0 = _mm_sha256rnds2_epu32(state0, state1, msg);
        ...
        /* Rounds 12-15: the first group that finishes a schedule vector --
         * W[16..19] = msg0's partial + W[9..12] (the alignr) + sigma1. */
        msg3 = _mm_shuffle_epi8(_mm_loadu_si128((const __m128i *)(block + 48)),
                                bswap);
        msg = _mm_add_epi32(msg3, _mm_loadu_si128((const __m128i *)
                                                  &sha256_K[12]));
        state1 = _mm_sha256rnds2_epu32(state1, state0, msg);
        ...
        /* Rounds 16-63: the four vectors rotate; DYN_SHA256_NI_QUAD(a, p, n)
         * names them per group so the schedule reads in order. */
        DYN_SHA256_NI_QUAD(msg0, msg3, msg1, 16, 1);
        DYN_SHA256_NI_QUAD(msg1, msg0, msg2, 20, 1);
        DYN_SHA256_NI_QUAD(msg2, msg1, msg3, 24, 1);
        DYN_SHA256_NI_QUAD(msg3, msg2, msg0, 28, 1);
        DYN_SHA256_NI_QUAD(msg0, msg3, msg1, 32, 1);
        DYN_SHA256_NI_QUAD(msg1, msg0, msg2, 36, 1);
        DYN_SHA256_NI_QUAD(msg2, msg1, msg3, 40, 1);
        DYN_SHA256_NI_QUAD(msg3, msg2, msg0, 44, 1);
        DYN_SHA256_NI_QUAD(msg0, msg3, msg1, 48, 1);
        DYN_SHA256_NI_QUAD(msg1, msg0, msg2, 52, 0);
        DYN_SHA256_NI_QUAD(msg2, msg1, msg3, 56, 0);
        DYN_SHA256_NI_QUAD(msg3, msg2, msg0, 60, 0);

        state0 = _mm_add_epi32(state0, abef_save);
        state1 = _mm_add_epi32(state1, cdgh_save);
    }

    /* And back to {a,b,c,d} / {e,f,g,h}. */
    tmp    = _mm_shuffle_epi32(state0, 0x1B);       /* FEBA */
    state1 = _mm_shuffle_epi32(state1, 0xB1);       /* DCHG */
    state0 = _mm_blend_epi16(tmp, state1, 0xF0);    /* DCBA */
    state1 = _mm_alignr_epi8(state1, tmp, 8);       /* HGFE -> EFGH */
    _mm_storeu_si128((__m128i *)&s->w32[0], state0);
    _mm_storeu_si128((__m128i *)&s->w32[4], state1);
}
#undef DYN_SHA256_NI_QUAD

/* SHA-NI is not in the x86-64 baseline ABI either, so this is a runtime probe
 * -- the crc32c_hw_ok pattern exactly: _Atomic because dyn_sha256 runs on any
 * JS thread and a plain static write from two of them is a race even when both
 * write the same value (CLAUDE.md sec.6); relaxed because the value is a pure
 * function of the CPU and nothing is published through it. No SSE4.1 check:
 * every CPU with SHA-NI has SSE4.1, and `sha` in the probe implies it in the
 * kernel's target attribute. */
static int sha_ni_ok(void)
{
    static _Atomic int state;   /* 0 unknown, 1 yes, -1 no */
    int s = atomic_load_explicit(&state, memory_order_relaxed);
    if (!s) {
        __builtin_cpu_init();
        s = __builtin_cpu_supports("sha") ? 1 : -1;
...
#if !defined(DYN_SHA256_HW) || defined(DYN_SHA256_X86)
/* The scalar path IS the implementation when there is no ISA one, and the
 * probe's fallback on x86-64 when SHA-NI is absent (an emulator without
 * SHA-NI TCG reports the CPUID bit clear -- honestly, since the instructions
 * would fault there). */
...
/* __ARM_FEATURE_SHA2 is a compile-time guarantee from the target, so the ARM
 * kernel has nothing to probe; on a target without it the scalar path is the
 * only one compiled. SHA-NI is not, so x86-64 asks the CPU first. Build with
 * -DDYN_SHA256_NO_HW=1 to force the scalar path, which is what
 * tests/test_sha256_hw.c diffs against.
 *
 * Verification status (both paths): FIPS 180-4 known answers (including the
 * million-'a' vector), 10,000 pseudorandom blocks chained through the
 * compression functions and 10,000 pseudorandom messages of random length,
 * every digest diffed against this file's scalar path; the ARM selector on
 * arm64 hardware, and the SHA-NI kernel under docker x86-64 where it was
 * compiled with gcc-14 and clang-19 and EXECUTED under qemu-x86_64 10.0
 * -cpu max (TCG has implemented SHA-NI since qemu 9.0) -- there the probe
 * ACCEPTED and the selector dispatched to the kernel, and selector, direct
 * kernel call and scalar all agreed on every input. The dev container's
 * stock QEMU carries no SHA-NI TCG and the probe refuses under it, which
 * selects scalar; that configuration was diffed as well. No SHA-NI silicon
 * was available, so nothing here is a hardware measurement, and the one
 * timing taken (QEMU TCG, doubly emulated: qemu under docker's qemu) put the
 * kernel ~1.5x ahead of scalar -- qemu-tcg timing, not representative of
 * hardware. */
static void sha256_compress(dyn_hash_state_t *s, const uint8_t *block)
{
#if defined(DYN_SHA256_HW)
    sha256_compress_hw(s, block);
#elif defined(DYN_SHA256_X86)
    if (sha_ni_ok())
        sha256_compress_sha_ni(s, block);
    else
                            ...
                            const uint8_t *msg, size_t msglen, uint8_t *out)
{
    ...
    memset(ipad, 0, sizeof(ipad));    /* all key-derived (K0 ^ 0x36/0x5c) */
...
void dyn_hmac_final(dyn_hmac_ctx_t *c, uint8_t *out)
{
    ...
    memset(inner, 0, sizeof(inner)); /* keyed digest scratch */
}
             ...
             uint8_t *out, size_t out_len)
{
    ...
    memset(prk, 0, sizeof(prk));  /* derived key material (extract) */
    memset(t, 0, sizeof(t));      /* derived key material (expand) */
    return 0;
}
               ...
               uint8_t *out, size_t out_len)
{
    ...
    dyn_secure_zero(u, sizeof(u));       /* keyed HMAC scratch (S10) */
    dyn_secure_zero(acc, sizeof(acc));   /* derived key material */
    return 0;
}
...
/* ---- XXH32 streaming (xxHash spec v0.7.3) ----
 * The one-shot above run incrementally: identical output for identical bytes,
 * any chunking. One round function shared with the one-shot (xxh32_round), so
 * the two cannot drift apart -- a second copy of this algorithm with its own
 * rotation constant is exactly how frame checksums stop being portable. */

void dyn_xxh32_init(dyn_xxh32_ctx_t *c, uint32_t seed)
{
    c->v1 = seed + XXH32_P1 + XXH32_P2;
    c->v2 = seed + XXH32_P2;
    c->v3 = seed;
    c->v4 = seed - XXH32_P1;
    c->total = 0;
...
void dyn_xxh32_update(dyn_xxh32_ctx_t *c, const uint8_t *data, size_t len)
{
    ...
    c->total += len;
    if (c->memn) {              /* complete the buffered stripe first */
        size_t fill = 16 - c->memn < len ? 16 - c->memn : len;
        memcpy(c->mem + c->memn, p, fill);
        c->memn += fill;
        p += fill;
        if (c->memn < 16)
            return;
        c->v1 = xxh32_round(c->v1, xxh_read32(c->mem));
        c->v2 = xxh32_round(c->v2, xxh_read32(c->mem + 4));
        c->v3 = xxh32_round(c->v3, xxh_read32(c->mem + 8));
        c->v4 = xxh32_round(c->v4, xxh_read32(c->mem + 12));
        c->memn = 0;
    }
    while (end - p >= 16) {
        c->v1 = xxh32_round(c->v1, xxh_read32(p));
        c->v2 = xxh32_round(c->v2, xxh_read32(p + 4));
        c->v3 = xxh32_round(c->v3, xxh_read32(p + 8));
        c->v4 = xxh32_round(c->v4, xxh_read32(p + 12));
        p += 16;
    }
    if (p < end) {
        memcpy(c->mem, p, (size_t)(end - p));
        c->memn = (size_t)(end - p);
    }
}
...
{
    const uint8_t *p = c->mem;
    size_t n = c->memn;
    uint32_t h;

    if (c->total >= 16) {
        h = xxh32_rotl(c->v1, 1) + xxh32_rotl(c->v2, 7) +
            xxh32_rotl(c->v3, 12) + xxh32_rotl(c->v4, 18);
    } else {
        h = c->v3 + XXH32_P5;   /* v3 is the seed: the spec's short base */
    }
    h += (uint32_t)c->total;
    while (n >= 4) {
        h += xxh_read32(p) * XXH32_P3;
