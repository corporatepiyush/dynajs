                                ...
                                int argc, JSValueConst *argv)
{
    ...
    /* utf-8 is not in DYN_CHARSETS -- it is the engine's own string encoding.
       ...
       checked first still got a throw. The lossy decoder (shared with toUtf8)
       applies the Encoding Standard's U+FFFD substitution; JS_NewStringLen
       alone would hand back lone surrogates for WTF-8 input. */
    if (dyn_cs_label_eq("utf-8", label, ln) || dyn_cs_label_eq("utf8", label, ln)) {
        ...
        return dyn_utf8_lossy_string(ctx, p, n);
    }
