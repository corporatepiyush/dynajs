...
typedef struct simd {
  ...
  /* silu(x) = x * sigmoid(x) via the Schraudolph fast exp. The historical
     x86 sign bug (a negative magic against neg_x double-negated into
     silu(-x)) is FIXED at source in every ISA kernel (positive magic, see
     dyna-simd-{avx2,sse42,avx512,neon}.c) and the JS binding calls this slot
     directly. It is a DIFFERENT formulation from a sigmoid+mul compose
     (~5% apart at x=-2.5, both inside the 2e-2 class bound); test_simd.js
     pins kernel ~ compose within the class and in-place == fresh. */
   ...
   *    dyna:strings and any delimiter/substring scan. ─────────────────────── */
  ...
  /* 16-bit subject variant for WIDE-string anchored scans (regexp /m
   * LINESTART): index of the first unit equal to any of `set`, else
   * SIZE_MAX. Backends without a kernel leave the slot NULL and callers
   * must keep their scalar loop. */
  size_t (*find_first_of_u16)(const uint16_t *restrict p, size_t n,
                              const uint16_t *restrict set, size_t setlen);
  /* First index in p[0..n) whose byte is a member of a 256-bit set, or
   * SIZE_MAX. `bitmap` is 32 bytes; byte v is a member iff
   * bitmap[v >> 3] & (1 << (v & 7)).
   *
   * find_first_of takes a LIST and is vectorised only to 8 elements; the common
   * character classes are far wider (\d is 10, [a-z] is 26, \w is 63), and a
   * list of that size falls back to a per-call membership table. This takes the
   * set already in bitmap form, so there is no per-call setup at all -- which
   * matters because the regexp prefilter calls it once per match ATTEMPT.
   *
   * Vectorised by two 16-byte shuffles: one selects the bitmap byte from
   * (v >> 3), the other turns (v & 7) into 1 << (v & 7). */
 ...
 *
 * The product and the bias add are separate STATEMENTS on purpose: an FP-contracting
 * compiler would otherwise fuse `m*x + bias` into one FMA in some translation units
 * and not others, so the same input could round differently inside dyna-simd-scalar.o
 * and dyna-simd-neon.o. The split pins one rounding order (mul, add, truncate) that
 * every ISA vector path mirrors instruction-for-instruction (see neon_fast_exp_f32),
 * making the fast-exp family bitwise identical across scalar tails and vector heads. */
static inline float fast_exp(float x) {
  if (unlikely(x != x))        /* exp(NaN) is NaN; the cast below is UB */
    return x;
  if (unlikely(x < -88.0f))
    return 0.0f;
  if (unlikely(x > 88.0f))
  ...
  float p = 12102203.0f * x; /* separate statement: no FMA contraction */
  p += 1065353216.0f;
  u.i = (int32_t)p;
  return u.f;
}
