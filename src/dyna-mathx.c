/*
 * dyna:mathx -- C99 `math` + MATLAB-tier special functions, integer/BigInt
 * helpers, the `bits` fixed-width surface, and the `stats` reductions over
 * typed arrays / number[] (see each section header for its contracts).
 * Pure functions: no `this`, no native handle, nothing to close.
 *
 ...
 * Full per-function table: see the module header.
 */
#include "dyna-nat.h"
...
/* Does this BigInt fit int64_t exactly? JS_ToBigInt64 silently reduces
 * modulo 2^64, so a wide BigInt (2n**64n + 1n) would arrive as 1n and, say,
 * isPrime would certify the wrong number. The decimal string decides: the
 * magnitude fits iff it is at most 19 digits and compares <= 2^63-1, with
 * one extra allowance: exactly -2^63 (which has no positive int64 twin) is
 * itself representable. Same domain the double path enforces. Returns
 * 1 = fits, 0 = too wide, -1 = JS exception. */
static int dyn_mathx_bigint_fits_i64(JSContext *ctx, JSValueConst v)
{
    ...
    if (n < 19)
        ...
        r = 1;                  /* 19 digits below 9e18 all fit */
...
/* Same idea for abs/bitLen/popcount, whose domain is the uint64 magnitude:
 * the magnitude must be < 2^64 (20 digits, <= 18446744073709551615).
 * Returns 1 = fits, 0 = too wide, -1 = JS exception. */
static int dyn_mathx_bigint_fits_u64(JSContext *ctx, JSValueConst v)
{
    ...
    if (n < 20)
        ...
        r = 1;                  /* 20 digits below 1e19 all fit */
    else
...
/* Shared refusal for the three functions whose domain is the uint64
 * magnitude. JS_ToBigInt64 would silently reduce a wider BigInt modulo 2^64:
 * bitLen(factorial(21)) returned 62 (the low word) instead of 66, and
 * popcount(-(2n**200n)) returned 0 -- wrong answers, not errors. */
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (JS_IsBigInt(ctx, argv[0])) {
        /* isPrime's domain is the full uint64 range ("proven for every
         * uint64"), and only a BigInt can carry the exact values in
         * [2^53, 2^64). Accept any BigInt whose magnitude fits 64 bits:
         * the low 64 bits of such a BigInt ARE the value, so the
         * JS_ToBigInt64 wrap is an exact unsigned read here. */
...
/* ==================================================================== *
 *  MATLAB-parity elementary math (W5.3, tier A)
 * ==================================================================== */
 ...
 *
 * The adjustment runs on fmod, NOT on a bare x - floor(x/y)*y: fmod is exact
 * (both operands are integers in ulps of the wider of the two), so the
 * remainder is the true mathematical one for every representable input. The
 * naive x/y-rounded form loses the low bits for large x and returned values
 * OUTSIDE [0, y) -- mod(1e17,3) was 4 and mod(1e17,7) was -2.
 *
 * The naive form survives as a guarded FAST PATH, because fmod() is a libm
 * call and the naive form is three inline flops: for q = x/y below 2^53 the
 * rounded quotient is within one integer of the true floor, so a wrong naive
 * answer is exactly a multiple of y away from the truth and lands OUTSIDE
 * the legal result range -- the range check catches every wrong case, and
 * only the pathological ratios (q >= 2^53) pay the fmod call. Adding y to an
 * r of opposite sign is exact: |r| < |y| means no rounding above y's scale. */
static double dyn_mathx_mod_floored(double x, double y)
{
    double r;
    if (y == 0.0)
        return x;               /* mod(a, 0) is a (MATLAB) */
    r = x - floor(x / y) * y;
    if (y > 0.0 ? (r >= 0.0 && r < y) : (r <= 0.0 && r > y))
        return r;
    r = fmod(x, y);
    if (r != 0.0 && ((r < 0.0) != (y < 0.0)))
        r += y;                 /* exact: opposite signs, |r| < |y| */
    return r;
}
 ...
 * an odd integer n, which is the case it exists for. n = 3 routes through
 * cbrt(), which is odd (defined for either sign) and correctly rounded,
 * where pow(x, 1/3) carries a ~ulp-of-1/3 exponent rounding that openlibm's
 * pow turns into ~1e-14 relative error for huge x (nthroot(1e300, 3) was
 * 9.999999999999872e+99). n = 1 is the identity for the same reason. */
DYN_MATHX_BINARY(nthroot,
    (y == 3.0) ? cbrt(x)                 /* exact for either sign */
               : (y == 1.0) ? x
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    a = fabs(x);
    na = nextafter(a, INFINITY);
    if (isinf(na)) {
        /* a == DBL_MAX: there is no next double, but the ulp of the top
         * binade is well defined and is what MATLAB reports:
         * eps(realmax) = 2^971 = 1.9958e292, not Inf. */
        return JS_NewFloat64(ctx, ldexp(1.0, 1023 - 52));
    }
    /* nextafter is exactly "the next representable value", so the
     * difference is the ulp -- correct for subnormals too, unlike 2^(ilogb-52) */
    return JS_NewFloat64(ctx, na - a);
}

/* Shared core of beta/betaln: log|B(a,b)| via the reentrant lgamma_r pieces
 * (see the header comment: bare lgamma is exactly what this module avoids),
 * with the sign product of the three Gamma factors in *sign. Poles --
 * non-positive integer a or b -- return INFINITY, matching lgamma's own pole
 * value. betaln returns this log alone: like gammaln it is a log of an
 * ABSOLUTE value, so the sign drops out of it, while beta multiplies the
 * sign back in. Keeping both on this one path means
 *   betaln(x,y) == gammaln(x) + gammaln(y) - gammaln(x+y)
 * holds for negative non-integer arguments too, not just positive ones. */
static double dyn_beta_ln(double a, double b, int *sign)
{
    int sa, sb, sab;
    double la, lb, lab;
    if (a <= 0.0 && a == floor(a)) { *sign = 1; return INFINITY; }
    if (b <= 0.0 && b == floor(b)) { *sign = 1; return INFINITY; }
    la = lgamma_r(a, &sa);
    lb = lgamma_r(b, &sb);
...
/* log|B(a,b)|: the same sign-aware computation with the sign discarded. */
 ...
 * Above x = 25 erfc(x) is still ~1e-273 (finite), but exp(x*x) would carry it
 * only through one rounding, so the asymptotic series takes over. The series
 * is kept to the term whose magnitude at x = 25 is ~1e-15: a 3-term cut left
 * a 7.6e-9 relative error at the switch (the next omitted term,
 * 15/(8 x^6)/sqrt(pi)/x, is exactly that at x = 25). Terms alternate and
 * decrease monotonically out to n ~ x^2, so extending cannot cancel. */
DYN_MATHX_UNARY(erfcx, (x < 25.0) ? exp(x * x) * erfc(x)
                                  : (1.0 / (x * 1.7724538509055160273))
                                    * (1.0 - 0.5 / (x * x)
                                            + 0.75 / (x * x * x * x)
                                            - 1.875 / (x * x * x * x * x * x)
                                            + 6.5625 / (x * x * x * x * x * x * x * x)
                                            - 29.53125 / (x * x * x * x * x * x * x * x * x * x)))
...
/* floor(sqrt(x)) for the trial-division limit. Inputs are <= 2^53 (the
   factor() domain), so r stays below 2^27 and r*r cannot overflow. sqrtl on
   the 64-bit-mantissa long double is exact enough that the correction loops
   adjust by at most 1 ulp. */
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    /* d <= isqrt(x) is exactly d <= x/d (floor division), so the hoisted
       limit is equivalent to dividing per candidate -- and x only shrinks
       when a factor is extracted, at most 53 times, vs one 64-bit division
       per odd candidate here. */
    limit = dyn_isqrt_u64(x);
    for (d = 3; d <= limit; d += 2) {
        if (x % d == 0) {
            do { out[n++] = (double)d; x /= d; } while (x % d == 0);
            limit = dyn_isqrt_u64(x);
        }
    }
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (isnan(nd) || isnan(kd) || nd < 0.0 || kd < 0.0 ||
        ...
        nd >= 18446744073709551616.0 || kd >= 18446744073709551616.0)
...
/* ==================================================================== *
 *  stats -- reductions over a bare Float64Array or number[].
 * ==================================================================== *
 *
 * The statistics stack has three layers and each owns a distinct input
 * shape (the full matrix is documented in the dyna:mathx .d.ts block and
 * in API.md):
 *
 *     Array.prototype  sum/mean/min/max/median/product  number[] convenience
 *     mathx.stats      THIS surface                     typed arrays + the
 *                                                       missing half
 *                                                       (variance/quantile/
 *                                                       corr/cov)
 *     DataFrame        VARIANCE/STDDEV/CORR/COV/...     columnar, masked,
 *                                                       full set
 *
 * Documented divergences from the neighbours, none of them bugs:
 *   - sum() is Neumaier-compensated (the DECISION below), so it is MORE
 *     accurate than both Array.prototype.sum and DataFrame.SUM, which are
 *     plain accumulators.
 *   - quantile()/median() use the exact R-7 / "linear" convention of
 *     DataFrame.QUANTILE (pos = q*(n-1) between closest ranks), but an
 *     EMPTY input throws RangeError here where DataFrame returns
 *     undefined, because a bare array has no mask to define "no rows
 *     selected" -- a caller who wanted a silent undefined has a bug.
 *     NaN ELEMENTS diverge the same way, for the same reason: a bare
 *     array has no mask, so the NaN poisons the order statistic --
 *     stats.median([1,NaN,3]) is NaN where DataFrame.MEDIAN answers 2
 *     (DataFrame drops unmasked NaNs columnar-ly; here there is nothing
 *     to drop against).
 *   - corr() throws RangeError on zero variance where DataFrame.CORR
 *     returns NaN: over a bare array a constant input is almost always a
 *     feeding bug (a mis-sliced window), not data to describe.
 *   - min()/max() follow Math.min/Math.max: NaN poisons the result, and
 *     an EMPTY input returns +Infinity/-Infinity, the zero-argument
 *     Math.min()/Math.max() values.
 *
 * THE SUM DECISION: Neumaier (Kahan-Babuska), not pairwise. Both fix what
 * plain accumulation gets wrong on long runs of similar magnitude (the
 * 1e7 x 0.1 accuracy table in CHANGELOG.md); Neumaier was chosen because
 *   (a) it is a strictly sequential single pass, so the Float64Array fast
 *       path and the number[] fallback run the IDENTICAL add sequence and
 *       agree bit-for-bit on identical values (a test pins this), where a
 *       pairwise tree would either diverge between the paths or force the
 *       fallback to buffer too;
 *   (b) O(1) state, three flops per element -- no recursion, no scratch
 *       buffer, comfortably inside the "mean within 2x of a naive C loop"
 *       performance gate over 1e7 elements; and
 *   (c) its error bound tracks eps * sum|x| independently of n, which is
 *       exactly the failure mode (cancellation) pairwise does NOT fix.
 * variance/covariance run the classic two-pass (mean first, then squared
 * deviations, Neumaier there too) rather than Welford: same choice and
 * rationale as DataFrame's B.4 kernel.
 *
 * Inputs: a Float64Array is read through its backing store with no
 * per-element machinery (the whole point of the namespace); everything
 * else with a `length` takes the dyn_mathx_scan per-element read. The
 * fast path is gated on the CLASS ID, not the 8-byte stride -- a
 * BigInt64Array shares the stride but never its meaning.
 */

/* Built-in Float64Array class id, captured once at module init from a
 * sample instance; built-in ids are process-global constants (see
 * dyna-simd.c, which established the idiom). _Atomic because a Worker's
 * runtime inits this module on its own thread. */
...
static JSClassID mxst_capture_f64_cid(JSContext *ctx)
{
    ...
    JSValue ta;
    JSClassID cid = 0;
    args[0] = JS_NewInt32(ctx, 0); /* immediate: new Float64Array(0) */
...
typedef struct { double s, c; } mxst_neumaier;
#define MXST_NEUMAIER_ZERO { 0.0, 0.0 }

/* The lower-order term: the part of x the add just dropped. Branchless in
 * intent, not in fact -- fabs() of NaN is NaN, so a NaN input lands in the
 * else arm and poisons c, then every later add; either way s ends NaN. */
...
typedef struct {
    const double *p;   /* Float64Array fast path: direct doubles, else NULL */
    JSValueConst arr;  /* fallback: read element by element */
    uint32_t len;
} mxst_src;

/* Route one stats argument. The pointer is valid only while no JS runs
 * (true for every kernel below: the fallback path holds no pointer, so
 * getters running user JS are safe there). */
...
/* One fallback element; the same read dyn_mathx_scan does. */
...
/* The {pop: boolean} options object of variance/stddev/cov. Absent, null
 * or a `pop` that is missing/undefined reads as sample (the default).
 * Returns 0/1, or -1 with an exception pending. */
static int mxst_read_pop(JSContext *ctx, JSValueConst opts)
{
    ...
    if (JS_IsException(p))
        return -1;
    /* JS_ToBool's -1 (a throwing toPrimitive) flows out as this function's
     * own -1, exception pending -- stated separately so the error return is
     * visible as a statement, not buried in a ternary */
    if (JS_IsUndefined(p))
        b = 0;
    else
        b = JS_ToBool(ctx, p);
...
/* sum: one Neumaier walk, identical add sequence on both input paths. */
...
/* Two-pass variance: pass 1 the mean, pass 2 the Neumaier-compensated sum
 * of squared deviations. mean of the empty array is 0/0, so it comes out
 * NaN without a special case. */
...
/* Paired two-pass moment: means of x and y, then the Neumaier-compensated
 * cross-product; corr's variants also carry both squared-deviation sums.
 * Length equality is checked by the caller before this runs. */
                         ...
                         int want_m2, double *pcxy, double *pm2x, double *pm2y)
{
    ...
    const double *xp, *yp;
    double *xa = NULL, *ya = NULL;   /* private copies of a fallback side */
    mxst_src ta, tb;
    double mx, my;
    uint32_t i, n = a->len;
    /* The two sources are routed INDEPENDENTLY (a Float64Array may be
     * paired with a number[] and vice versa), but raw pointers and user
     * JS must not MIX: a getter on the fallback side can detach or
     * transfer the typed array backing the other side's pointer (the
     * loop-2 probe did exactly that and read freed memory). So the
     * direct-zip fold runs only when BOTH sides are pointers; otherwise
     * BOTH passes -- the means too -- walk private copies: the fast side
     * is snapshotted first (re-resolved, so a buffer detached by earlier
     * JS fails cleanly right there), and only then do the fallback
     * getters run, against copies alone. The accumulator sequence is
     * identical on every route, so the input paths still agree
     * bit-for-bit. */
    if (a->p && b->p) {
        xp = a->p;
        yp = b->p;
    ...
    ta.p = xp; ta.arr = a->arr; ta.len = n;
    tb.p = yp; tb.arr = b->arr; tb.len = n;
...
/* Materialise the source into a scratch buffer ready to sort. Returns
 * NULL only with an exception pending; *pisnan says whether any element
 * is NaN (the caller answers NaN: order statistics of unordered input are
 * undefined, and sorting NaNs is a different bug). */
static double *mxst_materialize(JSContext *ctx, const mxst_src *s, int *pisnan)
{
    uint32_t i, n = s->len;
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    if (mxst_fold_sum(ctx, &s, &v))
        return JS_EXCEPTION;
    return JS_NewFloat64(ctx, v);   /* empty array sums to +0 (empty state) */
}
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    if (mxst_fold_sum(ctx, &s, &v))
        return JS_EXCEPTION;
    /* 0/0 for the empty array is the documented NaN. */
    return JS_NewFloat64(ctx, v / (double)s.len);
}

/* magic: 0 variance, 1 stddev. */
                             ...
                             int argc, JSValueConst *argv, int magic)
{
    ...
    int pop;
    (void)this_val;
    /* DataFrame's rule: coerce EVERY argument before binding any pointer.
     * A `pop` getter is user JS; it must not run while a raw pointer from
     * mxst_src_of is alive. */
    pop = mxst_read_pop(ctx, argc >= 2 ? argv[1] : JS_UNDEFINED);
    if (pop < 0)
        return JS_EXCEPTION;
    if (mxst_src_of(ctx, argv[0], &s))
        return JS_EXCEPTION;
    /* Sample needs two observations (n-1 divisor); a population of zero
     * is undefined, but a population of ONE has variance exactly 0, the
     * DataFrame convention -- so that case still runs the fold (it must,
     * a NaN element has to win over the shortcut). */
...
/* R-7 / "linear": pos = q*(n-1) between the closest ranks -- the exact
 * DataFrame.QUANTILE convention. magic 0 quantile (takes q), 1 median. */
                                  ...
                                  int argc, JSValueConst *argv, int median)
{
    ...
    uint32_t lo;
    (void)this_val; (void)argc;
    /* DataFrame's rule, q edition: JS_ToFloat64 on argv[1] can run user JS
     * (valueOf), so q is coerced AND validated BEFORE the source pointer is
     * bound -- the loop-2 detach battery originally missed this window
     * because cov/corr's getters were covered but q's was not, and ASan
     * heap-use-after-free read the freed store in mxst_materialize. */
    ...
    lo = (uint32_t)pos;
    frac = pos - (double)lo;
    /* The frac == 0 arm is LOAD-BEARING, as in DataFrame.QUANTILE: the
     * interpolation below evaluates a[lo] + 0 * (a[lo+1] - a[lo]) and
     * 0 * Infinity is NaN. Do not fold this branch away. */
...
/* Math.min/Math.max semantics: NaN poisons, empty returns the zero-ring
 * identity (+Inf for min, -Inf for max), and of two equal zeros the
 * signbit wins (-0 for min, +0 for max) per the ECMAScript spec. */
static JSValue dyn_stats_minmax(JSContext *ctx, JSValueConst this_val,
                                int argc, JSValueConst *argv, int ismax)
{
    mxst_src s;
    double best = ismax ? -INFINITY : INFINITY;
    uint32_t i;
    (void)this_val; (void)argc;
    if (mxst_src_of(ctx, argv[0], &s))
        return JS_EXCEPTION;
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    int pop;
    (void)this_val;
    /* DataFrame's rule again: the opts getter is user JS and runs BEFORE
     * either raw pointer is bound. The sources' own `length` reads can
     * also run getters (js_mathx_len), which is why fold_cov re-resolves
     * a fast side after any fallback read in the mixed case. */
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    if (mxst_fold_cov(ctx, &a, &b, 1, &cxy, &m2x, &m2y))
        return JS_EXCEPTION;
    /* NaN inputs poison a moment: the answer is NaN, like every other
     * stats function. An exactly-constant input is different: undefined,
     * and thrown (see the section comment) rather than a silent NaN. */
    if (isnan(m2x) || isnan(m2y))
        return JS_NewFloat64(ctx, NAN);
    if (!(m2x > 0.0) || !(m2y > 0.0))
        return JS_ThrowRangeError(ctx, "corr: zero variance (constant input)");
    /* sqrt each side BEFORE multiplying, as DataFrame.CORR does: the
       product can overflow where the roots cannot, and Inf in the
       denominator would report a perfectly correlated pair as 0. */
    r = cxy / (sqrt(m2x) * sqrt(m2y));
    /* Cauchy-Schwarz bounds |r| at 1; floating point does not know that. */
    if (r > 1.0)  r = 1.0;
    if (r < -1.0) r = -1.0;
...
/* One shared body: the three readers differ only in the width mask. */
...
/* RotateRight: the named twin of RotateLeft, added for readability.
 * Same k normalization, mirrored shift: s = k mod width (a NEGATIVE k,
 * reduced modulo the width, is exactly rotateLeft(-k)), s == 0 (k == 0 or
 * a multiple of the width) short-circuits the C `x<<width` UB, and the
 * width < 64 results are masked because `1 << 64` is UB. Lives here rather
 * than in core/dyn-bits.h because only this JS surface wants it. */
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (argc > 2 && !JS_IsUndefined(argv[2])) {
        ...
        if (!tail)
            return JS_EXCEPTION;
        /* a typo like "uper" silently computing the LOWER tail is how a
           p-value gets computed from the complement without anyone noticing */
                                 ...
                                 int argc, JSValueConst *argv)
{
    ...
    if (argc > 2 && !JS_IsUndefined(argv[2])) {
        ...
        if (JS_ToFloat64(ctx, &k, argv[2]))
            return JS_EXCEPTION;
        /* the (int) cast is UB for NaN/Inf/out-of-range; anything outside
           1..2 is rejected by dyn_besselh anyway, but only after the cast.
           A non-integer kind is refused here rather than silently truncated
           to its floor (2.5 used to compute as kind 2). */
                                    ...
                                    int argc, JSValueConst *argv)
{
    ...
    /* the recurrence is O(n) and overflows float64 past ~150: bound it
       (order above degree stays NaN, the documented contract) */
    if (dn > 150.0)
...
int js_nat_init_mathx(JSContext *ctx)
{
    ...
    if (!m)
        return -1;
    /* Capture the Float64Array class id before anything can call into
     * stats (see mxst_src_of; built-in ids are process-global constants).
     * 0 degrades the fast path to the per-element fallback, never to a
     * wrong answer. */
