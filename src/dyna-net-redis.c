 ...
 * not work to hand another thread.
 *
...
/* The constructor's options bag is checked against this table BEFORE any
 * option is read: an unknown key throws a TypeError naming the key AND the
 * full valid set, instead of being silently ignored. Same shape as the pilot
 * dyn_opts_strict in dyna-file.c: a non-object bag counts as absent; own
 * ENUMERABLE STRING keys only (symbol and inherited keys are invisible); a
 * Proxy ownKeys trap that throws propagates; an array bag's "0","1",...
 * keys are unknown -> throws. */
...
static const char *const rd_ctor_keys[] = {
    "tls", "ca", "host", "path", "username", "password", "port", "db",
    "binary", "bigint", "maxReplyBytes", "maxPending",
...
typedef struct {
    ...
    int hello_queued;       /* the handshake is enqueued EXACTLY once: the
                               constructor and the connect callback both want
                               it on the wire early, and enqueueing it twice
                               desyncs the reply stream (a peer that answers
                               once per read leaves the second HELLO's reply
                               to settle the first user command and every
                               later command waits for ever) */
...
#ifdef CONFIG_TLS
    int use_tls;                        /* tls:true — connect through TLS */
    char *tls_ca;                       /* optional CA PEM path (NULL = system) */
    dyn_tls_ctx_t *tls_ctx;             /* job/conn-owned; freed at teardown */
#endif
} dyn_redis_t;
...
/* The teardown shape of pend_free: no context may be read here (the runtime's
   final sweep runs after JS_FreeContext), and nothing may be CALLED -- a
   promise settled from a finalizer would enqueue reaction jobs that the dead
   loop can never drain, which JS_FreeRuntime then reports as object leaks. */
...
/* `depth` is the container nesting this call sits at (top level = 0). The
 * wire path already guarantees it stays under DYN_RESP_MAX_DEPTH: redis_on_recv
 * runs the ITERATIVE dyn_resp_scan over the exact buffer before dispatch, and
 * the scan refuses nesting past the cap. The recursion still carries its own
 * slot -- decode must not take a peer's word for it, or lean on every future
 * caller remembering to scan first . Over cap it raises the same
 * named protocol error the scan uses, never a stack overflow. */
                           ...
                           dyn_resp_reader_t *rd, int depth, int *is_err)
{
    ...
    case DYN_RESP_PUSH: {
        ...
        for (i = 0; i < it.count; i++) {
            ...
            /* DEFINE, not set: no setter walk per element of every reply. */
...
static void redis_one_reply(dyn_redis_t *r, const uint8_t *msg, size_t len)
{
    ...
    if (p->want > 1) {
        ...
        v = p->acc;
        p->acc = JS_UNDEFINED;
        redis_settle(r, p, 0, v);   /* a pipeline reports per-element errors */
        return;
    }
    pend_pop(&r->head, &r->tail);
    r->npending--;
...
static void redis_fail_all(dyn_redis_t *r, const char *msg)
{
    ...
    while ((p = pend_pop(&r->head, &r->tail)) != NULL) {
        ...
        else redis_settle(r, p, 1, redis_conn_error(ctx, msg));
    }
    while ((p = pend_pop(&r->wq_head, &r->wq_tail)) != NULL) {
        r->nwait--;
     ...
     * tick hook and reactor ref are pure liveness leaks -- without this drop
     * a script that forgets close() hangs the loop forever on the 250 ms
     * tick (same shape as pg_fail_all; see the note there). */
    if (r->hooked) {
        dyn_net_off_drain(r);
        r->hooked = 0;
    }
                          ...
                          unsigned len, void *ud)
{
    ...
    if (res < 0) {
        ...
        redis_gone(r);   /* a close() inside h_error deferred: finish it */
        return;
    }
    if (res == 0 && len == 0) {
...
#ifdef CONFIG_TLS
/* TLS handshake settled: fire the HELLO (the bytes encrypt through the
   engine dyn_aio_send already consults), or fail the client. */
...
/* Queue the HELLO/AUTH/SELECT handshake (used by both the plaintext and
   TLS paths; under TLS this runs only after the engine is up).
   IDEMPOTENT: the constructor queues it so it precedes any command issued on
   the next line, and the connect callback queues it so it goes out the moment
   the socket is up. Both cannot win: the handshake on the wire TWICE desyncs
   the reply stream -- a peer that answers once per read leaves the second
   HELLO's reply to settle the first user command, and every later command
   then waits for its reply for ever.
   Returns 0 (also when the handshake was already queued), or -1 on OOM. */
static int redis_queue_hello(dyn_redis_t *r)
{
                             ...
                             unsigned len, void *ud)
{
    ...
    if (res < 0) {
        ...
        redis_gone(r);   /* complete a close() deferred from a callback */
        return;
    }
    r->state = RD_ST_HANDSHAKE;
#ifdef CONFIG_TLS
    if (r->use_tls) {
        /* C9: TCP is up; start TLS on the same fd. The HELLO is queued from
         * the handshake-done callback below — dyn_aio_send encrypts once the
         * engine is attached, so the protocol bytes never leave plaintext. */
        dyn_tls_opts_t to;
        char terr[192];
        ...
        }
        {
            ...
            if (dyn_aio_recv(r->aio, r->fd, 0, /*multishot=*/1,
                             redis_on_recv, r) < 0
        ...
        return;   /* HELLO is sent from redis_tls_done */
    }
#endif
    ...
    redis_queue_hello(r);   /* plaintext: HELLO goes out immediately
                               (a no-op when the constructor already queued
                               it -- exactly one HELLO goes on the wire) */
    if (redis_flush(r) < 0)
        redis_fail_all(r, "Redis: cannot write to the socket");
}
...
static void redis_tick(void *udata)
{
    ...
    /* redis_gone, not a DEAD test: a close() deferred from inside a callback
     * is COMPLETED here -- the tick is the only frame left that can run it. */
    if (redis_gone(r))
        return;
...
typedef struct {
    ...
    void *blk;          /* ONE allocation backing all four arrays */
    int n, nkeep;
} redis_args_t;
                      ...
                      JSValueConst *argv)
{
    int i;
    memset(a, 0, sizeof(*a));
    /* One block, not four callocs per command: argv | lens | keep |
     * is_cstr. Every member is 8-aligned (sizes below are multiples of 8),
     * so the layout needs no padding. */
    a->blk = calloc(1, (size_t)argc * (sizeof(char *) + sizeof(size_t)
                                       + sizeof(JSValue) + 1));
    if (!a->blk) {
    ...
    a->keep = (JSValue *)(a->lens + argc);
    a->is_cstr = (uint8_t *)(a->keep + argc);
    for (i = 0; i < argc; i++) {
                             ...
                             int want, JSValue acc)
{
    ...
    p = pend_new(ctx);
    if (!p) { JS_FreeValue(ctx, acc); return JS_ThrowOutOfMemory(ctx); }

    promise = JS_NewPromiseCapability(ctx, funcs);
    if (JS_IsException(promise)) {
        pend_free(ctx, p); JS_FreeValue(ctx, acc);
        return promise;
    }
    p->resolve = funcs[0];
    p->reject = funcs[1];
    ...
    if (r->state == RD_ST_READY) {
        /* Encode STRAIGHT INTO the output buffer: a READY client used to pay
         * a malloc + memcpy + free for an intermediate that lived one call.
         * The pre-READY arm below still needs its own copy -- those bytes
         * are stashed until the handshake releases them. */
        if (buf_reserve(&r->obuf, &r->ocap, r->olen + need) < 0 ||
            dyn_resp_cmd_encode(r->obuf + r->olen, need, argc, argv, lens,
                                NULL) < 0) {
            pend_free(ctx, p);
        ...
        r->olen += need;
        /* p->bytes is NULL, so redis_arm is bookkeeping only (deadline +
         * queue push); the write already happened into obuf above. */
        redis_arm(r, p);
        redis_flush_soon(r);
    } else {
        b = (uint8_t *)malloc(need);
...
/* Finalizer-mode guard. The SAME dispose backs explicit close() and the
   ...
   command (a promise that never settles strands its awaiter), while a
   finalizer may run when the context is already being destroyed -- calling JS
   there is illegal, and a settled promise would enqueue reaction jobs the
   dying loop never drains. In finalizer mode everything is released rt-form,
   with no calls.
   PIN: the entry is forced by tests/test_net_final_sweep.c, which takes the
   only references to an armed client (the command still queued) and drops
   the client's, running this finalizer with work in flight; the row then
   asserts the in-flight promise is RELEASED, not settled (row:
   redis-final-sweep of that file). Reverting the guard fails that row. */
...
static void dyn_redis_teardown(dyn_redis_t *r)
{
    ...
    /* The optional CA PEM path is owned by the struct from the constructor's
       opt_str on (the ctor's own error exits free it); teardown missed it,
       stranding one strdup'd string per tls:true client. */
    free(r->tls_ca);
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    if (JS_IsObject(opt)) {
...
#ifdef CONFIG_TLS
        /* C9: TLS is SUPPORTED — the handshake runs through the aio TLS
         * engine after TCP connects. An optional `ca` PEM path pins the
         * trust store; absent = the platform's. */
        ...
        if (r->db < 0 || r->db > 255) {
        #ifdef CONFIG_TLS
    if (r->tls_ctx) dyn_tls_ctx_free(r->tls_ctx);
    free(r->tls_ca);
#endif
     ...
     * command the caller issues on the next line — EXCEPT under TLS, where
     * it is queued from the handshake-done callback (the bytes must not
     * leave in plaintext before the engine is up). The connect callback
     * wants it on the wire too; redis_queue_hello is idempotent, so exactly
     * one HELLO reaches the server however many call sites ask. */
#ifdef CONFIG_TLS
    if (r->use_tls)
        goto after_hello;
#endif
