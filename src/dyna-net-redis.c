 ...
 * caller remembering to scan first (T-CODEQL-7). Over cap it raises the same
...
static void redis_queue_hello(dyn_redis_t *r);
...
static void redis_queue_hello(dyn_redis_t *r)
{
    ...
    if (r->pass) {
        ...
        redis_send_internal(r, RD_INT_HELLO, 5, five, NULL);
    } else {
        redis_send_internal(r, RD_INT_HELLO, 2, argvh, NULL);
    }
}
                              ...
                              int argc, JSValueConst *argv)
{
     ...
     * leave in plaintext before the engine is up). */
#ifdef CONFIG_TLS
    if (r->use_tls)
        goto after_hello;
#endif
