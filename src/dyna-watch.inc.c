...
static const JSClassDef dyn_watch_class = {
    "Watcher", .finalizer = dyn_res_finalizer, .gc_mark = dyn_watch_gc_mark,
};
...
static void dyn_watch_emit(dyn_watch_t *w, const char *type, const char *path)
{
    ...
    if (w->closed)
        return;     /* closed by an earlier callback in this same sweep */
    w->n_events++;
    ...
    for (i = 0; i < w->n_snap; i++) {
        ...
        for (j = 0; j < n_old; j++) {
            ...
            if (!ne->is_dir &&
                ...
                if (w->closed)
                    goto dropped;   /* closed in the callback: bail safely */
            }
            break;
        }
        if (!ne->seen)
    ...
    for (j = 0; j < n_old; j++) {
        if (!old[j].seen) {
            dyn_watch_emit(w, old[j].is_dir ? "unlinkDir" : "unlink",
                           old[j].path);
            if (w->closed)
                break;              /* stop delivering; dispose runs below */
        }
    }
...
dropped:
    ...
    /* A close() from inside one of the callbacks above deferred its teardown
       ...
       stack has unwound by now, so the watcher can be torn down here. */
    if (w->closed)
        dyn_watch_dispose(w);
}
...
static void dyn_watch_dispose(void *native)
{
    ...
    if (w->in_emit) {
        /* dispose is running FROM the change callback (close() inside the
           handler, or the last JS reference dropped there): freeing the
           snapshot and `w` now would leave dyn_watch_rescan walking freed
           memory. Mark closed -- the sweep stops delivering and runs this
           teardown itself once the callback stack has unwound. */
        w->closed = 1;
        return;
    }
    w->closed = 1;
                               ...
                               int argc, JSValueConst *argv)
{
    ...
    w->hooked = 1;
    /* Pin self while armed: a started-but-unreferenced watcher would
       otherwise be collected and go permanently silent. */
    JS_FreeValue(ctx, w->self);
    w->self = JS_DupValue(ctx, this_val);
    return JS_UNDEFINED;
}
