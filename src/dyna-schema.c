/* dyna:schema -- JSON Schema validator, Draft 2020-12 core keywords.
   ...
   unevaluatedProperties/unevaluatedItems (2020-12 §11.3) via per-frame
...
/* Compile a subschema at <path>/<keyword>/<token>. Applicator members are
   ...
   schema DOCUMENT (2020-12 §8.2.3.2 evaluation paths), so "#/properties/foo"
                             ...
                             dyn_scnode_t *n, const char *path, size_t plen)
{
    ...
    /* ---- unevaluatedProperties / unevaluatedItems (2020-12 §11.3) ----- */
    ...
    /* min/maxContains apply only alongside contains; without it they are
       ...
       validation ignores them, per 2020-12 §7.4 ("only applies in the
...
/* RFC 3986 percent-decoding of a $ref fragment. 2020-12 §8.2.3.2: the
...
/* An EVALUATION RECORD: which properties / item indices of ONE instance
   ...
   2020-12 §11.3: unevaluated* consumes the union of these annotations.
...
static JSAtom dyn_sc_cache_hash_atom = JS_ATOM_NULL;
                                      ...
                                      int argc, JSValueConst *argv)
{
    ...
    if (JS_IsObject(argv[0])) {
        ...
        if (dyn_sc_cache_hash_atom == JS_ATOM_NULL)
            dyn_sc_cache_hash_atom = JS_NewAtom(ctx, "__dyna_compiled_hash");
        cached = JS_GetProperty(ctx, argv[0], dyn_sc_cache_atom);
        hv = have_hash ? JS_GetProperty(ctx, argv[0], dyn_sc_cache_hash_atom)
                       : JS_UNDEFINED;
        ...
        if (!JS_IsException(cached))
            JS_DeleteProperty(ctx, argv[0], dyn_sc_cache_atom, 0);
    }
    ...
    if (JS_IsObject(argv[0]) && dyn_sc_cache_atom != JS_ATOM_NULL) {
        uint64_t h2 = 0;
        ...
        if (hres == 0) {
            ...
            hex[16] = 0;
            JS_DefinePropertyValue(ctx, argv[0], dyn_sc_cache_hash_atom,
