...
struct dyn_aio {
    ...
    aio_fd_t *fds;
    int cap;
    /* Listening sockets armed on this reactor. XNU can lose the listen
     * socket's readiness wakeup (a connection that dies in the accept queue
     * leaves later connections unreported -- forever), so while ANY listener
     * is armed the reactor also wakes itself on a timer and re-drains them
     * (aio_listeners_sweep). Packed into cap's padding: the struct is pinned
     * to one cache line by the assert below. */
    short n_listen;
    uint8_t timer_armed;
...
/* Non-blocking drain of one listening socket. Shared by the readiness
 * dispatch and the periodic sweep -- identical semantics: run the callback
 * per accepted fd until the backlog is empty. */
static void aio_accept_drain(dyn_aio_t *a, int fd, dyn_aio_cb acb, void *aud)
{
    for (;;) {
        ...
        if (c < 0) {
            if (errno == EINTR) continue;
            /* BSD accept() reports a connection that died IN THE QUEUE as
               ECONNABORTED (EPROTO on Linux) while others are still
               pending; skipping it keeps the drain alive. */
...
/* Period: slow enough to be free, fast enough that a connection accepted
 * into the backlog on a listener whose wakeup the kernel lost is served
...
static void aio_listeners_sweep(dyn_aio_t *a)
{
    ...
    for (fd = 0; fd < a->cap; fd++) {
        /* Re-read the slot EVERY iteration: the callback below can close
           other listeners (and realloc a->fds). */
        aio_fd_t *s = &a->fds[fd];
    ...
    if ((events & (DYN_EV_READ | DYN_EV_ERROR)) && s->r_op == AIO_OP_RECV) {
        ...
        if (s->tls && n > 0) {
            ...
            if (dyn_tls_feed(t, a->rbuf, (size_t)n) != 0) {
                /* Disarm BEFORE the callback (audit S0): tls_fail's teardown
                   closes the fd, and without clearing r_op the close path's
                   inflight-- ran but this slot still looked armed, so a fd
                   REUSED by the next accept inherited a stale RECV state.
                   Disarm and re-apply interest so a callback that does NOT
                   close keeps a consistent table. */
                aio_read_done(a, fd);
                aio_apply_interest(a, fd);
            ...
            if (!s->tls_up) {
                ...
                if (st < 0) {
                    /* S0: same disarm-first rule as the feed failure above.
                       Two aborted handshakes used to leave stale armed slots;
                       fd reuse then wedged the accept path permanently. */
                    aio_read_done(a, fd);
                    aio_apply_interest(a, fd);
...
void dyn_aio_drain(void *aio)
{
    ...
    /* XNU listen-socket wakeup loss: serve anything already sitting in an
     * accept queue regardless of whether the kernel reported it -- but ONLY
     * on the periodic tick that exists for that purpose. Running it on every
     * readiness batch cost an accept(2) per event to learn "queue empty"
     * (measured: 2.3 accept calls per request served; the tick that carries
     * the mitigation fires every 250 ms either way). */
...
int dyn_aio_run(dyn_aio_t *a, int timeout_ms)
{
    ...
    /* the lost-wakeup drain rides the timer tick, not every batch (above) */
    if (a->n_listen > 0 && dyn_evloop_timer_fired(a->lp))
        aio_listeners_sweep(a);
    return n;
}
...
/* The shared connect tail behind dyn_aio_connect_addr (and the offloaded
 * hostname path below): `fd` is ALREADY open and configured. Returns `fd`, or
 * -1/errno with the fd NOT closed -- the two callers disagree on who owns it
 * after failure: connect_addr's caller never saw the fd (so IT closes), while
 * the offloaded path already handed the fd back and leaves the caller's
 * res < 0 cleanup to close it. */
...
fail:
    ...
    if (a->inflight)
        a->inflight--;
    return -1;              /* fd stays open: ownership note above */
}
                         ...
                         dyn_aio_cb cb, void *udata)
{
    ...
    /* Resolve BEFORE the socket: the family decides what to open, and the old
       ...
       entirely. (Callers reaching this entry point hold a resolved address.) */
    ...
    if (aio_connect_on(a, fd, (const struct sockaddr *)sa, salen,
                       cb, udata) < 0) {
        int e = errno;      /* close() must not clobber the failure cause */
        close(fd);          /* the caller never saw this fd */
        errno = e;
        return -1;
    }
    return fd;
...
/* ---- connect() by NAME: resolution off the loop thread (audit E11-04) ----
 *
 * The real getaddrinfo used to run on the CALLING thread; every server-shaped
 * caller (the App proxy upstream among them) invokes it from inside a reactor
 * turn, so one slow resolver froze every connection the loop was holding. The
 * lookup now rides dyn_aio_offload: work() is nothing but getaddrinfo on a
 * pool thread (no JS, no aio state), done() connects on the loop thread.
 *
 * The constraint the offload cannot hide: the caller wants an fd BACK, now,
 * to store and later recv/close -- net-tcp/redis/pg and the proxy all treat
 * fd < 0 as "failed synchronously, udata already freed", so a hostname path
 * that returned -1-but-calls-back-later would hand those callbacks freed
 * memory. But the address family is unknown until the lookup answers, so a
 * family-true socket cannot exist at return time. Resolution: hand out a
 * DUAL-STACK socket (AF_INET6, IPV6_V6ONLY=0) and, in done(), connect it to
 * a v4-mapped address when the resolver answers AF_INET -- one fd, either
 * family, and the synchronous fd-return contract survives unchanged. A host
 * with no IPv6 at all (socket(AF_INET6) fails) keeps the old blocking
 * resolve: without a dual-stack socket there is no fd to return before the
 * family is known, and correct-but-rare beats async-but-contract-breaking.
 *
 * dyn_aio_offload ALWAYS COMPLETES (inline when the pool refuses), so the
 * hostname contract is "always calls back, never blocks the loop except when
 * the pool is unavailable" -- the inline fallback is byte-for-byte today's
 * blocking behavior, including its synchronous -1 on a failed lookup. The
 * fd's slot is RESERVED at submit (r_op=CONNECT, no registration), so a
 * dyn_aio_close or dyn_aio_cancel during the resolve window retires the
 * connect exactly like an armed one: done() then connects nothing and fires
 * nothing. A resolve still on a worker when the loop tears down is dropped
 * with its done, exactly like a disk job (dyn_pool_chan_free).
 *
 * Failed answers RETRY in resolver order (the shared dyn_aio_cand_t): the
 * completion is interposed by aio_cand_done, which re-arms the next address
 * on the same fd number. The chain settles exactly once, with the first
...
typedef struct {
    ...
    /* Verdict: written by work (pool thread), read by done and the inline
     * return path; the pool's completion handoff under its mutex orders the
     * writes. err 0 = at least one candidate (cand valid) and, once done()
     * ran, armed. */
    dyn_aio_cand_t *cand;   /* heap: outlives the resolve into the retry chain */
    int err;                /* 0, or the errno this failure reports */
    /* Set by dyn_aio_connect immediately before returning the fd, ONLY when
     * the offload went to the pool: done() then runs after the caller holds
     * the fd, so failure must be delivered as the callback, never a return
     * code. Loop-thread-only (done runs from dyn_pool_drain on this same
     * thread, and nothing drains between dyn_pool_submit accepting and this
     * flag being set). */
    int past_return;
} aio_resolve_t;

static void aio_resolve_work(void *p)   /* POOL THREAD: no JS, no aio state */
{
                          ...
                          unsigned n, void *ud)
{
    ...
    if (res != 0 && cand->i + 1 < cand->n) {
        ...
        if (dyn_aio_cand_refresh_fd(cand) == 0 &&
            aio_connect_on(a, cand->fd,
                           (const struct sockaddr *)&cand->addr[cand->i],
                           cand->len[cand->i], aio_cand_done, cand) >= 0)
...
static void aio_resolve_done(void *p)   /* LOOP THREAD */
{
    aio_resolve_t *c = (aio_resolve_t *)p;
    dyn_aio_t *a = c->aio;
    aio_fd_t *s;

    /* The slot was RESERVED at submit. If the caller closed or cancelled
     * during the resolve window the reservation is gone (and the fd number
     * may even be in reuse by now) -- connecting would cross-wire an
     * unrelated descriptor, so retire silently, like any cancelled op.
     * Fresh read of a->fds: it may have been reallocated since submit. */
    s = &a->fds[c->fd];
    if (s->r_op != AIO_OP_CONNECT || s->r_cb != c->cb ||
        s->r_udata != c->udata) {
        free(c->cand);
        free(c->host);
        free(c);
    ...
    if (c->past_return) {
         ...
         * success fires from the writability arm, failure fires here. The fd
         * stays OPEN -- the caller's res < 0 path closes it, exactly as it
         * would for a connect the kernel refused. */
        if (c->err && c->cb)
         ...
         * the fd, so close it here and fire no callback -- the caller gets
         * today's synchronous -1/errno instead. */
        close(c->fd);
    }
    /* Inline success: the connect is armed and its completion deferred to the
     * writability dispatch; dyn_aio_connect returns the fd and frees the ctx. */
}
                    ...
                    dyn_aio_cb cb, void *udata)
{
    ...
    /* Fast path first, as before: AI_NUMERICHOST never invokes the name
     * resolution service, so an address literal cannot block and connect
     * by-address keeps today's synchronous family-true fd. */
    ...
    fd = socket(AF_INET6, SOCK_STREAM, 0);
    if (fd < 0) {
        /* No IPv6 here: keep today's synchronous resolve (see the block
         * comment above for why there is no safe async alternative). */
    ...
    fcntl(fd, F_SETFD, FD_CLOEXEC);
#ifdef IPV6_V6ONLY
    /* 0 is the platform default on macOS/Linux; set it so a sysctl cannot
     * flip this socket to v6-only and quietly break every AF_INET answer. */
    ...
    if (!c || !c->host || !c->cand) {
        /* Nothing was submitted, so no callback is owed -- the same shape as
         * today's synchronous failure when allocation fails under connect. */
        close(fd);
        free(c->cand);
        free(c->host);
        free(c);
    ...
    /* Reserve the fd's slot now: the resolve window is DNS-latency long, and
     * a dyn_aio_close/cancel in it must retire THIS op -- without the
     * reservation a reused fd number would get connected by done() below.
     * Registration (and the inflight count) still happen in the arm; the
     * reservation only marks ownership. */
    if (fd_ensure(a, fd) < 0) {
        close(fd);
        free(c->cand);
        free(c->host);
    ...
    if (r == 1) {
        /* Pool refused: work+done already ran inline on THIS thread -- today's
         * blocking behavior exactly, down to the synchronous -1/errno on a
         * failed lookup (done() closed the fd on that path). */
        if (c->err) {
            int e = c->err;
            free(c->cand);
            free(c->host);
            free(c);
            a->fds[fd].r_op = AIO_OP_NONE;   /* retire the reservation: the
                                              * fd is closed and the caller
                                              * never saw it */
            a->fds[fd].r_cb = NULL;
    ...
    /* Pooled: done() runs on a later drain, after the caller has the fd (it
     * only ever runs from dyn_pool_drain, on this same thread, and nothing
     * drains between the submit accepting and this flag being set). From here
     * every outcome -- including a failed lookup -- arrives as the callback,
     * never a return code; the errno is EINVAL, what the old synchronous path
     * reported for a name that would not resolve. */
    c->past_return = 1;
    return fd;
}
...
int dyn_aio_accept(dyn_aio_t *a, int listen_fd, dyn_aio_cb cb, void *udata)
{
    ...
    s->r_multishot = 1;
    a->inflight++;
    /* While anything is listening, wake the reactor periodically so the
     * sweep can serve connections whose readiness event the kernel lost. */
...
int dyn_aio_close(dyn_aio_t *a, int fd)
{
    ...
    if (s->r_op == AIO_OP_ACCEPT) {
        /* Last listener gone: the periodic sweep has nothing to serve. */
        if (--a->n_listen <= 0 && a->timer_armed) {
            a->n_listen = 0;
     ...
     * for the send -- a reference, or a query that then never resolves. */
    {
         ...
         * completion frees; closing mid-arm cancels that completion, so the
         * drop happens here or the ctx leaks. */
                     ...
                     size_t len, dyn_aio_cb cb, void *udata)
{
    ...
    if (fd_ensure(a, out_fd) < 0)
        return -1;
    s = &a->fds[out_fd];
    /* The file slot is single-depth BY DESIGN: buffered sends queue through
     * w_qhead, the file side deliberately has no such list. Overwriting an
     * in-flight transfer would leak its fd -- this function takes ownership --
     * and its callback would never fire, stranding whatever the caller holds
     * for it. Refuse; the caller sees EBUSY and can wait for the completion. */
    if (s->w_file_rem > 0) {
        errno = EBUSY;
        return -1;
    }
