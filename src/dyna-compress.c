 ...
 *               + RFC 1951 fixed codes), falling back to *stored*
...
/* ==================== strict options ====================
 * Until an unknown option key was silently ignored -- `{bytes:true}`-grade
 * bugs: gzip(data, {levle: 9}) "worked" and compressed at the default level.
 * Every options bag in this module is now checked against its valid-key table
 * and an unknown key throws a TypeError naming the key AND the full valid set.
 * Same shape as the pilot dyn_opts_strict in dyna-file.c:
 *   - a non-object bag (null / undefined / a primitive) is accepted as absent;
 *   - own ENUMERABLE STRING keys only: symbol-keyed and inherited keys are
 *     invisible by construction;
 *   - a Proxy ownKeys trap that throws propagates untouched;
 *   - an array bag has "0","1",... keys, which are unknown -> throws. */
                        ...
                        JSValueConst *argv)
{
    ...
    /* Optional level, two arities: the positional gzip(data, 9) and
     * the options gzip(data, { level: 9 }) -- the same C function handles
     * both, with the same validation. >= 6 selects the dynamic-Huffman
     * encoder (ratio over speed), anything below the fixed-Huffman fast path
     * -- the boundary is zlib's, for the same reason. A non-number is
     * refused, not swallowed: a mistyped level ("gzip(b, "9")") is a caller
     * bug worth naming. Coerce it FIRST (a throwing valueOf strands nothing). */
    if (argc > 1 && !JS_IsUndefined(argv[1])) {
        ...
        if (JS_IsObject(argv[1])) {
            ...
            if (JS_IsException(prop))
                return JS_EXCEPTION;
            lv_arg = prop; /* JS_UNDEFINED when the key is absent */
        } else {
            prop = JS_UNDEFINED;
        }
 ...
 * native is resolved or allocated (CLAUDE.md section 8). `level_hi` is the
 * codec's own ceiling (12 for the LZ4 family, 9 for gzip, zlib's convention);
 * the floor is 1 for all of them. */
                        ...
                        int *plevel, uint8_t **pdict, size_t *pdict_len)
{
    ...
    *pdict_len = 0;
    if (!JS_IsObject(opts))
        return 0;
    /* NO dyn_opts_strict here: this helper is shared by lz4Compress,
     * lz4Decompress, lz4Frame AND the Compressor ctor, and lz4Frame's table
     * (checksum) is a SUPERSET of this one -- checking here too would reject
     * lz4Frame's own {checksum} bag. Each caller checks against ITS table
                                     ...
                                     int argc, JSValueConst *argv)
{
    ...
    if (argc > 1 && !JS_IsUndefined(argv[1]) && !JS_IsNull(argv[1])
        && !JS_IsObject(argv[1]))   /* null = absent, the module convention */
                                  ...
                                  int argc, JSValueConst *argv)
{
        ...
        return JS_EXCEPTION;
    {
        ...
        if (rc < 0) {
            /* Each refusal NAMES its class (bad magic vs truncated vs
               corrupt block vs each checksum): the one generic "invalid LZ4
               frame" used to be every failure, which made a refused input
...
{
    int lo, hi, def;
    if (algo == 0) { lo = 1; hi = 9; def = 1; }       /* gzip */
    else if (algo == 3) { lo = 1; hi = 22; def = 3; } /* zstd */
                             ...
                             JSValueConst *argv)
{
    ...
    if (dyn_zstd_decode(src, src_len, DYN_MAX_OUTPUT, &o, &why) < 0) {
        free(src);
        free(o.buf);
        /* The zstd backend's own reason strings (ZSTD_getErrorName) are kept
           for detail, but always under the codec's name, in the module's
           "codec: reason" style. */
                                   ...
                                   int argc, JSValueConst *argv)
{
    ...
    if (argc > 0 && JS_IsObject(argv[0])) {
        JSValue v;
        /* dict is IN the table even for the algos that refuse it: the
         * "applies to algo lz4 only" message is the targeted one and must
         * keep firing, not a generic unknown-key error. */
        ...
        if (algo == 3 || algo == 4 || algo == 5) {
            ...
            /* gzip: zlib's 1..9 (6+ selects dynamic Huffman); the dict is
             * still read here so the "lz4 only" refusal path is unchanged. */
            return JS_EXCEPTION;
        }
                                         ...
                                         int argc, JSValueConst *argv)
{
    ...
    JSValue result;
    const char *why = NULL;
    const char *codec = NULL;    /* set when a backend hands back its own reason */
    int rc;

    /* Optional { asString: true } -> UTF-8 decode to a JS string, the same
     * option every one-shot decompressor takes. Read the (possibly
     * getter-backed) property before touching native buffers. */
    if (argc > 1 && JS_IsObject(argv[1])) {
        if (dyn_opts_strict(ctx, argv[1], cpr_asstring_keys, 1))
            return JS_EXCEPTION;
        JSValue v = JS_GetPropertyStr(ctx, argv[1], "asString");
    ...
    if (c->algo == 0)
        ...
        /* A zstd failure surfaces the backend's own reason string
           (ZSTD_getErrorName), so the codec is named here in the module's
           "codec: reason" style instead of leaking it raw. */
        if (rc < 0)
            codec = "zstd";
    }
    else if (c->algo == 4)
    ...
    if (rc < 0) {
        ...
        if (c->algo == 2)
            /* the LZ4 FRAME class names its refusal (see lz4Unframe) */
...
/* ---------- compressBound(n, { algo }) ------------------------------------- */

/* The worst-case output size of ONE codec, as this module's encoder can
 * produce it: the point is a caller-owned output buffer allocated
 * ONCE. Each formula is the bound ITS encoder actually reserves, not a
 * generic guess, so a buffer of this size always fits the codec's output:
 *
 *   gzip      RFC 1952 framing (10 + 8) around the all-stored fallback the
 *             encoder drops to on incompressible input: ceil(n/65535) stored
 *             blocks, each +5 bytes of block header (dyn_gzip_build_ctx).
 *   lz4       DYN_LZ4_BOUND: the raw-block bound the codec reserves
 *             (n + n/255 + 16, dyn-compress.c:2171).
 *   lz4frame  frame magic + descriptor (7) + per-block size fields (4 bytes
 *             per 4 MiB block) + the block bound + end mark (4) + the
 *             optional content checksum (4, included unconditionally so the
 *             bound holds for either checksum setting).
 *   zstd      ZSTD_compressBound, the linked library's own bound.
 *   brotli    BrotliEncoderMaxCompressedSize where libbrotli is linked;
 *             elsewhere the same formula from brotli's own source (it is the
 *             FORMAT's worst case, so any conforming encoder -- including the
 *             Apple compression backend -- stays under it).
 *   snappy    the vendored codec's own cap (n + n/50 + 16).
 */
                                     ...
                                     int argc, JSValueConst *argv)
{
    ...
    /* Every formula below is at most ~2n + small, so any n under 2^55 keeps
     * the arithmetic inside size_t. Nothing this engine will hand the codecs
     * is anywhere near that; the cap makes overflow reasoning local. */
    ...
    if (!strcmp(algo, "gzip")) {
        ...
        {   /* 10-byte header + 8-byte trailer + stored-block fallback
             * (ceil(n/65535), exactly the block count the encoder lays out) */
    ...
    if (!strcmp(algo, "lz4frame")) {
        JS_FreeCString(ctx, algo);
        {   /* magic+descriptor 7, 4 size bytes per 4 MiB block, block bound,
             * end mark 4, content checksum 4 */
    ...
    if (!strcmp(algo, "brotli")) {
        JS_FreeCString(ctx, algo);
#if defined(__APPLE__)
        /* The format's worst case (libbrotli's own formula): 2 header bytes,
         * 4 per 16 KiB large block, 4 tail. Holds for any conforming encoder. */
