                        ...
                        JSValueConst *argv)
{
    ...
    /* Optional level: >= 6 selects the dynamic-Huffman encoder (ratio over
     * speed), anything below the fixed-Huffman fast path -- the boundary is
     * zlib's, for the same reason. A non-number is refused, not swallowed: a
     * mistyped level ("gzip(b, "9")") is a caller bug worth naming. Coerce it
     * FIRST (a throwing valueOf strands nothing). */
