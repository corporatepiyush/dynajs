...
/* AVX-512 overrides — 512-bit ZMM registers, 16 floats per vector.
 * The DISPATCH CORE gates on AVX512F ALONE (dyna-simd-core.c): every kernel
 * installed below unconditionally is F-only, so F-only silicon (rare) runs
 * them correctly. The one DQ-dependent intrinsic (_mm512_cvtepi32_pd) lives
 * inside the opt-in DYNAJS_SIMD_INT_AVX512 gate, which an F-without-DQ host
 * must not enable. */
...
/* sqrtf over the vector _sq kernel, exactly the NEON/SSE4.2 shape: an
 * AVX-512 host previously ran the SCALAR loop here while an SSE4.2 host ran
 * the vector one (audit E13-15). */
__attribute__((target("avx512f,avx512bw,avx512dq"))) static float
simd_avx512_dist_l2(const float *restrict a,
                    const float *restrict b, size_t d) {
  return sqrtf(simd_avx512_dist_l2_sq(a, b, d));
}
