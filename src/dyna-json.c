...
#define DYN_JP_MAX_OPS   65536  /* ops per Patch.apply */
...
/* Decoded token as an array index. 1 + idx; 0 = not digits; -1 = malformed
   (empty, or a leading zero "01"/"00"); -2 = all digits but over 19, so no
   array can hold it -- an out-of-range outcome, not a syntax one. "-" is NOT
   handled. */
...
static JSValue dyn_jp_clone(JSContext *ctx, JSValueConst v, int depth)
{
    ...
    if (JS_IsArray(ctx, v)) {
        ...
        JS_FreeValue(ctx, lv);
        /* a sparse length-2^29 array would make one clone walk 500M holes */
        if (len > (1u << 22)) {
            JS_ThrowRangeError(ctx, "dyna:json: array too long to clone "
                                    "(%llu elements; cap is %u)",
                                (unsigned long long)len, 1u << 22);
            return JS_EXCEPTION;
        }
        res = JS_NewArray(ctx);
...
enum {
    ...
    JP_TOOBIG,    /* all-digit index over 19 digits -- out of range, not syntax */
    ...
    JP_TOODEEP,   /* pointer has more than DYN_JP_MAX_DEPTH tokens */
};
                            ...
                            const dyn_jp_walk_t *w, const char *p, size_t n)
{
    ...
    case JP_TOOBIG:
        JS_ThrowTypeError(ctx,
            "%s: array index in \"%s\" is out of range (over 19 digits)",
            pfx, p);
        break;
    case JP_APPEND:
        JS_ThrowTypeError(ctx,
    ...
    case JP_MISSING:
        ...
        break;
    case JP_TOODEEP:
        JS_ThrowRangeError(ctx, "%s: pointer deeper than %d levels: \"%s\"",
                           pfx, DYN_JP_MAX_DEPTH, p);
        break;
    }
    return JS_EXCEPTION;
}
                       ...
                       size_t tlen, dyn_jp_walk_t *w, JSValue *next)
{
    ...
    if (JS_IsArray(ctx, cur)) {
        ...
        if (r == -1)
            return JP_LEADZERO;
        if (r == -2)
            return JP_TOOBIG;   /* over 19 digits: out of range, not syntax */
        ...
        JSValue el = JS_GetPropertyUint32(ctx, cur, (uint32_t)idx);
        if (JS_IsException(el))
            return -1;
        /* Get() reads a HOLE as undefined. Classify an undefined result via
           the own property: an explicit undefined element EXISTS, a hole does
           not -- MISSING for every caller, matching the object walk's
           own-property discipline. Only the undefined result pays for it. */
        if (JS_IsUndefined(el)) {
            JSAtom ha = JS_NewAtomUInt32(ctx, (uint32_t)idx);
            JSValue probe;
            int hr;
            if (ha == JS_ATOM_NULL) {
                JS_FreeValue(ctx, el);
            ...
            hr = dyn_jp_get_own(ctx, cur, ha, &probe);
            JS_FreeAtom(ctx, ha);
            if (hr < 0) {
                JS_FreeValue(ctx, el);
                return -1;
            ...
            if (hr == 0) {
                JS_FreeValue(ctx, el);  /* hole: *next stays untouched */
                return JP_MISSING;
            }
                       ...
                       JSValue *out, dyn_jp_walk_t *w)
{
    ...
    if (p[0] != '/')
        return JP_SYNTAX;
    /* Documented bound (API.md): a pointer deeper than DYN_JP_MAX_DEPTH
       levels throws. ntok counts '/' bytes = the token count. Checked before
       any allocation; the walk itself is iterative, so this is a contract
       bound, not a stack-safety one. */
    ntok = 0;
    for (i = 0; i < n; i++)
        if (p[i] == '/')
            ntok++;
    if (ntok > (size_t)DYN_JP_MAX_DEPTH)
        return JP_TOODEEP;
...
/* ---- the ONE add/remove/replace ladder ---------------------------------- */

/* Where an add/remove/replace path landed: the parent container, the decoded
   final token, and (arrays) the validated index. */
typedef struct {
    JSValue  parent;    /* owned */
    char    *scratch;   /* owned; the decoded token is scratch[0..tlen) */
    size_t   tlen;
    uint64_t idx;       /* DYN_JP_AT_ARRAY: the index validated for the op */
} dyn_jp_at_t;

typedef enum {
    DYN_JP_AT_ERR = 0,  /* exception thrown; *at still owns resources */
    DYN_JP_AT_ARRAY,    /* parent is an array, idx validated */
    DYN_JP_AT_MEMBER,   /* parent is an object; token is scratch[0..tlen) */
} dyn_jp_at_kind_t;

static void dyn_jp_at_free(JSContext *ctx, dyn_jp_at_t *at)
{
    JS_FreeValue(ctx, at->parent);
    if (at->scratch)
        js_free(ctx, at->scratch);
    at->parent = JS_UNDEFINED;
    at->scratch = NULL;
    at->tlen = 0;
    at->idx = 0;
}

/* The shared parent-walk -> token-validate -> container-classify ladder under
   add/remove/replace (each op body carried its own diverging copy). Error
   messages are byte-identical to the ladders this replaces -- including
   remove's missing op tag -- with one deliberate change: an all-digit index
   over 19 digits now reports "out of range" instead of "leading zero".
   `label` is the op's message tag ("add: ", "replace: "; remove used none).
   `range_tail` completes the array range message for the non-add ops
   ("array index out of range " for remove, "no value at " for replace).
   `is_add` enables add's two special forms: a missing INTERMEDIATE member is
   reported as "parent does not exist" (not dyn_jp_throw's wording), and
   '-' / idx == len validate as an append. Returns the kind; on
   DYN_JP_AT_ERR the caller frees *at (uniform ownership) and propagates. */
static dyn_jp_at_kind_t dyn_jp_resolve_at(JSContext *ctx, JSValue *work,
                                          const char *pfx, const char *label,
                                          const char *range_tail, int is_add,
                                          ...
                                          dyn_jp_at_t *at)
{
    ...
    at->tlen = 0;
    at->idx = 0;
    at->scratch = js_malloc(ctx, plen + 1);
    if (!at->scratch)
        return DYN_JP_AT_ERR;
    code = dyn_jp_walk(ctx, *work, path, plen, 0, at->scratch, &at->tlen,
                       &at->parent, &w);
    if (code) {
        if (is_add && code == JP_MISSING)
            JS_ThrowTypeError(ctx,
                "%s: %sparent does not exist at \"%s\"", pfx, label, path);
        else
            dyn_jp_throw(ctx, pfx, code, &w, path, plen);
        dyn_jp_at_free(ctx, at);
        return DYN_JP_AT_ERR;
    }
    if (JS_IsArray(ctx, at->parent)) {
        uint64_t len, idx;
        int r;
        len = dyn_jp_arr_len(ctx, at->parent);
        if (len == (uint64_t)-1) {
            dyn_jp_at_free(ctx, at);
            return DYN_JP_AT_ERR;
        }
        if (is_add && at->tlen == 1 && at->scratch[0] == '-') {
            at->idx = len;                      /* append */
            return DYN_JP_AT_ARRAY;
        }
        r = dyn_jp_parse_index(at->scratch, at->tlen, &idx);
        if (r == 0)
            JS_ThrowTypeError(ctx,
                "%s: %stoken is not an array index at \"%s\"", pfx, label,
                path);
        else if (r == -1)
            JS_ThrowTypeError(ctx,
                "%s: %sarray index has a leading zero at \"%s\"", pfx, label,
                path);
        else if (r == -2)
            JS_ThrowTypeError(ctx,
                "%s: %sarray index in \"%s\" is out of range (over 19 digits)",
                pfx, label, path);
        else if (idx > len || (!is_add && idx >= len)) {
            if (is_add)
                JS_ThrowTypeError(ctx,
                    "%s: %sarray index %llu out of range (length %llu) at "
                    "\"%s\"", pfx, label, (unsigned long long)idx,
                    (unsigned long long)len, path);
            else
                JS_ThrowTypeError(ctx, "%s: %s%sat \"%s\"", pfx, label,
                                  range_tail, path);
            r = -1;
        }
        if (r != 1) {
    ...
    if (!JS_IsObject(at->parent)) {
        JS_ThrowTypeError(ctx,
            "%s: %scannot descend into a scalar at \"%s\"", pfx, label, path);
        dyn_jp_at_free(ctx, at);
        return DYN_JP_AT_ERR;
    }
    return DYN_JP_AT_MEMBER;
}
                         ...
                         const char *path, size_t plen, JSValue vadd)
{
    ...
    kind = dyn_jp_resolve_at(ctx, work, pfx, "add: ", NULL, 1, path, plen,
                             &at);
    if (kind == DYN_JP_AT_ERR) {
        JS_FreeValue(ctx, vadd);
        return -1;
    }
    if (kind == DYN_JP_AT_ARRAY) {
        if (dyn_jp_arr_insert(ctx, at.parent, at.idx, vadd) < 0) {
            dyn_jp_at_free(ctx, &at);
            return -1;              /* vadd consumed by the shift */
        }
    } else {
        /* DEFINE: an own data property, so a "__proto__" key cannot retarget
           the prototype. add defines unconditionally (RFC 6902 4.1). */
        JSAtom a = JS_NewAtomLen(ctx, at.scratch, at.tlen);
        int r;
        if (a == JS_ATOM_NULL) {
            dyn_jp_at_free(ctx, &at);
            JS_FreeValue(ctx, vadd);
        ...
        r = JS_DefinePropertyValue(ctx, at.parent, a, vadd, JS_PROP_C_W_E);
        JS_FreeAtom(ctx, a);
        if (r < 0) {
            dyn_jp_at_free(ctx, &at);
                            ...
                            const char *path, size_t plen)
{
    ...
    if (plen == 0) {
        /* pfx already names the op ("Patch.apply[i]") or the function
           ("Pointer.remove"); a second "remove: " tag read as a stutter. */
        JS_ThrowTypeError(ctx, "%s: cannot remove the document root", pfx);
        return -1;
    }
    kind = dyn_jp_resolve_at(ctx, work, pfx, "", "array index out of range ",
                             0, path, plen, &at);
    if (kind == DYN_JP_AT_ERR)
        return -1;
    if (kind == DYN_JP_AT_ARRAY) {
        r = dyn_jp_arr_remove(ctx, at.parent, at.idx);
    } else {
        JSAtom a = JS_NewAtomLen(ctx, at.scratch, at.tlen);
        JSValue old;
        int found;
        ...
        found = dyn_jp_get_own(ctx, at.parent, a, &old);
        if (found == 0) {
            ...
            r = JS_DeleteProperty(ctx, at.parent, a, 0);
        }
        JS_FreeAtom(ctx, a);
    }
    dyn_jp_at_free(ctx, &at);
                             ...
                             const char *pfx, const char *path, size_t plen)
{
    ...
    kind = dyn_jp_resolve_at(ctx, work, pfx, "replace: ", "no value ", 0,
                             path, plen, &at);
    if (kind == DYN_JP_AT_ERR) {
    ...
    if (kind == DYN_JP_AT_ARRAY) {
        /* SetProperty consumes vrep in all cases. */
        r = JS_SetPropertyUint32(ctx, at.parent, (uint32_t)at.idx, vrep);
    } else {
        /* DEFINE: an own data property, so a "__proto__" key cannot retarget
           the prototype; the target must exist (RFC 6902 4.3). */
        JSAtom a = JS_NewAtomLen(ctx, at.scratch, at.tlen);
        JSValue old;
        int found;
        if (a == JS_ATOM_NULL) {
        ...
        if (found == 0) {
            ...
            JS_FreeValue(ctx, vrep);            /* exception already pending */
        } else {
            JS_FreeValue(ctx, old);
            r = JS_DefinePropertyValue(ctx, at.parent, a, vrep, JS_PROP_C_W_E);
        }
        JS_FreeAtom(ctx, a);
    }
    dyn_jp_at_free(ctx, &at);
                            ...
                            JSValueConst *argv, int magic)
{
    ...
    JS_FreeValue(ctx, lenv);
    if (nops > DYN_JP_MAX_OPS) {
        JS_FreeValue(ctx, work);
        JS_ThrowRangeError(ctx,
            "Patch.apply: too many ops (%llu; cap is %d)",
            (unsigned long long)nops, DYN_JP_MAX_OPS);
        return JS_EXCEPTION;
    }
    for (i = 0; i < nops; i++) {
                                  ...
                                  int argc, JSValueConst *argv, int magic)
{
    ...
    if (code == JP_MISSING || code == JP_SCALAR || code == JP_NOTNUM ||
        code == JP_RANGE || code == JP_APPEND || code == JP_TOOBIG) {
        JS_FreeCString(ctx, p);
        return JS_FALSE;    /* not there, not misuse; an over-long index is
                               out of range exactly like JP_RANGE */
    }
    if (code) {
...
/* Newline-delimited JSON, buffer-at-once: the whole text arrives as one
   string and every line is parsed (or every item serialized) before anything
   is returned. The STREAMING form -- a chunk-fed AsyncIterable whose lines
   split on '\n' bytes wherever they fall -- is stream.ndjson(src) in
   dyna:stream; there is deliberately no second streaming entry point here
   (the plan cut Ndjson.reader in pass 3 as a duplicate surface).

   The line rules match dyna:stream's ndjson exactly, so a buffer and a
   stream over the same bytes produce the same value list:
     - lines split on '\n'; one trailing '\r' is stripped (CRLF input);
     - a line that is empty or only spaces/tabs/CRs is skipped;
     - line numbers in errors are 1-based and count SKIPPED lines too --
       they are positions in the file, not indexes into the result.
   A malformed line throws a SyntaxError naming that line number (the one
   thing a byte offset cannot tell a log-file author). */

static JSValue dyn_ndjson_parse(JSContext *ctx, JSValueConst this_val,
                                int argc, JSValueConst *argv)
{
    ...
    (void)this_val;
    if (argc < 1 || !JS_IsString(argv[0]))
        return JS_ThrowTypeError(ctx, "Ndjson.parse(text): text must be a string");
    src = JS_ToCStringLen(ctx, &n, argv[0]);
    if (!src)
        return JS_EXCEPTION;
    rt = JS_GetRuntime(ctx);
    /* Own mutable copy: each line is NUL-terminated in place, which is what
       JS_ParseJSON requires (buf[buf_len] == '\0'). */
    copy = (char *)js_malloc_rt(rt, n + 1);
    if (!copy) {
    ...
    for (pos = 0; pos < n; ) {
        char *line = copy + pos, saved;
        const char *nl = (const char *)memchr(copy + pos, '\n', n - pos);
        size_t llen = nl ? (size_t)(nl - line) : n - pos;
        size_t j = 0;
        JSValue val;

        pos += llen + 1;                    /* past the newline; > n ends it */
        lineno++;
        if (llen && line[llen - 1] == '\r')
            llen--;                         /* a CRLF file is not a different file */
        while (j < llen && (line[j] == ' ' || line[j] == '\t' || line[j] == '\r'))
            j++;
        if (j == llen)
            continue;                       /* a blank line carries no value */
        saved = line[llen];
        line[llen] = '\0';
        val = JS_ParseJSON(ctx, line, llen, "<dyna:json:ndjson>");
        line[llen] = saved;
        if (JS_IsException(val)) {
            /* Re-throw with the LINE NUMBER, the dyna:stream ndjson rule. */
            JSValue exc = JS_GetException(ctx);
            JSValue msg = JS_GetPropertyStr(ctx, exc, "message");
            const char *m = JS_IsString(msg) ? JS_ToCString(ctx, msg) : NULL;
            JS_FreeValue(ctx, exc);
            JS_ThrowSyntaxError(ctx, "Ndjson.parse: line %llu: %s",
        ...
        if (JS_DefinePropertyValueUint32(ctx, out, k++, val,
                                         JS_PROP_C_W_E) < 0) {
            JS_FreeValue(ctx, out);
            js_free_rt(rt, copy);
            return JS_EXCEPTION;
...
static JSValue dyn_ndjson_stringify(JSContext *ctx, JSValueConst this_val,
                                    int argc, JSValueConst *argv)
{
    JSRuntime *rt;
    JSValue lenv;
    ...
    if (argc < 1 || JS_IsArray(ctx, argv[0]) != 1)
        return JS_ThrowTypeError(ctx,
            "Ndjson.stringify(items): items must be an array");
    lenv = JS_GetPropertyStr(ctx, argv[0], "length");
    if (JS_ToIndex(ctx, &len, lenv)) {
        JS_FreeValue(ctx, lenv);
        return JS_EXCEPTION;
    }
    JS_FreeValue(ctx, lenv);
    ...
    for (i = 0; i < len; i++) {
        ...
        /* Compact by construction: JSON.stringify's output only gains
           whitespace from a `space` argument, and none is passed. */
        s = JS_JSONStringify(ctx, v, JS_UNDEFINED, JS_UNDEFINED);
        JS_FreeValue(ctx, v);
        if (JS_IsException(s)) {
            js_free_rt(rt, buf);            /* a BigInt TypeError propagates */
            return JS_EXCEPTION;
        }
        if (!JS_IsString(s)) {
            /* undefined/a function/a symbol stringify to NOTHING: an empty
               line here would corrupt the file for every reader after it. */
            JS_FreeValue(ctx, s);
            js_free_rt(rt, buf);
            return JS_ThrowTypeError(ctx,
                "Ndjson.stringify: item %llu has no JSON form "
                "(undefined, a function or a symbol)",
                (unsigned long long)(i + 1));
        }
        cs = JS_ToCStringLen(ctx, &cl, s);
        JS_FreeValue(ctx, s);
        if (!cs) {
            js_free_rt(rt, buf);
            return JS_EXCEPTION;
        }
        if (buflen + cl + 1 > bufcap) {
            size_t ncap = bufcap ? bufcap : 256;
            char *nb;
            while (ncap < buflen + cl + 1)
                ncap *= 2;
            nb = (char *)js_realloc_rt(rt, buf, ncap);
            if (!nb) {
        ...
        memcpy(buf + buflen, cs, cl);
        buflen += cl;
        buf[buflen++] = '\n';
        JS_FreeCString(ctx, cs);
    }
    {
        /* JS_NewStringLen COPIES the bytes: the growth buffer must be
           released here or every successful call retains its largest
           doubling at exit (the retained-buffer teardown class). */
        JSValue out = JS_NewStringLen(ctx, buf ? buf : "", buflen);
        js_free_rt(rt, buf);
        return out;
    }
}

static const JSCFunctionListEntry dyn_ndjson_funcs[] = {
    JS_CFUNC_DEF("parse", 1, dyn_ndjson_parse),
    JS_CFUNC_DEF("stringify", 1, dyn_ndjson_stringify),
};
...
static const JSCFunctionListEntry dyn_jp_funcs[] = {
    ...
    JS_OBJECT_DEF("Patch", dyn_jp_patch_funcs, countof(dyn_jp_patch_funcs),
                  JS_PROP_WRITABLE | JS_PROP_CONFIGURABLE),
    JS_OBJECT_DEF("Ndjson", dyn_ndjson_funcs, countof(dyn_ndjson_funcs),
                  JS_PROP_WRITABLE | JS_PROP_CONFIGURABLE),
};
