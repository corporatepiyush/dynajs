...
#define BL3_ROOT                8u
#define BL3_KEYED               0x10u    /* KEYED_HASH (blake3_impl.h) */
#define BL3_DERIVE_CTX          0x20u    /* DERIVE_KEY_CONTEXT */
#define BL3_DERIVE_MAT          0x40u    /* DERIVE_KEY_MATERIAL */
#define BL3_BLOCK               64
...
/*keyed (key = 32 bytes, flags BL3_KEYED) and derive-key material
   (key = the context hash, flags BL3_DERIVE_MAT) both ride the SAME fields:
   the key words seed every chunk/parent cv, and the flags reach every
   compression through h->flags. bl3_init_flags is the IV-keyed form the
   derive-key CONTEXT pass needs. */
static void bl3_init_key_words(bl3_hasher_t *h, const uint32_t key[8],
                               uint32_t flags)
{
    /* codegraph D22: the older spelling here was
     * `memcpy(h->key, key, 8 * sizeof(uint32_t))`, which the scanner flags
     * because it matches the sizeof token against the operands' ROOT names
     * (h->key / key), not their declared element types -- both ARE
     * uint32_t[8], so the copy was never wrong (keyed/derive-key vectors
     * in tests/test_hash_keyed.js + the python-hashlib differential pin
     * the digests). sizeof h->key says the same thing while tying the size
     * to the destination object, so a future field resize cannot desync. */
    memcpy(h->key, key, sizeof h->key);
    bl3_chunk_init(&h->chunk, h->key, 0, flags);
    h->stack_len = 0;
    h->flags = flags;
}

static void bl3_init_flags(bl3_hasher_t *h, uint32_t flags)
{
    bl3_init_key_words(h, BL3_IV, flags);
}

static void bl3_init_keyed(bl3_hasher_t *h, const uint8_t key[32],
                           uint32_t flags)
{
    uint32_t kw[8];
    int i;
    for (i = 0; i < 8; i++)
        kw[i] = bl3_load32(key + 4 * i);
    bl3_init_key_words(h, kw, flags);
}

/* Both derive-key spellings ({context} and {deriveKey}) land here once the
   string is validated non-empty: adopt it and measure it. Shared so the
   length is computed in exactly one place (codegraph D08). */
static void bl3_ctx_adopt(const char *cs, const uint8_t **ctxb, size_t *ctxn)
{
    *ctxb = (const uint8_t *)cs;
    *ctxn = strlen(cs);
}
...
/*keylen 0 = unkeyed (RFC 7693: key length 0..64, and 0 IS legal --
   it is the parameter block's key_length field, not a flag). Keyed, the key
   is zero-padded into the FIRST 128-byte block, counted by t, and with an
   empty input that key block is the last block. */
static void bl2b_hash_key(const uint8_t *p, size_t n, uint8_t *out,
                          size_t out_len, const uint8_t *key, size_t keylen)
{
    uint64_t h[8];
    uint8_t block[128];
    uint64_t t = 0;
    ...
    h[0] ^= 0x01010000ULL ^ (uint64_t)out_len ^ ((uint64_t)keylen << 8);
    if (keylen) {
...
/*the BLAKE2s twin of bl2b_hash_key (64-byte key block). */
static void bl2s_hash_key(const uint8_t *p, size_t n, uint8_t *out,
                          size_t out_len, const uint8_t *key, size_t keylen)
{
    uint32_t h[8];
    uint8_t block[64];
    ...
    h[0] ^= 0x01010000u ^ (uint32_t)out_len ^ ((uint32_t)keylen << 8);
    if (keylen) {
        memset(block, 0, sizeof block);
        memcpy(block, key, keylen);
        t = 64;
                         ...
                         int argc, JSValueConst *argv, int magic)
{
    ...
    uint8_t *out;
    JSValue result;
    /*the second argument may be an options object instead of a
       length: {key} (MAC), and for BLAKE3 {context} or {deriveKey} (the
       derive-key mode; the output is then exactly 32 bytes). */
    const uint8_t *key = NULL, *ctxb = NULL;
    const char *key_own = NULL, *ctx_own = NULL;
    /* WHICH derive option supplied the context ("context" or "deriveKey"),
       captured at intake: the mode-conflict message must name the option the
       caller actually passed, and it outlives ctx_own (the owned buffer is
       released before the error is thrown -- reading the pointer back there
       would be a use-after-free). */
    const char *ctx_opt = NULL;
    size_t keylen = 0, ctxn = 0;
    int derive = 0;
    ...
    if (argc > 2)
        return JS_ThrowTypeError(ctx,
            "%s(data, length|opts): at most two arguments", K[kind].name);
    if (argc > 1 && JS_IsObject(argv[1])) {
        JSValue v;
        if (kind == 3)
            return JS_ThrowTypeError(ctx,
                "Murmur3_128(data, seed): the second argument is the seed, a "
                "number -- it takes no options object");
        if (kind == 0) {
            static const char *const b3_keys[] =
                { "key", "length", "context", "deriveKey" };
            if (dyn_opts_strict(ctx, argv[1], b3_keys, 4))
                return JS_EXCEPTION;
        } else {
            static const char *const bl2_keys[] = { "key", "length" };
            if (dyn_opts_strict(ctx, argv[1], bl2_keys, 2))
                return JS_EXCEPTION;
        }
        /* length first: the key check messages can state the settled form */
        v = JS_GetPropertyStr(ctx, argv[1], "length");
        if (JS_IsException(v))
            return JS_EXCEPTION;
        if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
            int64_t lv;
            if (JS_ToInt64(ctx, &lv, v)) { JS_FreeValue(ctx, v); return JS_EXCEPTION; }
            if (lv < 1 || (size_t)lv > K[kind].max) {
                JS_FreeValue(ctx, v);
                return JS_ThrowRangeError(ctx, "%s(data, length): length is 1 "
                                          "to %zu bytes", K[kind].name, K[kind].max);
            }
            out_len = (size_t)lv;
        }
        JS_FreeValue(ctx, v);
        if (kind == 0) {
            int have_ctx = 0;
            v = JS_GetPropertyStr(ctx, argv[1], "key");
            if (JS_IsException(v))
                return JS_EXCEPTION;
            if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
                if (dyn_crypto_data(ctx, v, &key, &keylen, &key_own)) {
                    JS_FreeValue(ctx, v);
                    return JS_EXCEPTION;
                }
                if (keylen != 32) {
                    JS_FreeValue(ctx, v);
                    if (key_own) JS_FreeCString(ctx, key_own);
                    return JS_ThrowRangeError(ctx,
                        "BLAKE3: key must be exactly 32 bytes (got %zu) -- "
                        "the keyed mode keys with the full 32, unlike "
                        "BLAKE2's variable-length key", keylen);
                }
            }
            JS_FreeValue(ctx, v);
            v = JS_GetPropertyStr(ctx, argv[1], "context");
            if (JS_IsException(v)) {
                if (key_own) JS_FreeCString(ctx, key_own);
                return JS_EXCEPTION;
            }
            if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
                ...
                if (!*cs) {
                    ...
                    if (key_own) JS_FreeCString(ctx, key_own);
                    return JS_ThrowRangeError(ctx,
                        "BLAKE3: context must be a non-empty string -- it is "
                        "domain separation, and an empty one separates "
                        "nothing");
                }
                ctx_own = cs;
                ctx_opt = "context";
                bl3_ctx_adopt(cs, &ctxb, &ctxn);
                have_ctx = 1;
            } else {
                JS_FreeValue(ctx, v);
            }
            v = JS_GetPropertyStr(ctx, argv[1], "deriveKey");
            if (JS_IsException(v)) {
                if (key_own) JS_FreeCString(ctx, key_own);
                if (ctx_own) JS_FreeCString(ctx, ctx_own);
                return JS_EXCEPTION;
            }
            if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
                const char *cs;
                if (have_ctx) {
                    ...
                    if (ctx_own) JS_FreeCString(ctx, ctx_own);
                    return JS_ThrowTypeError(ctx,
                        "BLAKE3: use either context or deriveKey -- they are "
                        "the same option, spelled twice");
                }
            ...
            if (have_ctx) {
                if (key) {
                    /* name the option the CALLER passed (captured at intake,
                       before the owned buffers are released -- the old code
                       read ctx_own back after JS_FreeCString and guessed the
                       name from the freed pointer) */
                    const char *opt = ctx_opt ? ctx_opt : "context";
                    if (key_own) JS_FreeCString(ctx, key_own);
                    if (ctx_own) JS_FreeCString(ctx, ctx_own);
                    return JS_ThrowTypeError(ctx,
                        "BLAKE3: {key} and {%s} are different modes -- keyed "
                        "MAC, or derive-key; a call is one or the other", opt);
                }
                derive = 1;
                out_len = 32;    /* a derived key IS 32 bytes, per the spec */
            }
        } else {
            v = JS_GetPropertyStr(ctx, argv[1], "key");
            if (JS_IsException(v))
            ...
            if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
                ...
                /* RFC 7693: the key may be 0..64 (b) / 0..32 (s) bytes, and
                   key length 0 IS legal -- it means unkeyed, exactly the
                   same parameter block as no key at all. */
                if (keylen > K[kind].max) {
                    JS_FreeValue(ctx, v);
                    if (key_own) JS_FreeCString(ctx, key_own);
                    return JS_ThrowRangeError(ctx,
                        "%s: key must be 0 to %zu bytes (got %zu) -- 0 means "
                        "unkeyed", K[kind].name, K[kind].max, keylen);
                }
            }
            JS_FreeValue(ctx, v);
        }
        if (derive && argc > 0 && JS_IsObject(argv[1])) {
            /* length was forced; refuse an explicit conflicting one */
            v = JS_GetPropertyStr(ctx, argv[1], "length");
            if (JS_IsException(v)) {
                if (key_own) JS_FreeCString(ctx, key_own);
                if (ctx_own) JS_FreeCString(ctx, ctx_own);
            ...
            if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
                ...
                if (ctx_own) JS_FreeCString(ctx, ctx_own);
                return JS_ThrowTypeError(ctx,
                    "BLAKE3: derive-key output is exactly 32 bytes -- it "
                    "keys the next primitive, and takes no length");
            }
            JS_FreeValue(ctx, v);
        }
    ...
    case 0: {
        bl3_hasher_t h;
        if (derive) {
            /* The official derive_key: the context string is hashed under
               the IV key with DERIVE_CTX flags; that 32-byte ROOT hash then
               keys the material hash, whose flags are DERIVE_MAT (and not
               KEYED -- the reference's init_keyed_internal passes the key
               words with the material flag alone). Pinned against the
               reference implementation's vectors. */
            uint8_t ck[32];
            bl3_init_flags(&h, BL3_DERIVE_CTX);
            bl3_update(&h, ctxb, ctxn);
            bl3_final(&h, ck, sizeof ck);
            bl3_init_keyed(&h, ck, BL3_DERIVE_MAT);
            bl3_update(&h, data, n);
            bl3_final(&h, out, out_len);
        } else if (key) {
            bl3_init_keyed(&h, key, BL3_KEYED);
            bl3_update(&h, data, n);
            bl3_final(&h, out, out_len);
        } else {
            bl3_init(&h);
    ...
    case 1: bl2b_hash_key(data, n, out, out_len, key, keylen); break;
    case 2: bl2s_hash_key(data, n, out, out_len, key, keylen); break;
