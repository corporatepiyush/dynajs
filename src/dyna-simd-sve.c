...
/* ARM64 SVE SIMD overrides — scalable vectors (128b–2048b), ACLE via
 * arm_sve.h, vector length from svcntw() at runtime.
 *
 * Kernel shape: FULL-VECTOR bodies under an all-true predicate plus a
 * scalar tail loop (and a whole-kernel scalar path when n < cnt) — NOT
 * per-element predication. SVE is compile-gated (see dyna-simd-core.c):
 * a default arm64 build never links this file, and NO runtime
 * verification exists yet (no SVE hardware in CI) — treat every kernel
 * as compile-checked only until an SVE host runs the differential. */
...
static float simd_sve_dot_f(const float *restrict a,
                            const float *restrict b, size_t n) {
  /* Widen each HALF vector of f32 products to f64 (svcvt_f64_f32 reads the
   * LOW lanes), so the accumulation is exact-per-element. The previous body
   * used nonexistent intrinsics and squared the products. */
  svbool_t pg64 = svptrue_b64();
  /* HALF-width f32 predicate: svcvt_f64_f32 converts the LOW dcnt lanes, and
   * an all-true load at &a[i + dcnt] would read 2*dcnt elements -- dcnt past
   * the chunk end, a heap over-read when the chunk ends the array. */
  svbool_t pgh = svwhilelt_b32((size_t)0, (size_t)svcntd());
  svfloat64_t acc = svdup_f64(0.0);
  int dcnt = svcntd(); /* f64 lanes = half the f32 lanes at the same VL */
  size_t i = 0;
...
static void simd_sve_gelu(float *restrict out,
                                 const float *restrict in, size_t n) {
  /* Per-lane tanhf through memory: the previous Schraudolph fast-tanh body
   * lived in a different approximation domain than the libm tail (audit
   * E13-07's parity rule: lanes match the tail bit-for-bit). */
  svbool_t pg = svptrue_b32();
  size_t i = 0;
  int cnt = sve_f32_cnt();
  for (; i + (size_t)cnt <= n; i += cnt) {
    float tmp[64]; /* svcntw() <= 64 at the 2048-bit architectural maximum */
...
static void simd_sve_elu(float *restrict out,
  ...
  for (; i + (size_t)cnt <= n; i += cnt) {
    svfloat32_t vi = svld1_f32(pg, &in[i]);
    svbool_t mask = svcmpgt_f32(pg, vi, vzero);
    /* Per-lane libm through memory (SVE has no per-lane get/set): the
     * lanes then match the scalar tail bit-for-bit. */
    float tmp[64]; /* svcntw() <= 64 at the 2048-bit maximum */
    svst1_f32(pg, tmp, vi);
    for (int j = 0; j < cnt; j++)
...
static void simd_sve_softmax(float *restrict out,
  ...
  svbool_t pg = svptrue_b32();
  int cnt = sve_f32_cnt();
  /* Softmax is routinely called with tiny n (class counts): the old body
   * seeded an all-true load with no n == 0 or n < cnt guard, reading past
   * the buffer AND folding garbage into the max and the sum. */
  if (n == 0)
    return;
  if (n < (size_t)cnt) {
    float maxv = in[0];
...
static void simd_sve_log_softmax(float *restrict out,
  ...
  svbool_t pg = svptrue_b32();
  int cnt = sve_f32_cnt();
  /* Same tiny-n rule as softmax: the seeds were unguarded. */
  if (n == 0)
    return;
  if (n < (size_t)cnt) {
    float maxv = in[0];
  ...
  /* Sum the exps IN REGISTERS without storing them: the previous body
   * summed only the scalar tail and then wrote exps to out[] before a final
   * pass that re-read in[] -- for the documented in-place binding
   * log_softmax(a, a, n) that read clobbered data (the "~550% error" class
   * avx2/avx512 fixed). With no store, the final pass may read in[] freely:
   * nothing has overwritten it yet, aliased or not. */
  svfloat32_t vmaxv = svdup_f32(maxv);
  svfloat32_t vsum = svdup_f32(0.0f);
  i = 0;
  for (; i + (size_t)cnt <= n; i += cnt) {
    svfloat32_t vi = svsub_f32_z(pg, svld1_f32(pg, &in[i]), vmaxv);
    /* Schraudolph is (int)(12102203*x + 0x3F800000): the magic is POSITIVE
       and the bias is not optional. */
    svint32_t bits = svcvt_s32_f32_z(pg,
        svmla_f32_z(pg, svdup_f32(1065353216.0f), vi, svdup_f32(12102203.0f)));
    bits = svmax_s32_z(pg, bits, svdup_s32(0));
    bits = svmin_s32_z(pg, bits, svdup_s32(0x7F800000));
...
static void simd_sve_vlog(float *restrict out,
  ...
  int cnt = sve_f32_cnt();
  for (; i + (size_t)cnt <= n; i += cnt) {
    /* Per-lane libm through memory (no per-lane get/set in SVE); lanes then
     * match the scalar tail bit-for-bit. The previous svset_f32 form also
     * DISCARDED its result, so every lane stayed -FLT_MAX. */
    float tmp[64]; /* svcntw() <= 64 at the 2048-bit maximum */
    svst1_f32(pg, tmp, svld1_f32(pg, &in[i]));
    for (int j = 0; j < cnt; j++)
...
static float simd_sve_dist_cheb(const float *restrict a,
  ...
  for (; i + (size_t)cnt <= d; i += cnt) {
    svfloat32_t adiff =
        svabs_f32_x(pg, svsub_f32_z(pg, svld1_f32(pg, &a[i]), svld1_f32(pg, &b[i])));
...
static void simd_sve_gemm(float *restrict c,
  ...
  svbool_t pg = svptrue_b32();
  int cnt = sve_f32_cnt();
  /* BLAS beta=0 semantics: prior c never read (0 * NaN = NaN) — zero instead. */
  if (beta == 0.0f) {
    for (size_t i = 0; i < m; i++)
      for (size_t j = 0; j < n; j++)
        c[i * n + j] = 0.0f;
...
static float simd_sve_hamming(const uint32_t *restrict a,
  ...
  for (; i + (size_t)cnt <= n_words; i += cnt) {
    ...
    /* Base SVE counts set bits per BYTE only (32-bit CNT is SVE2): popcount
     * the bytes and fold lo+hi halves up through u16 to u32 -- the lane
     * grouping changes but the horizontal sum stays exact. */
...
/* ── Override table ─────────────────────────────────────────────── */
/* GATED OFF by default (DYNAJS_SIMD_F64_SVE undefined): NO SVE hardware
 * exists anywhere in CI or on the dev hosts, so these kernels are
 * compile-clean but NOT runtime-proven under the differential harness.
 * Per the same "do not ship unproven SIMD" rule the AVX-512 file applies,
 * they stay disabled; an SVE host runs the PROVEN NEON f64 kernels until
 * someone defines DYNAJS_SIMD_F64_SVE via tests/test_simd_f64.c on real
 * silicon. */
...
#endif /* DYNAJS_SIMD_F64_SVE */
#ifdef DYNAJS_SIMD_INT_SVE
/* i32 exactness class: opt-in on real SVE silicon (tests/test_simd_int.c). */

#endif /* DYNAJS_SIMD_INT_SVE */

/* i32 exactness class: same opt-in rule as f64 above — DYNAJS_SIMD_INT_SVE
 * on real SVE silicon (tests/test_simd_int.c). */
#ifdef DYNAJS_SIMD_INT_SVE
...
void simd_override_sve(simd_t *t) {
  ...
  /* f64/i32 ride the same provenness rule as the AVX-512 file: NOTHING in
   * this file has executed (no SVE hardware anywhere in CI), so the exactness
   * classes (f64 reductions, i32 wrap semantics) stay behind opt-in macros
   * until a real SVE host runs the differential harnesses
   * (tests/test_simd_f64.c / test_simd_int.c, both naming their enabling
   * -D). The kernels above (f32) install unconditionally like every other
   * backend's, with the same caveat this file's header states. */
