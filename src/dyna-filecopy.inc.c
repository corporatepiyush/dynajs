...
static int dyn_copy_fds(int in, int out)
{
    ...
    if (lseek(in, 0, SEEK_SET) < 0 || lseek(out, 0, SEEK_SET) < 0 ||
        ftruncate(out, 0) < 0 || lseek(out, 0, SEEK_SET) < 0)
        return -1;
#elif defined(__linux__)
    {
        struct stat st;
                                ...
                                int argc, JSValueConst *argv)
{
    ...
    /* NO O_TRUNC in the overwrite arm: with from == to, truncating at open
       would empty the shared inode before the copy read a byte -- destroying
       the file while reporting the stale source size. Open, detect the
       same-file case, and truncate only once the destination is known to be a
       different inode, so overwrite-onto-self is refused with the data
       intact. (Reported size still comes from the pre-open source fstat, as
       before.) The created file gets the source's permission bits minus the
       umask -- a copy of a private key that lands world-readable is the
...
/* copyFileAsync(from, to[, {overwrite?}]) -> Promise<number: bytes copied.
   The sync copy above, OFF the loop: the same strict opens, the same
   refusals, the same created-file mode, the same dyn_copy_fds kernel path --
   file_job_work runs that sequence verbatim on the pool, and the promise
   rejects with the very error the sync path would have thrown. The sync call
   stays as-is; this is a second door into one copy implementation, not a
   second implementation. */
static JSValue dyn_fs_copy_file_async(JSContext *ctx, JSValueConst this_val,
                                      int argc, JSValueConst *argv)
{
    const char *from, *to;
    file_job_t *j;
    struct stat st;
    int overwrite = 0, offload;
    (void)this_val;

    if (argc < 2)
        return JS_ThrowTypeError(ctx, "copyFileAsync(from, to): both are "
                                      "required");
    /* Argument validation in the sync call's ORDER: options first, then the
     * paths, and every coercion before anything is allocated -- a getter here
     * runs arbitrary JS that can close handles this call depends on. */
    if (argc > 2 && JS_IsObject(argv[2])) {
        static const char *const keys[] = { "overwrite" };
        JSValue v;
        int b;
    ...
    if (j) {
        j->op = FILE_OP_COPY;
        j->path = strdup(from);       /* the borrow ends at submit; the worker
                                         needs paths that outlive this turn */
        j->path2 = strdup(to);
        j->overwrite = overwrite;
    }
    /* stat IS the strategy predicate, as for reads: a source of DYN_FILE_
     * ASYNC_COPY_MIN or more offloads, because that is where the sync copy's
     * loop-block budget starts to matter, and a NON-REGULAR source offloads
     * whatever its size -- open(2) on a writer-less FIFO never returns, and
     * the inline arm opens on the LOOP thread. A stat failure stays inline:
     * the worker's strict open then fails at once, and ENOENT cannot block. */
    offload = (stat(from, &st) == 0 &&
               (!S_ISREG(st.st_mode) ||
                st.st_size >= (off_t)DYN_FILE_ASYNC_COPY_MIN));
    dyn_path_unborrow(ctx, from);
    dyn_path_unborrow(ctx, to);
    if (!j || !j->path || !j->path2) {
                           ...
                           JSValueConst *argv)
{
    ...
    out = dyn_open_strict(to, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, st.st_mode & 0777);
