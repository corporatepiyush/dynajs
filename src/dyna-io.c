...
int dyn_io_slurp_fd(int fd, dyn_iobuf_t *out, int flags, size_t maxsize)
{
    ...
    dyn_iobuf_init(out);
    {
        ...
        if (fstat_rc < 0 || !S_ISREG(st.st_mode)) {
            /* A successful open leaves stale errno behind, so the
             * not-regular case must set its OWN errno: EISDIR for a
             * directory (the common caller mistake), EINVAL for any other
             * non-regular object. */
