 ...
 *    port-ignore (8252, 9700 ). No WHATWG normalisation.
...
/* generateCodeVerifier: 32 octets CSPRNG -> base64url 43 chars (7636 ) */
...
static JSValue oauth_format_scope(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    if (JS_ToUint32(ctx, &n, lenv)) { JS_FreeValue(ctx, lenv); return JS_EXCEPTION; }
    JS_FreeValue(ctx, lenv);
    if (n == 0) return JS_NewString(ctx, "");
    /* ONE fetch per element: the old two-pass code read every element twice,
       and a getter could return a valid token to the validation pass and an
       arbitrary token to the build pass (validation bypass). Validate and
       append from a single read, same per-element error order as before
       (IsString -> length 1..64 -> NQCHAR charset). */
    out = (char *)js_malloc(ctx, (size_t)n * 65 + 1);   /* worst: 64 + 1 sep per element */
    if (!out) return JS_ThrowOutOfMemory(ctx);
    {
        size_t off = 0;
        for (i = 0; i < n; i++) {
            ...
            if (!JS_IsString(v)) { JS_FreeValue(ctx, v); js_free(ctx, out); return JS_ThrowTypeError(ctx, "scope element must be string"); }
            s = JS_ToCStringLen(ctx, &sl, v); JS_FreeValue(ctx, v);
            if (!s) { js_free(ctx, out); return JS_EXCEPTION; }
            if (sl == 0 || sl > 64) { JS_FreeCString(ctx, s); js_free(ctx, out); return JS_ThrowRangeError(ctx, "scope token length 1..64"); }
            { size_t k; for (k = 0; k < sl; k++) if (!is_nqchar((unsigned char)s[k])) { JS_FreeCString(ctx, s); js_free(ctx, out); return JS_ThrowTypeError(ctx, "scope token invalid char"); } }
            if (off) out[off++] = ' ';
...
static JSValue oauth_parse_bearer_from_req(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    /* HTTP field names are case-insensitive (RFC 9110 ): scan the own
       enumerable string keys for "authorization" in any spelling, not just
       the two exact forms "authorization"/"Authorization". */
    v = JS_GetPropertyStr(ctx, argv[0], "headers");
    if (JS_IsObject(v)) {
        JSPropertyEnum *htab = NULL; uint32_t hlen = 0, hk_i;
        JSValue hv = JS_UNDEFINED;
        if (JS_GetOwnPropertyNames(ctx, &htab, &hlen, v, JS_GPN_STRING_MASK|JS_GPN_ENUM_ONLY) >= 0) {
            for (hk_i = 0; hk_i < hlen; hk_i++) {
                const char *hk = JS_AtomToCString(ctx, htab[hk_i].atom);
                int is_auth = 0;
                if (hk) {
                    static const char want[] = "authorization";
                    size_t k;
                    is_auth = 1;
                    for (k = 0; k < sizeof want - 1; k++) {
                        unsigned char c = (unsigned char)hk[k];
                        if (c == '\0' || (c | 0x20) != (unsigned char)want[k]) { is_auth = 0; break; }
                    }
                }
                if (is_auth) {
                    JSValue cand = JS_GetProperty(ctx, v, htab[hk_i].atom);
                    if (JS_IsException(cand)) {
                        /* a throwing getter must stop the scan with the
                           pending exception, not be mistaken for "no header" */
                        if (hk) JS_FreeCString(ctx, hk);
                        JS_FreeValue(ctx, hv);
                        JS_FreePropertyEnum(ctx, htab, hlen);
                        JS_FreeValue(ctx, v);   /* the headers object reference */
                        return JS_EXCEPTION;
                    }
                    if (JS_IsString(cand)) { JS_FreeValue(ctx, hv); hv = cand; }
                    else JS_FreeValue(ctx, cand);
                }
                if (hk) JS_FreeCString(ctx, hk);
            }
            if (JS_IsString(hv)) {
...
static JSValue oauth_build_www(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv){
    ...
    /* RFC 6750: error is REQUIRED in a WWW-Authenticate challenge. A
...
/* verbatim exact except loopback port-ignore (8252 ). Also check private-use scheme contains '.' */
 ...
 * port. The port is IGNORED only when it is empty or digits-only (8252 );
...
static JSValue oauth_is_valid_redirect(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv){
    ...
    /* private-use scheme check: if scheme is not http/https, require '.' in scheme (8252 ) */
    {
    ...
    lenv=JS_GetPropertyStr(ctx, argv[1],"length"); if(JS_ToUint32(ctx,&n,lenv)){ JS_FreeValue(ctx,lenv); return JS_EXCEPTION; } JS_FreeValue(ctx,lenv);
    for(i=0;i<n;i++){
...
static JSValue oauth_build_auth_url(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv){
    ...
    v=JS_GetPropertyStr(ctx, opts,"state"); if(JS_IsString(v)){ state=JS_ToCStringLen(ctx,&scn,v);
        /* an empty state is no state: it is the CSRF token (6749 ) and
           the auto-generate path refuses to build a state-less request. */
        if(state && scn==0){ JS_FreeCString(ctx,state); state=NULL; JS_FreeValue(ctx,v); r=JS_ThrowTypeError(ctx,"buildAuthorizationUrl: state must not be empty"); goto cleanup; } } else if(JS_IsUndefined(v) || JS_IsNull(v)){
    ...
    /* challenge shape per method (7636 ): S256 is exactly 43 unreserved
    ...
    memcpy(url, endpoint, en); url[en]='\0'; url_len=en; url_cap=en+1;
    {
        ...
        /* The KEY is form-encoded too: extraParams keys are caller-controlled
           and a raw "a&admin=true" key forged extra parameters into the
           authorization URL (parameter smuggling). The fixed keys above are
           all-unreserved, so encoding them is a byte-identical no-op. */
        #define APPEND_PARAM(k,vn,v) do{ \
            char *kenc, *venc; \
            kenc = form_encode_component(ctx, k, strlen(k), &tmpl); \
            if(!kenc) goto oom; \
            tmp = form_encode_component(ctx, v, vn, &tmpl); \
            if(!tmp){ js_free(ctx,kenc); goto oom; } \
            venc = tmp; \
            size_t need = url_len + 1 + strlen(kenc) + 1 + tmpl + 1; \
            if(need > url_cap){ char*nb=(char*)js_realloc(ctx,url,need+256); if(!nb){ js_free(ctx,kenc); js_free(ctx,venc); goto oom; } url=nb; url_cap=need+256; } \
            url[url_len++] = has_q ? '&' : '?'; has_q=1; \
            memcpy(url+url_len, kenc, strlen(kenc)); url_len+=strlen(kenc); url[url_len++]='='; memcpy(url+url_len, venc, tmpl); url_len+=tmpl; url[url_len]='\0'; js_free(ctx,kenc); js_free(ctx,venc);}while(0)
        ...
        if(JS_IsObject(v) && !JS_IsArray(ctx, v)){
            ...
            if(JS_GetOwnPropertyNames(ctx,&tab,&len,v,JS_GPN_STRING_MASK|JS_GPN_ENUM_ONLY)>=0){
                for(uint32_t i=0;i<len;i++){
                    ...
                    if(k && !JS_IsString(ev)){ r=JS_ThrowTypeError(ctx,"buildAuthorizationUrl: extraParams values must be strings"); JS_FreePropertyEnum(ctx,tab,len); JS_FreeCString(ctx,k); JS_FreeValue(ctx,ev); JS_FreeValue(ctx,v); goto cleanup; }
                    if(k && JS_IsString(ev)){
                        es=JS_ToCStringLen(ctx,&esn,ev);
                        if(!es){ r=JS_EXCEPTION; JS_FreePropertyEnum(ctx,tab,len); JS_FreeCString(ctx,k); JS_FreeValue(ctx,ev); JS_FreeValue(ctx,v); goto cleanup; }
                        /* a CTL in a key or value is a smuggling attempt: REFUSE,
                           never silently drop (a dropped caller param silently
                           builds a request the caller did not ask for) */
                        if(has_ctl_or_crlf(k,strlen(k))||has_ctl_or_crlf(es,esn)){ r=JS_ThrowTypeError(ctx,"buildAuthorizationUrl: extraParams key/value must not contain CTL/CRLF"); JS_FreePropertyEnum(ctx,tab,len); JS_FreeCString(ctx,k); JS_FreeCString(ctx,es); JS_FreeValue(ctx,ev); JS_FreeValue(ctx,v); goto cleanup; }
 ...
 * response (tokens in the fragment, OAuth 2.1 ), code+error together
 * (6749 ), and duplicate code/state/error params (request forgery
...
static JSValue oauth_parse_auth_resp(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv){
 ...
 ); a code-flow client must refuse them, not silently
    ...
    JS_FreeCString(ctx,url);
    /* 6749: an error response MUST NOT also carry a code */
...
/* parseTokenResponse(body, headers?): JSON; RFC 6749 REQUIRED fields
 * access_token + token_type (Bearer case-insensitive, per 6750 the
...
static JSValue oauth_parse_token_resp(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv){
 ...
 validation, returning it as-is skips that entirely. */
    ...
    /* access_token REQUIRED, must be a b64token (6750 ) */
    {
    ...
    /* optional fields must have their documented types (6749 ) */
    {
...
static JSValue oauth_verify_jwt(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv){
    ...
    if(!JS_IsArray(ctx, v)){ JS_FreeValue(ctx, v); return JS_ThrowTypeError(ctx, "verifyJWT: algorithms must be array"); }
    {
        ...
        uint32_t n=0; if(JS_ToUint32(ctx,&n,lenv)){ JS_FreeValue(ctx,lenv); JS_FreeValue(ctx,payload); return JS_EXCEPTION; }
        JS_FreeValue(ctx,lenv);
    ...
    if(JS_IsNumber(v)){
        double expd; if (JS_ToFloat64(ctx, &expd, v)) { JS_FreeValue(ctx,v); JS_FreeValue(ctx,payload); return JS_EXCEPTION; }
    ...
    if(JS_IsNumber(v)){
        double nbfd; if (JS_ToFloat64(ctx, &nbfd, v)) { JS_FreeValue(ctx,v); JS_FreeValue(ctx,payload); return JS_EXCEPTION; }
    ...
    if(!JS_IsUndefined(v) && !JS_IsNull(v)){
        ...
        if(JS_IsString(tokAud) && JS_IsString(v)){
            ...
            JSValue lenv = JS_GetPropertyStr(ctx, tokAud, "length"); uint32_t n=0; if(JS_ToUint32(ctx,&n,lenv)){ JS_FreeValue(ctx,lenv); JS_FreeValue(ctx,payload); return JS_EXCEPTION; } JS_FreeValue(ctx,lenv);
            ...
            JSValue lenv = JS_GetPropertyStr(ctx, v, "length"); uint32_t n=0; if(JS_ToUint32(ctx,&n,lenv)){ JS_FreeValue(ctx,lenv); JS_FreeValue(ctx,payload); return JS_EXCEPTION; } JS_FreeValue(ctx,lenv);
            ...
            JSValue lenv1 = JS_GetPropertyStr(ctx, tokAud, "length"); uint32_t n1=0; if(JS_ToUint32(ctx,&n1,lenv1)){ JS_FreeValue(ctx,lenv1); JS_FreeValue(ctx,payload); return JS_EXCEPTION; } JS_FreeValue(ctx,lenv1);
            JSValue lenv2 = JS_GetPropertyStr(ctx, v, "length"); uint32_t n2=0; if(JS_ToUint32(ctx,&n2,lenv2)){ JS_FreeValue(ctx,lenv2); JS_FreeValue(ctx,payload); return JS_EXCEPTION; } JS_FreeValue(ctx,lenv2);
