/* dyna:yaml -- the YAML 1.2 core schema, block and flow, and NOTHING else.
   ...
   document. Full grammar and the refusal list: docs/dynajs-guide/API.md. */
#include "dyna-nat.h"
...
/* A native module does not reach for the engine's DynBuf; same reasoning and
   the same decaying growth factor as the other fragments. */
...
static int yml_emit_scalar(yml_w_t *w, JSValueConst v)
{
    ...
    if (!s)
        return -1;
    if (JS_IsNumber(v))
        yb_write(&w->b, s, n);          /* a number needs no quoting, ever */
    else
...
static const JSCFunctionListEntry dyn_yaml_funcs[] = {
    ...
    JS_CFUNC_DEF("Stringify", 2, yml_stringify),
};
