                            ...
                            int argc, JSValueConst *argv, int magic)
{
    ...
    /* Hold the scan until a leading BOM can be decided (needs 3 bytes, or the
       end of the stream): scanning earlier would bake partial BOM bytes into
       the pending text token. */
    ...
    for (;;) {
        ...
        if (skip < 0) { rc = -1; break; }
        if (!skip && dyn_sax_emit(ctx, s, this_val) < 0) { rc = -2; break; }
        xml_compact(&s->scan);
    }
    s->busy = 0;
    if (magic == 1) {
        ...
        if (rc == 0 && s->depth > 0) {
            ...
            rc = -1;
        } else if (rc == 0 && !s->root_closed) {
            snprintf(s->scan.err, sizeof s->scan.err, "no root element");
            rc = -1;
        } else if (rc == 0 && s->scan.pos < s->scan.buf.n) {
