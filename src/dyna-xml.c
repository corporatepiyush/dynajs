...
/* The tree front end enforces a document discipline the scanner does not:
   ...
   legal XML and stay legal; blank text at the top level is skipped the way
   xml_tree_text skips it. Returns 0 or -1 with s->scan.err set. */
    ...
    case XEV_CDATA:
        if (s->depth == 0) {
            if (ev->kind == XEV_TEXT && xml_all_space(ev->text.p, ev->text.n))
                break;              /* blank top-level text is skipped */
            snprintf(s->scan.err, sizeof s->scan.err,
                            ...
                            int argc, JSValueConst *argv, int magic)
{
    ...
    for (;;) {
        ...
        if (r == XS_NEED || r == XS_END)
            break;
        if (r == XS_ERR) { rc = -1; break; }
        if (dyn_sax_root_check(s, &s->ev)) { rc = -1; break; }
