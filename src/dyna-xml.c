...
#include "dyna-nat.h"
#include "dyna-simd-kernels.h"   /* simd.find_first_of for text & escape scans */
...
/* Thin wrapper over core/dyn-sb.h: xb_t, its sticky-oom void-return verbs
   and the seed (64) stay here; the curve and the realloc idiom are the
   core's. */
...
static void xb_reserve(xb_t *b, size_t need)
{
    if (b->oom || need <= b->cap)
        return;
    if (!dyn_sb_reserve((void **)&b->p, &b->cap, need, 64))
        b->oom = 1;
}
...
typedef struct {
    ...
    int      bom_done;                  /* a leading UTF-8 BOM has been decided */
    char     err[128];
    uint32_t line;
} xml_scan_t;

/* A UTF-8 BOM at byte 0 of the stream is a byte-order mark, not content
   (XML 1.0 App. F: a UTF-8 entity MAY begin with one). Decided once, before
   any byte is scanned: a BOM seen MID-stream is ordinary text. On a stream,
   deciding needs 3 bytes, so the caller must not scan before this returns
   "decided" -- otherwise partial BOM bytes would already sit in a pending
   text token. */
static void xml_scan_bom(xml_scan_t *s)
{
    if (s->bom_done || s->pos != 0)
        return;
    if (s->buf.n >= 3) {
        if (memcmp(s->buf.p, "\xEF\xBB\xBF", 3) == 0)
            s->pos = 3;
        s->bom_done = 1;
    } else if (s->final) {
        s->bom_done = 1;                /* too short to hold a BOM */
    }
}
                          ...
                          xb_t *out)
{
    ...
    while (j < n) {
        ...
        /* Batch clean bytes: most attribute values have no entities. */
        {
            size_t run = j;
            while (j < n && p[j] != quote && p[j] != '<' && p[j] != '&') j++;
            if (j > run) {
                /* Fast path when no space normalisation needed. */
                size_t k = run;
                int need_space = 0;
                while (k < j && !xml_space((unsigned char)p[k])) k++;
                if (k == j) {
                    xb_write(out, p + run, j - run);
                } else {
                    for (k = run; k < j; k++)
                        xb_put(out, (uint8_t)(xml_space((unsigned char)p[k]) ? ' ' : p[k]));
                }
                (void)need_space;
...
/* An open tag has no resume state: every XS_NEED restarts it from its first
   byte, and `s->pos` still points at its `<`. The token cap is therefore
   checked where NEED is returned -- there the tag provably runs to the end of
   the carry, so `n - pos` is the tag's own size, never post-token data. */
static int xml_tag_need(xml_scan_t *s, size_t n)
{
    if (n - s->pos > XML_MAX_TOKEN)
        return xml_fail(s, "open tag exceeds the token limit");
    return XS_NEED;
}
                         ...
                         xml_ev_t *ev)
{
    ...
    /* XML well-formedness: unique attribute names per element. Names are
       case-SENSITIVE bytes -- nothing here case-folds, so `X` and `x` are
       distinct. Checked at commit time, against what earlier attributes
       already committed in this tag. */
    {
        uint32_t a;
        for (a = 0; a < ev->nattr; a++)
            if (ev->attr[a].nlen == k - an
                && memcmp(ev->attr[a].n, p + an, k - an) == 0)
                return xml_fail(s, "duplicate attribute name");
    }
    while (k < n && xml_space((unsigned char)p[k])) k++;
    if (k >= n)
        return xml_tag_need(s, n);
                         ...
                         xml_ev_t *ev)
{
    ...
    ev->kind = XEV_TEXT;
    s->tok = 1;
    /* Reserve modestly for large-text case; element-heavy docs have
       many tiny text runs, so total remaining is a bad estimate. */
    if (ev->text.cap == 0 && n - k > 4096) {
        xb_reserve(&ev->text, 8192);
    }
    while (k < n && p[k] != '<') {
        ...
        if (p[k] == ']') {
            if (k + 2 < n) {
                ...
                /* A `]` whose `]]`/`>` could still arrive in the next chunk:
                   hold it back and re-scan it when more input lands, exactly
                   like an interrupted entity. Emitting it now would let a
                   `]]>` assembled across a chunk boundary through as text. */
                s->tscan = k - *i;
                return XS_NEED;
            }
        }
                          ...
                          xml_ev_t *ev)
{
    ...
    if (j >= n) {
        /* The same cap as every other token: an unterminated close tag must
           not be able to grow the carry without bound on a stream that never
           ends (and it has no resume state, so it would also rescan
           quadratically). */
        if (n - *i > XML_MAX_TOKEN)
            return xml_fail(s, "close tag exceeds the token limit");
        return XS_NEED;
    }
    if (p[j] != '>')
                          ...
                          int kind, int tok)
{
    ...
    while (j + 2 < n && !(p[j] == term[0] && p[j + 1] == term[1]
        ...
        j++;
    if (j + 2 >= n) {
        /* No terminator yet: every buffered byte from `st` is this token's
           body. Refuse a body that has already grown past the cap, so a
           streamed multi-GB comment cannot grow the carry without bound. */
        if (n - st > XML_MAX_TOKEN)
            return xml_fail(s, kind == XEV_CDATA ? "CDATA section exceeds the token limit"
                                                 : "comment exceeds the token limit");
        /* At end of input the missing terminator IS the error: name the token
           instead of letting the front ends blame a generic end-of-document
           (or, in SAX, the absent root). */
        if (s->final)
            return xml_fail(s, kind == XEV_CDATA ? "unterminated CDATA section"
                                                 : "unterminated comment");
        s->tscan = j - st;
        return XS_NEED;
    }
    if (j - st > XML_MAX_TOKEN)
        return xml_fail(s, kind == XEV_CDATA ? "CDATA section exceeds the token limit"
                                             : "comment exceeds the token limit");
                         ...
                         xml_ev_t *ev)
{
    ...
    if (n - *i >= 9 && memcmp(p + *i, "<!DOCTYPE", 9) == 0) {
        /* The DOCTYPE is skipped unread, but the SKIP itself must be exact:
           `[`/`]` only matter outside quoted literals, and a comment inside
           the internal subset may legally contain brackets, quotes and `>`
           (XML 1.0 2.8). */
        int depth = 0, q = 0, in_comment = 0;
        for (j = *i + 9; j < n; j++) {
            if (in_comment) {
                if (p[j] == '-' && n - j >= 3
                    && p[j + 1] == '-' && p[j + 2] == '>') {
        ...
        if (j >= n) {
            /* Same cap as comment/CDATA: an unterminated DOCTYPE must not be
               able to grow the carry without bound on a stream that never
               ends. */
            if (n - *i > XML_MAX_TOKEN)
                return xml_fail(s, "DOCTYPE exceeds the token limit");
            if (s->final)
                return xml_fail(s, "unterminated DOCTYPE");
            return XS_NEED;
        }
        *i = j + 1;
                       ...
                       xml_ev_t *ev)
{
    ...
    if (j - st > XML_MAX_TOKEN)
        return xml_fail(s, "processing instruction exceeds the token limit");
    if (j + 1 >= n)
        return XS_NEED;
    ev->kind = XEV_PI;
    ev->name = p + st;
...
typedef struct {
    ...
    int        allow_multi_roots;       /* multiple: true */
} xml_tree_t;
...
typedef struct { int trim, keep_entities, multiple; } xml_opts_t;
 ...
 * instead of being silently ignored -- `{multipe: true}` used to read as
 * single-root mode and the second root became a syntax error reported
 * somewhere else entirely. Symbol keys and non-enumerable properties are
 * not options and stay invisible (JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY).
 * null/undefined and primitive bags keep their historic reading at each
 * call site. Getters are NOT invoked here; values are read afterwards, so
 * a throwing getter on a KNOWN key still propagates. Returns 0 when the
 * bag is clean, -1 with the TypeError pending.
 * SCOPE NOTE: only XMLParse's bag goes through the check here --
 * SAXParser's single argument is a HANDLERS registry that merely reuses
 * xml_read_opts for {entities}; that positional/handlers duality is the
 * documented exception (a handlers object may carry extra metadata),
 * so its key set is deliberately NOT closed. */
static int xml_opts_check(JSContext *ctx, JSValueConst o,
...
static const char *const xml_parse_keys[] = { "multiple", "trim", "entities" };
static const char *const xml_stringify_keys[] = { "indent" };
...
static int xml_read_opts(JSContext *ctx, JSValueConst o, xml_opts_t *out)
{
    ...
    v = JS_GetPropertyStr(ctx, o, "multiple");
    if (JS_IsException(v)) return -1;
    if (!JS_IsUndefined(v)) {
        int b = JS_ToBool(ctx, v);   /* a throwing getter PROPAGATES, and
                                        stops the option walk here */
        JS_FreeValue(ctx, v);
        if (b < 0) return -1;
        out->multiple = b;
    }
                         ...
                         char *err, size_t errsz)
{
    ...
    if (JS_IsException(el))
        return -3;
    if (t->depth == 0 && ++t->nroots > 1 && !t->allow_multi_roots) {
        /* multiple: true lifts the single-root refusal and collects
         * every root into the fragment array instead. */
                         ...
                         int argc, JSValueConst *argv)
{
    ...
    /*unknown keys are refused BEFORE any parsing work. SAXParser's
     * ctor also calls xml_read_opts but deliberately does NOT get this
     * check (handlers/opts duality -- see xml_opts_check). */
    if (xml_opts_check(ctx, argc > 1 ? argv[1] : JS_UNDEFINED,
                       xml_parse_keys, countof(xml_parse_keys)) < 0)
        return JS_EXCEPTION;
    ...
    if (slen > 8192) {
        xb_reserve(&p.ev.text, 8192);
        xb_reserve(&p.ev.abuf, 4096);
    }
    p.scan.final = 1;
    p.scan.keep_entities = opts.keep_entities;
    xb_reserve(&p.scan.buf, slen);
    ...
    xml_scan_bom(&p.scan);              /* a leading BOM is not content */
    memset(&t, 0, sizeof t);
    t.ctx = ctx;
    t.trim = opts.trim;
    t.allow_multi_roots = opts.multiple;
    ...
    if (rc == 0) {
        if (opts.multiple) {
            /*every root element, as an array (parity with HTMLParse's
             * fragment array). The document-level strictness is unchanged:
             * malformed input still throws, and a document with no element
             * at all is still an error -- only the return SHAPE differs. */
            result = root_children;
            root_children = JS_UNDEFINED;   /* ownership moved to result */
        } else
...
/* Bytes that must be escaped in text and in an attribute value. ONE table per
   context, derived from the same idea, so the writer and the reader cannot
   disagree about what needs quoting. The attribute table adds \t \n \r: a
   literal tab/newline/CR is normalized to space on re-parse (XML 1.0 3.3.3),
   so the writer must emit the character reference or a round trip changes the
   value. TEXT keeps literal whitespace and uses the 4-entry table. */
static const uint8_t XML_ESC[256] = {
    ['<']=1, ['>']=1, ['&']=1, ['"']=1,
};
static const uint8_t XML_ESC_ATTR[256] = {
    ['<']=1, ['>']=1, ['&']=1, ['"']=1,
    ['\t']=1, ['\n']=1, ['\r']=1,
};
static const uint8_t XML_ESC_SET[4] = { '<', '>', '&', '"' };
static const uint8_t XML_ESC_ATTR_SET[7] = { '<', '>', '&', '"', '\t', '\n', '\r' };
...
{
    const uint8_t *tbl = in_attr ? XML_ESC_ATTR : XML_ESC;
    const uint8_t *set = in_attr ? XML_ESC_ATTR_SET : XML_ESC_SET;
    size_t setn = in_attr ? sizeof XML_ESC_ATTR_SET : sizeof XML_ESC_SET;
    size_t i = 0, run;
    while (i < n) {
        run = i;
        ...
        { size_t probe = i + XML_SIMD_MIN;
          if (probe > n) probe = n;
          while (i < probe && !tbl[(unsigned char)s[i]]) i++;
        ...
        case '>': xb_puts(b, "&gt;"); break;
        case '&': xb_puts(b, "&amp;"); break;
        case '\t': xb_puts(b, "&#9;"); break;    /* attribute only */
        case '\n': xb_puts(b, "&#10;"); break;   /* attribute only */
        case '\r': xb_puts(b, "&#13;"); break;   /* attribute only */
...
static int xml_ser_attrs(xml_ser_t *w, JSValueConst el)
{
    ...
    for (k = 0; k < len && rc == 0; k++) {
        ...
        /* hand-built trees are an injection vector: a name the tokenizer
           could never produce (e.g. `x" onload=`) must be refused, not
           emitted raw into the tag */
        if (anl == 0 || !xml_name_start((unsigned char)an[0])) {
            JS_ThrowTypeError(w->ctx,
                "XMLStringify: invalid attribute name");
        ...
        }
        {
            size_t ci;
            for (ci = 1; ci < anl; ci++)
                if (!xml_name_char((unsigned char)an[ci])) {
                    JS_ThrowTypeError(w->ctx,
                        "XMLStringify: invalid attribute name");
                    JS_FreeCString(w->ctx, an);
                    JS_FreeCString(w->ctx, av);
...
static int xml_ser_node(xml_ser_t *w, JSValueConst node)
{
    ...
    }
    {
        size_t ci;
        for (ci = 1; ci < nml; ci++)
            if (!xml_name_char((unsigned char)nm[ci])) {
                JS_FreeCString(w->ctx, nm);
                JS_ThrowRangeError(w->ctx, "XMLStringify: not a valid element name");
                return -1;
            }
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    if (argc > 1 && JS_IsObject(argv[1])) {
        ...
        if (xml_opts_check(ctx, argv[1], xml_stringify_keys,
                           countof(xml_stringify_keys)) < 0)
            return JS_EXCEPTION;
...
/*streaming writer (open(name, attrs?)/text(s)/close) for documents
 * too big for the string builder. The writer holds an incremental byte
 * buffer plus a stack of open element names -- no tree is ever built, so a
 * deep or long document costs buffer + stack, not nodes. open() writes
 * `<name attrs>`, text() escapes and appends, close() writes `</name>` for
 * the innermost open element. toString() returns the bytes so far without
 * closing anything; finish() closes every remaining element and returns the
 * complete document. Depth is NOT the serializer's XML_MAX_DEPTH: the point
 * of the writer is documents the 256-deep builder refuses. The name stack
 * is heap memory (no recursion anywhere), so the cap is a runaway guard
 * only. */
#define XML_WRITER_MAX_DEPTH 65536
...
static JSClassID dyn_xw_class_id;

static void xml_writer_free(xml_writer_t *w)
{
    size_t i;
    if (!w)
        return;
...
static void xml_writer_dispose(void *p) { xml_writer_free((xml_writer_t *)p); }

static void xml_writer_finalizer(JSRuntime *rt, JSValue val)
{
    (void)rt;
    xml_writer_free((xml_writer_t *)JS_GetOpaque(val, dyn_xw_class_id));
}

static const JSClassDef dyn_xw_class = {
    "XmlWriter", .finalizer = xml_writer_finalizer,
};

static const char *const xml_writer_keys[] = { "indent" };

static int xml_writer_valid_name(const char *s, size_t n)
{
    size_t i;
    if (n == 0 || !xml_name_start((unsigned char)s[0]))
        return 0;
    for (i = 1; i < n; i++)
...
static void xml_writer_nl_x(xb_t *out, int indent, size_t depth)
{
    int k;
    if (!indent)
        return;
    xb_put(out, '\n');
    for (k = 0; k < (int)depth * indent; k++)
        xb_put(out, ' ');
}

static void xml_writer_nl(xml_writer_t *w)
{
    xml_writer_nl_x(&w->out, w->indent, w->n);
}

/* XML 1.0: C0 controls other than tab/LF/CR are not legal characters
 * at all -- they cannot even be escaped as references. Refuse them in text
 * and attribute values instead of emitting a document that is malformed by
 * construction and that strict consumers reject outright. (The check is on
 * the UTF-8 bytes: every C0 byte is a one-byte encoding and no continuation
 * byte falls below 0x80.) */
static int xml_ok_chars(const char *s, size_t n)
{
    size_t i;
    for (i = 0; i < n; i++) {
...
static JSValue xml_writer_ctor(JSContext *ctx, JSValueConst new_target,
                               int argc, JSValueConst *argv)
{
    xml_writer_t *w;
    int32_t ind = 0;
    if (argc > 0 && JS_IsObject(argv[0])) {
        if (xml_opts_check(ctx, argv[0], xml_writer_keys,
                           countof(xml_writer_keys)) < 0)
            return JS_EXCEPTION;
        {
            JSValue v = JS_GetPropertyStr(ctx, argv[0], "indent");
            if (JS_IsException(v))
                return JS_EXCEPTION;
            if (!JS_IsUndefined(v) && !JS_IsNull(v)) {
                /* Strict types (the documented contract): a string "3" or a
                 * fractional 3.5 is a TYPE error, not a coercion; only a
                 * genuine integer gets range-checked. */
                double d;
                if (!JS_IsNumber(v)) {
                    JS_FreeValue(ctx, v);
                    return JS_ThrowTypeError(ctx,
                        "XmlWriter: indent must be an integer from 0 to 16");
                }
                ...
                if (!(d >= -2147483648.0 && d <= 2147483647.0) ||
                    d != (double)(int)d) {
                    /* NaN/Infinity/fractions/huge values: not integers. The
                     * bounds guard keeps the cast below defined. */
                    return JS_ThrowTypeError(ctx,
                        "XmlWriter: indent must be an integer from 0 to 16");
                }
                ind = (int)d;
        ...
        if (ind < 0 || ind > 16)
            return JS_ThrowRangeError(ctx, "XmlWriter: indent is 0 to 16");
    } else if (argc > 0 && !JS_IsUndefined(argv[0]) && !JS_IsNull(argv[0])) {
        return JS_ThrowTypeError(ctx, "XmlWriter(options): options must be an object");
    }
    w = (xml_writer_t *)calloc(1, sizeof *w);
    if (!w)
        return JS_ThrowOutOfMemory(ctx);
    xb_init(&w->out);
    w->indent = ind;
    return dyn_plain_wrap(ctx, new_target, dyn_xw_class_id, w, xml_writer_dispose);
}

static JSValue xml_writer_open(JSContext *ctx, JSValueConst this_val,
                               int argc, JSValueConst *argv)
{
    const char *nm;
    size_t nml;
    JSValue attrs = JS_UNDEFINED;
    xml_writer_t *w;
    if (argc < 1 || !JS_IsString(argv[0]))
        return JS_ThrowTypeError(ctx, "XmlWriter.open(name, attrs?): name must be a string");
    nm = JS_ToCStringLen(ctx, &nml, argv[0]);
    if (!nm)
        return JS_EXCEPTION;
    if (!xml_writer_valid_name(nm, nml)) {
        JS_FreeCString(ctx, nm);
        return JS_ThrowRangeError(ctx, "XmlWriter.open: not a valid element name");
    }
    if (argc > 1 && !JS_IsUndefined(argv[1]) && !JS_IsNull(argv[1])) {
        if (!JS_IsObject(argv[1])) {
            JS_FreeCString(ctx, nm);
            return JS_ThrowTypeError(ctx, "XmlWriter.open: attrs must be an object");
        }
        attrs = argv[1];
    }
    /* Coerce-then-resolve (the dyna-nat.h contract): every JS-invoking call
     * in this function -- JS_ToCStringLen here, the attribute enumeration
     * and the per-attribute getters below (user JS that may reenter the
     * writer or dispose `this`) -- runs BEFORE the native handle is
     * resolved. Nothing after the resolution invokes JS, so no user JS can
     * invalidate `w` between its resolution and its use. */
    {
        /* The attribute fragments are built in a scratch buffer, touching
         * only locals: an attribute getter that throws, an invalid attribute
         * name or a bad character mid-way must leave the writer exactly as
         * it was -- a half-written `<a ok="1"` with no matching push made
         * every later write permanently malformed. */
        xb_t ab;
        char *push;
        xb_init(&ab);
        if (JS_IsObject(attrs)) {
            JSPropertyEnum *tab = NULL;
            uint32_t len = 0, k;
            if (JS_GetOwnPropertyNames(ctx, &tab, &len, attrs,
                                       JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY) < 0) {
                xb_free(&ab);
                JS_FreeCString(ctx, nm);
                return JS_EXCEPTION;
            ...
            for (k = 0; k < len; k++) {
                ...
                an = JS_AtomToCStringLen(ctx, &anl, tab[k].atom);
                av = JS_ToCStringLen(ctx, &avl, v);
                JS_FreeValue(ctx, v);
                if (!an || !av) {
                    if (an) JS_FreeCString(ctx, an);
                    if (av) JS_FreeCString(ctx, av);
                    JS_FreePropertyEnum(ctx, tab, len);
                    xb_free(&ab);
                    JS_FreeCString(ctx, nm);
                    return JS_EXCEPTION;
                }
                if (!xml_writer_valid_name(an, anl)) {
                    JS_FreeCString(ctx, an);
                    JS_FreeCString(ctx, av);
                    JS_FreePropertyEnum(ctx, tab, len);
                    xb_free(&ab);
                    JS_FreeCString(ctx, nm);
                    return JS_ThrowTypeError(ctx, "XmlWriter.open: invalid attribute name");
                }
                if (!xml_ok_chars(av, avl)) {
                    JS_FreeCString(ctx, an);
                    JS_FreeCString(ctx, av);
                    JS_FreePropertyEnum(ctx, tab, len);
                    xb_free(&ab);
                    ...
                    return JS_ThrowRangeError(ctx,
                        "XmlWriter.open: attribute value contains a character "
                        "not legal in XML 1.0");
                }
                xb_put(&ab, ' ');
                xb_write(&ab, an, anl);
                xb_puts(&ab, "=\"");
                xml_escape(&ab, av, avl, 1);
                xb_put(&ab, '"');
                JS_FreeCString(ctx, an);
                JS_FreeCString(ctx, av);
                if (ab.oom) {
        ...
        /* The handle is resolved only NOW: the depth check (writer state,
         * so it cannot run before coercion) happens here too, after the
         * getters -- a reentrant open() from a getter counts toward it. */
        w = (xml_writer_t *)dyn_plain_get(ctx, this_val, dyn_xw_class_id);
        if (!w) {
            xb_free(&ab);
            JS_FreeCString(ctx, nm);
            return JS_EXCEPTION;
        }
        if (w->n >= XML_WRITER_MAX_DEPTH) {
            xb_free(&ab);
            JS_FreeCString(ctx, nm);
            return JS_ThrowRangeError(ctx, "XmlWriter: nesting exceeds %d",
                                      XML_WRITER_MAX_DEPTH);
        }
        /* Assemble the complete start tag and commit it, with the name push,
         * in one go: every failure path below unwinds with the writer
         * untouched. Nothing below this point invokes JS. */
        {
            xb_t tag;
            xb_init(&tag);
            if (w->out.n > 0)
                xml_writer_nl_x(&tag, w->indent, w->n);
            xb_put(&tag, '<');
            xb_write(&tag, nm, nml);
            if (ab.n)
                xb_write(&tag, ab.p, ab.n);
            if (ab.oom)
                tag.oom = 1;
            xb_free(&ab);
            xb_put(&tag, '>');
            if (w->n == w->cap) {
                ...
                char **ns = (char **)realloc(w->stack, nc * sizeof *ns);
                if (!ns) {
                    xb_free(&tag);
                    JS_FreeCString(ctx, nm);
                    return JS_ThrowOutOfMemory(ctx);
...
static JSValue xml_writer_text(JSContext *ctx, JSValueConst this_val,
                               int argc, JSValueConst *argv)
{
    xml_writer_t *w = (xml_writer_t *)dyn_plain_get(ctx, this_val, dyn_xw_class_id);
    const char *s;
    size_t n;
    if (!w)
        return JS_EXCEPTION;
    if (argc < 1 || !JS_IsString(argv[0]))
        return JS_ThrowTypeError(ctx, "XmlWriter.text(s): s must be a string");
    ...
    if (!xml_ok_chars(s, n)) {
        JS_FreeCString(ctx, s);
        return JS_ThrowRangeError(ctx,
            "XmlWriter.text: text contains a character not legal in XML 1.0");
    }
    xml_escape(&w->out, s, n, 0);
    JS_FreeCString(ctx, s);
    if (w->out.oom)
...
static JSValue xml_writer_close(JSContext *ctx, JSValueConst this_val,
                                int argc, JSValueConst *argv)
{
    xml_writer_t *w = (xml_writer_t *)dyn_plain_get(ctx, this_val, dyn_xw_class_id);
    char *nm;
    ...
    if (!w)
        return JS_EXCEPTION;
    if (w->n == 0)
        return JS_ThrowRangeError(ctx, "XmlWriter.close: no open element");
    nm = w->stack[--w->n];
    if (w->indent)
        xml_writer_nl(w);
    xb_puts(&w->out, "</");
...
static JSValue xml_writer_to_string(JSContext *ctx, JSValueConst this_val,
                                    int argc, JSValueConst *argv)
{
    xml_writer_t *w = (xml_writer_t *)dyn_plain_get(ctx, this_val, dyn_xw_class_id);
    (void)argc; (void)argv;
    ...
    if (w->out.oom)
        return JS_ThrowOutOfMemory(ctx);
    return JS_NewStringLen(ctx, (const char *)w->out.p, w->out.n);
}

static JSValue xml_writer_finish(JSContext *ctx, JSValueConst this_val,
                                 int argc, JSValueConst *argv)
{
    xml_writer_t *w = (xml_writer_t *)dyn_plain_get(ctx, this_val, dyn_xw_class_id);
    JSValue out;
...
static const JSCFunctionListEntry dyn_xw_proto[] = {
    JS_CFUNC_DEF("open", 1, xml_writer_open),
    JS_CFUNC_DEF("text", 1, xml_writer_text),
    JS_CFUNC_DEF("close", 0, xml_writer_close),
    JS_CFUNC_DEF("toString", 0, xml_writer_to_string),
    JS_CFUNC_DEF("finish", 0, xml_writer_finish),
};
...
typedef struct {
    ...
    JSValue    on[6];
    int        ended, busy;
    int        depth;                   /* open elements below the root */
    int        root_closed;             /* a close brought depth back to 0 */
    /*the pull form. Mode is fixed at construction: ANY handler
     * registered => push mode, which scans inside write()/end() and
     * delivers to handlers exactly as before; a parser with NO handlers
     * (the argument is now optional) is a pull parser: write() only
     * buffers, next() scans on demand, and errors surface from whichever
     * call performed the failing scan step. A push parser may still call
     * next(): handlers fire whenever the scan runs, and next() returns
     * every event it crosses. */
    int        next_mode;               /* 0 push, 1 pull */
    int        failed;                  /* a scan error happened; sticky */
    int        pull_done;               /* ended, scanned out, checks passed */
} dyn_sax_t;
                            ...
                            int argc, JSValueConst *argv)
{
    ...
    /*the MODE rule. Passing a handlers object (even `{}`) keeps the
     * push contract -- write()/end() scan synchronously and errors throw
     * from there, exactly as since day one. Constructing with NO argument
     * (or undefined/null) is the PULL form: write() only buffers, next()
     * scans and returns event objects, and a pull parser's options come in
     * the second argument. An argument that is present but not an object
     * is still refused. */
    {
        int have_handlers = argc >= 1 && JS_IsObject(argv[0]);
        JSValueConst opt_src;
        if (argc >= 1 && !JS_IsUndefined(argv[0]) && !JS_IsNull(argv[0])
            && !JS_IsObject(argv[0]))
            return JS_ThrowTypeError(ctx,
                "new SAXParser(handlers): handlers must be an object");
        opt_src = have_handlers ? argv[0]
            : (argc >= 2 && JS_IsObject(argv[1]) ? argv[1] : JS_UNDEFINED);
        if (xml_read_opts(ctx, opt_src, &opts) < 0)
            return JS_EXCEPTION;
        ...
        s->next_mode = !have_handlers;
    }
    return dyn_plain_wrap(ctx, new_target, dyn_sax_class_id, s, dyn_sax_dispose);
}
...
/* The tree front end enforces a document discipline the scanner does not:
   exactly one root element and no non-blank text outside it. SAX shares the
   scanner, so mirror that discipline here or `write("<a/>x"); end()` would
   silently swallow what XMLParse rejects. Comments and PIs after the root are
   legal and stay legal; BLANK text at the top level is dropped WITHOUT an
   event, exactly the way xml_tree_text skips it (xml_tree_text drops blank
   text at depth 0 before trim is ever consulted). Returns 0 = deliver the
   event, 1 = skip it silently, -1 = malformed with s->scan.err set. */
static int dyn_sax_root_check(dyn_sax_t *s, const xml_ev_t *ev)
{
    switch (ev->kind) {
    case XEV_OPEN:
        if (s->root_closed) {
            snprintf(s->scan.err, sizeof s->scan.err,
                     "a document has one root element");
            return -1;
        }
        if (ev->selfclose) {
            if (s->depth == 0)
                s->root_closed = 1;     /* a self-closed root is a whole root */
        } else {
            s->depth++;
        }
        break;
    case XEV_CLOSE:
        if (s->depth == 0) {
            /* before the root, or after it: xml_tree_close refuses both */
            snprintf(s->scan.err, sizeof s->scan.err,
                     "close tag with no open tag");
            return -1;
        }
        if (--s->depth == 0)
            s->root_closed = 1;
    ...
    case XEV_CDATA:
        if (s->depth == 0) {
            if (ev->kind == XEV_TEXT && xml_all_space(ev->text.p, ev->text.n))
                return 1;           /* blank top-level text: no event, like the tree */
            snprintf(s->scan.err, sizeof s->scan.err,
        ...
        break;
    default:
        break;                      /* comment / PI / skipped DOCTYPE: legal */
    }
    return 0;
}
...
/*one scanned event as the pull form's value object. Fields are
   present only where they mean something: `name` on open/close/pi, `attrs`
   on open, `text` on text/cdata/comment and for the PI's data. */
static JSValue dyn_sax_event_obj(JSContext *ctx, const xml_ev_t *ev,
                                 uint64_t offset)
{
    static const char *const KIND_NAMES[6] = {
        "open", "close", "text", "cdata", "comment", "pi"
    };
    static const int KIND_SLOT[6] = { 0, 1, 2, 3, 4, 5 };
    JSValue obj, v;
    int kind;

    switch (ev->kind) {
    case XEV_OPEN:    kind = 0; break;
    case XEV_CLOSE:   kind = 1; break;
    case XEV_TEXT:    kind = 2; break;
    case XEV_CDATA:   kind = 3; break;
    case XEV_COMMENT: kind = 4; break;
    case XEV_PI:      kind = 5; break;
    default:          return JS_UNDEFINED;   /* XEV_SKIP: never surfaced */
    }
    obj = JS_NewObject(ctx);
    if (JS_IsException(obj))
        return obj;
    v = JS_NewString(ctx, KIND_NAMES[kind]);
    if (JS_IsException(v) || JS_SetPropertyStr(ctx, obj, "event", v) < 0
        || JS_SetPropertyStr(ctx, obj, "offset", JS_NewInt64(ctx,
                              (int64_t)offset)) < 0)
        return JS_EXCEPTION;
    switch (KIND_SLOT[kind]) {
    case 0:
        if (JS_SetPropertyStr(ctx, obj, "name",
                              JS_NewStringLen(ctx, ev->name, ev->nlen)) < 0)
            return JS_EXCEPTION;
        v = dyn_sax_attrs(ctx, ev);
        if (JS_IsException(v) || JS_SetPropertyStr(ctx, obj, "attrs", v) < 0)
            return JS_EXCEPTION;
        break;
    case 1:
        if (JS_SetPropertyStr(ctx, obj, "name",
    ...
    case 5:
        if (JS_SetPropertyStr(ctx, obj, "name",
            ...
            || JS_SetPropertyStr(ctx, obj, "text",
                                 JS_NewStringLen(ctx, ev->val, ev->vlen)) < 0)
            return JS_EXCEPTION;
        break;
    default:
        if (JS_SetPropertyStr(ctx, obj, "text",
                              JS_NewStringLen(ctx, (const char *)ev->text.p,
                                              ev->text.n)) < 0)
            return JS_EXCEPTION;
        break;
    }
...
/* End-of-document checks, shared verbatim by end() (push mode) and the
   last next() (pull mode). 0 = clean, -1 = scan.err holds the message. */
static int dyn_sax_end_checks(dyn_sax_t *s)
{
    if (s->depth > 0) {
        snprintf(s->scan.err, sizeof s->scan.err, "unclosed element");
        return -1;
    }
    if (!s->root_closed) {
        snprintf(s->scan.err, sizeof s->scan.err, "no root element");
        return -1;
    }
    if (s->scan.pos < s->scan.buf.n) {
...
/* Run the scanner until ONE event is produced or the input says stop.
   Returns:
     1  an event was scanned; *out owns its value object (pull) or it was
        delivered to the handler (push: *out owns the value object too)
     0  the scanner needs more input (or, with final set, the document
        ended cleanly: the end checks ran)
    -1  malformed: scan.err holds the message
    -2  a JS exception is pending (a handler threw, or an OOM)
   `deliver` 0 = pull step (events become value objects), 1 = push step
   (events go to their handlers; the value object is still built so
   next() can return it). */
static int dyn_sax_pump(dyn_sax_t *s, JSContext *ctx, JSValueConst this_val,
                        int deliver, JSValue *out)
{
    uint64_t off;
    int r, skip;

    *out = JS_UNDEFINED;        /* the -2 path may free this without a store */

    /* Hold the scan until a leading BOM can be decided (3 bytes, or the
       end of the stream) -- the same gate write() has always had, so a
       partial BOM can never reach the pending text token. */
    if (!s->scan.bom_done && s->scan.buf.n < 3 && !s->scan.final)
        return 0;
    xml_scan_bom(&s->scan);
    for (;;) {
        off = s->scan.total - s->scan.buf.n + s->scan.pos;
        r = xml_step(&s->scan, &s->ev);
        if (r == XS_NEED || r == XS_END)
            return 0;
        if (r == XS_ERR)
            return -1;
        skip = dyn_sax_root_check(s, &s->ev);
        if (skip < 0)
            return -1;
        if (skip)
            continue;                   /* blank top-level text: no event */
        if (s->ev.kind == XEV_SKIP)     /* a consumed DOCTYPE: no event
                                           either -- a pull caller must not
                                           see a step with no object */
            continue;
        if (deliver) {
            if (dyn_sax_emit(ctx, s, this_val) < 0)
                return -2;
        }
        *out = dyn_sax_event_obj(ctx, &s->ev, off);
        if (JS_IsException(*out))
            return -2;
        /* The event object owns copies of everything borrowed from the
           carry, so the consumed prefix can slide now -- a pull parse of a
           long document must not pin the whole input in the carry. */
        xml_compact(&s->scan);
        return 1;
    }
}

/* The pull form's one step. */
static JSValue dyn_sax_next(JSContext *ctx, JSValueConst this_val,
                            int argc, JSValueConst *argv)
{
    ...
    if (!s)
        return JS_EXCEPTION;
    /* The scanner carry is not reentrant: handlers already run under this
       flag, so next() from inside a handler is the same refusal. */
    if (s->busy)
        return JS_ThrowTypeError(ctx, "SAXParser: next() called from a handler");
    if (s->failed)
        return JS_ThrowSyntaxError(ctx, "SAXParser: %s", s->scan.err);
    if (s->pull_done)
        return JS_NULL;
    s->busy = 1;
    r = dyn_sax_pump(s, ctx, this_val, !s->next_mode, &out);
    s->busy = 0;
    if (r == 1)
        return out;                     /* the caller owns it */
    if (r == -2) {
        JS_FreeValue(ctx, out);         /* a failed event object still owns */
        return JS_EXCEPTION;
    }
    if (r == -1) {
        s->failed = 1;
    ...
    /* r == 0: nothing more right now. */
    if (s->scan.final) {
        /* With the stream finalized, NEED meant a truncated token and END
           meant the document is over: run end()'s checks here, since a
           pull parser's end() only sets the flag. */
        if (dyn_sax_end_checks(s) < 0) {
            s->failed = 1;
            return JS_ThrowSyntaxError(ctx, "SAXParser: %s", s->scan.err);
        }
        s->pull_done = 1;
    ...
    return JS_NULL;                     /* caller may write() more */
}

/* The push-mode scan loop: pump until the input says stop, delivering each
   event to its handlers. Returns 0, -1 (malformed, err set), -2 (JS
   exception pending). Callers hold s->busy. */
static int dyn_sax_drive(dyn_sax_t *s, JSContext *ctx, JSValueConst this_val)
{
    for (;;) {
        JSValue out = JS_UNDEFINED;
        int r = dyn_sax_pump(s, ctx, this_val, 1, &out);
        if (r < 0) {
        ...
        if (r == 0)
            return 0;
        JS_FreeValue(ctx, out);         /* delivered; write() discards it */
    }
}
                            ...
                            int argc, JSValueConst *argv, int magic)
{
    ...
    if (magic == 0) {
        ...
        if (dyn_sax_append(ctx, s, argv[0]) < 0)
            return JS_EXCEPTION;
        /* A PULL parser (no handlers) defers every scan to next(): write()
           only buffers, so nothing but the bounded carry is held. */
        if (s->next_mode)
            return JS_UNDEFINED;
    } else {
        s->scan.final = 1;
        /* A PULL parser's end() is lazy too: it only finalizes. The
           end-of-document checks run at the last next(). */
        if (s->next_mode) {
            s->ended = 1;
            return JS_UNDEFINED;
        }
    ...
    s->busy = 1;
    rc = dyn_sax_drive(s, ctx, this_val);
    s->busy = 0;
    if (magic == 1) {
        s->ended = 1;
        /* Mirror the tree front end's end-of-document checks: an element
           still open and a document with no root are refused with the same
           texts XMLParse uses. depth/root_closed are maintained by
           dyn_sax_root_check. */
        if (rc == 0 && dyn_sax_end_checks(s) < 0)
            rc = -1;
    }
    if (rc == -2)
        return JS_EXCEPTION;
    if (rc == -1) {
        s->failed = 1;                  /* a later next() re-throws this */
        return JS_ThrowSyntaxError(ctx, "SAXParser: %s", s->scan.err);
    }
    return JS_UNDEFINED;
}
...
static const JSCFunctionListEntry dyn_sax_proto[] = {
    ...
    JS_CFUNC_DEF("next", 0, dyn_sax_next),
};
...
static int xml_collapse_attrs(JSContext *ctx, JSValueConst el, JSValue out)
{
    ...
    for (k = 0; k < alen && rc == 0; k++) {
        JSValue v = JS_GetProperty(ctx, attrs, tab[k].atom);
        size_t anl = 0;
        const char *an = JS_AtomToCStringLen(ctx, &anl, tab[k].atom);
        char *key;
        JSAtom ka;
        ...
        /* heap key of the FULL length: a fixed 256-byte buffer truncated
           distinct attribute names into collisions (last one silently won) */
        key = (char *)malloc(anl + 2);
        if (!key) {
            JS_FreeCString(ctx, an);
            JS_FreeValue(ctx, v);
            rc = -1;
        ...
        key[anl + 1] = '\0';
        JS_FreeCString(ctx, an);
        ka = JS_NewAtomLen(ctx, key, anl + 1);
        free(key);
        if (ka == JS_ATOM_NULL
...
/*the async-iterable wrapper. `SAXParser.iterate(source[, options])`
 * builds its own no-handler (pull) parser and drives it with each chunk
 * the source yields, so `for await (const ev of SAXParser.iterate(src))`
 * surfaces one event object per step.
 *
 * Shape notes. `iterate` itself is a C method: it constructs the parser
 * through the real constructor (so options and the pull mode rule live in
 * exactly one place) and then calls the async-generator function stored
 * beside it. That generator is a NON-CAPTURING eval'd function held as a
 * HIDDEN, non-enumerable property of the constructor: storing a closure
 * that captures the constructor ON the constructor forms a property cycle
 * (ctor -> prop -> closure scope -> ctor) this engine's teardown does not
 * tolerate, and eval'd-at-init global code is the house way to get
 * async-generator syntax where the public C API has no spelling for it
 * (the dyna:fetch glue precedent). The engine's own for-await machinery
 * owns Symbol.asyncIterator lookup for sync AND async sources. */
static const char dyn_sax_iterate_gen_js[] =
    "(async function* (parser, source) {\n"
    "    if (source == null)\n"
    "        throw new TypeError('SAXParser.iterate(source): source is required');\n"
    "    var ev;\n"
...
#define DYN_SAX_GEN_PROP "\x02" "iterateGen"    /* hidden; never collides */

static JSValue dyn_sax_iterate(JSContext *ctx, JSValueConst this_val,
                               int argc, JSValueConst *argv)
{
    JSValue parser, genfn, gen;
    JSValueConst cargs[2];

    if (argc < 1 || JS_IsUndefined(argv[0]) || JS_IsNull(argv[0]))
        return JS_ThrowTypeError(ctx,
            "SAXParser.iterate(source): source is required");
    /* this_val is the SAXParser constructor (a static on it): construct
       through it, so the pull-mode rule and options live in one place. */
    cargs[0] = JS_UNDEFINED;
    cargs[1] = argc >= 2 ? argv[1] : JS_UNDEFINED;
    parser = JS_CallConstructor(ctx, this_val, 2, cargs);
    if (JS_IsException(parser))
        return JS_EXCEPTION;
    genfn = JS_GetPropertyStr(ctx, this_val, DYN_SAX_GEN_PROP);
    if (!JS_IsFunction(ctx, genfn)) {
        JS_FreeValue(ctx, genfn);
        JS_FreeValue(ctx, parser);
        return JS_ThrowTypeError(ctx,
            "SAXParser.iterate: internal generator unavailable");
    }
    {
        JSValueConst gargs[2] = { parser, argv[0] };
        gen = JS_Call(ctx, genfn, JS_UNDEFINED, 2, gargs);
    }
    JS_FreeValue(ctx, genfn);
    JS_FreeValue(ctx, parser);
    return gen;
...
static int dyn_xml_init_module(JSContext *ctx, JSModuleDef *m)
{
    ...
    if (dyn_register_plain_class(ctx, m, &dyn_sax_class_id, &dyn_sax_class,
        ...
        return -1;
    if (dyn_register_plain_class(ctx, m, &dyn_xw_class_id, &dyn_xw_class,
                                 dyn_xw_proto, countof(dyn_xw_proto),
                                 xml_writer_ctor, "XmlWriter") < 0)
        return -1;
    proto = JS_GetClassProto(ctx, dyn_sax_class_id);
    ...
    genfn = JS_Eval(ctx, dyn_sax_iterate_gen_js,
                    strlen(dyn_sax_iterate_gen_js),
                    "<dyna:xml:iterate>", JS_EVAL_TYPE_GLOBAL);
    if (!JS_IsFunction(ctx, genfn)) {
        /* -1 from a module init requires a pending exception: keep the
           eval's, and supply one if the snippet simply mis-evaluated. */
        if (!JS_IsException(genfn))
            JS_ThrowTypeError(ctx,
                "dyna:xml: iterate glue did not evaluate to a function");
        JS_FreeValue(ctx, genfn);
        JS_FreeValue(ctx, ctor);
        return -1;
    }
    {
        JSValue itfn = JS_NewCFunction(ctx, dyn_sax_iterate, "iterate", 2);
        if (JS_IsException(itfn)
            || JS_SetPropertyStr(ctx, ctor, "iterate", itfn) < 0) {
            JS_FreeValue(ctx, itfn);
            JS_FreeValue(ctx, ctor);
            return -1;
        }
    }
    /* The generator function rides the constructor as a hidden,
     * non-enumerable property: reachable for GC, invisible to surface
     * sweeps and to Object.keys. NO reference back to the constructor is
     * stored anywhere (see the cycle note above). */
    if (JS_DefinePropertyValueStr(ctx, ctor, DYN_SAX_GEN_PROP, genfn,
                                  JS_PROP_WRITABLE | JS_PROP_CONFIGURABLE) < 0) {
        JS_FreeValue(ctx, genfn);
        JS_FreeValue(ctx, ctor);
        return -1;
    }
    /* genfn ownership moved to the property. */
    rc = JS_SetModuleExportList(ctx, m, dyn_xml_funcs, countof(dyn_xml_funcs));
    JS_FreeValue(ctx, ctor);
    return rc;
}
