...
Error messages follow one grammar — the `<module>.<context>:` prefix, the option-bag refuse-don't-coerce rule, and throwing-getter propagation — documented in [docs/ERRORS.md](docs/ERRORS.md); `make check-api` fails a build whose messages name APIs that no longer exist.
