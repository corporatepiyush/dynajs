...
-d  --dump         dump the memory usage stats
    ...
    --no-prototypes do not install the project-added Array/String/Number/
                   Date/Function/Iterator prototype extensions
...
Module autodetection looks at the FIRST statement: a file whose first statement is an
`import`/`export` is a module; anything else is a script, and an `import` appearing later is
a SyntaxError. Use `-m`/`--script` to force either mode.

### Running untrusted code (read this once)

DynaJS has **no in-process sandbox**: any script can `import` full-OS modules
(`dyna:sys` gives `Exec`, `dyna:file` the filesystem). For untrusted input, run in a
container/jail AND use the budget flags:

```sh
dynajs --timeout-ms 5000 --memory-limit 268435456 --native-memory-limit 268435456 untrusted.js
```

- `--timeout-ms N` — kill runaway loops (InternalError "interrupted") after N ms.
- `--memory-limit N` — cap the JS heap (JS_SetMemoryLimit).
- `--native-memory-limit N` — cap module-native allocations (setNativeMemoryLimit).

`std`/`os` modules are registered ONLY under `--std`; without the flag
`import "os"` fails, so `os.exec` is unreachable (the CLI's own helpers —
`console`, `print`, `TextEncoder`, `fetch` — are always present).

### Prototype extensions and `--no-prototypes`

By default DynaJS extends the standard prototypes with ~100 project-added
helpers (`Array.prototype.sum/first/unique/...`, `String.prototype.words/...`,
`Number.prototype.clamp/upto/...`, `Date.prototype.isLeapYear/...`,
`Function.prototype` combinators, lazy `Iterator` tiers, plus `Array.repeat`,
`Object.*` and `Function.*` statics). They are non-enumerable, so `for..in`
and `JSON.stringify` never see them.

Embedders and strict-baseline users can boot the engine WITHOUT them:

```sh
dynajs --no-prototypes script.js        # or: DYNAJS_NO_PROTOTYPES=1 dynajs script.js
```

Either mechanism opts out; the flag wins over an explicitly disabling
`DYNAJS_NO_PROTOTYPES=0`. Only the project-added tiers are removed — every
standard ES builtin (including standard Iterator helpers such as `map`/
`filter`/`take`/`drop`) is untouched, and `std`/`os` plus the `dyna:*`
modules keep working without them. Scripts that use the extensions will of
course fail with "not a function"; that is the point.
...
**`dyna:stream`** is the backbone for data bigger than memory: a pull-based `ByteSource` fills a buffer you own, a buffered `ByteSink` accepts bytes and flushes on demand, and `pipe()` moves bytes between any source and any sink — both are duck-typed interfaces, so a `fetch` body or subprocess output plugs in without glue. `lines()` and `ndjson()` turn a source into chunk-safe async iterators, and `inflate()`/`deflate()` wrap either side in streaming zstd, brotli, lz4 or gzip.
...
|---|---|
| [`dyna:stream`](API.md#dynastream) | Pull-based byte streaming: `pipe()`, file sources and sinks, chunk-safe `lines()`/`ndjson()` iterators, streaming zstd/brotli/lz4/gzip codecs |
| [`dyna:file`](API.md#dynafile) | Filesystem work: atomic writes, file locks, watchers, globs, temp files, platform directories |
| [`dyna:csv`](API.md#dynacsv) | CSV (RFC 4180): streaming reads, type inference, a file-backed table, in-memory `parse`/`stringify` with custom delimiters |
...
| Module | What it provides |
|---|---|
| [`dyna:crypto`](API.md#dynacrypto) | AEAD encryption (AES-GCM, ChaCha20-Poly1305), signatures (RSA, RSA-PSS, ECDSA, Ed25519), RSA-OAEP sealing, key exchange, Argon2id/scrypt/bcrypt, HOTP/TOTP verification, X.509 certificates, JWT — constant-time wherever secrets are compared |
...
| [`dyna:random`](API.md#dynarandom) | OS-entropy randomness plus a seedable PRNG whose full state checkpoints and replays: normal/exponential/Poisson draws, shuffle/sample/choice, byte fills, `jump()`/`longJump()` |
...
| [`dyna:time`](API.md#dynatime) | Dates that work: RFC 5545 recurrence (`RRule`), `Duration`, civil calendar types (`PlainDate`, `PlainTime`, `PlainDateTime`), time zones, duration parsing (`"1h30m"` → milliseconds) |

### Identity and validation

| Module | What it provides |
|---|---|
| [`dyna:uuid`](API.md#dynauuid) | UUID v1–v8 from the OS CSPRNG, plus NanoID and ULID with a monotonic mode |
| [`dyna:semver`](API.md#dynasemver) | Version numbers: semver 2.0.0 parsing, ranges, comparison, incrementing |
| [`dyna:validate`](API.md#dynavalidate) | One-line checks: email, URL, UUID, credit card, IBAN, JWT, IPv4/IPv6, ports, base64, hex, hex colors, JSON, MIME types, RFC 3339 timestamps, strong passwords — boolean predicates |
...
**Web-platform globals.** `fetch`, `Request`, `Response`, `Headers`, `FormData`, `AbortController`, `TextEncoder`, `TextDecoder`, `URL`, `URLSearchParams`, `structuredClone`, `atob`/`btoa`, `queueMicrotask`, `console`, and the timer family alongside a promise-based `sleep` exist on `globalThis` without imports — scripts written against web APIs run unmodified.
...
**Failures are named, not silent.** Library calls that refuse input throw with the reason in the message (`"gzip: level must be a number"`, `"invalid function bytecode (var_ref_idx)"`); option bags are checked key by key, so an unknown option is a `TypeError` naming the valid set — `unknown option "leve" (valid: level)` — and a typo can never quietly change behavior; parsers bound untrusted input with caps that reject rather than truncate. The README and API reference examples are executed by the build's own documentation gate, so what the docs claim is what the binary does.
...
### 8. Stream a compressed access log through bounded memory

A month of synthetic access events is compressed as it is written, read back through a decompressing source that fills one reused buffer, and the 5xx lines stream straight into a second compressed file. Peak memory is one 64 KiB buffer plus the current partial line — the 15,000-line log is never resident.
...
### 9. Validate input with the web globals and checked option bags

The web platform globals need no import, `dyna:validate` answers the checks in one call each, and every option bag in the standard library is checked key by key — a typo throws a `TypeError` that names the valid set instead of silently changing behavior.
...
### 10. Ship the program as one executable
...
DynaJS is a library first. The engine builds as `libdynajs.a` for direct embedding, and [recipe 10](#10-ship-the-program-as-one-executable) shows the `dynajsc` path that turns any of the programs above into a single self-contained native executable. The public headers are `src/dynajs.h` and `src/dyna-libc.h`; `examples/` holds three patterns — static JS compilation (`hello.js`), ES modules (`hello_module.js`), and an external native module (`fib.c` → `fib.so`).
...
The repository treats verification as part of the product, not a phase. Every push passes a ten-stage pre-push gate, installed as the `pre-push` git hook: source-shape analysis (struct padding, function complexity, hand-written link lines — regressions no compiler reports), a zero-warning clean build, fuzz-target link proofs, an import/orphan audit, the 41-suite core engine run, the 241-suite native-module run, the eight-layer API audit (every documented name exists in the binary and runs; every `README`/`API.md` example block executes against the current build), the adversarial security suites, a pty-driven REPL harness, and the TLS/AEAD vector checks.

A fuller local battery (`./dev.sh gate`) adds sanitizer legs on top: ASan and UBSan over the complete suite, TSan over the threaded HTTP paths, and test262 at a pinned baseline. WHATWG WPT-URL conformance sits at **zero recorded failures**, regression-locked by the conformance suite.

Three libc/ISA legs (glibc, musl, emulated amd64) run in containers with cached toolchains, so a warm leg recompiles only what changed. The hostile-input posture is regression-locked: hand-crafted bytecode blobs that would crash the reader are a permanent test target, and the reader rejects them with named errors.

Error messages follow one grammar — the `<module>.<context>:` prefix, the option-bag refuse-don't-coerce rule, and throwing-getter propagation — documented in [docs/ERRORS.md](docs/ERRORS.md); `make check-api` fails a build whose messages name APIs that do not exist.
