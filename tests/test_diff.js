...
/* A trailing-newline DIFFERENCE is a real edit: ["a\n","b"] vs ["a\n","b\n"]. */
{
...
/* \r stays inside its line token; a CRLF substitution has no trailing common */
{
    const h = checkPair(DiffLines, "a\r\nb\r\n", "a\r\nc\r\n", "CRLF substitution");
...
{
    /* tokens are RUNS: "a  b" is [a,"  ",b] while "ab" is ["ab"] -- nothing
       shared, so a full replace IS the minimal script (LCS 0). */
    const h = checkPair(DiffWords, "a  b", "ab", "run-tokenization full replace");
    eq(h.length, 2, "words: [a,'  ',b] vs [ab] is a minimal del+ins");
    /* but the same words with a one-space separator keep their commons */
