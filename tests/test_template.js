...
/* THE ESCAPING, which is the whole reason this lives beside the sanitizer. */
{
    ...
    {
        /* The escaper's window: a value whose only escapable byte sits past
           offset 64 must not get the byte AT the window edge rewritten. */
        eq(R("{{v}}", { v: "a".repeat(100) + "<" }), "a".repeat(100) + "&lt;",
           "an escape past the 64-byte window in a value is still escaped");
    }
}
