...
/* ============================== Varint (LEB128 unsigned) ============================== */
{
    ...
    /* overflow: value would need more than 64 bits -> RangeError (the old
     * [0, -11] sentinel leaked the codec's internal negative bytesRead
     * through a documented [value, bytesRead] API; the documented contract
     * is now a throw, like PutUvarint's argument overflow) */
    {
...
/* ============================== base85 (ascii85, optional bonus) ============================== */
{
    ...
    /* ---- full-group overflow: python a85decode parity ---- */
    /* audit E10-09: a 5-char group can name more than 32 bits ("uuuuu" is
     * 85^5-1 = 4,437,053,125 > UINT32_MAX); python base64.a85decode -- this
     * module's reference oracle -- raises ValueError("Ascii85 overflow") for
     * such groups, so the decoder must reject them (SyntaxError), not wrap a
     * 32-bit accumulator into non-canonical bytes. Boundary literals below
     * were verified against python3 base64.a85decode/a85encode; expected
    ...
    /* the overflow must be caught wherever it sits in the group stream */
    ...
    /* 510-byte python-oracle roundtrip corpus: (bytes(range(256))*2)[:510] --
     * every byte value 0..255 twice, truncated to 127 full groups + a 2-byte
     * partial tail. The two literals were generated by python3
     * (data.hex() / base64.a85encode(data), 638 chars) -- oracle-generated,
     * not engine roundtripped. */
    ...
    /* all-0xFF boundary sweep, len 0..8: every full group is the ceiling
     * group 's8W-!', and each partial tail (len%4 of 1..3) decodes a
     * '!'-padded near-max group -- stressing the accumulator ceiling from
     * the partial-group padding path too. Expected encodings python-
     * generated: base64.a85encode(b'\xff' * n). */
...
/* ============================== DetectEncoding (P1 3.4) ============================== */
{
    ...
    /* 3. CJK multibyte detection */
    /* GBK sample: "中文编码测试" in GBK */
    ...
    /* Shift-JIS sample: "日本語テストです" in Shift-JIS */
    ...
    /* EUC-JP with half-width kana (0x8E lead) */
    ...
    /* 4. Options: fallback and allowList */
...
/* ascii85: a partial group whose 'u'-padding pushes it past 2^32-1 is
 * refused, matching Python's a85decode ("Ascii85 overflow") -- it used to
 * silently truncate (uint8_t)(v>>24) and yield a wrong byte. */
{
