 ...
 * The pos+1 rescan is O(matches * n) byte-steps with no cap: "aa" over
 * "a".repeat(2e6) used to run ~4e12 of them uninterruptibly. A total-step
 * budget now refuses with a RangeError, fast. Normal inputs are untouched. */
{
    ...
    try { new Matcher("aa").countIn(dense); }
    catch (e) { threw = true; msg = String(e); }
    ms = Date.now() - ms;
    assert(threw, "a ~2e6-overlap countIn is refused (got: " + msg + ")");
    assert(/budget/.test(msg), "the refusal names the scan budget (got: " + msg + ")");
    assert(ms < 1000, "the budget trips quickly (took " + ms + " ms)");

    ms = Date.now(); threw = false;
    try { new Matcher("aa").allIn(dense); }
    catch (e) { threw = true; }
    ms = Date.now() - ms;
    assert(threw, "the same allIn is refused");
    assert(ms < 1000, "allIn trips quickly too (took " + ms + " ms)");
}
