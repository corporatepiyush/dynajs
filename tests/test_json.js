...
/* --------------------------------- over-19-digit index: RANGE, not LEADZERO */
{
    const doc = {arr: [1, 2]};
    let m = "";
    try { Pointer.get(doc, "/arr/99999999999999999999"); }
    catch (e) { m = String(e.message); }
    assert(m.indexOf("out of range") >= 0 && m.indexOf("leading zero") < 0,
        "a 20-digit index is a RANGE error, not 'leading zero' (got: " + m + ")");
    throws(() => Pointer.get(doc, "/arr/99999999999999999999"),
        "get still throws for the over-19-digit index");
    assert(Pointer.has(doc, "/arr/99999999999999999999") === false,
        "has: an over-19-digit index is out of range -> false, like RANGE");
    /* the same token through the Patch op ladder is range-flavored too */
    let m2 = "";
    try { Patch.apply(doc, [{op: "replace", path: "/arr/99999999999999999999", value: 0}]); }
    catch (e) { m2 = String(e.message); }
    assert(m2.indexOf("out of range") >= 0 && m2.indexOf("leading zero") < 0,
        "replace: a 20-digit index is range-flavored (got: " + m2 + ")");
    /* ...but a leading-zero index keeps its syntax-flavored message and the
       throwing has() (RFC 6901 4 unchanged). */
    let m3 = "";
    try { Pointer.get(doc, "/arr/01"); } catch (e) { m3 = String(e.message); }
    assert(m3.indexOf("leading zero") >= 0, "'01' is still a leading-zero error");
    throws(() => Pointer.has(doc, "/arr/01"), "has still throws on a leading zero");
}

/* ------------------------- __proto__: an own data property, never a retarget */
{
    /* Pointer.set uses DEFINE, so "/__proto__" creates an own data property
       and cannot repoint the object's prototype (pollution regression). */
    const d = {a: 1};
    Pointer.set(d, "/__proto__", 42);
    assert(Object.getPrototypeOf(d) === Object.prototype,
        "Pointer.set __proto__ does not change the prototype");
    assert(Object.getOwnPropertyDescriptor(d, "__proto__").value === 42,
        "Pointer.set __proto__ is an own data property");
    assert(Pointer.get(d, "/__proto__") === 42,
        "pointer reads resolve the OWN __proto__, never Object.prototype");
    assert(Pointer.has(d, "/__proto__") === true, "has: the own __proto__ is found");
    /* Patch add: same discipline on the copy-on-write result. */
    const r = Patch.apply({a: 1}, [{op: "add", path: "/__proto__", value: 7}]);
    assert(Object.getPrototypeOf(r) === Object.prototype,
        "Patch add __proto__ does not change the prototype");
    assert(Object.getOwnPropertyDescriptor(r, "__proto__").value === 7,
        "Patch add __proto__ is an own data property");
    /* clone keeps an own __proto__ as a data property. */
    const c = Patch.apply(d, []);
    assert(Object.getPrototypeOf(c) === Object.prototype,
        "cloning an own-__proto__ document keeps Object.prototype");
    assert(Object.getOwnPropertyDescriptor(c, "__proto__").value === 42,
        "clone keeps the own __proto__ as a data property");
    /* reads never fall through to Object.prototype members. */
    assert(Pointer.has({a: 1}, "/toString") === false,
        "has: prototype members are invisible to pointers");
    throws(() => Pointer.get({a: 1}, "/hasOwnProperty"),
        "get: prototype members are invisible to pointers");
    /* replace of __proto__ keeps it an own data property too. */
    const q = Patch.apply({a: 1}, [{op: "add", path: "/__proto__", value: 1},
                                   {op: "replace", path: "/__proto__", value: 8}]);
    assert(Object.getPrototypeOf(q) === Object.prototype,
        "Patch replace __proto__ does not change the prototype");
    assert(Object.getOwnPropertyDescriptor(q, "__proto__").value === 8,
        "Patch replace __proto__ is an own data property");
}

/* --------------------------------------------------- Patch.apply ops cap */
{
    const ops = [];
    for (let i = 0; i < 65537; i++) ops.push({op: "add", path: "/x", value: 1});
    let m = "";
    try { Patch.apply({}, ops); } catch (e) { m = String(e.message); }
    assert(/too many ops|cap/i.test(m),
        "65537 ops are refused by a clear cap error (got: " + m + ")");
    const capped = Patch.apply({}, ops.slice(0, 65536));
    assert(capped && typeof capped === "object" && capped.x === 1,
        "65536 ops (the cap) still apply");
}
...
{
    /* The graceful-throw DEPTH depends on the platform's stack (macOS throws
     * at ~10k, glibc's 8 MB stacks parse 10k fine and throw by 50k). Two
     * portable contracts:
     *   - 10k: either parses CORRECTLY or throws gracefully — never a crash,
     *     never a wrong answer;
     *   - 100k: overflows every supported platform, so it must throw
     *     gracefully. */
    function okThrow(e) {
        /* Stack overflow is node-aligned: RangeError "Maximum call stack size
           exceeded" (the quickjs InternalError "stack overflow" spelling is
           accepted too, so this guard stays portable across engine vintages;
           the parser's own recursion guard still says SyntaxError
           "stack overflow" and is covered by the same regexp). */
        return (e instanceof RangeError) || (e instanceof InternalError) ||
               /Maximum call stack size exceeded|stack overflow/.test(String(e));
    }
    const arr10k = "[".repeat(10000) + "0" + "]".repeat(10000);
    let handled = false;
    try {
        // the OUTER array has length 1 per nesting level; verify depth by walking
        const v = JSON.parse(arr10k);
        let d = v, steps = 0;
        while (Array.isArray(d) && d.length === 1) { d = d[0]; steps++; }
        handled = steps === 10000 && d === 0;
    } catch (e) {
        handled = okThrow(e);
    }
    assert(handled, "JSON.parse(10k-nested-array): parses correctly or throws stack overflow gracefully");

    const obj10k = '{"a":'.repeat(10000) + "0" + "}".repeat(10000);
    handled = false;
    try {
        const v = JSON.parse(obj10k);
        handled = !!v && typeof v === "object";
        // spot-check the nesting depth is real: walk to the centre
        let d = v, steps = 0;
        while (d && typeof d === "object" && "a" in d) { d = d.a; steps++; }
        handled = handled && steps === 10000;
    } catch (e) {
        handled = okThrow(e);
    }
    assert(handled, "JSON.parse(10k-nested-obj): parses correctly or throws stack overflow gracefully");

    for (const [label, s] of [
        ["array", "[".repeat(100000) + "]" .repeat(100000)],
        ["object", '{"a":'.repeat(100000) + "0" + "}".repeat(100000)],
    ]) {
        let threw = false;
        try {
            JSON.parse(s);
        ...
        assert(threw, "JSON.parse(100k-nested-" + label + ") throws stack overflow gracefully on every platform");
    }
}
