...
try {
  ...
  {
    /* overwrite:true onto ITSELF is REFUSED, on both arms, with the data
       intact. (The old O_TRUNC-at-open emptied the shared inode before the
       copy read a byte -- a quirk both arms used to match; audit F2 removed
       it, and the arms still agree, which is the invariant that matters.) */
  ...
  /* overwrite-onto-self must REJECT and leave the data intact, saying the
     same thing the sync call says (audit F2). */
  {
