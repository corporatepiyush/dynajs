...
/* RFC 9309: percent-encoded wildcard/end-anchor octets are matched
...
/* ==== audit regressions (ws-scrape): robots failure TTL + crawl pins ==== */
{
    /* a robots.txt fetch that fails on the NETWORK (the request throws) is
       fail-closed (RFC 9309 2.3.1.4) AND remembered for the TTL: the old
       code returned before stamping robots_next_ms, so every page fetch
       re-attempted robots. */
    let robotHits = 0;
    const dead = {
        request(method, url) {
            if (url.endsWith("/robots.txt")) { robotHits++; throw new Error("conn refused"); }
            return { status: 200, headers: {}, body: "page" };
        }
    };
    const f = new Fetcher({ agent: "ttl-bot/1.0", client: dead, minDelayMs: 0,
                            robotsTtlMs: 3600000 });
    ok(f.get("http://dead.test/a").skippedByRobots === true, "robots network error -> complete disallow");
    ok(f.get("http://dead.test/b").skippedByRobots === true, "and the second fetch skips too");
    eq(robotHits, 1, "S4: the failure is TTL-stamped (1 robots attempt for 2 fetches)");
    f.close();
}
{
    /* a client with per-path bodies drives Crawl without a socket */
    const pages = {
        "/robots.txt": null,   /* 404 -> MAY allow (2.3.1.3) */
        "/hub": '<a href="/dupA">a</a><a href="/dupB">b</a><a href="/master">m</a>',
        "/dupA": '<link rel="canonical" href="http://hub.test/master"><a href="/only-via-dupA">x</a>',
        "/dupB": '<link rel="canonical" href="http://hub.test/master"><a href="/only-via-dupB">x</a>',
        "/master": '<a href="/from-master">x</a>',
        "/n1": "",
        "/rel": '<a href="/n1" rel="nofollow">x</a><a href="/n2" rel="keep">y</a>',
        "/n2": ""
    };
    const client = {
        request(method, url) {
    ...
    const mkC = (seed, opts) => {
        ...
        const ex = new Extractor({
            links: { sel: new Selector("a"), attr: "href", all: true },
            rels: { sel: new Selector("a"), attr: "rel", all: true },
            mycanon: { sel: new Selector('link[rel="canonical"]'), attr: "href" }
        }, { text: HTMLText });
        const c = new Crawl(f, opts);
        c.start(seed, ex, HTMLParse);
        return c;
    };
    const drain = (c) => { const o = []; for (const p of c) o.push(p); return o; };
    const pathsOf = (c) => drain(c).map(p => pathOf(p.url));

    /* canonical dedup: duplicates EMIT but queue no links */
    const urls = pathsOf(mkC("http://hub.test/hub", { maxDepth: 2, canonicalField: "mycanon" }));
    ok(urls.includes("/dupA") && urls.includes("/dupB") && urls.includes("/master"),
       "canonical-declaring pages emit; the master itself is crawled");
    ok(!urls.includes("/only-via-dupA") && !urls.includes("/only-via-dupB"),
       "canonicalField: a page whose canonical target is already handled queues NO links");

    /* relField: a nofollow slot is skipped, a plain slot is followed */
    const rels = pathsOf(mkC("http://hub.test/rel", { maxDepth: 1, relField: "rels" }));
    ok(rels.includes("/n2") && !rels.includes("/n1"),
       "relField: the nofollow-marked link is skipped");

    /* iterator protocol: past-the-end stays done; the crawl is its own iterator */
    {
        const c = mkC("http://hub.test/rel", { maxDepth: 0 });
        c.next(); c.next();
        ok(c.next().done === true && c.next().done === true,
           "PIN: next() past the end keeps returning done:true");
        ok(typeof c[Symbol.iterator] === "function", "Symbol.iterator exists");
        ok(c.pages() === c, "PIN: pages() returns the crawl itself (the lazy page stream)");
        c.close();
    }

    /* seed scheme: only http(s) starts a crawl */
    {
        const c = mkC("http://hub.test/hub", {});
        throws(() => c.start("file:///etc/passwd"), "a file:// seed is refused");
        throws(() => c.start("ftp://x.test/"), "ftp:// is refused");
        c.close();
    }
}

/* ==== Option getters are user JS: their throws PROPAGATE (audit X-12) ==== */
{
    /* the ROBOTS response's `status` getter is user JS. The unchecked
       JS_ToInt32 used to swallow its exception, TTL-stamp the host
       "unreachable" (a JS error read as a robots outcome), and get()
       returned status:0 skippedByRobots -- a silent policy decision made
       from a bug in the caller's own object. */
    {
        const urls = [];
        const client = {
            request(method, url) {
                ...
                if (String(url).endsWith("/robots.txt"))
                    return { get status() { throw new Error("boom-status"); },
                             headers: {}, body: "x" };
                return { status: 200, headers: {}, body: "page" };
            }
        };
        const f = new Fetcher({ agent: "x12-bot/1.0", client, minDelayMs: 0 });
        let msg = "";
        try { f.get("http://seed.test/a"); } catch (e) { msg = e.message; }
        eq(msg, "boom-status", "robots response status getter throw propagates");
        eq(urls.length, 1, "no page request once the fetcher's own getter threw");
    }
    /* the PAGE response's status getter, robots off: the throw must reach
       the caller cleanly (no retries, no result built from status 0) */
    {
        let requests = 0;
        const client = {
            request() { requests++; return { get status() { throw new Error("boom-status"); },
                                             headers: {}, body: "x" }; }
        };
        const f = new Fetcher({ agent: "x12-bot/1.0", client, robots: false, minDelayMs: 0 });
        let msg = "";
        try { f.get("http://seed.test/a"); } catch (e) { msg = e.message; }
        eq(msg, "boom-status", "page response status getter throw propagates");
        eq(requests, 1, "no retry loop after the status getter threw");
    }
    /* CONTROL: good responses through the same shapes still work */
    {
        const client = { request(m, u) { return { status: 200, headers: {}, body: "fine" }; } };
        const f = new Fetcher({ agent: "x12-bot/1.0", client, minDelayMs: 0 });
        eq(f.get("http://seed.test/a").status, 200, "CONTROL: good robots+page still fetches");
    }
    /* Crawl: a throwing status getter propagates BEFORE the extractor runs
       (the old code extracted, queued links, and stored the exception value
       itself as page.status while crawling on at status 0) */
    {
        let extracted = 0;
        const fetcher = { get() { return { get status() { throw new Error("boom-status"); },
                                           headers: {}, body: "hello" }; } };
        /* note: this used to pass the extractor AS the ctor's options
           bag -- a shape the ctor silently ignored (argv[1] is options;
           argv[2] is never read), so the extractor was never attached. The
           extractor now goes through start(), where the API actually
           accepts it, making the "never runs" assertion meaningful. */
        const cr = new Crawl(fetcher);
        cr.start("http://seed.test/", { run() { extracted++; return { links: [] }; } }, (s) => s);
        let msg = "";
        try { cr.next(); } catch (e) { msg = e.message; }
        eq(msg, "boom-status", "Crawl: response status getter throw propagates");
        eq(extracted, 0, "Crawl: the extractor never runs after the getter threw");
        cr.close();
    }
    /* Crawl: the adjacent robotsDirectives getter, same rule */
    {
        const fetcher = { get() { return { status: 200, headers: {}, body: "hi",
                                           get robotsDirectives() { throw new Error("boom-rd"); } }; } };
        const cr = new Crawl(fetcher);
        cr.start("http://seed.test/", { run() { return { links: [] }; } }, (s) => s);
        let msg = "";
        try { cr.next(); } catch (e) { msg = e.message; }
        eq(msg, "boom-rd", "Crawl: robotsDirectives getter throw propagates");
        cr.close();
    }
    /* Crawl CONTROL: a good response still emits a page with a real status */
    {
        const fetcher = { get() { return { status: 200, headers: {}, body: "hi" }; } };
        const cr = new Crawl(fetcher);
        cr.start("http://seed.test/", { run() { return { links: [] }; } }, (s) => s);
        const it = cr.next();
        ok(it.done === false && it.value && it.value.status === 200,
           "CONTROL: Crawl still emits good pages");
        cr.close();
    }
    /* ctor option reads are strict too: a throwing getter fails the
       construction with the ORIGINAL error (no silent defaults -- a
       swallowed maxPages landed at 0, a Crawl that emits done immediately) */
    {
        const client = { request() { return { status: 200, headers: {}, body: "" }; } };
        let msg = "";
        try { new Fetcher({ agent: "x12-bot/1.0", client,
                            retries: { valueOf() { throw new Error("boom-retries"); } } }); }
        catch (e) { msg = e.message; }
        eq(msg, "boom-retries", "Fetcher ctor retries getter throw propagates");
        msg = "";
        try { new Fetcher({ agent: "x12-bot/1.0", client,
                            minDelayMs: { valueOf() { throw new Error("boom-delay"); } } }); }
        catch (e) { msg = e.message; }
        eq(msg, "boom-delay", "Fetcher ctor minDelayMs getter throw propagates");
        msg = "";
        try { new Crawl({ get() { return { status: 200, headers: {}, body: "" }; } },
                         { maxPages: { valueOf() { throw new Error("boom-maxpages"); } } }); }
        catch (e) { msg = e.message; }
        eq(msg, "boom-maxpages", "Crawl ctor maxPages getter throw propagates");
        /* CONTROL: numeric options still build */
        ok(new Fetcher({ agent: "x12-bot/1.0", client, retries: 2 }) instanceof Fetcher,
           "CONTROL: numeric Fetcher options still build");
        ok(new Crawl({ get() { return { status: 200, headers: {}, body: "" }; } },
                     { maxPages: 5 }) instanceof Crawl,
           "CONTROL: numeric Crawl options still build");
    }
}
