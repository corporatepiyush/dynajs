...
/* ---- audit regressions (ws-scrape) ---- */

/* S1: the wildcard fallback must search ALL groups. The argmax seeds the
   winner index with an unmatched group, and the old scan broke out at
   group 0 -- a named group BEFORE the '*' (the common layout) left
   unlisted agents pinned to its rules instead of allow-all. */
const s1 = new Robots(
  "User-agent: dynabot\nDisallow: /\n\nUser-agent: *\nAllow: /\n",
  { agent: "other" });
ok(s1.allows("/yes") === true, "S1: '*' AFTER a named group is still found for an unlisted agent");
const s1b = new Robots("User-agent: dynabot\nDisallow: /\n", { agent: "other" });
ok(s1b.allows("/yes") === true, "S1: no applicable group and no '*' allows everything (RFC 9309)");

/* S2/S5: normalization writes to a separate buffer (an in-place 1->3 byte
   expansion overran the read cursor: "/a?b" became "/a%3F3"), and a '$'
   NOT at pattern end is a literal byte that must be encoded like the uri
   side encodes it. */
ok(new Robots("User-agent: *\nDisallow: /a?b\n").allows("/a?b") === false,
   "S5: a raw reserved octet in a rule survives normalization (in-place overrun)");
ok(new Robots("User-agent: *\nDisallow: /a$b\n").allows("/a$b") === false,
   "S2: a mid-pattern '$' is a literal byte and matches the url's raw '$'");
ok(new Robots("User-agent: *\nDisallow: /a$b\n").allows("/ab") === true,
   "S2: the mid-pattern '$' rule is not a prefix rule");
ok(new Robots("User-agent: *\nDisallow: /x$\n").allows("/x") === false &&
   new Robots("User-agent: *\nDisallow: /x$\n").allows("/xy") === true,
   "S2: a TRAILING '$' is still the end anchor");
ok(new Robots("User-agent: *\nDisallow: /caf\u00e9\n").allows("/caf\u00e9") === false,
   "S5: a raw non-ASCII rule survives normalization end to end");

/* S3: Crawl-delay is DIGITS [. DIGITS]; strtod's inf/nan/hex/exponent
   shapes are meaningless lines (an Infinity delay parked every fetch). */
ok(new Robots("User-agent: *\nCrawl-delay: inf\n").crawlDelay() === null, "S3: 'inf' refused");
ok(new Robots("User-agent: *\nCrawl-delay: nan\n").crawlDelay() === null, "S3: 'nan' refused");
ok(new Robots("User-agent: *\nCrawl-delay: 0x10\n").crawlDelay() === null, "S3: hex refused");
ok(new Robots("User-agent: *\nCrawl-delay: 1e2\n").crawlDelay() === null, "S3: exponent refused");
ok(new Robots("User-agent: *\nCrawl-delay: +5\n").crawlDelay() === null, "S3: sign refused");
ok(new Robots("User-agent: *\nCrawl-delay: 12.5\n").crawlDelay() === 12.5, "S3: digits [.digits] still parse");
