...
try {
  ...
  /* ---- 8b.: the async bags are strict, like every bag in the module ---- */
  /* PINNED CONTRACT: options are validated on the loop, before the submit,
   * so a bad bag throws SYNCHRONOUSLY rather than rejecting -- the caller
   * learns about the typo at the call site, not three turns later. */
  {
    const p = tmp("strict");
    ...
    /* null/undefined opts keep meaning "no options" */
