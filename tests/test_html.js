...
{
    /* THE ESCAPER'S 64-BYTE WINDOW: the same unexamined-byte bug the text
       decoder had -- a 65..127-byte escape-free run used to get byte 64
       rewritten ('"' into text, "&quot;" into an attribute). */
    const para = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed.";
    eq(HTMLStringify(HTMLParse("<p>" + para + "</p>")), "<p>" + para + "</p>",
       "an escape-free 70-byte text run survives the escaper");
    eq(HTMLStringify({ name: "p", attrs: { t: "a".repeat(70) + "<x" }, children: [] }),
       '<p t="' + "a".repeat(70) + "&lt;x" + '"></p>',
       "and so does an attribute value with its escape past the window");
}
 ...
 * the classic mXSS opening. */
{
    ...
    /* The close tag needs the WHATWG delimiter: `</scripty` is NOT a close
       tag, and ending the raw scan there would drop the parser back into
       markup mode while still inside <script>. */
    eq(HTMLParse("<script>x</scripty>alert(1)</script>")[0].children[0],
       "x</scripty>alert(1)", "a longer tag name does not end raw text");
    eq(HTMLParse("<script>x</script >y</script>").map(v =>
           typeof v === "string" ? v : v.children[0]).join("|"), "x|y",
       "a space before > still closes raw text");
}
...
{
    /* THE 64-BYTE WINDOW: a text run of 65..127 bytes with no '&' in its
       first 64 used to lose byte 64 (replaced by '&') because the SIMD probe
       was skipped for short tails while the byte at the window edge had not
       been examined. Ordinary paragraph-length text is exactly this shape. */
    eq(HTMLParse("<p>" + "a".repeat(100) + "</p>")[0].children[0],
       "a".repeat(100), "a 100-byte text run survives intact");
    eq(HTMLParse("<p>" + "a".repeat(64) + "#65;b</p>")[0].children[0],
       "a".repeat(64) + "#65;b", "byte 64 starting a fake entity is NOT decoded");
    eq(HTMLParse("<p>" + "a".repeat(63) + "Xb</p>")[0].children[0],
       "a".repeat(63) + "Xb", "byte 64 of a 65-byte run is not eaten");
    eq(HTMLParse("<p>" + "a".repeat(100) + "&amp;" + "b".repeat(100) + "</p>")[0].children[0],
       "a".repeat(100) + "&" + "b".repeat(100),
       "an entity beyond the window still decodes");
    eq(HTMLParse('<p title="' + "x".repeat(70) + "&amp;y" + '">v</p>')[0].attrs.title,
       "x".repeat(70) + "&y", "and attribute values take the same path");
}
...
{
    /* DUPLICATE ATTRIBUTES: a browser keeps the FIRST occurrence, so a later
       one must not overwrite it (last-wins is how injection authors win). */
    const o = HTMLParse('<a href="1" href="2">t</a>');
    eq(o[0].attrs.href, "1", "the first duplicate attribute wins");
    eq(HTMLParse('<a HREF="1" href="2">t</a>')[0].attrs.href, "1",
...

{
    ...
    /* The nearest ancestor matching a compound is not always the right one
       when a child combinator follows: p#hit's nearest b has b for a parent,
       but the outer b has a. Matching must backtrack, not stop at the first. */
    {
        const bt = HTMLParse('<a><b><p id="miss"></p></b>' +
                             '<b><b><p id="hit"></p></b></b></a>');
        eq(new Selector("a > b p").all(bt).map(e => e.attrs.id).join(","),
           "miss,hit", "a > b p backtracks past the nearest b");
    }
    ...
    /* The parser lower-cases attribute names, so the selector's name is
       matched the same way: a hand-built or upper-case tree selects too. */
    eq(all("[CLASS=x]").length, 2, "[CLASS=...] matches case-insensitively");
    const first = new Selector("p").first(doc);
    eq(first.attrs.class, "x", "first() returns the first in document order");
    eq(new Selector("nope").first(doc), undefined, "or undefined when no match (API.md:824)");
...
{
    /* URL sinks BEYOND href/src: an allow-list that names srcset, background,
     * ping or style without a `protocols` rule gets the same safe-scheme
     * default, so a dangerous URL is dropped exactly like a javascript: href. */
    const s2 = new Sanitizer({
        allow: { img: ["src", "srcset"], td: ["background"], a: ["href", "ping"] },
    });
    eq(s2.clean('<img srcset="javascript:alert(1) 1x, //e.com/a.png 2x">'),
       "<img>", "a srcset carrying javascript: is dropped whole");
    eq(s2.clean('<img srcset="a.png 1x, b.png 2x">'),
       '<img srcset="a.png 1x, b.png 2x">', "and a relative srcset stays");
    eq(s2.clean('<td background="javascript:alert(1)">x</td>'),
       "<td>x</td>", "a background carrying javascript: is dropped");
    eq(s2.clean('<a ping="javascript:alert(1)" href="/r">y</a>'),
       '<a href="/r">y</a>', "a ping carrying javascript: goes, the href stays");
}
