...
/* Stubs the DNS resolver grew after the harness was written (error paths and
   the cache-cap probe): never driven here, link-only, same shape as above. */
...
{ (void)ctx; return JS_EXCEPTION; }
/* signature must match dyna-nat.h exactly: the TU includes dyna-net-dns.c,
   which includes the header -- a drifted stub is a conflicting declaration
   (found by the gate's C regression stage, stale since new_target landed) */
