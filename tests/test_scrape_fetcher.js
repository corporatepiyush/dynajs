...
if (!Which("python3")) {
    ...
    for (let i = 0; i < 60 && !(PORT && PORT2 && PORT3); i++) {
        ...
        try { PORT2 = parseInt(readFile(new Path(P("port2")), { encoding: "utf8" }), 10) || 0; }
        catch (e) {}
        try { PORT3 = parseInt(readFile(new Path(P("port3")), { encoding: "utf8" }), 10) || 0; }
        catch (e) {}
    }
    if (!PORT) {
    ...
    const BASE2 = "http://127.0.0.1:" + PORT2;   /* open robots: a DIFFERENT origin */
    const BASE3 = "http://127.0.0.1:" + PORT3;   /* robots 302 -> a dead port */
    ...
    /* robots apply PER ORIGIN (scheme+host+port): the same host on another
       port has its own robots.txt, its own crawl-delay, its own state. */
    {
        const f = mk({});
    ...
    /* a robots fetch that fails on the NETWORK (a 30x to a dead port) is
       fail-closed AND TTL-stamped: the old code returned before stamping,
       so every page fetch re-attempted robots (one dead canonical-host
       redirect turned each fetch into a fresh robots fetch). */
    {
        const f = mk({ robotsTtlMs: 3600000 });
        f.get(BASE3 + "/allowed");
        f.get(BASE3 + "/allowed");
        /* a fail-closed host skips EVERY path, counters included: read /hits
           with robots off (a different fetcher, same server counters) */
