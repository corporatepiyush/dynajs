...
/* ================================================================= simd */
{
    ...
    if (!s) { skip++; print("-- simd SKIP"); }
        ...
        const UNARY = [
            ...
            /* leakyRelu/elu take a DOCUMENTED second argument (dynajs.d.ts:
               "leakyRelu(a: F32Like, slope: number)" / "elu(a: F32Like,
               alpha: number)"; API.md: "leakyRelu(a, slope) -> a" with
               x<0 ? slope*x : x, "elu(a, alpha) -> a" with
               x<0 ? alpha*(exp(x)-1) : x). The docs pin no default, so the
               row carries the canonical constants: slope 0.02 (the leaky-ReLU
               constant every framework defaults to) and alpha 1 (the
               canonical ELU, where alpha*(exp(x)-1) reduces to exp(x)-1).
               A row may append trailing args; the driver spreads them
               verbatim, so if a kernel's arity tightens again the call THROWS
               in the try below and the row fails loudly -- no throw is
               swallowed. */
        ...
        for (const [name, ref, tol, extra] of UNARY) {
            ...
            for (const n of LENGTHS) {
                ...
                let got;
                /* Trailing row args (leakyRelu's slope, elu's alpha) are
                   passed verbatim; a missing required argument must THROW
                   here and fail the row, never be defaulted silently. */
