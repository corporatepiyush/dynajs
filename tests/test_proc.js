...
/* --------------------------------------- NUL refusal at every boundary */

{
    /* JS strings carry NUL; the OS takes C strings. Passing a NUL-bearing
     * string used to act on its silent TRUNCATED PREFIX -- here, the exec
     * ran /bin/echo even though the full string was "/bin/echo\0-evil": a
     * JS-level validator that approved the full string was bypassed by the
     * prefix the syscall actually saw. setEnv's policy (refuse, don't
     * truncate) now holds for every Exec input and for Which. */
    ...
    /* and nothing was half-set up by the refusals */
    eq(Exec("echo", ["still-working"]).stdout, "still-working\n",
       "the engine survives the refusals and keeps spawning");
}

/* --------------------------------- argv past the OS limit: clean E2BIG */

{
    /* An argv the kernel refuses must surface as the shell-convention 127
     * from the child's _exit after a failed execve -- a clean nonzero code,
     * NEVER a signal or a crash, and the parent returns rather than hangs.
     * Where exactly the line sits is per-OS (macOS ~1MB total, Linux
     * MAX_ARG_STRLEN 128KB per arg), so the assertion is the failure MODE:
     * either it runs, or it is a tidy 127 with no signal. */
...
/* -------------------------- numeric options fail CLOSED on NaN */

{
    /* NaN coerces to 0, and 0 has dangerous meanings on two options:
     * timeoutMs 0 = "no deadline" (a silent HANG), and uid 0 = root. An
     * option whose coercion fails open must refuse non-finite values, the
     * same rule setNativeMemoryLimit enforces. (Infinity has no integer
     * form; maxBuffer NaN already failed safe via the positivity floor,
     * maxBuffer 0, and still does.) */
    ...
    /* fractional values keep their plain int64 truncation semantics */
