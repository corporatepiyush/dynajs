/* test_bytes.js — dyna:bytes (byte-buffer utilities: byte-slice search and
 * compare operations + Node Buffer read/write helpers, over
...
/* hex/base64 live in dyna:encoding -- dyna:bytes no longer duplicates them.
 * Imported here under the old names so the same assertions still run. */
...
/* ============================== count (non-overlapping semantics) ============================== */
{
    ...
    /* EMPTY-NEEDLE CONVENTIONS, pinned deliberately: count(s, []) === n+1
     * (the byte-count convention -- an empty needle sits at every seam),
     * while dyna:matcher's Matcher.countIn("") === 0 and allIn("") === []
     * (it reports only matches worth enumerating). The modules disagree ON
     * PURPOSE; the cross-reference lives in tests/test_matcher.js's
     * empty-pattern section too. */
...
/* ============================== DataView is a byte view ============================== */
{
    ...
    /* the engine's own DataView accessors see the same bytes */
    ...
    /* a DataView respects its own offset/length window, not the whole buffer */
...
/* ============================== bytesOf: explicit reinterpretation ============================== */
{
    /* a wider view is REJECTED rather than silently reinterpreted */
    ...
    /* bytesOf makes it legal, and covers exactly the view's own range */
    ...
    /* it ALIASES — the one non-copying function in the module */
    ...
    /* the offset/length footgun bytesOf exists to remove: a mid-buffer view */
    const buf = new ArrayBuffer(32);
    const mid = new Uint16Array(buf, 8, 4);          /* bytes 8..15 */
    ...
    /* DataView and ArrayBuffer arguments */
    ...
    /* a byte view passes through unchanged in extent */
...
/* ============================== reentrancy: valueOf detaching the buffer ============================== */
{
    ...
    {
        /* same discipline through the DataView resolve path */
        const ab = new ArrayBuffer(8);
        const dv = new DataView(ab);
...
/* ============================== Bytes: cached summaries stay honest under mutation ============================== */
{
    /* isAscii/isValidUtf8 are computed at construction, but the class's own
     * mutating paths (fill, write*, mutation through .array) used to leave
     * them stale: new Bytes("abc").fill(0xFF).isAscii answered TRUE. Every
     * mutation now invalidates them and the next read rescans. */
    ...
    /* the free fill() takes a Bytes handle too -- it must invalidate it */
    const h = new Bytes("abc");
    fill(h, 0xFF);
    assert(h.isAscii === false, "Bytes: free fill() on a handle invalidates its flags");

    /* a slice computes its OWN summaries, so a mutated slice reports honestly */
...
/* ============================== text family: wide views and detached buffers ============================== */
{
    /* The module's bpe rule ("reject a wider-element view; use bytesOf()"),
     * enforced one function over in dyn_bytes_view, now holds for the Text
     * helpers too: a Uint16Array's raw bytes are not silently reinterpreted. */
    ...
    /* A detached buffer is a clear error, never "[object Uint8Array]" text. */
    {
        const ab = new ArrayBuffer(8);
...
/* ============================== utf8ToLatin1: two causes, two messages ============================== */
{
    /* invalid UTF-8 and a code point above latin1 used to share one message */
    let msg = null;
    ...
    msg = null;
    try { utf8ToLatin1(u8(0xC3, 0x28)); }          /* truncated 2-byte sequence */
    catch (e) { msg = String(e); }
    assert(msg !== null && /invalid UTF-8/.test(msg) && !/> 0xFF/.test(msg),
        "utf8ToLatin1: invalid UTF-8 is its own error (got " + msg + ")");
}

/* ============================== countUtf16: an odd byte length is ill-formed ============================== */
{
    /* Family consistency: isValidUtf16 answers FALSE for an odd byte length
     * and utf16ToUtf8 throws "byte length must be even" -- countUtf16 used to
     * be the one lenient sibling, silently counting past a truncated unit. */
...
/* ====================== ws-bytes audit regressions (2026-09) ====================== */
/* WHATWG lossy UTF-8 decode: every invalid byte becomes U+FFFD per the
 * Encoding Standard's maximal-subpart policy. The old path (JS_NewStringLen
 * raw) handed back a LONE SURROGATE for a WTF-8-encoded surrogate and
 * collapsed overlong/out-of-range sequences to a single replacement char. */
{
    const expect = (bytes, want, what) => {
        ...
        // Bytes.toUtf8 and toString share the same kernel via the handle
...
/* Bytes.slice: a start past the end is an EMPTY view (subarray parity), not
 * the engine's "invalid length" RangeError the typed-array constructor
 * produced before the clamp. */
{
...
/* free concat: a non-array is a TypeError (matching Bytes.concat and Node's
 * Buffer.concat); concat(5) used to return an empty buffer because
 * ToUint32(undefined) is 0. */
{
...
/* Bytes.alloc: the engine's ArrayBuffer ceiling is INT32_MAX; the module
 * guard now matches it so the module's own error is the one thrown. */
{
    ...
    /* NB: alloc(2^31 - 1) is also allowed but materializes a 2 GB zeroed
     * buffer, which a portable suite must not demand of every machine. */
