 ...
 * secret or on constant-time execution is `dyna:crypto`. Since P14, dyna:crypto
 * ALSO re-exports dyna:hash's unkeyed one-shots under the same names (aliases
 * over one shared C table -- a digest is the primitive every crypto user needs
 * next, and the alias set cannot drift from dyna:hash by construction). The
 * keyed operations remain one-directional: dyna:hash never exports them. This
 * file asserts that reality and then tests the four operations that only exist
