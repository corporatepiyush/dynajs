/* test_native_subclass.js --: native-module classes are subclassable.
 *
...
/* ---------- Fetcher + Crawl (dyna:scrape, DynResource; nt was dropped) ---- */
{
    ...
    /*`client` is now optional -- omitted, the Fetcher auto-builds
       its own HTTPClient, so construction SUCCEEDS. */
    const autoFe = new Fetcher({ agent: "x/1" });
    assert(autoFe instanceof Fetcher && !autoFe.closed,
        "Fetcher: construction without client succeeds (SP-3 auto-client)");
    autoFe.close();
