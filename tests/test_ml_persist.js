...
/* per-run unique scratch base: fixed /tmp names collided across runs */
...
/* ==================================================================== *
 *  6. Forged structural fields (audit batch, 2026-08): counts that fit the
 *  payload but violate what a fitted model IS, deterministically. The byte
 *  sweep above mutates without knowing the layout; these know it.
 * ==================================================================== */
{
    function repair(b) {
        const crc = CRC32C(b.subarray(0, b.length - 4)) >>> 0;
    ...
    /* payload layouts (little-endian, packed; the envelope header is 20 bytes:
     * magic 4 + version u16 + type u16 + flags u32 + payload_len u64): KMeans
     * k u64 @0; SVC degree u32 @20, n_classes u64 @56; forest n_classes u64
     * @32; a non-boosting single tree's first nn u64 @208 (192-byte forest
     * header + 2 class doubles) */
    ...
    /* gmm payload: k u64 @0 (k=0 would make predictProba divide by zero);
     * nb payload: n_features u64 @0, n_classes u64 @8 (ncl=0 loads an
     * uninitialized classes[0] that predict returns as a label) */
