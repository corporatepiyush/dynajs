...
try {
  ...
  for (const [name, pw, salt, opts] of matrix) {
      /*the default form is the PHC string; compare BOTH forms
         sync-vs-async (the string carries no NUL, so byte comparison holds) */
      ...
      check(bytesEqual(typeof TextEncoder !== "undefined" ? enc.encode(sync) : sync,
                       typeof TextEncoder !== "undefined" ? enc.encode(got) : got),
            name + ": hashAsync must equal the sync form byte for byte");
      eq(sync, got, name + ": hashAsync PHC string equals the sync form");
      const rawSync = Argon2id.hash(pw, salt, { ...(opts ?? {}), encoded: false });
      const rawGot = await Argon2id.hashAsync(pw, salt, { ...(opts ?? {}), encoded: false });
      check(bytesEqual(rawSync, rawGot), name + ": raw form also sync==async");
     ...
     land on the same bytes. */
  {
      ...
      const gotPhc = await Argon2id.hashAsync("password", salt,
          { iterations: 3, memory: 65536, parallelism: 4, hashLen: 32 });
      eq(gotPhc.startsWith("$argon2id$v=19$m=65536,t=3,p=4$c29tZXNhbHQ$"), true,
         "async PHC string carries the parameters (CY-4)");
  }

  /* ---- 3. verifyAsync: identical verdicts ----------------------------- */
  {
      const salt = enc.encode("0123456789abcdef");
      const opts = { iterations: 2, memory: 2048, parallelism: 4, encoded: false };
