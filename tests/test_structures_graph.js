...
/* --------------------------------------------------------------------- *
 * REGRESSION (structures audit): connectedComponents() on a DIRECTED
 * graph must answer WEAK components -- the connected parts of the
 * underlying undirected graph -- which is what both the d.ts and the
 * implementation's own comment promise. The old outgoing-only walk
 * answered reachability classes and split {0->1, 2->1} into two.
 * --------------------------------------------------------------------- */
{
    const dg = new Graph({ directed: true });
    dg.addNode(); dg.addNode(); dg.addNode();
    dg.addEdge(0, 1); dg.addEdge(2, 1);
    assert(eqArr(dg.connectedComponents(), [0, 0, 0]), "weak components: 0->1, 2->1 is ONE component");
    /* two weak components: {0,1,2} and {3,4}, node 5 isolated */
    const dg2 = new Graph({ directed: true });
    for (let i = 0; i < 6; i++) dg2.addNode();
    dg2.addEdge(0, 1); dg2.addEdge(2, 0); dg2.addEdge(3, 4);
    assert(eqArr(dg2.connectedComponents(), [0, 0, 0, 1, 1, 2]), "two weak comps plus an isolated node");
    /* undirected behaviour is unchanged */
    const ug = new Graph();
    for (let i = 0; i < 6; i++) ug.addNode();
    ug.addEdge(0, 1); ug.addEdge(1, 2); ug.addEdge(3, 4);
    assert(eqArr(ug.connectedComponents(), [0, 0, 0, 1, 1, 2]), "undirected behaviour unchanged");
}

/* ----------------: strict options bags ---------------- */
{
    let m = "";
    try { new Graph({ weight: true }); } catch (e) { m = e.message; }
    assert(m === 'unknown option "weight" (valid: directed, weighted)',
        "ctor bag names the key AND the valid set, got: " + m);
    m = "";
    try { new Graph({ directd: true, weighted: true }); } catch (e) { m = e.message; }
    assert(m === 'unknown option "directd" (valid: directed, weighted)',
        "ctor bag rejects a second unknown key, got: " + m);
    const g = new Graph({ directed: true, weighted: true });
    g.addNode(); g.addNode();
    assert(g.addEdge(0, 1, 2.5) === g && g.edgeCount === 1,
        "a fully valid bag is unchanged");
    assert(new Graph(null) instanceof Graph && new Graph() instanceof Graph &&
           new Graph(42) instanceof Graph,
        "absent / null / primitive bags keep the historic no-options reading");
    const sym = Symbol("s");
    assert(new Graph({ directed: true, [sym]: 1 }) instanceof Graph,
