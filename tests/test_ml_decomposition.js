...
/* --- fitTransform missing-data refusal (audit batch, 2026-08) ------------- */
{
    const p = new PCA(1);
    throws(() => p.fitTransform([[NaN, 1], [2, 3], [4, 5]]),
           "pca fitTransform rejects NaN", RangeError);
    const good = new PCA(1).fitTransform([[1, 2], [3, 4], [5, 6]]);
    assert(Array.isArray(good) && good.length === 3, "clean fitTransform still works");
}

/* --- variance-overflow refusal (2nd audit batch, 2026-08) ------------------ */
{
    throws(() => new GaussianNB().fit([[1e200, 1], [1e200, 2]], [0, 1]),
           "gaussianNB variance overflow", RangeError);
}
