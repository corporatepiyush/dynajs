/* test_mathx.js — dyna:mathx (Math additions + int/BigInt helpers).
...
import {
    ...
    gamma, lgamma, beta, betaln, gammaln, erf, erfc, cbrt, hypot, copysign, nextafter,
 ...
 * ================================================================ */
{
    ...
    /* The documented special cases for Gamma. */
    ...
    {
        const [v] = lgamma(-3); /* lgamma(-integer) = +Inf (a pole) */
 ...
 * ================================================================ */
{
    /* cbrt is a libm transcendental and ES leaves its precision
       implementation-defined, so an exact-equality assertion on a non-trivial
       cube root is a bug in the TEST, not the engine: macOS libm returns
       exactly 3 for cbrt(27) while glibc/x86-64 returns 3.0000000000000004.
       Found by tools/xplat-verify.sh -- this file had never run on x86-64.
       Exactness is asserted only where the value is exactly representable and
       the result is spec-forced (zero, and the sign symmetry). */
    assertClose(cbrt(27), 3, 1e-15, "cbrt(27) ~ 3");
    assertClose(cbrt(-27), -3, 1e-15, "cbrt(-27) ~ -3");
    assertEq(cbrt(0), 0, "cbrt(0) = 0");
    assertClose(cbrt(8), 2, 1e-15, "cbrt(8) ~ 2");
    assertEq(cbrt(-27), -cbrt(27), "cbrt is odd");
    /* same libm on both sides, so this one IS exact on every platform */
    ...
    /* C99 rule: infinity trumps NaN in either position. */
 ...
 * ================================================================ */
{
    ...
    assertEq(ilogb(-Infinity), MaxInt32, "ilogb(-Inf) = MaxInt32");
    assertEq(ilogb(NaN), MaxInt32, "ilogb(NaN) = MaxInt32, NOT the raw libc sentinel");
}

/* ================================================================ *
 *  modf: [intPart, fracPart] -- note this order, and the module's own
 ...
 * ================================================================ */
{
    ...
    }
    {
        ...
        assertNaN(fp, "modf(+Inf) fracPart = NaN, not C99's +-0");
    }
    {
        const [ip, fp] = modf(-Infinity);
        assertEq(ip, -Infinity, "modf(-Inf) intPart = -Inf");
 ...
 *  round is round-half-AWAY-from-zero; Math.round rounds a negative
 ...
 * ================================================================ */
{
    ...
    assertEq(round(-2.5), -3, "round(-2.5) = -3 (away from zero, NOT Math.round's -2)");
 ...
 * in mx_besselk_small is UB on NaN, so the core now guards it. */
{
    ...
    if (typeof besselk === "function") {
        ...
        /* Huge FINITE order: the UB guard must sit AFTER the x==0 branch —
         * K_nu(0) is +inf for every finite nu (regression: an earlier guard
         * returned 0 here). x>0 huge-nu is the infinity limit -> 0. */
        assertEq(besselk(1e300, 0), Infinity, "besselk(1e300,0) Inf");
        assertEq(besselk(-1e300, 0), Infinity, "besselk(-1e300,0) Inf");
        assertEq(besselk(1e300, 0.25), 0, "besselk(1e300,0.25) 0");
        assertEq(besselkScaled(1e300, 0.5), 0, "besselkScaled(1e300,0.5) 0");
...
/* ================================================================ *
 *  beta / betaln / gammaln: one sign-aware lgamma_r path
 * ================================================================ */
{
    /* betaln goes through the SAME sign-aware lgamma_r pieces beta uses, so
     * the log identity
     *     betaln(x,y) == gammaln(x) + gammaln(y) - gammaln(x+y)
     * must hold for negative non-integer arguments too, not just positive
     * ones. All three functions return logs of ABSOLUTE values, so the Gamma
     * signs drop out on both sides. The reference is computed from the
     * engine's own exported gammaln pieces (an independent sign-aware source
     * would be identical, since gammaln is lgamma's log|Gamma| either way). */
    for (const [x, y] of [[2.5, 1.5], [-2.5, 5.5], [-0.5, 2.5], [4.5, -1.5]]) {
        const ref = gammaln(x) + gammaln(y) - gammaln(x + y);
        const got = betaln(x, y);
        const rel = Math.abs(got - ref) / Math.abs(ref);
        assert(rel < 1e-12, `betaln(${x},${y}) == gammaln identity (rel ${rel})`);
    }

    /* x+y = -1 is a pole of Gamma(x+y): 1/Gamma(x+y) is 0, so |B| is 0 and
     * BOTH sides of the identity are -Infinity. */
    assertEq(betaln(-2.5, 1.5), -Infinity, "betaln(-2.5,1.5): Gamma(x+y) pole -> -Inf");
    assertEq(gammaln(-2.5) + gammaln(1.5) - gammaln(-1), -Infinity,
             "the identity's other side at the same pole");

    /* a non-positive integer ARGUMENT is a pole of B itself: +Infinity, the
     * same convention lgamma (and beta) already use. */
    assertEq(betaln(-1, 0.5), Infinity, "betaln(-1,0.5): pole -> +Inf");
    assertEq(beta(-1, 0.5), Infinity, "beta(-1,0.5): pole -> +Inf");

    /* beta carries the sign betaln drops: Gamma(-2.5) < 0, Gamma(5.5) > 0,
     * Gamma(3) > 0, so B(-2.5,5.5) is negative -- exactly -exp(betaln). */
    assertEq(beta(-2.5, 5.5), -Math.exp(betaln(-2.5, 5.5)),
             "beta(-2.5,5.5) === -exp(betaln(-2.5,5.5)): betaln is log|B|");

    assertClose(betaln(2, 3), Math.log(beta(2, 3)), 1e-12, "betaln(2,3) == log(beta(2,3))");
    assertNaN(betaln(NaN, 1), "betaln(NaN,1) stays NaN");
}
