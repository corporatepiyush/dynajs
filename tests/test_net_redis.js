...
/* T-CODEQL-7: the nesting cap, pinned from BOTH sides. 31 containers is the
 * deepest reply the client accepts (the scan refuses the 32nd); 300 must be
 * refused BY NAME -- a hostile peer controls the reply, not our stack. */
const srvDeepOk   = hostile(() => "*1\r\n".repeat(31) + ":7\r\n");
const srvDeep300  = hostile(() => "*1\r\n".repeat(300) + ":7\r\n");
/* T-CODEQL-7, the vector the scan cap misses: a CHAIN of consecutive
 * top-level attributes. The iterative scan's sp never passes 1 for the
 * whole chain, but the decoder recurses ONE LEVEL PER ATTRIBUTE (each
 * decorated value is the next attribute), so 50k units crashed the engine
 * with a 900KB reply. 300 units parses on an unfixed tree and must refuse
 * here -- the decode cap fires during the handshake. */
