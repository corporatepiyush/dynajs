...
/* ----: unknown option keys are rejected before any IO ---- */
{
    ...
    throwsMsgC6(() => new PostgreSQL({ host: "127.0.0.1", usr: "x" }),
                /unknown option "usr" \(valid: tls, ca, host, path, user, password, database, applicationName, port, raw, bytes, textResults, statementCacheSize, prepareAfter, bigint, insecureAuth, maxMessageBytes, maxPending, queryTimeoutMs, connectTimeoutMs\)/,
                "PostgreSQL ctor bag");
}
...
function handle(type, body, srv, opts) {
  ...
  if (type === "Q") {
    ...
    return reply(sql, [], opts, false, srv);
  }
  if (type === "P") {                       /* Parse */
  ...
  if (type === "E") {
    ...
    return reply(lastSql, srv.lastParams || [], opts, true, srv);
  }
...
function reply(sql, params, opts, extended, srv) {
  ...
  /* ----: a cursor backend. DECLARE/FETCH/CLOSE over a per-server row
   * budget, so the driver's wire shape is an oracle, not a hope. ---- */
  if (srv) {
    let m = /^DECLARE\s+(_\w+)\s+CURSOR FOR /.exec(sql);
    if (m) {
      srv.cursorLeft.set(m[1], srv.iterRows || 0);
      return msg("C", cstr("DECLARE CURSOR")) + z;
    }
    m = /^FETCH FORWARD (\d+) FROM (_\w+)/.exec(sql);
    if (m) {
      const want = +m[1], name = m[2];
      srv.fetchCount = (srv.fetchCount || 0) + 1;
      if (srv.fetchFailAt === srv.fetchCount)
        return msg("E", "S" + cstr("ERROR") + "V" + cstr("ERROR") +
                        "C" + cstr("54000") + "M" + cstr("fetch blew up") +
                        "\0") + z;
      const left = srv.cursorLeft.get(name) || 0;
      const k = Math.min(want, left);
      srv.cursorLeft.set(name, left - k);
      let b = rowDesc([{ name: "n", oid: 23 }]);
      const base = (srv.iterRows || 0) - left;
      for (let i = 0; i < k; i++) b += dataRow([String(base + i + 1)]);
      return b + msg("C", cstr("FETCH " + k)) + z;
    }
    m = /^CLOSE\s+(_\w+)/.exec(sql);
    if (m) {
      srv.cursorLeft.delete(m[1]);
      srv.closes = (srv.closes || 0) + 1;
      return msg("3", "") + msg("C", cstr("CLOSE CURSOR")) + z;
    }
    if (/^BEGIN$/.test(sql)) return msg("C", cstr("BEGIN")) + z;
    if (/^COMMIT$/.test(sql)) { srv.commits = (srv.commits || 0) + 1;
                                return msg("C", cstr("COMMIT")) + z; }
    if (/^ROLLBACK$/.test(sql)) { srv.rollbacks = (srv.rollbacks || 0) + 1;
                                  return msg("C", cstr("ROLLBACK")) + z; }
  }
...
/* ----: queryIter, the native cursor -------------------------------- */

const iterSrv = makeBackend({});
iterSrv.cursorLeft = new Map();
iterSrv.iterRows = 7;
const iterErrSrv = makeBackend({});
iterErrSrv.cursorLeft = new Map();
iterErrSrv.iterRows = 100;
iterErrSrv.fetchFailAt = 2;            /* the SECOND FETCH blows up */
const iterBrSrv = makeBackend({});
iterBrSrv.cursorLeft = new Map();
iterBrSrv.iterRows = 7;
D.it = new PostgreSQL({ port: iterSrv.port, host: "127.0.0.1", user: "u" });
D.it2 = new PostgreSQL({ port: iterErrSrv.port, host: "127.0.0.1", user: "u" });
D.it3 = new PostgreSQL({ port: iterBrSrv.port, host: "127.0.0.1", user: "u" });

rec("iterAll", (async () => {
  const out = [];
  for await (const row of D.it.queryIter("SELECT n FROM gen"))
    out.push(row.n);
  return out;
})());
rec("iterBatch", (async () => {
  const out = [];
  for await (const row of D.it.queryIter("SELECT n FROM gen", undefined, { batch: 3 }))
    out.push(row.n);
  return out;
})());
rec("iterParams", (async () => {
  const out = [];
  for await (const row of D.it.queryIter("SELECT n FROM gen WHERE x = $1",
                                         ["p"], { batch: 2 }))
    out.push(row.n);
  return out;
})());
/* r2: CONCURRENT next() calls. The contract (API.md): they queue and
 * resolve in FIFO order, one row each, WITHOUT a further arrival -- a
 * runner that finished without re-pumping parked every later op until a
 * new next() happened along (Promise.all wedged; tx stuck 'T'; exit
 * aborted on the leaked async frame). */
rec("iterConcurrent", (async () => {
  const it = D.it.queryIter("SELECT n FROM gen", undefined, { batch: 2 });
  const wave = await Promise.all([it.next(), it.next(), it.next()]);
  const okWave = wave.map((r) => r.value && r.value.n);
  const wave2 = await Promise.all([it.next(), it.next()]);
  const ok2 = wave2.map((r) => r.value && r.value.n);
  const pr = await it["return"]();          /* parked nothing: mid-run return */
  return { okWave, ok2, retDone: pr.done === true,
           tx: D.it.transactionStatus };
})());
rec("iterBreak", (async () => {
  const out = [];
  for await (const row of D.it3.queryIter("SELECT n FROM gen", undefined, { batch: 2 })) {
    out.push(row.n);
    if (out.length === 3) break;       /* return() mid-batch: row 4 is buffered */
  }
  return out;
})());
rec("iterMax", (async () => {
  const out = [];
  for await (const row of D.it2.queryIter("SELECT n FROM big", undefined,
                                          { maxRows: 5 }))
    out.push(row.n);
  return out;
})());
...
{
  let threw = null;
  try { D.it2.queryIter("SELECT 1", undefined, { batchz: 2 }); } catch (e) { threw = e; }
  check(threw instanceof TypeError &&
        /unknown option "batchz" \(valid: batch, maxRows\)/.test(threw.message),
        "queryIter opts bag is strict (got " + threw + ")");
  /* laziness: a stream on a CLOSED connection constructs, then the first
   * next() rejects. A throwaway connection: the streams under test must
   * not share it. */
  const deadConn = new PostgreSQL({ port: iterSrv.port, host: "127.0.0.1",
                                    user: "u" });
  deadConn.close();
  const dead = deadConn.queryIter("SELECT 1");
  let nextErr = null, syncThrew = null;
  try { dead.next().then(() => {}, (e) => { nextErr = e; }); }
  catch (e) { syncThrew = e; }
  check(syncThrew === null,
        "queryIter() itself stays lazy on a closed connection (got " + syncThrew + ")");
  rec("iterClosedOk", new Promise((res) => setTimeout(() => res(nextErr !== null), 60)));
}
...
const t = setInterval(() => {
  ...
  // C9: tls:true is accepted at construction (TLS runs on the aio engine);
  // the connection itself now proceeds to the TLS handshake
  ...
  setTimeout(() => {
    ...
    {
      const v = (k) => val(k);
      check(JSON.stringify(v("iterAll")) === "[1,2,3,4,5,6,7]",
            "queryIter streams all rows in order, got " + JSON.stringify(v("iterAll")));
      check(JSON.stringify(v("iterBatch")) === "[1,2,3,4,5,6,7]",
            "batch 3 still streams all 7 in order, got " + JSON.stringify(v("iterBatch")));
      check(JSON.stringify(v("iterParams")) === "[1,2,3,4,5,6,7]",
            "params bind through DECLARE, got " + JSON.stringify(v("iterParams")));
      check(JSON.stringify(v("iterBreak")) === "[1,2,3]",
            "a mid-batch break yields exactly the rows before it, got " +
            JSON.stringify(v("iterBreak")));
      check(JSON.stringify(v("iterMax")) === "[1,2,3,4,5]",
            "maxRows caps the stream, got " + JSON.stringify(v("iterMax")));
      check(err("iterErr") !== undefined,
            "a FETCH that errors rejects the iteration, got " + JSON.stringify(v("iterErr")));
      check(val("iterClosedOk") === true, "the closed-connection laziness check ran");
      {
        const c = val("iterConcurrent");
        check(JSON.stringify(c && c.okWave) === "[1,2,3]",
              "three concurrent next()s resolve FIFO, one row each, got " +
              JSON.stringify(c && c.okWave));
        check(JSON.stringify(c && c.ok2) === "[4,5]",
              "the second wave continues the stream without a wedge, got " +
              JSON.stringify(c && c.ok2));
        check(c && c.retDone === true && c.tx === "I",
              "return() after concurrent nexts cleans up (tx " + (c && c.tx) + ")");
      }
      const iterSqls = iterSrv.queries.map((q) => q.sql);
      check(iterSqls[0] === "BEGIN", "the stream opens with BEGIN, got " + iterSqls[0]);
      check(iterSqls.some((q) => /^DECLARE _djq\d+ CURSOR FOR SELECT n FROM gen$/.test(q)),
            "each stream DECLAREs <name> CURSOR FOR <sql>");
      check(iterSrv.closes === 4,
            "every finished stream CLOSEd its cursor (" + iterSrv.closes + ")");
      check(iterSrv.commits >= 3 && (iterSrv.rollbacks || 0) === 0,
            "clean streams (including a break) COMMIT (" + iterSrv.commits +
            " commits, " + iterSrv.rollbacks + " rollbacks)");
      check(iterErrSrv.rollbacks >= 1,
            "a failed stream ROLLBACKs (" + iterErrSrv.rollbacks + ")");
      /* THE BREAK CASE, in full: its own backend, so the wire sequence is
       * an exact oracle -- BEGIN, DECLARE, FETCH 2 (rows 1-2), FETCH 2
       * (rows 3-4; 3 delivered, 4 still buffered), and the break drops the
       * buffer, CLOSEs the cursor and COMMITs. A regression here used to
       * emit the buffered row from return() and strand the transaction
       * open (caught by the live probe, not by a weak CLOSE-count). */
      {
        const br = iterBrSrv.queries.map((q) => q.sql);
        const want = [
          "BEGIN",
          /^DECLARE _djq\d+ CURSOR FOR SELECT n FROM gen$/,
          /^FETCH FORWARD 2 FROM _djq\d+$/, /^FETCH FORWARD 2 FROM _djq\d+$/,
          /^CLOSE _djq\d+$/, "COMMIT",
        ];
        let okseq = br.length === want.length;
        for (let i = 0; okseq && i < want.length; i++)
          okseq = want[i] instanceof RegExp ? want[i].test(br[i]) : br[i] === want[i];
        check(okseq, "a break mid-batch drives BEGIN/DECLARE/FETCH/FETCH/CLOSE/COMMIT, got " +
              JSON.stringify(br));
        check((iterBrSrv.rollbacks || 0) === 0,
              "a CLEAN break COMMITs, it does not ROLLBACK");
      }
    }
    ...
    setTimeout(() => {
      ...
      iterSrv.close(); iterErrSrv.close(); iterBrSrv.close();
