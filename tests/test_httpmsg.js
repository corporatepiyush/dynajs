...
/*the opts bag is strict -- an unknown key names itself and the valid set */
{
    let m = null;
    try { CookieSerialize("a", "1", { maxAgee: 5 }); } catch (e) { m = e; }
    assert(m !== null && m instanceof TypeError &&
           /unknown option "maxAgee" \(valid: maxAge, domain, path, sameSite, secure, httpOnly\)/.test(m.message),
           "cookie bag names key + valid set (got " + (m && m.message) + ")");
}
eq(CookieSerialize("a", "1", { maxAge: undefined, secure: true }), "a=1; Secure",
   "undefined keys are absent; valid bag unchanged");
...
/* ---------------------------------------------- media type hygiene (W-HTTP) */

/* RFC 9110 8.3.1: only ";"-introduced parameters may follow the subtype. */
assert(ContentTypeParse("text/plain JUNK") === null,
       "trailing junk after the subtype is not a parameter");
assert(ContentTypeParse("text/plain ; a=1").parameters.a === "1",
       "OWS before the parameter semicolon is tolerated");
/* A CTL in a quoted parameter value is not qdtext; the parser refuses so the
 * value can never re-enter a header through ContentTypeFormat. */
assert(ContentTypeParse('text/plain; a="x\r\nSet-Cookie: 1"') === null,
       "a CRLF in a quoted parameter is refused (CWE-113)");
/* ContentTypeFormat is the encode side of the same boundary. */
throws(() => ContentTypeFormat({ type: "text", subtype: "plain\r\nX-Evil: 1",
                                 parameters: {} }),
       "a CRLF in the subtype is refused");
throws(() => ContentTypeFormat({ type: "text", subtype: "plain",
                                 parameters: { a: "x\r\nSet-Cookie: p=1" } }),
       "a CRLF in a parameter value is refused");
eq(ContentTypeFormat({ type: "text", subtype: "plain", parameters: { a: "x y" } }),
   'text/plain; a="x y"', "non-token parameter values still quote");

/* An empty etag matches nothing -- and is not matched by an empty (malformed)
 * header ELEMENT, whose 0-byte compare used to succeed. */
assert(ETagMatch(",", "") === false, "an empty etag matches nothing");
assert(ETagMatch('"a"', "") === false, "an empty etag argument matches nothing");
