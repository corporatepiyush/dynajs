...
async function spawnChild(scriptBody) {
    ...
    sh(`./dynajs --std ${js} >${T}/${name}.log 2>&1 &`);
    let port = 0;
...
{
    /* C8: wss:// is now SUPPORTED (TLS handshake on the connect pool, steady
       state through the aio TLS engine). The constructor must NOT throw. */
    ok(true, "wss:// accepted at construction (C8)");
        ...
        settle into a freed husk without use-after-free ---- */
{
    ...
    const s = new TCPServer({ port: 0 });
    s.start({
        connect: (c) => { peer = c; },   /* accept and stall: the upgrade never answers */
        data: () => {},
        close: () => {},
    });
    const got = { closed: null };
    ...
    /* The handshake job is still parked on the stalled socket, and nothing
       JS-visible marks its end -- the close() settled synchronously from
       close(). Wake the worker (drop the accepted socket) and give the loop
       the moment to run its completion before the hard std.exit(0) at the
       end of this file: exiting mid-handshake abandons the engine's
       job/handshake/client allocations (LSan: dyn_aio_offload <- dyn_wsc_ctor),
       which is the suite discarding live engine state, not the engine
       leaking it. */
    if (peer) peer.close();
    s.close();
    await wait(250);
