...
/* ---- ctor and setter are ONE validator (audit X-2) ----------------------
 * The setter `L.level = v` throws on every non-level value; the ctor used to
 * silently fall back to "info" for the same values (42, null, a throwing
 * object) while string levels threw. A logger whose level was silently
 * downgraded is worse than a constructor error. */
{
    const levelThrower = { toString() { throw new Error("boom-level"); } };
    let msg = "";

    /* non-level values are refused by the ctor, exactly like the setter */
    msg = "";
    ...
    /* a throwing getter/to-string propagates the ORIGINAL error, ctor AND setter */
    msg = "";
    ...
    /* the ctor validates EXACTLY like the setter: a coercing object is
       accepted by both (the setter defines this, not us) */
    ...
    /* the deliberate cases are untouched */
...
{
    /* buffer: a throwing getter used to be IGNORED silently; a wrong-typed
       value used to fall through to "no batching" while the caller believed
       output was batched. Same rule as the level setter: the conversion's
       own throw propagates with the ORIGINAL error; a value that converts
       but is out of range is refused. */
    let msg = "";
    ...
    /* a coercing object is accepted, exactly like the setter accepts one for level */
...
/* -------------------------------------------: strict options bags ---
 * An unknown key is a TypeError naming the key AND the valid set; valid
 * bags are unchanged. */
...
{
    /* ctor bag: bogus key + exact full message */
    ...
    /* child opts bag (fields arg is NOT a bag and stays open) */
    ...
    /* non-object bags keep the historic reading; symbols are not options */
