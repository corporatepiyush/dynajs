...
int main(void)
{
     ...
     * The 88-byte job ctx dropped by the documented teardown contract (the
     * pool channel is freed with the job undelivered) is EXPECTED here: this
     * case exists for the hang/UAF coverage, so the known drop is scoped out
     * of LeakSanitizer rather than avoided -- suppressing the whole binary
     * would hide real regressions. Do NOT fork this case: the pool's workers
     * do not survive fork, so teardown in a child deadlocks on a condvar no
     * one can signal -- an artifact of fork, not of the code under test. */
    g_calls = 0;
#if defined(__SANITIZE_ADDRESS__) || \
    (defined(__has_feature) && __has_feature(address_sanitizer))
    __lsan_disable();
