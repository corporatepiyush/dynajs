 ...
 * ==================================================================== *
 * A CSR cannot CARRY a non-finite value -- both constructors refuse one (the
 * symmetry is pinned in test_ml_fit_parity) -- and a finite-valued row whose
 * READING is non-finite (a duplicate-sum overflow) is refused by the fit
 * exactly as the dense form refuses its cells. NaN in y is refused on the CSR
 * path like on the dense one. */
{
    ...
    /* overflow-class X: every stored value finite, the reading infinite */
    ...
    /* a clean CSR fit is now the dense fit BIT-FOR-BIT (the fit reads rows
     * through their summed reading with the dense arm's rounding; pinned
     * bitwise in test_ml_fit_parity) -- the tolerance here is belt */
