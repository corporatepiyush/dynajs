...
try {
    ...
    /* --- overwrite-onto-self refuses WITHOUT destroying the source ---
       The old overwrite arm opened the destination with O_TRUNC: for
       from === to that emptied the shared inode before the copy read a
       byte, destroying the file while reporting the stale source size. */
