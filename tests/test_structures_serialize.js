...
/* --------------------------------------------------------------------- *
 * REGRESSION (structures audit): IntervalTree.deserialize dropped POINT
 * spans. The reader shared RangeMap's `hi > lo` guard, but a RangeMap
 * cannot hold an empty span (put() no-ops it) while insert(5, 5, v)
 * stores [5,5] and queries match it -- so the guard silently discarded
 * every point interval a live tree legally contained (size 2 -> 1).
 * --------------------------------------------------------------------- */
{
