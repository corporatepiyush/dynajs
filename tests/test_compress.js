...
/* --- 7. a mistyped gzip level is refused, not silently ignored --- */
{
    ...
    /*an OPTIONS object is now a legal second argument ({ level }).
     * The validation is shared with the positional form, so a non-number
     * level inside the object is still refused by name. */
...
/* --- 5b. RFC 1952 framing: members, padding, FHCRC ----------------------- */
{
    function throws(fn, msg) {
        let threw = false;
        try { fn(); } catch { threw = true; }
    ...
    /* RFC 1952: "a gzip file consists of a series of members". `cat a.gz b.gz`
       is one file, and a reader that stops after the first member silently
       returns a PREFIX of the data. */
    {
    ...
    /* Trailing all-zero padding (files padded to a block) is tolerated. */
    {
    ...
    /* Any other trailing junk is a malformed file, not something to ignore. */
    throws(() => gunzip(new Uint8Array([...gzip(corpora.small), ...enc.encode("junk")])),
           "trailing non-member garbage throws");
    /* FHCRC: the header's own CRC-16 (RFC 1952). Built here with an
       independent bit-serial CRC-32 so the test does not consult the module. */
    {
        ...
        hdr.set([0x1f, 0x8b, 0x08, 0x02, 0, 0, 0, 0, 0, 0xff]); /* FLG=FHCRC */
        ...
        bad[6] ^= 0x01;                     /* corrupt an FNAME-region byte */
        throws(() => gunzip(bad), "a corrupted FHCRC header throws");
        const badhc = member.slice();
        badhc[10] ^= 0x01;                  /* corrupt the HCRC field itself */
        throws(() => gunzip(badhc), "a wrong FHCRC value throws");
    }
}

/* ---------- compressBound: preallocate the output buffer ---------- */
{
    ...
    // the formulas are the encoders' own reserved bounds, exact here:
    ...
    // THE actual guarantee: every codec's output on random and incompressible
    // input fits its own bound
...
/* ---------- gzip options overload ---------- */
{
    ...
    // both arities agree byte-for-byte
    const a = gzip(msg, 9), b = gzip(msg, { level: 9 });
    ok(a.length === b.length && a.join() === b.join(), "positional == options output");
    // validation is shared: a mistyped level is a named TypeError either way
...
/* ----------: unknown option keys are rejected ---------- */
{
    ...
    // each bag names the key AND the full valid set
...
/* ---------- loop 2: adversarial bags ---------- */
{
    ...
    // Proxy ownKeys trap throw propagates untouched
    ...
    // symbol keys are invisible (own enumerable STRING keys only)
    const symBag = {};
    ...
    assert(gunzip(data, Object.create({ asStrng: true })) instanceof Uint8Array,
           "inherited keys invisible");
    // own NON-enumerable keys invisible
    const ne = {};
    ...
    // primitive bag counts as absent (pilot convention)
