// GENERATED probe (df_core) -- do not edit; run materialize.sh
...
var df = new DataFrame({}).FROM_RECORDS([{a:1,b:"x"},{a:2},{b:"y",a:3}]);
assert_eq(df.COLUMNS.join(","), "a,b", "FROM_RECORDS union order");
assert_eq(df.ROWS, 3, "FROM_RECORDS rows");
var rec = df.TO_RECORDS();
assert_eq(rec[1].b, "", "missing string key fills empty");
assert_true(isNaN(rec[1].a) === false && rec[1].a === 2, "present keys keep values");
var rec2 = new DataFrame({}).FROM_RECORDS([{a:1},{b:2}]).TO_RECORDS();
assert_true(isNaN(rec2[0].b) || rec2[0].b === undefined, "missing numeric fills NaN-ish");
var buf = new Float64Array([10, 20, 30]);
var dz = new DataFrame({ id: new Int32Array([1,2,3]), v: buf, tag: ["x","y","x"] });
buf[0] = 99;
assert_eq(dz.TO_COLUMNS().v[0], 99, "numeric column aliases the buffer (live)");
var dc = new DataFrame({ id: new Int32Array([1,2,3]), v: new Float64Array([3.5,1,NaN]), s: ["plain","has,comma",'has"quote'] });
var csv = dc.TO_CSV();
assert_eq(csv.split("\n")[0], "id,v,s", "csv header");
assert_true(csv.indexOf('"has,comma"') >= 0, "csv quotes commas");
assert_true(csv.indexOf('"has""quote"') >= 0, "csv escapes quotes per RFC4180");
var sl = dz.SLICE(-2);
assert_eq(sl.ROWS, 2, "slice(-2) rows");
assert_arr_eq(Array.from(sl.TO_COLUMNS().id), [2,3], "slice(-2) values");
assert_eq(dz.SLICE(1,3).ROWS, 2, "slice(1,3)");
assert_eq(dz.SLICE(5,10).ROWS, 0, "slice clamps");
var dm = dz.MASK(new Uint8Array([1,0,1]));
assert_true(isNaN(dm.TO_COLUMNS().v[1]), "MASK default fill numeric NaN");
assert_eq(dm.TO_COLUMNS().tag[1], "", "MASK default fill string empty");
var dm0 = dz.MASK(new Uint8Array([1,0,1]), 0);
assert_eq(dm0.TO_COLUMNS().v[1], 0, "MASK explicit fill 0");
var s1 = dz.SAMPLE(2, 42), s2 = dz.SAMPLE(2, 42);
assert_arr_eq(Array.from(s1.TO_COLUMNS().id), Array.from(s2.TO_COLUMNS().id), "sample seeded reproducible");
var s3 = dz.SAMPLE(3, 7); assert_eq(new Set(Array.from(s3.TO_COLUMNS().id)).size, 3, "sample without replacement");
assert_throws(function () { dz.SAMPLE(4, 1); }, null, "sample n > ROWS refused");
assert_throws(function () { new DataFrame({ a: new Int32Array([1]), b: new Int32Array([1,2]) }); }, null, "length mismatch refused");
assert_throws(function () { dz.SELECT(["id", "id"]); }, null, "duplicate select refused");
assert_throws(function () { dz.RENAME({ nope: "x" }); }, null, "unknown rename refused");
assert_throws(function () { dz.RENAME({ id: "tag" }); }, null, "colliding rename refused");
assert_eq(dz.DROP_COLUMNS(["tag"]).COLUMNS.join(","), "id,v", "drop columns");
assert_eq(dz.RENAME({ id: "key" }).COLUMNS.join(","), "key,v,tag", "rename");
var dt = dz.DTYPES();
assert_eq(JSON.stringify(dt), '{"id":"i32","v":"f64","tag":"str"}', "dtypes");
var dj = new DataFrame({ x: new Float64Array([1, NaN]) });
assert_eq(dj.TO_JSON(), '[{"x":1},{"x":null}]', "TO_JSON NaN -> null");
summary(__TAG); // 32 cases
