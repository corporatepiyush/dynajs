...
__EXP = null;
test("rp42_default_param.js per-engine behavior contract", function () {
    var __SRC = "const OP = { HALT: 1 };\nfunction f(x = OP.HALT, OP) { return x + (OP ? OP.HALT : 0); }\nconsole.log(f(5, { HALT: 2 }), f(5, { HALT: 3 }), f(undefined, { HALT: 4 }));\n";
    ...
    if (__ENGINE === 'dynajs') {
        ...
        assert_eq(__e && __e.message, "OP is not initialized", "dynajs error message");
    ...
    if (__ENGINE !== 'dynajs') {
        ...
        assert_eq(__e && __e.message, "Cannot access 'OP' before initialization", "node error message");
