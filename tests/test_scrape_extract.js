...
import { Extractor, Robots } from "dyna:scrape";
...
/*unknown option keys are rejected, naming the key and the valid set */
function throwsMsgSC(fn, re, msg) {
    let threw = null;
    try { fn(); } catch (e) { threw = e; }
    ok(threw !== null && threw instanceof TypeError && re.test(threw.message),
       msg + " (got " + (threw ? threw.name + ": " + threw.message : "no throw") + ")");
}
throwsMsgSC(() => new Extractor({ t: { sel: new Selector("h1"), al: true } }),
            /^unknown option "al" \(valid: sel, attr, all, required, trim, source, default, as\)$/,
            "field spec bag");
throwsMsgSC(() => new Extractor({ t: { sel: new Selector("h1") } }, { txt: HTMLText }),
            /unknown option "txt" \(valid: text\)/, "Extractor ctor bag");
throwsMsgSC(() => new Extractor({ t: { sel: new Selector("h1") } }).run(doc, { baz: "/x" }),
            /unknown option "baz" \(valid: base\)/, "run bag");
throwsMsgSC(() => new Robots("User-agent: *\nDisallow: /", { agnt: "b" }),
            /unknown option "agnt" \(valid: agent\)/, "Robots bag");
/* valid bags unchanged */
ok(new Extractor({ t: { sel: new Selector("h1") }, u: { sel: new Selector("a"), attr: "href", all: true } },
                 { text: HTMLText }).run(doc).ok === true, "valid field + ctor bags still extract");
