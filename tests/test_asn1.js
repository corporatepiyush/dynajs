...
}
{
    const refused = [
        ...
        ["02" + "41" + "80".repeat(65), /64-byte cap/,
         "an INTEGER beyond the 64-byte cap is refused (SZ-1)"],
...
/* --------------------------------------- audit regressions (S6, S7, S8) */

{
    /* S6: a PRIMITIVE SEQUENCE/SET is not DER (X.690 8.9.1/8.12.1) -- the
     * decoder used to fall into the children loop and parse the content
     * octets as TLVs. */
    throws(() => ASN1.decode(bytes("100100")),
           "a primitive SEQUENCE refuses as BER");
    throws(() => ASN1.decode(bytes("1100")),
           "a primitive SET refuses too");
    eq(ASN1.decode(bytes("3002" + "0500").slice(0, 4)).tag, 16,
       "a constructed SEQUENCE still decodes");
}
{
    /* S7, superseded by: JS_ToBigInt64 still WRAPS modulo 2^64, but a
     * value past int64 now takes the exact decimal path instead of trusting
     * the wrap, so the contract is exact round-trip, not refusal. */
    assert(ASN1.decode(ASN1.encode(ASN1.int(12345678901234567890n))).value
               === 12345678901234567890n,
           "a BigInt beyond int64 round-trips exactly (SZ-1), never wrapped");
    assert(ASN1.decode(ASN1.encode(ASN1.int(123456789012345678n))).value
               === 123456789012345678n,
           "an in-range BigInt round trips exactly");
}
{
    /* S8: constructed:true on a primitive-only universal type used to be
     * silently ignored and encoded primitive. */
    throws(() => ASN1.encode({ cls: 0, tag: 4, constructed: true,
                               value: new Uint8Array([1]) }),
           "constructed OCTET STRING refuses instead of dropping the flag");
    /* high-tag boundary pins (the audit's byte battery) */
    throws(() => ASN1.decode(bytes("9fffffffff7f00")),
           "a tag number over 2^31-1 refuses");
    eq(ASN1.decode(bytes("9f876800")).tag, 1000,
       "a minimal high-tag number decodes");
}
