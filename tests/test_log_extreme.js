 ...
 * but framed. */
{
    ...
    /* each cut line is still ONE parseable record -- the repair pass closes
       the frame whatever byte the cut landed on */
    for (const l of ls) {
        let ok = true;
        try { JSON.parse(l); } catch (e) { ok = false; }
        assert(ok, "cut huge line STILL parses: tail=" + JSON.stringify(l.slice(-30)));
    }
}

/* The 64 KiB cap can cut ANY byte: sweep padding so the cut walks across the
 * structural region after a big field, and cut mid-KEY, inside a nested
 * object, inside an array, and at the `}\n` close itself. Every line must
 * parse and the line count must equal the emit count (a dropped newline
 * would fuse two records). */
{
    const log = new Logger({ dest: P("worst/cutsweep.log").toString(), timestamp: false });
    for (let p = 65470; p < 65520; p++)
        log.info({ a: "x".repeat(p), b: "y", c: "z" }, "sweep");
    {
        const o = {};
        o["k".repeat(70 * 1024)] = 1;
        log.info(o, "long key");
    }
    log.info({ outer: { inner: "z".repeat(100 * 1024) } }, "nested cut");
    log.info({ arr: new Array(1000).fill("y".repeat(80)) }, "array cut");
    log.info("\u0001".repeat(100 * 1024), "escape cut");
    log.flush();
    const ls = lines("worst/cutsweep.log");
    eq(ls.length, 50 + 4, "one line per emit: no lost newlines, no fusing");
    for (let i = 0; i < ls.length; i++) {
        let ok = true;
        try { JSON.parse(ls[i]); } catch (e) { ok = false; }
        assert(ok, "cut line " + i + " parses: tail=" + JSON.stringify(ls[i].slice(-40)));
    }
}
