...
/*a top-level symlink key is refused (it belongs INSIDE rollover --
 * it used to be silently ignored here, which is why this block exists).
 * The file-shape guarantee is kept: without a rollover the dest is a
 * regular file. */
{
    let m = "";
...
/* "Rotate at this size" counts the INCOMING line: a file never ends up over
 * the cap -- the line that would overflow opens the next file whole. (A
 * single line bigger than the cap still lands whole in its own file; a log
 * line is never split across files.) */
{
    ...
    for (let i = 0; i < 12; i++)
        log.info("r".repeat(580));          /* ~600-byte records */
    log.info("Z".repeat(2000));             /* one oversized line, lands whole */
    log.flush();
