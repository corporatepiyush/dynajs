...
/* A header NAME must be an RFC 9110 token: the proxy re-emits it verbatim into
   the upstream request, and "X(Bad)" passed the head validation (which only
   bars CTL and SP inside the name) and reached the backend as header text no
   conforming parser agrees on. The proxy refuses it with a 400. Parentheses
   are the point: a SPACED name is already refused by the head validation, so
   it would make this assertion pass for the wrong reason. */
let badReply = "";
live.push(TCPServer.connect({ host: "127.0.0.1", port: app.port }, {
    connect: (c, e) => {
        if (!e) c.write(bytes("GET /api/thing HTTP/1.1\r\n" +
                              "Host: x\r\n" +
                              "X(Bad): 1\r\n" +
                              "Content-Length: 0\r\n\r\n"));
    },
    data: (c, b) => { badReply += str(b); },
}));
...
setTimeout(() => {
    ...
    /* a non-token header NAME is refused client-side and never forwarded */
    ok(badReply.indexOf("HTTP/1.1 400") === 0,
       "a header name that is not a token is a 400 (got " +
       JSON.stringify(badReply.slice(0, 40)) + ")");
    ok(seen.indexOf("X(Bad)") < 0,
       "the non-token name never reached the backend");
