...
/* --- frame header fields the writer does not emit, the reader must police --
 *
 * Both crafted frames use STORED blocks (high bit in the size field), so the
 * test needs no compressor. The XXH32 below is a literal reimplementation of
 * the format's checksum, so the header checksum byte is computed, not
 * guessed. */
{
    function xxh32(bytes, seed = 0) {
        const P1 = 2654435761, P2 = 2246822519, P3 = 3266489917,
              P4 = 668265263, P5 = 374761393;
        /* tail-4 round: h += lane * P3; h = rotl(h,17) * P4 (spec v0.7.3) */
        const mul = (a, b) => Math.imul(a, b) >>> 0;
        const rotl = (x, r) => ((x << r) | (x >>> (32 - r))) >>> 0;
    ...
    /* magic + FLG + BD + optional 8B content size + optional dictID + HC */
    ...
    /* A frame that DECLARES its content size is lying or truncated when the
       decoded bytes disagree. The field used to be read and dropped. */
    {
    ...
    /* The per-block checksum is XXH32 over the block's bytes AS STORED. A
       corrupted block used to pass whenever no content checksum followed. */
    {
        ...
        bad[bad.length - 5] ^= 0xff;        /* the block checksum's last byte */
        let threw = false;
        try { lz4Unframe(bad); } catch (e) { threw = true; }
