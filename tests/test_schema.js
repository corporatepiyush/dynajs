...
/* ------------------------------------- equality runs no getters (own only) */

{
    let count = 0;
    const inst = {};
    Object.defineProperty(inst, "x", { get() { count++; return 1; }, enumerable: true });
    no(Schema.validate({ const: { x: 1 } }, inst),
       "an accessor compares as undefined: the getter is never invoked");
    assert(count === 0, "const equality must not run a getter (ran " + count + " times)");
}

/* --------------------------- anyOf/oneOf run each failing branch ONCE */

{
    let count = 0;
    const inst = [1];
    Object.defineProperty(inst, 0, { get() { count++; return 1; }, enumerable: true });
    const r = Schema.validate({ anyOf: [{ const: [2] }, { const: [3] }] }, inst);
    no(r, "anyOf with no matching branch fails");
    assert(r.errors.length === 1 && r.errors[0].keyword === "const",
           "the best branch's errors surface exactly once");
    assert(count === 2,
           "each branch evaluates the value once; the best branch is not "
           + "re-run (ran " + count + " times)");
}
{
    const r = Schema.validate(
        { anyOf: [{ type: "number", minimum: 100 }, { type: "array" }] }, 42);
    no(r, "anyOf failure keeps the best branch's error");
    assert(r.errors.length === 1 && r.errors[0].keyword === "minimum",
           "the tie goes to the first-best branch and its errors only");
}

/* ------------------------------------------- pattern source byte cap */

{
    const big = "a".repeat(64 * 1024 + 1);
    throws(() => Schema.compile({ type: "string", pattern: big }),
           "a pattern source over 64 KiB is refused at compile time");
    throws(() => Schema.compile({ type: "object",
                                  patternProperties: { [big]: {} } }),
           "patternProperties go through the same cap");
    ok(Schema.compile({ type: "string", pattern: "a+" }).validate("aaa").valid,
       "a normal pattern still compiles");
}

/* ----------------- cache hash failure degrades to uncached validation */

{
    /* A function has no canonical encoding, so the cache fingerprint refuses
       to build -- with an exception pending. That must degrade to a plain
       uncached validate, not leak the exception into the result. */
    const sch = { type: "number", describe() {} };
    assert(Schema.validate(sch, 42).valid === true,
           "a schema the hasher refuses still validates");
    assert(Schema.validate(sch, "x").valid === false,
           "and it still enforces its rules");
}
