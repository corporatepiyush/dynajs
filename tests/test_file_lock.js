...
/* Regression (audit F1): fn may close the lock itself. withLock must release
 * at most once -- the second dispose of an already-freed native was a double
 * free (the suite aborted with heap corruption before the fix). */
{
...
/* A read-only file must still be lockable: flock(2) locks the open file
 * description, not the access mode, so withLock falls back to an O_RDONLY
 * descriptor when O_RDWR is refused (audit F7). */
{
    const RO = T + "/ro.txt";
