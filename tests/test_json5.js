...
/* \1..\9 are not JSON5 escapes (DecimalDigit is an EscapeCharacter), and \0
   must not be followed by a digit. Both were silently accepted before. */
throws(() => JSON5Parse("'\\1'"), "\\1 is refused");
throws(() => JSON5Parse("'\\9'"), "\\9 is refused");
throws(() => JSON5Parse("'\\01'"), "\\0 followed by a digit is refused");
eq(JSON5Parse("'\\0'"), "\0", "lone \\0 still decodes to NUL");

/* Unicode whitespace (NBSP, ZWNBSP, Zs) is JSON5 whitespace per the spec. */
eq(JSON5Parse("\u00a01"), 1, "NBSP is whitespace");
eq(JSON5Parse("\ufeff1"), 1, "ZWNBSP is whitespace");
eq(JSON5Parse("\u20031"), 1, "U+2003 (Zs) is whitespace");
eq(JSON5Parse("\u202f1"), 1, "U+202F is whitespace");
eq(JSON5Parse("\u30001"), 1, "U+3000 is whitespace");
eq(JSON5Parse("1\u00a0"), 1, "trailing NBSP is whitespace");

/* BigInt throws (JSON.stringify parity) instead of silently serialising null. */
throws(() => StableStringify(10n), "StableStringify rejects BigInt");
throws(() => JSON5Stringify(10n), "JSON5Stringify rejects BigInt");
eq(StableStringify({ a: 1 }), '{"a":1}', "stringify still works after the BigInt guard");

/* function-valued members are omitted, as JSON.stringify does. */
eq(StableStringify({ f: function () {}, g: 1 }), '{"g":1}', "function member omitted");
eq(JSON5Stringify({ f: function () {}, g: 1 }), '{"g":1}', "function member omitted (json5)");
eq(StableStringify([function () {}]), "[null]", "function in array is null");
