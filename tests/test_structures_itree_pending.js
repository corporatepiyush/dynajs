...
/* --------------------------------------------------------------------- *
 * REGRESSION (structures audit): a query whose hi is below the smallest
 * built lo must still report PENDING-tail matches. The old descent
 * computed k (built intervals with lo <= hi) and returned 0 at k == 0,
 * discarding the tail scan's results -- a built [100,110] plus a pending
 * [1,2] answered overlapping(0,5) with [] instead of [[1,2,"b"]].
 * --------------------------------------------------------------------- */
{
    const t = new IntervalTree();
    t.insert(100, 110, "built");
    t.overlapping(0, 200);              /* forces the index build */
    t.insert(1, 2, "pending");          /* sits in the unindexed tail */
    const hits = t.overlapping(0, 5);
    check(hits.length === 1 && hits[0][0] === 1 && hits[0][1] === 2 && hits[0][2] === "pending",
          "tail-only query with k == 0 -- got " + JSON.stringify(hits));
    const t2 = new IntervalTree();
    t2.insert(500, 600, "big");
    t2.overlapping(0, 1000);
    t2.insert(1, 2, "x");
    t2.insert(3, 4, "y");
    check(t2.overlapping(0, 5).length === 2,
          "several tail matches with k == 0 -- got " + t2.overlapping(0, 5).length);
    check(t2.at(3).length === 1, "at() on the tail path with k == 0");
}
