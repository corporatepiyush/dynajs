/* rss_plain_classes.js — peak-RSS-plateau leak proof for GC-reclaimed natives.
 *
 * Random has no close(): its lifetime is the GC's. Hasher carries the
 * DynResource surface since X-10 but dropping it UN-closed must still release
 * the native state through the GC finalizer (the close path is pinned in
 * test_hasher_lifecycle.js). LeakSanitizer is
