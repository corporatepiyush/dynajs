...
/* ==================== parse: valid table (exact fields) ==================== */
{
    ...
    for (const [inp, ma, mi, pa, pre, build, ver] of T) {
        ...
        /* prerelease(v) -> null when there is no prerelease (API contract),
         * the parsed identifiers otherwise */
        if (pre.length === 0)
            assert(sv.prerelease(inp) === null, "prerelease() null " + inp);
        else
...
/* ==================== parse: invalid table ==================== */
{
    const bad = [
        ...
        /* "v1.2.3" moved to the valid table: parse allows one leading v
         * (API contract, consistent with clean() and range comparators);
         * NOTE: "1.2.3 " / " 1.2.3" are VALID since the node-parity trim
         * (ECMA-262 whitespace skipped around the version) -- see the
         * whitespace table below the bad list */
        "=1.2.3",
        ...
        "1.2.3-99999999999999999999",     /* numeric prerelease id > MAX_SAFE */
        "1.2.3-9007199254740992",         /* ... by exactly one */
    ];
    for (const s of bad) {
    ...
    /* node-parity: parse skips leading/trailing ECMA-262 whitespace
     * (node-semver parses String.prototype.trim()'ed input) */
    const padded = [" 1.2.3 ", "\t1.2.3", "1.2.3\n", "\f1.2.3",
        "\u00a0v1.2.3", "1.2.3\u3000", "  v0.0.0  ", " 1.0.0-alpha.1+b "];
    for (const s of padded) {
        assert(sv.isValid(s) === true, "isValid trims " + JSON.stringify(s));
        assert(sv.major(s) === sv.major(sv.clean(s)),
            "major on trimmed " + JSON.stringify(s));
    }
    /* build metadata is opaque (sec.10): a huge numeric BUILD id is fine --
     * only PRERELEASE numeric ids are numbers capped at MAX_SAFE */
    assert(sv.isValid("1.2.3+99999999999999999999") === true,
        "huge numeric BUILD id is valid (opaque)");
    throws(() => sv.parse("1.2.3-99999999999999999999"),
        "huge numeric PRERELEASE id still rejected (MAX_SAFE cap)");
    /* node parity: a numeric prerelease id equal to MAX_SAFE stays a STRING
     * in the prerelease array (node: "num < MAX_SAFE_INTEGER" is a Number) */
    assert(sv.prerelease("1.0.0-9007199254740991")[0] === "9007199254740991",
        "MAX_SAFE prerelease id stays a string");
    assert(sv.prerelease("1.0.0-9007199254740990")[0] === 9007199254740990,
        "below MAX_SAFE prerelease id is a number");
    assert(JSON.stringify(sv.parse("1.0.0-9007199254740991").prerelease) ===
        '["9007199254740991"]', "parse().prerelease MAX_SAFE string");
...
/* ============= randomized differential compare vs a JS twin ============= */
{
    ...
    /* a frozen input sorts fine now that nothing is written back */
    const frozen = Object.freeze(["2.0.0", "1.0.0"]);
    const outF = sv.sort(frozen);
    assert(arrEq(outF, ["1.0.0", "2.0.0"]), "sort frozen array");
    assert(frozen[0] === "2.0.0", "frozen input untouched");
}
...
/* ==================== satisfies: big documented table ==================== */
{
    ...
    const T = [
        ...
        [" 1.2.3", "1.2.3", true],          /* node parity: the version side
                                               is trimmed, so this MATCHES */
        /* caret */
        ["1.2.3", "^1.2.3", true],
        ["1.2.4", "^1.2.3", true],
        ["1.9.9", "^1.2.3", true],
    ...
    /* invalid RANGE answers false (node parity: satisfies catches the Range
     * ctor error); an invalid VERSION still throws */
    assert(sv.satisfies("1.2.3", "not a range !!") === false, "bad range false");
    assert(sv.satisfies("1.2.3", ">=x.y.z") === false, "bad range 2 false");
    assert(sv.satisfies("1.2.3", "==1.2.3") === false, "bad operator false");
    assert(sv.satisfies("1.2.3", ">=1.2.3<2.0.0") === false, "glued comps false");
    assert(sv.maxSatisfying([ "1.2.3" ], "not a range !!") === null,
        "maxSatisfying bad range -> null");
    assert(sv.minSatisfying([ "1.2.3" ], "not a range !!") === null,
        "minSatisfying bad range -> null");
    throws(() => sv.satisfies("bogus", ">=1.0.0"), "bad version still throws");
    throws(() => sv.satisfies("1.2.3"), "satisfies missing range");
    /* junk ELEMENTS are skipped by max/min/filter (node parity), but sort
     * remains strict */
    assert(sv.maxSatisfying(["bogus", "1.2.3"], "^1.0.0") === "1.2.3",
        "maxSat skips junk element");
    assert(sv.minSatisfying(["bogus"], "*") === null, "minSat junk -> null");
    assert(new sv.Range("*").filter(["bogus", "1.2.3"]).length === 1,
        "filter skips junk element");
    throws(() => sv.sort(["bogus", "1.2.3"]), "sort still rejects junk");
}
...
/* ==================== inc: every release type ==================== */
{
    ...
    /* 'release': drop the prerelease, return X.Y.Z. Pinned against the
     * locally installed node semver@7.8.5:
     *   inc('1.0.0-rc.1','release') === '1.0.0'
     *   inc('1.0.0','release')      === null   (node returns null, not the
     *                                           input, when there is no
     *                                           prerelease to strip)
     *   inc('1.0.0+build.7','release') === null
     * Build metadata never survives inc in node-semver. */
    assert(sv.inc("1.0.0-rc.1", "release") === "1.0.0",
           "inc release drops the prerelease");
    assert(sv.inc("1.2.3-5", "release") === "1.2.3",
           "inc release drops a numeric prerelease");
    assert(sv.inc("1.0.0-alpha.beta", "release") === "1.0.0",
           "inc release drops an alphanumeric prerelease");
    assert(sv.inc("2.0.0-0", "release") === "2.0.0",
           "inc release drops the '0' prerelease");
    assert(sv.inc("1.0.0-rc.1+build.7", "release") === "1.0.0",
           "inc release strips prerelease and build metadata");
    assert(sv.inc("1.0.0", "release") === null,
           "inc release with no prerelease is null (node pins null)");
    assert(sv.inc("1.0.0+build.7", "release") === null,
           "inc release with only build metadata is null (node pins null)");
    assert(sv.inc("1.0.0-rc.1", "release", "beta") === "1.0.0",
           "inc release ignores the identifier (node pins this)");
    throws(() => sv.inc("1.2.3", "releaze"), "inc near-miss release refused");
    throws(() => sv.inc("1.2.3", "Release"), "inc release is case-sensitive");

    /* inc must not emit a version its own parser rejects: a bump that would
     * cross MAX_SAFE_INTEGER (any numeric field or identifier) is refused. */
    throws(() => sv.inc("9007199254740991.0.0", "major"),
           "inc major past MAX_SAFE is refused");
    throws(() => sv.inc("1.9007199254740991.0", "minor"),
           "inc minor past MAX_SAFE is refused");
    throws(() => sv.inc("1.2.3-9007199254740991", "prerelease"),
           "inc numeric prerelease id past MAX_SAFE is refused");
    /* the legal boundary still increments */
    assert(sv.inc("9007199254740990.0.0", "major") === "9007199254740991.0.0",
           "inc up TO MAX_SAFE still works");
}

/* ==================== error messages carry the diagnosis ==================== */
{
    /* satisfies answers false for a bad range now; the diagnosis lives on
     * the compiled Range constructor, which quotes the offending text
     * (bounded to 64 chars) */
    assert(sv.satisfies("1.2.3", ">=1.0.0 !!!oops") === false,
           "invalid range -> false");
    let msg = null;
    try { new sv.Range(">=1.0.0 !!!oops"); }
    catch (e) { msg = String(e); }
    assert(msg !== null && msg.includes(">=1.0.0 !!!oops"),
           "invalid range quotes the range (got " + msg + ")");
    msg = null;
    try { new sv.Range("not " + "very ".repeat(30) + "good"); }
    catch (e) { msg = String(e); }
    assert(msg !== null && msg.includes("very"),
           "Range() quotes an over-long range too");
    /* "invalid version in array" names the index */
    msg = null;
    try { sv.sort(["1.2.3", "1.2.4", "nope"]); }
    catch (e) { msg = String(e); }
    assert(msg !== null && /index 2/.test(msg),
           "invalid version in array names the index (got " + msg + ")");
}

/* ==================== Range: the compiled capability ==================== */
{
    /* [range, versions, expected] -- the subset Range.test accepts, in input
       order, one row per range grammar: exact, caret, tilde, x-range,
       hyphen, comparator pair, OR sets, prerelease gating. Expected values
       reuse the satisfies table above; Range must agree with it everywhere. */
    const T = [
        ["1.2.3", ["1.2.3", "1.2.4"], ["1.2.3"]],
        ["=1.2.3", ["1.2.3", "1.2.4"], ["1.2.3"]],
        ["^1.2.3", ["1.2.2", "1.2.3", "1.9.9", "2.0.0"], ["1.2.3", "1.9.9"]],
        ["^0.2.3", ["0.2.2", "0.2.3", "0.2.9", "0.3.0"], ["0.2.3", "0.2.9"]],
        ["~1.2.3", ["1.2.2", "1.2.3", "1.2.9", "1.3.0"], ["1.2.3", "1.2.9"]],
        ["1.2.x", ["1.2.0", "1.2.9", "1.3.0"], ["1.2.0", "1.2.9"]],
        ["1.2", ["1.2.5", "1.3.0"], ["1.2.5"]],
        ["*", ["0.0.0", "9.9.9"], ["0.0.0", "9.9.9"]],
        ["", ["0.0.0", "9.9.9"], ["0.0.0", "9.9.9"]],
        ["1.2.3 - 2.3.4", ["1.2.2", "1.5.0", "2.3.4", "2.3.5"],
         ["1.5.0", "2.3.4"]],
        [">=1.2.0 <1.3.0", ["1.2.0", "1.2.9", "1.3.0"], ["1.2.0", "1.2.9"]],
        ["^1.0.0 || ^2.0.0", ["1.5.0", "2.5.0", "3.0.0"], ["1.5.0", "2.5.0"]],
        ["^1.0.0-rc.1", ["1.0.0-rc.1", "1.0.0-rc.2", "1.0.0", "1.0.1-rc.1"],
         ["1.0.0-rc.1", "1.0.0-rc.2", "1.0.0"]],
    ];
    for (const [range, versions, expected] of T) {
        const r = new sv.Range(range);
        assert(r instanceof sv.Range, "new Range(" + JSON.stringify(range) + ")");
        assert(r.source === range,
            "Range.source round-trips " + JSON.stringify(range));
        assert(r.setCount >= 1, "Range.setCount is at least 1");
        for (const v of versions) {
            const want = expected.includes(v);
            assert(r.test(v) === want,
                "Range(" + JSON.stringify(range) + ").test(" + v + ") == " + want);
            /* the compiled range agrees with the free function */
            assert(r.test(v) === sv.satisfies(v, range),
                "Range.test agrees with satisfies for " + v + " in " + range);
        }
        assert(arrEq(r.filter(versions), expected),
            "Range.filter(" + JSON.stringify(range) + ") keeps the satisfying " +
            "elements in order");
    }
    /* an OR range really compiles to two sets */
    assert(new sv.Range("^1.0.0 || ^2.0.0").setCount === 2, "setCount counts || sets");
    assert(new sv.Range("^1.0.0").setCount === 1, "setCount of one set");
    /* an invalid range still throws, with the text quoted */
    throws(() => new sv.Range("!!"), "Range rejects an invalid range");
    throws(() => new sv.Range(123), "Range rejects a number (node parity)");
    throws(() => new sv.Range(), "Range requires an argument");
    /* regression: a compiled range with prerelease comparators must stay
     * correct long after construction (the ctor now parses from its OWNED
     * copy of the range text; parsing from the freed JS C string made the
     * comparator prerelease spans dangle) */
    {
        const r = new sv.Range("^1.2.3-beta.2 || ~3.4.0-rc.1");
        /* churn the allocator so a stale pointer would now hold garbage */
        for (let i = 0; i < 50000; i++) { try { sv.parse("junk" + i); } catch (e) {} }
        assert(r.test("1.2.3-beta.2") === true, "UAF probe: same tuple pre in");
        assert(r.test("1.2.3-beta.1") === false, "UAF probe: below beta.2 out");
        assert(r.test("1.2.3-beta.3") === true, "UAF probe: higher pre in");
        assert(r.test("1.2.3-alpha") === false, "UAF probe: other pre out");
        assert(r.test("3.4.0-rc.2") === true, "UAF probe: tilde pre in");
        assert(r.test("3.4.1") === true, "UAF probe: release not pre-gated");
        assert(r.test("3.5.0") === false, "UAF probe: tilde upper out");
        assert(r.test("1.2.4") === true, "UAF probe: caret release still in");
        assert(r.source === "^1.2.3-beta.2 || ~3.4.0-rc.1", "source intact");
    }
    /* a junk version is false, not a throw (documented .test policy) */
    assert(new sv.Range("^1.0.0").test("junk") === false,
           "Range.test answers false for junk");
}
...
/* ==================== sort: shuffled -> known order ==================== */
{
    ...
    assert(arrEq(out, ordered), "sort produces the known order: " +
        JSON.stringify(out));
    /* pure function: a NEW array is returned and the input is untouched
     * (d.ts: "does not mutate the input") */
    assert(out !== shuffled, "sort returns a fresh array");
    assert(arrEq(shuffled, before), "sort left the input untouched");
    /* empty + singleton */
...
/* ==================== maxSatisfying / minSatisfying ==================== */
{
    ...
    assert(sv.maxSatisfying(["bad"], "*") === null, "maxSat skips invalid");
    assert(sv.maxSatisfying(versions, "!!!") === null, "maxSat bad range null");
...
/* ==================== field accessors on prerelease + edge ==================== */
{
    ...
    assert(sv.prerelease("1.2.3") === null, "prerelease empty -> null");
