...
/* ==================== parse: invalid table ==================== */
{
    const bad = [
        ...
        "1.2.3+99999999999999999999",     /* numeric BUILD id > MAX_SAFE too */
    ];
    for (const s of bad) {
...
{
    /* "invalid range" names the offending text (bounded to 64 chars) */
    let msg = null;
    try { sv.satisfies("1.2.3", ">=1.0.0 !!!oops"); }
...
/* ==================== sort: shuffled -> known order ==================== */
{
    ...
    assert(arrEq(out, ordered), "sort produces the known order: " +
        JSON.stringify(out));
    /* in place: the SAME array is returned and mutated (API contract) */
    assert(out === shuffled, "sort returns the same array");
    assert(arrEq(shuffled, ordered), "sort mutated the input in place");
    /* empty + singleton */
