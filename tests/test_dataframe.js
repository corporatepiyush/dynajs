...
S("masked exclusion");
{
    ...
    /* a mask LONGER than the column is refused like a shorter one: the extra
       bytes are silently meaningless, so a caller passing one has misaligned
       the mask with some other frame */
    throwsLike(() => d.SUM("v", new Uint8Array(9).fill(1)), "Uint8Array of 5 bytes",
               "mask longer than the column is refused");
    throwsLike(() => d.SUM("v", new Uint8Array(2)), "Uint8Array of 5 bytes",
               "mask shorter than the column");
    throwsLike(() => d.SUM("v", new Float64Array(5)), "Uint8Array", "8-byte-element mask refused");
...
S("binary ops");
{
    ...
    /* NaN and Infinity propagate through the column form elementwise */
    {
        const a = new Float64Array([1, NaN, 3, 4]), b = new Float64Array([2, 2, NaN, 0]);
        const d = new DataFrame({ a, b });
        elemEq(d.ADD("a", "b"), [3, NaN, NaN, 4], "col-col NaN propagates");
        elemEq(d.DIV("a", "b"), [0.5, NaN, NaN, Infinity], "col-col div: NaN and Infinity");
    }
    /* an empty frame zips to an empty result */
    {
        const e = new DataFrame({ a: new Float64Array([]), b: new Float64Array([]) });
        elemEq(e.ADD("a", "b"), [], "empty frame col-col is empty");
    }
...
S("scan NaN and mask contract");
{
    ...
    /* mask rules are the shared ones: truthiness not equality, and the length
       must match the rows exactly -- a longer mask is silently meaningless,
       so it is refused like a shorter one; a plain Array is not a mask */
    elemEq(d2.CUM_SUM("v", new Uint8Array([0, 255, 2])), [0, 2, 5],
           "any nonzero mask byte selects");
    throwsLike(() => d2.CUM_SUM("v", new Uint8Array(9).fill(1)), "Uint8Array of 3 bytes",
               "a mask longer than the column is refused");
    throwsLike(() => d2.CUM_SUM("v", new Uint8Array(2)), "Uint8Array of 3 bytes",
...
S("positional: head/tail under a mask");
{
    ...
    throwsLike(() => df.HEAD("c", 3, new Uint8Array(2)), "Uint8Array of",
...
S("moments: refusals");
{
    ...
    throwsLike(() => dm.VARIANCE_POP("a", new Uint8Array(3)), "Uint8Array of",
               "a short mask is refused");
...
S("logical reductions");
{
       ...
       string column is named rather than answered. */
    {
        ...
        throwsLike(() => d.BOOL_AND("a", new Uint8Array(1)), "Uint8Array of",
                   "a short mask is refused");
...
S("SUM_CHECKED");
{
    ...
    throwsLike(() => new DataFrame({ a: new Int32Array(4) }).SUM_CHECKED("a", new Uint8Array(2)),
               "Uint8Array of", "SUM_CHECKED inherits the mask length check");
...
S("select/slice");
{
    ...
    throwsLike(() => df.FILTER(new Uint8Array(2)), "Uint8Array of",
...
/* ----------------------------------------------- isDataFrame (the brand check) */
S("isDataFrame");
const isdfFrame = new DataFrame({ x: new Float64Array([1, 2, 3]) });
ok(isDataFrame(isdfFrame) === true, "isDataFrame(frame) is true");
ok(isDataFrame(isdfFrame.SELECT(["x"])) === true, "derived frames are frames");
ok(isDataFrame(isdfFrame.FROM_RECORDS([{ x: 1 }, { y: 2 }])) === true,
   "FROM_RECORDS result is a frame");
{
    const negatives = [
        ["plain object with frame-ish props", { ROWS: 3, COLS: 1, COLUMNS: ["x"] }],
        ["object with a TO_COLUMNS function", { TO_COLUMNS: () => ({}) }],
        ["array", [1, 2, 3]],
        ["the DataFrame constructor", DataFrame],
        ["DataFrame.prototype-ish object", Object.create(DataFrame.prototype)],
        ["null", null],
        ["undefined", undefined],
        ["number", 42],
        ["string", "DataFrame"],
    ...
    for (const [what, v] of negatives)
        ok(isDataFrame(v) === false, "isDataFrame rejects: " + what);
    /* Object.create(DataFrame.prototype) is the interesting one: same proto
       chain, no brand. Pin that it stays false -- that IS the brand check. */
    ok(Object.create(DataFrame.prototype).SUM === undefined ||
       isDataFrame(Object.create(DataFrame.prototype)) === false,
       "proto-chain membership alone is not frame-ness");
    ok(isDataFrame() === false, "isDataFrame() with no argument is false");
    console.log("  isDataFrame: 3 positive, " + negatives.length + " negative cases");
}
