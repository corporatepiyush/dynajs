...
function log(s){ try{ const f=std.open(`${std.getenv("TMPDIR") || "/tmp"}/dj_cov_net.${Date.now() % 10000000}.log`,"a"); f.puts(new Date().toISOString().slice(11,19)+" "+s+"\n"); f.close(); }catch(e){} }
...
{
  let dir=`${std.getenv("TMPDIR") || "/tmp"}/watch_cov_${Date.now()}_${Math.floor(Math.random() * 1e9)}`;
...
// ContentTypeParse/Format
{
  ...
  // token vs quoted-string, CTL injection: the formatter REFUSES CTLs in
  // type/subtype/parameter values since the W-HTTP audit -- the result is a
  // header value the caller puts on the wire, so a raw CR/LF here was header
  // injection (CWE-113). These rows previously pinned the gap itself.
  throws(()=>ContentTypeFormat({type:"a\rb",subtype:"b"}),null,"CTL in type throws");
  throws(()=>ContentTypeFormat({type:"a",subtype:"b\na"}),null,"CTL in subtype throws");
  throws(()=>ContentTypeFormat({type:"a",subtype:"b",parameters:{p:"x\ny"}}),null,"CTL in param value throws");
  // param name validation: must be token - non-token but non-CTL names still emit
...
log("validate start");
{
  ...
  ok(IsSemver("v1.2.3"),"semver v prefix allowed (parse contract)");
