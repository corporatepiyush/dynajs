...
/* ---------- edge parity pins (audit ws-simd) ---------- */
{
    /* NaN never wins: f64Max/f64Min match the scalar reference (strict > / <),
     * all-NaN yields the ±DBL_MAX seed — same rule as the f32 max/min kernels. */
    assert(f64Max(Float64Array.of(1, NaN, 3)) === 3, "f64Max skips NaN");
    assert(f64Min(Float64Array.of(1, NaN, 3)) === 1, "f64Min skips NaN");
    assert(f64Max(Float64Array.of(NaN, NaN)) === -Number.MAX_VALUE, "f64Max all-NaN = -DBL_MAX");
    assert(f64Min(Float64Array.of(NaN, NaN)) === Number.MAX_VALUE, "f64Min all-NaN = +DBL_MAX");
    /* f64Axpy never reads y through the multiply: NaN in y is preserved exactly */
