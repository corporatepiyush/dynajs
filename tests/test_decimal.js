...
{
    /* A hostile big/big division must fail FAST, not run the schoolbook core
       (O(nn*bn), ~10^10 byte-ops on 100k-digit operands) uninterruptibly. The
       fix caps the cell count at DEC_MAX_DIV_CELLS; before it, a 5000x5000-digit
       divide ran unbounded. Watchdog: if the cap regresses, the suite fails
       instead of hanging. */
    const N = 5000;
    let a = "", b = "";
    const pat = "3141592653589793238462643383279502884197169399375105820974944592";
    for (let i = 0; i < N; i++) { a += pat[i % pat.length]; b += pat[(i + 7) % pat.length]; }
    const t0 = Date.now();
    let threw = false, msg = null;
    try { new Decimal(a).div(new Decimal(b)).toString(); }
    catch (e) { threw = true; msg = String(e); }
    const ms = Date.now() - t0;
    assert(threw, "a 5000x5000-digit division is refused (got: " + msg + ")");
    assert(ms < 1000, "the cell cap is enforced quickly (took " + ms + " ms)");
    assert(msg !== null && /too large/i.test(msg),
           "the refusal is the established 'too large' error (got: " + msg + ")");
}

{
    /* Boundary: a division whose quotient cell count lands exactly AT the cap
       still works; one cell over throws. With prec=34, b=2048 digits and
       a=2012 gives nn=4096 and cells=8388608 == 2^23 exactly; a=2013 -> over. */
    const at = new Decimal("9".repeat(2012)).div(new Decimal("7".repeat(2048)));
    assert(at.toString().length > 0, "cell count exactly at the cap still divides");
    let threw = false;
    try { new Decimal("9".repeat(2013)).div(new Decimal("7".repeat(2048))).toString(); }
    catch (e) { threw = true; }
    assert(threw, "a division one cell over the cap is refused");
}

/* --------------------------------------- Money.allocate overflow (P1b-3) */

{
    /* Weights whose int64 sum overflows: the old code wrapped the total (to 0,
       hitting a division-by-zero / SIGFPE, or to a huge negative that produced
       shares not summing to the original). Both must now throw cleanly, and a
       normal allocate is unchanged. */
    const w62 = Math.pow(2, 62);
    throws(() => new Money(100, "USD").allocate([w62, w62, w62, w62]),
           "weights summing past int64 to zero are refused");
    throws(() => new Money(1, "USD").allocate([w62, w62, w62]),
           "weights summing to a wrapped nonzero total are refused");
    eq(JSON.stringify(new Money(100, "USD").allocate([40, 30, 20, 10])
                        .map((m) => m.amount())),
       "[40,30,20,10]", "a normal allocate is unchanged");
}

/* --------------------------------- div exponent overflow (int32 wrap) */

{
    /* 1e2000000000 / 1e-2000000000: the exponent difference is 4e9, which
       silently WRAPPED in int32 and returned 1e-294967333 -- a wrong answer
       instead of an error. The exponent is now computed in int64 with the
       same +-2e9 bound and error convention as mul. */
    let threw = false, msg = null;
    try { D("1e2000000000").div(D("1e-2000000000")).toString(); }
    catch (e) { threw = true; msg = String(e); }
    assert(threw, "a division whose exponent overflows is refused (got: " + msg + ")");
    assert(/result exceeds/.test(msg),
           "the refusal is the established convention (got: " + msg + ")");
    /* a LEGAL large division still works: 1e2000000000 / 1e1999999990 == 1e10 */
    eq(D("1e2000000000").div(D("1e1999999990")).toString(), "10000000000",
       "a legal large division still works");
}

/* --------------------------------------- pow names the operation budget */

{
    /* pow's too-slow path used to report "result exceeds %u digits" -- the
       wrong reason. Squaring a wide base hits the multiply CELL budget long
       before any digit bound: 10000x10000 = 1e8 digit-pairs > 2^26. */
    const wide = "9".repeat(10000);
    let t0 = Date.now(), threw = false, msg = null;
    try { D(wide).pow(4).toString(); }
    catch (e) { threw = true; msg = String(e); }
    assert(threw, "a pow over the cell budget is refused (got: " + msg + ")");
    assert(/operation budget/.test(msg),
           "the refusal names the operation budget (got: " + msg + ")");
    assert(Date.now() - t0 < 1000, "and is refused quickly");
}

/* ---------------------------------- precision must be an integer option */

{
    let t = false;
    try { D("1").div(D("3"), { precision: 34.9 }); }
    catch (e) { t = e instanceof TypeError; }
    assert(t, "a fractional precision is a TypeError (was silently truncated)");
    eq(D("1").div(D("3"), { precision: 5 }).toString(), "0.33333",
       "an integer precision is unchanged");
}

/* --------------------------- one rounding-mode lookup, two spellings */

{
    /* {rounding: X} and the bare string X used to be looked up by two loops
       with different accept-sets and different errors; one helper serves both */
    eq(D("2.5").round(0, "halfOdd").toString(), "3", "a bare-string mode still works");
    eq(D("2.5").round(0, { rounding: "halfOdd" }).toString(), "3",
       "so does the object form");
    let msg = null;
    try { D("1").round(0, "sideways"); } catch (e) { msg = String(e); }
    assert(msg !== null && /halfEven/.test(msg),
           "a bad bare string now gets the full mode listing (got " + msg + ")");
}

/* --------------------------------- Money: mixed scales and INT64_MIN */

{
    /* same code, different minorDigits: the same integer means different
       money at different scales -- refused, not renormalized */
    const a = new Money(100, "USD", { minorDigits: 6 });
    const b = new Money(100, "USD");
    throws(() => a.add(b), "add refuses mixed minor digits");
    throws(() => b.sub(a), "sub refuses mixed minor digits");
    throws(() => a.cmp(b), "cmp refuses mixed minor digits");
    throws(() => a.equals(b), "equals refuses mixed minor digits");
    eq(a.add(new Money(5, "USD", { minorDigits: 6 })).amount(), 105,
       "matching scales still add");
}
{
    /* -a->amount was UB for INT64_MIN and (int64_t)d was UB at 2^63: both
       are range-checked now, with the standard overflow error */
    throws(() => new Money(-9223372036854775808, "USD").allocate([1, 1]),
           "allocate of an INT64_MIN amount is refused, not UB");
    throws(() => new Money(100, "USD").allocate([Math.pow(2, 63)]),
           "a 2^63 weight is refused, not UB-cast");
}

/* ---------------------: integrals, divmod, toBigInt, statics */
{
    eq(D("2.7").floor().toString(), "2", "floor");
    eq(D("-2.7").floor().toString(), "-3", "floor is toward -Infinity");
    eq(D("2.7").ceil().toString(), "3", "ceil");
    eq(D("-2.7").ceil().toString(), "-2", "ceil is toward +Infinity");
    eq(D("2.7").trunc().toString(), "2", "trunc is toward zero");
    eq(D("-2.7").trunc().toString(), "-2", "trunc keeps the sign, drops the fraction");
    eq(D("-0.5").ceil().toString(), "0", "ceil(-0.5) is 0, never -0");
    assert(D("1e2000000000").trunc().equals(D("1e2000000000")),
           "an integer passes through exact");
}
{
    const [q, r] = D("-7").divmod("2");
    eq(q.toString(), "-3", "divmod quotient is truncated toward zero");
    eq(r.toString(), "-1", "divmod remainder takes the dividend's sign");
    assert(D("-7").equals(q.mul("2").add(r)), "a == q*b + r on the nose");
    throws(() => D("1").divmod("0"), "divmod by zero throws");
}
{
    eq(String(D("42").toBigInt()), "42", "toBigInt");
    throws(() => D("42.5").toBigInt(), "a fractional value is refused, not truncated");
    eq(String(D("9223372036854775807").toBigInt()), "9223372036854775807", "int64 edge in");
    throws(() => D("1e100").toBigInt(), "past int64 is refused");
}
{
    eq(Decimal.ZERO.toString(), "0", "Decimal.ZERO");
    eq(Decimal.TEN.toString(), "10", "Decimal.TEN");
    eq(Decimal.NEG_ONE.toString(), "-1", "Decimal.NEG_ONE");
    assert(Decimal.ZERO.add(Decimal.ONE).equals(Decimal.ONE), "constants are Decimals");
    assert(Decimal.TEN.mul(Decimal.TEN).equals(D("100")), "TEN*TEN == 100");
}
