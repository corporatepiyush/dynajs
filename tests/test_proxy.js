...
function throwsMsgC6(fn, re, m) {
    ...
    ok(threw !== null && threw instanceof TypeError && re.test(threw.message),
       m + " (got " + (threw ? threw.name + ": " + threw.message : "no throw") + ")");
}
throwsMsgC6(() => new TCPProxy({ port: 0, upstream: { host: "127.0.0.1", port: 9 }, maxConnz: 4 }),
            /unknown option "maxConnz" \(valid: port, maxConns, idleTimeoutMs, connectTimeoutMs, upstream\)/,
            "TCPProxy ctor bag");
throwsMsgC6(() => new TCPProxy({ port: 0, upstream: { host: "127.0.0.1", prt: 9 } }),
            /unknown option "prt" \(valid: host, port\)/,
            "TCPProxy upstream entry bag");
...
function finish() {
    ...
    /* 5. option conversion failures must PROPAGATE. The old
       `JS_ToInt32(...) == 0 && n >= 0` silently DROPPED an option whose value
       threw (a Symbol, a throwing valueOf), so the proxy ran on defaults with
       no error -- while the sibling tcp_read_bounds throws on the same shape. */
    let leaked = null, threwOpt = false, optMsg = "";
    try {
        leaked = new TCPProxy({ port: 0,
                                upstream: { host: "127.0.0.1", port: back.port },
                                maxConns: { valueOf() { throw new Error("boom"); } } });
    } catch (e) { threwOpt = true; optMsg = String(e && e.message); }
    if (leaked) { try { leaked.close(); } catch (e) {} }
    ok(threwOpt && optMsg.includes("boom"),
       "a throwing maxConns valueOf propagates, not a silent default (got "
       + optMsg + ")");
    leaked = null; threwOpt = false;
    try {
        leaked = new TCPProxy({ port: 0,
                                upstream: { host: "127.0.0.1", port: back.port },
                                idleTimeoutMs: Symbol("x") });
    } catch (e) { threwOpt = true; }
    if (leaked) { try { leaked.close(); } catch (e) {} }
    ok(threwOpt, "a Symbol idleTimeoutMs throws, it does not fall back silently");
