...
/* The params array's `length` read must not swallow the real error: an
   unchecked JS_ToInt64 left nparam = 0 and answered with the MISLEADING
   "takes N parameters, 0 given" on top of the pending conversion exception.
   JS_IsArray unwinds a Proxy, so a Proxy of an array with a throwing length
   trap reaches the exact conversion being pinned. */
{
...
/* ---- the statement cache keys on the EXACT caller text (audit F6) ----
 * sqlite3_sql(st) can be shorter than the query text; a key built from it
 * under the JS text's length over-read the heap and stored a key that no
 * later lookup could match. Under ASan the over-read aborts; functionally
 * the second exec below must still hit the cache and return the same
 * changes count. */
{
    const kdb = new SQLite(":memory:");
    kdb.exec("CREATE TABLE kf (a)");
    kdb.exec("INSERT INTO kf VALUES (1);  ");   // trailing junk after ';'
    const c1 = kdb.exec("DELETE FROM kf;  ");   // same text twice
