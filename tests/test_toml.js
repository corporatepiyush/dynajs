 ...
 * be redefined by a `[header]`. */
{
    ...
    /* TOML 1.0: an inline table "cannot be added to or modified after the
     * fact" -- dotted keys and headers may not extend it either. */
...
/* --------------------------------- 09-16 audit regressions (toml-test cat.) */

/* Float exponent signs are part of the TOML float grammar; the number scanner
 * used to stop at +/- after e/E and refuse valid documents. */
{
...
/* Underscores are digits-of-the-base: hex/oct/bin allow them between digits. */
{
...
/* -inf must stringify whole (a shared length used to emit "-in"). */
...
/* Floats that stringify via %g (exponent forms) must re-parse. */
{
    const big = 9.223372036854776e18;
    eq(TOML.parse(TOML.stringify({ a: big })).a, big, "%g float round trips");
}

/* Quoted keys: basic strings, literal strings, escapes, empty quoted parts. */
{
...
/* A key = value pair owns its line: two pairs on one line is INVALID. */
...
/* Inline tables: single line, NO trailing comma (unlike arrays). */
throwsMsg(() => TOML.parse("a = {x = 1,}"), "expected a key in an inline table",
    "invalid/inline-table/trailing-comma");

/* A line-ending backslash in a multiline basic string trims ALL whitespace,
 * including newlines and the next line's indentation. */
{
    eq(TOML.parse('a = """a\\ \n   b"""').a, "ab",
...
/* DEL (U+007F) is a control character: forbidden raw in strings. */
throwsMsg(() => TOML.parse('a = "x\u007Fy"'), "control character",
    "invalid/string/del-char");

/* [table] paths resolve THROUGH an array of tables into its LAST element
 * (the spec's own [[fruits]] / [fruits.physical] example). */
{
...
/* The no-redefine set is hashed: duplicate detection must still fire after
 * thousands of distinct headers (the linear scan hid this from neither). */
{
    let doc = "", i;
