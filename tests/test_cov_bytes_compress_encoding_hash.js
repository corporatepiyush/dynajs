...
}
{
  ...
  assert(new Text("héllo").isValidUtf8===true,"Text valid utf8 (DT-8 getter)");
...
}
{
  ...
  // overflow -> RangeError (documented contract; the old [0,-11] sentinel
  // leaked the codec's internal negative bytesRead through the pair API)
  { let over=new Uint8Array(11).fill(0x80); throws(()=>Uvarint(over),null,"Uvarint overflow throws"); }
 ...
 * streaming-vs-one-shot agreement. */
{
    ...
    // Bcrypt / Argon2id password hashing — TLS-only build
    {
        ...
        /*the default return is the PHC string; raw form is {encoded:false} */
        const aphc = CryptoNS.Argon2id.hash("pw", salt, { iterations: 1, memory: 1024 });
        eq(typeof aphc, "string", "Argon2id default form is the PHC string");
        assert(aphc.startsWith("$argon2id$v=19$m=1024,t=1,"), "Argon2id PHC params in string");
        const ah = CryptoNS.Argon2id.hash("pw", salt, { iterations: 1, memory: 1024, encoded: false });
