...
int main(void)
{
    /* ZEROED, per the documented precondition on dyn_scram_client_first:
       re-entry frees the previous parts, so a garbage struct is a garbage
       free (bit us on arm64 macOS, whose stack garbage is not zero). */
    dyn_scram_t sc = {0};
    mock_srv_t srv;
