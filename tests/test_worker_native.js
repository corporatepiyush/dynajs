...
/* Per-run unique scratch PREFIX (mkdtemp-style): the fixed /tmp/worker_*.js
   names collided across concurrent runs of this suite. Honors TMPDIR; the
   generated files stay FLAT (${T}_wkr.js, ...) because nothing here mkdirs. */
const T = `${std.getenv("TMPDIR") || "/tmp"}/dj_wkr.${Date.now()}.${Math.floor(Math.random() * 1e9)}`;
...
print("=== 1. dyna:net classes construct+start on a worker runtime ===");
{
    ...
    const sh = [
        DYN + ` --std ${T}_main.js >${T}_nat_out.txt 2>&1 & pid=$!`,
        "sleep 5",
...
print("=== 2. control: plain worker still round-trips ===");
{
    ...
    write(`${T}_plain_main.js`, [
        'import * as os from "os";',
        'import * as std from "std";',
        `const w = new os.Worker("${T}_plain.js");`,
        "w.onmessage = function (e) { std.out.puts('GOT ' + e.data.n + '\\n'); w.onmessage = null; };",
        "w.onerror = function (err) { std.out.puts('WORKER-ERR\\n'); };",
    ].join("\n"));
    const sh = [
        DYN + ` --std ${T}_plain_main.js > ${T}_plain_out.txt 2>&1 & pid=$!`,
        "sleep 3",
        "if kill -0 $pid 2>/dev/null; then echo ALIVE; kill $pid 2>/dev/null; wait $pid 2>/dev/null; else echo DEAD; fi",
        `cat ${T}_plain_out.txt`,
    ].join("\n");
    const r = Exec("sh", ["-c", sh], { timeoutMs: 30000 });
    const out = String(r.stdout);
