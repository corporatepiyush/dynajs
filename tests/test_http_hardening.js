...
`);
        ...
        sh(`${EXE} --std ${T}/app.js >${T}/app.log 2>&1 &`);
        let appPid = 0;
...
`);
     ...
     * ================================================================ */
    {
        const CAP2 = 18760 + (Date.now() % 100);
        write(`${T}/capture2.py`, `import socket, sys, threading as th
s = socket.socket(); s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
s.bind(("127.0.0.1", ${CAP2})); s.listen(16)
th.Timer(60, lambda: sys.exit(0)).start()
def serve(cn):
    try:
        d = b""
...
`);
        sh(`python3 ${T}/capture2.py >${T}/capture2.log 2>&1 & sleep 0.4`);
        ...
        ok(rN.status === 200 && rN.body.indexOf("X-Foo: bar\r\nConnection: close") >= 0,
           ...
           rN.body.replace(/\r\n/g, "|").slice(-40) + "')");
        cc.close();
        sh(`pkill -f '${T}/capture2.py' 2>/dev/null`);
    }
     ...
     * ================================================================ */
    {
        const AP2 = 18800 + (Date.now() % 100);
        const UP2 = AP2 + 1;
        write(`${T}/up2.py`, `import socket, threading as th, sys
s = socket.socket(); s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
s.bind(("127.0.0.1", ${UP2})); s.listen(8)
th.Timer(60, lambda: sys.exit(0)).start()
def serve(cn):
    try:
        d = b""
...
import { Path } from "dyna:file";
const app = new App({ port: ${AP2}, idleTimeoutMs: 8000 });
app.rpc("/rpc", { ping: () => "pong" });
app.proxy("/p", { host: "127.0.0.1", port: ${UP2} });
app.ws("/ws", { open: () => {}, message: () => {}, close: () => {} });
app.start();
`);
        sh(`python3 ${T}/up2.py >${T}/up2.log 2>&1 & sleep 0.3`);
        sh(`${EXE} --std ${T}/app2.js >${T}/app2.log 2>&1 & sleep 0.6`);
