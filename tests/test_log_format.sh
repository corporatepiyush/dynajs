...
n=$((n+1)); [ -n "$(dbg ' , * , ')" ] || { fails=$((fails+1)); echo "FAIL: patterns are trimmed of spaces"; }
n=$((n+1)); [ -n "$(dbg 'app*secret')" ] || { fails=$((fails+1)); echo "FAIL: '*' matches a mid-pattern span"; }
# Multiple arguments join with a space; controls are escaped so one call
# cannot forge a second line.
OUTD=$(DEBUG='app' "$DYNAJS" -e 'import("dyna:log").then(({Debug}) => Debug("app")("one", "two", 3, "a\nb"))' 2>&1)
n=$((n+1)); [ "$(printf '%s\n' "$OUTD" | grep -c .)" = 1 ] || { fails=$((fails+1)); echo "FAIL: a Debug arg cannot forge a second line"; }
check '^app one two 3 a\\nb$' "$OUTD" "Debug joins arguments with a space and escapes controls"
