...
/* Size/timestamp arguments must not wrap modulo 2^32/2^64: NanoID(2**32+5)
 * used to silently become NanoID(5) and ULID(2**64+n) a valid ULID(n). */
{
...
/* BigInt atMillis is accepted (audit: the old JS_ToInt64 path never accepted
 * a BigInt at all -- it ran ToNumber, which throws on BigInt, so ULID(5n)
 * was always "TypeError: cannot convert bigint to number"). Exact BigInts
 * inside 48 bits work; wrapping or out-of-range values are refused. */
{
    ...
    /* Fractions are refused, matching NanoID (audit: ULID(6.5) used to
     * silently truncate to ULID(6)). */
    threw = false;
