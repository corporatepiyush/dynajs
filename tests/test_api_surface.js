...
function drive(label, fn, ctor, host) {
    ...
    for (const a of ARGS) {
        ...
        try {
            ...
            reap(rv);
            /* An ASYNC export's rejection is the async twin of a sync throw,
               and the suite's contract ("returns or throws rather than
               aborting") counts it as a bounded outcome. The handler is
               attached HERE, synchronously, so the rejection is always
               OBSERVED -- no unhandled-rejection process report, and the
               sweep continues to the next row. (reap() also catches, but a
               rejected promise must be handled even when this fast path
               changes, so the handling lives in both.) */
        ...
        catch (e) { bounded++; /* refusing is a correct outcome */ }
        const d = os.now() - t0;
        calls++;
