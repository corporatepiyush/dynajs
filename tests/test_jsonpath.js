...
/* first() stops at the FIRST match instead of running the query out. The
 * observable: the same query over the same document hits the 1M-node cap for
 * all() but never materialises the rest for first(). */
{
    const wide = {};
    for (let i = 0; i < 1100; i++)
        wide["k" + i] = new Array(1000).fill(0);   /* 1.1M nodes when fully walked */
    throws(() => q("$.*.*").all(wide), "all() over 1.1M nodes hits the node cap");
    eq(q("$.*.*").first(wide), 0,
       "first() stops at the first match: no cap, no full walk");
    eq(q("$.*.*").first({ a: [7] }), 7, "and first() still answers small documents");
}

/* RFC 9535 s2.3.5.2.2: objects/arrays compare STRUCTURALLY. */
{
    const shared = { v: 1 };
    const doc = { items: [{ v: shared }, { v: shared }, { v: { v: 1 } },
                          { v: { v: 2 } }, { v: { w: 1 } }],
                  want: shared };
    eq(new JSONPath("$.items[?(@.v == $.want)]").all(doc).length, 3,
       "== matches same object AND structurally equal objects");
    eq(new JSONPath("$.items[?(@.v != $.want)]").all(doc).length, 2,
       "!= rejects structurally equal objects");
    const arrdoc = { rows: [[1, 2], [1, 2], [1]], want: [1, 2] };
    eq(new JSONPath("$.rows[?(@ == $.want)]").all(arrdoc).length, 2,
       "arrays compare elementwise per the RFC");
}

/* A length-only array (attacker-shaped) must not be walked index by index:
 * length 2^32-1 with zero elements used to cost ~106 s of atom churn. */
{
    const big = new Array(4294967295);
    const t0 = Date.now();
    eq(new JSONPath("$.b[*]").all({ b: big }).length, 0,
       "wildcard over an empty max-length array selects nothing");
    eq(Date.now() - t0 < 5000, true,
       "empty max-length array walks in bounded time, not O(length)");
    const sparse = [];
    sparse[3] = "c"; sparse[0] = "a"; sparse[1] = "b";
    eq(JSON.stringify(new JSONPath("$.s[*]").all({ s: sparse })),
       JSON.stringify(["a", "b", "c"]),
       "sparse array children come in ascending index order, holes skipped");
}
