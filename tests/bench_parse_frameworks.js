 ...
 * variable; they are excluded by default so a routine
...
function parseOnce(src) {
    ...
    try {
        ...
        /* The parser's recursion guard still says SyntaxError "stack overflow"
           (only the RUNTIME recursion overflow was node-aligned to
           RangeError "Maximum call stack size exceeded"); this bench pins the
           parser spelling, which is the one `new Function` can produce. */
