...
/* Negative integers past the int64 range degrade to double (like major 0),
 * never wrap through the int64 cast: the old path decoded 0xffffffffffffffff
 * as 0 and 0x3b8000000000000000 as +9223372036854775807. */
eq(CBORDecode(bytes("3b7fffffffffffffff")), -9223372036854775808,
   "-2^63 exactly is the last value the int64 path serves");
eq(CBORDecode(bytes("3b8000000000000000")), -9223372036854775808,
   "-2^63-1 has no exact int64; the nearest double it is");
eq(CBORDecode(bytes("3bffffffffffffffff")), -18446744073709551616,
   "major-1 0xffffffffffffffff is -2^64, not a wrap to 0");
...
/* ------------------------------------------------ audit regressions (S1-S8)
 * The decode surface is untrusted: every row below is a byte battery that
 * used to behave differently before the audit fixes. */

{
    /* S1: a tag (major 6) used to recurse WITHOUT a depth slot -- 300 KB of
     * 0xC6 bytes segfaulted the stack. Every container byte must pay depth. */
    const a = new Uint8Array(100001);
    a.fill(0xc6, 0, 100000);
    a[100000] = 0;
    throws(() => CBORDecode(a), "a 100k-deep tag chain hits the depth cap, not the stack");
    const b = new Uint8Array(256);
    b.fill(0xc6, 0, 255);
    b[255] = 0;
    eq(CBORDecode(b), 0, "255 nested tags still decode (the cap itself is the limit)");
    throws(() => { const t = new Uint8Array(258); t.fill(0xc6, 0, 257); t[257] = 0; CBORDecode(t); },
           "257 tags are one past the cap");
    throws(() => CBORDecode(bytes("c6")), "a truncated tag refuses");
    /* P2 pinned: tags unwrap to the value that matters */
    eq(CBORDecode(bytes("c11a514b67b0")), 1363896240, "tag 1 (epoch) unwraps to the number");
    eq(CBORDecode(bytes("d82a63414263")), "ABc", "an unknown tag (42) unwraps too");
}
{
    /* W1: RFC 8949 4.2.1 orders by the ENCODED key bytes. For text keys the
     * minimal length header makes that identical to (length, then bytes) --
     * the discriminating cases, byte for byte: */
    eq(hex(CBORCanonical({ ab: 1, b: 2, z: 3 })), "a3616202617a0362616201",
       "b < z < ab is BOTH the bytewise and the (len,bytes) order");
    eq(hex(CBORCanonical({ "01": 1, "3": 2 })), "a261330262303101",
       "encoded 6133 < 6230.. across length classes");
    eq(j(CBORDecode(bytes("a1810102"))), j({ "1": 2 }),
       "an array key decodes through its string form, never crashes");
}
{
    /* S2: Date / RegExp / Map / Set clone as THEMSELVES (they used to become
     * plain objects, silently losing the value). */
    const d = new Date(1234567890123);
    const cd = structuredClone({ d }).d;
    assert(cd instanceof Date && cd.getTime() === 1234567890123,
           "a Date clones to a Date with its time");
    const cr = structuredClone({ r: /ab+c/gi }).r;
    assert(cr instanceof RegExp && cr.source === "ab+c" && cr.flags === "gi",
           "a RegExp clones with source and flags");
    const cm = structuredClone({ m: new Map([["k", 1]]) }).m;
    assert(cm instanceof Map && cm.get("k") === 1, "a Map clones with its entries");
    const cs = structuredClone({ s: new Set([1, 2]) }).s;
    assert(cs instanceof Set && cs.size === 2 && cs.has(2), "a Set clones with its values");
    /* shared built-ins stay shared, exactly like plain objects */
    const d2 = new Date(7), m2 = new Map(), s2 = new Set(), r2 = /x/g;
    const o = structuredClone({ a: d2, b: d2, c: m2, d: m2, e: s2, f: s2, g: r2, h: r2 });
    assert(o.a === o.b && o.a !== d2, "two refs to a Date clone to ONE Date");
    assert(o.c === o.d, "Maps share");
    assert(o.e === o.f, "Sets share");
    assert(o.g === o.h, "Regexps share");
    /* cycles through Map and Set close on the CLONE */
    const cyc = new Map();
    cyc.set("self", cyc);
    const cc = structuredClone(cyc);
    assert(cc.get("self") === cc, "a cycle through a Map closes on the clone");
    const scyc = new Set();
    scyc.add(scyc);
    assert(structuredClone(scyc).has(scyc) === false, "the set no longer contains the SOURCE");
    /* and a value shared twice through a Map stays shared */
    const inner = { v: 1 };
    const sh = structuredClone(new Map([["a", inner], ["b", inner]]));
    assert(sh.get("a") === sh.get("b"), "map values reached twice clone once");
    /* subclass instances keep the CLASS DATA (the clone itself is a plain
     * Date/Map, which is the HTML rule) */
    class MyDate extends Date {}
    assert(structuredClone(new MyDate(5000)).getTime() === 5000,
           "a Date subclass instance clones its time");
    class MyMap extends Map {}
    assert(structuredClone(new MyMap([["k", 1]])).get("k") === 1,
           "a Map subclass instance clones its entries");
}
{
    /* S3: a getter is an accessor, not data -- HTML says DataCloneError, and
     * the getter must NOT run during the clone. */
    let calls = 0;
    const o = {};
    Object.defineProperty(o, "g", { enumerable: true, configurable: true,
                                    get() { calls++; return 42; } });
    o.plain = 1;
    throws(() => structuredClone(o), "an enumerable accessor is refused");
    eq(calls, 0, "and the getter never executed");
}
