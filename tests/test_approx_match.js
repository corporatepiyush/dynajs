...
/* Transposition is TWO ops: this is Levenshtein, NOT Damerau. */
eq(Levenshtein("ab", "ba"), 2, "lev transposition costs 2, not 1");
eq(Levenshtein("ab", "ba", { max: 1 }), 2, "transposition exceeds max 1 -> 2");
...
/* The SAME max contract over the banded TWO-ROW DP path (min length > 64):
 * the fuzz above never leaves the bit-parallel kernel, and the band logic
 (lo/hi clamps, the INF walls) is different code. */
{
    let bandDone = 0, broke = false;
    for (let trial = 0; trial < 120 && !broke; trial++) {
        const a = randStr(60 + rnd() % 70, "abcd");
        const b = randStr(60 + rnd() % 70, "abcd");
        const exact = Levenshtein(a, b);
        const max = rnd() % 25;
        const got = Levenshtein(a, b, { max: max });
        const want = exact <= max ? exact : max + 1;
        if (got !== want) {
            assert(false, "lev max band (DP path): exact " + exact + " max " + max +
                   " got " + got + " want " + want);
            broke = true;
            break;
        }
    ...
    eq(bandDone, 120, "lev max contract held on the DP band path (120 pairs)");
}
...
/* after stripping, whitespace-only sides are EMPTY, and both-empty is the
 * equal case -> 1.0, not 0 (the strip runs before the length checks) */
near(DiceCoefficient(" ", ""), 1, "dice whitespace-only vs empty strips to equal");
near(DiceCoefficient(" \t\n", " "), 1, "dice two whitespace-only sides -> 1");
/* only ASCII whitespace is stripped: NBSP is a word character */
assert(DiceCoefficient("\u00A0ab", "ab") > 0 && DiceCoefficient("\u00A0ab", "ab") < 1,
       "dice NBSP is NOT stripped (2/3: 'ab' is one shared bigram)");
