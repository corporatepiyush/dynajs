...
int main(int argc, char **argv)
{
    ...
    /* direct wait: block in kevent once per wakeup instead of poll+kevent */
    js_std_set_io_reactor_wait(
        ctx, (int (*)(void *, int))dyn_aio_run);

    /* Publish the real bound port when a harness asks: port 0 + a port file
     * is race-free, a pinned port is not. */
