...
/* ============ v7 monotonicity: RFC 9562 same-ms counter =========== */
{
    const N = 100;
    const ids = [];
    for (let i = 0; i < N; i++) ids.push(v7());

    /* ms timestamps are non-decreasing (the held-floor property) ... */
    ...
    /* ... and the FULL strings strictly increase: within a constant
 millisecond the 12-bit rand_a counter rises by one per call,
       and a higher millisecond always sorts above by the big-endian time
       field. Two ids in the same ms can no longer land in random order. */
    let ordered = true;
    for (let i = 1; i < N; i++)
...
/* =================: v4/v7 {as:"bytes"} + strict option bags ====
 * v4() used to IGNORE its argument entirely: v4({as: "bytes"}) silently
 * returned a string. These pins keep the strict check honest. ============ */
{
    /* {as:"bytes"} returns the 16 bytes that string would encode */
    const b = v4({ as: "bytes" });
    ...
    /* roundtrip: bytes -> fromBytes -> bytes is the identity */
    const back = bytes(fromBytes(b));
    assert(back.join(",") === b.join(","), "v4 bytes roundtrip via fromBytes");
    /* a fresh copy each call: mutating one result cannot alias the module */
    b[0] = 0xff;
    ...
    /* v7 bytes still sort: two consecutive byte-forms never decrease */
    ...
    /* STRICT bags: unknown keys, bad enum values, wrong types (fix) */
    ...
    /* a refused options bag consumed no OS entropy we can assert on, but the
     * string forms must be byte-valid regardless */
