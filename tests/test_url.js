...
/* gave the mutable components setters (WHATWG parity); these five stay
   getter-only, pinned so a future pass cannot add one silently. Assignment
   throws like any getter-only property. The setter contract itself is
   test_url_setters.js. */
{
...
/* -------------------------------------------------: the URL statics
 * Parity matrix: every input the CONSTRUCTOR throws for must make parse()
 * answer null and canParse() answer false; every input it accepts must give
 * parse() a URL whose href matches the constructor's, and canParse() true.
 * Hostile inputs: empty, scheme-only, space in host, bad %, broken IPv6
 * bracket, over-65536-byte input, interior tab/LF/CR (stripped, not fatal). */
{
    ...
    const CASES = [
        ...
        ["", "https://b.test/", true],               /* empty reference resolves to the base (RFC 3986 5.2) */
        ["", undefined, false],
        ["http://", undefined, false],
        [":", undefined, false],
        ["https://ex ample.com/", undefined, false],
        ["http://a.test/%zz", undefined, true],      /* malformed escapes stay LITERAL, not fatal */
        ["http://[::1", undefined, false],
        ["::::", undefined, false],                  /* no base: a scheme must be present */
        ["/p", "not a url", false],                  /* bad base */
        ["https://a.test/p", "::::", false],
        [OVER, undefined, false],                    /* over DYN_URL_MAX */
        ["ht\ttp://x.test/", undefined, true],       /* interior TAB is stripped */
        ["http://x.test/\n.", undefined, true],      /* interior LF is stripped */
    ];
    for (const [input, base, good] of CASES) {
        ...
        /* null/undefined base behave the same (absent) */
        assert(URL.canParse(input, null) === (base === undefined ? c : URL.canParse(input, base)) || base !== undefined,
               "null base tolerated");
    }
    /* parse() results carry the plain class, even under a subclass */
    {
    ...
    /* parse()/canParse() throw the ctor TypeError on a non-string input */
    {
    ...
    /* join: RFC 3986-style resolution, absolute rel wins, empty rel keeps */
...
/* -------------------------------------------------: iteration
 * entries()/keys()/values() are IterableIterators (WHATWG shape) over a live
 * index; the instance [Symbol.iterator] is the entries iterator; the *Array
 * twins keep the old array returns. THIS IS A BEHAVIOR CHANGE from the array
 * returns (CHANGELOG); these pins exist so it cannot silently flip back. */
{
    ...
    /* for..of / spread / Array.from / Map over the instance */
    {
        const seen = [];
    ...
    /* manual next(): value/done shape, exhaustion stays done */
    {
        const it = sp.entries();
        const first = it.next();
    ...
    /* WHATWG finish latch: once exhausted, an iterator stays done forever --
       pairs appended afterward do NOT resurrect it (live-vs-finished split:
       mutation between steps is observed, mutation after the walk ends is not) */
    for (const [nm, get] of [
        ["entries", (p) => p.entries()],
        ["keys", (p) => p.keys()],
        ["values", (p) => p.values()],
    ...
    /* iterator dressing: %IteratorPrototype% chaining + Symbol.toStringTag */
    {
        const it = sp.entries();
    ...
    /* Array twins: the pre-UL-3 forms, byte-for-byte */
    ...
    /* live iteration: an append between steps is observed (index-based) */
    {
    ...
    /* empty params iterate to immediate exhaustion */
    {
    ...
    /* a bound instance iterates and writes through */
    {
    ...
    /* iterator survives while the plain URL object is dropped (gc_mark traces
       the iterator -> searchParams edge; a bound sp holds the URL C state) */
    {
        let it;
        {
...
/* regression: searchParams is [SameObject] per WHATWG URL sec.6.1 --
   the getter returns THE query object, not a fresh wrapper (the old
   fresh-wrapper-per-access behavior made the append idiom O(n^2)). */
{
