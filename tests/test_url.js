...
/* The components are READ-ONLY: this engine's URL has no setters (the
   WHATWG mutator API is the documented divergence, pinned here so a future
   setter pass cannot land silently). Assignment throws like any
   getter-only property. */
{
    const u = new URL("http://a.com/p?q#f");
    for (const k of ["href", "protocol", "username", "password", "host",
                     "hostname", "port", "pathname", "search", "hash",
                     "origin", "searchParams"])
        throws(() => { u[k] = "zz"; }, k + " has no setter (readonly by contract)");
}
...
/* T-URL-6 regression: searchParams is [SameObject] per WHATWG URL sec.6.1 --
