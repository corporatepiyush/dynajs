...
threw(() => diskUsage("/\u0000tmp"), "diskUsage refuses a NUL-bearing path");

/* A path answers with the statistics of the filesystem CONTAINING it: a file
 * or a nested directory reports its VOLUME, not its own size. */
{
    const f = diskUsage("/");
    ok(diskUsage("/etc").total === f.total,
       "a directory reports its volume's totals");
    ok(diskUsage("/etc/hosts").total === f.total,
       "a file reports its volume's totals, not its own size");
}
