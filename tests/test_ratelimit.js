...
/* --- cost overflow must not defeat the limiter (audit 13.3.3) --- */
{
    const rl = new RateLimiter({ tokensPerSec: 10, burst: 10 });
    const before = rl.tokens("k");
    eq(rl.allow("k", 9223372036854776), false,  /* 2^63/1000: wraps int64 */
...
/* --- a cost above the burst is denied outright, never granted (audit F7's
   guard; an oversized cost used to be able to wrap the refill math) --- */
{
