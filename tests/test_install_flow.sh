...
#
# The shipped installer has ONE mechanism (upstream master, latest commit) and
# no --repo/--ref options. The test therefore patches the repository constant
# in a scratch copy instead of passing flags, and asserts the removed options
...
# The hermetic installer copy: same script, one constant repointed at the
# fake repository. A sed of source is not a user-facing mechanism.
...
# A --ref that stopped being an error would be a second install mechanism
# shipped back in by accident; both it and --repo must be refused loudly.
