...
class Repl:
    ...
    def read_until(self, pred, timeout=READ_TIMEOUT, what="condition"):
        ...
        if pred():
            return
        msg = "timed out after %.1fs waiting for %s" % (timeout, what)
        if not self.alive:
            msg += (" -- the REPL process is already gone (crashed at startup"
                    " or a pty/environment issue, not slowness)")
        raise Timeout(msg)
...
@case("new", "\\load ./CLAU<TAB> completes a path")
def t_path_completion(ctx):
    ...
    r.settle()
    line = r.screen.input_line()
    want(line == "\\load ./CLAUDE.md",
         "path completion should give '\\load ./CLAUDE.md', got %r" % line)
...
def main(argv):
    ...
    dead_start = sum(1 for rows in results.values()
                     for _, s, d in rows
                     if s == TMOUT and "process is already gone" in d)
    if dead_start >= 3:
        print("note   : %d case(s) died before the first prompt -- the binary or" % dead_start)
        print("         the pty is broken in THIS environment, not the tests being slow.")
        print("         reproduce directly:  python3 tests/test_repl.py --binary ./dynajs")
