...
{
    /* The part before ':' is a SCHEME when it has the scheme shape (an ALPHA
     * first, then alnum/+/-/.): a dot no longer makes a value relative, so an
     * unknown dotted scheme is refused like any other. */
    eq(MD("[x](a.b:x)\n"), '<p><a href="">x</a></p>\n',
       "a dotted unknown scheme (a.b:) is not waved through as relative");
    eq(MD("[ok](relative/x:y)\n"), '<p><a href="relative/x:y">ok</a></p>\n',
       "while a slash before the colon really is a relative path");
    eq(MD("[ok](1234:abc)\n"), '<p><a href="1234:abc">ok</a></p>\n',
       "a token that cannot start a scheme stays relative");
    /* Encoded and backslash spellings must not smuggle a scheme past the
     * check: the scan percent-decodes first, and a backslash in scheme
     * position is refused outright. */
    eq(MD("[x](java%73cript:alert(1))\n"), '<p><a href="">x</a></p>\n',
       "a percent-encoded scheme is decoded before the check");
    eq(MD("[x](JAVA%53CRIPT:alert(1))\n"), '<p><a href="">x</a></p>\n',
       "including in any casing");
    eq(MD("[x](java\\script:alert(1))\n"), '<p><a href="">x</a></p>\n',
       "a backslash in scheme position is refused");
    eq(MD("![x](java%73cript:alert(1))\n"), '<p><img src="" alt="x"></p>\n',
       "and an image source too");
}
