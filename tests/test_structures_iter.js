...
const build = {
    ...
    /* LRU iteration is [key, value] pairs, most-recently-used FIRST (the
     * recency list's own order): d was put last, so it comes out first. */
...
/* ---- 7. LRU entries() order; SortedMap entries() matches keys() ---- */
{
    /* LRU: entries() is a plain Array (no iterator needed) of [key, value]
     * pairs, MRU-first; get() re-orders, has() does not. */
    const l = new LRU(4);
    ["a", "b", "c", "d"].forEach((k, i) => l.put(k, i + 1));
    eqJSON(l.entries(), [["d", 4], ["c", 3], ["b", 2], ["a", 1]],
           "LRU entries() is MRU-first by insertion recency");
    l.get("b");                                  /* b becomes the MRU */
    eqJSON(l.entries(), [["b", 2], ["d", 4], ["c", 3], ["a", 1]],
           "get() moves a pair to the front");
    l.has("a");                                  /* has must not re-order */
    eqJSON(l.entries(), [["b", 2], ["d", 4], ["c", 3], ["a", 1]],
           "has() leaves the recency order alone");
    l.put("b", 20);                              /* update in place -> MRU */
    eqJSON(l.entries()[0], ["b", 20], "put() on an existing key restamps recency");
    /* the snapshot is taken at entries() time: puts afterwards do not
     * retro-edit the returned Array */
    const snap = l.entries();
    l.put("e", 5);                               /* evicts "a" (LRU end) */
    eqJSON(snap.length, 4, "entries() is a snapshot, not a live view");
    eqJSON(snap[3], ["a", 1], "and the evicted pair stays in the snapshot");

    /* SortedMap: entries() walks the same ascending-key order as keys() */
