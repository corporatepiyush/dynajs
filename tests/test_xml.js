...
{
    /*a handlers object keeps the push contract; the argument is
       optional now -- NO argument is the pull form (next()), and a
       non-object argument is still refused. */
    throws(() => new SAXParser(42), "a non-object handlers argument is refused");
    const pull = new SAXParser();
    pull.write("<a>x</a>");
    const seen = [];
    let ev;
    while ((ev = pull.next()) !== null) seen.push(ev.event);
    pull.end();
    eq(seen.join(","), "open,text,close", "no-argument SAXParser is a pull parser");
}
...
/* --------------------------------------- audit regressions (ws-xml) */

/* F1: a literal TAB/LF/CR in an attribute value normalizes on re-parse
 * (XML 1.0 3.3.3), so the writer must emit the character reference. */
{
    const src = '<a b="x&#10;y&#9;z&#13;w"/>';
    const want = XMLParse(src).attrs.b;
    eq(want, "x\ny\tz\rw", "attr ws refs decode to the real chars");
    const out = XMLStringify(XMLParse(src));
    eq(XMLParse(out).attrs.b, want, "attr ws survives stringify (char refs emitted)");
    assert(out.indexOf("\n") < 0 && out.indexOf("\t") < 0,
       "no literal tab/newline in the emitted attribute");
    /* text keeps literal whitespace: only the attribute context escapes it */
    eq(XMLStringify({ name: "a", attrs: {}, children: ["x\ny\tz"] }), "<a>x\ny\tz</a>",
       "text context keeps literal whitespace");
}

/* F3: an unterminated close tag carries the same token cap as an open tag. */
{
    const CAP = 16 * 1024 * 1024;
    let msg = "";
    try { const p = new SAXParser({}); p.write("<r>x</r></" + "a".repeat(CAP + 1)); p.end(); }
    catch (e) { msg = String(e.message); }
    assert(msg.indexOf("close tag exceeds the token limit") >= 0,
           "an over-cap close tag fails at the cap, not with a generic error (" + msg + ")");
}

/* F4: the DOCTYPE skip survives comments and quoted literals inside. */
{
    eq(XMLParse("<!DOCTYPE r [ <!-- [ --> <!ELEMENT r ANY> ]><r/>").name, "r",
       "a bracket inside a subset comment does not break the skip");
    eq(XMLParse('<!DOCTYPE r SYSTEM "http://x/a>b"><r/>').name, "r",
       "a > inside a quoted SYSTEM literal does not end the DOCTYPE early");
    eq(XMLParse("<!DOCTYPE r [<!ENTITY g \">\">]><r/>").name, "r",
       "a > inside a quoted entity value still skips");
}

/* F5: a leading UTF-8 BOM is a byte-order mark, not content (XML 1.0 App F). */
{
    eq(XMLParse("\uFEFF<a/>").name, "a", "a leading BOM is skipped when parsing");
    const seen = [];
    const p = new SAXParser({ onOpen: (n2) => seen.push(n2), onText: (t) => seen.push("text:" + t) });
    p.write(new Uint8Array([0xEF, 0xBB, 0xBF, 0x3C, 0x61, 0x2F, 0x3E]));
    p.end();
    eq(j(seen), '["a"]', "a BOM fed as bytes produces no phantom text event");
    /* byte-at-a-time: the decision must wait for the third byte */
    const seen2 = [];
    const p2 = new SAXParser({ onOpen: (n2) => seen2.push(n2), onText: (t) => seen2.push("text:" + t) });
    for (const b of [0xEF, 0xBB, 0xBF, 0x3C, 0x61, 0x2F, 0x3E]) p2.write(new Uint8Array([b]));
    p2.end();
    eq(j(seen2), '["a"]', "a BOM assembled one byte at a time is still skipped");
    /* a BOM NOT at the start is ordinary text */
    eq(XMLParse("<a>\uFEFF</a>").children[0], "\uFEFF", "a mid-document BOM stays content");
}

/* F6: an unterminated comment/CDATA/DOCTYPE names itself at end(). */
for (const [doc, what] of [["<a/><!--x", "unterminated comment"],
                           ["<a><![CDATA[x", "unterminated CDATA"],
                           ["<!DOCTYPE x", "unterminated DOCTYPE"]]) {
    let msg = "";
    try { const p = new SAXParser({}); p.write(doc); p.end(); }
    catch (e) { msg = String(e.message); }
    assert(msg.indexOf(what) >= 0, doc + " -> " + what + " (" + msg + ")");
    let msg2 = "";
    try { XMLParse(doc); } catch (e) { msg2 = String(e.message); }
    assert(msg2.indexOf(what) >= 0, "tree parse of " + doc + " names the token too (" + msg2 + ")");
}

/* F7: blank text OUTSIDE the root is dropped without an event, exactly like
 * the tree parse drops it -- a pretty-printed document must not deliver
 * phantom onText events for its padding. */
{
    const out = [];
    const p = new SAXParser({ onText: (t) => out.push(t) });
    p.write("\n  <a>real</a>\n  ");
    p.end();
    eq(j(out), '["real"]', "blank top-level text is silent in SAX (padding dropped)");
    /* non-blank top-level text is still refused, and CDATA is never blank */
    throws(() => { const q = new SAXParser({}); q.write("x<a/>"); q.end(); },
           "non-blank text before the root is still refused");
    throws(() => { const q = new SAXParser({}); q.write("<a><![CDATA[ ]]></a><![CDATA[ ]]>"); q.end(); },
           "top-level CDATA is still refused even when whitespace-only");
}

/* -------------------------------------------: strict options bags ---
 * XMLParse / XMLStringify refuse unknown keys; SAXParser keeps its
 * handlers/opts duality open (documented exception). */
...
{
    throwsMsg(() => XMLParse("<a/>", { multipe: true }),
              /^unknown option "multipe" \(valid: multiple, trim, entities\)$/,
              "XMLParse bag names the key AND the valid set");
    throwsMsg(() => XMLParse("<a/>", { trim: true, entites: "keep" }),
              /^unknown option "entites" \(valid: /, "XMLParse bag rejects a second unknown key");
    eq(XMLParse("<a><b/></a>", { multiple: true, trim: false, entities: "keep" }).length,
       1, "a fully valid XMLParse bag is unchanged");
    throwsMsg(() => XMLStringify({ name: "a" }, { indnt: 2 }),
              /^unknown option "indnt" \(valid: indent\)$/,
              "XMLStringify bag is strict");
    eq(XMLStringify(XMLParse("<a><b/></a>"), { indent: 2 }),
       "<a>\n  <b/>\n</a>", "a valid XMLStringify bag is unchanged");
    /* SAXParser: duality stays open -- extra keys on the handlers object
     * are metadata, not typos (documented skip) */
    assert(typeof new SAXParser({ onOpen: () => {}, meta: 1 }).write === "function",
           "SAXParser handlers object tolerates extra keys (documented duality skip)");
    /* XMLParse keeps its pre-CC-6 non-object behavior: a primitive bag is
     * refused outright (stricter than "absent", and unchanged by ) */
    throws(() => XMLParse("<a/>", 42), "a primitive bag is still refused by XMLParse");
    /* XMLStringify historically IGNORES a non-object bag; unchanged */
    eq(XMLStringify({ name: "a" }, 42), "<a/>", "a primitive bag reads as absent for XMLStringify");
}
