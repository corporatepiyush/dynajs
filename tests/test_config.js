...
/* A UTF-8 BOM is encoding metadata for the fence detector too: a BOM-led
 * document still splits (it used to fall through to "no front matter"). */
{
    const r = FrontMatter.split("\uFEFF---\na: 1\n---\nbody\n");
    eq(r.lang, "yaml", "BOM before the fence does not block front matter");
    eq(r.data, "a: 1", "BOM front matter data");
    eq(r.body, "body\n", "BOM front matter body");
}
/* TOML.parse of a BOM-led document in a loop: the BOM used to advance the
 * pointer that JS_FreeCString frees (misaligned JSRefCountHeader free, only
 * visible under sanitizers). The values below pin the parse itself. */
{
    const d = TOML.parse("\uFEFFa = 1\n[t]\nb = \"x\"\n");
    eq(d.a, 1, "TOML BOM-led document parses");
    eq(d.t.b, "x", "TOML BOM table content");
}
