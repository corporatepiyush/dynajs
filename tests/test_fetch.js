...
print("=== 0. CC-6 strict init bags ===");
{
    ...
    throwsMsg(() => new Response(null, { staus: 201 }),
              /^unknown option "staus" \(valid: status, statusText, headers, url\)$/,
              "Response init bag names key + valid set");
    throwsMsg(() => new Request("http://x/", { methd: "GET" }),
              /unknown option "methd" \(valid: method, headers, body, signal\)/,
              "Request init bag names key + valid set");
    // fetch is async, but an unknown init key throws SYNCHRONOUSLY (before any IO)
    let sync = false;
    try { fetch("http://127.0.0.1:1/", { timout: 5 }); } catch (e) {
        sync = e instanceof TypeError &&
               /unknown option "timout" \(valid: method, headers, body, signal, timeout\)/.test(e.message);
    }
    assert(sync, "fetch unknown init key throws synchronously");
    // Proxy ownKeys trap throw propagates through Object.keys
    let pf = null;
    try { fetch("http://127.0.0.1:1/", new Proxy({}, { ownKeys() { throw new RangeError("ownKeys trap"); } })); }
    catch (e) { pf = e; }
    assert(pf instanceof RangeError && pf.message === "ownKeys trap",
           "fetch: Proxy ownKeys trap propagates (got " + (pf && pf.message) + ")");
    // symbol and inherited keys are invisible to the check
    const symInit = {};
    symInit[Symbol("methd")] = "GET";
    eq(new Request("http://x/", symInit).method, "GET", "symbol keys invisible");
    eq(new Request("http://x/", Object.create({ methd: "GET" })).method, "GET",
       "inherited keys invisible");
    // a throwing getter on a KNOWN key propagates (no silent swallow)
    const tg = {};
    Object.defineProperty(tg, "method", { get() { throw new RangeError("getter boom"); }, enumerable: true });
    let pg = null;
    try { new Request("http://x/", tg); } catch (e) { pg = e; }
    assert(pg instanceof RangeError && pg.message === "getter boom",
           "Request: throwing getter on known key propagates (got " + (pg && pg.message) + ")");
    // valid bags unchanged
    eq(new Request("http://x/", { method: "post", body: "hi" }).method, "POST",
       "valid Request init bag unchanged");
    eq(new Response("x", { status: 201 }).status, 201, "valid Response init bag unchanged");
}
...
async function testRedirects() {
    ...
    try {
        ...
        sh(`pkill -f '${T}/act1.py' 2>/dev/null; pkill -f '${T}/act2.py' 2>/dev/null; true`);
        sh(`rm -rf ${T}`);
    }
}
