/* test_array_ext.js — native Array extension methods (unprefixed, non-enumerable).
...
eq([1, 2, 3].aperture(0), [[], [], [], []], "aperture(0) → len+1 empties");
...
eq([1, 2, 3].insert(99, "x"), [1, 2, 3, "x"], "insert idx>len → append");
eq([1, 2, 3].insert(-1, "x"), [1, 2, 3, "x"], "insert idx<0 → append, NOT from the end");
...
eq([7, 2, 10, 9].median(), 8, "median unsorted even → sorts a copy first");
...
/* ---- regression: array-literal spread bulk append vs empty spreads ----
 * The OP_append bulk path (js_append_enumerate) presizes via
 * expand_fast_array(pos + count32); for consecutive EMPTY spreads that is 0,
 * which used to free the (zero-size) values buffer and return -1 with NO
 * pending exception — the VM then raised a stale JS_TAG_UNINITIALIZED value
 * ("uninitialized" as the uncaught exception). Also covers the count-over-write
 * elision guard shape ([, ...x]) and literal-after-spread pos tracking. */
{
    const E = [];
    eq([...E, 'x'], ["x"], "single empty spread then literal");
    eq([...E, ...E, 'x'], ["x"], "two empty spreads then literal");
    eq([...E, ...E], [], "two empty spreads only");
    eq([...E, ...E, ...E, 'x', 'y'], ["x", "y"], "three empty spreads then literals");
    eq([...E, ...E, ...['x']], ["x"], "two empty spreads then non-empty spread");
    eq(['x', ...E, ...E, 'y'], ["x", "y"], "literal, two empty spreads, literal");
    eq([...E, ...[1], ...E, 'y'], [1, "y"], "empty/non-empty/empty/literal interleave");
    eq([...E, ...E, ...E, ...E, ...['v'], ...E, ...E, ...E], ["v"],
       "8-way mixed spread (test_module_surface decl-set shape)");
    const el = [, ...['x']];               /* elision before spread: hole + "x" */
    assert(el.length === 2 && !(0 in el) && el[1] === "x", "elision before spread keeps hole");
    eq(new Set([...E, ...E, 'a', 'b']).size, 2, "Set from spread chain with empty leads");
}
