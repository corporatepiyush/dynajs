 ...
 * `{valueOf{ ... }}` attack that CLAUDE.md prescribes SILENTLY DOES NOT
 ...
 * the WebSocket upgrade depends on against the published RFC 6455 vector.
...
/* ---- audit regression: Hmac.sign()/verify() are COMPLETE MACs -----------
 * A pending update() prefix must not silently extend the message sign() or
 * verify() MACs -- update()+digest() is the streaming path; sign() is the
 * one-shot. The old behavior continued the stream, so
 * `h.update("ab"); h.signHex("")` returned HMAC(key,"ab") while the doc
 * promises "a complete MAC". */
{
    ...
    /* the streaming path is unchanged: update()+digest() still accumulates */
