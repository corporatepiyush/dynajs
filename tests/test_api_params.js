...
section("crypto", await mod("crypto"), (c) => [
    ...
    ["PBKDF2 is deterministic", () =>
        String(c.PBKDF2({ password: "p", salt: "s", iterations: 1000, length: 16 })) ===
        String(c.PBKDF2({ password: "p", salt: "s", iterations: 1000, length: 16 })), true],
]);
...
section("scrape", await mod("scrape"), (s) => [
    ...
    /* product-token group selection (RFC 9309 2.2.1.2 conventions) */
    ["Robots token match wins its named group", () =>
        new s.Robots("User-agent: *\nDisallow: /\n\nUser-agent: nb\nAllow: /\n",
                     { agent: "nb/1.2 (+x)" }).allows("/y"), true],
    ["Robots exact beats token lookalike", () =>
        new s.Robots("User-agent: b/9\nDisallow: /q\n", { agent: "b/9" })
            .allows("/q"), false],
    ["Robots delay of the winning group only", () =>
        new s.Robots("User-agent: *\nCrawl-delay: 9\n\nUser-agent: q\nCrawl-delay: 1\n",
                     { agent: "q/1" }).crawlDelay(), 1],
]);
