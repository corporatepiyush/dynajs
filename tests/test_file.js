...
/* ====================================================================== *
 *  Whole-file read cap (P1b-2): an over-cap st_size must fail fast, never
 *  drive an unbounded malloc/mmap on the JS thread.
 *
 *  A sparse file (truncate -s 2G) has st_size = 2 GiB but occupies 0 disk
 *  blocks. Before the cap, readFile did malloc(2 GiB) and read 2 GiB of
 *  zeros; the cap makes it fail in microseconds with a clear RangeError.
 *
 *  The over-cap proof files are sparse (truncate -s cap+1, which is what the
 *  old code would have malloc'd blindly); the under-cap proof uses a modest
 *  size so the test stays fast. We readBytes rather than readFile so the proof
 *  is about the slurp cap, not the JS String length limit.
 *
 *  Sparse files are crafted with the system `truncate(1)` (present on every
 *  dev/CI host this suite runs on). If it is missing, the proof is skipped
 ...
 * ====================================================================== */
{
    ...
    try {
        ...
        /* --- normal (under-cap) file reads intact, incl. the mmap path --- */
        ...
        /* --- a file over the cap is REFUSED before any malloc/read --- */
        if (trunc(big, cap + 1) === 0) {
            ...
            /* 2 GiB + reading even one page of zeros takes far longer than the
             * microsecond fstat+compare the cap costs. */
...
/* --- regression (audit F4): a DataView is a byte view, not a string ---
 * The payload resolver used JS_GetTypedArrayBuffer, which rejects a DataView;
 * the view then fell through to ToString and wrote the literal text
 * "[object DataView]". dyna-crypto fixed the identical defect first. */
{
    const dvPath = new Path(String(path) + ".dv");
    const w = new FileWriter(dvPath);
    w.write("A");
    w.write(new DataView(new Uint8Array([66, 67]).buffer)); /* "BC" */
    w.close();
...
/* ====================================================================== *
 *  sync readFile {bytes:true} and the free readBytes(path).
 *
 *  The async form always had the flag; the sync one silently ignored it --
 *  readFile(p, {bytes: true}) returned a STRING, the exact failure
 *  shape (the call works and does the wrong thing). The bytes path wraps
 *  the raw slurp, so invalid UTF-8 round-trips instead of becoming U+FFFD.
 * ====================================================================== */
{
    ...
    /* a string read of the SAME bytes is lossy: 0xFE/0xFF have no UTF-8
     * meaning and arrive as U+FFFD -- this is WHY the flag exists. The bytes
     * form round-trips them; the string form cannot even represent them. */
    ...
    /* bare readFile is byte-identical to what it always was */
    ...
    /* File.readBytes forwards to the same free implementation now */
...
/* ====================================================================== *
 *  copyTo(dest, {overwrite}) -- parity with the free copyFile.
 *  Also pins the return pairing: moveTo returns THIS (the handle follows
 *  the file it moved), copyTo returns a NEW File (the source survives).
 * ====================================================================== */
{
...
/* ====================================================================== *
 *  STRICT options bags -- every bag in the module, by name.
 *  Unknown keys used to be silently ignored; now each throws a TypeError
 *  naming the key and the valid ones (dyna:csv pilot format).
 * ====================================================================== */
{
    ...
    /* strictness is about the whole bag, not just key names: a THROWING
     * GETTER on a known key propagates (JS_ToBool's -1 used to land silently
     * as truthy), and a throwing valueOf on a known NUMBER key does too. */
    let evil = false;
    try {
    ...
    /* non-bags keep the historic reading */
