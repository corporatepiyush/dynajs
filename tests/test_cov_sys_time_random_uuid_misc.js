...
/* ============================ dyna:log ============================ */
{
    ...
    //was `new Logger({ svc: "cov" })` -- `svc` was a silently-ignored
    // unknown key; option bags are strict now, so use a documented one.
    const lg = new Logger({ name: "cov" });
    lg.info("hello"); n++;
...
print("=== semver ===");
{
    ...
    for (const bad of ["", "1", "1.2", "01.2.3", "1.2.3.4", "not-semver", "1.2.x"])
        eq(semver.isValid(bad), false, "isValid bad " + bad);
    eq(semver.isValid("v1.2.3"), true, "isValid ok v prefix (parse contract)");
    // comparators
    ...
    // sort: ascending, NON-MUTATING -- returns a fresh sorted array, input
    // untouched (semver-audit F2; d.ts: "does not mutate the input")
    ...
    eq(outSorted === unsorted, false, "sort returns a fresh array");
    eq(JSON.stringify(unsorted), '["2.0.0","0.1.0","1.5.0"]', "sort does not mutate the input");
