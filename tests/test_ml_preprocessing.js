...
{
    /* a CONSTANT column must scale by 1, not divide by zero. sklearn's scale_
     * convention reports 1.0 for the constant column's std (pinned by the
     * sklearn differential in test_ml_sklearn.js) -- the transform divides by
     * the same 1.0 either way. */
    const s = new StandardScaler();
    try {
        s.fit([[5, 1], [5, 2], [5, 3]]);
...
/* --- two-pass variance and overflow refusal (audit batch, 2026-08) --------- */
{
    /* large mean, small spread: E[x^2]-mean^2 cancels to 0 and would snap the
     * scale to 1.0 (transform would give +-0.5); the deviation form keeps
     * spread == 0.5 -> transform == +-1 */
    const s = new StandardScaler();
    s.fit([[1e12], [1e12 + 1]]);
    ...
    /* variance overflow from finite input must refuse, not emit NaN scales:
     * deviations of +-2e200 square to 1e400 = inf */
    throws(() => new StandardScaler().fit([[2e200, 1], [0, 2]]),
           "scaler variance overflow", RangeError);
    /* the elementwise metric family refuses NaN silently-answering NaN */
