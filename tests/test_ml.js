...
import { LinearRegression, LogisticRegression, KMeans, SVC, trainTestSplit, kFold, gridSearch } from "dyna:ml";
...
/* --- LogisticRegression: separable 2-class toy set ------------------------ */
{
    ...
    try {
        ...
        /* rows x n_classes since W9.5: column k is P(classes[k]|row), which is
         * the shape every other classifier here already returned. */
        const proba = m.predictProba([[0, 0], [5, 5]]);
        assert(proba[0][1] >= 0 && proba[0][1] < 0.5, "P(class1|origin) < 0.5");
        assert(proba[1][1] > 0.5 && proba[1][1] <= 1, "P(class1|far) > 0.5");
        assert(Math.abs(proba[0][0] + proba[0][1] - 1) < 1e-12, "row sums to 1");
    } finally { m.close(); }
}
...
/* --- Caps and adversarial-shape refusals (audit batch, 2026-08) ---------- */
{
    const X = [[1], [2], [3], [4]], y = [1, 2, 3, 4];
    ...
    throwsRange(() => new LogisticRegression({ maxIter: 100001 }),
                "logreg maxIter cap");
    throwsRange(() => new KMeans(80).fit(Array.from({ length: 100 }, (_, i) => [i])),
                "kmeans k near rows refusal");
    assert(new KMeans(50).fit(Array.from({ length: 100 }, (_, i) => [i])).inertia >= 0,
           "kmeans k <= rows/2 still fits");
    throwsRange(() => {
        const w = new LinearRegression();
        w.fit(X, y, { sampleWeight: [1e308, 1e308, 1e308, 1e308] });
    }, "sampleWeight sum overflow refusal");
    const km = new KMeans(2, { seed: 1 }); km.fit(X);
    throwsRange(() => km.predict([[NaN]]), "kmeans predict NaN refusal");
}

/* ----: every estimator/search bag rejects unknown keys ---- */
{
    ...
    throwsMsgC6(() => new LogisticRegression({ C: 1, l2: 0.1, classWieght: "balanced" }),
                /unknown option "classWieght" \(valid: learningRate, tol, l1, l2, maxIter, C, penalty, classWeight\)/,
                "LogisticRegression bag");
    throwsMsgC6(() => new SVC({ kernel: "rbf", gamm: 0.1 }),
                /unknown option "gamm" \(valid: kernel, C, gamma, coef0, tol, degree, maxIter\)/,
                "SVC bag");
    throwsMsgC6(() => trainTestSplit(10, { testSize: 0.2, seeds: 1 }),
                /unknown option "seeds" \(valid: seed, shuffle, testSize\)/,
                "trainTestSplit bag");
    throwsMsgC6(() => kFold(10, { k: 3, scoring: () => 0 }),
                /unknown option "scoring" \(valid: seed, shuffle, k, folds\)/,
                "kFold bag refuses crossValScore-only keys");
    throwsMsgC6(() => new LinearRegression().fit([[1], [2]], [0, 1], { sampleWeight2: [1, 1] }),
                /unknown option "sampleWeight2" \(valid: sampleWeight\)/,
                "fit() bag");
    throwsMsgC6(() => gridSearch(() => new KMeans(2), [[1], [2], [3]], [0, 1, 0],
                                 { nClusters: [2] }, { nItr: 3 }),
                /unknown option "nItr" \(valid: scoring, seed, nIter, shuffle, k, folds, nJobs, onProgress\)/,
                "gridSearch bag");
    // valid bags unchanged
    const lr = new LogisticRegression({ C: 2, penalty: "l2", maxIter: 100 });
    lr.fit([[0], [1], [2]], [0, 1, 1]);
    assert(lr.predict([[1.5]])[0] === 1, "valid LogisticRegression bag still trains");
    const sp = trainTestSplit(10, { testSize: 0.25, seed: 7 });
    assert(sp.train.length + sp.test.length === 10, "valid trainTestSplit bag unchanged");
}
