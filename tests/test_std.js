...
function test_printf()
{
    ...
    /* the "l" modifier is only supported for 64-bit integers: a %ls would
       read a char* as wchar_t* (audit 13.7.2), so it is refused like any
       other invalid conversion */
    let threw = false;
    try { std.sprintf("%ls", "abc"); } catch (e) { threw = true; }
    assert(threw, true, "a %ls conversion is refused");
    threw = false;
    try { std.sprintf("%lc", 65); } catch (e) { threw = true; }
    assert(threw, true, "a %lc conversion is refused");
}
...
function test_legacy_removed()
{
    ...
    /* std.popen and std.urlGet are removed (r1 audit): popen handed its
       command string to the shell verbatim -- a command-injection surface,
       superseded by os.exec (explicit argv) and fetch; urlGet shelled out to
       curl. They must be GONE, not broken. */
    assert(typeof std.popen, "undefined", "std.popen is removed");
    assert(typeof std.urlGet, "undefined", "std.urlGet is removed");
}

function test_rw_detach()
{
    /* os.read/os.write used to capture the ArrayBuffer pointer BEFORE
       resolving pos/len; those coercions can run a user valueOf, and a
       valueOf that transfers (detaches) the buffer freed the memory under
       the captured pointer. The view is now fetched after every coercion,
       so the same sequence is a clean TypeError. */
    var ab = new ArrayBuffer(8);
    var evil = { valueOf: function () { ab.transfer(4); return 0; } };
    var fd = os.open("/dev/null", os.O_WRONLY);
    var threw = false;
    try {
        os.write(fd, ab, evil, 4);
    } catch (e) {
        threw = true;
    }
    assert(threw, true, "write: a valueOf that transfers the buffer throws");

    ab = new ArrayBuffer(8);
    evil = { valueOf: function () { ab.transfer(); return 0; } };
    threw = false;
    try {
        os.read(fd, ab, evil, 4);
    } catch (e) {
        threw = true;
    }
    os.close(fd);
    assert(threw, true, "read: a valueOf that transfers the buffer throws");

    /* the buffer was detached mid-call; a normal call on an intact buffer
       must still work */
    fd = os.open("/dev/null", os.O_WRONLY);
    ab = new ArrayBuffer(4);
    assert(os.write(fd, ab, 0, 4), 4, "an intact buffer still writes");
    os.close(fd);
}
