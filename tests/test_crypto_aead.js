...
if (!crypto.AESGCM || !crypto.ChaCha20Poly1305) {
    ...
    /* ---------- sealRandom: fresh CSPRNG nonce beside the sealed bytes ----------
     * No published vector can apply (the nonce is random), so this pins the
     * PROPERTIES: the pair round-trips through open(), two calls never share
     * a nonce, tampering still throws, and the refusal shapes hold. */
    {
        const key = new Uint8Array(32).fill(11);
        const pt  = new TextEncoder().encode("nonce-helper-probe");
        const aad = "ctx";

        for (const C of [crypto.AESGCM, crypto.ChaCha20Poly1305]) {
            const c = new C(key);
            const r1 = c.sealRandom(pt, aad);
            ok(r1.nonce instanceof Uint8Array && r1.nonce.length === 12,
               C.name + ".sealRandom returns a 12-byte nonce");
            ok(r1.sealed instanceof Uint8Array && r1.sealed.length === pt.length + 16,
               C.name + ".sealRandom sealed is ct||tag (+" + (r1.sealed.length - pt.length) + ")");
            ok(new TextDecoder().decode(c.open(r1.nonce, r1.sealed, aad)) ===
               new TextDecoder().decode(pt),
               C.name + ".sealRandom pair round-trips through open()");
            const r2 = c.sealRandom(pt, aad);
            ok(tohex(r1.nonce) !== tohex(r2.nonce),
               C.name + ".sealRandom never repeats a nonce");
            ok(tohex(r1.sealed) !== tohex(r2.sealed),
               C.name + ".sealRandom ciphertexts differ under differing nonces");
            const tampered = r2.sealed.slice();
            tampered[0] ^= 1;
            throws(() => c.open(r2.nonce, tampered, aad),
                   C.name + ".sealRandom output is still authenticated");
            throws(() => c.sealRandom(),
                   C.name + ".sealRandom with no plaintext is refused");
            c.close();
        }

        /* The explicit-nonce path and the helper must agree byte-for-byte
         * when fed the SAME nonce: the helper is seal() plus entropy, nothing
         * else. */
        const c = new crypto.AESGCM(key);
        const r = c.sealRandom(pt);
        const manual = c.seal(r.nonce, pt);
        ok(tohex(manual) === tohex(r.sealed),
           "sealRandom(n) === seal(fresh n): the helper adds entropy only");
        c.close();
    }
