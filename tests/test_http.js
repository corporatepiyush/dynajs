...
import { mkdir, open, read, close, kill, exec, waitpid,
         O_RDONLY, O_WRONLY, O_CREAT, O_TRUNC } from "os";
...
`);
        f.close();
        std.gc();
        /* std.popen was removed (it fed the command to the shell verbatim):
           os.exec with an explicit argv + cwd spawns the same background
           peer, with its stdout/stderr parked in peer.log. */
        const plog = open(dir + "/peer.log", O_WRONLY | O_CREAT | O_TRUNC, 0o644);
        exec([cwd() + "/dynajs", "--std", "peer.js"],
             { cwd: dir, stdout: plog, stderr: plog, block: false });
        close(plog);
        let port = null;
        ...
        try {
            ...
            const s = pf.getline(); pf.close();
            if (s) {
                const wpid = parseInt(s.trim(), 10);
                kill(wpid, 15);
                /* the peer is now a direct child (exec block:false, not a
                   shell background job), so it can be genuinely reaped */
                const [wret] = waitpid(wpid, 0);
                assert(wret === wpid, "wire-capture peer was reaped");
                killed = true;
            }
        } catch (e) { }
        assert(killed, "wire-capture peer was signalled down");
    }
...
/* ---: server option bags reject unknown keys before any IO --- */
{
    ...
    throwsMsg(() => new HTTPServer({ port: 0, workerz: 2 }),
              /^unknown option "workerz" \(valid: port, workers, backlog, requestTimeoutMs, host, routes\)$/,
              "HTTPServer ctor bag");
    throwsMsg(() => new HTTPServer({
        port: 0,
        routes: { "/x": { status: 200, contentType: "text/plain", body: "ok", extra: 1 } },
    }), /unknown option "extra" \(valid: status, contentType, body\)/,
       "route value spec bag");
    // valid bags unchanged
    const s3 = new HTTPServer({ port: 0, workers: 1, routes: { "/": "ok" } });
    try {
        s3.start();
        const c = new HTTPClient();
        try { assert(c.get("http://127.0.0.1:" + s3.port + "/").body === "ok",
                      "valid HTTPServer bag still serves"); }
        finally { c.close(); }
    } finally { s3.close(); }
}
