...
declare module "dyna:crypto" {
    ...
    namespace Argon2id {
         ...
         *  asyncStats offloadMin gate. */
        function hashAsync(password: BytesInput, salt: ByteView, opts?: Argon2idOpts): Promise<Uint8Array>;
...
declare module "dyna:decimal" {
    ...
    class Money {
        constructor(minorUnits: number, currency: string, opts?: { minorDigits?: number });  /* minorUnits and minorDigits must be integers; the code is upper-cased */
...
declare module "dyna:file" {
    ...
    class Watcher implements DynResource {
        ...
        /** `path` is a plain string, not a Path. */
        start(cb: (event: { type: string; path: string }) => void): void;
...
declare module "dyna:net" {
    ...
    class PostgreSQL implements DynResource {
        ...
        /* queryIter() is declared on this class by the src/pool.js block:
         * pool.js installs it on the prototype when imported, so a fresh
         * dyna:net does NOT have it (importing it here yields undefined). */
    ...
    class TCPServer implements DynResource {
        constructor(opts?: { port?: number; path?: string; maxConnections?: number; idleTimeoutMs?: number; tls?: TCPServerTLSOptions });
...
/* Importing src/pool.js ALSO installs queryIter() on the PostgreSQL prototype
 * (a DECLARE CURSOR stream: constant memory for any result size). It is
 * declared as an augmentation of the dyna:net class because that is where the
 * method appears at runtime -- but only after pool.js has been imported; a
 * fresh dyna:net has no queryIter, which is why it is not on the class in the
 * dyna:net block above. */
declare module "dyna:net" {
    interface PostgreSQL {
        /** Streaming rows via a cursor: constant memory for any result size. */
        queryIter(sql: string, params?: unknown[], opts?: { batch?: number; maxRows?: number }): AsyncGenerator<Record<string, unknown>>;
    }
}
...
declare module "dyna:log" {
    ...
    class Logger {
        ...
        /** A new Logger with fields appended to the base prefix; shares the parent's destination. */
...
declare module "dyna:random" {
    ...
    class Random {
         ...
         *  A DataView or bare ArrayBuffer is refused (TypeError). */
        fill(typedArray: AnyTypedArray): this;
...
declare module "dyna:scrape" {
    ...
    class Extractor {
        constructor(spec: Record<string, unknown>, opts?: { text?: (node: object) => string });
        /**
         * Runs against a parsed document; options.base resolves `as: "url"`.
         * Fields support { sel, attr?, all?, required?, trim?, source?,
         * default?, as? } with as: "number" | "url" | "json". `source`
         * reads raw child source (script/style/ld+json).
         */
        run(doc: unknown, opts?: { base?: string }): { ok: boolean; value: Record<string, unknown>; missing: string[] };
    }
    ...
    class Crawl implements DynResource {
        ...
        /** One page per call: {value, done}. Each page has
         *  { url, depth, status, value, robots? } -- robots is the merged
         *  X-Robots-Tag + meta-robots directive list, when either exists. */
...
declare module "dyna:uuid" {
    ...
    function v4(): string;
    /** Time-ordered version-7 UUID (48-bit ms + RFC 9562 §6.2 same-ms counter): ids generated later in the process sort after earlier ones. */
    function v7(): string;
    /** MD5-based name UUID. */
...
declare module "dyna:oauth2" {
    ...
    /** Client Basic auth per RFC 6749 §2.3.1: form-encodes id:secret then base64 */
    ...
    function verifyJWT(token: string, key: BytesInput, opts: { algorithms: string[]; aud?: string | string[]; iss?: string; requiredScope?: string[]; clockSkewSec?: number }): unknown;
    ...
    /** WWW-Authenticate: Bearer realm, error, error_description, error_uri, scope. error is REQUIRED (RFC 6750 §3). */
    ...
    /** Verbatim exact except loopback port-ignore per RFC 8252 §7.3 */
    ...
    /** Token response per RFC 6749 §5.1: access_token + token_type Bearer required, b64token access_token */
...
declare module "dyna:xml" {
    ...
    class SAXParser {
        /** The handlers object doubles as the options bag: `trim` and `entities` are read from it. */
        constructor(handlers: SAXHandlers & { trim?: boolean; entities?: "strict" | "keep" });
