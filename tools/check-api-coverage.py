#!/usr/bin/env python3
"""Cross-check API.md against the binary's own exports.
...
def main():
    ...
    # uses).
    inv_text = subprocess.run(
        ["./dynajs", "--std", "tools/api-inventory.js", "--json"],
        capture_output=True, text=True).stdout
    inv = json.loads(inv_text)
    # standard-vs-extension classification scans test262/ (see
    # check-dts-coverage). Linux CI legs exclude test262, so every ES builtin
    # reads non-standard there -- 100+ false misses. Skip loudly instead.
