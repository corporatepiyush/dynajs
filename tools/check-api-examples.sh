...
# A run must not share its scratch dir with any other run: the fixed
# $TMPDIR/api-examples path races when two trees run their gates at once
# (a second instance's rm -rf deletes the first's .meta mid-loop and the
# failure reads as doc rot). mktemp -d gives each run its own directory.
OUT="$(mktemp -d "${TMPDIR:-/tmp}/api-examples.XXXXXX")"
trap 'rm -rf "$OUT"' EXIT
