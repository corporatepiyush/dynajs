...
KNOWN_UNRUN = {
    ...
    'tests/test_scrape_concurrency_child.js':
        'launched AS a child process by tests/test_scrape_concurrency.js (it '
        'is the polite-crawl peer); run standalone it has no driver.',
    'tests/test_worker_module.js':
        ...
        'gated.',
    'tests/test_html_large.js':
        'large portfolio (bytes/KB/MB) manual bench, too heavy for prepush gate',
    'tests/test_template_large.js':
        'large portfolio (bytes/KB/MB) manual bench, too heavy for prepush gate',
    'tests/test_scrape_close_lifetime_child.js':
        'launched AS a child process by tests/test_scrape_close_lifetime.js '
        '(its exit code is the row verdict for the teardown shapes); run '
        'standalone it has no driver.',
    'tests/test_shutdown_parked_child.js':
        'launched AS a child process by tests/test_shutdown_parked.js (each '
        'park shape is judged by the child\'s exit code); run standalone it '
        'has no driver.',
    'tests/test_shutdown_parked_worker.js':
        'launched AS a worker by os.Worker from tests/test_shutdown_parked.js '
        '(the worker-runtime teardown shape); run standalone it throws on '
        'onmessage.',
    'tests/test_shutdown_repark_child.js':
        'launched AS a child process by tests/test_shutdown_repark.js (the '
        're-park shapes are judged by the child\'s exit code); run standalone '
        'it has no driver.',
}
