...
#   ./dev.sh one MODULE [--build-only]   ONE module's minimal binary + its tests
#                            (--build-only: just link .obj/slim/dynajs-one-NAME)
...
#   ./dev.sh quick [FILE...]         native build + core sanity suite (+ run FILEs)
...
#   ./dev.sh gate  [TEST.js...]      full proof: 0-warn + ASan/UBSan/TSan + regression/fuzz + make test + test-api + test-security + test262
#   ./dev.sh clean                   make clean + drop temp gate trees and config stamp
#
...
# ------------------------------------------------------- module test builds --
# mod/one test modules through slim binaries in .obj/slim; ./dynajs itself is
# never a goal of theirs. But a slim link pulls its OBJECTS from the on-disk
# cache, so the cache must hold the config the tests assume (native + TLS when
# present) -- this is the documented "mod needs CONFIG_NATIVE_MODULES=y built
# first" precondition, now satisfied HERE instead of by the caller. Reuses
# _build's clean-on-config-change rule; unlike _build it does NOT gate on
# warnings (the 0-warning gate is for source changes; pre-existing warnings
# must not hold module testing hostage), but every error still fails.
_ensure_native_build(){
  local cfg="$*" stamp mcfg log rc=0
  stamp=$(cat "$STAMP" 2>/dev/null || true)
  [ "$stamp" = "$cfg" ] && [ -x ./dynajs ] && return 0
  echo "building module-test base binary ($cfg) -- one full build, then slim links only"
  [ "$stamp" = "$cfg" ] || make clean >/dev/null 2>&1 || true
  log=$(mktemp)
  mcfg="$cfg"
  if [ -n "$CCACHE_PREFIX" ] && [[ "$mcfg" != *"CC="* ]]; then mcfg="$CCACHE_PREFIX$mcfg"; fi
  make -j"$BUILD_J" $mcfg >"$log" 2>&1 || rc=1
  if [ "$rc" = 1 ]; then
    echo "FAIL: build [$cfg]"
    grep -iE "error:" "$log" | head -12 | sed 's/^/    /'
    tail -20 "$log" | sed 's/^/    /'
  else
    echo "$cfg" >"$STAMP" 2>/dev/null || true
  fi
...
# _dynajs_guard BEFORE LABEL: the slim runs must leave the full binary
# byte-identical. What was once a comment ("test targets skip the config-stamp
# wipe, so the main binary is safe") is here a checksum, so a Makefile change
# that reintroduces the clobber fails loudly instead of being discovered by
# the next person to run ./dynajs.
_dynajs_guard(){
  local before="$1" label="$2" after
  [ -n "$before" ] || return 0   # no binary on entry: nothing to protect
  after=$(shasum -a 256 ./dynajs 2>/dev/null) \
    || die "$label: ./dynajs vanished during the run"
  [ "$before" = "$after" ] \
    || die "$label: ./dynajs CHANGED (clobber): was ${before%% *}, now ${after%% *}"
}

# `make test` quiet on success but never a bare "FAIL: make test": the output
# goes to a log and the tail is shown on failure, so the failing test is named.
_make_test(){
  local log; log=$(mktemp)
  if make test >"$log" 2>&1; then rm -f "$log"; echo "make test: ok"; return 0; fi
  echo "FAIL: make test"
...
_stage(){
  ...
  case "$kind" in
    ...
    # Honour the per-suite flag header (// flags: --std), exactly as
    # tools/run-tests-parallel.sh does: a suite that imports "std" failed this
    # stage outright because the flag was never passed.
    tsan)   for t in "$@"; do
              flags=$(head -3 "$t" 2>/dev/null | grep -E '^// *flags:' | head -1 | \
                      sed -E 's|^//[[:space:]]*flags:||')
              env $env_kv ./dynajs $flags "$t" </dev/null >/dev/null || return 1
            done ;;
...
case "$cmd" in
  ...
  # slim binary holding ONLY the named modules (+ their companions) and runs
  # just those modules' tests. The slim binaries live in .obj/slim and the
  # run is checksum-guarded: ./dynajs must survive byte-identical. If the tree
  # is in another config, the full native build happens once here first.
  mod)      [ $# -ge 1 ] || die "usage: mod MODS  (e.g. mod csv,net -- see 'make test-mods')"
            _ensure_native_build $NATIVE_CFG || exit 1
            sum_before=$([ -f ./dynajs ] && shasum -a 256 ./dynajs)
            _bound make $NATIVE_CFG test-mods MODS="$*" \
              || { _dynajs_guard "$sum_before" "mod $*"; die "test-mods MODS=$*"; }
            _dynajs_guard "$sum_before" "mod $*"
            echo "mod: ok (./dynajs untouched: sha256 ${sum_before:0:12})" ;;

  # ONE module, minimal binary: `make one-<m>` links exactly MODDEFS_<m> +
  # the companions the link provably needs (MINCOMPANION_<m>, derived by
  # tools/slim-prune.sh; falls back to the full SLIMCOMPANION where nothing
  # could be pruned), then runs ONLY MODTESTS_<m>. --build-only skips the
  # tests and just produces the binary. Same guard as mod: ./dynajs untouched.
  one)      m=""; build_only=0
            for a in "$@"; do
              case "$a" in
                --build-only) build_only=1 ;;
                -*) die "one: unknown flag $a" ;;
                *) [ -z "$m" ] || die "one: one module at a time (got '$m' and '$a')"; m="$a" ;;
              esac
            done
            [ -n "$m" ] || die "usage: one MODULE [--build-only]  (e.g. one csv)"
            known=$(make --no-print-directory $NATIVE_CFG slim-vars 2>/dev/null \
                      | sed -n 's/^SLIM_MODS=//p')
            [ -n "$known" ] || die "one: slim-vars produced nothing (needs a CONFIG_NATIVE_MODULES=y parse)"
            case " $known " in *" $m "*) ;; *)
              die "one: unknown module '$m'. Known: $known" ;; esac
            _ensure_native_build $NATIVE_CFG || exit 1
            sum_before=$([ -f ./dynajs ] && shasum -a 256 ./dynajs)
            goal="test-one-$m"; [ "$build_only" = 1 ] && goal="one-$m"
            _bound make $NATIVE_CFG "$goal" \
              || { _dynajs_guard "$sum_before" "one $m"; die "$goal"; }
            _dynajs_guard "$sum_before" "one $m"
            if [ "$build_only" = 1 ]; then
              echo "one: ok -- binary under .obj/slim (./dynajs untouched)"
            else
              echo "one: ok (./dynajs untouched)"
            fi ;;
  ...
  gate)     BUILD_J="$GATE_BUILD_J"
            ...
            _serial "tsan (threaded http)" tsan \
                    ...
                    tests/test_http.js tests/test_http_keepalive.js \
                    tests/test_http_perf_regress.js || rc=1
