...
Constructs a multi-threaded HTTP/1.1 server serving static routes, one worker thread per connection from a pool. It binds in the constructor, so construction fails if the port is taken. When the connection queue (256) is full, new connections are closed without a response — put a proxy or rate limiter in front if overload signaling matters.
...
so its throws are swallowed -- but the swallow is now observable: read this
...
(further throws of that kind are counted silently). Default runs without `-d`
are bit-identical to the previous behavior apart from the counter itself.
...
**`new Fetcher({ agent, client, headers?, revalidate?, robots?, robotsTtlMs?, minDelayMs?, retries?, maxRedirects?, maxBodyBytes?, allowPrivateHosts? })`**
...
**`fetcher.stats() -> { fetched, skippedByRobots, retried, throttledMs, bytes, revalidated, savedBytes }`**

Returns the crawl statistics; `savedBytes` sums cached-body sizes served on revalidation. A crawler whose politeness cannot be observed cannot be trusted.
...
Note: `PgPool` and `RedisPool` are NOT exports of this module. They are pure-JS pools over the native `PostgreSQL`/`Redis` clients, shipped as the source file `src/pool.js` — import them by file path (`import { PgPool, RedisPool } from "./src/pool.js"`). Importing `pool.js` also installs `queryIter(sql, params?, opts?)` on `PostgreSQL.prototype`: a DECLARE-CURSOR stream that keeps memory constant for any result size. A fresh `dyna:net` has none of these — importing them from `"dyna:net"` yields `undefined`.
...
Issues a query. At most 64 queries can be in flight at once; further calls throw. The records array keeps only answers whose owner name matches the question (case-insensitive, RFC 1035 2.3.3) — unrelated records a response legally carries never surface in it.
...
`import { Bcrypt, Argon2id, RSA, ECDSA, ECDH, X509, AESGCM, ChaCha20Poly1305, Ed25519Generate, Ed25519Sign, Ed25519Verify, X25519Generate, X25519Derive, HMAC, HMACHex, Hmac, HKDF, PBKDF2, Scrypt, RandomBytes, TimingSafeEqual, HOTPGenerate, TOTPGenerate, JWTSign, JWTVerify, MD5, MD5Hex, SHA1, SHA1Hex, SHA224, SHA224Hex, SHA256, SHA256Hex, SHA384, SHA384Hex, SHA512, SHA512Hex, CRC32, CRC32C, XXHash32, XXHash64, SHA3_224, SHA3_224Hex, SHA3_256, SHA3_256Hex, SHA3_384, SHA3_384Hex, SHA3_512, SHA3_512Hex, Keccak256, Keccak256Hex, SHAKE128, SHAKE128Hex, SHAKE256, SHAKE256Hex, BLAKE3, BLAKE3Hex, BLAKE2b, BLAKE2bHex, BLAKE2s, BLAKE2sHex, Murmur3_128, Murmur3_128Hex } from "dyna:crypto";`
...
Returns the Argon2id v0x13 (RFC 9106) hash. The hash runs synchronously on the calling thread — at the allowed cap (1 GiB of memory, bounded iterations) that is seconds of blocking — so call it from a worker when hashing while requests are being served.
...
**`Argon2id.hashAsync(password, salt, opts?) -> Promise<Uint8Array>`** / **`Argon2id.verifyAsync(password, salt, expectedHash, opts?) -> Promise<boolean>`**
...
same results. Needs the dyna:net reactor: without dyna:net these names are
absent from the module (not exported-and-blocking). Calls below the offload
gate (`memory x iterations` block-fills, see `asyncStats().offloadMin`) settle
inline and still return a promise.
...
`seal`, `open`, `close`, `dispose` and `closed` behave exactly as `AESGCM`'s, with the same 12-byte nonce, 16-byte tag, and throw-on-forgery rule. `sealRandom(plaintext, aad?)` is also inherited in shape: a fresh CSPRNG 12-byte nonce per message, returned as `{ nonce, sealed }`, safe to about 2^32 messages per key.
...
**`gzip(data, level?) -> Uint8Array`**

- `data` *(string | TypedArray | ArrayBuffer)* — the input.
- `level` *(number, optional)* — 1..9; below 6 emits a fixed-Huffman stream, 6 and above builds dynamic-Huffman blocks from the input's own statistics (typically 5-15% smaller on text).
...
Returns a 26-character Crockford base32 ULID: 48-bit millisecond timestamp then 80 bits of entropy, so ULIDs sort by time. The 80 bits are fresh OS entropy on every call — ULIDs generated within the same millisecond are NOT monotonic (monotonicity is spec-optional; use `uuidv7`'s counter form when same-millisecond ordering matters).
...
- `timestamp` *(string | false, default `"epoch"`)* — `"epoch"` | `"iso"` | `false`; any other string throws (it used to silently mean epoch).
- `base` *(object)* — fields pre-serialized into a prefix every line carries.
- `dest` *(string)* — write to this path (append mode) instead of stderr. Two Loggers naming the same path share one open file and one rollover state.
...
Lands any buffered file output now. A no-op on stderr.
...
`Basic` header per RFC 6749 §2.3.1: form-encodes `id:secret`, then base64.
...
Buffered sequential reader over a strictly-opened fd. A resource: closed explicitly or by the GC finalizer, both paths ASan-clean. Like `readFile`'s whole-file read, the accumulating reads are capped at 1 GiB (`DYN_MAX_INPUT`): a `read()` with `n` omitted, a `readAll()`, or a `readLine()` over a newline-less stream throws a `RangeError` past it instead of growing without bound.
...
Returns the whole file as a string; strict open first. Like `FileReader`'s accumulating reads, the whole-file read is capped at 1 GiB (`DYN_MAX_INPUT`): a larger file throws a `RangeError` instead of growing without bound.
...
`import { Exec, Which, env, getEnv, setEnv, args, cwd, chDir, platform, pid, hostName, homeDir, cpuInfo, memInfo, loadAvg, uptime, diskUsage, memoryUsage, setNativeMemoryLimit } from "dyna:sys";`
