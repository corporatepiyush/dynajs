...
Accepts multipart file uploads into `dir`, then calls the handler with the saved path and metadata. The body is written to disk synchronously as it arrives, so a stalled or full disk pauses every connection on the same App instance until the write completes; very large uploads on slow storage belong on a dedicated App.
